...
async function globalTeardown(config: FullConfig) {
  console.log('🧹 Starting cache test environment teardown...');
  ...
  try {
    ...
    try {
      ...
      console.warn('⚠️  Could not clear test caches');
    }
    ...
    try {
      ...
      console.log('✅ Test authentication files cleaned up');
    } catch (error) {
      console.warn('⚠️  Could not clean up auth files');
    }
    ...
    console.log('✅ Cache test environment teardown complete!');
...
async function generateTestReport(): Promise<TestReport> {
  ...
  try {
    ...
    if (fs.existsSync(resultsFile)) {
      const results: TestResults = JSON.parse(fs.readFileSync(resultsFile, 'utf8'));
      ...
      report.statistics = {
        totalTests: results.suites?.reduce((total: number, suite: TestSuite) =>
          total + (suite.specs?.length || 0), 0) || 0,
        passed: results.suites?.reduce((passed: number, suite: TestSuite) =>
          passed + (suite.specs?.filter((spec: TestSpec) => spec.ok).length || 0), 0) || 0,
        failed: results.suites?.reduce((failed: number, suite: TestSuite) =>
          failed + (suite.specs?.filter((spec: TestSpec) => !spec.ok).length || 0), 0) || 0,
...
function extractPerformanceMetrics(results: TestResults): PerformanceMetrics {
...
function extractSecurityMetrics(results: TestResults): SecurityMetrics {
...
async function validateCleanup(page: Page): Promise<void> {
  try {
      ...
      try {
        ...
        if (testResponse.ok()) {
          ...
          if (!data.user_context?.user?.email?.includes('test-cleanup')) {
            console.log('✅ No test data found in production endpoints');
          }
        }
      } catch (error) {
    ...
    console.log('✅ Cleanup validation complete');
  } catch (error) {
