...
check_prerequisites() {
    ...
    if ! docker ps | grep -q "migrate-ui-orchestrator"; then
        echo -e "${RED}❌ Docker containers not running. Please run 'docker-compose up -d'${NC}"
        exit 1
    else
...
run_test_suite() {
    ...
    if [ $exit_code -eq 0 ]; then
        ...
        echo -e "${RED}❌ ${test_name} tests TIMED OUT after 30 minutes${NC}"
        return 1
    else
...
generate_summary() {
    ...
    if [ $passed_count -eq 3 ]; then
        ...
        echo -e "${YELLOW}⚠️  PARTIAL SUCCESS (2/3 test suites passed)${NC}"
        echo "Some functionality may have issues. Review failed tests."
    elif [ $passed_count -eq 1 ]; then
        echo -e "${RED}🚨 CRITICAL ISSUES (Only 1/3 test suites passed)${NC}"
