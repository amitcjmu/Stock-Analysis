...
test.describe('Discovery Flow - TRUE End-to-End Regression Test Suite', () => {
  ...
  test.beforeAll(async ({ playwright }) => {
    ...
    console.log('\n🚀 TRUE E2E DISCOVERY FLOW REGRESSION TEST STARTED');
    console.log(`📊 Test data: ${testDataFile} (${TEST_CSV_RECORDS} records)`);
  ...
  test.afterAll(async () => {
    ...
    console.log('\n📋 TRUE E2E REGRESSION TEST COMPLETED');
    console.log(`⏱️  Duration: ${totalDuration}ms`);
    console.log(`✅ Passed: ${REGRESSION_REPORT.summary.passedPhases}/${REGRESSION_REPORT.summary.totalPhases}`);
    console.log(`❌ Failed: ${REGRESSION_REPORT.summary.failedPhases}/${REGRESSION_REPORT.summary.totalPhases}`);
    console.log(`🔍 MFO Validation: ${REGRESSION_REPORT.summary.mfoValidationPassed ? 'PASS' : 'FAIL'}`);
    console.log(`📝 Field Naming: ${REGRESSION_REPORT.summary.fieldNamingCompliant ? 'COMPLIANT' : 'VIOLATIONS'}`);
    ...
    try {
      ...
      if (hasBlockingFlow) {
        ...
        try {
          ...
          if (statusResponse.ok()) {
            ...
            if (status.blocked || status.requires_action) {
              console.log('✓ Backend confirms blocking state:', status.blocking_reason || status.required_action);
            }
          }
        } catch (apiError) {
        ...
        if (blockingText?.toLowerCase().includes('attribute') || blockingText?.toLowerCase().includes('mapping')) {
          console.error('❌ CRITICAL: Attribute mapping incomplete - user cannot proceed with discovery');
        }
      }
    } catch (e) {
  ...
  test('PHASE 0: MFO Flow Creation and Lifecycle', async ({ page }) => {
    ...
    try {
      console.log('\n🎯 PHASE 0: MFO FLOW CREATION - Master/Child Lifecycle Validation');

      // Step 1: Create master flow via MFO
      const step1Start = Date.now();
      console.log('📍 Step 1: Create Master Flow via MFO');
      ...
      if (createFlowResponse.ok() && createFlowData) {
        ...
        console.log(`✅ Discovery Flow ID: ${discoveryFlowId}`);
      ...
      console.log('🔍 Step 2: Verify Atomic Transaction - Check Both Tables');
      ...
      console.log('🚀 Step 3: Start Discovery Flow');
      ...
      console.log(`✨ Phase 0 completed in ${phaseResult.duration}ms - Status: ${phaseResult.status}`);
    }
  });
  ...
  test('PHASE 1: Data Import with Proper Validation', async ({ page }) => {
    ...
    try {
      console.log('\n📦 PHASE 1: DATA IMPORT - Enhanced Validation');
      ...
      const step1Start = Date.now();
      console.log('📤 Step 1: Upload CSV with Flow Context');
      ...
      if (!discoveryFlowId) {
        ...
        if (await flowIdElement.isVisible({ timeout: 5000 })) {
          ...
          phaseResult.flowId = discoveryFlowId;
          console.log(`📊 Captured Flow ID: ${discoveryFlowId}`);
        }
      }
      ...
      const step2Start = Date.now();
      console.log('🔍 Step 2: Validate Import via Backend API');
      ...
      const step3Start = Date.now();
      console.log('➡️  Step 3: Continue to Attribute Mapping');
      ...
      console.log(`✨ Phase 1 completed in ${phaseResult.duration}ms - Status: ${phaseResult.status}`);
    }
  });
  ...
  test('PHASE 2: Attribute Mapping Validation', async ({ page }) => {
    ...
    try {
      console.log('\n🗺️  PHASE 2: ATTRIBUTE MAPPING - Full Validation');

      // Step 1: Navigate to Attribute Mapping
      const step1Start = Date.now();
      console.log('📍 Step 1: Navigate to Attribute Mapping');
      ...
      const step2Start = Date.now();
      console.log('🔍 Step 2: Fetch and Validate Mappings');
      ...
      while (retryCount < maxRetries) {
        ...
        retryCount++;
        console.log(`⏳ Waiting for mappings to be generated... (attempt ${retryCount}/${maxRetries})`);
      ...
      if (mappingsResponse.ok()) {
        ...
        if (mappingCount === 0) {
          ...
          phaseResult.status = 'FAIL';
          console.error('❌ CRITICAL: No field mappings exist after waiting - user is blocked from proceeding');
        }
        ...
        if (unmappedFields.length > 0) {
          ...
          console.warn(`⚠️ ${unmappedFields.length} fields need mapping:`, unmappedFields.map(f => f.source_field).join(', '));
        }
      ...
      const step3Start = Date.now();
      console.log('✅ Step 3: Apply Attribute Mappings');
      ...
      console.log(`✨ Phase 2 completed in ${phaseResult.duration}ms - Status: ${phaseResult.status}`);
    }
  });
  ...
  test('PHASE 3: Data Cleansing Validation', async ({ page }) => {
    ...
    try {
      console.log('\n🧹 PHASE 3: DATA CLEANSING - Quality Validation');

      // Step 1: Trigger data cleansing
      const step1Start = Date.now();
      console.log('🚀 Step 1: Trigger Data Cleansing');
      ...
      const step2Start = Date.now();
      console.log('⏳ Step 2: Monitor Cleansing Progress');
      ...
      const step3Start = Date.now();
      console.log('📊 Step 3: Validate Data Quality Metrics');
      ...
      console.log(`✨ Phase 3 completed in ${phaseResult.duration}ms - Status: ${phaseResult.status}`);
    }
  });
  ...
  test('PHASE 4: Inventory with Enhanced Validation', async ({ page }) => {
    ...
    try {
      console.log('\n📦 PHASE 4: INVENTORY - Asset Validation');
      ...
      const step2Start = Date.now();
      console.log('🔍 Step 2: Fetch and Validate Assets');
      ...
      const step3Start = Date.now();
      console.log('✅ Step 3: Validate Asset Normalization');
      ...
      console.log(`✨ Phase 4 completed in ${phaseResult.duration}ms - Status: ${phaseResult.status}`);
    }
  });
  ...
  test('PHASE 5: Dependencies Analysis', async ({ page }) => {
    ...
    try {
      console.log('\n🔗 PHASE 5: DEPENDENCIES - Relationship Validation');

      // Step 1: Trigger dependency analysis
      const step1Start = Date.now();
      console.log('🚀 Step 1: Trigger Dependency Analysis');
      ...
      const step2Start = Date.now();
      console.log('📊 Step 2: Fetch Dependency Graph');
      ...
      const step3Start = Date.now();
      console.log('💾 Step 3: Validate Dependency Persistence');
      ...
      console.log(`✨ Phase 5 completed in ${phaseResult.duration}ms - Status: ${phaseResult.status}`);
    }
  });
  ...
  test('PHASE 6: Database Diagnostics and Validation', async () => {
    ...
    try {
      console.log('\n🗄️  PHASE 6: DATABASE DIAGNOSTICS - Deep Validation');

      // Step 1: Call diagnostics endpoint
      const step1Start = Date.now();
      console.log('🔍 Step 1: Fetch Database Diagnostics');
      ...
      const step2Start = Date.now();
      console.log('✅ Step 2: Validate Data Integrity');
      ...
      console.log(`✨ Phase 6 completed in ${phaseResult.duration}ms - Status: ${phaseResult.status}`);
    }
  });
});
