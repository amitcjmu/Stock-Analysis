...
class TestCollectionFlowE2E:
    ...
    @pytest.fixture
    async def test_client_engagement(
        ...
        return {"client": client, "user": user, "engagement": engagement}
    ...
    async def test_complete_collection_flow_e2e(
        ...
        if gap_result["gaps_identified"] > 0:
            ...
            questionnaire_result = await collection_flow.generate_questionnaires(
                gap_result
            )
            ...
            manual_result = await collection_flow.manual_collection(
            ...
            responses = await db_session.execute(
                select(CollectionQuestionnaireResponse).where(
                    CollectionQuestionnaireResponse.collection_flow_id
                    == collection_flow._flow_id
                )
            )
        ...
        assert assessment_package["client_account_id"] == str(
            test_client_engagement["client"].id
        )
        assert assessment_package["engagement_id"] == str(
            test_client_engagement["engagement"].id
        )
    ...
    async def test_data_persistence_integrity(
        ...
        assert state_record.client_account_id == uuid.UUID(
            test_client_engagement["client"].id
        )
        assert state_record.engagement_id == uuid.UUID(
            test_client_engagement["engagement"].id
        )
        ...
        for tier in tiers:
            ...
            flow = UnifiedCollectionFlow(
                ...
                config={
                    ...
                    "client_requirements": {"sixr_requirements": {"rehost": True}},
                },
            )
    ...
    async def test_performance_metrics(
        ...
        assert (
            phase_times["platform_detection"] < 10.0
