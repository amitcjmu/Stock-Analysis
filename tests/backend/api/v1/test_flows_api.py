...
def mock_request_context():
    ...
    return RequestContext(user_id="user_123", client_account_id=1, engagement_id=1)
...
def mock_dependencies(
    ...
    with patch("app.api.v1.flows.get_db", return_value=mock_db_session), patch(
        "app.api.v1.flows.get_current_user", return_value=mock_user
...
class TestFlowCreation:
    ...
    def test_create_flow_success(
        self, api_client, authenticated_headers, mock_dependencies
    ):
        ...
        response = api_client.post(
            "/api/v1/flows", json=flow_data, headers=authenticated_headers
        )
    ...
    def test_create_flow_invalid_type(
        ...
        flow_data = {"flow_type": "", "flow_name": "Test Flow"}  # Invalid empty type
    ...
    def test_create_flow_orchestrator_error(
        self, api_client, authenticated_headers, mock_dependencies
    ):
        """Test flow creation when orchestrator fails"""
        mock_dependencies["orchestrator"].create_flow.side_effect = ValueError(
            "Invalid flow type"
        )

        flow_data = {"flow_type": "invalid_type", "flow_name": "Test Flow"}
...
class TestFlowListing:
    ...
    def test_list_flows_with_filters(
        self, api_client, authenticated_headers, mock_dependencies
    ):
        ...
        mock_dependencies["orchestrator"].get_active_flows.assert_called_with(
            flow_type="discovery", status="running"
        )
    ...
    def test_list_flows_with_pagination(
        ...
        response = api_client.get(
            "/api/v1/flows?page=2&page_size=5", headers=authenticated_headers
        )
...
class TestFlowRetrieval:
    ...
    def test_get_flow_success(
        ...
        response = api_client.get(
            f"/api/v1/flows/{flow_id}", headers=authenticated_headers
        )
        ...
        mock_dependencies["orchestrator"].get_flow_status.assert_called_once_with(
            flow_id
        )
    ...
    def test_get_flow_not_found(
        ...
        response = api_client.get(
            "/api/v1/flows/nonexistent", headers=authenticated_headers
        )
...
class TestFlowExecution:
    ...
    def test_execute_phase_success(
        ...
        flow_id = "flow_12345"
        execution_data = {"phase_input": {"data": "test"}, "force_execution": False}
        ...
        mock_dependencies["orchestrator"].execute_phase.assert_called_once_with(
            flow_id=flow_id, phase_input={"data": "test"}, force_execution=False
        )
    ...
    def test_execute_phase_failure(
        ...
        mock_dependencies["orchestrator"].execute_phase.return_value = (
            False,
            {"error": "Phase failed"},
        )
...
class TestFlowLifecycle:
    ...
    def test_pause_flow_success(
        ...
        response = api_client.post(
            f"/api/v1/flows/{flow_id}/pause", headers=authenticated_headers
        )

        assert response.status_code == 200
        mock_dependencies["orchestrator"].pause_flow.assert_called_once_with(flow_id)

    def test_resume_flow_success(
        self, api_client, authenticated_headers, mock_dependencies
    ):
        ...
        response = api_client.post(
            f"/api/v1/flows/{flow_id}/resume", headers=authenticated_headers
        )
    ...
    def test_lifecycle_operation_failure(
        self, api_client, authenticated_headers, mock_dependencies
    ):
        """Test lifecycle operation failure"""
        mock_dependencies["orchestrator"].pause_flow.return_value = (
            False,
            {"error": "Cannot pause"},
        )
...
class TestFlowStatus:
    ...
    def test_get_flow_status_success(
        self, api_client, authenticated_headers, mock_dependencies
    ):
        ...
        response = api_client.get(
            f"/api/v1/flows/{flow_id}/status", headers=authenticated_headers
        )
    ...
    def test_get_flow_status_not_found(
        ...
        response = api_client.get(
            "/api/v1/flows/nonexistent/status", headers=authenticated_headers
        )
...
class TestFlowAnalytics:
    ...
    def test_get_analytics_success(
        ...
        response = api_client.get(
            "/api/v1/flows/analytics/summary", headers=authenticated_headers
        )
    ...
    def test_get_analytics_with_date_filters(
        self, api_client, authenticated_headers, mock_dependencies
    ):
...
class TestBackwardCompatibility:
    ...
    def test_legacy_discovery_create(
        self, api_client, authenticated_headers, mock_dependencies
    ):
    ...
    def test_legacy_assessment_create(
        self, api_client, authenticated_headers, mock_dependencies
    ):
...
class TestErrorHandling:
    ...
    def test_database_error(self, api_client, authenticated_headers, mock_dependencies):
        ...
        mock_dependencies["orchestrator"].create_flow.side_effect = Exception(
            "Database error"
        )
    ...
    def test_validation_error(self, api_client, authenticated_headers):
        ...
        response = api_client.post(
            "/api/v1/flows", json=invalid_data, headers=authenticated_headers
        )
