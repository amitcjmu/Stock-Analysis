...
class TestDataImportPageLoad:
    ...
    async def setup_test_data(self, db: AsyncSession):
        ...
        # Create client
        client = Client(
            id=uuid.uuid4(), name="Test Client", code="TC001", is_active=True
        )
        db.add(client)
    ...
    async def test_data_import_page_load_sequence(
        ...
        response = await client.get(
            "/api/v1/flows/?flowType=discovery", headers=headers
        )
    ...
    async def test_data_import_page_with_existing_flows(
        self, client: AsyncClient, db: AsyncSession
    ):
    ...
    async def test_admin_user_with_demo_context(
        self, client: AsyncClient, db: AsyncSession
    ):
        ...
        admin_profile = UserProfile(
            id=uuid.uuid4(), user_id=admin.id, status="active", role="platform_admin"
        )
