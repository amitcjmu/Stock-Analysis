...
def test_user_deactivation():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            active_users = [
                u for u in users_data.get("users", []) if u.get("status") == "active"
            ]
            ...
            if active_users:
                ...
                if response.status_code == 200:
                    ...
                    if response.status_code == 200:
                        ...
                        print(
                            f"   ❌ User reactivation FAILED: {response.status_code} - {response.text}"
                        )
                        return False
                else:
                    print(
                        f"   ❌ User deactivation FAILED: {response.status_code} - {response.text}"
                    )
                    return False
            else:
        ...
        if response.status_code == 200:
            ...
            if clients:
                ...
                if response.status_code == 200:
                    ...
                    print(
                        f"   ❌ Demo client creation FAILED: {response.status_code} - {response.text}"
                    )
                    return False
        else:
            print(
                f"   ❌ Failed to get clients: {response.status_code} - {response.text}"
            )
            return False
