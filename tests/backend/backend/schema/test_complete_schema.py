...
async def test_all_models():
    ...
    models_to_test = [
        ...
        (
            "ClientAccount",
            lambda: ClientAccount(id=client_id, name="Test Client", slug="test-client"),
        ),
        ("User", lambda: User(id=user_id, email="test@example.com")),
        (
            "Engagement",
            lambda: Engagement(
                id=engagement_id,
        ...
        (
            "UserAccountAssociation",
            lambda: UserAccountAssociation(
                user_id=user_id, client_account_id=client_id, role="member"
            ),
        ),
        # Discovery flow models
        (
            "DiscoveryFlow",
            lambda: DiscoveryFlow(
                flow_id=flow_id,
                client_account_id=client_id,
        ...
        (
            ...
            lambda: CrewAIFlowStateExtensions(
                flow_id=flow_id,
                client_account_id=client_id,
                engagement_id=engagement_id,
                user_id=str(user_id),
        ...
        (
            "RawImportRecord",
            lambda: RawImportRecord(
                import_id=uuid.uuid4(), client_account_id=client_id, raw_data={}
            ),
        ),
        (
            "ImportFieldMapping",
            lambda: ImportFieldMapping(
                client_account_id=client_id,
                engagement_id=engagement_id,
                source_field="test",
                target_field="test",
        ...
        (
            ...
            lambda: ImportProcessingStep(
                import_id=uuid.uuid4(), step_name="Test Step", step_order=1
            ),
        ),
        # Asset models
        (
        ...
        (
            "CMDBSixRAnalysis",
            lambda: CMDBSixRAnalysis(
                asset_id=uuid.uuid4(), analysis_date=datetime.utcnow()
            ),
        ),
        # RBAC models
        ("UserProfile", lambda: UserProfile(user_id=user_id)),
        (
            "UserRole",
            lambda: UserRole(
                user_id=user_id, role_type="ANALYST", role_name="Data Analyst"
            ),
        ),
        (
            "ClientAccess",
        ...
        (
            "AccessAuditLog",
            lambda: AccessAuditLog(
                user_id=user_id, action_type="login", result="success"
            ),
        ),
        # Enhanced RBAC models
        ("EnhancedUserProfile", lambda: EnhancedUserProfile(user_id=user_id)),
        (
            "RolePermissions",
            lambda: RolePermissions(role_type="ANALYST", permission_name="view_data"),
        ),
        (
            "SoftDeletedItems",
            lambda: SoftDeletedItems(
                table_name="assets",
        ...
        (
            "SecurityAuditLog",
            lambda: SecurityAuditLog(
                event_type="login_success", user_id=user_id, ip_address="127.0.0.1"
            ),
        ),
        (
            "RoleChangeApproval",
            lambda: RoleChangeApproval(
