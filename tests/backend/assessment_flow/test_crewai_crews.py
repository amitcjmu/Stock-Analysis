...
try:
    ...
    from app.services.crewai_flows.assessment.crews.architecture_standards_crew import (
        ArchitectureStandardsCrew,
    )
    from app.services.crewai_flows.assessment.crews.component_analysis_crew import (
        ComponentAnalysisCrew,
    )
    from app.services.crewai_flows.assessment.crews.sixr_strategy_crew import (
        SixRStrategyCrew,
    )
...
class TestComponentAnalysisCrew:
    ...
    async def test_component_analysis_execution(
        ...
        backend_component = next(
            (c for c in components if c["name"] == "backend_api"), None
        )
        ...
        java_debt = next(
            (t for t in tech_debt if t["category"] == "version_obsolescence"), None
        )
    ...
    async def test_component_analysis_with_compliance_check(
        ...
        component_crew.crew.kickoff = AsyncMock(
            return_value=mock_result_with_compliance
        )
    ...
    async def test_large_application_analysis(self, component_crew):
        ...
        mock_large_result = {
            ...
            "tech_debt_analysis": [
                {"category": "version_obsolescence", "score": 7.0} for _ in range(20)
            ],
        ...
        result = await component_crew.execute(
            {"applications": large_app_set, "architecture_standards": []}
        )
...
class TestSixRStrategyCrew:
    ...
    async def test_sixr_strategy_determination(
        ...
        assert (
            frontend_treatment["rationale"] == "React upgrade and modernization needed"
        )
...
class TestAppOnPageCrew:
    ...
    async def test_app_on_page_generation(self, app_page_crew):
        ...
        frontend_app = next(
            (app for app in apps_summary if app["application_id"] == "app-1"), None
        )
    ...
    async def test_app_on_page_with_prioritization(self, app_page_crew):
        ...
        critical_app = next(
            (app for app in apps if app["application_id"] == "app-1"), None
        )
...
class TestCrewIntegration:
    ...
    async def test_crew_output_compatibility(
        ...
        comp_crew.crew.kickoff = AsyncMock(
            return_value={
        ...
        sixr_crew.crew.kickoff = AsyncMock(
            return_value={
                "component_treatments": [],
                "overall_strategy": "refactor",
