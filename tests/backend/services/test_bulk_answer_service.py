...
from app.models.collection_flow import AdaptiveQuestionnaire, CollectionAnswerHistory
...
class TestCollectionBulkAnswerService:
    ...
    async def test_preview_bulk_answers_with_conflicts(
        ...
        existing_answers = [
            ...
            {"asset_id": sample_asset_ids[1], "answer_value": "Java"},  # Different value = conflict
        ]

        with patch.object(service, "_get_existing_answers", return_value=existing_answers):
            result = await service.preview_bulk_answers(
                child_flow_id=child_flow_id,
                asset_ids=sample_asset_ids,
                question_ids=sample_question_ids,
    ...
    async def test_submit_bulk_answers_success(
        ...
        with patch.object(service, "_get_or_create_questionnaire") as mock_get:
            ...
            with patch.object(service, "_record_answer_history", new_callable=AsyncMock):
    ...
    async def test_get_existing_answers(
        ...
        service.questionnaire_repo.get_by_filters = AsyncMock(return_value=mock_questionnaires)
    ...
    @pytest.mark.asyncio
    async def test_get_or_create_questionnaire_new(
        self, mock_db_session, mock_context
    ):
    ...
    @pytest.mark.asyncio
    async def test_should_apply_answer_no_existing(
        self, mock_db_session, mock_context
    ):
