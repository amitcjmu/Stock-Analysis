...
class TestCrewAIFlowServiceDeletedFlows:
    ...
    async def test_resume_deleted_flow_raises_invalid_flow_state_error(
        ...
        with patch.object(
            crewai_flow_service,
            "_get_discovery_flow_service",
            return_value=mock_discovery_flow_service,
        ):
    ...
    async def test_resume_flow_validates_terminal_statuses(
        ...
        for status in terminal_statuses:
            ...
            with patch.object(
                ...
                with patch(
                    "app.services.crewai_flow_service.CREWAI_FLOWS_AVAILABLE", True
                ):
        ...
        with patch.object(
            ...
            with patch("app.services.crewai_flow_service.CREWAI_FLOWS_AVAILABLE", True):
                with patch("app.core.database.AsyncSessionLocal"):
                    with patch(
                        "app.services.master_flow_orchestrator.MasterFlowOrchestrator"
                    ):
                        with patch(
                            "app.services.crewai_flow_service.UnifiedDiscoveryFlow"
                        ):
                            try:
                                ...
                                if "transition from" in str(e) and (
                                    "deleted" in str(e)
                                    or "cancelled" in str(e)
                                    or "completed" in str(e)
                                    or "failed" in str(e)
                                ):
                                    pytest.fail(
                                        f"Should not raise terminal status error for '{flow.status}' status"
                                    )
