...
def sample_csv_data():
    ...
    return [
        {"app_name": "MyApp1", "language": "Python", "database": "PostgreSQL", "owner": "John"},
        {"app_name": "MyApp2", "language": "Java", "database": "MySQL", "owner": "Jane"},
        {"app_name": "MyApp3", "language": "Python", "database": "MongoDB", "owner": "Bob"},
    ]
...
class TestUnifiedImportOrchestrator:
            ...
            with patch.object(
                ...
                AsyncMock(
                    return_value=FieldMapping(
                        ...
                        confidence=0.95,
                        suggestions=[{"field": "app_name", "confidence": 0.95, "method": "fuzzy"}],
                    )
                ),
            ):
                # Mock validation
    ...
    async def test_execute_import_fast_mode(
        ...
        mock_task.id = uuid4()
        service.background_tasks_repo.create_no_commit = AsyncMock(return_value=mock_task)
    ...
    @pytest.mark.asyncio
    async def test_validate_data_duplicates(
        self, mock_db_session, mock_context
    ):
