    ...
    class ErrorSimulator:
        ...
        def simulate_database_error(self):
            ...
            if self.error_count <= 2:
                raise sqlalchemy.exc.DisconnectionError("Database connection lost", None, None)
        ...
        async def execute_async(self, inputs: Dict[str, Any]) -> Dict[str, Any]:
            ...
            if self.failure_mode and self.attempt_count <= self.failure_count:
                if self.failure_mode == "network":
                    ...
                    raise sqlalchemy.exc.DisconnectionError("DB connection lost", None, None)
...
def recovery_manager():
    ...
    class RecoveryManager:
        ...
        async def retry_with_backoff(self, operation, max_retries: int = 3, base_delay: float = 1.0):
        ...
        async def execute_with_fallback(self, operation_name: str, primary_operation, inputs: Dict[str, Any]):
...
class TestDiscoveryFlowErrorHandling:
    ...
    async def test_crew_execution_failure_and_retry(
        ...
        # Act
        result = await recovery_manager.retry_with_backoff(crew_operation, max_retries=3)

        # Assert
        assert result['status'] == 'completed', "Crew should eventually succeed after retries"
        assert result['recovered'], "Should indicate recovery occurred"
    ...
    async def test_network_timeout_recovery(
        ...
        # Act
        result = await recovery_manager.retry_with_backoff(network_operation, max_retries=2)
    ...
    async def test_database_connection_failure_recovery(
        ...
        with patch('time.sleep', return_value=None):  # Speed up test
            result = await recovery_manager.retry_with_backoff(db_operation, max_retries=3)
    ...
    async def test_partial_workflow_failure_handling(
        ...
        for i, crew in enumerate(crews):
            try:
                ...
                result = await recovery_manager.retry_with_backoff(crew_operation, max_retries=2)
        ...
        assert len(successful_crews) == 4, "All crews should eventually succeed with retries"
    ...
    async def test_graceful_degradation_with_fallback(
        ...
        # Arrange
        failing_crew = mock_failing_crew(failure_mode="memory", failure_count=5)  # Always fails
        ...
        assert result['fallback_used'], "Should indicate fallback was used"
        assert result['reduced_scope'], "Should indicate reduced scope operation"
    ...
    async def test_workflow_state_preservation_during_errors(
        ...
        crews = [
            ...
            ("data_cleansing", mock_failing_crew(failure_mode="network", failure_count=1)),
            ("inventory_building", mock_failing_crew(failure_mode="timeout", failure_count=1))
        ]
        ...
        for crew_name, crew in crews:
            try:
                ...
                workflow_state['retry_counts'][crew_name] = result.get('attempt_count', 1)
        ...
        # Assert
        assert len(workflow_state['completed_crews']) == 3, "All crews should eventually complete"
        assert len(workflow_state['failed_crews']) == 0, "No crews should fail permanently"
        assert all(name in workflow_state['partial_results'] for name, _ in crews), "All results should be preserved"
        assert workflow_state['retry_counts']['data_cleansing'] > 1, "Network failure crew should retry"
        assert workflow_state['retry_counts']['inventory_building'] > 1, "Timeout crew should retry"

    @pytest.mark.asyncio
    async def test_error_propagation_and_isolation(
        self,
        mock_failing_crew
    ):
        ...
        class WorkflowOrchestrator:
            ...
            async def execute_workflow(self, crews: List[tuple], inputs: Dict[str, Any]):
                results = {}
                errors = {}
        ...
        crews = [
            ...
            ("crew_2", mock_failing_crew(failure_mode="validation", failure_count=5)),  # Always fails
        ...
        # Assert
        assert len(results) == 3, "All crews should have results (success or isolated failure)"
        ...
        assert results['crew_2']['isolated'], "Failed crew should be marked as isolated"
    ...
    async def test_resource_cleanup_after_failures(
        ...
        assert resource_manager.cleanup_performed, "Cleanup should occur despite errors"
        assert len(resource_manager.allocated_resources) == 0, "All resources should be cleaned up"

    @pytest.mark.asyncio
    async def test_error_reporting_and_monitoring(
        self,
        mock_failing_crew
    ):
        ...
        class ErrorMonitor:
            ...
            def log_error(self, crew_name: str, error: Exception, context: Dict[str, Any]):
                error_entry = {
                    'crew_name': crew_name,
        ...
        assert error_monitor.should_alert("test_crew", "ConnectionError"), "Should trigger alert"
        ...
        assert first_error['error_type'] == 'ConnectionError', "Error type should be logged"
