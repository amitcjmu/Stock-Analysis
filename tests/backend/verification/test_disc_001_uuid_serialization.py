...
class TestDISC001UUIDSerialization:
    ...
    async def test_data_cleansing_analysis_no_uuid_error(
        ...
        async with httpx.AsyncClient() as client:
            try:
                ...
                return (
                    False,
                    f"JSON decode error (likely UUID serialization issue): {str(e)}",
                )
            except Exception as e:
                return False, f"Unexpected error: {str(e)}"

    async def test_data_cleansing_stats_no_uuid_error(
        self, base_url: str, auth_headers: Dict[str, str]
    ):
        """
    ...
    def test_uuid_string_conversion(self):
        ...
        assert "UUID" in str(exc_info.value) or "not JSON serializable" in str(
            exc_info.value
        )
...
if __name__ == "__main__":
    ...
    async def run_verification():
        ...
        success, message = await test.test_data_cleansing_analysis_no_uuid_error(
            base_url, headers
        )
        if success:
            print(f"✅ {message}")
        ...
        success, message = await test.test_data_cleansing_stats_no_uuid_error(
            base_url, headers
        )
        if success:
            print(f"✅ {message}")
