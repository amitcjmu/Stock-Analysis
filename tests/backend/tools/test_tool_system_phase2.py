...
class TestPhase2ToolSystem:
    ...
    def test_phase_tool_mapping(self):
        ...
        assert (
            "field_matcher" in field_mapping_tools
        ), "field_matcher should be in field_mapping phase"

        # Test data validation phase
        validation_tools = get_tools_for_phase("data_validation")
        assert (
            "schema_analyzer" in validation_tools
        ), "schema_analyzer should be in data_validation phase"
        assert (
            "pii_scanner" in validation_tools
        ), "pii_scanner should be in data_validation phase"
    ...
    def test_tool_capabilities(self):
        ...
        assert (
            "field_matcher" in capabilities["mapping"]
        ), "field_matcher should be in mapping category"
    ...
    def test_agent_factory_integration(self):
        ...
        ), "Agent factory should have tool registry"
    ...
    def test_context_awareness_pattern(self):
        ...
        # Check inheritance
        assert issubclass(
            BaseDiscoveryTool, ContextAwareTool
        ), "Tools should be context-aware"
