    ...
    queryFn: async () => {
      try {
        ...
        const incompleteFlows = allFlows.filter((flow: unknown) =>
          ...
          created_at: flow.createdAt || flow.created_at || flow.startTime || new Date().toISOString(),
          updated_at: flow.updatedAt || flow.updated_at || flow.lastUpdated || new Date().toISOString(),
          error: flow.error,
