...
export function useFlowUpdates(
  ...
  const connectSSE = useCallback(() => {
    ...
    try {
      ...
      const sseUrl = `${process.env.NEXT_PUBLIC_API_URL}/api/v1/unified-discovery/flows/${flowId}/events`;
  ...
  const pollStatus = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/v1/unified-discovery/flows/${flowId}/status`,
        {
          method: 'GET',
          headers: getHeaders(),
        }
