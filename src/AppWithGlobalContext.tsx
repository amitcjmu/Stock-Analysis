...
// New GlobalContext system
import { GlobalContextProvider, useGlobalAuth } from './contexts/GlobalContext';
import { ContextDebugger, PerformanceMetrics, CacheStatusIndicator } from './contexts/GlobalContext/debugger';
import { ContextMigrationProvider } from './contexts/GlobalContext/compatibility';
...
import { BundleAnalyzer } from './components/lazy/EnhancedLazyComponents';

// All lazy-loaded route components (same as original App.tsx)
import {
...
// Component to handle authenticated routes with new GlobalContext
const AuthenticatedAppWithGlobalContext = (): JSX.Element => {
  const { isLoading, isAuthenticated, user } = useGlobalAuth();
  ...
  useEffect(() => {
    if (isAuthenticated && !appInitialized) {
      AppInitializer.initialize()
        ...
        .catch(() => setAppInitialized(true)); // Continue anyway
    }
  ...
  // Authenticated routes (same as original App.tsx)
  return (
    <>
      <Routes>
...
// New App component with GlobalContext
const AppWithGlobalContext = (): JSX.Element => {
