...
const QualityIssuesSummary: React.FC<QualityIssuesSummaryProps> = ({
  ...
  const handleIssueClick = (issueId: string) => {
    try {
      ...
      if (onIssueSelect) {
        onIssueSelect(selectedIssue === issueId ? null : issueId);
      }
    } catch (error) {
    ...
    <div className="bg-white rounded-lg shadow-md">
      <div className="p-4 border-b border-gray-200">
        <h3 className="text-lg font-semibold text-gray-900 flex items-center">
          ...
          Quality Issues ({qualityIssues?.length || 0})
        </h3>
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <div className="flex items-center space-x-2 mb-1">
                        <p className="text-sm font-medium text-gray-900">{issue.asset_name || 'Unknown Asset'}</p>
                      ...
                      <p className="text-xs text-gray-600 font-medium">{issue.issue_type || 'Quality Issue'}</p>
                      <p className="text-xs text-gray-500">Field: {issue.field_name || 'unknown'}</p>
                      {issue.current_value && (
