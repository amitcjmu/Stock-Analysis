...
const FieldMappingsTab: React.FC<FieldMappingsTabProps> = ({
  ...
  const fetchAvailableFields = useCallback(async () => {
    ...
    try {
      ...
      if (response && response.fields) {
        ...
        const fieldsWithCategories = response.fields.map((field: unknown) => ({
          ...field,
  ...
  const editableFieldMappings = fieldMappings.map((mapping: unknown) => ({
    ...mapping,
    status: mapping.status === 'suggested' ? 'pending' : mapping.status || 'pending'
  }));
  
  // Apply filters
  const filteredMappings = editableFieldMappings.filter((mapping: unknown) => {
