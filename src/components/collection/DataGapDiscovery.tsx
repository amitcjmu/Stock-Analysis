...
import { Sparkles, RefreshCw, Save, CheckCircle, X, AlertCircle } from 'lucide-react';

import { Card, CardContent, CardDescription, CardHeader, CardTitle } from '@/components/ui/card';
import { Button } from '@/components/ui/button';
import { Badge } from '@/components/ui/badge';
import { useToast } from '@/hooks/use-toast';
import { collectionFlowApi, type DataGap, type GapUpdate } from '@/services/api/collection-flow';
...
const DataGapDiscovery: React.FC<DataGapDiscoveryProps> = ({
  ...
  const handleScanGaps = async () => {
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Failed to scan gaps';
      toast({
        title: "Scan Failed",
  ...
  const handleAnalyzeGaps = async () => {
    ...
    try {
      setIsAnalyzing(true);
      const response = await collectionFlowApi.analyzeGaps(flowId, gaps, selectedAssetIds);
  ...
  const handleCellEditingStopped = useCallback((event: CellEditingStoppedEvent<GapRowData>) => {
  ...
  const handleSaveGap = useCallback(async (gap: GapRowData) => {
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Failed to save gap';
      toast({
        title: "Save Failed",
        description: errorMessage,
        variant: "destructive"
  ...
  const handleBulkAccept = async () => {
    ...
    const highConfidenceGaps = gaps.filter(gap =>
      gap.confidence_score !== null && gap.confidence_score >= 0.8
    );
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Failed to bulk accept';
      toast({
  ...
  const confidenceRenderer = (params: { value: number | null }) => {
    if (params.value === null || params.value === undefined) {
      return '<span class="text-gray-400">No AI</span>';
    }
    ...
    return `<span class="${colorClass} font-semibold">${percentage}%</span>`;
  };

  // Save button renderer
  const actionRenderer = (params: { data: GapRowData }) => {
    ...
    return `
      <button
        class="save-btn px-2 py-1 bg-blue-600 text-white rounded hover:bg-blue-700 text-sm"
        data-gap-id="${gap.id}"
      >
        Save
      </button>
  ...
  const columnDefs = useMemo<Array<ColDef<GapRowData>>>(() => [
    ...
    },
    {
      ...
      cellRenderer: (params: { value: number }) => {
        const badges = {
          1: '<span class="px-2 py-1 bg-red-100 text-red-800 rounded text-xs font-medium">Critical</span>',
          2: '<span class="px-2 py-1 bg-orange-100 text-orange-800 rounded text-xs font-medium">High</span>',
          3: '<span class="px-2 py-1 bg-yellow-100 text-yellow-800 rounded text-xs font-medium">Medium</span>',
          4: '<span class="px-2 py-1 bg-blue-100 text-blue-800 rounded text-xs font-medium">Low</span>'
        };
    ...
    params.api.getRenderedNodes().forEach(node => {
      ...
      if (eGui) {
        eGui.addEventListener('click', (event) => {
          ...
          const gapId = (event.target as HTMLElement).getAttribute('data-gap-id');
    ...
    <div className="space-y-6">
          ...
          <CardContent>
            <div className="grid grid-cols-3 gap-4">
              <div>
                ...
                <div className="text-2xl font-bold">{scanSummary.total_gaps}</div>
              </div>
              <div>
                <div className="text-sm text-gray-600">Critical Gaps</div>
                <div className="text-2xl font-bold text-red-600">{scanSummary.critical_gaps}</div>
              </div>
              <div>
                <div className="text-sm text-gray-600">Scan Time</div>
                <div className="text-2xl font-bold">{scanSummary.execution_time_ms}ms</div>
              </div>
            </div>
          </CardContent>
        </Card>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Click cells to edit, then save individually. Color-coded AI confidence:
          ...
          <CardContent className="pt-6">
            <div className="text-center">
              ...
              <p className="text-gray-600 mb-4">
                All selected assets have complete critical attributes. Ready to proceed.
              </p>
              {onComplete && (
