...
export const useClientOperations = (): {
  ...
  const createClient = useCallback(
    async (formData: ClientFormData): Promise<Client | null> => {
      try {
        ...
        if (errorObj.response && errorObj.response.detail) {
          if (Array.isArray(errorObj.response.detail)) {
            const validationErrors = errorObj.response.detail.map(
              (d: { loc?: string[]; msg?: string; message?: string }) => {
                if (d.loc && d.msg) {
  ...
  const updateClient = useCallback(
    async (client: Client, formData: ClientFormData): Promise<Client | null> => {
      try {
        ...
        const errorObj = error as {
          message?: string;
          response?: { detail?: string | Array<{ msg?: string; message?: string }> };
        };
        let errorMessage = errorObj.message || 'Failed to update client. Please try again.';
        if (errorObj.response && errorObj.response.detail) {
          if (Array.isArray(errorObj.response.detail)) {
            errorMessage = errorObj.response.detail
              .map(
                (d: { msg?: string; message?: string }) => d.msg || d.message || JSON.stringify(d)
              )
              .join(', ');
          } else {
            errorMessage = String(errorObj.response.detail);
