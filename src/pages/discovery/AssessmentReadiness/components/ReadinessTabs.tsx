                ...
                <div key={key} className="space-y-2">
                  <h4 className="font-medium capitalize">
                    ...
                    <span className="ml-2 text-sm text-gray-500">
                      ({Math.round((value as unknown).score * 100)}%)
                    </span>
                  </h4>
                  <div className="h-2 w-full bg-gray-200 rounded-full overflow-hidden">
                    <div 
                      className={`h-full ${getReadinessColor((value as unknown).score)}`}
                      style={{ width: `${(value as unknown).score * 100}%` }}
                    />
                  </div>
                  
                  {(value as unknown).gaps?.length > 0 && (
                    <div className="mt-2 text-sm text-gray-600">
                      <p className="font-medium">Areas for Improvement:</p>
                      <ul className="list-disc pl-5 mt-1 space-y-1">
                        {(value as unknown).gaps.slice(0, 3).map((gap: string, i: number) => (
                          <li key={i}>{gap}</li>
                        ))}
                      </ul>
                    </div>
