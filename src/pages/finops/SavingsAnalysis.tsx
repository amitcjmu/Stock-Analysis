...
const SavingsAnalysis = (): JSX.Element => {
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-lg text-gray-600">
                    Track and analyze cost savings
                  </p>
                </div>
                      ...
                      <CardContent className="p-4">
                        <div className="flex items-start justify-between mb-4">
                          <div>
                            ...
                            <div className="flex items-center space-x-2 mt-1">
                              <Badge variant={saving.type === 'Immediate' ? 'destructive' : 'secondary'}>
                                {saving.type}
                              </Badge>
                              <Badge variant={saving.status === 'Active' ? 'default' : 'outline'}>
                                {saving.status}
                              </Badge>
                            </div>
                          </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm mb-4">
                          <div>
                            ...
                            <span className="ml-2 font-medium">${saving.currentCost}</span>
                          </div>
                          <div>
                            <span className="text-gray-600">Potential Savings:</span>
                            <span className="ml-2 font-medium">${saving.potentialSavings}</span>
                          </div>
                          <div>
                            <span className="text-gray-600">ROI:</span>
                            <span className="ml-2 font-medium">{saving.roi}x</span>
                          </div>
                        </div>
                        <div className="space-y-4">
                          {saving.categories.map((category) => (
                            <div key={category.name} className="space-y-2">
                              <div className="flex justify-between text-sm mb-1">
                                <span className="text-gray-600">{category.name}</span>
                                <span className="font-medium">${category.savings}</span>
                              </div>
                        ...
                        {saving.recommendation && (
                          <div className="mt-4 p-3 bg-blue-50 rounded-lg">
                            <p className="text-sm text-blue-800">
                              <strong>Implementation:</strong> {saving.recommendation}
                            </p>
                          </div>
                        )}
                      </CardContent>
