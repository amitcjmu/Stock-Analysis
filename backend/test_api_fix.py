    ...
    async with AsyncSessionLocal() as db:
        try:
            ...
            no_flows = await get_active_master_flows(
                flowType="nonexistent", db=db, current_user=mock_user_context
            )
            ...
            expected_discovery = sum(
                1 for flow in all_flows if flow["flow_type"] == "discovery"
            )
