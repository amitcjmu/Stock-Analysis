...
def upgrade() -> None:
    ...
    for column_name, column_spec in required_columns.items():
        if not check_column_exists(connection, 'crewai_flow_state_extensions', column_name):
            print(f"   + Adding column: {column_name}")
            if column_spec['default']:
                op.add_column('crewai_flow_state_extensions', 
                    sa.Column(column_name, column_spec['type'], 
    ...
    indexes_to_create = [
        ('ix_crewai_flow_state_extensions_flow_id', 'crewai_flow_state_extensions', ['flow_id'], True),
        ('ix_crewai_flow_state_extensions_client_account_id', 'crewai_flow_state_extensions', ['client_account_id'], False),
        ('ix_crewai_flow_state_extensions_engagement_id', 'crewai_flow_state_extensions', ['engagement_id'], False),
    ...
    if check_column_exists(connection, 'crewai_flow_state_extensions', 'discovery_flow_id'):
