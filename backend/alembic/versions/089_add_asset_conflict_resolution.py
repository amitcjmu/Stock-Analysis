...
# CC: Idempotency helper functions for resilient migrations
...
def upgrade() -> None:
    ...
    if not table_exists("asset_conflict_resolutions"):
        op.create_table(
        ...
        print("Table asset_conflict_resolutions already exists, skipping creation")

    # Indexes for common query patterns (idempotent - skip if already exist)

    # Query 1: List pending conflicts for a flow (most common in UI)
    if not index_exists(
        "ix_asset_conflicts_pending_by_flow", "asset_conflict_resolutions"
    ):
        op.create_index(
    ...
    if not index_exists(
        "ix_asset_conflicts_tenant_scope", "asset_conflict_resolutions"
    ):
        op.create_index(
    ...
    if not index_exists("ix_asset_conflicts_master_flow", "asset_conflict_resolutions"):
        op.create_index(
    ...
    if not index_exists(
        "ix_asset_conflicts_existing_asset", "asset_conflict_resolutions"
    ):
        op.create_index(
    ...
    if not index_exists("ix_asset_conflicts_dedup_key", "asset_conflict_resolutions"):
        op.create_index(
...
def downgrade() -> None:
    """Drop asset_conflict_resolutions table and indexes (idempotent)"""

    # Drop indexes first (idempotent using IF EXISTS)
    # CC: Using raw SQL for idempotent drops
    op.execute("DROP INDEX IF EXISTS migration.ix_asset_conflicts_dedup_key")
    op.execute("DROP INDEX IF EXISTS migration.ix_asset_conflicts_existing_asset")
    op.execute("DROP INDEX IF EXISTS migration.ix_asset_conflicts_master_flow")
    op.execute("DROP INDEX IF EXISTS migration.ix_asset_conflicts_tenant_scope")
    op.execute("DROP INDEX IF EXISTS migration.ix_asset_conflicts_pending_by_flow")

    # Drop table (idempotent)
    if table_exists("asset_conflict_resolutions"):
