...
def upgrade() -> None:
    ...
    op.add_column('users', sa.Column('default_client_id', postgresql.UUID(as_uuid=True), nullable=True))
    op.add_column('users', sa.Column('default_engagement_id', postgresql.UUID(as_uuid=True), nullable=True))
    
    # Add foreign key constraints
    op.create_foreign_key(
        'fk_users_default_client', 'users', 'client_accounts', 
        ['default_client_id'], ['id'], ondelete='SET NULL'
    )
    op.create_foreign_key(
        'fk_users_default_engagement', 'users', 'engagements', 
        ['default_engagement_id'], ['id'], ondelete='SET NULL'
    )
