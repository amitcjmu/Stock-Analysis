"""revert_gaps_fk_to_id

Revision ID: 084_revert_gaps_fk_to_id
Revises: 083_revert_questionnaire_fk_to_id
Create Date: 2025-10-05

REVERT INCORRECT FK RETARGETING - Migration 082 was wrong!

Same root cause as questionnaires:
- ORM models define FK as: ForeignKey("collection_flows.id", ondelete="CASCADE")
- Migration 082 changed FK to point to "collection_flows.flow_id" (business identifier)
...
revision = "084_revert_gaps_fk_to_id"
down_revision = "083_revert_questionnaire_fk_to_id"
branch_labels = None
depends_on = None
...
def upgrade():
    ...
    # Step 1: Drop FK pointing to flow_id (from migration 082)
    result = conn.execute(
        sa.text(
            """
    ...
    # CRITICAL: Gaps currently store flow_id, need to map to id for FK
    conn.execute(
        sa.text(
            """
