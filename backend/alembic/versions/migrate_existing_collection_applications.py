"""Migrate collection apps to canonical system"""
...
down_revision = "canonical_apps_001"
branch_labels = None
depends_on = None
...
def upgrade() -> None:
    ...
    try:
        # Get all existing applications with unique names
        ...
        existing_apps = existing_apps_result.fetchall()

        if not existing_apps:
            logger.info("No existing applications found")
            return
        ...
        for app_row in existing_apps:
            ...
            canonical_name_query = text(
                """
                SELECT canonical_name FROM migration.canonical_applications
            ...
            if app_name.strip() != canonical_name:
                ...
                if not existing_variant_result.fetchone():
                    # Create new variant
                    insert_variant = text(
                        """
        ...
        # Update collection_flow_applications with canonical references
        ...
        logger.info(
            f"Updated tenant fields for {tenant_update_result.rowcount} applications"
        )
        ...
        for app_row in apps_to_update:
            ...
            if canonical_id:
                ...
                if (
                    canonical_name_row
                    and canonical_name_row.canonical_name != app_name.strip()
                ):
                    # Find the variant
                    variant_query = text(
                        """
        ...
        logger.info(
            f"Updated usage statistics for {usage_result.rowcount} applications"
        )
