...
def upgrade():
    ...
    op.create_table('agent_task_history',
        ...
        sa.Column('thinking_phases_count', sa.INTEGER(), nullable=False, server_default='0', 
                  comment='Number of thinking/reasoning phases during execution'),
        sa.Column('token_usage', postgresql.JSONB(astext_type=sa.Text()), 
                  nullable=False, server_default=sa.text("'{}'::jsonb"), 
                  comment='Detailed token usage: {input_tokens, output_tokens, total_tokens}'),
        sa.Column('memory_usage_mb', sa.DECIMAL(precision=8, scale=2), nullable=True, 
        ...
        sa.Column('created_at', sa.TIMESTAMP(timezone=True), nullable=False, 
                  server_default=sa.text('now()'), comment='When this record was created'),
        
        # Primary key
        sa.PrimaryKeyConstraint('id', name=op.f('pk_agent_task_history')),
        
        # Foreign key constraints
        sa.ForeignKeyConstraint(['flow_id'], ['crewai_flow_state_extensions.flow_id'], 
        ...
        sa.CheckConstraint('duration_seconds >= 0', 
                          name='chk_agent_task_history_duration_seconds'),
        sa.CheckConstraint("status IN ('pending', 'starting', 'running', 'thinking', 'waiting_llm', 'processing_response', 'completed', 'failed', 'timeout')", 
                          name='chk_agent_task_history_status'),
        sa.CheckConstraint("agent_type IN ('individual', 'crew_member')", 
                          name='chk_agent_task_history_agent_type')
    )
    
    # Create indexes for agent_task_history
    op.create_index(op.f('ix_agent_task_history_agent_name'), 'agent_task_history', 
                    ['agent_name', sa.text('created_at DESC')], unique=False)
    op.create_index(op.f('ix_agent_task_history_flow_id'), 'agent_task_history', 
                    ['flow_id'], unique=False)
    op.create_index(op.f('ix_agent_task_history_status'), 'agent_task_history', 
                    ['status', sa.text('created_at DESC')], unique=False)
    op.create_index(op.f('ix_agent_task_history_client'), 'agent_task_history', 
                    ['client_account_id', 'engagement_id'], unique=False)
    op.create_index(op.f('ix_agent_task_history_task_id'), 'agent_task_history', 
    ...
    op.create_table('agent_performance_daily',
        ...
        sa.Column('avg_duration_seconds', sa.DECIMAL(precision=10, scale=3), nullable=True, 
                  comment='Average task duration in seconds'),
        sa.Column('avg_confidence_score', sa.DECIMAL(precision=3, scale=2), nullable=True, 
        ...
        sa.Column('updated_at', sa.TIMESTAMP(timezone=True), nullable=False, 
                  server_default=sa.text('now()'), comment='When this record was last updated'),
        ...
        sa.UniqueConstraint('agent_name', 'date_recorded', 'client_account_id', 'engagement_id', 
    ...
    op.create_index(op.f('ix_agent_performance_daily_agent'), 'agent_performance_daily', 
                    ['agent_name', sa.text('date_recorded DESC')], unique=False)
    op.create_index(op.f('ix_agent_performance_daily_client'), 'agent_performance_daily', 
                    ['client_account_id', 'engagement_id'], unique=False)
    op.create_index(op.f('ix_agent_performance_daily_date'), 'agent_performance_daily', 
                    ['date_recorded'], unique=False)
    ...
    op.create_table('agent_discovered_patterns',
        ...
        sa.Column('pattern_id', sa.VARCHAR(length=255), nullable=False, 
                  comment='Unique identifier for the pattern'),
        sa.Column('pattern_type', sa.VARCHAR(length=100), nullable=False, 
                  comment='Type of pattern discovered'),
        sa.Column('pattern_name', sa.VARCHAR(length=255), nullable=False, 
        ...
        sa.Column('discovered_by_agent', sa.VARCHAR(length=100), nullable=False, 
        ...
        sa.Column('pattern_data', postgresql.JSONB(astext_type=sa.Text()), 
                  nullable=False, server_default=sa.text("'{}'::jsonb"), 
        ...
        sa.Column('execution_context', postgresql.JSONB(astext_type=sa.Text()), 
                  nullable=False, server_default=sa.text("'{}'::jsonb"), 
                  comment='Context in which pattern was discovered'),
        sa.Column('user_feedback_given', sa.BOOLEAN(), nullable=False, server_default='false', 
                  comment='Whether user has provided feedback on this pattern'),
        sa.Column('pattern_effectiveness_score', sa.DECIMAL(precision=3, scale=2), nullable=True, 
        ...
        sa.Column('created_at', sa.TIMESTAMP(timezone=True), nullable=False, 
                  server_default=sa.text('now()'), comment='When this pattern was discovered'),
        sa.Column('updated_at', sa.TIMESTAMP(timezone=True), nullable=False, 
                  server_default=sa.text('now()'), comment='When this record was last updated'),
        ...
        sa.UniqueConstraint('pattern_id', 'client_account_id', 'engagement_id', 
                           name='uq_agent_discovered_patterns_pattern_client_engagement'),
        
        # Check constraints
        sa.CheckConstraint('confidence_score >= 0 AND confidence_score <= 1', 
                          name='chk_agent_discovered_patterns_confidence_score'),
        sa.CheckConstraint('pattern_effectiveness_score >= 0 AND pattern_effectiveness_score <= 1', 
                          name='chk_agent_discovered_patterns_effectiveness_score')
    )
    
    # Create indexes for agent_discovered_patterns
    op.create_index(op.f('ix_agent_discovered_patterns_agent'), 'agent_discovered_patterns', 
                    ['discovered_by_agent'], unique=False)
    op.create_index(op.f('ix_agent_discovered_patterns_pattern_id'), 'agent_discovered_patterns', 
                    ['pattern_id'], unique=False)
    op.create_index(op.f('ix_agent_discovered_patterns_pattern_type'), 'agent_discovered_patterns', 
                    ['pattern_type'], unique=False)
    op.create_index(op.f('ix_agent_discovered_patterns_client'), 'agent_discovered_patterns', 
                    ['client_account_id', 'engagement_id'], unique=False)
    op.create_index(op.f('ix_agent_discovered_patterns_confidence'), 'agent_discovered_patterns', 
                    [sa.text('confidence_score DESC')], unique=False)
    ...
    op.execute("COMMENT ON TABLE agent_task_history IS 'Detailed history of all agent task executions for performance tracking and analysis';")
    op.execute("COMMENT ON TABLE agent_performance_daily IS 'Daily aggregated performance metrics for each agent';")
    op.execute("COMMENT ON TABLE agent_discovered_patterns IS 'Patterns discovered by agents during task execution for learning and optimization';")
...
def downgrade():
    ...
    # Drop triggers first
    op.execute("DROP TRIGGER IF EXISTS update_agent_discovered_patterns_updated_at ON agent_discovered_patterns;")
    op.execute("DROP TRIGGER IF EXISTS update_agent_performance_daily_updated_at ON agent_performance_daily;")
    
    # Drop the trigger function
    op.execute("DROP FUNCTION IF EXISTS update_agent_observability_updated_at();")
    
    # Drop indexes for agent_discovered_patterns
    op.drop_index(op.f('ix_agent_discovered_patterns_confidence'), table_name='agent_discovered_patterns')
    op.drop_index(op.f('ix_agent_discovered_patterns_client'), table_name='agent_discovered_patterns')
    op.drop_index(op.f('ix_agent_discovered_patterns_pattern_type'), table_name='agent_discovered_patterns')
    op.drop_index(op.f('ix_agent_discovered_patterns_pattern_id'), table_name='agent_discovered_patterns')
    op.drop_index(op.f('ix_agent_discovered_patterns_agent'), table_name='agent_discovered_patterns')
    
    # Drop agent_discovered_patterns table
    op.drop_table('agent_discovered_patterns')
    
    # Drop indexes for agent_performance_daily
    op.drop_index(op.f('ix_agent_performance_daily_date'), table_name='agent_performance_daily')
    op.drop_index(op.f('ix_agent_performance_daily_client'), table_name='agent_performance_daily')
    op.drop_index(op.f('ix_agent_performance_daily_agent'), table_name='agent_performance_daily')
    
    # Drop agent_performance_daily table
    op.drop_table('agent_performance_daily')
    
    # Drop indexes for agent_task_history
    op.drop_index(op.f('ix_agent_task_history_task_id'), table_name='agent_task_history')
    ...
    op.drop_index(op.f('ix_agent_task_history_flow_id'), table_name='agent_task_history')
    op.drop_index(op.f('ix_agent_task_history_agent_name'), table_name='agent_task_history')
