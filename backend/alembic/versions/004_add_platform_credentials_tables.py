...
def upgrade() -> None:
    ...
    result = bind.execute(
        sa.text(
            "SELECT EXISTS(SELECT 1 FROM pg_type WHERE typname = 'credentialstatus')"
        )
    ).scalar()
    if not result:
        sa.Enum(
            "active", "expired", "revoked", "pending_rotation", name="credentialstatus"
        ).create(bind)
    ...
    result = bind.execute(
        sa.text("SELECT EXISTS(SELECT 1 FROM pg_type WHERE typname = 'vaultprovider')")
    ).scalar()
    if not result:
        sa.Enum(
            "local",
    ...
    create_table_if_not_exists(
        ...
        sa.ForeignKeyConstraint(
            ["client_account_id"],
            ["client_accounts.id"],
            name=op.f("fk_platform_credentials_client_account_id_client_accounts"),
        ),
        sa.ForeignKeyConstraint(
            ["created_by"],
            ["users.id"],
            name=op.f("fk_platform_credentials_created_by_users"),
        ),
        sa.ForeignKeyConstraint(
            ["platform_adapter_id"],
            ["platform_adapters.id"],
            name=op.f("fk_platform_credentials_platform_adapter_id_platform_adapters"),
        ),
        sa.ForeignKeyConstraint(
            ["updated_by"],
            ["users.id"],
            name=op.f("fk_platform_credentials_updated_by_users"),
        ),
    ...
    create_table_if_not_exists(
        ...
        sa.ForeignKeyConstraint(
            ...
            ["platform_credentials.id"],
            name=op.f(
                "fk_credential_rotation_history_credential_id_platform_credentials"
            ),
            ondelete="CASCADE",
        ),
        sa.ForeignKeyConstraint(
    ...
    if table_exists("collected_data_inventory"):
        ...
        constraint_name = (
            "fk_collected_data_inventory_credential_id_platform_credentials"
        )
