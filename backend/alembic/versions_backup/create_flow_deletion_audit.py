...
def upgrade():
    ...
    op.create_table(
        ...
        sa.Column('data_deleted', postgresql.JSONB(astext_type=sa.Text()), 
                 nullable=False, server_default='{}',
                 comment='Summary of what was deleted'),
        sa.Column('deletion_impact', postgresql.JSONB(astext_type=sa.Text()), 
                 nullable=False, server_default='{}',
                 comment='Impact analysis'),
        sa.Column('cleanup_summary', postgresql.JSONB(astext_type=sa.Text()), 
                 nullable=False, server_default='{}',
        ...
        sa.Column('knowledge_base_refs_cleaned', postgresql.JSONB(astext_type=sa.Text()), 
                 nullable=False, server_default='[]'),
        sa.Column('agent_memory_cleaned', sa.Boolean(), nullable=False, server_default='false'),
        
        # Audit trail
        sa.Column('deleted_at', sa.TIMESTAMP(timezone=True), nullable=False, server_default=sa.text('NOW()')),
        ...
        sa.Column('recovery_data', postgresql.JSONB(astext_type=sa.Text()), 
                 nullable=False, server_default='{}'),
    ...
    op.create_index(
        'idx_flow_deletion_audit_session', 
        'flow_deletion_audit', 
        ['session_id']
    )
