...
def upgrade() -> None:
    ...
    op.add_column(
        "workflow_states",
        sa.Column("phase_completion", sa.JSON(), nullable=False, server_default="{}"),
    )
    op.add_column(
        "workflow_states",
        sa.Column("crew_status", sa.JSON(), nullable=False, server_default="{}"),
    )

    # Results storage
    op.add_column(
        "workflow_states", sa.Column("field_mappings", sa.JSON(), nullable=True)
    )
    op.add_column(
        "workflow_states", sa.Column("cleaned_data", sa.JSON(), nullable=True)
    )
    op.add_column(
        "workflow_states", sa.Column("asset_inventory", sa.JSON(), nullable=True)
    )
    op.add_column(
        "workflow_states", sa.Column("dependencies", sa.JSON(), nullable=True)
    )
    op.add_column(
        "workflow_states", sa.Column("technical_debt", sa.JSON(), nullable=True)
    )

    # Quality and metrics
    op.add_column(
        "workflow_states", sa.Column("data_quality_metrics", sa.JSON(), nullable=True)
    )
    op.add_column(
        "workflow_states", sa.Column("agent_insights", sa.JSON(), nullable=True)
    )
    op.add_column(
        "workflow_states", sa.Column("success_criteria", sa.JSON(), nullable=True)
    )
    ...
    op.add_column(
        "workflow_states",
        sa.Column("workflow_log", sa.JSON(), nullable=False, server_default="[]"),
    )

    # Final results
    op.add_column(
        "workflow_states", sa.Column("discovery_summary", sa.JSON(), nullable=True)
    )
    op.add_column(
        "workflow_states",
        sa.Column("assessment_flow_package", sa.JSON(), nullable=True),
    )
    ...
    op.add_column(
        "workflow_states",
        sa.Column(
            "database_assets_created", sa.JSON(), nullable=False, server_default="[]"
        ),
    )
    op.add_column(
        "workflow_states",
    ...
    op.add_column(
        "workflow_states",
        sa.Column(
            "learning_scope", sa.String(), nullable=False, server_default="'engagement'"
        ),
    )
    op.add_column(
        "workflow_states",
    ...
    op.add_column(
        "workflow_states", sa.Column("shared_memory_id", sa.String(), nullable=True)
    )
    ...
    op.create_index(
        "ix_workflow_states_current_phase", "workflow_states", ["current_phase"]
    )
    op.create_index(
        "ix_workflow_states_progress_percentage",
        "workflow_states",
        ["progress_percentage"],
    )
    op.create_index(
        "ix_workflow_states_database_integration_status",
        "workflow_states",
        ["database_integration_status"],
    )
    ...
    op.execute(
        "UPDATE migration.workflow_states SET flow_id = session_id WHERE flow_id IS NULL"
    )

    # Update existing records to have user_id = 'system' where user_id is null
    op.execute(
        "UPDATE migration.workflow_states SET user_id = 'system' WHERE user_id IS NULL"
    )
