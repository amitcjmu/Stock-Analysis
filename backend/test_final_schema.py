...
async def test_final_schema():
    ...
    async with AsyncSessionLocal() as session:
        ...
        critical_tables = [
            'client_accounts', 'users', 'engagements', 'user_account_associations',
            'discovery_flows', 'assets', 'data_imports', 'crewai_flow_state_extensions',
            'user_profiles', 'user_roles', 'client_access', 'engagement_access',
            'workflow_progress', 'enhanced_user_profiles', 'role_permissions',
            'soft_deleted_items'
        ]
        ...
        critical_asset_fields = [
            'flow_id', 'session_id', 'master_flow_id', 'discovery_flow_id',
            'name', 'description', 'business_owner', 'technical_owner',
            'six_r_strategy', 'migration_status', 'dependencies'
        ]
        ...
        critical_flow_fields = [
            'learning_scope', 'memory_isolation_level', 'assessment_ready',
            'phase_state', 'agent_state'
        ]
        ...
        critical_import_fields = [
            'source_system', 'error_message', 'error_details', 'failed_records'
        ]
        ...
        user_role_fields = [
            'role_name', 'description', 'scope_type', 'scope_client_id',
            'scope_engagement_id', 'assigned_at', 'assigned_by'
        ]
