...
class DiscoveryWorkflowDebugger:
        ...
        try:
            async with AsyncSessionLocal() as session:
                ...
                if ENGAGEMENT_AVAILABLE:
                    engagement_result = await session.execute(select(Engagement).limit(1))
                ...
                baseline = {
                    "client_account_id": str(client_account.id) if client_account else None,
                    ...
                    "recent_assets": [
                        {
                            ...
                            "discovery_status": getattr(asset, 'discovery_status', None),
                ...
                if recent_assets:
                    ...
                    print(f"   Sample Asset: {sample.name} ({sample.asset_type}) - {sample.discovery_method}")
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                ...
                with open(csv_path, 'rb') as f:
                    ...
                    data.add_field('file', f, filename='debug_test.csv', content_type='text/csv')
                    
                    async with session.post(f"{base_url}/api/v1/data-import/upload-csv", data=data) as response:
                        if response.status == 200:
                            ...
                            print(f"❌ CSV Upload failed: {response.status} - {error_text}")
    ...
    async def test_crewai_processing(self):
        ...
        try:
            ...
            import_session_id = self.test_results.get("csv_upload", {}).get("import_session_id")
            
            if not import_session_id:
                print("⚠️ No import session ID available, checking for recent imports...")
            ...
            async with aiohttp.ClientSession() as session:
                ...
                async with session.post(
                    ...
                    if response.status == 200:
                        ...
                        print(f"❌ CrewAI Processing failed: {response.status} - {error_text}")
        ...
        try:
            async with AsyncSessionLocal() as session:
                ...
                new_assets_result = await session.execute(
                    select(Asset).where(Asset.created_at >= ten_minutes_ago).order_by(Asset.created_at.desc())
                )
                new_assets = new_assets_result.scalars().all()
                
                # Get debug test assets specifically
                debug_assets_result = await session.execute(
                    select(Asset).where(Asset.name.like('debug-%')).order_by(Asset.created_at.desc())
                )
                ...
                if debug_assets:
                    for asset in debug_assets:
                        print(f"   Debug Asset: {asset.name} ({asset.asset_type}) - {asset.discovery_method} - {asset.created_at}")
                        ...
                        if response.status == 200:
                            try:
                                ...
                                if api["name"] == "discovery_metrics":
                                    ...
                                    print(f"   ✅ Discovery Metrics: {total_assets} assets, {total_apps} applications")
                                    ...
                                    print(f"   ✅ Applications List: {len(apps)} applications returned")
                                ...
                                api_results[api["name"]] = {
                                    "success": True,
                                    "status": response.status,
                                    "data": text_data[:200] + "..." if len(text_data) > 200 else text_data
                                }
                                print(f"   ✅ {api['name']}: Success (non-JSON response)")
                            ...
                            print(f"   ❌ {api['name']}: {response.status} - {error_text[:100]}")
    ...
    async def identify_workflow_gaps(self):
        ...
        crewai_success = self.test_results.get("crewai_processing", {}).get("success", False)
        ...
        assets_created = self.test_results.get("crewai_processing", {}).get("assets_created", 0)
        actual_assets_added = self.test_results.get("database_final", {}).get("assets_added", 0)
        ...
        api_discovery_metrics = self.test_results.get("api_tests", {}).get("discovery_metrics", {})
        ...
        if api_discovery_metrics.get("success") and api_assets_list.get("success"):
            api_asset_count = api_discovery_metrics.get("data", {}).get("totalAssets", 0)
            db_asset_count = self.test_results.get("database_final", {}).get("total_assets_after", 0)
    ...
    async def generate_diagnostic_report(self):
        ...
        try:
            ...
            csv_success = self.test_results.get("csv_upload", {}).get("success", False)
            crewai_success = self.test_results.get("crewai_processing", {}).get("success", False)
            api_count = len([api for api in self.test_results.get("api_tests", {}).values() if api.get("success")])
            ...
            if discovery_metrics_working:
                metrics_data = self.test_results.get("api_tests", {}).get("discovery_metrics", {}).get("data", {})
