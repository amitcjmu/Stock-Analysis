...
STRATEGY_RULES = {
    ...
    "database": {
        "strategies": [SixRStrategy.REPLATFORM, SixRStrategy.REHOST, SixRStrategy.REFACTOR],
        "rationale": "Database migration strategy depends on version and complexity"
    },
    "application": {
        "strategies": [SixRStrategy.REPLATFORM, SixRStrategy.REFACTOR, SixRStrategy.REHOST, SixRStrategy.REPLACE],
        "rationale": "Application strategy varies by technology stack and business criticality"
    },
    "server": {
        "strategies": [SixRStrategy.REHOST, SixRStrategy.REPLATFORM, SixRStrategy.RETIRE],
...
async def assign_6r_strategies_and_waves(session: AsyncSession):
    ...
    result = await session.execute(
        select(Asset).where(Asset.client_account_id == DEMO_CLIENT_ID)
    ...
    for i, asset in enumerate(assets):
        ...
        asset.custom_attributes.update({
            "migration_strategy_rationale": STRATEGY_RULES.get(asset.asset_type, {}).get("rationale", "Standard migration approach"),
            ...
            "risk_factors": [
                ...
                "High availability requirements" if asset.business_criticality == "Critical" else None,
                "Compliance requirements" if asset.custom_attributes.get("compliance_scope", "None") != "None" else None
            ]
        })
        
        # Remove None values from risk factors
        if asset.custom_attributes.get("risk_factors"):
            asset.custom_attributes["risk_factors"] = [rf for rf in asset.custom_attributes["risk_factors"] if rf is not None]
...
async def update_wave_statistics(session: AsyncSession, waves: list, wave_assignments: dict):
...
async def create_migration_planning():
    ...
    async with AsyncSessionLocal() as session:
        ...
        assets, strategy_counts, wave_assignments, readiness_counts = await assign_6r_strategies_and_waves(session)
