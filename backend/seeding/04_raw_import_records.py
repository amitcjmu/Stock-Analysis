...
def generate_server_record(row_num: int, is_valid: bool = True) -> dict:
    ...
    # Add some variation
    base_record["hostname"] = base_record["hostname"].replace(
        f"{row_num:03d}", f"{row_num:03d}"
    )
...
def generate_dependency_record(row_num: int, is_valid: bool = True) -> dict:
    ...
    if asset_type == "database":
        base_record.update(
            {
                "database_type": ["Oracle", "SQL Server", "PostgreSQL", "MySQL"][
                    row_num % 4
                ],
    ...
    async with AsyncSessionLocal() as session:
        ...
        imports_query = await session.execute(
            select(DataImport).where(
                DataImport.id.in_([csv_import_id, json_import_id, excel_import_id])
            )
        )
        ...
        for i in range(csv_dist["total_records"]):
            ...
            record = RawImportRecord(
                ...
                validation_errors=(
                    None
                    if is_valid
                    else ["Missing required fields", "Invalid data types"]
                ),
        ...
        for i in range(json_dist["total_records"]):
            ...
            record = RawImportRecord(
                ...
                validation_errors=(
                    None
                    if is_valid
                    else ["Invalid JSON structure", "Missing application name"]
                ),
        ...
        for i in range(excel_dist["total_records"]):
            ...
            record = RawImportRecord(
                ...
                validation_errors=(
                    None
                    if is_valid
                    else ["Missing dependency information", "Invalid asset type"]
                ),
        ...
        # Summary
        total_records = (
            csv_dist["total_records"]
            + json_dist["total_records"]
            + excel_dist["total_records"]
        )
        total_valid = (
            csv_dist["valid_records"]
            + json_dist["valid_records"]
            + excel_dist["valid_records"]
        )
        total_invalid = (
            csv_dist["invalid_records"]
            + json_dist["invalid_records"]
            + excel_dist["invalid_records"]
        )
