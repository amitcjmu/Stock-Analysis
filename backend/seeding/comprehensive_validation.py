...
async def validate_models_import():
    ...
    try:
        ...
        print(
            f"{Colors.GREEN}✅ AssetType enum values: {len(asset_types)} types{Colors.ENDC}"
        )

        asset_statuses = [e.value for e in AssetStatus]
        print(
            f"{Colors.GREEN}✅ AssetStatus enum values: {len(asset_statuses)} statuses{Colors.ENDC}"
        )

        sixr_strategies = [e.value for e in SixRStrategy]
        print(
            f"{Colors.GREEN}✅ SixRStrategy enum values: {len(sixr_strategies)} strategies{Colors.ENDC}"
        )
...
async def validate_multi_tenant_data():
    ...
    async with AsyncSessionLocal() as session:
        ...
        if client_count >= 4:
            print(
                f"{Colors.GREEN}✅ Client accounts: {client_count} (Expected: 4+){Colors.ENDC}"
            )
        else:
            print(
                f"{Colors.RED}❌ Client accounts: {client_count} (Expected: 4+){Colors.ENDC}"
            )
            return False
        ...
        for client in clients:
            ...
            print(
                f"{Colors.BLUE}   {client.name}: {eng_count} engagements{Colors.ENDC}"
            )

        # Validate user-client associations
        result = await session.execute(select(func.count(UserAccountAssociation.id)))
    ...
    async with AsyncSessionLocal() as session:
        ...
        for name, model, expected, operator in validations:
            ...
            if operator == "==" and count == expected:
                print(
                    f"{Colors.GREEN}✅ {name}: {count} (Expected: {expected}){Colors.ENDC}"
                )
                ...
                print(
                    f"{Colors.RED}❌ {name}: {count} (Expected: {operator} {expected}){Colors.ENDC}"
                )
                all_passed = False
    ...
    async with AsyncSessionLocal() as session:
        ...
        for asset_type, count in asset_type_counts.items():
            if asset_type in valid_types:
                ...
                print(
                    f"{Colors.RED}   ❌ {asset_type}: {count} assets (INVALID){Colors.ENDC}"
                )
                all_valid = False

        # Check migration statuses
        result = await session.execute(
            select(Asset.migration_status, func.count(Asset.id)).group_by(
        ...
        for status, count in status_counts.items():
            if status in valid_statuses:
                ...
                print(
                    f"{Colors.RED}   ❌ {status}: {count} assets (INVALID){Colors.ENDC}"
                )
                all_valid = False
        ...
        for strategy, count in strategy_counts.items():
            if strategy in valid_strategies:
                ...
                print(
                    f"{Colors.RED}   ❌ {strategy}: {count} assets (INVALID){Colors.ENDC}"
                )
                all_valid = False
...
async def validate_foreign_keys():
    ...
    async with AsyncSessionLocal() as session:
        ...
        if not orphaned_flows:
            print(
                f"{Colors.GREEN}✅ All discovery flows have valid client accounts{Colors.ENDC}"
            )
        else:
            print(
                f"{Colors.RED}❌ {len(orphaned_flows)} discovery flows with invalid client accounts{Colors.ENDC}"
            )

        # Check assets have valid flow IDs
        result = await session.execute(
            select(Asset)
            .where(Asset.flow_id.isnot(None))
            .outerjoin(DiscoveryFlow, Asset.flow_id == DiscoveryFlow.flow_id)
        ...
        result = await session.execute(
            ...
            .outerjoin(DataImport, ImportFieldMapping.data_import_id == DataImport.id)
        ...
        if not orphaned_mappings:
            print(
                f"{Colors.GREEN}✅ All field mappings have valid data imports{Colors.ENDC}"
            )
        else:
            print(
                f"{Colors.RED}❌ {len(orphaned_mappings)} field mappings with invalid imports{Colors.ENDC}"
            )

        return (
            len(orphaned_flows) == 0
            and len(orphaned_assets) == 0
            and len(orphaned_mappings) == 0
        )
...
async def validate_demo_scenarios():
    ...
    async with AsyncSessionLocal() as session:
        ...
        if complete_flows:
            print(
                f"{Colors.GREEN}✅ Complete discovery flow ready for assessment{Colors.ENDC}"
            )
        else:
        ...
        result = await session.execute(
            select(ImportFieldMapping).where(ImportFieldMapping.status == "pending")
        )
        pending_mappings = result.scalars().all()

        if pending_mappings:
            print(
                f"{Colors.GREEN}✅ Field mappings pending approval: {len(pending_mappings)}{Colors.ENDC}"
            )
        else:
            print(
                f"{Colors.YELLOW}⚠️ No pending field mappings for approval demo{Colors.ENDC}"
            )

        # Scenario 3: Migration waves defined
        result = await session.execute(
            select(MigrationWave).order_by(MigrationWave.wave_number)
        )
        ...
        if len(waves) >= 4:
            ...
            print(
                f"{Colors.RED}❌ Insufficient migration waves: {len(waves)}{Colors.ENDC}"
            )
            scenarios_passed = False
        ...
        if multi_client_users:
            print(
                f"{Colors.GREEN}✅ Users with multi-client access: {len(multi_client_users)}{Colors.ENDC}"
            )
        else:
            print(
                f"{Colors.YELLOW}⚠️ No users with multi-client access (OK for demo){Colors.ENDC}"
            )
