        ...
        try:
            ...
            """), {
                ...
                "permissions": json.dumps({
                    ...
                    "can_manage_users": user_data["role"] in ["ENGAGEMENT_MANAGER", "CLIENT_ADMIN"],
...
async def seed_discovery_flows(db: AsyncSession) -> list[dict]:
    ...
    for i, flow_data in enumerate(FLOWS):
        try:
            ...
            print(f"  ✓ Created flow: {flow_data['name']} ({flow_data['state']}, {flow_data['progress']}%)")
...
async def seed_data_imports(db: AsyncSession) -> list[dict]:
    ...
    for import_data in IMPORTS:
        try:
            ...
            print(f"  ✓ Created import: {import_data['filename']} ({import_data['status']})")
...
async def generate_seeded_ids_json(users: list[dict], flows: list[dict], imports: list[dict]):
    ...
    async with AsyncSessionLocal() as db:
        try:
            ...
            result = await db.execute(text(
                "SELECT name FROM client_accounts WHERE id = :id"
            ), {"id": DEMO_CLIENT_ID})
            client = result.first()
            ...
            result = await db.execute(text(
                "SELECT name FROM engagements WHERE id = :id"
