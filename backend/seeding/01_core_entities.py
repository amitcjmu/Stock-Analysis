...
from app.models.rbac import UserRole, RoleType, AccessLevel, UserStatus
...
async def create_engagement(db: AsyncSession, client_id: str) -> Engagement:
    ...
    engagement = Engagement(
        ...
        engagement_lead_id=None,  # Will be set after users are created
...
async def update_engagement_lead(db: AsyncSession, engagement_id: str):
    """Update engagement lead after users are created."""
    print("Updating engagement lead...")
    
    # Get the engagement and set the lead
    engagement = await db.get(Engagement, engagement_id)
    if engagement:
        engagement.engagement_lead_id = USER_IDS["engagement_manager"]
        await db.commit()
        print("✓ Updated engagement lead")
    else:
        print("⚠️  Engagement not found for lead update")
...
async def main():
    ...
    async with AsyncSessionLocal() as db:
        try:
            ...
            engagement = await create_engagement(db, client.id)  # Create engagement before users
            users = await create_users(db)  # Create users after engagement (for FK references)
            await update_users_default_engagement(db, engagement.id)  # Update users after engagement
            await update_engagement_lead(db, engagement.id)  # Update engagement lead after users
