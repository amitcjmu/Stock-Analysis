...
async def get_assets_by_type_raw(session: AsyncSession, asset_type: str) -> list:
    ...
    result = await session.execute(
        text(sql), {"client_account_id": str(DEMO_CLIENT_ID), "asset_type": asset_type}
    )
    return result.fetchall()


async def insert_dependency_raw_sql(
...
async def update_asset_dependencies_json(
    session: AsyncSession, asset_id: str, dependencies: list
):
    ...
    """
    await session.execute(
        text(sql), {"asset_id": asset_id, "dependencies": json.dumps(dependencies)}
    )


async def create_application_dependencies(
    session: AsyncSession,
    applications: list,
    servers: list,
    databases: list,
    ...
    for i, app in enumerate(applications):
        ...
        app_servers = (
            servers[i : i + server_count]
            if i + server_count <= len(servers)
            else servers[i : i + 1]
        )
        ...
        app_databases = (
            databases[i : i + db_count]
            if i + db_count <= len(databases)
            else databases[i : i + 1]
        )
...
async def create_cross_application_dependencies(
    session: AsyncSession, applications: list
):
    ...
    if len(applications) >= 4:
        ...
        for app, target, dep_type in [
            (app1, app2, "api_integration"),
            (app2, app3, "data_feed"),
            (app3, app1, "authentication"),
        ]:
        ...
        print(
            f"    ⚠️ Created circular dependency: {app1.name} -> {app2.name} -> {app3.name} -> {app1.name}"
        )
    ...
    if len(applications) >= 6:
        ...
        print(
            f"    ⚠️ Created circular dependency: {app4.name} -> {app5.name} -> {app4.name}"
        )

    return dependencies


async def create_infrastructure_dependencies(
    session: AsyncSession, servers: list, databases: list, network_devices: list
):
    ...
    for i, database in enumerate(databases):
        if i < len(servers):
            ...
            await update_asset_dependencies_json(
                session,
                database.id,
                [
                    {
    ...
    for i, server in enumerate(servers):
        ...
        if i % 3 == 0 and i // 3 < len(network_devices):
            ...
            dep_type = (
                "firewall_rule"
                if "firewall" in network_device.name.lower()
                else "load_balancing"
            )
...
async def create_dependencies_raw_sql():
    ...
    async with AsyncSessionLocal() as session:
        ...
        """
        result = await session.execute(
            text(network_sql), {"client_account_id": str(DEMO_CLIENT_ID)}
        )
        network_devices = result.fetchall()

        # Get load balancers specifically
        load_balancers = [
            asset for asset in network_devices if asset.asset_type == "load_balancer"
        ]
        ...
        app_deps = await create_application_dependencies(
            session, applications, servers, databases, load_balancers
        )
        ...
        infra_deps = await create_infrastructure_dependencies(
            session, servers, databases, network_devices
        )
        ...
        for i, app in enumerate(applications[:3]):
            ...
            if deps_data and deps_data[0]:
                ...
                dep_names = [
                    f"{dep['asset_name']} ({dep['dependency_type']})" for dep in deps
                ]
                circular_note = (
                    " [CIRCULAR]"
                    if any(dep.get("circular_risk") for dep in deps)
                    else ""
                )
