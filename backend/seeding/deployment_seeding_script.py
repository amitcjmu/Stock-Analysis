...
class DeploymentSeeder:
        ...
        async with AsyncSessionLocal() as session:
            ...
            for i, flow_data in enumerate(flows):
                flow = DiscoveryFlow(
                    ...
                    crewai_state_data=generate_crewai_state_data(flow_data["status"], flow_data["progress_percentage"]),
                    **flow_data
                )
                session.add(flow)
    ...
    async def verify_deployment(self):
        ...
        async with AsyncSessionLocal() as session:
            # Count records
            counts = {
                "Client Accounts": await session.scalar(select(func.count(ClientAccount.id))),
                "Users": await session.scalar(select(func.count(User.id))),
                "Discovery Flows": await session.scalar(select(func.count(DiscoveryFlow.id))),
                "Data Imports": await session.scalar(select(func.count(DataImport.id))),
                "Assets": await session.scalar(select(func.count(Asset.id))),
                "Field Mappings": await session.scalar(select(func.count(ImportFieldMapping.id)))
            }
