...
async def get_assets_by_type(session: AsyncSession, asset_type: str) -> list:
    ...
    result = await session.execute(
        select(Asset)
        .where(
            Asset.client_account_id == DEMO_CLIENT_ID, Asset.asset_type == asset_type
        )
        .order_by(Asset.name)
    )
...
async def create_dependencies():
    ...
    async with AsyncSessionLocal() as session:
        ...
        print("\n   🔗 Dependency Summary:")
        print(
            f"     Applications with dependencies: {len([app for app in applications if app.dependencies])}"
        )
        print(
            f"     Servers with dependencies: {len([srv for srv in servers if srv.dependencies])}"
        )
        print(
            f"     Databases with dependencies: {len([db for db in databases if db.dependencies])}"
        )
        ...
        for i, app in enumerate(applications[:3]):
            if app.dependencies:
                deps = [
                    f"{dep['asset_name']} ({dep['dependency_type']})"
                    for dep in app.dependencies
                ]
                circular_note = (
                    " [CIRCULAR]"
                    if any(dep.get("circular_risk") for dep in app.dependencies)
                    else ""
                )
