...
def generate_raw_records(sample_data: list[dict], total_rows: int) -> dict:
    ...
    for i in range(total_rows):
        ...
        for key in record:
            if key.endswith("_id"):
                base_id = (
                    record[key].split("_")[0] if "_" in record[key] else record[key][:3]
                )
...
async def create_data_imports(db: AsyncSession) -> list[DataImport]:
    ...
    for import_data in IMPORTS:
        ...
        generate_sample_data(
            import_data["import_type"], min(import_data["total_rows"], 10)
        )
        ...
        data_import = DataImport(
            ...
            progress_percentage=import_data["processed_rows"]
            / import_data["total_rows"],
            # User context
            imported_by=import_data["created_by"],
            # Error handling for failed imports
            error_message=(
                "Data validation failed" if import_data["failed_rows"] > 0 else None
            ),
            error_details=(
                {
                ...
                if import_data["failed_rows"] > 0
                else None
            ),
            # Timestamps
            started_at=created_at,
            completed_at=(
                updated_at if import_data["status"] in ["completed", "failed"] else None
            ),
        )
...
async def generate_seeded_ids_json():
    ...
    seeded_data = {
        "client_account": {"id": str(DEMO_CLIENT_ID), "name": "DemoCorp International"},
        "engagement": {
...
async def main():
    ...
    async with AsyncSessionLocal() as db:
        try:
            ...
            for imp in imports:
                print(
                    f"   - {imp.filename} ({imp.status}, {imp.processed_records}/{imp.total_records} rows)"
                )
