...
async def create_engagement(
    db: AsyncSession, client_id: str, engagement_lead_id: str = None
) -> Engagement:
...
async def create_rbac_setup(db: AsyncSession, users: list[User]):
    ...
    for i, user_data in enumerate(USERS):
        ...
        engagement_access = EngagementAccess(
            ...
            access_level=(
                "ADMIN" if user_data["role"] == "ENGAGEMENT_MANAGER" else "READ_WRITE"
            ),
    ...
    async with AsyncSessionLocal() as db:
        try:
            ...
            engagement = await create_engagement(
                db, client.id, users[0].id
            )  # Use first user as lead
