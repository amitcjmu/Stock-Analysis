        ...
        try:
            ...
            if flow_db_id:
                ...
                import_result = await db.execute(
                    import_query, {"flow_db_id": flow_db_id}
                )
                ...
                if data_import:
                    ...
                    records_result = await db.execute(
                        records_query, {"import_id": data_import.id}
                    )
                    ...
                    if record_count and record_count > 0:
                        logger.info(
                            f"🎉 SUCCESS: Found {record_count} records for flow {FLOW_ID}"
                        )
                        logger.info(
                            "    This should resolve the '0 records' issue in the data cleansing page!"
                        )
                        return True
                    else:
