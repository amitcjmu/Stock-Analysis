...
class DatabaseTracker:
    ...
    async def get_raw_records_sample(self, data_import_id: str, limit: int = 2) -> List[Dict[str, Any]]:
        ...
        async with AsyncSessionLocal() as db:
            ...
            for idx, record in enumerate(SAMPLE_CSV_DATA):
                raw_record = RawImportRecord(
                    ...
                    client_account_id=int(test_context.client_account_id) if test_context.client_account_id.isdigit() else 11111111
                )
                db.add(raw_record)
            ...
            for table, count in post_mapping_counts.items():
                if count != post_import_counts.get(table):
                    logger.info(f"  - {table}: {post_import_counts.get(table)} → {count}")
            ...
            if import_info and import_info.get('import_id'):
                raw_records = await tracker.get_raw_records_sample(import_info['import_id'])
                logger.info(f"📊 Sample raw records: {json.dumps(raw_records, indent=2)}")
            ...
            if result.phase.value == "field_mapping_approval" and result.requires_user_input:
                ...
                if import_info:
                        ...
                        if source != "confidence_scores":
                            await db.execute(
                                ...
                                """),
                                {
                                    ...
                                    "confidence": field_mappings.get("confidence_scores", {}).get(source, 0.8),
                                    "approved": False,
                                    "created_at": datetime.utcnow()
                                }
                            )
                    ...
                    await db.commit()
                    logger.info(f"✅ Created {len(field_mappings) - 1} field mapping records")
                
                # Check field mappings in DB
                if import_info:
                    db_mappings = await tracker.get_field_mappings(import_info['import_id'])
                    logger.info(f"📊 Field mappings in DB: {json.dumps(db_mappings, indent=2)}")
                ...
                for table, count in final_counts.items():
                    ...
                    if count != initial:
                        logger.info(f"  - {table}: {initial} → {count} (+{count - initial})")
                ...
                logger.info(f"📊 Sample final assets: {json.dumps(final_assets, indent=2)}")
                ...
                logger.info(f"📊 Final discovery flow state: {json.dumps(final_disc_state, indent=2)}")
