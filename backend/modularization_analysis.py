...
class ModularizationAnalyzer:
    ...
    def calculate_complexity_score(self, file_path: Path, code_lines: int) -> int:
        ...
        try:
            ...
            for line in content.split("\n"):
                ...
                if any(
                    keyword in stripped
                    for keyword in ["if ", "for ", "while ", "def ", "class ", "try:"]
                ):
                    nesting_level += 1
    ...
    def generate_refactoring_suggestions(
        self, file_path: Path, metrics: Dict
    ) -> List[str]:
        ...
        if code_lines > 500:
            suggestions.append(
                "CRITICAL: Split into multiple smaller modules (target <200 LOC each)"
            )
            ...
            suggestions.append(
                "MEDIUM: Consider extracting reusable components or utilities"
            )
    ...
    def get_typescript_suggestions(self, file_path: Path, code_lines: int) -> List[str]:
        ...
        try:
            ...
            if (
                "return (" in content
                and len(content.split("return (")[1].split(")")[0]) > 1000
            ):
    ...
    def analyze_file(self, file_path: Path) -> FileMetrics:
        ...
        total_lines, blank_lines, comment_lines, code_lines = self.count_lines(
            file_path
        )
    ...
    def generate_report(self) -> Dict[str, Any]:
        ...
        oversized_files = [
            f for f in self.file_metrics if f.code_lines >= self.thresholds["medium"]
        ]
        ...
        compliance_rate = (
            ((total_files - oversized_count) / total_files * 100)
            if total_files > 0
            else 100
        )
        ...
        top_offenders = sorted(
            oversized_files, key=lambda x: x.code_lines, reverse=True
        )[:20]
        ...
        report = {
            ...
            "by_file_type": {
                file_type: {
                    ...
                    "oversized_files": len(
                        [f for f in files if f.code_lines >= self.thresholds["medium"]]
                    ),
                    "avg_code_lines": (
                        round(sum(f.code_lines for f in files) / len(files), 2)
                        if files
                        else 0
                    ),
    ...
    def save_report(
        self,
        report: Dict[str, Any],
        output_file: str = "modularization_analysis_report.json",
    ):
...
def main():
    ...
    analyzer = ModularizationAnalyzer(
    ...
    print(
        f"Files exceeding 350 LOC threshold: {report['summary']['files_exceeding_threshold']}"
    )
    ...
    for i, file_info in enumerate(report["top_offenders"][:10], 1):
        print(
            f"  {i:2d}. {file_info['path']} ({file_info['code_lines']} LOC, {file_info['priority']} priority)"
        )
