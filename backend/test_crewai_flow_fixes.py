...
def test_unified_discovery_flow_state_completed_phases():
    ...
    assert (
        len(completed_phases) == 2
    ), f"Expected 2 completed phases, got {len(completed_phases)}"

    for phase in expected_completed:
        assert (
            phase in completed_phases
        ), f"Phase '{phase}' should be in completed_phases"

    logger.info(
        "✅ UnifiedDiscoveryFlowState.completed_phases property works correctly"
    )
    return True
...
async def test_phase_transition_agent_decision_methods():
    ...
    assert hasattr(
        agent, "get_decision"
    ), "PhaseTransitionAgent should have get_decision method"
    ...
    assert hasattr(
        agent, "get_post_execution_decision"
    ), "PhaseTransitionAgent should have get_post_execution_decision method"
    assert callable(
        getattr(agent, "get_post_execution_decision")
    ), "get_post_execution_decision should be callable"
    ...
    try:
        ...
        logger.info(
            f"✅ get_decision returned: {decision.action.value} with confidence {decision.confidence}"
        )
    ...
    try:
        ...
        ), "get_post_execution_decision should return a decision"
...
async def test_crewai_flow_service_execution_methods():
    ...
    for method_name in required_methods:
        assert hasattr(
            service, method_name
        ), f"CrewAIFlowService should have {method_name} method"
        assert callable(
    ...
    try:
        ...
        assert isinstance(
            result, dict
        ), "execute_data_import_validation should return a dict"
        ...
        assert isinstance(
            mapping_result, dict
        ), "generate_field_mapping_suggestions should return a dict"
        assert (
            mapping_result.get("status") == "completed"
        ), "Mapping suggestions should complete successfully"

        logger.info(
            f"✅ generate_field_mapping_suggestions returned: {mapping_result['status']}"
        )
...
def test_phase_name_mapping():
    ...
    for mapped_phase, method_name in service_method_map.items():
        assert hasattr(
            service, method_name
        ), f"CrewAIFlowService should have method '{method_name}' for phase '{mapped_phase}'"
        logger.info(
            f"✅ Mapped phase '{mapped_phase}' has corresponding method '{method_name}'"
        )
...
async def run_all_tests():
    ...
    tests = [
        (
            "UnifiedDiscoveryFlowState completed_phases",
            test_unified_discovery_flow_state_completed_phases(),
        ),
        (
            "PhaseTransitionAgent decision methods",
            test_phase_transition_agent_decision_methods(),
        ),
        (
            "CrewAIFlowService execution methods",
            test_crewai_flow_service_execution_methods(),
        ),
    ...
    if failed == 0:
        logger.info(
            "🎉 ALL TESTS PASSED! CrewAI flow execution fixes are working correctly."
        )
        return True
    else:
