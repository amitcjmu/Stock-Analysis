    ...
    def analyze_log_line(self, line):
        ...
        for pattern_name, pattern in ERROR_PATTERNS.items():
            if re.search(pattern, line, re.IGNORECASE):
                ...
                # Extract log level
                level_match = re.search(r' - (INFO|WARNING|ERROR|CRITICAL|DEBUG) - ', line)
        ...
        try:
            for line in process.stdout:
                ...
                for error in errors:
                    ...
                    if error['level'] == 'INFO' and error['pattern'] not in ['FAILED', 'EXCEPTION']:
                        continue
                    ...
                    if error['level'] in ['ERROR', 'CRITICAL']:
                        ...
                        if 'UUID is not JSON serializable' in error['message']:
                            ...
                            print("🔧 LIKELY FIX: Convert UUID to string before JSON serialization")
                            ...
                            print("🔧 LIKELY FIX: Ensure X-Client-Account-Id header is included")
            ...
            print("\n" + "=" * 80)
            print("📊 ERROR SUMMARY:")
            for error_key, count in sorted(self.error_count.items(), key=lambda x: x[1], reverse=True):
