                ...
                content=json.dumps(
                    {
                        ...
                        "suggestions": (
                            ["Consider edge cases"] if confidence < 0.8 else []
                        ),
                    }
                )
            )
    ...
    for scenario in scenarios:
        ...
        try:
            ...
            mock_client = create_mock_openai_client(
                decision=scenario["decision"], confidence=scenario["confidence"]
            )
            ...
            with patch(
                ...
                print(
                    f"  ✅ Agent decided: {result['decision']} (confidence: {result['confidence']})"
                )
...
async def test_sse_updates():
    ...
    for event in sse_events:
        print(
            f"  📡 Event: {event['current_phase']} - {event['phases'][event['current_phase']]['progress']}%"
        )
