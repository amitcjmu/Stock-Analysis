...
def upgrade():
    ...
    try:
        op.add_column(
            "assets",
            sa.Column(
                "flow_id", postgresql.UUID(as_uuid=True), nullable=True, index=True
            ),
        )
    ...
    for session in orphaned_sessions:
        ...
        """
            ...
            {"session_id": session.id, "flow_id": new_flow_id},
        )
    ...
    print(
        f"✅ Updated {assets_no_session} assets without session_id using discovery_flows"
    )
    ...
    try:
        op.create_index(
            "idx_flow_deletion_audit_flow_id", "flow_deletion_audit", ["flow_id"]
        )
    ...
    total_sessions = (
        conn.execute(text("SELECT COUNT(*) as count FROM data_import_sessions"))
        .fetchone()
        .count
    )
    total_mappings = (
        conn.execute(text("SELECT COUNT(*) as count FROM session_flow_mapping"))
        .fetchone()
        .count
    )
...
def downgrade():
    ...
    try:
        op.drop_constraint(
            "fk_flow_deletion_audit_flow_id", "flow_deletion_audit", type_="foreignkey"
        )
    ...
    try:
        op.drop_index(
            "idx_flow_deletion_audit_flow_id", table_name="flow_deletion_audit"
        )
