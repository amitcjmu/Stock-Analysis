...
def upgrade():
    ...
    for table in tables_with_is_mock:
        if table_exists(connection, table) and column_exists(connection, table, 'is_mock'):
...
def downgrade():
    ...
    raise NotImplementedError("Downgrade not supported - legacy schema elements should not be restored")