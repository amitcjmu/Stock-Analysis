...
def rename_fields():
    ...
    if column_exists('data_imports', 'source_filename') and not column_exists('data_imports', 'filename'):
    ...
    if column_exists('data_imports', 'file_size_bytes') and not column_exists('data_imports', 'file_size'):
    ...
    if column_exists('data_imports', 'file_type') and not column_exists('data_imports', 'mime_type'):
    ...
    if column_exists('discovery_flows', 'attribute_mapping_completed') and not column_exists('discovery_flows', 'field_mapping_completed'):
        op.alter_column('discovery_flows', 'attribute_mapping_completed', new_column_name='field_mapping_completed')
        logger.info("Renamed discovery_flows.attribute_mapping_completed to field_mapping_completed")
    
    if column_exists('discovery_flows', 'inventory_completed') and not column_exists('discovery_flows', 'asset_inventory_completed'):
        op.alter_column('discovery_flows', 'inventory_completed', new_column_name='asset_inventory_completed')
        logger.info("Renamed discovery_flows.inventory_completed to asset_inventory_completed")
    
    if column_exists('discovery_flows', 'dependencies_completed') and not column_exists('discovery_flows', 'dependency_analysis_completed'):
        op.alter_column('discovery_flows', 'dependencies_completed', new_column_name='dependency_analysis_completed')
        logger.info("Renamed discovery_flows.dependencies_completed to dependency_analysis_completed")
    
    if column_exists('discovery_flows', 'tech_debt_completed') and not column_exists('discovery_flows', 'tech_debt_assessment_completed'):
        op.alter_column('discovery_flows', 'tech_debt_completed', new_column_name='tech_debt_assessment_completed')
        logger.info("Renamed discovery_flows.tech_debt_completed to tech_debt_assessment_completed")
    
    # assets table field renames
    if column_exists('assets', 'memory_mb') and not column_exists('assets', 'memory_gb'):
...
def add_new_columns():
    ...
    columns_to_add = [
        ('discovery_flows', 'flow_state', sa.Column('flow_state', postgresql.JSONB, nullable=False, server_default='{}')),
        ('discovery_flows', 'phase_state', sa.Column('phase_state', postgresql.JSONB, nullable=False, server_default='{}')),
        ('discovery_flows', 'agent_state', sa.Column('agent_state', postgresql.JSONB, nullable=False, server_default='{}')),
        ('discovery_flows', 'error_message', sa.Column('error_message', sa.Text, nullable=True)),
        ('discovery_flows', 'error_phase', sa.Column('error_phase', sa.String(100), nullable=True)),
        ('discovery_flows', 'error_details', sa.Column('error_details', postgresql.JSON, nullable=True)),
        ('data_imports', 'source_system', sa.Column('source_system', sa.String(100), nullable=True))
    ]
...
def create_indexes():
    ...
    multi_tenant_indexes = [
        ...
        ('idx_discovery_flows_client_account', 'discovery_flows', ['client_account_id']),
    ...
    for index_name, table, columns in multi_tenant_indexes:
        ...
        if all_columns_exist and not index_exists(index_name, table):
            ...
            logger.warning(f"Skipping index {index_name} - not all columns exist in {table}")
...
def revert_field_renames():
    ...
    with op.batch_alter_table('discovery_flows') as batch_op:
        try:
            batch_op.alter_column('field_mapping_completed', new_column_name='attribute_mapping_completed')
            batch_op.alter_column('asset_inventory_completed', new_column_name='inventory_completed')
            batch_op.alter_column('dependency_analysis_completed', new_column_name='dependencies_completed')
            batch_op.alter_column('tech_debt_assessment_completed', new_column_name='tech_debt_completed')
        except Exception as e:
...
def recreate_deprecated_columns():
    ...
    for table in tables_with_is_mock:
        try:
            with op.batch_alter_table(table) as batch_op:
                batch_op.add_column(sa.Column('is_mock', sa.Boolean, nullable=False, server_default='false'))
