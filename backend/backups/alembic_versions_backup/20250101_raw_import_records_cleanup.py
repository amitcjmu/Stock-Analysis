...
def downgrade():
    ...
    if column_exists("raw_import_records", "record_index") and not column_exists(
        "raw_import_records", "row_number"
    ):
        op.alter_column(
            "raw_import_records", "record_index", new_column_name="row_number"
        )
        logger.info("Reverted raw_import_records.record_index to row_number")

    if column_exists("raw_import_records", "cleansed_data") and not column_exists(
        "raw_import_records", "processed_data"
    ):
        op.alter_column(
            "raw_import_records", "cleansed_data", new_column_name="processed_data"
        )
