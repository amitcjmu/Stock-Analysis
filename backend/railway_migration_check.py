...
async def check_database_schema_state() -> Dict[str, Any]:
    ...
    try:
        async with engine.begin() as conn:
            ...
            try:
                alembic_versions = await conn.execute(sa.text(
                    "SELECT version_num FROM alembic_version ORDER BY version_num"
                ))
                results['alembic_versions'] = [row.version_num for row in alembic_versions.fetchall()]
            except Exception as e:
                results['alembic_versions'] = f"ERROR: {e}"
            
            # 2. Check crewai_flow_state_extensions table structure
            try:
                crewai_columns = await conn.execute(sa.text("""
            ...
            try:
                discovery_columns = await conn.execute(sa.text("""
            ...
            try:
                assets_flow_id = await conn.execute(sa.text("""
                    SELECT column_name, data_type, is_nullable
                    FROM information_schema.columns 
            ...
            try:
                constraints = await conn.execute(sa.text("""
                    SELECT 
                        tc.table_name,
                        tc.constraint_name,
                        tc.constraint_type,
            ...
            try:
                ...
                crewai_count = await conn.execute(sa.text(
                    "SELECT COUNT(*) as count FROM crewai_flow_state_extensions"
                ))
                discovery_count = await conn.execute(sa.text(
                    "SELECT COUNT(*) as count FROM discovery_flows"
                ))
                results['data_counts'] = {
...
def analyze_migration_safety(schema_state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if isinstance(schema_state.get('crewai_flow_state_extensions'), list):
        crewai_columns = [col['column_name'] for col in schema_state['crewai_flow_state_extensions']]
        required_columns = ['client_account_id', 'engagement_id', 'user_id', 'flow_type', 'flow_status']
        
        missing_columns = [col for col in required_columns if col not in crewai_columns]
        if missing_columns:
            analysis['issues'].append(f"crewai_flow_state_extensions missing columns: {missing_columns}")
    ...
    if isinstance(data_counts, dict):
        ...
        if discovery_count > 0 and crewai_count == 0:
            analysis['issues'].append(f"Data inconsistency: {discovery_count} discovery flows but 0 crewai extensions")
            analysis['recommendations'].append("Run orphan flow migration to fix missing extensions")
...
async def main():
    ...
    # Alembic versions
    print(f"📋 Alembic Versions Applied: {schema_state.get('alembic_versions', 'ERROR')}")
    ...
    if isinstance(crewai_cols, list):
        ...
        required_cols = ['client_account_id', 'engagement_id', 'user_id', 'flow_type', 'flow_status']
