...
async def test_collection_flow():
    ...
    try:
        ...
        logger.info(
            f"✅ Collection Flow registered with {collection_details['phases']} phases"
        )
        ...
        if actual_phases != expected_phases:
            logger.error(
                f"❌ Phase mismatch! Expected: {expected_phases}, Got: {actual_phases}"
            )
            return False
        ...
        quality_thresholds = collection_config.phases[1].metadata.get(
            "quality_thresholds", {}
        )
        ...
        supported_integrations = collection_config.metadata.get(
        ...
        collection_summary = next(
            (f for f in flow_summary if f["name"] == "collection"), None
        )
        ...
        if collection_config.error_handler:
            logger.info(
                f"✅ Error handler configured: {collection_config.error_handler}"
            )
