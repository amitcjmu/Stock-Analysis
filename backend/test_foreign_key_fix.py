            ...
            if result.success:
                ...
                async with AsyncSessionLocal() as verify_db:
                    ...
                    check_master_flow = await verify_db.execute(
                        text("SELECT flow_id, flow_status FROM crewai_flow_state_extensions WHERE flow_id = :flow_id"),
                        {"flow_id": result.flow_id}
                    )
                    master_flow = check_master_flow.first()
                
                    if master_flow:
                        logger.info(f"✅ Master flow found: {master_flow.flow_id} (status: {master_flow.flow_status})")
                    else:
                    ...
                    check_data_import = await verify_db.execute(
                        text("SELECT id, master_flow_id FROM data_imports WHERE master_flow_id = :flow_id"),
                        {"flow_id": result.flow_id}
                    )
                    data_imports = check_data_import.fetchall()
                    
                    if data_imports:
                        logger.info(f"✅ Data import linkage successful: {len(data_imports)} records linked")
                        for di in data_imports:
                            logger.info(f"  - DataImport {di.id} → MasterFlow {di.master_flow_id}")
                    else:
                    ...
                    check_field_mappings = await verify_db.execute(
                        text("SELECT COUNT(*) as count FROM import_field_mappings WHERE master_flow_id = :flow_id"),
                        {"flow_id": result.flow_id}
                    )
                    field_mapping_count = check_field_mappings.scalar()
                    
                    if field_mapping_count and field_mapping_count > 0:
                        logger.info(f"✅ Field mappings linkage successful: {field_mapping_count} mappings linked")
                    else:
                    ...
                    check_raw_records = await verify_db.execute(
                        text("SELECT COUNT(*) as count FROM raw_import_records WHERE master_flow_id = :flow_id"),
                        {"flow_id": result.flow_id}
                    )
                    raw_record_count = check_raw_records.scalar()
                    
                    if raw_record_count and raw_record_count > 0:
                        logger.info(f"✅ Raw import records linkage successful: {raw_record_count} records linked")
                    else:
