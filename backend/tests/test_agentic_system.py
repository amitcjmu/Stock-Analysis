...
async def test_agentic_system():
    ...
    try:
        ...
        print(
            f"   Asset Type Detected: {analysis_result.get('asset_type_detected', 'unknown')}"
        )
        ...
        print(
            f"   Missing Fields: {analysis_result.get('missing_fields_relevant', [])}"
        )
    ...
    try:
        ...
        print(
            f"   Patterns Identified: {len(learning_result.get('patterns_identified', []))}"
        )
        ...
        if "corrected_analysis" in learning_result:
            ...
            print(
                f"   Relevant Missing Fields: {corrected.get('relevant_missing_fields', [])}"
            )
    ...
    memory_stats = {
        ...
        "analysis_attempts": len(
            service.memory.experiences.get("analysis_attempt", [])
        ),
    ...
    try:
        ...
        print(
            f"   Asset Type Detected: {second_analysis.get('asset_type_detected', 'unknown')}"
        )
        ...
        if improvement > 0:
            print(
                f"   🎯 Confidence Improved by: {improvement:.2f} ({improvement*100:.1f}%)"
            )
        else:
