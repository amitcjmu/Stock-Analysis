        ...
        async with AsyncSessionLocal() as db:
            ...
            if demo_client:
                ...
                access_result = await db.execute(
                    select(ClientAccess)
                    .where(ClientAccess.client_account_id == demo_client.id)
                    .limit(5)
                )
                ...
                for access in accesses:
                    ...
                    if user_profile:
                        print(
                            f"  - User: {user_profile.email}, Access Level: {access.access_level}, Active: {access.is_active}"
                        )
                ...
                if demo_engagement:
                    ...
                    eng_access_result = await db.execute(
                        select(EngagementAccess)
                        .where(EngagementAccess.engagement_id == demo_engagement.id)
                        .limit(5)
                    )
                    ...
                    for access in eng_accesses:
                        ...
                        if user_profile:
                            print(
                                f"  - User: {user_profile.email}, Role: {access.engagement_role}, Active: {access.is_active}"
                            )
                else:
