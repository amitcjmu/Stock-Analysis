...
async def test_rbac_auto_mapping():
    ...
    async with AsyncSessionLocal() as db:
        ...
        test_email = f"test_rbac_{datetime.now().strftime('%Y%m%d_%H%M%S')}@demo-corp.com"
        ...
        if pending_client_access.is_active:
            ...
            print(f"   - Permissions: {json.dumps(pending_client_access.permissions, indent=2)}")
        else:
        ...
        if pending_engagement_access.is_active:
            ...
            print(f"   - Permissions: {json.dumps(pending_engagement_access.permissions, indent=2)}")
        else:
        ...
        admin_test_email = f"admin_test_{datetime.now().strftime('%Y%m%d_%H%M%S')}@demo-corp.com"
        ...
        if admin_result["status"] != "success":
            ...
            if admin_engagement_access and admin_engagement_access.is_active:
                print("✅ EngagementAccess auto-created and active for admin-created user")
            else:
