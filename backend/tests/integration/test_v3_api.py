...
class TestV3DiscoveryFlowAPI:
    ...
    @pytest.mark.asyncio
    async def test_create_flow(self, async_client: AsyncClient, auth_headers, sample_flow_data):
        """Test creating a new flow"""
        with patch('app.services.crewai_flows.unified_discovery_flow.UnifiedDiscoveryFlow') as mock_flow_class:
    ...
    async def test_get_flow_status(self, async_client: AsyncClient, auth_headers):
        ...
        with patch('app.repositories.discovery_v2_repository.DiscoveryV2Repository') as mock_repo_class:
            mock_repo = AsyncMock()
            ...
            response = await async_client.get(
                f"/api/v3/discovery-flow/flows/{flow_id}",
                headers=auth_headers
            )
        ...
        with patch('app.repositories.discovery_v2_repository.DiscoveryV2Repository') as mock_repo_class:
            ...
            mock_flows = [
                WorkflowState(
                    ...
                    phases_completed=["initialization", "attribute_mapping", "data_cleansing"]
                ),
                WorkflowState(
                    id=str(uuid.uuid4()),
                    flow_id=str(uuid.uuid4()),
    ...
    @pytest.mark.asyncio
    async def test_flow_lifecycle(self, async_client: AsyncClient, auth_headers, sample_flow_data):
    ...
    @pytest.mark.asyncio
    async def test_error_handling_invalid_flow_id(self, async_client: AsyncClient, auth_headers):
        ...
        with patch('app.repositories.discovery_v2_repository.DiscoveryV2Repository') as mock_repo_class:
            ...
            response = await async_client.get(
                f"/api/v3/discovery-flow/flows/{invalid_flow_id}",
                headers=auth_headers
            )
    ...
    @pytest.mark.asyncio
    async def test_multi_tenant_isolation(self, async_client: AsyncClient, auth_headers):
    ...
    @pytest.mark.asyncio
    async def test_flow_state_persistence(self, async_client: AsyncClient, auth_headers, sample_flow_data):
    ...
    @pytest.mark.asyncio
    async def test_concurrent_flow_operations(self, async_client: AsyncClient, auth_headers):
        ...
        with patch('app.repositories.discovery_v2_repository.DiscoveryV2Repository') as mock_repo_class:
            ...
            mock_repo.update_flow_phase.side_effect = [True, False]  # Second update fails due to concurrency
