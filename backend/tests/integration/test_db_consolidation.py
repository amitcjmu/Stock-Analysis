...
class TestDatabaseConsolidation:
    ...
    async def test_no_is_mock_columns_exist(self):
        ...
        async with AsyncSessionLocal() as session:
            ...
            assert (
                len(is_mock_columns) == 0
            ), f"is_mock columns still exist: {is_mock_columns}"
    ...
    async def test_asset_table_preserved_fields(self):
        ...
        async with AsyncSessionLocal() as session:
            ...
            for field in infrastructure_fields:
                assert (
                    field in columns
                ), f"Infrastructure field {field} missing from assets table"
    ...
    async def test_multi_tenant_indexes(self):
        ...
        async with AsyncSessionLocal() as session:
            ...
            for table in tables_to_check:
                ...
                assert (
                    has_client_index
                ), f"Table {table} missing client_account_id index"
                ...
                assert (
                    has_engagement_index
                ), f"Table {table} missing engagement_id index"
    ...
    async def test_create_data_import_with_new_fields(self):
        ...
        async with AsyncSessionLocal() as session:
            try:
                ...
                result = await session.execute(
                    text(
                        "SELECT id, filename, file_size, mime_type, source_system FROM data_imports WHERE id = :id"
                    ),
