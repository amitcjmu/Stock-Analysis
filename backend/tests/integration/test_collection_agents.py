...
def test_platform_detection_agents():
    ...
    try:
        ...
        assert PlatformDetectionAgent.agent_metadata().name == "platform_detection_agent"
        assert CredentialValidationAgent.agent_metadata().name == "credential_validation_agent"
        assert TierRecommendationAgent.agent_metadata().name == "tier_recommendation_agent"
...
def test_collection_orchestration_agent():
    ...
    try:
        from app.services.agents.collection_orchestrator_agent_crewai import CollectionOrchestratorAgent
        
        assert CollectionOrchestratorAgent.agent_metadata().name == "collection_orchestrator_agent"
...
def test_gap_analysis_agents():
    ...
    try:
        ...
        assert CriticalAttributeAssessorAgent.agent_metadata().name == "critical_attribute_assessor"
        assert GapPrioritizationAgent.agent_metadata().name == "gap_prioritization_agent"
...
def test_all_agents_have_required_attributes():
    ...
    for agent_path in agents:
        ...
        assert hasattr(agent_class, 'agent_metadata'), f"{class_name} missing agent_metadata method"
        assert callable(getattr(agent_class, 'agent_metadata')), f"{class_name} agent_metadata is not callable"
        
        # Check metadata structure
        metadata = agent_class.agent_metadata()
        assert hasattr(metadata, 'name'), f"{class_name} metadata missing name"
        assert hasattr(metadata, 'description'), f"{class_name} metadata missing description"
        assert hasattr(metadata, 'agent_class'), f"{class_name} metadata missing agent_class"
        assert hasattr(metadata, 'required_tools'), f"{class_name} metadata missing required_tools"
        assert hasattr(metadata, 'capabilities'), f"{class_name} metadata missing capabilities"
