...
class SmartWorkflowIntegrationTests:
    ...
    async def test_context(self):
        ...
        async with AsyncSessionLocal() as session:
            ...
            await session.execute(
                delete(Asset).where(Asset.engagement_id == engagement.id)
            )
            await session.execute(
                delete(CollectionFlow).where(
                    CollectionFlow.engagement_id == engagement.id
                )
            )
            await session.execute(
                delete(DiscoveryFlow).where(
                    DiscoveryFlow.engagement_id == engagement.id
                )
            )
            await session.execute(
                delete(AssessmentFlow).where(
                    AssessmentFlow.engagement_id == engagement.id
                )
            )
            await session.execute(
                delete(Engagement).where(Engagement.id == engagement.id)
            )
            await session.execute(
                delete(ClientAccount).where(ClientAccount.id == client.id)
            )
    ...
    @pytest.mark.asyncio
    async def test_cross_flow_integration(
        self, test_context, orchestrator, synchronizer
    ):
        ...
        try:
            ...
            async with AsyncSessionLocal() as session:
                for context in engagement_contexts:
                    ...
                    await session.execute(
                        delete(Asset).where(Asset.engagement_id == engagement_id)
                    )
                    await session.execute(
                        delete(CollectionFlow).where(
                    ...
                    await session.execute(
                        delete(ClientAccount).where(ClientAccount.id == client_id)
                    )
