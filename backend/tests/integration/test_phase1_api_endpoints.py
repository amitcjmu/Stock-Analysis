...
class MockAsyncClient:
    ...
    def set_response(self, url, status_code=200, json_data=None):
        ...
        self._responses[url] = {"status_code": status_code, "json": json_data or {}}
    ...
    async def post(self, url, **kwargs):
        ...
        response = self._responses.get(
            url, {"status_code": 201, "json": {"success": True}}
        )
        mock_response = MagicMock()
        mock_response.status_code = response["status_code"]
        mock_response.json.return_value = response["json"]
    ...
    async def patch(self, url, **kwargs):
        """Mock PATCH request"""
        response = self._responses.get(
            url, {"status_code": 200, "json": {"success": True}}
        )
        mock_response = MagicMock()
        mock_response.status_code = response["status_code"]
        mock_response.json.return_value = response["json"]
...
class TestPhase1APIEndpoints:
    ...
    @pytest.mark.asyncio
    async def test_discovery_flow_creation_pattern(
        self, async_client, auth_headers, sample_flow_data
    ):
        ...
        async_client.set_response(
            "/api/v1/discovery-flow/flows", 201, expected_response
        )

        response = await async_client.post(
            "/api/v1/discovery-flow/flows", json=sample_flow_data, headers=auth_headers
        )
    ...
    async def test_flow_status_retrieval_pattern(self, async_client, auth_headers):
        ...
        async_client.set_response(
            f"/api/v1/discovery-flow/flows/{flow_id}", 200, expected_response
        )

        response = await async_client.get(
            f"/api/v1/discovery-flow/flows/{flow_id}", headers=auth_headers
        )
    ...
    async def test_error_handling_pattern(self, async_client, auth_headers):
        ...
        response = await async_client.get(
            "/api/v1/discovery-flow/flows/nonexistent", headers=auth_headers
        )
    ...
    async def test_pagination_pattern(self, async_client, auth_headers):
        ...
        response = await async_client.get(
            "/api/v1/discovery-flow/flows?page=1&limit=10", headers=auth_headers
        )
    ...
    async def test_client_isolation_pattern(self, async_client, auth_headers):
        ...
        response = await async_client.get(
            f"/api/v1/discovery-flow/flows?client_id={client_id}", headers=auth_headers
        )
