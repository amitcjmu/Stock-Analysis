...
class TestDatabaseConsolidation:
    ...
    async def test_discovery_flow_consolidated_fields(self, db_session):
        ...
        if 'data_validation_completed' in columns:
            ...
            pytest.skip("data_validation_completed column not found - migration not yet applied")
    ...
    async def test_multi_tenant_indexes(self, db_session):
        ...
        for table in tables_to_check:
            ...
            if len(indexes) == 0:
                pytest.skip(f"No client_account_id index found for table {table} - migration not yet applied")
            assert len(indexes) > 0, f"No client_account_id index found for table {table}"
...
class TestV3ServicesIntegration:
    ...
    async def test_complete_discovery_flow(self, db_session, test_context):
        ...
        await mapping_service.create_flow_mappings(
            str(flow.id),
            mappings,
            confidence_scores={field: 0.9 for field in mappings}
        )
...
class TestPerformance:
    ...
    async def test_multi_tenant_query_performance(self, db_session, test_context):
        ...
        for service, prefix in [(asset_service1, "client1"), (asset_service2, "client2")]:
            assets_data = []
            for i in range(500):
                assets_data.append({
                    "asset_name": f"{prefix}-server-{i:03d}",
                    "asset_type": "server"
                })
...
async def test_database_consolidation_suite():
    ...
    async with AsyncSessionLocal() as session:
        ...
        await integration_tests.test_field_rename_backward_compatibility(session, test_context)
