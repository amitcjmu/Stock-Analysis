...
class TestAPIConsistency:
    ...
    async def complete_test_data(self, test_context):
        ...
        async with AsyncSessionLocal() as session:
            ...
            master_flow = CrewAIFlowStateExtensions(
                ...
                flow_status="active",
                flow_configuration={"auto_process": True, "test_mode": True},
            )
    ...
    @pytest.mark.asyncio
    async def test_master_flow_orchestrator_api_consistency(
        self, test_context, complete_test_data
    ):
        """
    ...
    @pytest.mark.asyncio
    async def test_discovery_flow_api_data_relationships(
        self, test_context, complete_test_data
    ):
        """
    ...
    @pytest.mark.asyncio
    async def test_data_import_api_linkage_validation(
        self, test_context, complete_test_data
    ):
        """
    ...
    @pytest.mark.asyncio
    async def test_asset_api_relationship_consistency(
        self, test_context, complete_test_data
    ):
        """
    ...
    @pytest.mark.asyncio
    async def test_cross_api_data_integrity_validation(
        self, test_context, complete_test_data
    ):
        """
        ...
        async with AsyncSessionLocal() as session:
            ...
            assert str(master_flow_data.flow_id) == str(
                discovery_flow_data.master_flow_id
            )
            assert str(master_flow_data.client_account_id) == str(
                discovery_flow_data.client_account_id
            )
            assert str(master_flow_data.engagement_id) == str(
                discovery_flow_data.engagement_id
            )

            # Test 2: Data import vs raw records consistency
            assert data_import.total_records == len(
                [r for r in complete_test_data["raw_records"]]
            )

            processed_count = sum(
                1 for r in complete_test_data["raw_records"] if r.is_processed
            )
            ...
            discovery_assets = [
                a
                for a in assets
                if str(a.discovery_flow_id) == str(discovery_flow.flow_id)
            ]
            assert len(discovery_assets) == len(assets)

            # Test 4: Master flow vs all related entities consistency
            master_flow_assets = [
                a for a in assets if str(a.master_flow_id) == str(master_flow.flow_id)
            ]
            assert len(master_flow_assets) == len(assets)

            master_flow_raw_records = [
                r
                for r in complete_test_data["raw_records"]
                if str(r.master_flow_id) == str(master_flow.flow_id)
            ]
            assert len(master_flow_raw_records) == len(
                complete_test_data["raw_records"]
            )

            logger.info("✅ Cross-API data integrity validation passed")

    @pytest.mark.asyncio
    async def test_api_response_format_standardization(
        self, test_context, complete_test_data
    ):
        """
        ...
        for field in timestamp_fields:
            if field in master_flow_dict and master_flow_dict[field]:
                ...
                try:
                    ...
                    pytest.fail(
                        f"Invalid timestamp format for {field}: {timestamp_value}"
                    )
        ...
        for phase_name, phase_value in phases.items():
            assert isinstance(
                phase_value, bool
            ), f"Phase {phase_name} should be boolean"
    ...
    @pytest.mark.asyncio
    async def test_api_tenant_isolation_consistency(
        self, test_context, complete_test_data
    ):
        """
        ...
        async with AsyncSessionLocal() as session:
            ...
            master_flow_data = await different_master_repo.get_by_flow_id(
                str(master_flow.flow_id)
            )
            ...
            different_discovery_repo = DiscoveryFlowRepository(
                session, different_tenant_context.client_account_id
            )

            # Should not find discovery flow from different tenant
            discovery_flow_data = await different_discovery_repo.get_by_flow_id(
                str(discovery_flow.flow_id)
            )
            assert discovery_flow_data is None  # Should not access other tenant's data

            # Test master flow orchestrator isolation
            different_orchestrator = MasterFlowOrchestrator(
                session, different_tenant_context
            )

            # Should not access flow from different tenant
            try:
                flow_status = await different_orchestrator.get_flow_status(
                    str(master_flow.flow_id)
                )
                assert flow_status is None or "error" in flow_status
            except Exception:
