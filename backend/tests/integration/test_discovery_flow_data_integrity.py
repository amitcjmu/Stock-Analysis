...
class TestDiscoveryFlowDataIntegrity:
    ...
    @pytest.mark.asyncio
    async def test_complete_data_import_to_discovery_flow_pipeline(self, test_context, sample_raw_data):
        """
        ...
        async with AsyncSessionLocal() as session:
            try:
                logger.info("🧪 Starting complete data import to discovery flow pipeline test")
                ...
                master_flow_check = await session.get(CrewAIFlowStateExtensions, master_flow.id)
                ...
                assert str(discovery_flow_check.master_flow_id) == str(master_flow.flow_id)
                ...
                complex_query = select(
                    ...
                    DataImport, DataImport.id == DiscoveryFlow.data_import_id
                ).join(
                    RawImportRecord, RawImportRecord.data_import_id == DataImport.id
                ).join(
                    Asset, Asset.discovery_flow_id == DiscoveryFlow.flow_id
                ).where(
                    CrewAIFlowStateExtensions.flow_id == master_flow.flow_id
                ).group_by(
                    CrewAIFlowStateExtensions.id,
                    DiscoveryFlow.id,
                    DataImport.id
                )
                ...
                logger.info("✅ Complete data import to discovery flow pipeline test passed")
    ...
    async def test_foreign_key_constraints_prevent_invalid_data(self, test_context):
        ...
        async with AsyncSessionLocal() as session:
            try:
                ...
                await session.rollback()
                logger.info("✅ Foreign key constraint properly rejected invalid master_flow_id")
                ...
                await session.rollback()
                logger.info("✅ Foreign key constraint properly rejected invalid data import")
                ...
                await session.rollback()
                logger.info("✅ Foreign key constraint properly rejected invalid raw record")
    ...
    async def test_cascade_deletion_operations(self, test_context, sample_raw_data):
        ...
        async with AsyncSessionLocal() as session:
            try:
                ...
                logger.info(f"✅ Created test data: master_flow={master_flow.flow_id}, "
                ...
                assert await session.get(CrewAIFlowStateExtensions, master_flow.id) is not None
                ...
                assert await session.get(CrewAIFlowStateExtensions, master_flow.id) is None
        ...
        async with AsyncSessionLocal() as session:
            try:
                ...
                try:
                    ...
                    logger.warning("⚠️  Discovery flow created without master flow - check constraints")
                    await session.rollback()
    ...
    @pytest.mark.asyncio
    async def test_api_returns_properly_linked_data(self, test_context, sample_raw_data):
        """
        ...
        async with AsyncSessionLocal() as session:
            try:
                ...
                assert str(master_flow_data.client_account_id) == test_context.client_account_id
                ...
                assert str(discovery_flow_data.master_flow_id) == str(master_flow.flow_id)
                ...
                assert str(data_import_result.master_flow_id) == str(master_flow.flow_id)
                assert str(data_import_result.client_account_id) == test_context.client_account_id
                assert str(data_import_result.engagement_id) == test_context.engagement_id
                ...
                for field in required_fields:
                    assert field in discovery_flow_dict, f"Field {field} missing from discovery flow dict"
                
                assert discovery_flow_dict["data_import_id"] == str(data_import.id)
                assert discovery_flow_dict["client_account_id"] == test_context.client_account_id
                assert discovery_flow_dict["engagement_id"] == test_context.engagement_id
                ...
                for field in required_master_fields:
                    assert field in master_flow_dict, f"Field {field} missing from master flow dict"
                
                assert master_flow_dict["flow_type"] == "discovery"
                assert master_flow_dict["client_account_id"] == test_context.client_account_id
    ...
    async def test_query_performance_with_relationships(self, test_context):
        ...
        async with AsyncSessionLocal() as session:
            try:
                ...
                logger.info(f"✅ Created {len(master_flows)} master flows with related data")
                ...
                join_query = select(
                    ...
                    CrewAIFlowStateExtensions.client_account_id == test_context.client_account_id
                )
                ...
                logger.info(f"✅ Aggregation query completed in {aggregation_query_time:.3f}s")
                ...
                tenant_query = select(
                    ...
                    DataImport, DataImport.master_flow_id == CrewAIFlowStateExtensions.flow_id
                ).where(
                    CrewAIFlowStateExtensions.client_account_id == test_context.client_account_id
                )
                ...
                logger.info(f"✅ Multi-tenant query completed in {tenant_query_time:.3f}s")
    ...
    async def test_constraint_performance_impact(self, test_context):
        ...
        async with AsyncSessionLocal() as session:
            try:
                ...
                logger.info(f"✅ Batch insert of 50 records completed in {batch_insert_time:.3f}s")
                ...
                logger.info(f"✅ Batch update of 50 records completed in {batch_update_time:.3f}s")
                
                # Test 3: Constraint validation performance
                start_time = datetime.now()
                
                # Verify all constraints are satisfied
                constraint_check_query = select(
                    ...
                    DataImport.master_flow_id == master_flow.flow_id
                )
                ...
                logger.info(f"✅ Constraint validation completed in {constraint_check_time:.3f}s")
        ...
        async with AsyncSessionLocal() as session:
            try:
                ...
                expected_constraints = [
                    ...
                    ('raw_import_records', 'master_flow_id', 'crewai_flow_state_extensions'),
                ...
                for table, column, foreign_table in expected_constraints:
                    ...
                    assert constraint_key in constraint_map, f"Missing FK constraint: {table}.{column} -> {foreign_table}"
                    assert constraint_map[constraint_key] == foreign_table, f"Incorrect FK target: {table}.{column}"
                ...
                cascade_map = {
                    (row.table_name, row.column_name): (row.delete_rule, row.update_rule)
                ...
                for table, column in cascade_map:
                    ...
                    assert delete_rule in ['CASCADE', 'SET NULL'], f"Inappropriate delete rule for {table}.{column}: {delete_rule}"
                ...
                for issue in consistency_issues:
                    assert issue.count == 0, f"Data consistency issue found: {issue.issue_type} = {issue.count}"
    ...
    async def test_monitoring_queries_performance(self):
        ...
        async with AsyncSessionLocal() as session:
            try:
                ...
                logger.info(f"✅ Orphan check query completed in {orphan_query_time:.3f}s")
                ...
                logger.info(f"✅ Integrity summary query completed in {integrity_query_time:.3f}s")
                ...
                logger.info(f"✅ Performance metrics query completed in {performance_query_time:.3f}s")
    ...
    async def _cleanup_test_data(self, session: AsyncSession, flow_id: str):
        ...
        try:
            ...
            # Delete master flow
            await session.execute(
                text("DELETE FROM crewai_flow_state_extensions WHERE flow_id = :flow_id"),
                {"flow_id": flow_id}
            )
