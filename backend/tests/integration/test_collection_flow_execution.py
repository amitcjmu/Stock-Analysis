...
async def test_platform_detection_crew_execution():
    ...
    crew.crew.kickoff = Mock(return_value={
        "detected_platforms": [
...
async def test_collection_flow_with_mocked_crewai():
    ...
    with patch('app.services.crewai_flows.unified_collection_flow.CREWAI_FLOW_AVAILABLE', True):
        with patch('app.services.crewai_flows.unified_collection_flow.Flow') as MockFlow:
