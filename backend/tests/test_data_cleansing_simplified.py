...
class MockDataCleansingStats:
    ...
    def __init__(self, total_records=1000, clean_records=850, records_with_issues=150):
        ...
        self.issues_by_severity = {"low": 80, "medium": 50, "high": 15, "critical": 5}
...
class MockDataCleansingAnalysis:
    ...
    def __init__(self, flow_id, include_issues=True):
        ...
        self.recommendations = (
            [MockDataCleansingRecommendation()] if include_issues else []
        )
...
class TestDataCleansingEndpointsSimplified:
    ...
    @pytest.mark.asyncio
    async def test_mock_repository_flow_retrieval(
        self, mock_session, mock_context, sample_flow_id
    ):
        ...
        with patch(
            ...
            repo = mock_repo_class(
                mock_session, mock_context.client_account_id, mock_context.engagement_id
            )
    ...
    @pytest.mark.asyncio
    async def test_mock_repository_flow_not_found(
        self, mock_session, mock_context, sample_flow_id
    ):
    ...
    async def test_master_flow_orchestrator_failure_simulation(self, sample_flow_id):
        ...
        # Test orchestrator execution simulation
        result = await mock_orchestrator.execute_phase(
            flow_id=sample_flow_id, phase_name="data_cleansing", phase_input={}
        )
    ...
    def test_endpoint_response_models(self, sample_flow_id):
        ...
        # Test stats model
        stats = MockDataCleansingStats(
            total_records=500, clean_records=425, records_with_issues=75
        )
    ...
    def test_recommendation_model(self):
        ...
        recommendation = MockDataCleansingRecommendation(
            "validation", "Validate IP addresses"
        )
