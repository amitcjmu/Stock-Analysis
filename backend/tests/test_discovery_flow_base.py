...
class BaseDiscoveryFlowTest:
    ...
    async def upload_file(self, flow_id: str, file_content: bytes, filename: str = "test.csv") -> Dict[str, Any]:
    ...
    async def wait_for_phase(self, flow_id: str, phase: str, timeout: int = 30) -> Dict[str, Any]:
        """Wait for a flow to reach a specific phase."""
        start_time = datetime.utcnow()
        while (datetime.utcnow() - start_time).seconds < timeout:
            status = await self.get_flow_status(flow_id)
            if status.get("current_phase") == phase or phase in status.get("phases_completed", []):
                return status
            await asyncio.sleep(1)
        raise TimeoutError(f"Flow did not reach phase {phase} within {timeout} seconds")
    
    async def stream_sse_events(self, flow_id: str) -> AsyncGenerator[Dict[str, Any], None]:
    ...
    async def mock_crewai_flow(self):
        ...
        with patch("app.services.crewai_flows.unified_discovery_flow.UnifiedDiscoveryFlow") as mock:
            instance = mock.return_value
