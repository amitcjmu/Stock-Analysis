...
class TestSessionFlowMigration:
    ...
    @pytest.mark.asyncio
    async def test_migrate_data_import(self, compatibility_service, mock_db_session, sample_data_import):
        """Test migrating data import from session to flow"""
        # Arrange
        new_flow_id = str(uuid.uuid4())
        mock_db_session.query.return_value.filter.return_value.first.return_value = sample_data_import
    ...
    @pytest.mark.asyncio
    async def test_migrate_raw_records(self, compatibility_service, mock_db_session, sample_raw_records):
        """Test migrating raw records from session to flow"""
        # Arrange
        new_flow_id = str(uuid.uuid4())
        mock_db_session.query.return_value.filter.return_value.all.return_value = sample_raw_records
    ...
    @pytest.mark.asyncio
    async def test_backward_compatibility_lookup(self, compatibility_service, mock_db_session):
        """Test backward compatibility layer"""
        # Arrange
        expected_flow_id = str(uuid.uuid4())
        mock_data_import = MagicMock()
        mock_data_import.flow_id = expected_flow_id
        mock_db_session.query.return_value.filter.return_value.first.return_value = mock_data_import
    ...
    @pytest.mark.asyncio
    async def test_migration_rollback(self, compatibility_service, mock_db_session, sample_data_import):
    ...
    @pytest.mark.asyncio
    async def test_migrate_nonexistent_session(self, compatibility_service, mock_db_session):
    ...
    async def test_batch_migration(self, compatibility_service, mock_db_session):
        ...
        with patch.object(compatibility_service, 'migrate_data_import', return_value=True) as mock_migrate_data:
            with patch.object(compatibility_service, 'migrate_raw_records', return_value=5) as mock_migrate_raw:
    ...
    @pytest.mark.asyncio
    async def test_data_integrity_validation(self, compatibility_service, mock_db_session, sample_data_import):
        ...
        assert sample_data_import.flow_id == new_flow_id
        assert sample_data_import.session_id == original_data['session_id']  # Should be preserved
        assert sample_data_import.client_account_id == original_data['client_account_id']
        assert sample_data_import.original_filename == original_data['original_filename']
    
    @pytest.mark.asyncio
    async def test_concurrent_migration_protection(self, compatibility_service, mock_db_session):
        ...
        mock_db_session.query.return_value.filter.return_value.first.return_value = sample_import
        
        # Act - simulate concurrent migrations
        result_1 = await compatibility_service.migrate_data_import(session_id, flow_id_1)
        
        # Second migration should detect existing flow_id
        sample_import.flow_id = flow_id_1  # Simulate first migration completed
        result_2 = await compatibility_service.migrate_data_import(session_id, flow_id_2)
