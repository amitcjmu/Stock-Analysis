...
class TestDiscoveryFlowAPIEndpoints:
    ...
    def test_discovery_flow_initialization_endpoint(
        self, api_client, authenticated_headers, mock_discovery_data
    ):
        ...
        assert (
            "crew_sequence" in response_data
        ), "Should include crew execution sequence"
        assert (
            len(response_data["crew_sequence"]) > 0
        ), "Should have planned crew executions"

    def test_field_mapping_crew_execution_endpoint(
        self, api_client, authenticated_headers
    ):
    ...
    def test_complete_discovery_flow_execution_endpoint(
        ...
        for crew_name in expected_crews:
            ...
            assert (
                "execution_time" in crew_result
            ), f"{crew_name} should track execution time"

    def test_flow_status_monitoring_endpoint(self, api_client, authenticated_headers):
        ...
        response = api_client.get(
            f"/api/v1/discovery/flow/{flow_id}/status", headers=authenticated_headers
        )
        ...
        assert (
            "estimated_completion" in response_data
    ...
    def test_flow_results_retrieval_endpoint(self, api_client, authenticated_headers):
        ...
        # Act
        response = api_client.get(
            f"/api/v1/discovery/flow/{flow_id}/results", headers=authenticated_headers
        )
        ...
        assert (
            "total_assets_processed" in summary_metrics
        ), "Should count processed assets"
        assert (
            "field_mappings_created" in summary_metrics
        ), "Should count field mappings"
        assert "data_quality_score" in summary_metrics, "Should include quality score"

    def test_multi_tenant_api_isolation(self, api_client):
        ...
        response_2 = api_client.get(
            f"/api/v1/discovery/flow/{flow_id_1}/status", headers=client_2_headers
        )

        # Assert
        assert response_1.status_code == 200, "Client 1 should create flow successfully"
        assert (
            response_2.status_code == 403
        ), "Client 2 should be denied access to client 1's flow"

    def test_api_authentication_and_authorization(
        self, api_client, mock_discovery_data
    ):
        """Test API authentication and authorization (Task 63)"""
        # Test without authentication
        response_no_auth = api_client.post(
            "/api/v1/discovery/flow/initialize", json=mock_discovery_data
        )
        ...
        assert (
            response_limited_auth.status_code == 403
        ), "Should enforce role permissions"

    @pytest.mark.asyncio
    async def test_websocket_real_time_updates(self, mock_websocket_server):
    ...
    def test_api_error_handling_and_responses(self, api_client, authenticated_headers):
    ...
    def test_api_performance_and_timeouts(
        ...
        ), "Large dataset should be handled successfully"
    ...
    def test_api_data_consistency_and_validation(
        ...
        assert (
            init_data["flow_id"] == results_data["flow_id"]
        ), "Flow ID should be consistent"
        assert (
            len(mock_discovery_data["assets"])
            <= results_data["summary_metrics"]["total_assets_processed"]
