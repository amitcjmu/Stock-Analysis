...
class TestUnifiedAssessmentFlow:
    ...
    async def test_initialize_assessment_success(
        ...
        assessment_flow.repository.create_assessment_flow = AsyncMock(return_value="test-flow-123")
    ...
    async def test_capture_architecture_minimums_with_defaults(
        ...
        with patch.object(assessment_flow, '_load_engagement_standards') as mock_load:
            ...
            with patch.object(assessment_flow, '_initialize_default_standards') as mock_init:
                ...
                java_standard = next((s for s in standards if s["requirement_type"] == "java_versions"), None)
    ...
    async def test_analyze_technical_debt_with_components(
        ...
        assessment_flow.crewai_service.run_crew.return_value = sample_component_analysis_result
    ...
    async def test_determine_sixr_strategies_with_validation(
        ...
        flow_state.tech_debt_analysis = {
            "app-1": {"overall_tech_debt_score": 7.2}
        }
        
        # Setup mock crew result
        assessment_flow.crewai_service.run_crew.return_value = sample_sixr_strategy_result
        ...
        with patch.object(assessment_flow, '_get_application_name') as mock_name:
            ...
            # Execute phase
            result = await assessment_flow.determine_component_sixr_strategies(flow_state)
    ...
    async def test_resume_from_phase_with_user_input(
        ...
        flow_state = AssessmentFlowState(**sample_assessment_flow_state)
        assessment_flow.postgres_store.load_flow_state = AsyncMock(return_value=flow_state)
    ...
    async def test_error_handling_during_crew_execution(
        ...
        assessment_flow.crewai_service.run_crew.side_effect = Exception("Crew execution failed")
    ...
    async def test_phase_transition_validation(
        ...
        assert "Invalid phase transition" in str(exc_info.value) or "Prerequisites not met" in str(exc_info.value)
    ...
    async def test_flow_state_persistence(
        ...
        with patch.object(assessment_flow, '_load_engagement_standards') as mock_load:
            ...
            with patch.object(assessment_flow, '_initialize_default_standards') as mock_init:
                ...
                assert saved_state.current_phase == AssessmentPhase.ARCHITECTURE_MINIMUMS
...
class TestAssessmentFlowEdgeCases:
    ...
    async def test_crew_timeout_handling(
        ...
        # Mock crew timeout
        assessment_flow.crewai_service.run_crew.side_effect = asyncio.TimeoutError("Crew execution timed out")
    ...
    async def test_invalid_user_input_handling(
        ...
        assert "Invalid user input" in str(exc_info.value) or "Required fields missing" in str(exc_info.value)
