...
def test_dependency_api():
    ...
    print(
        f'Sample asset: {assets[0].get("asset_name")} has related_ci: {assets[0].get("related_ci")}'
    )
        ...
        if response.status_code == 200:
            ...
            if "dependency_intelligence" in result:
                ...
                for i, dep in enumerate(deps[:3]):
                    print(
                        f'  Dep {i+1}: {dep.get("source_application")} -> {dep.get("target_application")} (Impact: {dep.get("impact_level")})'
                    )

                # Show overall stats
                total_deps = dep_intel.get("dependency_analysis", {}).get(
