...
class TestTenantIsolation:
    ...
    async def test_master_flow_isolation(
        ...
        assert all(f.client_account_id == uuid.UUID(client1_context.client_account_id) for f in client1_active)
        
        client2_active = await client2_repo.get_active_flows()
        assert all(f.client_account_id == uuid.UUID(client2_context.client_account_id) for f in client2_active)
    ...
    async def test_context_enforcement_in_queries(
        self,
        db_session: AsyncSession
    ):
...
class TestAPISecurityEnforcement:
    ...
    async def test_api_requires_context_headers(self):
        ...
        request = Request(
            {"type": "http", "headers": [], "method": "GET", "url": {"path": "/api/v1/flows"}},
            receive=None,
            send=None
        )
    ...
    async def test_api_validates_uuid_format(self):
        ...
        request = Request(
            {"type": "http", "headers": headers, "method": "GET", "url": {"path": "/api/v1/flows"}},
            receive=None,
            send=None
        )
