...
def mock_flow_registry():
    ...
    mock_flow_config.phases = [
        Mock(name="phase1", validators=[], crew_config={"agents": ["agent1"], "tasks": ["task1"]}),
        Mock(name="phase2", validators=[], crew_config={"agents": ["agent2"], "tasks": ["task2"]})
    ]
    mock_flow_config.get_phase_config = lambda name: next((p for p in mock_flow_config.phases if p.name == name), None)
    mock_flow_config.get_next_phase = lambda last: "phase2" if last == "phase1" else None
...
def orchestrator(mock_db, test_context, mock_flow_registry, mock_master_repo):
    ...
    with patch('app.services.master_flow_orchestrator.FlowTypeRegistry', return_value=mock_flow_registry):
        with patch('app.services.master_flow_orchestrator.CrewAIFlowStateExtensionsRepository', return_value=mock_master_repo):
...
class TestMasterFlowOrchestrator:
    ...
    @pytest.mark.asyncio
    async def test_get_flow_status_without_details(self, orchestrator, mock_master_repo):
    ...
    async def test_error_handling_with_retry(self, orchestrator, mock_master_repo):
        ...
        with patch.object(orchestrator.error_handler, 'handle_error') as mock_error_handler:
    ...
    async def test_performance_tracking(self, orchestrator):
        ...
        # Arrange
        with patch.object(orchestrator.performance_tracker, 'start_operation') as mock_start:
            with patch.object(orchestrator.performance_tracker, 'end_operation') as mock_end:
    ...
    async def test_phase_validation(self, orchestrator):
        ...
        with patch.object(orchestrator.validator_registry, 'get_validator', return_value=mock_validator):
            # Update phase config to include validator
            phase_config = orchestrator.flow_registry.get_flow_config("discovery").phases[0]
    ...
    async def test_multi_tenant_isolation(self, orchestrator, test_context):
        ...
        assert orchestrator.master_repo.client_account_id == test_context.client_account_id