...
class TestSessionFlowMigration:
    ...
    def compatibility_service(self, db_session, sample_client_data):
        ...
        return SessionFlowCompatibilityService(
            db_session, sample_client_data["client_account_id"]
        )
    ...
    def test_migration_script_orphaned_session_handling(
        self, db_session, sample_client_data
    ):
        ...
        created_flow = (
            db_session.query(DiscoveryFlow)
            .filter(DiscoveryFlow.import_session_id == orphaned_session.id)
            .first()
        )
    ...
    def test_compatibility_service_session_to_flow_mapping(
        self, compatibility_service, db_session, sample_client_data
    ):
        ...
        # Test mapping
        result_flow_id = compatibility_service.get_flow_id_from_session_id(
            str(session.id)
        )
        assert result_flow_id == str(flow.flow_id)

        # Test reverse mapping
        result_session_id = compatibility_service.get_session_id_from_flow_id(
            str(flow.flow_id)
        )
        assert result_session_id == str(session.id)

    def test_compatibility_service_caching(
        self, compatibility_service, db_session, sample_client_data
    ):
    ...
    def test_compatibility_service_asset_migration(
        self, compatibility_service, db_session, sample_client_data
    ):
    ...
    def test_migration_validation_integrity(
        self, compatibility_service, db_session, sample_client_data
    ):
    ...
    def test_migration_rollback_scenario(self, db_session, sample_client_data):
        ...
        db_session.execute(
            text("UPDATE assets SET flow_id = NULL WHERE flow_id IS NOT NULL")
        )
        ...
        rollback_flow = (
            db_session.query(DiscoveryFlow).filter(DiscoveryFlow.id == flow.id).first()
        )
    ...
    def test_performance_impact_measurement(
        ...
        assert (
            avg_time_ms < 100
        ), f"Average mapping time {avg_time_ms:.2f}ms exceeds 100ms threshold"
    ...
    def test_data_integrity_preservation(self, db_session, sample_client_data):
        ...
        assert (
            session.total_assets_processed
            == original_session_data["total_assets_processed"]
        )
    ...
    def test_backward_compatibility_during_transition(
        self, compatibility_service, db_session, sample_client_data
    ):
        ...
        legacy_found = compatibility_service.find_assets_by_identifier(
            str(legacy_asset.session_id)
        )
        ...
        migrated_by_session = compatibility_service.find_assets_by_identifier(
            str(migrated_asset.session_id)
        )
        migrated_by_flow = compatibility_service.find_assets_by_identifier(
            str(migrated_asset.flow_id)
        )
        ...
        new_found = compatibility_service.find_assets_by_identifier(
            str(new_asset.flow_id)
        )
...
class TestMigrationEdgeCases:
    ...
    def test_migration_with_database_errors(self, db_session, sample_client_data):
        ...
        service = SessionFlowCompatibilityService(
            db_session, sample_client_data["client_account_id"]
        )
