...
class MockPsutil:
    ...
    def cpu_percent(interval=1):
        return 25.5  # Mock CPU usage percentage
...
def performance_data_sets():
    ...
    return {
        "small": {"asset_count": 100, "field_count": 10, "complexity": "low"},
        "medium": {"asset_count": 1000, "field_count": 25, "complexity": "moderate"},
        "large": {"asset_count": 10000, "field_count": 50, "complexity": "high"},
        "enterprise": {
            "asset_count": 50000,
            "field_count": 100,
    ...
    class MockCrewWithPerformance:
        ...
        async def execute_async(self, inputs: Dict[str, Any]) -> Dict[str, Any]:
            ...
            complexity_multiplier = {
                "low": 0.01,  # Faster for small datasets
                "moderate": 0.02,  # Moderate speed
                "high": 0.05,  # Slower for large datasets
                "enterprise": 0.1,  # Much slower for enterprise
            }.get(inputs.get("complexity", "low"), 0.01)

            # More predictable processing time calculation
            processing_time = (asset_count / 10000) * complexity_multiplier
            await asyncio.sleep(
                min(processing_time, 2.0)
            )  # Cap at 2 seconds for testing
...
class TestDiscoveryFlowPerformance:
    ...
    async def test_crew_execution_performance_small_dataset(
        self, performance_monitor, performance_data_sets, mock_crew_performance
    ):
        ...
        ), "Memory usage should be minimal for small data"
    ...
    async def test_crew_execution_performance_enterprise_dataset(
        ...
        ), "Enterprise processing should complete in reasonable time"
        assert (
            result["processed_assets"] == 50000
        ), "All enterprise assets should be processed"
    ...
    async def test_memory_optimization_during_workflow(
        self, performance_monitor, performance_data_sets
    ):
    ...
    async def test_scalability_with_increasing_data_loads(
        ...
        # With more assets, throughput per second should decrease but remain reasonable
        assert (
            medium_throughput > small_throughput * 0.1
        ), "Medium scale should maintain some throughput"
        assert (
            large_throughput > medium_throughput * 0.1
        ), "Large scale should maintain some throughput"
    ...
    async def test_performance_regression_detection(
        ...
        baseline_performance = {
            "small": {"max_time": 0.1, "max_memory": 50},
            "medium": {"max_time": 0.5, "max_memory": 200},
            "large": {"max_time": 1.0, "max_memory": 500},
        }
        ...
        for size, results in regression_results.items():
            assert not results[
                "time_regression"
            ], f"{size} dataset shows time regression"
            assert not results[
                "memory_regression"
            ], f"{size} dataset shows memory regression"
    ...
    async def test_concurrent_workflow_performance(
        ...
        # Assert
        assert (
            len(all_results) == num_concurrent_workflows
        ), "All workflows should complete"
        for workflow_results in all_results:
            assert (
                len(workflow_results) == 3
            ), "Each workflow should have 3 crew results"
            assert all(
                r["status"] == "completed" for r in workflow_results
            ), "All crews should succeed"
