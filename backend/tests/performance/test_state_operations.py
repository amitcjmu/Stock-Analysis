...
from app.services.crewai_flows.persistence.postgres_store import PostgresFlowStateStore, ConcurrentModificationError
...
def large_state():
    ...
    phases = ["initialization", "attribute_mapping", "data_cleansing", "inventory_building"]
    for phase in phases:
...
class TestStatePerformance:
    ...
    @pytest.mark.asyncio
    async def test_small_state_save_performance(self, mock_db_session, test_context, small_state):
    ...
    @pytest.mark.asyncio
    async def test_large_state_save_performance(self, mock_db_session, test_context, large_state):
    ...
    @pytest.mark.asyncio
    async def test_state_load_performance(self, mock_db_session, test_context, large_state):
    ...
    @pytest.mark.asyncio
    async def test_concurrent_state_updates(self, mock_db_session, test_context, small_state):
    ...
    @pytest.mark.asyncio
    async def test_bulk_state_operations_performance(self, mock_db_session, test_context):
    ...
    async def test_state_serialization_performance(self, large_state):
        ...
        assert serialize_duration < 20, f"Serialization took {serialize_duration:.2f}ms, expected <20ms"
        
        start = time.perf_counter()
        deserialized = json.loads(serialized)
        deserialize_duration = (time.perf_counter() - start) * 1000
        
        assert deserialize_duration < 20, f"Deserialization took {deserialize_duration:.2f}ms, expected <20ms"
    ...
    @pytest.mark.asyncio
    async def test_database_connection_efficiency(self, mock_db_session, test_context, small_state):
        ...
        assert operation_count <= expected_operations, f"Used {operation_count} DB operations, expected ≤{expected_operations}"
    
    @pytest.mark.asyncio
    async def test_state_version_increment_performance(self, mock_db_session, test_context, small_state):
    ...
    @pytest.mark.asyncio
    async def test_concurrent_read_performance(self, mock_db_session, test_context, large_state):
        ...
        tasks = [
            store.load_state("test-flow-large") 
            for _ in range(20)
        ]
        ...
        assert duration < 100, f"20 concurrent reads took {duration:.2f}ms, expected <100ms"
