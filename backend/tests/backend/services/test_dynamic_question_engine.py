...
class TestDynamicQuestionEngine:
    ...
    async def test_agent_pruning_success(
        ...
        with patch(
            "app.services.collection.dynamic_question_engine.multi_model_service"
        ) as mock_mms:
    ...
    async def test_handle_dependency_change_critical_field(
        ...
        assert (
            len(result.reopened_question_ids) == 3
        )  # tech_stack, supported_versions, patch_status
    ...
    async def test_handle_dependency_change_non_critical_fallback(
        ...
        with patch(
            ...
            assert (
                result.reopened_question_ids == []
            )  # No hardcoded dependency for app_name
    ...
    async def test_reopen_dependent_questions_fallback(
        ...
        result = await service._reopen_dependent_questions_fallback(
            asset_id, "os_version"
        )
