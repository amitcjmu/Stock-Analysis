...
async def test_master_child_flow_status_synchronization(db_session):
    ...
    assert master_flow.flow_persistence_data["completion"]["completed_by"] == "collection_flow_state_management"
...
async def test_collection_flow_application_linkage(db_session):
    ...
    request_data = CollectionApplicationSelectionRequest(
        selected_application_ids=[asset1.id, asset2.id],
        action="select"
    )
    ...
    for app in collection_apps:
        ...
        assert app.canonical_application_id is not None  # Should be set by deduplication
