                ...
                content=json.dumps(
                    {
                        ...
                        "reasoning": "High quality mapping with strong semantic alignment",
                        "suggestions": [
                            "Consider adding additional context for edge cases"
                        ],
                    }
                )
            )
...
class TestAgenticDiscoveryFlow:
    ...
    async def test_no_hardcoded_thresholds(self, test_session, mock_openai_client):
        ...
        with patch(
            ...
            assert (
                result["agent_decision"]["reasoning"]
                == "High quality mapping with strong semantic alignment"
            )

    @pytest.mark.asyncio
    async def test_agent_dynamic_decision_making(
        self, test_session, mock_openai_client
    ):
    ...
    async def test_sse_realtime_updates(self, test_session):
        ...
        discovery_flow = DiscoveryFlow(
            ...
            state_data={
                ...
                "phases": {"data_import": {"status": "in_progress", "progress": 45}},
            },
        )
        test_session.add(discovery_flow)
        await test_session.commit()
    ...
    async def test_agent_learning_from_feedback(self, test_session, mock_openai_client):
        ...
        mock_response = MagicMock()
        mock_response.choices = [
            MagicMock(message=MagicMock(content=json.dumps(initial_decision)))
        ]
        mock_openai_client.chat.completions.create = AsyncMock(
            return_value=mock_response
        )
        ...
        with patch(
            ...
            mock_response.choices = [
                MagicMock(message=MagicMock(content=json.dumps(improved_decision)))
            ]
            ...
            assert (
                result2["agent_decision"]["confidence"]
                > result1["agent_decision"]["confidence"]
            )
    ...
    async def test_master_flow_integration(self, test_session, mock_openai_client):
        ...
        with patch(
            ...
            assert result["agent_decision"]["decision"] in [
                "approve",
                "review",
                "reject",
            ]
...
async def test_complete_flow_execution(
    test_session, mock_openai_client, sample_mapping_data
):
    ...
    def update_mock_response(phase):
        ...
        mock_response = MagicMock()
        mock_response.choices = [
            MagicMock(message=MagicMock(content=json.dumps(decision)))
        ]
        mock_openai_client.chat.completions.create = AsyncMock(
            return_value=mock_response
        )
    ...
    with patch(
        ...
        results["data_import"] = await flow._execute_data_import_phase(
            {"file_path": "/tmp/test.csv"}
        )

        # Field mapping phase
        update_mock_response("field_mapping")
        results["field_mapping"] = await flow._execute_field_mapping_phase(
            sample_mapping_data
        )

        # Data cleansing phase
        update_mock_response("data_cleansing")
        results["data_cleansing"] = await flow._execute_data_cleansing_phase(
            sample_mapping_data
        )

        # Transformation phase
        update_mock_response("transformation")
        results["transformation"] = await flow._execute_transformation_phase(
            sample_mapping_data
        )
