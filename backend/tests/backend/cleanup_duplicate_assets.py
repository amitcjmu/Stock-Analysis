...
async def find_duplicate_assets(
    ...
    ).order_by(Asset.created_at.desc())  # Order by newest first
    ...
    duplicate_groups = {
        key: assets for key, assets in asset_groups.items()
        if len(assets) > 1
    }
...
async def cleanup_duplicate_assets(
    ...
    for key, assets in duplicate_groups.items():
        ...
        logger.info(f"\nGroup: {key}")
        logger.info(f"  Keeping: {assets_to_keep.name} (created: {assets_to_keep.created_at})")
...
async def analyze_duplicates_by_flow(
    ...
    for key, assets in duplicate_groups.items():
        for asset in assets:
            flow_id = str(asset.discovery_flow_id) if asset.discovery_flow_id else "unknown"
    ...
    for flow_id in flow_analysis:
        flow_analysis[flow_id]["unique_keys"] = list(flow_analysis[flow_id]["unique_keys"])
