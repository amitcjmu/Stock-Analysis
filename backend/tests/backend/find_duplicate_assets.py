            ...
            for client_id, engagement_id, total_assets in client_engagements:
                ...
                for hostname, group in hostname_groups.items():
                    if len(group) > 1:
                        ...
                        logger.info(f"   Duplicate hostname '{hostname}': {len(group)} instances")
                        for asset in group[:3]:  # Show first 3
                            logger.info(f"     - {asset.name} (created: {asset.created_at})")
                
                for name, group in name_groups.items():
                    if len(group) > 1:
                        duplicate_count += len(group) - 1
                        logger.info(f"   Duplicate name '{name}': {len(group)} instances")
                        for asset in group[:3]:  # Show first 3
