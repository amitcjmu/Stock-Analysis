...
async def test_ai_learning_scenario():
    ...
    ram_learning = field_mapping_tool.learn_field_mapping(
        "RAM_GB", "Memory (GB)", "user_feedback_test"
    )
    owner_learning = field_mapping_tool.learn_field_mapping(
        "APPLICATION_OWNER", "Business Owner", "user_feedback_test"
    )
    tier_learning = field_mapping_tool.learn_field_mapping(
        "DR_TIER", "Criticality", "user_feedback_test"
    )
    ...
    print(
        f"   Missing Fields Feedback: {user_feedback['user_corrections']['missing_fields_feedback']}"
    )
    ...
    try:
        ...
        print(
            f"   Patterns identified: {learning_result.get('patterns_identified', [])}"
        )
    ...
    print(
        f"   Memory field learning SUCCESS: {memory_missing_before and not memory_missing_after}"
    )
    ...
    print(
        f"   Success rate: {(improvement / len(missing_before) * 100):.1f}% reduction in false alerts"
    )
...
if __name__ == "__main__":
    ...
    print(
        f"   APPLICATION_OWNER → Business Owner learned: {result['application_owner_learned']}"
    )
    ...
    field_mappings_learned = (
        result["ram_gb_learned"]
        and result["application_owner_learned"]
        and result["dr_tier_learned"]
    )

    if (
        result["improvement"] >= 1
        and memory_learning_success
        and field_mappings_learned
    ):
        ...
        print(
            "   Expected: improvement >= 1, memory_learning=True, all_mappings_learned=True"
        )
        print(
            f"   Actual: improvement={result['improvement']}, memory_learning={memory_learning_success}, all_mappings={field_mappings_learned}"
        )
        print(
            f"   Details: ram_gb_learned={result['ram_gb_learned']}, application_owner_learned={result['application_owner_learned']}, dr_tier_learned={result['dr_tier_learned']}"
        )
