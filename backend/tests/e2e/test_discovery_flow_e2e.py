...
class TestDiscoveryFlowE2E:
    ...
    async def test_complete_discovery_flow_with_file_upload(self, test_client, auth_headers, test_context_headers):
...
db-server-01,10.0.3.100,Ubuntu 20.04,32,128,2000,production,datacenter-1"""
        ...
        files = {
            "file": ("servers.csv", csv_content.encode(), "text/csv")
        }
    ...
    async def test_discovery_flow_with_json_data(self, test_client, auth_headers, test_context_headers):
        ...
        response = await test_client.post(
            ...
            json={
                "flow_id": flow_id,
                "source_fields": [
                    "name", "type", "specs.cpu", "specs.memory_mb",
                    "network.ip", "software.os"
                ],
                "target_schema": "asset_inventory",
                "mapping_rules": {
                    "name": "asset_name",
    ...
    async def test_flow_pause_resume_functionality(self, test_client, auth_headers, test_context_headers):
        """Test pausing and resuming a discovery flow"""
        headers = {**auth_headers, **test_context_headers}
        
        # Create a flow with substantial data
        test_data = [{"server": f"server-{i:03d}", "ip": f"10.0.1.{i}"} for i in range(100)]
    ...
    async def test_flow_error_handling_and_recovery(self, test_client, auth_headers, test_context_headers):
    ...
    async def test_bulk_flow_operations(self, test_client, auth_headers, test_context_headers):
        ...
        # List all flows
        response = await test_client.get(
            "/api/v3/discovery-flow/flows?page_size=10",
            headers=headers
        )
    ...
    async with AsyncClient(app=app, base_url="http://test") as client:
        ...
        for i in range(1000):
            large_dataset.append({
                ...
                "operating_system": ["Ubuntu 20.04", "RHEL 8", "Windows Server 2019"][i % 3],
                "cpu_cores": 4 + (i % 28),
                "memory_gb": 8 + (i % 120),
