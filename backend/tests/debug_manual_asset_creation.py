    ...
    try:
        ...
        async with AsyncSessionLocal() as session:
            ...
            query = (
                ...
                .where(RawImportRecord.asset_id.is_(None))
            )
            ...
            print(
                f"📊 Found {len(raw_records)} unprocessed raw records for Marathon Petroleum"
            )
            ...
            print(
                f"🔍 Verification: Marathon Petroleum now has {len(marathon_assets)} assets"
            )
            for asset in marathon_assets[
                -processed_count:
            ]:  # Show the newly created ones
