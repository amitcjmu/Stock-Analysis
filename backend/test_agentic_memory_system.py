...
async def test_agentic_memory_system():
            ...
            try:
                ...
                if result_data.get("patterns"):
                    for pattern in result_data["patterns"][:2]:  # Show first 2
                        print(
                            f"   - {pattern['pattern_name']} (confidence: {pattern['confidence']})"
                        )
            ...
            try:
                ...
                if result_data.get("assets"):
                    for asset in result_data["assets"][:2]:
                        print(
                            f"   - {asset.get('name', 'unknown')} ({asset.get('asset_type', 'unknown')})"
                        )
            except json.JSONDecodeError:
                print(f"   - Raw result: {result[:100]}...")
        else:
        ...
        if len(tools) > 3:
            ...
            try:
                ...
                if result_data.get("status") == "success":
                    ...
                    print(
                        f"   - Agent: {result_data.get('enrichment_agent', 'unknown')}"
                    )
                else:
    ...
    try:
        if len(tools) > 2:
            ...
            try:
                ...
                if result_data.get("status") == "success":
                    print(
                        f"   - Pattern ID: {result_data.get('pattern_id', 'unknown')}"
                    )
    ...
    try:
        ...
        for i, result in enumerate(results[:3]):  # Show first 3
            print(
                f"   - Result {i+1}: {result.tier} tier, confidence {result.confidence_score}"
            )
    ...
    try:
        ...
        different_memory_manager = ThreeTierMemoryManager(
            different_client_id, different_engagement_id
        )
        ...
        if len(isolated_results) == 0:
            print(
                "✅ Multi-tenant isolation verified: different tenant sees no patterns"
            )
        else:
            print(
                f"⚠️ Multi-tenant isolation issue: found {len(isolated_results)} patterns"
            )
