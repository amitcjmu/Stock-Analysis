...
async def test_agentic_intelligence_system():
    ...
    try:
        ...
        for asset in sample_assets:
            print(
                f"   - {asset['name']} ({asset['asset_type']}) - {asset['environment']}"
            )
    ...
    try:
        ...
        print(
            f"   - Business Value Score: {business_result.get('business_value_score')}/10"
        )
    ...
    try:
        ...
        modernization_result = (
            await modernization_agent.analyze_modernization_potential(test_asset)
        )

        print("✅ ModernizationAgent analysis completed:")
        print(f"   - Asset: {test_asset['name']}")
        print(
            f"   - Cloud Readiness: {modernization_result.get('cloud_readiness_score')}/100"
        )
        print(
            f"   - Modernization Potential: {modernization_result.get('modernization_potential')}"
        )
        print(
            f"   - Recommended Strategy: {modernization_result.get('recommended_strategy')}"
        )
    ...
    try:
        ...
        print(
            f"✅ Complete agentic enrichment completed for {len(enriched_assets)} assets:"
        )

        for asset in enriched_assets:
            print(f"\n   📊 {asset['name']}:")
            print(
                f"      - Business Value: {asset.get('business_value_score', 'N/A')}/10"
            )
            print(f"      - Risk Level: {asset.get('risk_assessment', 'N/A')}")
            print(
                f"      - Cloud Readiness: {asset.get('cloud_readiness_score', 'N/A')}/100"
            )
            print(f"      - Enrichment Status: {asset.get('enrichment_status', 'N/A')}")
            print(
                f"      - Overall Score: {asset.get('overall_enrichment_score', 'N/A')}"
            )
    ...
    try:
        ...
        if enriched_data:
            ...
            print(
                f"   - Sample Asset Business Value: {sample_enriched.get('business_value_score', 'N/A')}"
            )
            print(
                f"   - Sample Asset Risk: {sample_enriched.get('risk_assessment', 'N/A')}"
            )
            print(
                f"   - Sample Asset Cloud Readiness: {sample_enriched.get('cloud_readiness_score', 'N/A')}"
            )
    ...
    try:
        ...
        async with AsyncSessionLocal() as session:
            ...
            await session.execute(
                text(
                    "DELETE FROM agent_discovered_patterns WHERE client_account_id = :client_id"
                ),
                {"client_id": client_account_id},
            )
            await session.execute(
