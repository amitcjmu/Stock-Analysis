...
class DatabaseSetup:
    ...
    def __init__(
        self, force: bool = False, seed_only: bool = False, validate_only: bool = False
    ):
        self.force = force
    ...
    async def create_database_if_not_exists(self) -> bool:
        ...
        try:
            ...
            if db_exists and self.force:
                logger.info(
                    f"🗑️  Force mode: Dropping database '{self.db_config['database']}'..."
                )
            ...
            async with AsyncSessionLocal() as db:
                ...
                try:
                    ...
                    validation_results["tables_exist"] = (
                        validation_results["table_count"] > 0
                    )
                except Exception as e:
                ...
                try:
                    result = await db.execute(
                        text("SELECT COUNT(*) FROM client_accounts")
                    )
    ...
    def print_validation_report(self, results: Dict[str, Any]) -> None:
        ...
        overall_ok = (
            ...
            and results["extensions_installed"]
            and results["admin_exists"]
        ...
        logger.info(
            f"  📊 Tables: {'✅' if results['tables_exist'] else '❌'} ({results['table_count']} found)"
        )
        logger.info(
            f"  🔌 Extensions: {'✅' if results['extensions_installed'] else '❌'}"
        )
    ...
    async def run_full_setup(self) -> None:
        ...
        try:
            ...
            if len(validation_results["errors"]) == 0:
                ...
                logger.error(
                    "⚠️  Database setup completed with warnings. See validation report above."
                )
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Initialize AI Modernize Migration Platform Database"
    )
    parser.add_argument(
        "--force", action="store_true", help="Drop and recreate database if it exists"
    )
    parser.add_argument(
        "--seed-only",
        action="store_true",
    ...
    parser.add_argument(
        "--validate-only", action="store_true", help="Only validate database state"
    )
    ...
    try:
        setup = DatabaseSetup(
            force=args.force, seed_only=args.seed_only, validate_only=args.validate_only
        )
