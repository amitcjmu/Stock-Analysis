...
OS_TYPES = ['Windows Server 2019', 'RHEL 8', 'Ubuntu 20.04', 'CentOS 7', 'Amazon Linux 2']
...
class TestDataSeeder:
    ...
    async def _create_users(self, session: AsyncSession):
        ...
        for i in range(5):
            ...
            users.append({
                ...
                'default_client_account_id': TEST_TENANT_1 if i < 3 else TEST_TENANT_2,
    ...
    async def _create_discovery_data(self, session: AsyncSession, master_flows):
        ...
        for i in range(10):
            ...
            engagement_id = random.choice(engagement_ids[:3] if i < 5 else engagement_ids[3:])
            master_flow_id = master_flows[i % len(master_flows)]['flow_id']
            
            status = random.choice(['completed', 'processing', 'completed', 'completed'])
            ...
            await session.execute(
                ...
                """),
                {
                    ...
                    'mime': 'text/csv',
                    'source': random.choice(['servicenow', 'cmdb_export', 'manual_upload']),
                    'status': status,
            ...
            phases_completed = []
            if status == 'completed':
                phases_completed = ['data_validation', 'field_mapping', 'data_cleansing', 
            ...
            await session.execute(
                ...
                """),
                {
                    ...
                    'progress': 100 if status == 'completed' else random.randint(20, 80),
                    'created': datetime.now() - timedelta(days=random.randint(1, 30))
                }
            )
                ...
                for j in range(random.randint(5, 15)):
                    ...
                    await session.execute(
                        ...
                        """),
                        {
                            ...
                            'client_id': client_id,
                            'master_id': master_flow_id,
                            'source': fake.word() + '_' + random.choice(['name', 'id', 'type', 'status']),
                            'target': random.choice(['hostname', 'ip_address', 'os_type', 'department']),
                            'match': random.choice(['exact', 'fuzzy', 'pattern', 'custom']),
                            'confidence': round(random.uniform(0.7, 1.0), 2),
                            'status': 'approved' if status == 'completed' else 'suggested',
                            'suggested': 'ai_mapper',
                            'created': datetime.now() - timedelta(days=random.randint(1, 30))
                        }
                    )
    ...
    async def _create_assets(self, session: AsyncSession, master_flows):
        ...
        result = await session.execute(
            text("SELECT id, master_flow_id FROM discovery_flows WHERE status = 'completed'")
        )
        ...
        for flow_id, master_id in discovery_flows:
            ...
            for i in range(random.randint(3, 8)):
                ...
                asset_data = {
                    ...
                    'hostname': fake.slug() + '-' + random.choice(['app', 'db', 'web', 'api']),
                ...
                if asset_type == 'server':
                    ...
                    asset_data.update({
                        ...
                        'vendor': fake.company(),
                        'runtime': random.choice(['Java', '.NET', 'Python', 'Node.js', 'Go']),
                        'user_count': random.randint(10, 1000)
                    })
                elif asset_type == 'database':
                    asset_data.update({
                        'database_engine': random.choice(['PostgreSQL', 'MySQL', 'Oracle', 'SQL Server', 'MongoDB']),
                ...
                asset_data.update({
                    ...
                    'migration_complexity': random.choice(['low', 'medium', 'high', 'very_high']),
                    'estimated_migration_cost': round(random.uniform(1000, 50000), 2)
                })
                ...
                if random.random() > 0.7 and assets_created > 1:
                    ...
                    await session.execute(
                        ...
                        """),
                        {
                            ...
                            'type': random.choice(['network', 'database', 'api', 'shared_storage']),
...
async def main():
    """Main entry point"""
    print(f"Seeding test data to: {DATABASE_URL.split('@')[1] if '@' in DATABASE_URL else 'local'}\n")
