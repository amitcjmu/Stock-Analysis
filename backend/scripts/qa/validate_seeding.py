...
class DatabaseValidator:
    ...
    async def _validate_client_accounts(self, session: AsyncSession):
        ...
        result = await session.execute(
            select(ClientAccount).where(ClientAccount.name.is_(None))
        )
        incomplete_accounts = result.fetchall()

        if incomplete_accounts:
            self._add_result(
                "client_accounts_complete",
                False,
        ...
        result = await session.execute(
            select(ClientAccount.industry, func.count(ClientAccount.id)).group_by(
        ...
        if len(industries) < 1:
            self._add_result("client_accounts_diversity", False, "No industries found")
        else:
            self._add_result(
                "client_accounts_diversity",
                True,
    ...
    async def _validate_engagements(self, session: AsyncSession):
        ...
        if count != self.expected.engagements:
            ...
            self._add_result(
                "engagements_count", True, f"✅ Found {count} engagements as expected"
            )
        ...
        result = await session.execute(
            select(Engagement.status, func.count(Engagement.id)).group_by(
                Engagement.status
            )
        )
        ...
        if len(statuses) == 0:
            self._add_result(
                "engagement_status_diversity", False, "No engagement statuses found"
            )
        else:
            self._add_result(
                "engagement_status_diversity",
                True,
    ...
    async def _validate_users(self, session: AsyncSession):
        ...
        if count != self.expected.users:
            ...
            self._add_result("users_count", True, f"✅ Found {count} users as expected")
    ...
    async def _validate_user_roles(self, session: AsyncSession):
        ...
        result = await session.execute(
            select(UserRole.role_name, func.count(UserRole.id)).group_by(
                UserRole.role_name
            )
        )
        ...
        if missing_roles:
            self._add_result(
                "user_roles_diversity", False, f"Missing user roles: {missing_roles}"
            )
        else:
            self._add_result(
                "user_roles_diversity",
                True,
        ...
        if users_without_roles:
            ...
            self._add_result(
                "users_have_roles", True, "✅ All users have assigned roles"
            )
    ...
    async def _validate_assets(self, session: AsyncSession):
        ...
        if count != self.expected.assets:
            ...
            self._add_result(
                "assets_count", True, f"✅ Found {count} assets as expected"
            )
        ...
        if missing_types:
            self._add_result(
                "asset_type_diversity", False, f"Missing asset types: {missing_types}"
            )
        else:
            self._add_result(
        ...
        result = await session.execute(
            select(Asset).where(
                or_(Asset.name.is_(None), Asset.name == "", Asset.asset_type.is_(None))
            )
        )
        ...
        if incomplete_assets:
            ...
            self._add_result(
                "assets_complete", True, "✅ All assets have required fields"
            )
    ...
    async def _validate_asset_dependencies(self, session: AsyncSession):
        ...
        result = await session.execute(
            select(AssetDependency).where(
                AssetDependency.asset_id == AssetDependency.depends_on_asset_id
            )
        )
        ...
        if circular_deps:
            ...
            self._add_result(
                "asset_deps_circular", True, "✅ No circular dependencies found"
            )
    ...
    async def _validate_discovery_flows(self, session: AsyncSession):
        ...
        # Status diversity check
        result = await session.execute(
            select(DiscoveryFlow.current_phase, func.count(DiscoveryFlow.id)).group_by(
    ...
    async def _validate_data_imports(self, session: AsyncSession):
        ...
        if count != self.expected.data_imports:
            ...
            self._add_result(
                "data_imports_count", True, f"✅ Found {count} data imports as expected"
            )
    ...
    async def _validate_assessments(self, session: AsyncSession):
        ...
        if count != self.expected.assessments:
            ...
            self._add_result(
                "assessments_count", True, f"✅ Found {count} assessments as expected"
            )
    ...
    async def _validate_migrations(self, session: AsyncSession):
        ...
        if count != self.expected.migrations:
            ...
            self._add_result(
                "migrations_count", True, f"✅ Found {count} migrations as expected"
            )
    ...
    async def _validate_wave_plans(self, session: AsyncSession):
        ...
        if count != self.expected.wave_plans:
            ...
            self._add_result(
                "wave_plans_count", True, f"✅ Found {count} wave plans as expected"
            )
    ...
    async def _validate_multi_tenant_isolation(self, session: AsyncSession):
        ...
        result = await session.execute(select(ClientAccount.id, ClientAccount.name))
    ...
    async def _validate_data_completeness(self, session: AsyncSession):
        ...
        for model, field, description in critical_checks:
            result = await session.execute(
                select(func.count(model.id)).where(or_(field.is_(None), field == ""))
            )
            null_count = result.scalar()

            if null_count > 0:
                self._add_result(
                    f"completeness_{description.replace(' ', '_')}",
                    False,
    ...
    async def _validate_business_rules(self, session: AsyncSession):
        ...
        result = await session.execute(
            select(func.count(Asset.id)).where(Asset.discovery_timestamp > func.now())
        )
        future_discoveries = result.scalar()

        if future_discoveries > 0:
            self._add_result(
                "business_rule_discovery_dates",
                False,
    ...
    def _add_result(
        ...
        if self.verbose:
            status = (
                "✅ PASS" if passed else "❌ FAIL" if severity == "error" else "⚠️ WARN"
            )
    ...
    def _generate_report(self) -> Dict[str, Any]:
        ...
        report = {
            ...
            "summary": {
                ...
                "success_rate": (
                    (passed_count / len(self.results)) * 100 if self.results else 0
                ),
            },
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        if any("count" in r.name for r in failed_results):
            recommendations.append(
                "Review seeding scripts - record counts don't match expectations"
            )

        if any("foreign" in r.name or "orphaned" in r.name for r in failed_results):
            recommendations.append(
                "Check referential integrity - some foreign key relationships are broken"
            )

        if any("tenant" in r.name for r in failed_results):
            recommendations.append(
                "Multi-tenant isolation may be compromised - review data separation"
            )

        if any("complete" in r.name for r in failed_results):
            recommendations.append(
                "Some records are missing required fields - review data quality"
            )
        ...
        if not failed_results:
            recommendations.append(
                "All validations passed! Database seeding appears successful."
            )
...
async def main():
    ...
    parser.add_argument(
        "--fix-issues", action="store_true", help="Attempt to fix detected issues"
    )
