...
from sqlalchemy import and_, distinct, func, or_, select, text  # noqa: E402
...
class PerformanceValidator:
    ...
    async def _test_complex_joins(self, session: AsyncSession):
        ...
        result = await session.execute(
            ...
            .outerjoin(
                Asset.alias("dependent"),
                AssetDependency.depends_on_asset_id == Asset.alias("dependent").id,
            )
    ...
    async def _test_search_performance(self, session: AsyncSession):
        ...
        result = await session.execute(
            ...
            .where(
                and_(
                    or_(Asset.name.ilike("%prod%"), Asset.asset_type.ilike("%server%")),
    ...
    async def _test_reporting_queries(self, session: AsyncSession):
        ...
        result = await session.execute(
            select(
                ...
                func.count(distinct(Asset.id)).label("total_assets"),
                func.count(
                    distinct(func.case((Assessment.id.is_not(None), Asset.id)))
                ).label("assessed_assets"),
                func.count(
                    distinct(func.case((Migration.id.is_not(None), Asset.id)))
    ...
    async def _test_index_performance(self, session: AsyncSession):
        ...
        start_time = time.time()
        result = await session.execute(select(Asset).where(Asset.id == 1))
    ...
    async def _test_memory_usage(self, session: AsyncSession):
        ...
        start_time = time.time()
        result = await session.execute(select(Asset.id, Asset.name).order_by(Asset.id))
        ...
        report = {
            ...
            "category_performance": {
                name: {
                    ...
                    "avg_execution_time": (
                        round(mean([r.execution_time for r in results]), 3)
                        if results
                        else 0
                    ),
                    "max_execution_time": (
                        round(max([r.execution_time for r in results]), 3)
                        if results
                        else 0
                    ),
                }
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        if any("simple" in r.test_name for r in failed_results):
            recommendations.append(
                "Consider adding basic indexes on frequently queried columns"
            )

        if any("complex" in r.test_name for r in failed_results):
            recommendations.append(
                "Optimize complex joins - consider query restructuring or additional indexes"
            )

        if any("search" in r.test_name for r in failed_results):
            recommendations.append(
                "Implement full-text search indexes for text search operations"
            )

        if any("reporting" in r.test_name for r in failed_results):
            recommendations.append(
                "Consider implementing materialized views for complex reporting queries"
            )

        if any("bulk" in r.test_name for r in failed_results):
            recommendations.append(
                "Optimize bulk operations - consider batch processing or query optimization"
            )

        if any("memory" in r.test_name for r in failed_results):
            recommendations.append(
                "Implement proper pagination and result streaming for large datasets"
            )

        if not failed_results:
            recommendations.append(
                "Performance is excellent - maintain current optimization level"
            )
            recommendations.append(
...
def print_performance_report(report: Dict[str, Any]):
    ...
    if summary["failed_tests"] > 0:
        ...
        for result in failed:
            ...
            print(
                f"    Time: {result['execution_time']}s (threshold: {result['threshold']}s)"
            )
    ...
    if summary["failed_tests"] == 0:
        ...
        print(
            f"Found {summary['failed_tests']} tests exceeding performance thresholds."
        )
...
async def main():
    ...
    parser.add_argument(
        ...
        "-b",
        action="store_true",
        help="Benchmark mode (relaxed thresholds)",
    )
    ...
    validator = PerformanceValidator(
        verbose=args.verbose, benchmark_mode=args.benchmark
    )
