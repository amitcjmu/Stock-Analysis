...
class FullValidationSuite:
    ...
    async def _run_performance_tests(self):
        ...
        try:
            validator = PerformanceValidator(
                verbose=False, benchmark_mode=self.quick_mode
            )
    ...
    def _generate_consolidated_report(self, start_time: datetime) -> Dict[str, Any]:
        ...
        if "seeding" in self.results and "summary" in self.results["seeding"]:
            ...
            if seeding_summary.get("failed", 0) > 0:
                critical_issues.append(
                    f"Database seeding has {seeding_summary['failed']} failed validations"
                )
        ...
        if "isolation" in self.results and "summary" in self.results["isolation"]:
            ...
            if isolation_summary.get("total_data_leaks", 0) > 0:
                critical_issues.append(
                    f"Multi-tenant isolation has {isolation_summary['total_data_leaks']} data leaks"
                )

        # Performance test metrics
        if "performance" in self.results and not self.results["performance"].get(
            "skipped", False
        ):
            if "summary" in self.results["performance"]:
                ...
                if (
                    perf_summary.get("failed_tests", 0) > 5
                ):  # Allow some performance variance
                    critical_issues.append(
                        f"Performance validation has {perf_summary['failed_tests']} slow queries"
                    )
        ...
        consolidated_report = {
            ...
            "component_results": {
                ...
                "performance_validation": self._extract_component_summary(
                    "performance"
                ),
            },
    ...
    def _extract_component_summary(self, component: str) -> Dict[str, Any]:
        ...
        if component == "seeding":
            ...
            return {
                "status": (
                    "PASS" if summary.get("failed_tests", 1) <= 2 else "FAIL"
                ),  # Allow minor perf issues
                "tests": summary.get("total_tests", 0),
                "passed": summary.get("passed_tests", 0),
                "failed": summary.get("failed_tests", 0),
                "avg_time": summary.get("avg_execution_time", 0),
                "performance_assessment": result.get(
    ...
    def _generate_overall_recommendations(self) -> List[str]:
        ...
        if (
            "seeding" in self.results
            and self.results["seeding"].get("summary", {}).get("failed", 0) > 0
        ):
            recommendations.append(
                "🔧 Fix database seeding issues before proceeding with demo"
            )

        # Check isolation results
        if (
            "isolation" in self.results
            and self.results["isolation"].get("summary", {}).get("total_data_leaks", 0)
            > 0
        ):
            recommendations.append(
                "🚨 URGENT: Address multi-tenant data leaks immediately"
            )

        # Check performance results
        if (
            "performance" in self.results
            and not self.results["performance"].get("skipped", False)
            and self.results["performance"].get("summary", {}).get("failed_tests", 0)
            > 5
        ):
            recommendations.append(
                "⚡ Consider performance optimizations for better demo experience"
            )
    ...
    async def _export_reports(self, consolidated_report: Dict[str, Any]):
        ...
        consolidated_path = os.path.join(
            self.export_dir, f"consolidated_validation_{timestamp}.json"
        )
        ...
        for component, results in self.results.items():
            if "error" not in results and not results.get("skipped", False):
                component_path = os.path.join(
                    self.export_dir, f"{component}_validation_{timestamp}.json"
                )
        ...
        summary_path = os.path.join(
            self.export_dir, f"validation_summary_{timestamp}.txt"
        )
        with open(summary_path, "w") as f:
            f.write("AI FORCE MIGRATION PLATFORM - VALIDATION SUMMARY\n")
            f.write("=" * 50 + "\n\n")
            f.write(
                f"Generated: {consolidated_report['validation_metadata']['completed_at']}\n"
            )
            f.write(
                f"Duration: {consolidated_report['validation_metadata']['total_duration_seconds']}s\n"
            )
            f.write(f"Status: {consolidated_report['overall_summary']['status']}\n")
            f.write(
                f"Success Rate: {consolidated_report['overall_summary']['overall_success_rate']}%\n"
            )
            f.write(
                f"Ready for Demo: {consolidated_report['overall_summary']['ready_for_demo']}\n\n"
            )
...
async def main():
    ...
    parser.add_argument(
        ...
        default="/tmp/validation_reports",
    )
