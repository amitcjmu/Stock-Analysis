...
class MultiTenantIsolationTester:
    ...
    async def run_isolation_tests(
        ...
        async with AsyncSessionLocal() as session:
            ...
            if target_client_id:
                ...
                client_info = next(
                    (c for c in self.client_accounts if c[0] == target_client_id), None
                )
                if client_info:
                    await self._test_client_isolation(
    ...
    async def _load_client_accounts(self, session: AsyncSession):
        ...
        result = await session.execute(
            select(ClientAccount.id, ClientAccount.name).order_by(ClientAccount.name)
        )
    ...
    async def _test_client_isolation(
        self, session: AsyncSession, client_id: int, client_name: str
    ):
    ...
    async def _test_engagement_isolation(
        self, session: AsyncSession, client_id: int, client_name: str
    ):
    ...
    async def _test_asset_isolation(
        self, session: AsyncSession, client_id: int, client_name: str
    ):
    ...
    async def _test_user_isolation(
        self, session: AsyncSession, client_id: int, client_name: str
    ):
        """Test user access isolation."""
        # Users associated with this client
        result = await session.execute(
            select(func.count(distinct(UserAccountAssociation.user_id))).where(
        ...
        result = await session.execute(
            select(
                ...
                func.count(distinct(UserAccountAssociation.client_account_id)).label(
                    "client_count"
                ),
            )
        ...
        for user_id, client_count in multi_client_users:
            result = await session.execute(
                select(func.count(UserRole.id)).where(
                    and_(UserRole.user_id == user_id, UserRole.role == "system_admin")
                )
            )
            if result.scalar() > 0:
    ...
    async def _test_discovery_flow_isolation(
        self, session: AsyncSession, client_id: int, client_name: str
    ):
    ...
    async def _test_data_import_isolation(
        self, session: AsyncSession, client_id: int, client_name: str
    ):
    ...
    async def _test_assessment_isolation(
        self, session: AsyncSession, client_id: int, client_name: str
    ):
    ...
    async def _test_migration_isolation(
        self, session: AsyncSession, client_id: int, client_name: str
    ):
        ...
        result = await session.execute(
            select(func.count(WavePlan.id)).where(
    ...
    async def _test_cross_client_contamination(self, session: AsyncSession):
        ...
        if contaminated_assets:
            self._add_result(
                ...
                details={
                    "contaminated_assets": [
                        dict(r._mapping) for r in contaminated_assets[:10]
                    ]
                },
            )
        else:
    ...
    async def _test_permission_boundaries(self, session: AsyncSession):
        ...
        result = await session.execute(
            select(UserRole.role, func.count(UserRole.id)).group_by(UserRole.role)
        )
        ...
        missing_roles = [
            role for role in expected_roles if role not in role_distribution
        ]
        ...
        for admin_id in system_admin_users:
            result = await session.execute(
                select(
                    func.count(distinct(UserAccountAssociation.client_account_id))
    ...
    def _generate_security_recommendations(self) -> List[str]:
        ...
        if any("contamination" in r.test_name for r in failed_results):
            recommendations.append(
                "URGENT: Fix cross-client data contamination before deployment"
            )
        ...
        if any(r.data_leak_count > 0 for r in failed_results):
            recommendations.append(
                "Investigate and remediate all data leakage incidents"
            )

        if not failed_results:
            recommendations.append("Multi-tenant isolation is working correctly")
            recommendations.append(
                "Continue regular isolation testing as part of CI/CD"
            )
