...
async def test_asset_repository_master_flow_methods():
    ...
    async with AsyncSessionLocal() as session:
        ...
        if master_flow_test:
            assets = await asset_repo.get_by_master_flow(str(master_flow_test.master_flow_id))
        ...
        if master_flow_test:
            summary = await asset_repo.get_master_flow_summary(str(master_flow_test.master_flow_id))
            print(f"      ✅ Master flow summary: {summary['total_assets']} assets, {len(summary['phases'])} phases")
        ...
        print(f"      ✅ Cross-phase analytics: {len(analytics['master_flows'])} master flows, {len(analytics['phase_transitions'])} transitions")
...
async def test_discovery_flow_repository_master_coordination():
    ...
    async with AsyncSessionLocal() as session:
        ...
        discovery_repo = DiscoveryFlowRepository(session, str(test_context.client_account_id), str(test_context.engagement_id))
        ...
        if master_flow_test:
            flow = await discovery_repo.get_by_master_flow_id(str(master_flow_test.master_flow_id))
            if flow:
        ...
        print(f"      ✅ Coordination summary: {summary['total_discovery_flows']} flows, {summary['coordination_percentage']:.1f}% coordinated")
        ...
        if flow_test:
            print(f"      ✅ Found test flow: {flow_test.flow_id}, master: {flow_test.master_flow_id}")
...
async def test_cross_table_relationships():
    ...
    async with AsyncSessionLocal() as session:
        ...
        for rel in relationships:
            print(f"      ✅ Master Flow {str(rel.master_flow_id)[:8]}... -> Flow '{rel.flow_name}' -> {rel.asset_count} assets")
        ...
        for stat in phase_stats:
            print(f"      ✅ Phase transition {stat.source_phase} → {stat.current_phase}: {stat.asset_count} assets")
        ...
        coordination_rate = (completeness.flows_with_master / completeness.total_discovery_flows * 100) if completeness.total_discovery_flows > 0 else 0
...
async def test_error_handling_and_edge_cases():
    ...
    async with AsyncSessionLocal() as session:
        ...
        analytics = await asset_repo.get_cross_phase_analytics()
        print(f"      ✅ Analytics completed: {len(analytics['master_flows'])} flows analyzed")
...
async def test_performance_and_scalability():
    ...
    async with AsyncSessionLocal() as session:
        ...
        print(f"      ✅ Queried {total_assets} assets, {unique_masters} master flows in {query_time:.3f}s")
        ...
        print(f"      ✅ Complex join on {len(join_results)} master flows in {join_time:.3f}s")
        ...
        if client_context:
            ...
            print(f"      ✅ Found {len(analytics['master_flows'])} master flows, {len(analytics['phase_transitions'])} transitions")
