    ...
    async with AsyncSessionLocal() as db:
        ...
        if ext and ext.flow_persistence_data:
            ...
            ext.flow_persistence_data['field_mappings'] = {
                'mappings': {
                    'Name': {'source_column': 'Name', 'asset_field': 'name', 'confidence': 95, 'match_type': 'direct'},
                    'CI Type': {'source_column': 'CI Type', 'asset_field': 'asset_type', 'confidence': 90, 'match_type': 'semantic'},
                    'CI ID': {'source_column': 'CI ID', 'asset_field': 'asset_id', 'confidence': 100, 'match_type': 'direct'},
                    'Version/Hostname': {'source_column': 'Version/Hostname', 'asset_field': 'version', 'confidence': 85, 'match_type': 'semantic'},
                    'IP Address': {'source_column': 'IP Address', 'asset_field': 'ip_address', 'confidence': 100, 'match_type': 'direct'},
                    'OS': {'source_column': 'OS', 'asset_field': 'operating_system', 'confidence': 95, 'match_type': 'semantic'},
                    'CPU (Cores)': {'source_column': 'CPU (Cores)', 'asset_field': 'cpu_cores', 'confidence': 90, 'match_type': 'semantic'},
                    'RAM (GB)': {'source_column': 'RAM (GB)', 'asset_field': 'ram_gb', 'confidence': 90, 'match_type': 'semantic'},
                    'Storage (GB)': {'source_column': 'Storage (GB)', 'asset_field': 'storage_gb', 'confidence': 90, 'match_type': 'semantic'},
                    'Location': {'source_column': 'Location', 'asset_field': 'data_center', 'confidence': 85, 'match_type': 'semantic'},
                    'Owner': {'source_column': 'Owner', 'asset_field': 'owner', 'confidence': 100, 'match_type': 'direct'},
                    'Environment': {'source_column': 'Environment', 'asset_field': 'environment', 'confidence': 100, 'match_type': 'direct'},
                    'Status': {'source_column': 'Status', 'asset_field': 'status', 'confidence': 100, 'match_type': 'direct'},
                    'Related CI': {'source_column': 'Related CI', 'asset_field': 'related_assets', 'confidence': 80, 'match_type': 'semantic'}
                },
                'agent_insights': {
                    'mapping_quality': 'High quality mappings achieved with 92% average confidence',
                    'recommendations': ['All critical fields mapped successfully', 'Consider reviewing version field mapping']
                },
                'unmapped_fields': [],
                'confidence_scores': {
                    'Name': 0.95,
        ...
        if flow:
            ...
            flow.crewai_state_data['field_mappings'] = ext.flow_persistence_data['field_mappings'] if ext else {}
