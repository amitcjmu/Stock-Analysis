    ...
    if isinstance(data, list):
        ...
        for key, value in data.items():
            ...
            if key == "score" and isinstance(value, (int, float)) and value > 100:
                changes.append(
                    {"path": current_path, "old_value": value, "new_value": 100.0}
                )
                data[key] = 100.0
    ...
    async with AsyncSessionLocal() as session:
        ...
        for rec in recommendations:
            ...
            if changes_made:
                total_fixed += 1
                print(
                    f"  ✅ FIXED {len(changes_made)} score(s) in recommendation {rec.id}:"
                )
        ...
        if total_fixed > 0:
            ...
            try:
                ...
                if remaining_issues:
                    print(
                        f"⚠️  Found {len(remaining_issues)} records still containing '108.0':"
                    )
