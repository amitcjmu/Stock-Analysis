...
class MasterFlowBenchmark:
        ...
        async with AsyncSession(self.async_engine) as session:
            ...
            for i in range(0, NUM_FLOWS, batch_size):
                ...
                for j in range(min(batch_size, NUM_FLOWS - i)):
                    flow_created = datetime.utcnow() - timedelta(days=random.randint(0, 365))
                    flows.append({
                        'id': str(uuid.uuid4()),
                        ...
                        'updated_at': flow_created + timedelta(hours=random.randint(1, 48)),
                        'phase_transitions': [
                            {'phase': 'init', 'timestamp': flow_created.isoformat()},
                            {'phase': 'phase1', 'timestamp': (flow_created + timedelta(hours=1)).isoformat()}
                        ],
    ...
    async def benchmark_query(self, name: str, query: str, params: Dict[str, Any] = None) -> Dict[str, Any]:
    ...
    def print_results(self):
        ...
        print("="*80)
        print(f"Test Configuration: {NUM_FLOWS} flows, {NUM_QUERIES} queries per benchmark")
        print("="*80 + "\n")
        
        headers = ["Query", "Avg (ms)", "Min (ms)", "Max (ms)", "Median (ms)", "StdDev (ms)"]
        rows = []
        ...
        # Index effectiveness
        indexed_queries = ['Flow Type + Status Filter', 'Client + Status Filter', 'Recent Flows']
