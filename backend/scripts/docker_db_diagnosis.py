...
async def check_database_connection():
    ...
    try:
        ...
        logger.info(f"🔍 Connecting to database: {db_url.split('@')[1] if '@' in db_url else 'hidden'}")
...
async def check_table_structure():
    ...
    try:
        ...
        with engine.connect() as conn:
            ...
            expected_columns = [
                'flow_id', 'user_id', 'progress_percentage', 'phase_completion',
                'crew_status', 'field_mappings', 'cleaned_data', 'asset_inventory',
                'dependencies', 'technical_debt', 'data_quality_metrics',
                'agent_insights', 'success_criteria', 'errors', 'warnings',
                'workflow_log', 'discovery_summary', 'assessment_flow_package',
                'database_assets_created', 'database_integration_status',
                'learning_scope', 'memory_isolation_level', 'shared_memory_id',
...
def test_crewai_llm():
    ...
    try:
        ...
        llm = get_crewai_llm()
        logger.info(f"✅ CrewAI LLM configured: {llm.model if hasattr(llm, 'model') else 'Unknown model'}")
