    ...
    try:
        ...
        async with AsyncSessionLocal() as session:
            ...
            raw_query = select(RawImportRecord).where(
                ...
                == "73dee5f1-6a01-43e3-b1b8-dbe6c66f2990",
            ...
            for record in raw_records:
                try:
                    ...
                    print(
                        f"🔄 Processing record {record.row_number}: {raw_data.get('Asset_Name', 'Unknown')}"
                    )
                    ...
                    asset = Asset(
                        ...
                        name=extract_asset_name(
                            mapped_data, raw_data, record.row_number
                        ),
                        ...
                        or raw_data.get("Operating_System"),
                        os_version=mapped_data.get("os_version")
                        or raw_data.get("OS_Version"),
                        cpu_cores=safe_int(
                            mapped_data.get("cpu_cores") or raw_data.get("CPU_Cores")
                        ),
                        memory_gb=safe_float(
                            mapped_data.get("memory_gb") or raw_data.get("RAM_GB")
                        ),
                        storage_gb=safe_float(
                            mapped_data.get("storage_gb") or raw_data.get("Storage_GB")
                        ),
                        # Hardware details
                        manufacturer=mapped_data.get("manufacturer")
                        or raw_data.get("Manufacturer"),
                        model=mapped_data.get("model") or raw_data.get("Model"),
                        serial_number=mapped_data.get("serial_number")
                        or raw_data.get("Serial_Number"),
                        # Environment and business info
                        environment=mapped_data.get("environment")
                        or raw_data.get("DR_Tier", "Unknown"),
                        location=mapped_data.get("location")
                        or raw_data.get("Location_DataCenter"),
                        business_owner=mapped_data.get("business_owner")
                        or raw_data.get("Application_Owner"),
                        ...
                        custom_attributes={
                            ...
                            "migration_readiness_score": raw_data.get(
                                "Cloud_Migration_Readiness_Score", "Unknown"
                            ),
                        },
                        # Audit
                    ...
                    record.processing_notes = (
                        "Processed by manual asset creation script"
                    )
                    ...
                    print(
                        f"✅ Created asset {asset.id}: {asset.name} ({asset.asset_type})"
                    )
            ...
            for asset in all_assets[-assets_created:]:  # Show newly created ones
                print(
                    f"   - {asset.name} ({asset.asset_type}) - {asset.environment} - {asset.discovery_method}"
                )
...
def apply_field_mappings(
    raw_data: Dict[str, Any], field_mappings: Dict[str, str]
) -> Dict[str, Any]:
...
def extract_asset_name(
    mapped_data: Dict[str, Any], raw_data: Dict[str, Any], row_number: int
) -> str:
...
def determine_asset_type(mapped_data: Dict[str, Any], raw_data: Dict[str, Any]) -> str:
    ...
    asset_type = (
        mapped_data.get("asset_type")
        or raw_data.get("Asset_Type")
