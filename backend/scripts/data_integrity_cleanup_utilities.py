...
class DataIntegrityCleanupUtilities:
    ...
    async def cleanup_session_references(self, session: Session) -> CleanupOperation:
        ...
        for row in session_refs.fetchall():
            ...
            if count > 0:
                ...
                self.logger.info(
                    f"Found {count} records with {column_name} in {table_name}"
                )
    ...
    async def fix_missing_tenant_contexts(self, session: Session) -> CleanupOperation:
        ...
        if not self.dry_run and affected_count > 0:
            ...
            await session.commit()
            self.logger.info(
                f"Fixed tenant context for {affected_count} raw import records"
            )
    ...
    async def deduplicate_flows(self, session: Session) -> CleanupOperation:
        ...
        count_result = await session.execute(
            text(
                duplicates_sql.replace(
                    "SELECT COUNT(*) FROM to_delete", "SELECT COUNT(*) FROM to_delete"
                )
            )
        )
    ...
    async def remove_orphaned_records(
        self,
        session: Session,
        table_name: str,
        foreign_key: str,
        ...
        operation = CleanupOperation(
            ...
            sql_command=(
                delete_sql if orphaned_count > 0 else "-- No orphaned records found"
            ),
            rollback_command="Manual restoration from backup required",
        )
    ...
    async def create_backup(self, session: Session, backup_name: str = None) -> str:
        ...
        backup_commands.append(
            "-- Run this script to restore the database to this state"
        )
        backup_commands.append("")
    ...
    async def audit_flow_health(self, session: Session) -> Dict[str, Any]:
        ...
        for table in tables_to_check:
            ...
            health_report["tables"][table] = {
                ...
                "oldest_record": (
                    stats.oldest_record.isoformat() if stats.oldest_record else None
                ),
                "newest_record": (
                    stats.newest_record.isoformat() if stats.newest_record else None
                ),
        ...
        for rel in relationships:
            ...
            integrity_score = (
                ((total_count - orphaned_count - missing_count) / total_count * 100)
                if total_count > 0
                else 100
            )
            ...
            if orphaned_count > 0:
                health_report["issues"].append(
                    f"{orphaned_count} orphaned records in {rel['child_table']}"
                )
            if missing_count > 0:
                health_report["issues"].append(
                    f"{missing_count} records missing {rel['child_key']} in {rel['child_table']}"
                )
            ...
            "operations": [
                {
                    ...
                    "sql_command": (
                        op.sql_command if self.verbose else "Available in verbose mode"
                    ),
                    "rollback_command": (
                        op.rollback_command
                        if self.verbose
                        else "Available in verbose mode"
                    ),
                }
...
async def main():
    ...
    utilities = DataIntegrityCleanupUtilities(
