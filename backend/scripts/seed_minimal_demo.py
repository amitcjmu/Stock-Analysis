...
async def seed_minimal_demo_data():
    ...
    async with AsyncSessionLocal() as session:
        ...
        for flow_data in demo_flows:
            flow = DiscoveryFlow(
                **flow_data,
                data_import_completed=True if flow_data["progress_percentage"] >= 16.7 else False,
                field_mapping_completed=True if flow_data["progress_percentage"] >= 33.3 else False,
                data_cleansing_completed=True if flow_data["progress_percentage"] >= 50.0 else False,
                asset_inventory_completed=True if flow_data["progress_percentage"] >= 66.7 else False,
                dependency_analysis_completed=True if flow_data["progress_percentage"] >= 83.3 else False,
                tech_debt_assessment_completed=True if flow_data["progress_percentage"] >= 100.0 else False,
