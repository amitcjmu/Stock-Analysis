...
async def fix_engagement_data_association():
    ...
    async with AsyncSessionLocal() as session:
        ...
        wrong_engagement = (
            "baf640df-433c-4bcd-8c8f-7b01c12e9005"  # Debug Test Engagement
        )
        correct_engagement = (
            "3362a198-c917-459c-be10-b10e19b1810e"  # Azure Transformation 2
        )
        ...
        for imp in wrong_imports:
            print(
                f"   - {imp.id}: {imp.source_filename} ({imp.total_records} records, {imp.created_at})"
            )

        # Ask for confirmation
        choice = (
            input("\n🤔 Move these imports to Azure Transformation 2? (y/N): ")
            .strip()
            .lower()
        )
        if choice != "y":
        ...
        for imp in wrong_imports:
            try:
                ...
                print(
                    f"   ✅ Updated import and {raw_records_result.rowcount} raw records"
                )
                moved_count += 1
