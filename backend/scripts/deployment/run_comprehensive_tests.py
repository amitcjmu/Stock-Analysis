...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("comprehensive_tests.log"), logging.StreamHandler()],
)
...
class ComprehensiveTestRunner:
        ...
        try:
            if service == "backend":
                ...
                response = requests.get(
                    f"{self.config['backend_url']}/health", timeout=10
                )
        ...
        try:
            ...
            result = subprocess.run(
                [
                    ...
                    "python",
                    "-m",
                    "app.core.database_initialization",
                ],
                timeout=120,
                capture_output=True,
                text=True,
        ...
        try:
            ...
            for endpoint in endpoints:
                ...
                if response.status_code >= 500:
                    logger.error(
                        f"❌ Master Flow Orchestrator endpoint {endpoint} failed: {response.status_code}"
                    )
                    return False
    ...
    async def _run_unit_tests(self) -> bool:
        ...
        try:
            result = await self._run_pytest_suite(
                "tests/unit/", "unit_tests", markers="unit"
            )
    ...
    async def _run_integration_tests(self) -> bool:
        ...
        try:
            result = await self._run_pytest_suite(
                "tests/integration/", "integration_tests", markers="integration"
            )
    ...
    async def _run_master_flow_orchestrator_tests(self) -> bool:
        ...
        try:
            ...
            total_success = (
                result["success"]
                and registry_result["success"]
                and state_result["success"]
            )
    ...
    async def _run_api_tests(self) -> bool:
        ...
        try:
            result = await self._run_pytest_suite(
                "tests/api/", "api_tests", markers="api"
            )
        ...
        try:
            ...
            try:
                if result.stdout:
                    ...
                    e2e_result["duration"] = playwright_results.get("stats", {}).get(
                        "duration", 0
                    )
    ...
    async def _run_performance_tests(self) -> bool:
        ...
        try:
            result = await self._run_pytest_suite(
                "tests/performance/", "performance_tests", markers="performance"
            )
    ...
    async def _run_security_tests(self) -> bool:
        ...
        try:
            result = await self._run_pytest_suite(
                "tests/security/", "security_tests", markers="security"
            )
    ...
    async def _run_pytest_suite(
        self, test_path: str, suite_name: str, markers: str = None
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            result = subprocess.run(
                cmd, timeout=self.config["test_timeout"], capture_output=True, text=True
            )
            ...
            if test_result["success"]:
                logger.info(
                    f"✅ {suite_name} passed ({test_result['passed']} tests, {duration:.2f}s)"
                )
            else:
                logger.error(
                    f"❌ {suite_name} failed ({test_result['failed']} failures)"
                )
                if result.stderr:
            ...
            logger.error(
                f"❌ {suite_name} timed out after {self.config['test_timeout']} seconds"
            )
            return {
                "success": False,
