...
class DataIntegrityValidator:
    ...
    def __init__(self, client_account_id: Optional[str] = None, 
                 engagement_id: Optional[str] = None, fix_issues: bool = False):
        """
        ...
        async with AsyncSessionLocal() as session:
            try:
                ...
                results["summary"] = {
                    ...
                    "overall_status": "passed" if failed_tests == 0 and len(self.issues_found) == 0 else "failed"
                }
    ...
    async def _validate_foreign_key_relationships(self, session: AsyncSession) -> Dict[str, Any]:
    ...
    async def _validate_cascade_configuration(self, session: AsyncSession) -> Dict[str, Any]:
    ...
    async def _test_constraint_enforcement(self, session: AsyncSession) -> Dict[str, Any]:
        ...
        try:
            ...
            failed_tests = [test for test in constraint_tests if test["status"] == "failed"]
            if failed_tests:
            ...
            return {
                ...
                "tests_passed": len([t for t in constraint_tests if t["status"] == "passed"]),
    ...
    async def _validate_data_consistency(self, session: AsyncSession) -> Dict[str, Any]:
        ...
        try:
            ...
            consistency_metrics = {
                "orphaned_discovery_flows": consistency_stats.orphaned_discovery_flows or 0,
                "orphaned_data_imports": consistency_stats.orphaned_data_imports or 0,
                "discovery_flows_missing_imports": consistency_stats.discovery_flows_missing_imports or 0,
                "raw_records_missing_assets": consistency_stats.raw_records_missing_assets or 0
            }
    ...
    async def _validate_query_performance(self, session: AsyncSession) -> Dict[str, Any]:
        ...
        try:
            ...
            asset_aggregation_query = text(f"""
                SELECT 
            ...
            failed_performance_tests = [test for test in performance_tests if not test["passed"]]
            ...
            for test in performance_tests:
                self.performance_metrics[f"{test['test']}_time"] = test["execution_time_seconds"]
    ...
    async def _validate_tenant_isolation(self, session: AsyncSession) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.client_account_id:
                ...
                result = await session.execute(cross_tenant_query, {"client_account_id": self.client_account_id})
                ...
                isolation_tests.append({
                    ...
                    "status": "passed" if len(cross_tenant_issues) == 0 else "failed",
            ...
            failed_tests = [test for test in isolation_tests if test["status"] == "failed"]
            
            return {
                "status": "passed" if len(failed_tests) == 0 else "failed",
                "execution_time_seconds": execution_time,
                "tests_run": len(isolation_tests),
                "tests_passed": len([t for t in isolation_tests if t["status"] == "passed"]),
...
async def main():
    ...
    parser = argparse.ArgumentParser(description="Validate discovery flow data integrity")
    parser.add_argument("--client-id", help="Limit validation to specific client account ID")
    parser.add_argument("--engagement-id", help="Limit validation to specific engagement ID")
    ...
    parser.add_argument("--monitoring-queries", action="store_true", help="Generate monitoring queries")
    ...
    try:
        ...
        if args.monitoring_queries:
            ...
            monitoring_file = args.output.replace('.json', '_monitoring.sql') if args.output else 'monitoring_queries.sql'
