...
"""
    ...
    async def _verify_backup(self) -> bool:
        ...
        try:
            ...
            if create_result.returncode != 0:
                logger.error(
                    f"❌ Could not create test database: {create_result.stderr}"
                )
                return False
            ...
            try:
                ...
                if restore_result.returncode == 0:
                    ...
                    logger.error(
                        f"❌ Backup verification failed: {restore_result.stderr}"
                    )
                    return False
    ...
    async def _wait_for_backend_health(self, environment: str) -> bool:
        ...
        if environment == "green":
            health_url = (
                f"{self.config['production_url'].replace('api', 'api-green')}/health"
            )
        else:
                    ...
                    async with session.get(
                        ...
                        if response.status == 200:
                            ...
                            if health_data.get("status") == "healthy":
                                logger.info(
                                    f"✅ {environment} backend health check passed"
                                )
                                return True
        ...
        logger.error(
            f"❌ {environment} backend health check timed out after {timeout} seconds"
        )
        return False
        ...
        try:
            ...
            result = secure_subprocess_run(
                [
                    "docker",
                    "exec",
                    f"migration_{container_name}_prod",
                    "alembic",
                    "upgrade",
                    "head",
                ],
            ...
            if migration_result.returncode != 0:
                logger.warning(
                    f"⚠️  Master flow migration warning: {migration_result.stderr}"
                )
    ...
    async def _validate_migration_success(self) -> bool:
        ...
        try:
            ...
            result = secure_subprocess_run(
                ...
                env={
                    **os.environ,
                    "STAGING_DATABASE_URL": os.getenv("PRODUCTION_DATABASE_URL"),
                },
            )
    ...
    async def _wait_for_frontend_health(self, environment: str) -> bool:
        ...
        logger.error(
            f"❌ {environment} frontend health check timed out after {timeout} seconds"
        )
        return False
    ...
    async def _blue_green_traffic_switch(self) -> bool:
        ...
        for percentage in self.config["traffic_shift_steps"]:
            ...
            monitoring_duration = (
                120 if percentage < 100 else 300
            )  # 2 minutes, 5 minutes for 100%
    ...
    async def _monitor_deployment_health(self, duration: int) -> bool:
        ...
        while time.time() < end_time:
            ...
            if error_rate > self.config["error_rate_threshold"]:
                logger.error(
                    f"❌ Error rate too high: {error_rate:.2%} > {self.config['error_rate_threshold']:.2%}"
                )
            ...
            if response_time > self.config["response_time_threshold"]:
                logger.error(
                    f"❌ Response time too high: {response_time:.2f}s > {self.config['response_time_threshold']:.2f}s"
                )
            ...
            logger.info(
                f"📊 Health check: Error rate {error_rate:.2%}, Response time {response_time:.2f}s"
            )
...
"""
    ...
    async def _generate_deployment_report(self) -> None:
        ...
        report = {
            ...
            "success": len(
                [
                    issue
                    for issue in self.deployment_state["issues"]
        ...
        report_file = (
            self.logs_path / f"production_deployment_report_{self.deployment_id}.json"
        )
        ...
        logger.info(
            f"Traffic: {self.deployment_state['traffic_percentage']}% on new environment"
        )
    ...
    async def _handle_deployment_failure(self, error: Exception) -> None:
        ...
        # Save failure report
        failure_report_file = (
            self.logs_path / f"production_deployment_failure_{self.deployment_id}.json"
        )
