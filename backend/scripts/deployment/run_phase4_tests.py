...
class Phase4TestRunner:
    ...
    def __init__(self, output_dir: Optional[str] = None):
        ...
        """
        self.output_dir = Path(output_dir) if output_dir else BACKEND_DIR / "test_results"
    ...
    async def _run_integration_tests(self) -> Dict[str, Any]:
        ...
        try:
            ...
            integration_cmd = [
                ...
                str(TESTS_DIR / "integration" / "test_discovery_flow_data_integrity.py"),
    ...
    async def _run_performance_tests(self) -> Dict[str, Any]:
        ...
        try:
            ...
            performance_cmd = [
                ...
                "--benchmark",
                "--output", str(self.output_dir / "performance_benchmark_results.json")
            ]
            ...
            if benchmark_file.exists():
                try:
                    ...
                    logger.warning(f"Could not parse performance benchmark results: {e}")
    ...
    async def _run_deployment_validation(self) -> Dict[str, Any]:
        ...
        try:
            ...
            validation_cmd = [
                ...
                "--output", str(self.output_dir / "deployment_validation_results.json"),
                "--monitoring-queries"
            ]
            ...
            if validation_file.exists():
                try:
                    ...
                    logger.warning(f"Could not parse deployment validation results: {e}")
    ...
    async def _generate_monitoring_queries(self) -> Dict[str, Any]:
        ...
        try:
            ...
            monitoring_file = self.output_dir / "deployment_validation_results_monitoring.sql"
...
"""
        ...
        for suite_name, suite_results in test_suites.items():
            suite_status = suite_results.get("status", "unknown")
            css_class = "passed" if suite_status == "passed" else "failed" if suite_status in ["failed", "error", "timeout"] else "warning"
...
async def main():
    ...
    parser = argparse.ArgumentParser(description="Run Phase 4 Discovery Flow Data Integrity Tests")
    parser.add_argument("--all", action="store_true", help="Run all test suites")
    parser.add_argument("--integration", action="store_true", help="Run integration tests only")
    parser.add_argument("--api", action="store_true", help="Run API consistency tests only")  
    parser.add_argument("--performance", action="store_true", help="Run performance tests only")
    parser.add_argument("--deployment", action="store_true", help="Run deployment validation only")
    parser.add_argument("--output-dir", help="Output directory for test results")
    parser.add_argument("--generate-report", action="store_true", help="Generate HTML and summary reports")
    
    args = parser.parse_args()
    
    if not any([args.all, args.integration, args.api, args.performance, args.deployment]):
    ...
    try:
        if args.all:
            ...
            if args.integration:
                logger.info("🧪 Running integration tests...")
                results["test_suites"]["integration"] = await runner._run_integration_tests()
            
            if args.api:
                logger.info("🔗 Running API consistency tests...")
                results["test_suites"]["api_consistency"] = await runner._run_api_consistency_tests()
            
            if args.performance:
                logger.info("⚡ Running performance tests...")
                results["test_suites"]["performance"] = await runner._run_performance_tests()
            
            if args.deployment:
                logger.info("🚀 Running deployment validation...")
                results["test_suites"]["deployment"] = await runner._run_deployment_validation()
        ...
        logger.info(f"📊 Test Summary: {summary.get('passed_test_suites', 0)}/{summary.get('total_test_suites', 0)} test suites passed")
        ...
        if summary.get("overall_status") == "passed":
            ...
            logger.warning("⚠️ Some Phase 4 tests failed - Review issues before deployment")
            exit(1)
        else:
