...
class LoadTestRunner:
    ...
    async def run_load_testing(self) -> bool:
        ...
        try:
            ...
            avg_response_time = (
                statistics.mean(self.test_results["metrics"]["response_times"])
                if self.test_results["metrics"]["response_times"]
                else 0
            )
            ...
            if success:
                ...
                logger.error(
                    f"❌ Load testing FAILED - Error rate: {error_rate:.2%}, Avg response time: {avg_response_time:.2f}s"
                )
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(
                    ...
                    if response.status >= 500:
                        logger.error(
                            f"❌ Master Flow Orchestrator not available: {response.status}"
                        )
                        return False
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                for scenario in self.scenarios:
                    response_time, success, error = await self._execute_scenario(
                        session, scenario
                    )

                    if success:
                        logger.info(
                            f"✅ Baseline {scenario['name']}: {response_time:.2f}s"
                        )
                    else:
                        logger.warning(
                            f"⚠️  Baseline {scenario['name']} failed: {error}"
                        )
    ...
    async def _run_sustained_load_test(self) -> bool:
        ...
        try:
            ...
            logger.info(
                f"🔄 Running {concurrent_users} concurrent users for {duration} seconds..."
            )
            ...
            for user_id in range(concurrent_users):
                task = asyncio.create_task(
                    self._sustained_user_scenario(end_time, user_id)
                )
    ...
    async def _run_spike_test(self) -> bool:
        ...
        try:
            ...
            logger.info(
                f"⚡ Spike test: {successful} successful, {failed} failed in {duration:.2f}s"
            )
    ...
    async def _run_flow_concurrency_test(self) -> bool:
        ...
        try:
            ...
            for i in range(concurrent_flows):
                task = asyncio.create_task(
                    self._create_test_flow(f"Concurrent Flow {i}")
                )
                flow_creation_tasks.append(task)

            created_flows = await asyncio.gather(
                *flow_creation_tasks, return_exceptions=True
            )

            # Count successful flow creations
            successful_flows = [
                f for f in created_flows if isinstance(f, dict) and "flow_id" in f
            ]
            ...
            if successful_flows:
                ...
                for flow in successful_flows[:10]:  # Test on first 10 flows
                    ...
                    operation_tasks.append(
                        asyncio.create_task(self._test_flow_operations(flow["flow_id"]))
                    )
        ...
        try:
                ...
                if self.created_flows:
                    ...
                    if isinstance(flow_result, dict) and "flow_id" in flow_result:
                        ...
                        return (
                            ...
                            False,
                            "No flow available for operation",
                        )
            ...
            if scenario["method"] == "GET":
                ...
                return (
                    time.time() - start_time,
                    False,
                    f"Unsupported method: {scenario['method']}",
                )
    ...
    async def _generate_load_test_report(self) -> None:
        ...
        if total_requests > 0:
            error_rate = (
                self.test_results["metrics"]["failed_requests"] / total_requests
            )
            self.test_results["metrics"]["error_rate"] = error_rate

        response_times = self.test_results["metrics"]["response_times"]
        if response_times:
            self.test_results["metrics"]["avg_response_time"] = statistics.mean(
                response_times
            )
            self.test_results["metrics"]["median_response_time"] = statistics.median(
                response_times
            )
            self.test_results["metrics"]["p95_response_time"] = self._percentile(
                response_times, 95
            )
            self.test_results["metrics"]["p99_response_time"] = self._percentile(
                response_times, 99
            )
        ...
        logger.info(
            f"Successful Requests: {self.test_results['metrics']['successful_requests']}"
        )
        logger.info(
            f"Failed Requests: {self.test_results['metrics']['failed_requests']}"
        )
        ...
        if response_times:
            logger.info(
                f"Avg Response Time: {self.test_results['metrics']['avg_response_time']:.2f}s"
            )
            logger.info(
                f"95th Percentile: {self.test_results['metrics']['p95_response_time']:.2f}s"
            )
            logger.info(
                f"99th Percentile: {self.test_results['metrics']['p99_response_time']:.2f}s"
            )

        logger.info(
            f"Concurrent Flows Created: {self.test_results['metrics']['concurrent_flows']}"
        )
        ...
        if error_rate <= self.config["error_threshold"]:
            logger.info(
                f"✅ Error Rate: {error_rate:.2%} (≤ {self.config['error_threshold']:.2%})"
            )
        else:
            logger.error(
                f"❌ Error Rate: {error_rate:.2%} (> {self.config['error_threshold']:.2%})"
        ...
        if response_times:
            ...
            if avg_time <= self.config["response_time_threshold"]:
                logger.info(
                    f"✅ Avg Response Time: {avg_time:.2f}s (≤ {self.config['response_time_threshold']:.2f}s)"
                )
            else:
                logger.error(
                    f"❌ Avg Response Time: {avg_time:.2f}s (> {self.config['response_time_threshold']:.2f}s)"
        ...
        # Overall assessment
        success = error_rate <= self.config["error_threshold"] and (
            not response_times
            or self.test_results["metrics"]["avg_response_time"]
    ...
    async def _handle_load_test_failure(self, error: Exception) -> None:
        ...
        # Save failure report
        failure_report_file = (
            self.results_path / f"load_test_failure_{self.test_id}.json"
        )
