...
async def prepare_for_railway_deployment():
    ...
    try:
        async with engine.begin() as conn:
            ...
            try:
                current_versions = await conn.execute(
                    sa.text(
            ...
            try:
                ...
                orphan_check = await conn.execute(
                    sa.text(
                        """
                ...
                if orphan_count > 0:
                    print(
                        f"   Found {orphan_count} orphaned discovery flows - fixing..."
                    )
                    ...
                    for flow in orphaned_flows.fetchall():
                        ...
                        """
                            ),
                            {
                                ...
                                or f"Migrated Flow {str(flow.flow_id)[:8]}",
