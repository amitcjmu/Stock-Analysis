...
class PerformanceMonitor:
    ...
    async def run_performance_benchmark(self) -> Dict[str, Any]:
        ...
        async with AsyncSessionLocal() as session:
            try:
                ...
                benchmark_results["summary"] = self._generate_benchmark_summary(benchmark_results["tests"])
                benchmark_results["recommendations"] = self._generate_performance_recommendations(benchmark_results["tests"])
    ...
    async def _benchmark_basic_queries(self, session: AsyncSession) -> Dict[str, Any]:
        ...
        test_results["discovery_flow_listing"] = {
            ...
            "alert": discovery_time_ms > self.alert_thresholds["query_execution_time_ms"]
        }
        ...
        import_query = select(
            DataImport.status,
            func.count(DataImport.id).label('count')
        ...
        asset_query = select(Asset).where(
            Asset.migration_readiness_score >= 0.7
        ).limit(50)
    ...
    async def _benchmark_join_queries(self, session: AsyncSession) -> Dict[str, Any]:
        ...
        discovery_import_query = select(
            ...
            DataImport, DiscoveryFlow.data_import_id == DataImport.id
        ).limit(100)
        ...
        test_results["discovery_flow_data_import_join"] = {
            ...
            "alert": discovery_import_time_ms > self.alert_thresholds["join_query_time_ms"]
        }
        
        # Test 2: Master flow with all related entities
        start_time = time.perf_counter()
        
        master_flow_full_query = select(
            ...
            DiscoveryFlow, DiscoveryFlow.master_flow_id == CrewAIFlowStateExtensions.flow_id
        ).outerjoin(
            DataImport, DiscoveryFlow.data_import_id == DataImport.id
        ).outerjoin(
            Asset, Asset.discovery_flow_id == DiscoveryFlow.flow_id
        ).group_by(
            CrewAIFlowStateExtensions.flow_id,
            CrewAIFlowStateExtensions.flow_name,
            CrewAIFlowStateExtensions.flow_status,
        ...
        asset_raw_query = select(
            ...
            RawImportRecord, Asset.raw_import_record_id == RawImportRecord.id
        ).where(
            Asset.migration_readiness_score >= 0.8
        ).limit(100)
    ...
    async def _benchmark_constraint_impact(self, session: AsyncSession) -> Dict[str, Any]:
        ...
        constraint_query = select(
            func.count(DataImport.id).label('valid_imports')
        ).select_from(
            DataImport
        ).join(
            CrewAIFlowStateExtensions,
        ...
        test_results["foreign_key_validation"] = {
            ...
            "alert": constraint_time_ms > self.alert_thresholds["constraint_check_time_ms"]
        }
        ...
        test_results["constraint_insert_validation"] = {
            ...
            "alert": constraint_insert_time_ms > self.alert_thresholds["constraint_check_time_ms"]
        }
        
        return test_results
    
    async def _benchmark_aggregation_queries(self, session: AsyncSession) -> Dict[str, Any]:
        ...
        test_results["discovery_flow_statistics"] = {
            ...
            "alert": discovery_stats_time_ms > self.alert_thresholds["query_execution_time_ms"]
        }
        ...
        asset_readiness_query = select(
            ...
            func.count(func.case(
                (Asset.migration_readiness_score >= 0.8, 1)
            )).label('high_readiness'),
            func.count(func.case(
                (Asset.migration_readiness_score >= 0.6, 1)
            )).label('medium_readiness'),
            func.count(func.case(
                (Asset.migration_readiness_score < 0.6, 1)
            )).label('low_readiness')
        )
        ...
        test_results["asset_readiness_analysis"] = {
            ...
            "alert": asset_readiness_time_ms > self.alert_thresholds["query_execution_time_ms"]
        }
        ...
        test_results["data_import_volume_analysis"] = {
            ...
            "alert": import_volume_time_ms > self.alert_thresholds["query_execution_time_ms"]
        }
        
        return test_results
    
    async def _benchmark_batch_operations(self, session: AsyncSession) -> Dict[str, Any]:
        ...
        test_results["batch_insert"] = {
            ...
            "alert": batch_insert_time_ms > self.alert_thresholds["batch_operation_time_ms"]
        }
        ...
        test_results["batch_update"] = {
            ...
            "alert": batch_update_time_ms > self.alert_thresholds["batch_operation_time_ms"]
        }
        ...
        test_results["batch_delete"] = {
            ...
            "alert": batch_delete_time_ms > self.alert_thresholds["batch_operation_time_ms"]
        }
        
        return test_results
    
    async def _benchmark_concurrent_operations(self, session: AsyncSession) -> Dict[str, Any]:
        ...
        successful_reads = sum(1 for result in concurrent_results if not isinstance(result, Exception))
        ...
        test_results["concurrent_reads"] = {
            ...
            "alert": concurrent_read_time_ms > self.alert_thresholds["query_execution_time_ms"] * 2
        }
    ...
    def _generate_benchmark_summary(self, test_results: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _generate_performance_recommendations(self, test_results: Dict[str, Any]) -> List[str]:
        ...
        for category, tests in test_results.items():
            for test_name, test_data in tests.items():
                if test_data.get("alert", False) and "execution_time_ms" in test_data:
                    slow_queries.append((category, test_name, test_data["execution_time_ms"]))
        ...
        slow_joins = [test for test, data in join_tests.items() 
        ...
        if slow_joins:
            ...
            recommendations.append(
                "  - Review foreign key indexes on joined columns"
            )
            recommendations.append(
        ...
        slow_batches = [test for test, data in batch_tests.items() 
        ...
        if slow_batches:
            ...
            recommendations.append(
                "  - Use bulk insert/update methods where possible"
            )
            recommendations.append(
                "  - Consider batching operations in smaller chunks"
            )
        
        # Check constraint impact
        constraint_tests = test_results.get("constraint_impact", {})
        slow_constraints = [test for test, data in constraint_tests.items() 
                          if data.get("execution_time_ms", 0) > 100]
        
        if slow_constraints:
            recommendations.append(
                "Foreign key constraint validation is slow"
            )
            recommendations.append(
        ...
        if not recommendations:
            recommendations.append("No performance issues detected - system performing well")
        
        return recommendations
    
    async def monitor_real_time_performance(self, interval_seconds: int = 60, 
        ...
        """
        logger.info(f"🔍 Starting real-time performance monitoring for {duration_minutes} minutes...")
        ...
        while datetime.now() < end_time:
            ...
            try:
                ...
                logger.info(f"📊 Performance measurement: avg_query_time={measurement.get('avg_query_time_ms', 0):.2f}ms")
        ...
        # Generate summary
        monitoring_results["summary"] = self._generate_monitoring_summary(monitoring_results["measurements"])
        monitoring_results["end_time"] = datetime.now().isoformat()
        
        logger.info(f"✅ Real-time monitoring completed. {len(monitoring_results['alerts'])} alerts generated.")
    ...
    async def _take_performance_measurement(self) -> Dict[str, Any]:
        ...
        async with AsyncSessionLocal() as session:
            ...
            start_time = time.perf_counter()
            result = await session.execute(select(func.count(CrewAIFlowStateExtensions.id)))
            result.scalar()
    ...
    async def _get_connection_stats(self, session: AsyncSession) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "active_queries": connection_stats.active_queries if connection_stats else 0,
    ...
    def _check_performance_alerts(self, measurement: Dict[str, Any]) -> List[Dict[str, Any]]:
        """Check measurement against alert thresholds"""
        alerts = []
        
        # Check query execution time
        if measurement.get("avg_query_time_ms", 0) > self.alert_thresholds["query_execution_time_ms"]:
            alerts.append({
        ...
        if measurement.get("join_query_time_ms", 0) > self.alert_thresholds["join_query_time_ms"]:
            alerts.append({
                "timestamp": measurement.get("timestamp"),
                "type": "slow_join",
        ...
        if measurement.get("active_connections", 0) > self.alert_thresholds["active_connections"]:
            alerts.append({
                "timestamp": measurement.get("timestamp"),
    ...
    def _generate_monitoring_summary(self, measurements: List[Dict[str, Any]]) -> Dict[str, Any]:
...
async def main():
    ...
    parser = argparse.ArgumentParser(description="Discovery flow performance monitoring")
    ...
    parser.add_argument("--alert-thresholds", help="JSON file with custom alert thresholds")
    ...
    try:
        ...
        if args.benchmark:
            ...
            logger.info(f"🔍 Starting real-time monitoring (interval: {args.interval}s, duration: {args.duration}m)...")
