...
class RailwayDatabaseFixer:
    ...
    def run_migrations(self) -> None:
        ...
        try:
            ...
            try:
                ...
                logger.warning(f"Migration failed, trying to stamp first: {migration_error}")
                try:
                    ...
                    logger.error(f"❌ Migration failed even after stamping: {stamp_error}")
                    raise
    ...
    def print_validation_report(self, validation: Dict[str, Any]) -> None:
        ...
        overall_ok = (
            validation['tables_exist'] and 
            validation['admin_exists'] and
            len(validation['errors']) == 0
        )
        
        status = "✅ READY" if overall_ok else "❌ ISSUES FOUND"
        logger.info(f"Overall Status: {status}")
        logger.info(f"Tables: {'✅' if validation['tables_exist'] else '❌'} ({validation['table_count']} found)")
    ...
    async def run_fix_process(self) -> None:
        ...
        try:
            ...
            if current_version is None and not self.force_reset:
                logger.warning("No migration version found - database may need initialization")
