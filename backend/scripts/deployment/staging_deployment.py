...
class StagingDeployment:
    ...
    def __init__(self):
        ...
        self.config = {
            ...
            "frontend_url": os.getenv(
                "STAGING_FRONTEND_URL", "https://staging-frontend.example.com"
            ),
    ...
    async def _check_git_state(self) -> bool:
        ...
        try:
            ...
            self.deployment_state["git_info"] = {"branch": branch, "commit": commit}
        ...
        try:
            ...
            result = secure_subprocess_run(
                [
                    "docker-compose",
                    "-f",
                    self.config["docker_compose_file"],
                    "exec",
                    "-T",
                    "backend",
                    "alembic",
    ...
    async def _check_service_health(self, service: str) -> bool:
        ...
        try:
            ...
            # For database services, check logs
                ...
                if (
                    "ready" in result.stdout.lower()
                    or "started" in result.stdout.lower()
                ):
...
"""
            ...
            if result.returncode == 0:
                ...
                self.deployment_state["validation"][
                    "master_flow_orchestrator"
                ] = "passed"
                return True
            else:
                logger.error(
                    f"❌ Master Flow Orchestrator validation failed: {result.stderr}"
                )
                return False
        ...
        try:
            ...
            for service in services:
                try:
                    ...
                    log_file = (
                        self.logs_path
                        / f"failure_logs_{service}_{self.deployment_id}.log"
                    )
