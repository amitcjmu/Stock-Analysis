...
class DatabaseModelChecker:
    ...
    def __init__(self, verbose: bool = False):
        ...
        self.sync_engine = create_engine(self.db_url.replace('postgresql+asyncpg://', 'postgresql://'))
        
    async def check_schema_sync(self, table_name: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if table_name:
                if table_name in model_tables and table_name in db_tables:
                    await self._compare_table_schema(table_name, model_tables[table_name], db_tables[table_name])
                else:
    ...
    def _get_model_tables(self) -> Dict[str, Dict[str, Any]]:
        ...
        for table_name, table in metadata.tables.items():
            ...
            for constraint in table.constraints:
                constraint_name = constraint.name or f"unnamed_{type(constraint).__name__}"
            ...
            for fk in table.foreign_keys:
                fk_name = fk.constraint.name if fk.constraint else f"fk_{fk.parent.name}_{fk.column.name}"
    ...
    def _normalize_db_type(self, data_type: str, max_length: Optional[int], 
    ...
    async def _compare_all_tables(self, model_tables: Dict, db_tables: Dict):
        ...
        for table_name in sorted(all_tables):
            if table_name in model_tables and table_name in db_tables:
                ...
                self._add_difference('missing_table_in_db', table_name, 
                                   f"Table '{table_name}' exists in models but not in database")
            else:
                self._add_difference('extra_table_in_db', table_name,
                                   f"Table '{table_name}' exists in database but not in models")
    
    async def _compare_table_schema(self, table_name: str, model_schema: Dict, db_schema: Dict):
        ...
        # Compare constraints
        self._compare_constraints(table_name, model_schema['constraints'], db_schema['constraints'])
        
        # Compare indexes
        self._compare_indexes(table_name, model_schema['indexes'], db_schema['indexes'])
        
        # Compare foreign keys
        self._compare_foreign_keys(table_name, model_schema['foreign_keys'], db_schema['foreign_keys'])
        ...
        for column_name in all_columns:
            if column_name in model_columns and column_name in db_columns:
                ...
                if model_col['type'] != db_col['type']:
                    self._add_difference('column_type_mismatch', table_name,
                                       f"Column '{column_name}' type mismatch: "
                                       f"model={model_col['type']}, db={db_col['type']}")
                
                if model_col['nullable'] != db_col['nullable']:
                    self._add_difference('column_nullable_mismatch', table_name,
                ...
                if model_col['primary_key'] != db_col['primary_key']:
                    self._add_difference('column_pk_mismatch', table_name,
                ...
                self._add_difference('missing_column_in_db', table_name,
                                   f"Column '{column_name}' exists in model but not in database")
            else:
                self._add_difference('extra_column_in_db', table_name,
                                   f"Column '{column_name}' exists in database but not in model")
    
    def _compare_constraints(self, table_name: str, model_constraints: Dict, db_constraints: Dict):
    ...
    def _compare_indexes(self, table_name: str, model_indexes: Dict, db_indexes: Dict):
        ...
        filtered_db_indexes = {k: v for k, v in db_indexes.items() 
                              if not k.endswith('_pkey')}
    ...
    def _compare_foreign_keys(self, table_name: str, model_fks: Dict, db_fks: Dict):
        ...
        for fk_name in all_fks:
            if fk_name in model_fks and fk_name in db_fks:
                ...
                if (model_fk['referenced_table'] != db_fk['referenced_table'] or
                    model_fk['referenced_column'] != db_fk['referenced_column']):
                    self._add_difference('fk_reference_mismatch', table_name,
                ...
                self._add_difference('missing_fk_in_db', table_name,
                                   f"Foreign key '{fk_name}' exists in model but not in database")
            else:
                self._add_difference('extra_fk_in_db', table_name,
    ...
    def _generate_report(self, model_tables: Set[str], db_tables: Set[str]) -> Dict[str, Any]:
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        if any(d['type'] == 'missing_table_in_db' for d in self.differences):
            recommendations.append("Run 'alembic revision --autogenerate' to create migration for missing tables")
        
        if any(d['type'] == 'missing_column_in_db' for d in self.differences):
            recommendations.append("Run 'alembic revision --autogenerate' to add missing columns")
        
        if any(d['type'].endswith('_mismatch') for d in self.differences):
            recommendations.append("Review and resolve type/constraint mismatches manually")
        
        if any(d['type'] == 'extra_table_in_db' for d in self.differences):
            recommendations.append("Consider removing unused tables from database or adding models")
...
async def main():
    ...
    parser.add_argument('--fix', action='store_true', help='Generate migration for differences')
    ...
    try:
        ...
        if args.fix and report['differences']:
            ...
            migration_file = f"migration_sync_{datetime.now().strftime('%Y%m%d_%H%M%S')}.py"
            with open(migration_file, 'w') as f:
                f.write(migration_script)
            print(f"\n🔧 Migration script generated: {migration_file}")
            print("Run: alembic revision --autogenerate -m 'sync schema' for proper migration")
