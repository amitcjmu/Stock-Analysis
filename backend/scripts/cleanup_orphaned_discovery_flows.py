...
async def cleanup_orphaned_discovery_flows():
    ...
    async with AsyncSessionLocal() as db:
        ...
        # Confirm deletion
        print(f"\n⚠️  About to delete {len(orphaned_flows)} orphaned discovery flows.")
        ...
        print("\n💾 Creating backup of orphaned flows...")
        backup_data = []
        for flow in orphaned_flows:
            backup_data.append({
                'id': str(flow.id),
        ...
        print("\n🗑️  Deleting orphaned discovery flows...")
        deletion_count = 0
        ...
        if final_stats.total_flows > 0:
            coverage = (final_stats.flows_with_master_id / final_stats.total_flows) * 100
