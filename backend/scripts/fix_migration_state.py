...
class MigrationStateFixer:
    ...
    async def __aenter__(self):
        ...
        self.engine = create_async_engine(
            self.settings.database_url_async, echo=False, pool_pre_ping=True
        )
        return self
    ...
    async def check_migration_files_exist(self) -> bool:
        ...
        expected_migration_file = (
            alembic_versions_dir / f"{EXPECTED_MIGRATION_VERSION}.py"
        )
    ...
    async def fix_migration_state(self) -> bool:
        ...
        if has_core_tables and not current_version:
            ...
            logger.info(
                "🔧 Fix Strategy: Tables exist but no migration version recorded"
            )
            ...
            logger.info(
                f"🔧 Fix Strategy: Updating version from {current_version} to {EXPECTED_MIGRATION_VERSION}"
            )
            await self.set_migration_version(EXPECTED_MIGRATION_VERSION)
            ...
            logger.info(
                "Database appears to be fresh - migration system should handle initialization"
            )
            return True
