...
async def debug_full_analysis():  # noqa: C901
    ...
    try:
        ...
        print(
            f"Assets with OS: {len([a for a in assets if a.get('operating_system')])}"
        )
        ...
        print("\n=== STEP 2: APP ANALYSIS ===")
        try:
            app_analysis = await tech_debt_analysis_agent._analyze_application_versions(
                assets
            )
            print(
                f"App analysis completed: {len(app_analysis.get('application_inventory', {}))}"
            )
        except Exception as e:
        ...
        try:
            infrastructure_analysis = (
                await tech_debt_analysis_agent._analyze_infrastructure_debt(assets)
            )
        ...
        try:
            security_analysis = await tech_debt_analysis_agent._analyze_security_debt(
                assets
            )
        ...
        try:
            ...
            business_risk_assessment = (
                await tech_debt_analysis_agent._assess_business_risk_context(
                    technical_analysis,
                    {},  # stakeholder_context
        ...
        try:
            stakeholder_questions = (
                await tech_debt_analysis_agent._generate_stakeholder_questions(
                    prioritized_debt, {}
                )
            )
        ...
        try:
            migration_recommendations = (
                await tech_debt_analysis_agent._generate_migration_recommendations(
