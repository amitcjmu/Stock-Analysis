...
async def test_rls_isolation():
    ...
    print("🧪 Testing Row-Level Security Implementation\n")
    ...
    async with AsyncSessionLocal() as session:
        try:
            ...
            print("\n✅ Row-Level Security tests completed!")

        except Exception as e:
            print(f"\n❌ Error during RLS testing: {e}")
            import traceback
...
async def test_rls_functions():
    ...
    async with AsyncSessionLocal() as session:
        try:
            ...
            print(f"🔧 Testing set_tenant_context with UUID: {test_uuid}")
            await session.execute(
                text("SELECT migration.set_tenant_context(:client_id)"),
                {"client_id": test_uuid},
            )
            print("   ✅ set_tenant_context executed successfully")

            # Test get_current_tenant function
            print("\n🔧 Testing get_current_tenant")
            result = await session.execute(
                text("SELECT migration.get_current_tenant()")
            )
            current_tenant = result.scalar()
            print(f"   ✅ Current tenant: {current_tenant}")
            print(f"   ✅ Matches set value: {current_tenant == test_uuid}")

            # Test with no context
            print("\n🔧 Testing get_current_tenant with no context")
            await session.execute(text("RESET app.client_id"))
            result = await session.execute(
                text("SELECT migration.get_current_tenant()")
            )
            current_tenant = result.scalar()
            print(f"   ✅ Current tenant (should be None): {current_tenant}")

        except Exception as e:
            print(f"\n❌ Error testing RLS functions: {e}")
            import traceback
