...
def validate_authentication_bypass_fix():
    ...
    if "pass" in content and "password_hash" in content:
        ...
        if re.search(
            r"else:\s*#.*demo.*\s*pass", content, re.IGNORECASE | re.MULTILINE
        ):
...
def validate_hardcoded_credentials():
    ...
    if docker_compose_path.exists():
        ...
        if "${DEEPINFRA_API_KEY}" not in content and "DEEPINFRA_API_KEY=" in content:
            issues.append(
                "DeepInfra API key not using environment variable in docker-compose.yml"
            )
...
def validate_rate_limiting():
    ...
    rate_limiter_path = (
        Path(__file__).parent.parent / "app" / "middleware" / "rate_limiter.py"
    )
