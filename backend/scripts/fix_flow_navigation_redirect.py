...
async def analyze_and_fix_flow_navigation():
    async with AsyncSessionLocal() as db:
        ...
        flows_result = await db.execute(
            ...
            .join(CrewAIFlowStateExtensions, DiscoveryFlow.flow_id == CrewAIFlowStateExtensions.flow_id)
        ...
        for discovery_flow, crewai_state in flows:
            ...
            if not discovery_flow.field_mapping_completed and awaiting_approval:
                if discovery_flow.current_phase != 'field_mapping':
                    ...
                    print(f"⚠️  ISSUE: Flow is awaiting field mapping approval but phase is {discovery_flow.current_phase}")
            ...
            if needs_correction and correct_phase:
                ...
                await db.commit()
                print(f"✅ Flow {discovery_flow.flow_id} phase corrected to {correct_phase}")
            else:
