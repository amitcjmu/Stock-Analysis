...
class Phase3SummaryReporter:
    ...
    async def get_current_metrics(self, session):
        ...
        # Data imports
        data_imports_total_result = await session.execute(text("SELECT COUNT(*) FROM migration.data_imports"))
        data_imports_total = data_imports_total_result.scalar()
        
        data_imports_with_master_result = await session.execute(text("SELECT COUNT(*) FROM migration.data_imports WHERE master_flow_id IS NOT NULL"))
        ...
        metrics["data_imports"] = {
            ...
            "health_percentage": (data_imports_with_master / data_imports_total * 100) if data_imports_total > 0 else 0
        }
        
        # Raw import records
        raw_records_total_result = await session.execute(text("SELECT COUNT(*) FROM migration.raw_import_records"))
        raw_records_total = raw_records_total_result.scalar()
        
        raw_records_with_master_result = await session.execute(text("SELECT COUNT(*) FROM migration.raw_import_records WHERE master_flow_id IS NOT NULL"))
        ...
        metrics["raw_import_records"] = {
            ...
            "health_percentage": (raw_records_with_master / raw_records_total * 100) if raw_records_total > 0 else 0
        }
        
        # Discovery flows
        discovery_flows_total_result = await session.execute(text("SELECT COUNT(*) FROM migration.discovery_flows"))
        discovery_flows_total = discovery_flows_total_result.scalar()
        
        discovery_flows_with_master_result = await session.execute(text("SELECT COUNT(*) FROM migration.discovery_flows WHERE master_flow_id IS NOT NULL"))
        ...
        metrics["discovery_flows"] = {
            ...
            "health_percentage": (discovery_flows_with_master / discovery_flows_total * 100) if discovery_flows_total > 0 else 0
        }
        
        # Master flows
        master_flows_total_result = await session.execute(text("SELECT COUNT(*) FROM migration.crewai_flow_state_extensions"))
        master_flows_total = master_flows_total_result.scalar()
        
        master_flows_with_children_result = await session.execute(text("""
        ...
        metrics["master_flows"] = {
            ...
            "utilization_percentage": (master_flows_with_children / master_flows_total * 100) if master_flows_total > 0 else 0
        }
    ...
    async def generate_summary(self):
        ...
        async with AsyncSessionLocal() as session:
            ...
            self.summary["achievements"] = {
                "data_imports_fixed": self.summary["before_state"]["data_imports"]["orphaned"] - self.summary["after_state"]["data_imports"]["orphaned"],
                "raw_records_fixed": self.summary["before_state"]["raw_import_records"]["orphaned"] - self.summary["after_state"]["raw_import_records"]["orphaned"],
                "discovery_flows_fixed": self.summary["before_state"]["discovery_flows"]["orphaned"] - self.summary["after_state"]["discovery_flows"]["orphaned"],
                "health_improvement": {
                    "data_imports": self.summary["after_state"]["data_imports"]["health_percentage"] - self.summary["before_state"]["data_imports"]["health_percentage"],
                    "raw_records": self.summary["after_state"]["raw_import_records"]["health_percentage"] - self.summary["before_state"]["raw_import_records"]["health_percentage"],
                    "discovery_flows": self.summary["after_state"]["discovery_flows"]["health_percentage"] - self.summary["before_state"]["discovery_flows"]["health_percentage"]
                },
                "total_records_fixed": (
                    (self.summary["before_state"]["data_imports"]["orphaned"] - self.summary["after_state"]["data_imports"]["orphaned"]) +
                    (self.summary["before_state"]["raw_import_records"]["orphaned"] - self.summary["after_state"]["raw_import_records"]["orphaned"]) +
                    (self.summary["before_state"]["discovery_flows"]["orphaned"] - self.summary["after_state"]["discovery_flows"]["orphaned"])
                )
            }
            
            # Identify remaining issues
            if self.summary["after_state"]["data_imports"]["orphaned"] > 0:
                self.summary["remaining_issues"].append(f"{self.summary['after_state']['data_imports']['orphaned']} orphaned data imports")
            
            if self.summary["after_state"]["raw_import_records"]["orphaned"] > 0:
                self.summary["remaining_issues"].append(f"{self.summary['after_state']['raw_import_records']['orphaned']} orphaned raw import records")
            
            if self.summary["after_state"]["discovery_flows"]["orphaned"] > 0:
                self.summary["remaining_issues"].append(f"{self.summary['after_state']['discovery_flows']['orphaned']} orphaned discovery flows")
            
            if self.summary["after_state"]["master_flows"]["orphaned"] > 0:
                self.summary["remaining_issues"].append(f"{self.summary['after_state']['master_flows']['orphaned']} unused master flows")
            ...
            self.summary["overall_success_rate"] = ((total_before - total_after) / total_before * 100) if total_before > 0 else 100
    ...
    def print_summary(self):
        ...
        print(f"  Improvement: +{self.summary['achievements']['health_improvement']['data_imports']:.1f}%")
        print()
        ...
        print(f"  Improvement: +{self.summary['achievements']['health_improvement']['raw_records']:.1f}%")
        print()
        ...
        print(f"  Improvement: +{self.summary['achievements']['health_improvement']['discovery_flows']:.1f}%")
        print()
        
        # Overall Results
        print("OVERALL RESULTS:")
        print("-" * 80)
        print(f"Total Records Fixed: {self.summary['achievements']['total_records_fixed']}")
        ...
        print("  • fix_orphaned_data_imports.py - Links orphaned data imports to master flows")
        print("  • validate_flow_relationships.py - Comprehensive relationship validation")
        ...
        # Export JSON summary
        json_file = Path(__file__).parent / f"phase3_summary_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
