    ...
    async with AsyncSessionLocal() as db:
        try:
            ...
            for user, role in users_with_roles:
                ...
                elif email.startswith(('admin@', 'demo@')) and email not in LEGITIMATE_ACCOUNTS:
                    violations.append({
...
def calculate_security_score(violations: List[Dict], warnings: List[Dict]) -> Dict[str, Any]:
...
def generate_recommendations(violations: List[Dict], warnings: List[Dict]) -> List[str]:
    ...
    if violations:
        recommendations.append("🚨 IMMEDIATE ACTION: Disable all unauthorized demo accounts")
        recommendations.append("🔒 SECURITY: Remove admin privileges from demo accounts")
        recommendations.append("📋 AUDIT: Investigate how unauthorized accounts were created")
    
    if warnings:
        recommendations.append("⚠️  REVIEW: Verify role assignments for flagged accounts")
        recommendations.append("🔍 MONITOR: Set up automated monitoring for demo account creation")
                ...
                if violation['severity'] in ['CRITICAL', 'HIGH']:
                    ...
                    try:
                        ...
                        if user:
                            ...
                            user.email = f"DISABLED_{email}_{int(datetime.utcnow().timestamp())}"
                            user.password_hash = "DISABLED_SECURITY_VIOLATION"
                            
                            # Disable all roles
                            role_query = select(UserRole).where(UserRole.user_id == user.id)
...
async def main():
    ...
    if audit_results['violations']:
        ...
        for violation in audit_results['violations']:
            print(f"  - {violation['type']}: {violation['email']} ({violation['severity']})")
