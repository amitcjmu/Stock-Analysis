...
async def verify_seeded_data():
    ...
    async with AsyncSessionLocal() as session:
        try:
            ...
            # Count flows
            flow_count = await session.execute(text("SELECT COUNT(*) FROM discovery_flows"))
            flow_total = flow_count.scalar()
            
            # Count assets
            asset_count = await session.execute(text("SELECT COUNT(*) FROM discovery_assets"))
            asset_total = asset_count.scalar()
            
            # Get flow details
            flow_details = await session.execute(text("""
            ...
            for flow in flow_details:
                ...
                logger.info(f"     - Phases: Import={flow.data_import_completed}, Mapping={flow.attribute_mapping_completed}, Cleansing={flow.data_cleansing_completed}")
