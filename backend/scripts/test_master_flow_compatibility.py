...
async def test_asset_repository_master_flow():
    ...
    async with AsyncSessionLocal() as session:
        ...
        print(
            f"   🎯 Test Context: Client {test_context.client_account_id}, Engagement {test_context.engagement_id}"
        )
        ...
        if master_flow_test:
            ...
            test_assets = await session.execute(
                text(
                    """
            ...
            print(
                f"      ✅ Found {count} assets for master flow {master_flow_test.master_flow_id}"
            )
        ...
        for stat in phase_stats:
            print(
                f"      ✅ {stat.source_phase} → {stat.current_phase}: {stat.asset_count} assets"
            )
        ...
        for fa in flow_assets:
            print(
                f"      ✅ Discovery Flow {fa.discovery_flow_id}: {fa.asset_count} assets linked to master flow {fa.master_flow_id}"
            )
...
async def test_discovery_flow_master_integration():
    ...
    async with AsyncSessionLocal() as session:
        ...
        for coord in coordinations:
            print(
                f"      ✅ Flow '{coord.flow_name}' - Phase: {coord.current_phase}, Status: {coord.status}"
            )
        ...
        for stat in progression_stats:
            print(
                f"      ✅ {stat.current_phase} Phase: {stat.count} flows, {stat.progression_rate:.2%} with progression data"
            )
...
async def test_crewai_master_flow_coordination():
    ...
    async with AsyncSessionLocal() as session:
        ...
        for ctx in contexts:
            ...
            print(
                f"      ✅ Flow {ctx.flow_id}: Phase {ctx.current_phase}, Context: {context_data}"
            )
        ...
        for rel in relationships:
            print(
                f"      ✅ Master Flow {rel.master_flow_id}: Discovery '{rel.flow_name}' with {rel.linked_assets} assets"
            )
