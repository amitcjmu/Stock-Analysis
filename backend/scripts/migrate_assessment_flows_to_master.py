    ...
    async with AsyncSessionLocal() as db:
        try:
            ...
            count_result = await db.execute(
                select(func.count(AssessmentFlow.id))
            )
            ...
            for flow in assessment_flows:
                ...
                master_check = await db.execute(
                    ...
                    .where(CrewAIFlowStateExtensions.flow_id == flow.id)
                )
                existing_master = master_check.scalar_one_or_none()
                
                if existing_master:
                    logger.info(f"Assessment flow {flow_id} already registered in master flow system")
                ...
                try:
                    ...
                    await extensions_repo.create_master_flow(
                        ...
                        flow_configuration={
                            "selected_applications": flow.selected_application_ids or [],
                        ...
                        initial_state={
                            ...
                            "status": flow_status,
                            "progress": progress,
                            "applications_count": len(flow.selected_application_ids or []),
                    ...
                    logger.info(f"✅ Migrated assessment flow {flow_id} to master flow system")
                    migrated_count += 1
...
async def verify_migration():
    ...
    async with AsyncSessionLocal() as db:
        ...
        master_count = await db.execute(
            ...
            .where(CrewAIFlowStateExtensions.flow_type == 'assessment')
        )
        ...
        if total_assessment == total_master:
            ...
            logger.warning(f"⚠️  Mismatch: {total_assessment - total_master} assessment flows not registered")
        logger.info("="*50)
