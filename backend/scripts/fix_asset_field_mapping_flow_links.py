    ...
    async with AsyncSessionLocal() as db:
        ...
        for asset_row in unlinked_assets:
            try:
                ...
                master_flow_id = (
                    asset_row.discovery_flow_master_flow_id
                    or asset_row.data_import_master_flow_id
                )
                ...
                if master_flow_id:
                    ...
                    linked_count += 1
                    print(
                        f"✅ Linked asset {asset_row.asset_id} to master flow {master_flow_id}"
                    )
...
async def fix_field_mapping_master_flow_links():
    ...
    async with AsyncSessionLocal() as db:
        ...
        print(
            f"📊 Found {len(unlinked_mappings)} field mappings to link to master flows"
        )
        ...
        for mapping_row in unlinked_mappings:
            try:
                ...
                linked_count += 1
                print(
                    f"✅ Linked field mapping {mapping_row.mapping_id} to master flow {master_flow_id}"
                )
    ...
    async with AsyncSessionLocal() as db:
        ...
        for invalid_link in invalid_links:
            if invalid_link.invalid_count > 0:
                print(
                    f"⚠️  {invalid_link.table_name}: {invalid_link.invalid_count} invalid foreign key references"
                )
            else:
