...
async def test_agentic_flow():
    try:
        ...
        test_metadata = {"filename": "test_cmdb.csv", "total_records": 2}
        ...
        async with AsyncSessionLocal() as db:
            ...
            print(
                f"   ✅ Data source agent available: {flow.data_source_agent is not None}"
            )
            print(
                f"   ✅ Dependency agent available: {flow.dependency_agent is not None}"
            )
