...
def get_railway_db_url():
    ...
    if general_url:
        logger.info(f"Using DATABASE_URL: {general_url.split('@')[1] if '@' in general_url else 'hidden'}")
        return general_url
    ...
    try:
        ...
        with engine.connect() as conn:
            ...
            try:
                ...
                key_columns = ['flow_id', 'user_id', 'progress_percentage', 'field_mappings']
            ...
            return {
                ...
                'version': version,
                'columns': len(columns),
                'has_key_columns': len(missing) == 0 if 'missing' in locals() else False,
