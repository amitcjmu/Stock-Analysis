...
async def fix_railway_schema():
    ...
    if not database_url:
        logger.warning("❌ DATABASE_URL environment variable not found - skipping schema fix")
        return False
    ...
    try:
        ...
        required_client_cols = [
            'headquarters_location', 'primary_contact_name', 'primary_contact_email',
            'primary_contact_phone', 'subscription_tier', 'billing_contact_email',
            'settings', 'branding', 'business_objectives', 'it_guidelines',
            'decision_criteria', 'agent_preferences'
        ]
        ...
        for col in required_client_cols:
            if col not in existing_client_cols:
                try:
                    if col in ['settings', 'branding', 'business_objectives', 'it_guidelines', 'decision_criteria', 'agent_preferences']:
                        await conn.execute(f"ALTER TABLE client_accounts ADD COLUMN {col} JSON")
                    else:
                        size = '50' if col in ['subscription_tier', 'primary_contact_phone'] else '255'
                        await conn.execute(f"ALTER TABLE client_accounts ADD COLUMN {col} VARCHAR({size})")
