...
def test_cors_preflight(
    ...
    try:
        ...
        if "Access-Control-Allow-Origin" in response.headers:
            ...
            print(
                "   ⚠️  No CORS headers in response (may be normal for non-CORS requests)"
            )
...
if __name__ == "__main__":
    ...
    backend_url = (
        sys.argv[1]
        if len(sys.argv) > 1
        else "https://migrate-ui-orchestrator-production.up.railway.app"
    )
    frontend_origin = (
        sys.argv[2] if len(sys.argv) > 2 else "https://aiforce-assess.vercel.app"
    )
