    ...
    async with get_db_session() as session:
        try:
            ...
            if not result.fetchone():
                logger.error(
                    "❌ Embedding column not found. Please run migration 017 first."
                )
                return
            ...
            if total_count == 0:
                logger.info(
                    "✅ All patterns already have embeddings. Nothing to backfill."
                )
                return

            if dry_run:
                logger.info(f"🔍 DRY RUN: Would backfill {total_count} patterns")
            ...
            while processed < total_count:
                ...
                logger.info(f"✅ Updated {len(batch_rows)} patterns in this batch")

            logger.info(
                f"🎉 Backfill completed! Updated {processed} patterns with embeddings and insight types"
            )
            ...
            logger.info(
                f"✅ Verification: {final_count} patterns now have embeddings and insight types"
            )
