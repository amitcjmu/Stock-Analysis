...
async def check_master_flow():
    ...
    async with AsyncSessionLocal() as session:
        ...
        linked_flows = await session.execute(
            select(DiscoveryFlow).where(DiscoveryFlow.master_flow_id == master_flow_id)
        )
        linked_flows = linked_flows.scalars().all()

        if linked_flows:
            print(
                f"Found {len(linked_flows)} discovery flow(s) linked to this master flow:"
            )
            ...
            for flow in linked_flows:
                ...
                assets_count = await session.execute(
                    select(text("COUNT(*)"))
                    .select_from(Asset)
                    .where(Asset.discovery_flow_id == flow.id)
                )
                ...
                if discovery_ext:
                    print(
                        f"    ✅ Has CrewAI Extension (Active: {discovery_ext.is_active})"
                    )
                else:
        ...
        active_flows = await session.execute(
            select(DiscoveryFlow).where(
                DiscoveryFlow.status.in_(
                    ["running", "in_progress", "waiting_for_approval"]
                )
            )
        )
        ...
        if master_flow and master_flow.flow_type == "master":
            ...
            if linked_flows:
                ...
                for flow in linked_flows:
                    print(
                        f"   • {flow.id} (Status: {flow.status}, Phase: {flow.current_phase})"
                    )
            else:
                print(
