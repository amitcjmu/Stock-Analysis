...
def refactor_master_orchestrator():
    ...
    file_path = Path(
        "/Users/chocka/CursorProjects/migrate-ui-orchestrator/backend/app/services/master_flow_orchestrator.py"
    )
        ...
        if skip_until_next_method:
            ...
            for method_name in METHODS_TO_REMOVE:
                if f"async def {method_name}(" in line or f"def {method_name}(" in line:
                    ...
                    new_lines.append(
                        f"{indent}# {method_name} method extracted to service"
                    )
                    break
    ...
    print(
        f"📊 Reduced from {original_lines} to {new_lines_count} lines ({saved_lines} lines saved)"
    )
