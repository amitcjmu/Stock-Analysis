    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if result.get('agent_insights'):
                print(f"   Agent Insights: {len(result['agent_insights'])} insights generated")
...
def check_flow_status(flow_id: str):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"   Data Import Completed: {status.get('phases', {}).get('data_import_completed', False)}")
            print(f"   Agent Insights: {len(status.get('agent_insights', []))} insights")
            return status
        else:
