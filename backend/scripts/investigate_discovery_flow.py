...
async def investigate_flow(flow_id: str):
    ...
    async with AsyncSessionLocal() as session:
        ...
        if discovery_flow:
            ...
            print(
                f"    - Dependency Analysis: {discovery_flow.dependency_analysis_completed}"
            )
            print(
                f"    - Tech Debt Assessment: {discovery_flow.tech_debt_assessment_completed}"
            )
        ...
        if data_imports:
            ...
            for di in data_imports:
                ...
                if di.field_mapping_config:
                    print(
                        f"    Field Mapping Config: {json.dumps(di.field_mapping_config, indent=6)}"
                    )
        else:
        ...
        if data_imports:
            for di in data_imports:
                raw_records_count = await session.execute(
                    ...
                    .where(RawImportRecord.data_import_id == di.id)
                )
        ...
        if (
            ...
            and discovery_flow.status != "in_progress"
        ):
            issues.append(
                f"Flow status is '{discovery_flow.status}' - may not appear in active flows"
            )
        ...
        if not crewai_state:
            issues.append(
                "Missing CrewAI flow state extension - flow orchestration may be broken"
            )
        elif not crewai_state.is_active:
            issues.append(
                "CrewAI flow state is inactive - flow won't appear in active flows"
            )

        if not discovery_flow.master_flow_id:
            issues.append(
                "No master flow ID - flow may not be properly linked to master orchestration"
            )
        ...
        if not assets:
            issues.append(
                "No assets created - asset creation phase may not have completed"
            )

        if (
            discovery_flow.current_phase == "data_import"
            and discovery_flow.progress_percentage > 0
        ):
        ...
        if issues:
            ...
            if "No master flow ID" in str(issues):
                print(
                    "  • Ensure discovery flows are created through master flow orchestration"
                )
