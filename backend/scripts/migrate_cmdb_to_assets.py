...
class CMDBToAssetsDataMigrator:
    ...
    async def _analyze_current_state(self, session: AsyncSession):
        ...
        embeddings_refs = await session.execute(
            text("SELECT COUNT(*) FROM migration.cmdb_asset_embeddings")
        )
        embeddings_count = embeddings_refs.scalar()

        tags_refs = await session.execute(
            text(
    ...
    async def _migrate_cmdb_assets_data(self, session: AsyncSession):
        ...
        cmdb_assets_query = await session.execute(
            text("SELECT * FROM migration.cmdb_assets ORDER BY created_at")
        )
        ...
        for cmdb_asset in cmdb_assets:
            try:
                ...
                if migrated_count % 10 == 0:
                    print(
                        f"   📦 Migrated {migrated_count}/{len(cmdb_assets)} assets..."
                    )
    ...
    async def _update_foreign_key_references(self, session: AsyncSession):
        ...
        print(
            "   ⚠️  Note: Foreign key updates skipped - new assets table uses auto-generated integer IDs"
        )
        print(
            "   ⚠️  Existing foreign key relationships will need to be rebuilt through application logic"
        )
    ...
    async def _verify_migration_integrity(self, session: AsyncSession):
        ...
        print(
            "   🔍 Foreign key integrity check skipped - foreign keys require application-level rebuild"
        )
    ...
    def _print_migration_summary(self):
        ...
        print("=" * 40)
        print(
            f"   📦 CMDB Assets Migrated: {self.migration_stats['cmdb_assets_migrated']}"
        )
        print(
            f"   📝 Raw Import Records Updated: {self.migration_stats['raw_import_records_updated']}"
        )
        print(
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Migrate CMDB Assets to Unified Assets"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    parser.add_argument(
        "--no-backup", action="store_true", help="Skip creating backup tables"
    )
