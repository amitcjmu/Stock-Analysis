    ...
    async with AsyncSessionLocal() as db:
        try:
            ...
            for flow in orphaned_flows:
                logger.info(f"  - Flow ID: {flow.flow_id}, Status: {flow.status}, "
                          f"Phase: {flow.current_phase}, Created: {flow.created_at}")
            
            # Mark all orphaned flows as deleted
            update_stmt = update(DiscoveryFlow).where(
                and_(
                    DiscoveryFlow.master_flow_id.is_(None),
                    DiscoveryFlow.status != 'deleted'
                )
            ...
            audit_record = FlowDeletionAudit.create_audit_record(
                ...
                cleanup_summary={
                    "orphaned_flows_cleaned": flows_deleted
                },
            ...
            logger.info(f"Successfully marked {flows_deleted} orphaned flows as deleted")
            ...
            if cancelled_masters:
                ...
                for master in cancelled_masters:
                    logger.info(f"  - Master Flow ID: {master.flow_id}, Type: {master.flow_type}, "
