...
async def analyze_model_differences():
    ...
    async with AsyncSessionLocal() as session:
        ...
        ai_fields = [col for col in assets_only if 'ai' in col.lower() or 'intelligent' in col.lower() or 'confidence' in col.lower()]
        app_fields = [col for col in assets_only if 'application' in col.lower() or 'programming' in col.lower() or 'framework' in col.lower()]
        business_fields = [col for col in assets_only if 'business' in col.lower() or 'department' in col.lower() or 'owner' in col.lower()]
        
        if ai_fields:
            print(f"  📱 AI/Intelligence fields ({len(ai_fields)}):")
            for col in ai_fields: print(f"    - {col}")
        
        if app_fields:
            print(f"  💻 Application fields ({len(app_fields)}):")
            for col in app_fields: print(f"    - {col}")
        
        if business_fields:
            print(f"  🏢 Business fields ({len(business_fields)}):")
            for col in business_fields: print(f"    - {col}")
        
        other_assets_only = set(assets_only) - set(ai_fields) - set(app_fields) - set(business_fields)
        if other_assets_only:
            print(f"  🔧 Other enhanced fields ({len(other_assets_only)}):")
            for col in sorted(other_assets_only): print(f"    - {col}")
        ...
        missing_critical = [field for field in critical_fields if field not in common_columns]
    ...
    async with AsyncSessionLocal() as session:
        ...
        if foreign_keys:
            ...
            for fk in foreign_keys:
                ...
                print(f"  - {table}.{column} → {ref_table}.{ref_column} (constraint: {constraint})")
        else:
