...
TEST_ADAPTERS = [
    {
        ...
        "supported_platforms": ["ServiceNow Madrid", "ServiceNow Paris", "ServiceNow Quebec"],
    ...
    },
    {
        ...
        "version": "1.2.0", 
        "status": "active",
        "capabilities": ["instance_discovery", "patch_compliance", "inventory_collection"],
        "configuration_schema": {
            "type": "object",
            "properties": {
                "region": {"type": "string"},
    ...
    },
    {
        ...
        "metadata": {
            "description": "Manual questionnaire adapter for gap filling"
        }
    }
]
    ...
    for adapter_data in TEST_ADAPTERS:
        ...
        await session.execute(
            ...
            """),
            {
                ...
                "configuration_schema": json.dumps(adapter_data["configuration_schema"]),
                "supported_platforms": json.dumps(adapter_data["supported_platforms"]),
                "required_credentials": json.dumps(adapter_data["required_credentials"]),
...
async def create_test_collection_flows(
    ...
    test_flows = [
        ...
        },
        {
            ...
            "metadata": {
                "manual_entries_required": 125,
                "automated_discoveries": 312
            },
            "collection_config": {
    ...
    for flow_data in test_flows:
        ...
        await session.execute(
            ...
            """),
            {
                ...
                "error_details": json.dumps(flow_data.get("error_details", {})) if flow_data.get("error_details") else None,
    ...
    sample_data_types = [
        {
            ...
            "sample_data": {
                "servers": [
                    {"name": "PROD-WEB-01", "os": "Windows Server 2019", "ip": "10.0.1.10"},
        ...
        },
        {
            ...
            "sample_data": {
                "vms": [
                    {"name": "vm-prod-001", "size": "Standard_D4s_v3", "region": "eastus"},
                    {"name": "vm-prod-002", "size": "Standard_D8s_v3", "region": "eastus"}
                ]
            },
            "resource_count": 87,
            "quality_score": 92.0,
        ...
        },
        {
            ...
            "sample_data": {
                "vms": [
                    {"name": "VM-Legacy-01", "vcpu": 4, "memory_gb": 16, "datastore": "PROD-DS01"}
                ]
            },
            "resource_count": 234,
            "quality_score": 88.5,
            ...
            if i < len(adapter_ids):
                await session.execute(
                    ...
                    """),
                    {
                        ...
                        "normalized_data": json.dumps(data_type["sample_data"]),  # Simplified
...
async def main():
    ...
    async with async_session_maker() as session:
        try:
            ...
            print(f"✅ Test context ready - User: {user_id}, Client: {client_id}, Engagement: {engagement_id}")
