...
class FlowRelationshipValidator:
    ...
    async def validate_master_flow_integrity(
        ...
        total_issues = (
            invalid_count + duplicate_count + invalid_type_count + orphaned_child_count
        )
    ...
    async def validate_data_import_linkages(self, session: Session) -> ValidationResult:
        ...
        missing_master_flow = await session.execute(
            text(
                "SELECT COUNT(*) FROM migration.data_imports WHERE master_flow_id IS NULL"
            )
        )
        ...
        total_issues = (
            orphaned_master_count
            + missing_master_count
            + orphaned_client_count
            + orphaned_engagement_count
        )
        valid_count = total_count - total_issues
        health_score = (valid_count / total_count * 100) if total_count > 0 else 0
    ...
    async def validate_raw_import_record_linkages(
        self, session: Session
    ) -> ValidationResult:
        ...
        missing_master_flow = await session.execute(
            text(
                "SELECT COUNT(*) FROM migration.raw_import_records WHERE master_flow_id IS NULL"
            )
        )
        missing_master_count = missing_master_flow.scalar()
            ...
            for row in orphaned_details.fetchall():
                self.orphaned_records.append(
                    OrphanedRecord(
                        ...
                        foreign_key_value=None,
                        client_account_id=(
                            str(row.client_account_id)
                            if row.client_account_id
                            else "N/A"
                        ),
                        engagement_id=(
                            str(row.engagement_id) if row.engagement_id else "N/A"
                        ),
                        created_at=row.created_at,
    ...
    async def validate_discovery_flow_linkages(
        self, session: Session
    ) -> ValidationResult:
        ...
        missing_master_flow = await session.execute(
            text(
                "SELECT COUNT(*) FROM migration.discovery_flows WHERE master_flow_id IS NULL"
            )
        )
        missing_master_count = missing_master_flow.scalar()
    ...
    async def validate_assessment_flow_linkages(
        self, session: Session
    ) -> ValidationResult:
        ...
        missing_master_flow = await session.execute(
            text(
                "SELECT COUNT(*) FROM migration.assessment_flows WHERE master_flow_id IS NULL"
            )
        )
        missing_master_count = missing_master_flow.scalar()
    ...
    async def validate_cross_table_consistency(
        self, session: Session
    ) -> ValidationResult:
        ...
        total_issues = (
            orphaned_master_count + imports_without_records + mismatched_tenant_count
        )
        ...
        try:
            async with AsyncSessionLocal() as session:
                ...
                self.validation_results.append(
                    await self.validate_master_flow_integrity(session)
                )
                self.validation_results.append(
                    await self.validate_data_import_linkages(session)
                )
                self.validation_results.append(
                    await self.validate_raw_import_record_linkages(session)
                )
                self.validation_results.append(
                    await self.validate_discovery_flow_linkages(session)
                )
                self.validation_results.append(
                    await self.validate_assessment_flow_linkages(session)
                )
                self.validation_results.append(
                    await self.validate_cross_table_consistency(session)
                )
    ...
    def _print_summary(self):
        ...
        overall_health = sum(r.health_score for r in self.validation_results) / len(
        ...
        if self.orphaned_records:
            ...
            for table_name, records in by_table.items():
                ...
                for record in records[:3]:  # Show first 3
                    ...
                    print(
                        f"  Tenant: {record.client_account_id[:8]}.../{record.engagement_id[:8]}..."
                    )
            ...
            if result.health_score < 100:
                if result.orphaned_records > 0:
                    recommendations.append(
                        f"- Fix {result.orphaned_records} orphaned records in {result.table_name}"
                    )
                if result.invalid_records > 0:
                    recommendations.append(
                        f"- Address {result.invalid_records} data integrity issues in {result.table_name}"
                    )
...
async def main():
    ...
    validator = FlowRelationshipValidator(
        verbose=args.verbose, export_csv=args.export_csv
    )
