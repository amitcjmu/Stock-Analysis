...
def run_psql_command(command: str) -> tuple[bool, str]:
    ...
    try:
        result = subprocess.run([
            "docker", "exec", "migration_postgres", 
            "psql", "-U", "postgres", "-d", "migration_db", "-c", command
...
def check_table_exists(table_name: str) -> bool:
    ...
    if success:
        ...
        print(f"{status} Table migration.{table_name}: {'EXISTS' if exists else 'MISSING'}")
        return exists
    else:
...
def check_workflow_states_columns():
    ...
    if success:
        ...
        expected_columns = [
            'flow_id', 'user_id', 'progress_percentage', 'field_mappings', 
            'cleaned_data', 'asset_inventory', 'dependencies', 'technical_debt'
        ]
