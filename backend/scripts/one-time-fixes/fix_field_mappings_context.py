...
async def fix_field_mappings_context():
    async for db in get_db():
        try:
            ...
            print(
                f"Current context - Client: {data_import.client_account_id}, Engagement: {data_import.engagement_id}"
            )
            ...
            await db.execute(
                ...
                .values(
                    client_account_id=demo_client_id, engagement_id=demo_engagement_id
                )
            )
            ...
            if flow:
                ...
                print(
                    f"Current flow context - Client: {flow.client_account_id}, Engagement: {flow.engagement_id}"
                )
