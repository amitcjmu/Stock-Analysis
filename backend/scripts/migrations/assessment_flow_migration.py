...
class AssessmentFlowMigration:
    ...
    async def migrate_to_assessment_flow(self, dry_run: bool = False):
        ...
        try:
            ...
            if not SQLALCHEMY_AVAILABLE:
                raise MigrationError("SQLAlchemy not available - cannot proceed with migration")
            ...
            self._log_step("Migration completed", {"tables_created": len(self.tables_created)})
    ...
    async def _validate_environment(self):
        ...
        if missing_vars:
            raise MigrationError(f"Missing required environment variables: {missing_vars}")
    ...
    async def _initialize_existing_engagements(self, dry_run: bool = False):
        ...
        async with AsyncSessionLocal() as db:
            try:
                    ...
                    for standard in default_standards:
                        ...
                        if not existing.fetchone():
                            ...
                            """), {
                                ...
                                "exceptions_allowed": standard.get("exceptions_allowed", False)
                            })
                ...
                self._log_step("Engagement standards initialized", {"count": len(engagements)})
                
            except Exception as e:
                await db.rollback()
                raise MigrationError(f"Failed to initialize engagement standards: {str(e)}")
    ...
    async def _verify_migration(self):
        ...
        async with AsyncSessionLocal() as db:
            try:
                ...
                constraint_check = await db.execute(text("""
                ...
                self._log_step("Migration verified", {"tables": len(required_tables), "fk_constraints": fk_count})
    ...
    async def _optimize_performance(self, dry_run: bool = False):
        ...
        if dry_run:
            logger.info(f"DRY RUN: Would create {len(performance_sql)} performance indexes")
            return
        ...
        async with AsyncSessionLocal() as db:
            try:
                ...
                self._log_step("Performance optimized", {"indexes": len(performance_sql)})
