...
class DataIntegrityTester:
    ...
    async def test_data_import_linkage(self, session):
        ...
        total_result = await session.execute(text("SELECT COUNT(*) FROM migration.data_imports"))
        total_count = total_result.scalar()
        
        # Check linked data imports
        linked_result = await session.execute(
            text("SELECT COUNT(*) FROM migration.data_imports WHERE master_flow_id IS NOT NULL")
        )
    ...
    async def test_raw_import_record_linkage(self, session):
        ...
        total_result = await session.execute(text("SELECT COUNT(*) FROM migration.raw_import_records"))
        total_count = total_result.scalar()
        
        # Check records with master_flow_id
        master_linked_result = await session.execute(
            text("SELECT COUNT(*) FROM migration.raw_import_records WHERE master_flow_id IS NOT NULL")
        )
        master_linked_count = master_linked_result.scalar()
        
        # Check records with data_import_id
        data_linked_result = await session.execute(
            text("SELECT COUNT(*) FROM migration.raw_import_records WHERE data_import_id IS NOT NULL")
        )
    ...
    async def test_discovery_flow_linkage(self, session):
        ...
        total_result = await session.execute(text("SELECT COUNT(*) FROM migration.discovery_flows"))
        total_count = total_result.scalar()
        
        # Check linked discovery flows
        linked_result = await session.execute(
            text("SELECT COUNT(*) FROM migration.discovery_flows WHERE master_flow_id IS NOT NULL")
        )
        linked_count = linked_result.scalar()
    ...
    async def test_master_flow_utilization(self, session):
        ...
        total_result = await session.execute(text("SELECT COUNT(*) FROM migration.crewai_flow_state_extensions"))
        total_count = total_result.scalar()
    ...
    async def run_all_tests(self):
        ...
        try:
            ...
            self.test_result("Database Connection", False, f"Failed to connect: {str(e)}")
