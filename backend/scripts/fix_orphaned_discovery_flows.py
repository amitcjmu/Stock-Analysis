...
class DiscoveryFlowLinker:
    ...
    def find_matching_master(
        self, orphan: DiscoveryFlowOrphan, masters: List[MasterFlowMatch]
    ) -> Optional[MasterFlowMatch]:
        ...
        tenant_matches = [
            ...
            for master in masters
            if (
                master.client_account_id == orphan.client_account_id
                and master.engagement_id == orphan.engagement_id
            )
        ]
        ...
        if tenant_matches:
            ...
            closest_master = min(
                tenant_matches, key=lambda m: abs(m.created_at - orphan.created_at)
            )

            time_diff = abs(closest_master.created_at - orphan.created_at)
            if time_diff.total_seconds() <= 300:  # 5 minutes
                self.logger.debug(
                    f"Found timestamp match within {time_diff}: {closest_master.flow_name}"
                )
    ...
    async def link_discovery_flows(self, session) -> bool:
        ...
        for i, orphan in enumerate(orphans):
            try:
                self.logger.info(
                    f"Processing discovery flow {i+1}/{len(orphans)}: {orphan.flow_id}"
                )
                ...
                if self.dry_run:
                    self.logger.info(
                        f"DRY RUN: Would link {orphan.flow_id} to master {matching_master.flow_id}"
                    )
                    self.linked_count += 1
                else:
                    ...
                    self.logger.info(
                        f"Linked discovery flow {orphan.flow_id} to master {matching_master.flow_id}"
                    )
                    self.linked_count += 1

            except Exception as e:
                self.logger.error(
                    f"Error linking discovery flow {orphan.flow_id}: {str(e)}"
                )
                self.failed_count += 1
