...
class SchemaValidator:
    ...
    def _check_critical_fields(self):
        ...
        for table, expected_fields in CRITICAL_FIELDS.items():
            ...
            if missing_fields:
                self.errors.append(f"Table '{table}' missing fields: {sorted(missing_fields)}")
            else:
    ...
    def _check_removed_fields(self):
        ...
        for table in self.inspector.get_table_names():
            ...
            if found_removed:
                self.errors.append(f"Table '{table}' still has removed fields: {sorted(found_removed)}")
    ...
    def _check_indexes(self):
        ...
        for table in tables_needing_client_index:
            ...
            has_client_index = any(
                'client_account_id' in idx.get('column_names', [])
                for idx in indexes
            )
    ...
    def _check_foreign_keys(self):
        ...
        for table in tables_with_master_flow:
            ...
            has_master_flow_fk = any(
                'master_flow_id' in fk.get('constrained_columns', [])
                for fk in fks
            )
            
            if not has_master_flow_fk:
                self.warnings.append(f"Table '{table}' missing master_flow_id foreign key")
