...
class DataImportMigrator:
    ...
    async def identify_orphaned_records(
        self, session: Session
    ) -> Tuple[List[OrphanedRecord], List[OrphanedRecord]]:
    ...
    async def get_master_flows(self, session: Session) -> List[MasterFlow]:
        ...
        self.logger.info(
            f"Found {len(master_flows)} master flows available for linking"
        )
        return master_flows

    def find_matching_master_flow(
        self, orphan: OrphanedRecord, master_flows: List[MasterFlow]
    ) -> Optional[MasterFlow]:
        ...
        for flow in master_flows:
            if (
                flow.client_account_id == orphan.client_account_id
                and flow.engagement_id == orphan.engagement_id
                and orphan.id in flow.flow_name
            ):
                self.logger.debug(
                    f"Found exact match by name pattern: {flow.flow_name}"
                )
                return flow
        ...
        if closest_flow and min_time_diff <= timedelta(minutes=5):
            self.logger.debug(
                f"Found timestamp match within {min_time_diff}: {closest_flow.flow_name}"
            )
            return closest_flow
        ...
        if tenant_matches:
            closest_flow = min(
                tenant_matches, key=lambda f: abs(f.created_at - orphan.created_at)
            )
            time_diff = abs(closest_flow.created_at - orphan.created_at)
            self.logger.debug(
                f"Fallback match with time diff {time_diff}: {closest_flow.flow_name}"
            )
            return closest_flow

        return None

    async def link_orphaned_data_imports(
        self,
        session: Session,
        orphans: List[OrphanedRecord],
        master_flows: List[MasterFlow],
    ) -> MigrationResult:
        ...
        for i, orphan in enumerate(orphans):
            try:
                if self.verbose:
                    self.logger.info(
                        f"Processing data import {i+1}/{len(orphans)}: {orphan.id}"
                    )

                matching_flow = self.find_matching_master_flow(orphan, master_flows)
                if not matching_flow:
                    error_msg = (
                        f"No matching master flow found for data import {orphan.id}"
                    )
                ...
                if self.dry_run:
                    self.logger.info(
                        f"DRY RUN: Would link data import {orphan.id} to master flow {matching_flow.flow_name}"
                    )
                    ...
                    self.logger.info(
                        f"Linked data import {orphan.id} to master flow {matching_flow.flow_name}"
                    )
                    result.linked_successfully += 1
    ...
    async def link_orphaned_raw_records(
        self,
        session: Session,
        orphans: List[OrphanedRecord],
        master_flows: List[MasterFlow],
            ...
            for i, orphan in enumerate(batch):
                try:
                    if self.verbose:
                        ...
                        self.logger.info(
                            f"Processing raw record {overall_index}/{len(orphans)}: {orphan.id}"
                        )

                    matching_flow = self.find_matching_master_flow(orphan, master_flows)
                    if not matching_flow:
                        error_msg = (
                            f"No matching master flow found for raw record {orphan.id}"
                        )
                        self.logger.warning(error_msg)
                        result.errors.append(error_msg)
                        result.failed_to_link += 1
                    ...
                    if self.dry_run:
                        self.logger.debug(
                            f"DRY RUN: Would link raw record {orphan.id} to master flow {matching_flow.flow_name}"
                        )
                        result.linked_successfully += 1
                    else:
                        await session.execute(
...
"""
    ...
    async def run_migration(self) -> bool:
        ...
        try:
            async with AsyncSessionLocal() as session:
                ...
                if not master_flows:
                    self.logger.error(
                        "No master flows found. Cannot proceed with migration."
                    )
                    return False
                ...
                if orphaned_data_imports:
                    data_import_result = await self.link_orphaned_data_imports(
                        session, orphaned_data_imports, master_flows
                    )
                    self.stats["errors"].extend(data_import_result.errors)

                # Step 5: Link orphaned raw records
                if orphaned_raw_records:
                    raw_record_result = await self.link_orphaned_raw_records(
                        session, orphaned_raw_records, master_flows
                    )
    ...
    def _print_summary(self):
        ...
        print("=" * 60)
        print(
            f"Total orphaned data imports: {self.stats['total_orphaned_data_imports']}"
        )
