...
async def test_orphaned_data_discovery():
    ...
    async with AsyncSessionLocal() as db:
        ...
        try:
            ...
            try:
                status = await orchestrator.get_flow_status(TEST_FLOW_ID, include_details=True)
                ...
                if status.get("metadata", {}).get("orphaned_data_found"):
                    print(f"🔍 Orphaned data detected: {status['metadata']['orphaned_data_summary']}")
                ...
                if smart_status:
                    ...
                    if 'repair_options' in smart_status:
                        print(f"🔧 Repair options available: {len(smart_status['repair_options'])}")
...
async def test_orphaned_data_repair():
    ...
    async with AsyncSessionLocal() as db:
        ...
        try:
            ...
            discovered_data = await orchestrator._find_related_data_by_context(TEST_FLOW_ID)
            if not discovered_data:
                discovered_data = await orchestrator._find_related_data_by_timestamp(TEST_FLOW_ID)
            
            if discovered_data and "data_import" in discovered_data:
                data_import = discovered_data["data_import"]
                print(f"📦 Found orphaned data import: {data_import.id} ({data_import.filename})")
    ...
    async with AsyncSessionLocal() as db:
        try:
            ...
            for import_record in orphaned_imports:
                print(f"   - {import_record.id}: {import_record.filename} ({import_record.status})")
