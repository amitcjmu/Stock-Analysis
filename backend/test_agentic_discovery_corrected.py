...
async def test_master_flow_orchestrator():
    ...
    try:
        ...
        key_methods = ['create_master_flow', 'start_discovery_flow', 'update_master_flow_status']
...
async def test_hardcoded_threshold_removal():
    ...
    try:
        ...
        threshold_patterns = [
            "0.8", "0.85", "0.9", "0.95",  # Common confidence thresholds
            "threshold", "THRESHOLD",
        ...
        if found_thresholds:
            ...
            print("   Note: These may be acceptable if they're configurable or agent-driven")
        else:
            print("✅ No obvious hardcoded thresholds found")
        
        # Check for agent/LLM usage
        if "llm" in source.lower() or "agent" in source.lower() or "openai" in source.lower():
...
async def test_flow_execution_structure():
    ...
    try:
        ...
        for pattern in crewai_patterns:
            ...
            if matching_methods:
                print(f"✅ CrewAI pattern '{pattern}' found in methods: {matching_methods}")
            else:
