...
class FlowAnalyzer:
        ...
        for table_name in sorted(all_tables):
            try:
                ...
                for col in column_names:
                    ...
                    if any(t in col_type.lower() for t in ['char', 'text', 'uuid', 'string']):
                ...
                if rows:
                    ...
                    console.print(f"[green]✓[/green] Found {len(rows)} record(s) in [bold]{table_name}[/bold]")
                    
            except Exception as e:
                # Skip tables we can't query
                if "permission denied" not in str(e).lower():
                    console.print(f"[yellow]![/yellow] Error scanning {table_name}: {str(e)}")
        ...
        if unified_state:
            ...
            if unified_state.raw_data:
                analysis['persisted_data']['raw_data'] = {
                    ...
                    'sample_keys': list(unified_state.raw_data.keys())[:5] if isinstance(unified_state.raw_data, dict) else 'Not a dict'
                }
            ...
            if unified_state.cleaned_data:
                analysis['persisted_data']['cleaned_data'] = {
                    ...
                    'record_count': len(unified_state.cleaned_data) if isinstance(unified_state.cleaned_data, list) else 'Not a list'
                }
    ...
    def _determine_processing_status(self, analysis: Dict[str, Any]) -> Dict[str, str]:
        ...
        if analysis['field_mappings'] and analysis['field_mappings']['count'] > 0:
            approved_count = sum(1 for m in analysis['field_mappings']['mappings'] if m['is_approved'])
        ...
        # Finalization Phase
        if analysis['unified_state'] and analysis['unified_state']['status'] == 'completed':
    ...
    def _identify_gaps(self, analysis: Dict[str, Any]) -> List[str]:
        ...
        if analysis['unified_state']:
            ...
            if current_phase != expected_phase:
                gaps.append(f"Phase mismatch: current='{current_phase}', expected='{expected_phase}'")
    ...
    def _get_expected_phase(self, status: Dict[str, str]) -> str:
        ...
        elif status['field_mapping'] in ['COMPLETE_APPROVED', 'PARTIAL_APPROVED', 'COMPLETE_UNAPPROVED', 'READY_TO_START']:
    ...
    def display_analysis(self, table_results: Dict[str, Any], flow_analysis: Dict[str, Any]):
        ...
        if table_results:
            ...
            for table_name, data in table_results.items():
                ...
                if table_name == 'unified_discovery_flow_states':
                    if data['data']:
                        key_info.append(f"Phase: {data['data'][0].get('current_phase', 'N/A')}")
                        key_info.append(f"Status: {data['data'][0].get('status', 'N/A')}")
                elif table_name == 'field_mappings':
                    key_info.append(f"Mappings: {data['count']}")
                elif table_name == 'crewai_flow_state_extensions':
                    if data['data']:
                        key_info.append(f"Type: {data['data'][0].get('flow_type', 'N/A')}")
...
[bold]Error:[/bold] {state['error'] or 'None'}
            """
            console.print(Panel(state_info.strip(), title="Current Flow State", border_style="green"))
...
[bold]Reason:[/bold] {resume['reason']}
        ...
        console.print(Panel(recommendation.strip(), title="Resume Point", border_style=style))
        
        # 7. Display field mappings if present
        if flow_analysis['field_mappings'] and flow_analysis['field_mappings']['count'] > 0:
            ...
            for mapping in flow_analysis['field_mappings']['mappings'][:10]:  # Show first 10
    ...
    async def resume_flow(self, session: AsyncSession, resume_point: Dict[str, Any], force: bool = False):
        ...
        if resume_point['action'] == 'none':
            console.print("[yellow]Flow appears to be complete. Nothing to resume.[/yellow]")
            return
        ...
        try:
            if resume_point['phase'] == 'data_cleaning':
                ...
                if resume_point['action'] == 'save_existing':
                    console.print("[yellow]Saving existing field mappings to database...[/yellow]")
                    ...
                    if choice == '1':
                        ...
                        await session.execute(
                            text("UPDATE field_mappings SET is_approved = true WHERE discovery_flow_id = :flow_id"),
                            {'flow_id': self.flow_id}
                        )
                        await session.commit()
                        ...
                        await session.execute(
                            text("DELETE FROM field_mappings WHERE discovery_flow_id = :flow_id"),
                            {'flow_id': self.flow_id}
                        )
                        await session.commit()
                        
                        result = await flow_manager.process_phase('field_mapping')
                        console.print(f"[green]Field mapping regenerated: {result}[/green]")
                ...
                console.print(f"[red]Unknown resume phase: {resume_point['phase']}[/red]")
