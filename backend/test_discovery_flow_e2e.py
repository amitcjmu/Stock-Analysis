    ...
    try:
        ...
        async with AsyncSessionLocal() as db:
            ...
            master_flow = await flow_repo.create_master_flow(
                ...
                flow_configuration={"source": "e2e_test", "test": True},
            ...
            discovery_flow = create_unified_discovery_flow(
                ...
                metadata={"source": "e2e_test", "master_flow_id": flow_id},
            ...
            logger.info(
                f"  - Next phase: {result.next_phase.value if result.next_phase else 'None'}"
            )
            ...
            if (
                ...
                if hasattr(discovery_flow, "state") and hasattr(
                    ...
                    if (
                        "0" in field_mappings
                        or "1" in field_mappings
                        or "2" in field_mappings
                    ):
                        logger.error(
                            "❌ ERROR: Field mappings contain numeric indices instead of field names!"
                        )
                    else:
                ...
                logger.info(
                    f"  - Requires user input: {resume_result.requires_user_input}"
                )
                ...
                if (
                    resume_result.phase.value == "finalization"
            ...
            if master_flow_state:
                ...
                logger.info(
                    f"✅ Master flow phase data: {json.dumps(master_flow_state.flow_persistence_data, indent=2)}"
                )
            else:
