...
async def test_flow_status_etag(flow_id: str):
    ...
    async with httpx.AsyncClient() as client:
        ...
        response1 = await client.get(
            f"{BASE_URL}{API_PREFIX}/flows/{flow_id}/status", headers=HEADERS
        )
        ...
        if response1.status_code == 200:
            ...
            logger.info(
                f"   X-Flow-Updated-At: {response1.headers.get('X-Flow-Updated-At', 'Not present')}"
            )
            ...
            if etag:
                ...
                if response2.status_code == 304:
                    logger.info(
                        "✅ Second request returned 304 Not Modified - Efficient polling working!"
                    )
                    logger.info("   No body returned, saving bandwidth")
                else:
                    logger.warning(
                        f"❌ Second request returned {response2.status_code} instead of 304"
                    )
...
async def test_active_flows_etag():
    ...
    async with httpx.AsyncClient() as client:
        ...
        # First request
        response1 = await client.get(
            f"{BASE_URL}{API_PREFIX}/flows/active", headers=HEADERS
        )
        ...
        if response1.status_code == 200:
            ...
            if etag:
                ...
                response2 = await client.get(
                    f"{BASE_URL}{API_PREFIX}/flows/active", headers=headers_with_etag
                )
...
async def test_processing_status_etag(flow_id: str):
    ...
    async with httpx.AsyncClient() as client:
        ...
        # First request
        response1 = await client.get(
            f"{BASE_URL}{API_PREFIX}/flow/{flow_id}/processing-status", headers=HEADERS
        )
        ...
        if response1.status_code == 200:
            ...
            if etag:
                ...
                if response2.status_code == 304:
                    logger.info(
                        "✅ Polling returned 304 Not Modified - Bandwidth saved!"
                    )
                else:
                    logger.warning(
                        f"❌ Polling returned {response2.status_code} instead of 304"
                    )
        else:
            logger.error(f"❌ First request failed - Status: {response1.status_code}")
    ...
    async with httpx.AsyncClient() as client:
        ...
        while (datetime.now() - start_time).total_seconds() < duration_seconds:
            ...
            if response.status_code == 200:
                ...
                bytes_saved += 2048
                logger.info(
                    f"   Request #{request_count}: 304 Not Modified - Bandwidth saved"
                )
        ...
        logger.info(
            f"   Efficiency rate: {(not_modified_count/request_count)*100:.1f}%"
        )
