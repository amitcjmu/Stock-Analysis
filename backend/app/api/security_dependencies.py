...
def get_client_context(request: Request) -> RequestContext:
    ...
    return get_verified_context(
        request, require_client=True, require_engagement=False, require_user=False
    )
...
def get_project_context(request: Request) -> RequestContext:
    ...
    return get_verified_context(
        request, require_client=True, require_engagement=True, require_user=False
    )
...
def get_full_context(request: Request) -> RequestContext:
    ...
    return get_verified_context(
        request, require_client=True, require_engagement=True, require_user=True
    )


async def verify_tenant_access(
    db: AsyncSession, context: RequestContext, resource_type: str, resource_id: str
) -> bool:
    """
...
def SecureClient(request: Request = Depends()) -> RequestContext:
    """FastAPI dependency for client-scoped endpoints"""
    return get_client_context(request)


def SecureProject(request: Request = Depends()) -> RequestContext:
    """FastAPI dependency for project-scoped endpoints"""
    return get_project_context(request)


def SecureUser(request: Request = Depends()) -> RequestContext:
