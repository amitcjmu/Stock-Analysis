...
async def process_agent_learning_action(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to process learning action: {str(e)}"
        )


async def handle_field_mapping_action(
    learning_input: Dict[str, Any], db: AsyncSession, context: RequestContext
) -> Dict[str, Any]:
    ...
    try:
        ...
        if not action or not source_field or not target_field:
            raise HTTPException(
                status_code=400,
                detail="action, source_field, and target_field are required",
            )

        logger.info(
            f"🔄 Processing field mapping action: {action} for {source_field} -> {target_field}"
        )
        logger.info(
            f"🔧 Debug: FIELD_MAPPING_AVAILABLE={FIELD_MAPPING_AVAILABLE}, context.user_id={context.user_id}"
        )
        ...
        if action == "approve" and FIELD_MAPPING_AVAILABLE:
            ...
            try:
                ...
                if existing_mapping:
                    ...
                    existing_mapping.confidence_score = mapping_data.get(
                        "confidence", 0.9
                    )
                    ...
                    logger.info(
                        f"✅ Updated existing mapping: {source_field} -> {target_field}"
                    )
                else:
                    # Create new mapping (handle None user_id with fallback)
                    user_id = (
                        context.user_id or "3ee1c326-a014-4a3c-a483-5cfcf1b419d7"
                    )  # Demo user fallback
                    ...
                    db.add(new_mapping)
                    logger.info(
                        f"✅ Created new mapping: {source_field} -> {target_field}"
                    )
            ...
            logger.info(
                f"Field mapping action processed: {action} for {source_field} -> {target_field}"
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to {action} mapping: {str(e)}"
        )
...
async def handle_field_mapping_change(
    ...
    try:
        ...
        if not mapping_id or not new_mapping:
            raise HTTPException(
                status_code=400, detail="mapping_id and new_mapping are required"
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to update mapping: {str(e)}"
        )
...
async def answer_agent_clarification(
    ...
    return {"status": "success", "message": "Clarification processed successfully"}
...
async def process_agent_learning(
    ...
    return {"status": "success", "message": "Learning processed successfully"}


@router.get("/health")
async def learning_health():
    """Health check for learning endpoints."""
    return {"status": "healthy", "service": "learning", "version": "1.0.0"}
