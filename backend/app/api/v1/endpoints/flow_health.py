...
async def get_flow_health(
    ...
    try:
        ...
        flow_retry_metrics = {
            k: v for k, v in retry_metrics.retry_counts.items()
            if flow_id in k
        }
        ...
        flow_errors = [
            e for e in error_summary.get("recent_errors", [])
...
async def get_health_overview(
    ...
    try:
        ...
        for flow_metrics in all_metrics.get("flows", {}).values():
            status = flow_metrics.get("status", {}).value if hasattr(flow_metrics.get("status", {}), "value") else str(flow_metrics.get("status", "unknown"))
        ...
        return {
            ...
            "monitoring_active": flow_health_monitor._monitoring_task is not None and not flow_health_monitor._monitoring_task.done()
        }
...
async def recover_flow(
    flow_id: str,
    recovery_action: str = Query(..., description="Recovery action: restart, complete, fail, checkpoint_restore"),
    checkpoint_id: Optional[str] = Query(None, description="Checkpoint ID for restore action"),
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(get_current_user)
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if recovery_action == "checkpoint_restore":
            if not checkpoint_id:
                ...
                if not latest:
                    raise HTTPException(status_code=400, detail="No checkpoints available for restore")
