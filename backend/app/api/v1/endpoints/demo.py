...
async def get_demo_asset(
    asset_id: str, demo_repo: DemoRepository = Depends(get_demo_repository)
) -> DemoAssetResponse:
...
async def update_demo_asset(
    ...
    try:
        asset = await demo_repo.update_demo_asset(
            asset_id, asset_data.dict(exclude_unset=True)
        )
        if not asset:
            raise HTTPException(status_code=404, detail="Demo asset not found")
...
async def search_demo_assets_by_text(
    ...
    try:
        assets = await embedding_service.search_similar_assets(
            query_text=query_text, limit=limit, is_mock_only=True
        )
...
async def find_similar_demo_assets(
    ...
    try:
        ...
        similar_assets = await embedding_service.find_similar_assets(
            asset_id=asset_id, limit=limit, is_mock_only=True
        )
...
async def auto_tag_demo_asset(
    ...
    try:
        ...
        applied_tags = await demo_repo.apply_tags_to_asset(
            asset_id, [tag.name for tag in suggested_tags]
        )
