...
class FieldValidator:
    ...
    def validate_target_field(
        self, target_field: str, valid_targets: List[str]
    ) -> Dict[str, Any]:
    ...
    def validate_sample_values(self, values: List[Any]) -> Dict[str, Any]:
        ...
        result = {"valid": True, "errors": [], "warnings": [], "statistics": {}}
        ...
        if len(unique_values) == len(non_null_values) and len(non_null_values) > 10:
            result["warnings"].append(
                "All values are unique - might be an identifier field"
            )
    ...
    def validate_data_consistency(self, values: List[Any]) -> Dict[str, Any]:
        ...
        result = {"consistent": True, "issues": [], "type_analysis": {}}
        ...
        if len(type_counts) > 1:
            ...
            result["issues"].append(
                f"Mixed data types found: {list(type_counts.keys())}"
            )
    ...
    def _check_string_format_consistency(self, string_values: List[str]) -> bool:
        ...
        patterns = {
            ...
            "is_mixed_case": any(
                v != v.upper() and v != v.lower() for v in string_values
            ),
        }
    ...
    def suggest_validation_rule(
        self, field_name: str, sample_values: List[Any]
    ) -> Optional[str]:
