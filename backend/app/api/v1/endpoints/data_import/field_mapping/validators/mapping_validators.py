...
class MappingValidator:
    ...
    def __init__(self):
        ...
        self.required_fields = {"name", "asset_type"}

    async def validate_mapping(
        self, mapping: FieldMappingCreate
        ...
        try:
            ...
            for node in ast.walk(tree):
                ...
                if isinstance(node, ast.Subscript):
                    if isinstance(node.slice, ast.Str):
                        dangerous_keys = {"__builtins__", "__globals__", "__locals__"}
            ...
            has_only_safe_operations = all(
                ...
                or (
                    ...
                    and node.func.id in ["int", "float", "str", "len", "bool"]
                )
                for node in ast.walk(tree)
            )
    ...
    def validate_field_compatibility(
        ...
        result = {"compatible": True, "issues": [], "recommendations": []}

        # Check field name compatibility
        if source_field.lower() == target_field.lower():
            result["recommendations"].append(
                "Direct field name match - high confidence"
            )
