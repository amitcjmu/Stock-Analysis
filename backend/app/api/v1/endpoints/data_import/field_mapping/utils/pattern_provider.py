...
class LearnedPatternProvider(FieldPatternProvider):
    ...
    def add_learned_pattern(self, target_field: str, source_patterns: List[str]) -> None:
...
class HeuristicPatternProvider(FieldPatternProvider):
    ...
    def _get_default_patterns(self) -> Dict[str, List[str]]:
        ...
        return {
            "hostname": ["host_name", "server_name", "servername", "name", "host", "server"],
            "ip_address": ["ip", "ipaddress", "ip_addr", "address", "private_ip", "public_ip"],
            "operating_system": ["os", "os_name", "operating_sys", "platform", "os_type", "system"],
            "os": ["os", "os_name", "operating_sys", "platform", "os_type", "system"],
            "cpu": ["cpu", "cores", "processors", "vcpu", "cpu_count", "num_cpus", "cpus"],
            "memory": ["memory", "ram", "ram_gb", "mem", "total_memory", "memory_size", "ram (gb)"],
            "ram": ["memory", "ram", "ram_gb", "mem", "total_memory", "memory_size", "ram (gb)"],
            "storage": ["storage", "disk", "disk_gb", "disk_space", "total_storage", "disk_size"],
            "disk": ["storage", "disk", "disk_gb", "disk_space", "total_storage", "disk_size"],
        }
...
class PluggablePatternManager:
    ...
    def register_provider(self, provider: FieldPatternProvider) -> None:
        ...
        logger.info(f"Registered pattern provider: {provider.__class__.__name__} with priority {provider.get_priority()}")
    ...
    def get_patterns(self, target_field: str) -> List[str]:
        ...
        for provider in self.providers:
            try:
                ...
                logger.warning(f"Error getting patterns from {provider.__class__.__name__}: {e}")
        
        return all_patterns

    def build_field_patterns(self, target_field_names: List[str]) -> Dict[str, List[str]]:
...
def configure_pattern_manager(learned_patterns: Optional[Dict[str, List[str]]] = None,
                             heuristic_config: Optional[Dict[str, List[str]]] = None,
                             custom_providers: Optional[List[FieldPatternProvider]] = None) -> None:
