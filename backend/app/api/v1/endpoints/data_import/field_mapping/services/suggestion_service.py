...
class SuggestionService:
    ...
    async def get_field_mapping_suggestions(
        ...
        ).limit(5)  # Get first 5 records for analysis
        ...
        logger.info(f"Analyzing {len(source_fields)} fields with {len(sample_data)} sample records")
    ...
    async def regenerate_suggestions(
        ...
        logger.info(f"Regenerating suggestions for import {import_id} with feedback: {feedback}")
    ...
    async def get_suggestion_confidence_metrics(self, import_id: str) -> Dict[str, Any]:
        ...
        confidence_levels = {
            ...
            "medium": len([s for s in analysis.suggestions if 0.5 <= s.confidence < 0.8]),
