...
class MappingService:
    ...
    async def generate_mappings_for_import(self, import_id: str) -> Dict[str, Any]:
        ...
        logger.info(
            "🔍 Field mappings should have been generated by discovery flow field mapping phase"
        )
        logger.info(
            "📋 The discovery flow contains a real CrewAI field mapping crew that automatically generates mappings"
        )
        logger.error(
            "❌ No field mappings found - discovery flow may not have completed field mapping phase"
        )
        raise RuntimeError(
            f"Field mappings not found for import {import_id}. The discovery flow should have generated these automatically via the real CrewAI field mapping phase."
        )
