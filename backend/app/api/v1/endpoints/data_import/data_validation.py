...
async def validate_file_upload(
    ...
    try:
        ...
        try:
            ...
            elif file.content_type == "application/json" or file.filename.endswith(
                ".json"
            ):
                import json
        ...
        return {
            ...
            "security_clearances": {
                "format_validation": any(
                    r["agent_id"] == "format_validator" and r["validation"] == "passed"
                    for r in agent_results
                ),
                "security_clearance": any(
                    r["agent_id"] == "security_scanner" and r["validation"] == "passed"
                    for r in agent_results
                ),
                "privacy_clearance": any(
                    r["agent_id"] == "privacy_analyzer" and r["validation"] == "passed"
                    for r in agent_results
                ),
            },
            "next_step": (
                "attribute_mapping"
                if final_status in ["approved", "approved_with_warnings"]
                else "fix_issues"
            ),
        }
...
async def simulate_agent_validation(
    ...
    if agent_id == "format_validator":
        ...
        detected_threats = [
            pattern for pattern in threat_patterns if pattern in content_str
        ]
            ...
            result.update(
                {
                    ...
                    "details": [
                        f"Detected pattern: {threat}" for threat in detected_threats
                    ],
                }
            )
        else:
                ...
                result.update(
                    {
                        ...
                        "details": [
                            "File appears to be empty or contain insufficient data"
                        ],
                    }
                )
            else:
