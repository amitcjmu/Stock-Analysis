...
async def get_agentic_critical_attributes(
    ...
    try:
        ...
        logger.info(
            f"Analysis completed: {result.analysis_id}, success: {result.success}"
        )
        return result
...
async def trigger_field_mapping_crew_analysis(
    ...
    try:
        ...
        if request.background_execution:
            ...
            execution_id = (
                f"crew_{request.import_id}_{int(datetime.utcnow().timestamp())}"
            )
            ...
            crew_result = await coordinator.execute_crew_analysis(
                data_import,
                sample_data,
                request.parameters.get("analysis_depth", "standard"),
            )
            ...
            return CrewExecutionResponse(
                ...
                status=(
                    "completed"
                    if crew_result.get("crew_execution") == "completed"
                    else "failed"
                ),
...
async def get_analysis_status(
    analysis_id: str, analyzer: AttributeAnalyzer = Depends(get_attribute_analyzer)
):
