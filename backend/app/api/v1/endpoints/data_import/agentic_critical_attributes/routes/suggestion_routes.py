...
async def get_attribute_suggestions(
    ...
    try:
        ...
        filtered_suggestions = [
            suggestion for suggestion in analysis_result.suggestions
            if suggestion.confidence_score >= min_confidence
        ]
        
        logger.info(f"Returning {len(filtered_suggestions)} suggestions (min confidence: {min_confidence})")
        return filtered_suggestions
        
    except Exception as e:
        logger.error(f"Error getting attribute suggestions: {e}")
        raise HTTPException(status_code=500, detail="Failed to get attribute suggestions")
...
async def get_critical_attributes(
    ...
    try:
        ...
        critical_attributes = [
            attr for attr in analysis_result.attributes
        ...
        logger.info(f"Returning {len(critical_attributes)} critical attributes (min importance: {min_importance})")
...
async def get_field_mapping_recommendations(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Failed to get mapping recommendations")
