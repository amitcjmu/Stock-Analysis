...
class AttributeAnalyzer:
    ...
    async def analyze_critical_attributes(
        self, request: AttributeAnalysisRequest
    ) -> AttributeAnalysisResponse:
        """
        ...
        try:
            ...
            if not request.force_reanalysis:
                ...
                if cached_result:
                    logger.info(
                        f"📊 Returning cached analysis for import {data_import.id}"
                    )
            ...
            logger.info(
                f"✅ Analysis {analysis_id} completed in {statistics.analysis_duration_seconds:.2f}s"
            )
            return response
    ...
    async def _get_sample_data(self, data_import: DataImport) -> List[Dict[str, Any]]:
        ...
        query = (
            ...
            .limit(10)
        )  # Get first 10 records for analysis
    ...
    async def _execute_fallback_analysis(
        self, data_import: DataImport, sample_data: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
    ...
    def _calculate_field_importance(self, field_name: str, values: List[Any]) -> float:
        ...
        if any(
            pattern in field_lower for pattern in ["type", "os", "env", "cpu", "memory"]
        ):
            importance += 0.3

        # Business importance
        if any(
            pattern in field_lower for pattern in ["owner", "dept", "critical", "app"]
        ):
            importance += 0.2
    ...
    def _extract_critical_attributes(
        self, analysis_result: Dict[str, Any]
    ) -> List[CriticalAttribute]:
        ...
        for attr_data in attributes_data:
            try:
                ...
                logger.warning(
                    f"Failed to parse attribute {attr_data.get('name', 'unknown')}: {e}"
                )
    ...
    def _extract_suggestions(
        ...
        for sugg_data in suggestions_data:
            try:
                ...
                logger.warning(
                    f"Failed to parse suggestion for {sugg_data.get('source_field', 'unknown')}: {e}"
                )
    ...
    async def _get_cached_analysis(
        self, import_id: str
    ) -> Optional[AttributeAnalysisResponse]:
        """Get cached analysis result if available."""
        return self._analysis_cache.get(import_id)

    async def _cache_analysis_result(
        self, import_id: str, response: AttributeAnalysisResponse
    ):
    ...
    def _create_no_data_response(
        self, analysis_id: str, start_time: datetime
    ) -> AttributeAnalysisResponse:
    ...
    def _create_error_response(
        self, analysis_id: str, start_time: datetime, error: str
    ) -> AttributeAnalysisResponse:
