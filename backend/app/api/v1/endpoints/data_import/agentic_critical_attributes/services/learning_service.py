...
class LearningService:
    ...
    def __init__(self, db: AsyncSession, context: RequestContext):
        ...
        self._feedback_store = (
            []
        )  # Simple in-memory store - would use database in production
    ...
    async def get_learning_insights(self) -> Dict[str, Any]:
        ...
        correct_count = len(
            [f for f in self._feedback_store if f.feedback_type == "correct"]
        )
        incorrect_count = len(
            [f for f in self._feedback_store if f.feedback_type == "incorrect"]
        )
    ...
    async def get_attribute_recommendations(
        ...
        related_patterns = [
            pattern
            for key, pattern in self._learning_patterns.items()
        ...
        if confidence_adjustments:
            avg_confidence_adjustment = sum(confidence_adjustments) / len(
                confidence_adjustments
            )
        ...
        if avg_confidence_adjustment < -0.2:
            recommendations.append(
                "Consider reducing importance score for this attribute"
            )
        elif avg_confidence_adjustment > 0.2:
            recommendations.append(
                "Consider increasing importance score for this attribute"
            )

        if corrections:
            unique_corrections = list(set(corrections))
            recommendations.extend(
                [f"User suggested: {correction}" for correction in unique_corrections]
            )
        ...
        try:
            ...
            for fb_data in feedback_data:
                ...
                feedback = AgentFeedback(
                    ...
                    timestamp=(
                        datetime.fromisoformat(fb_data["timestamp"])
                        if fb_data.get("timestamp")
            ...
            logger.info(
                f"Imported {imported_feedback} feedback records and {len(imported_patterns)} patterns"
            )
