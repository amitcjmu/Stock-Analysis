...
async def learn_from_user_mapping(
    ...
    try:
        ...
        if learning_data.get("is_custom_field"):
            await update_custom_field_usage(
                learning_data["corrected_target_field"],
                learning_data.get("was_correct", True),
                db,
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to learn from mapping: {str(e)}"
        )
...
async def update_custom_field_usage(
    field_name: str, was_successful: bool, db: AsyncSession
):
    ...
    try:
        query = select(CustomTargetField).where(
            and_(
                ...
                CustomTargetField.client_account_id
                == "d838573d-f461-44e4-81b5-5af510ef83b7",
            )
        )
        ...
        if custom_field:
            ...
            # Update success rate
            if was_successful:
                current_successes = custom_field.success_rate * (
                    custom_field.usage_count - 1
                )
                custom_field.success_rate = (
                    current_successes + 1
                ) / custom_field.usage_count
            else:
                current_successes = custom_field.success_rate * (
                    custom_field.usage_count - 1
                )
...
async def get_learning_statistics(db: AsyncSession = Depends(get_db)):
    ...
    #     return {
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get learning statistics: {str(e)}"
        )
