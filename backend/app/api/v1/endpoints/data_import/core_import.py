...
async def clear_cache(request: Request = None, db: AsyncSession = Depends(get_db)):
    """
    ...
    try:
        ...
        if request:
            try:
                ...
                logger.warning(
                    f"Could not extract context for cache clearing: {context_error}"
                )
        ...
        return {
            ...
            "cleared_caches": ["sqlalchemy_session", "application_cache"],
