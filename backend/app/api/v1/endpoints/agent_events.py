        ...
        try:
            while True:
                ...
                try:
                    ...
                    current_version = current_state.version if hasattr(current_state, 'version') else last_version + 1
                    ...
                    if current_version > last_version:
                        ...
                        update_data = {
                            "flow_id": flow_id,
                            "status": current_state.flow_status if hasattr(current_state, 'flow_status') else "unknown",
                            "current_phase": current_state.current_phase if hasattr(current_state, 'current_phase') else None,
                            "progress": current_state.progress_percentage if hasattr(current_state, 'progress_percentage') else 0,
                    ...
                    agent_messages = agent_ui_bridge.get_pending_messages(flow_id, since_version=last_version)
                    ...
                    if error_count >= max_errors:
                        logger.error(f"Too many errors in SSE stream for flow {flow_id}, closing connection")
                        yield {
                            "event": "error",
                            "data": json.dumps({
...
async def get_flow_status_with_etag(
    ...
    response_data = {
        "flow_id": flow_id,
        "status": flow_state.flow_status if hasattr(flow_state, 'flow_status') else "unknown",
        "current_phase": flow_state.current_phase if hasattr(flow_state, 'current_phase') else None,
        "progress": flow_state.progress_percentage if hasattr(flow_state, 'progress_percentage') else 0,
        "agent_insights": insights,
        "updated_at": flow_state.updated_at.isoformat() if hasattr(flow_state, 'updated_at') else datetime.utcnow().isoformat(),
