...
async def get_crewai_flow_status(
    request: Request, service: CrewAIFlowService = Depends(get_crewai_flow_service)
):
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get CrewAI flow status: {str(e)}"
        )
...
async def get_specific_flow_details(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get flow details: {str(e)}"
        )


@router.get("/crewai-flows/{flow_id}/agent-tasks")
async def get_flow_agent_tasks(
    flow_id: str, service: CrewAIFlowService = Depends(get_crewai_flow_service)
):
    """
    ...
    try:
        ...
        if "error" in task_history:
            ...
            if not flow_status or flow_status.get("status") == "not_found":
                raise HTTPException(
                    status_code=404, detail=f"Flow not found: {flow_id}"
                )
        ...
        return {
            ...
            "agent_tasks": {
                ...
                "statistics": {
                    ...
                    "success_rate": (
                        task_history.get("completed_tasks", 0)
                        / max(task_history.get("total_tasks", 1), 1)
                    )
                    * 100,
                    "total_duration": sum(t.get("duration", 0) for t in all_tasks),
                    "avg_task_duration": sum(t.get("duration", 0) for t in all_tasks)
                    / max(len(all_tasks), 1),
