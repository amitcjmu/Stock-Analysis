...
@validation_router.get(
    "/flow/{flow_id}/validation-status", response_model=Dict[str, Any]
)
async def get_flow_validation_status(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get validation status: {str(e)}"
        )
...
async def validate_flow(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to validate flow: {str(e)}"
        )
...
async def check_flow_prerequisites(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to check prerequisites: {str(e)}"
        )
        ...
        healthy_flows_stmt = select(func.count(DiscoveryFlow.flow_id)).where(
            and_(
                ...
                DiscoveryFlow.status.in_(
                    ["active", "running", "processing", "completed"]
                ),
            )
        )
...
async def get_system_diagnostics(
    ...
    try:
        ...
        phase_distribution = {
            row.current_phase or "unknown": row.count for row in phase_result.fetchall()
        }
        ...
        recent_activity_stmt = (
            ...
            .order_by(DiscoveryFlow.updated_at.desc())
            .limit(10)
        )
        ...
        return {
                ...
                "performance_metrics": {
                    ...
                    "error_rate": (
                        len(
                            [
                                s
                                for s in status_distribution.keys()
            ...
            "recommendations": _get_diagnostic_recommendations(
                status_distribution, phase_distribution
            ),
    ...
    try:
        ...
        if flow.status in ["failed", "error"]:
            errors.append(
                {
                    "type": "flow_error",
                    "message": f"Flow is in error state: {flow.status}",
                    "severity": "critical",
                }
            )
        else:
        ...
        for i, phase in enumerate(StatusCalculator.PHASE_ORDER):
            if phase_completion.get(phase, False):
                checks_passed += 1
            elif i > 0 and phase_completion.get(
                StatusCalculator.PHASE_ORDER[i - 1], False
            ):
        ...
        return {
            ...
            "validation_score": 0,
            "errors": [
                {"type": "validation_error", "message": str(e), "severity": "critical"}
            ],
            "warnings": [],
...
async def _perform_phase_validation(
    flow, phase: str, db: AsyncSession
) -> Dict[str, Any]:
...
async def _perform_full_validation(
    flow, db: AsyncSession, include_data_quality: bool, include_configuration: bool
) -> Dict[str, Any]:
...
async def _check_phase_prerequisites(
    flow, phase: str, db: AsyncSession
) -> Dict[str, Any]:
    ...
    try:
        ...
        if phase == "field_mapping":
            checks.append(
                {
                    "name": "data_import_completed",
                    "status": "passed" if flow.data_import_completed else "failed",
                }
            )
            if not flow.data_import_completed:
                blocking_issues.append(
                    "Data import must be completed before field mapping"
                )
            ...
            checks.append(
                {
                    ...
                    "status": "passed" if flow.field_mapping_completed else "failed",
                }
            )
            if not flow.field_mapping_completed:
                blocking_issues.append(
                    "Field mapping must be completed before data cleansing"
                )

        elif phase == "asset_inventory":
            checks.append(
                {
                    "name": "data_cleansing_completed",
                    "status": "passed" if flow.data_cleansing_completed else "failed",
                }
            )
            if not flow.data_cleansing_completed:
                blocking_issues.append(
                    "Data cleansing must be completed before asset inventory"
                )
...
def _get_system_recommendations(health_score: float, failed_flows: int) -> List[str]:
    ...
    if health_score < 75:
        recommendations.append(
            "System health is below optimal. Consider investigating failed flows."
        )

    if failed_flows > 0:
        recommendations.append(
            f"There are {failed_flows} failed flows that need attention."
        )
...
def _get_diagnostic_recommendations(
    status_distribution: Dict[str, int], phase_distribution: Dict[str, int]
) -> List[str]:
    ...
    failed_count = status_distribution.get("failed", 0) + status_distribution.get(
        "error", 0
    )
    if failed_count > 0:
        recommendations.append(
            f"Address {failed_count} failed flows to improve system health."
        )
...
def _get_validation_recommendations(
    ...
    if critical_errors:
        recommendations.append(
            "Address critical errors immediately to ensure flow stability."
        )

    if len(warnings) > 0:
        recommendations.append(
            "Review and address warnings to improve flow reliability."
        )
