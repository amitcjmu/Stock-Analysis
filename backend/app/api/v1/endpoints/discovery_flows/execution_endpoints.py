...
@execution_router.post("/flow/{flow_id}/resume-intelligent", response_model=Dict[str, Any])
    ...
    try:
        ...
        if not force_execution:
            if execution_phase == "field_mapping" and not flow.data_import_completed:
                raise HTTPException(status_code=400, detail="Cannot execute field_mapping phase: data_import not completed")
            elif execution_phase == "data_cleansing" and not flow.field_mapping_completed:
...
async def retry_flow_execution(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to retry flow execution: {str(e)}")