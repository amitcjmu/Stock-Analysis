...
class StatusCalculator:
    ...
    def calculate_progress_percentage(flow, current_phase: str = None) -> float:
        ...
        try:
            ...
            if flow.data_import_completed and (
                ...
                or flow.current_phase in ["field_mapping", "attribute_mapping"]
            ):
                ...
                if not flow.field_mapping_completed:
                    min_progress += (
                        0.5 / total_phases
                    ) * 100  # Add 8.3% for in-progress field mapping
    ...
    def determine_final_status(
        ...
        try:
            ...
            if flow_status == "active" and not awaiting_approval:
                return "processing"
            elif progress_percentage >= 100 and flow_status not in [
                "paused",
    ...
    def calculate_processing_status(flow, phase: str = None) -> Dict[str, Any]:
        ...
        """
        try:
            current_phase, steps_completed = StatusCalculator.calculate_current_phase(
                flow
            )
            progress_percentage = StatusCalculator.calculate_progress_percentage(
                flow, current_phase
            )
            phase_completion = StatusCalculator.build_phase_completion_dict(flow)
