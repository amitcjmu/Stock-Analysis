...
async def delete_discovery_flow(
    ...
    try:
        ...
        if not StatusCalculator.can_flow_be_deleted(flow):
            raise HTTPException(status_code=400, detail="Flow cannot be deleted in current state")
...
async def clone_discovery_flow(
    ...
    try:
        ...
        if not source_flow:
            raise HTTPException(status_code=404, detail=f"Source flow {flow_id} not found")
        ...
        clone_name = request.get("flow_name", f"Clone of {source_flow.flow_name or flow_id}")
...
async def archive_discovery_flow(
    ...
    try:
        ...
        if flow.status in ["running", "processing", "active"]:
            raise HTTPException(status_code=400, detail="Cannot archive an active flow. Please pause or complete it first.")
...
async def restore_discovery_flow(
    ...
    try:
        ...
        if flow.status != "archived":
            raise HTTPException(status_code=400, detail="Only archived flows can be restored")
        ...
        logger.info(f"✅ Flow {flow_id} restored successfully to status: {previous_status}")
        ...
        return {
            ...
            "current_phase": flow.current_phase,
            "next_phase": StatusCalculator.get_next_phase(flow.current_phase or "unknown"),
