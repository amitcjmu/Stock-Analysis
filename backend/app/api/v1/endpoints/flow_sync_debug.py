...
async def pause_flow_atomic(
    ...
    try:
        orchestrator = MasterFlowOrchestrator(db, context)
        result = await orchestrator.pause_flow_with_atomic_sync(flow_id, flow_type, reason)
        return result
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def update_operational_status(
    ...
    try:
        sync_service = FlowStatusSyncService(db, context)
        await sync_service.update_operational_status(flow_id, flow_type, status, metadata)
...
async def debug_flow_status(
    ...
    try:
        ...
        master_repo = CrewAIFlowStateExtensionsRepository(db, context.client_account_id, context.engagement_id, context.user_id)
        master_flow = await master_repo.get_by_flow_id(flow_id)
        
        # Get child flow status (assuming discovery for now)
        discovery_repo = DiscoveryFlowRepository(db, context.client_account_id, context.engagement_id, context.user_id)
        ...
        return {
            ...
            "master_flow": {
                ...
                "updated_at": master_flow.updated_at.isoformat() if master_flow and master_flow.updated_at else None
            },
            "child_flow": {
                ...
                "updated_at": child_flow.updated_at.isoformat() if child_flow and child_flow.updated_at else None
            }
        }
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))