...
try:
    from app.services.agent_learning.core.learning.client_context import (
...
async def learn_field_mapping_pattern(
    request: Request, learning_data: Dict[str, Any] = Body(...)
):
...
async def suggest_field_mapping(
    request: Request, field_name: str, context_data: Optional[Dict[str, Any]] = None
):
    ...
    try:
        ...
        suggestion = await agent_learning_system.suggest_field_mapping(
            field_name, context_data, context
        )
        return {
...
async def learn_data_source_pattern(source_data: Dict[str, Any] = Body(...)):
...
async def learn_quality_assessment(quality_data: Dict[str, Any] = Body(...)):
...
async def learn_user_preferences(
    preference_data: Dict[str, Any] = Body(...), engagement_id: Optional[str] = None
):
    """Learn user preferences for client/engagement-specific context."""
    try:
        await agent_learning_system.learn_user_preferences(
            preference_data, engagement_id
        )
        return {
            "success": True,
...
async def track_agent_performance(
    agent_id: str, performance_data: Dict[str, Any] = Body(...)
):
...
async def get_agent_accuracy_metrics(agent_id: str):
    ...
    try:
        ...
        return {"agent_id": agent_id, "accuracy_metrics": metrics}
    except Exception as e:
...
async def set_cross_page_context(context_data: Dict[str, Any]):
...
async def get_cross_page_context(
    key: str, service: CrewAIFlowService = Depends(get_crewai_flow_service)
):
    # This functionality is now part of the UIInteractionHandler within the flow service
    # context = service.ui_interaction_handler.context_handler.get_cross_page_context(key)
    return {
        "status": "deprecated",
        "message": "Functionality moved to flow-based service.",
    }
...
async def get_agent_state(
    agent_id: str, service: CrewAIFlowService = Depends(get_crewai_flow_service)
):
    # This functionality is now managed within individual agent flows.
...
async def get_learning_experiences(
    ...
    return {
        "status": "deprecated",
        "message": "Functionality moved to agent_learning_system.",
    }
...
async def get_agent_learning_health(
    ...
    try:
        ...
        return {
            "status": "healthy",
            "agent_learning_system": {"status": "active", "statistics": learning_stats},
...
async def learn_from_user_response(response_data: Dict[str, Any] = Body(...)):
    ...
    try:
        ...
        if engagement_id and client_context_manager:
            await client_context_manager.store_clarification_response(
                engagement_id, response_data
            )
        ...
        if response_data.get("share_across_pages"):
            context_key = (
                f"user_response_{response_type}_{datetime.utcnow().timestamp()}"
            )
            agent_ui_bridge.set_cross_page_context(
                context_key, response_data, response_data.get("page_source", "unknown")
            )
