...
class UserService:
        ...
        if basic_context.engagement:
            try:
                ...
                if not basic_context.client:
                    logger.warning(
                        f"User {current_user.id} has engagement but no client context"
                    )
                    ...
                    for flow in flows:
                        if flow.get("status") in ["active", "in_progress", "pending"]:
                            flow_info = FlowBase(
                                id=flow.get(
                                    "id", flow.get("flow_id")
                                ),  # Handle both id and flow_id keys
                                name=flow.get(
                                    "name", flow.get("flow_name", "Unnamed Flow")
                                ),
                                flow_type=flow.get("flow_type", "unknown"),
                                status=flow.get("status", "unknown"),
                                engagement_id=flow.get(
                                    "engagement_id", basic_context.engagement.id
                                ),  # Use context engagement as fallback
                            ...
                            if (
                                flow.get("flow_type") == "discovery"
                                and not primary_flow_id
                            ):
                                primary_flow_id = flow.get("id", flow.get("flow_id"))

            except AttributeError as e:
                logger.error(
                    f"AttributeError getting flows for user {current_user.id}: {e}",
                    exc_info=True,
                )
                logger.error(
                    f"Basic context state - client: {basic_context.client}, engagement: {basic_context.engagement}"
                )
                # Continue with empty flows list
            except KeyError as e:
                logger.error(
                    f"KeyError getting flows for user {current_user.id}: Missing key {e}",
                    exc_info=True,
                )
                # Continue with empty flows list
            except Exception as e:
                logger.warning(
                    f"Failed to get active flows for user {current_user.id}: {e}"
                )
                # Continue with empty flows list
    ...
    async def get_user_context(self, current_user: User) -> UserContext:
        ...
        is_platform_admin, user_role = await self._get_user_admin_status(
            current_user.id
        )

        logger.info(
            f"Context for user {current_user.id}: admin={is_platform_admin}, role={user_role}"
        )

        # Handle platform admin access
        if is_platform_admin:
            context = await self._get_platform_admin_context(
                current_user, user_role, now
            )
            if context:
                return context
    ...
    async def _get_user_admin_status(self, user_id: UUID) -> tuple[bool, str]:
        ...
        is_platform_admin = (
            user_role_obj and user_role_obj.role_type == "platform_admin"
        )

        user_role = (
            "admin"
            if is_platform_admin
            else (user_role_obj.role_type if user_role_obj else "analyst")
        )

        return is_platform_admin, user_role

    async def _get_platform_admin_context(
        self, current_user: User, user_role: str, now: datetime
    ) -> Optional[UserContext]:
        """Get context for platform admin"""
        logger.info(
            "Platform admin detected - providing access to all clients/engagements"
        )
        ...
        if target_client:
            ...
            if current_user.default_engagement_id:
                target_engagement = await self._get_engagement_by_id(
                    current_user.default_engagement_id, target_client.id
                )

            # If no default engagement, use first available
            if not target_engagement:
                target_engagement = await self._get_first_active_engagement(
                    target_client.id
                )
    ...
    async def _get_engagement_by_id(
        self, engagement_id: UUID, client_id: UUID
    ) -> Optional[Engagement]:
    ...
    async def _get_first_active_engagement(
        self, client_id: UUID
    ) -> Optional[Engagement]:
    ...
    def _create_demo_context(
        self, user: User, user_role: str, now: datetime
    ) -> UserContext:
        """Create demo context"""
