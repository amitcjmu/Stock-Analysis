...
class ValidationService:
    ...
    async def validate_context(
        ...
        if user_id and client_id:
            has_access, access_error = await self._validate_user_access(
                user_id, client_id
            )
            if not has_access:
    ...
    async def _validate_engagement(
        self, engagement_id: str, client_id: str
        ...
        try:
            ...
            if not engagement:
                return (
                    False,
                    f"Engagement {engagement_id} not found or doesn't belong to client",
                )
