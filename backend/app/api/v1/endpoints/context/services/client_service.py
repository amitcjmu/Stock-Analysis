...
class ClientService:
    ...
    async def get_user_role(self, user_id: UUID) -> tuple[str, Optional[str]]:
        ...
        try:
            ...
            if user_roles:
                ...
                highest_role = max(
                    user_roles, key=lambda r: role_hierarchy.get(r.role_type, 0)
                )
    ...
    def create_demo_context(
        self, user_id: str, user_email: str, user_role: str
    ) -> UserContext:
    ...
    async def get_user_context_from_access(
        self, user: User, user_role: str
    ...
    async def get_user_clients(
        self, user_id: UUID, is_platform_admin: bool
    ) -> ClientsListResponse:
