...
async def update_user_defaults(
    ...
    try:
        ...
        result = await service.update_user_defaults(
            current_user, request.client_id, request.engagement_id
        )
...
async def activate_flow(
    ...
    if not flow_id:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="flow_id is required"
        )
    ...
    try:
        ...
        if not user_context.engagement or str(user_context.engagement.id) != str(
            flow_info.get("engagement_id")
        ):
            raise HTTPException(
        ...
        await service.update_user_defaults(
            current_user, engagement_id=str(flow_info.get("engagement_id"))
        )
