...
from ..models.context_schemas import (ValidateContextRequest,
...
@router.get("/context")
async def get_context(context=Depends(get_current_context)) -> dict:
    """
...
async def validate_context(
    request: ValidateContextRequest, db: AsyncSession = Depends(get_db)
) -> ValidateContextResponse:
    """
    ...
    return await service.validate_context(
        request.client_id, request.engagement_id, request.user_id
    )
