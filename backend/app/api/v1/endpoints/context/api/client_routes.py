    ...
    try:
        ...
        if str(current_user.id) == "44444444-4444-4444-4444-444444444444":
            context = service.create_demo_context(
                str(current_user.id), current_user.email, user_role
            )
            ...
            if context.client:
                return {
                    ...
                    "type": "enterprise",
                    "created_at": (
                        context.client.created_at.isoformat()
                        if context.client.created_at
                        else None
                    ),
                    "updated_at": (
                        context.client.updated_at.isoformat()
                        if context.client.updated_at
                        else None
                    ),
                    "metadata": {
                        "industry": "Technology",
        ...
        if context and context.client:
            return {
                ...
                "metadata": {
                    ...
                    "engagement_id": (
                        str(context.engagement.id) if context.engagement else None
                    ),
                    "engagement_name": (
                        context.engagement.name if context.engagement else None
                    ),
                },
            }
