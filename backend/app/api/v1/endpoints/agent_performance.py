...
async def get_agent_performance(
    ...
    try:
        ...
        active_tasks = [
            task for task in monitor_status.get("active_task_details", [])
...
async def get_agent_task_history(
    ...
    status_filter: Optional[str] = Query(None, description="Filter by task status (completed, failed, timeout)"),
    db: AsyncSession = Depends(get_db),
    context: RequestContext = Depends(get_request_context)
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to get agent task history: {str(e)}"
        )
...
async def get_agent_analytics(
    agent_name: str,
    period_days: int = Query(default=7, ge=1, le=90, description="Analysis period in days"),
    db: AsyncSession = Depends(get_db),
    context: RequestContext = Depends(get_request_context)
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if "error" in analytics:
            raise HTTPException(
                status_code=500,
                detail=f"Failed to get analytics: {analytics['error']}"
            )
        
        # Get aggregated performance trends
        performance_trends = agent_performance_aggregation_service.get_agent_performance_trends(
            agent_name=agent_name,
            client_account_id=context.client_account_id,
            engagement_id=context.engagement_id,
            days=period_days
...
async def get_agents_activity_feed(
    limit: int = Query(default=100, ge=1, le=500, description="Maximum number of activities"),
    agent_filter: Optional[str] = Query(None, description="Filter by specific agent name"),
    ...
    try:
        ...
        if include_completed:
            ...
            if "tasks" in recent_tasks:
                for task in recent_tasks["tasks"]:
                    if agent_filter and task.get("agent_name", "").lower() != agent_filter.lower():
                        continue
        ...
        logger.error(
            f"Error getting activity feed: {e}",
            exc_info=True
        )
        raise HTTPException(
            status_code=500,
...
async def get_discovered_patterns(
    ...
    limit: int = Query(default=100, ge=1, le=500, description="Maximum patterns to return"),
    db: AsyncSession = Depends(get_db),
    context: RequestContext = Depends(get_request_context)
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        logger.error(
            f"Error getting discovered patterns: {e}",
            exc_info=True
        )
        raise HTTPException(
            status_code=500,
            detail=f"Failed to get discovered patterns: {str(e)}"
        )
...
async def get_all_agents_summary(
    ...
    try:
        ...
        logger.error(
            f"Error getting all agents summary: {e}",
            exc_info=True
        )
        raise HTTPException(
            status_code=500,
            detail=f"Failed to get agents summary: {str(e)}"
        )


@router.post("/agents/performance/aggregate")
async def trigger_performance_aggregation(
    target_date: Optional[str] = Query(None, description="Target date (YYYY-MM-DD) to aggregate, defaults to yesterday"),
    db: AsyncSession = Depends(get_db),
    context: RequestContext = Depends(get_request_context)
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        logger.error(
            f"Error triggering aggregation: {e}",
            exc_info=True
        )
        raise HTTPException(
            status_code=500,
