...
async def stream_assessment_events(
    ...
    async def event_generator() -> AsyncGenerator[str, None]:
        ...
        try:
            ...
            logger.info(f"SSE connection established: {connection_id} for flow {flow_id}")
            ...
            while True:
                try:
                    ...
                    current_flow_state = await repository.get_assessment_flow_state(flow_id)
                    ...
                    heartbeat_event = AssessmentFlowEvent(
                        ...
                        timestamp=datetime.utcnow(),
                        data={"status": current_flow_state.status.value, "progress": current_flow_state.progress}
                    )
        ...
        try:
            ...
            while True:
                try:
                    ...
                    agent_events = await get_agent_progress_events(flow_id, client_account_id)
                    ...
                    logger.info(f"Agent progress SSE connection cancelled: {connection_id}")
                    break
                except Exception as e:
...
async def broadcast_agent_progress(
    ...
    # Store event for retrieval by SSE connections
    logger.info(f"Broadcasting agent progress for {agent_name} in flow {flow_id}: {progress_percentage}%")
...
def notify_phase_started(flow_id: str, phase: str):
    ...
    asyncio.create_task(broadcast_flow_event(
        flow_id=flow_id,
...
def notify_user_input_required(flow_id: str, phase: str, input_requirements: Dict[str, Any]):
...
def notify_agent_progress(
    ...
    asyncio.create_task(broadcast_agent_progress(
        flow_id=flow_id,
        agent_name=agent_name,
        task_name=task_name,
