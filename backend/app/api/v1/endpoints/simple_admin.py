...
async def create_simple_client(
    ...
    try:
        ...
        logger.info(f"Simple client account created: {client_data.name} by user {current_user.id}")
...
async def create_simple_engagement(
    ...
    try:
        ...
        logger.info(f"Simple engagement created: {engagement_data.name} for client {client.name} by user {current_user.id}")
...
async def list_simple_engagements(
    ...
    try:
        query = select(Engagement, ClientAccount).join(
            ClientAccount, Engagement.client_account_id == ClientAccount.id
