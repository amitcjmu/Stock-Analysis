...
class AnalysisEndpointsHandler:
    ...
    async def create_analysis(
        ...
        try:
            ...
            analysis = self.SixRAnalysis(
                name=request.get('analysis_name') or f"6R Analysis {datetime.utcnow().strftime('%Y%m%d_%H%M%S')}",
            ...
            if request.get('application_types') and len(request.get('application_ids', [])) == 1:
            ...
            background_tasks.add_task(
                bg_handler.run_initial_analysis,
                analysis.id,
                initial_params,
                "system"
            )
    ...
    async def list_analyses(
        ...
        try:
            ...
            query = query.offset(skip).limit(limit).order_by(self.SixRAnalysis.created_at.desc())
    ...
    def _fallback_create_analysis(self, request: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "description": request.get('description', 'Analysis created in fallback mode'),
            "status": "pending",
            "priority": request.get('priority', 'medium'),
            "application_ids": request.get('application_ids', []),
            "current_iteration": 1,
