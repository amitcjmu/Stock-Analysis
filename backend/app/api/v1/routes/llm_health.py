...
async def get_llm_health():
    ...
    try:
        ...
        return {
            ...
            "message": "All LLMs configured and ready" if all_healthy else "Some LLMs may have issues"
        }
    ...
    try:
        ...
        for llm_type, llm_instance in llm_instances.items():
            test_results[llm_type] = {
                ...
                "status": "ready" if connection_results.get(llm_type, False) else "error"
            }
        
        overall_status = "all_ready" if all(connection_results.values()) else "some_errors"
...
async def get_llm_configuration():
    ...
    try:
        ...
        config_info = {
            "models": {
                "crewai": getattr(settings, 'CREWAI_LLM_MODEL', 'meta-llama/Llama-4-Maverick-17B-128E-Instruct-FP8'),
                "embedding": getattr(settings, 'EMBEDDING_LLM_MODEL', 'thenlper/gte-large'),
            ...
            "environment_variables": {
                "OPENAI_API_KEY": "configured" if llm_config.deepinfra_api_key else "missing",
                "OPENAI_API_BASE": llm_config.deepinfra_base_url,
                "DEEPINFRA_API_KEY": "configured" if llm_config.deepinfra_api_key else "missing"
            },
            "crewai_settings": {
