...
class AssetAnalysisHandler:
    ...
    async def get_data_issues(self) -> Dict[str, Any]:
        ...
        try:
            ...
            ai_insights = self._generate_ai_insights(missing_data_count, format_issues_count, duplicates_count)
            ...
            metrics = {
                ...
                "completion_percentage": max(0, 100 - (total_issues * 10))  # Rough estimate
            }
    ...
    def _analyze_format_issues(self, df: pd.DataFrame, format_issues: List[Dict]) -> int:
    ...
    def _analyze_missing_data(self, df: pd.DataFrame, missing_data_issues: List[Dict]) -> int:
            ...
            for field in critical_fields:
                if field in df.columns:
                    ...
                    for _, asset in missing_assets.iterrows():
                        asset_name = asset.get('hostname') or asset.get('asset_name') or f"Asset-{asset.get('id', 'unknown')}"
                        suggested_value = self._suggest_bulk_value_for_field(field, asset, df)
    ...
    def _analyze_duplicates(self, df: pd.DataFrame, duplicate_issues: List[Dict]) -> int:
    ...
    def _generate_ai_insights(self, missing_data_count: int, format_issues_count: int, duplicates_count: int) -> List[Dict]:
    ...
    def _suggest_bulk_value_for_field(self, field: str, asset: pd.Series, df: pd.DataFrame) -> str:
