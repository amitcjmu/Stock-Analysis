...
class AssetProcessingHandler:
        ...
        try:
            ...
            for asset in all_assets:
                try:
                    ...
                    logger.warning(f"Error reprocessing asset {asset.get('id', 'unknown')}: {e}")
                    continue
    ...
    def _enhance_asset_data(self, asset: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if 'environment' in asset:
                asset['environment'] = self._standardize_environment(asset['environment'])
            ...
            asset['compliance_requirements'] = self._assess_compliance_requirements(asset)
            
            # Assess technical debt
            asset['technical_debt_level'] = self._assess_technical_debt(asset)
            
            # Assess modernization potential
            asset['modernization_potential'] = self._assess_modernization_potential(asset)
    ...
    def _transform_asset_for_analysis(self, asset: Dict[str, Any]) -> Dict[str, Any]:
        ...
        transformed = {
            "id": asset.get('id') or asset.get('ci_id') or asset.get('asset_id', 'unknown'),
            "name": asset.get('asset_name') or asset.get('hostname', 'Unknown Application'),
            "type": "Application",
            "environment": asset.get('environment', 'Unknown'),
            "department": asset.get('department') or asset.get('business_owner', 'Unknown'),
            "criticality": self._map_criticality(asset.get('status') or asset.get('criticality', 'Medium')),
            "technology_stack": asset.get('technology_stack') or self._build_tech_stack_from_asset(asset),
            "complexity_score": asset.get('complexity_score') or self._calculate_complexity_score(asset),
            "business_value_score": asset.get('business_value_score') or self._get_business_value_score(asset),
            "data_sensitivity": asset.get('data_sensitivity') or self._assess_data_sensitivity(asset),
            "compliance_requirements": asset.get('compliance_requirements') or self._assess_compliance_requirements(asset),
            "technical_debt_level": asset.get('technical_debt_level') or self._assess_technical_debt(asset),
            "modernization_potential": asset.get('modernization_potential') or self._assess_modernization_potential(asset),
            "cloud_readiness": asset.get('cloud_readiness') or self._assess_cloud_readiness(asset),
    ...
    def _generate_applications_summary(self, applications: List[Dict]) -> Dict[str, Any]:
        """
        ...
        summary = {
            ...
            "technology_distribution": self._extract_technology_distribution(applications)
        }
    ...
    def _standardize_asset_type(self, asset_type: str) -> str:
        ...
        # Application types
        if any(term in asset_type_lower for term in ['app', 'application', 'service', 'web']):
            return "Application"
        
        # Server types  
        if any(term in asset_type_lower for term in ['server', 'host', 'vm', 'virtual']):
            return "Server"
        
        # Database types
        if any(term in asset_type_lower for term in ['database', 'db', 'sql', 'mongo', 'oracle']):
            return "Database"
        
        # Network types
        if any(term in asset_type_lower for term in ['network', 'switch', 'router', 'firewall']):
    ...
    def _standardize_os_family(self, os_name: str) -> str:
        ...
        if any(term in os_lower for term in ['windows', 'win']):
            return "Windows"
        elif any(term in os_lower for term in ['linux', 'ubuntu', 'redhat', 'centos', 'debian']):
            return "Linux"
    ...
    def _assess_data_sensitivity(self, asset: Dict[str, Any]) -> str:
        ...
        if any(term in asset_name + tech_stack for term in ['payment', 'finance', 'bank', 'pii', 'customer']):
            return "High"
        
        # Medium sensitivity indicators
        if any(term in asset_name + tech_stack for term in ['user', 'account', 'profile', 'auth']):
            return "Medium"
    ...
    def _assess_compliance_requirements(self, asset: Dict[str, Any]) -> List[str]:
        ...
        if any(term in asset_name + tech_stack for term in ['health', 'medical', 'patient']):
    ...
    def _assess_cloud_readiness(self, asset: Dict[str, Any]) -> str:
        ...
        if any(term in tech_stack for term in ['docker', 'kubernetes', 'microservice', 'api']):
            return "High"
    ...
    def _extract_technology_distribution(self, applications: List[Dict]) -> Dict[str, int]:
