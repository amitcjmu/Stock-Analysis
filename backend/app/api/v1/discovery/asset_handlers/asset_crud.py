...
class AssetCRUDHandler:
    ...
    async def get_assets_paginated(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # Apply filters
            filtered_assets = self._apply_filters(all_assets, {
            ...
            total_pages = math.ceil(total_filtered / page_size) if total_filtered > 0 else 0
            start_idx = (page - 1) * page_size
            end_idx = start_idx + page_size
            paginated_assets = filtered_assets[start_idx:end_idx]
            
            # Transform assets for frontend
            transformed_assets = [self._transform_asset_for_frontend(asset) for asset in paginated_assets]
    ...
    def _apply_filters(self, assets: List[Dict], filters: Dict[str, Any]) -> List[Dict]:
        ...
        for asset in assets:
            # Asset type filter
            if filters.get('asset_type') and asset.get('asset_type', '').lower() != filters['asset_type'].lower():
                continue
            
            # Environment filter
            if filters.get('environment') and asset.get('environment', '').lower() != filters['environment'].lower():
                continue
            ...
            if filters.get('search'):
                ...
                search_fields = ['hostname', 'asset_name', 'ip_address', 'operating_system', 'asset_type']
                search_match = False
    ...
    def _map_frontend_to_backend_fields(self, updates: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _transform_asset_for_frontend(self, asset: Dict[str, Any]) -> Dict[str, Any]:
        ...
        transformed['id'] = asset.get('id') or asset.get('ci_id') or asset.get('asset_id', 'unknown')
        transformed['name'] = asset.get('asset_name') or asset.get('hostname', 'Unknown Asset')
    ...
    def _fallback_update_asset(self, asset_id: str, asset_data: Dict[str, Any]) -> Dict[str, Any]:
