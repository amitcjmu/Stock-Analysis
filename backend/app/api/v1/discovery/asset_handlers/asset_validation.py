...
class AssetValidationHandler:
    ...
    async def reject_data_issue(self, issue_id: str, rejection_reason: str = None) -> Dict[str, Any]:
        """
        ...
        try:
            if field_name == "ip_address":
                if not self.is_valid_ip(str(value)):
                    ...
                    validation_result["suggested_value"] = self.suggest_ip_format(str(value), "")
                    validation_result["error_message"] = "Invalid IP address format"
            
            elif field_name == "cpu_cores":
                if not self._is_valid_numeric(value):
                    validation_result["is_valid"] = False
                    validation_result["suggested_value"] = self._extract_numeric_from_text(str(value)) or "4"
                    validation_result["error_message"] = "CPU cores should be numeric"
            
            elif field_name == "memory_gb":
                if not self._is_valid_numeric(value):
                    validation_result["is_valid"] = False
                    validation_result["suggested_value"] = self._clean_memory_value(str(value))
                    validation_result["error_message"] = "Memory should be numeric (GB only)"
            
            elif field_name == "hostname":
                if not self._is_valid_hostname(str(value)):
                    validation_result["is_valid"] = False
                    validation_result["suggested_value"] = self._clean_hostname(str(value))
    ...
    def _extract_numeric_from_text(self, text: str) -> Optional[str]:
        ...
        try:
            ...
            text_numbers = {
                ...
                'sixteen': '16', 'thirty-two': '32', 'sixty-four': '64'
            }
