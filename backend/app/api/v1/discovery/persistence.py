...
def add_processed_asset(asset: Dict[str, Any]) -> None:
    ...
    for existing_asset in processed_assets_store:
        ...
        if asset_hostname and existing_hostname and asset_hostname == existing_hostname:
            is_duplicate = True
        elif asset_name and existing_name and asset_name == existing_name and asset_type == existing_type:
            is_duplicate = True
...
def bulk_update_assets(asset_ids: List[str], updates: Dict[str, Any]) -> int:
    ...
    for asset in processed_assets_store:
        ...
        if asset_id and str(asset_id) in asset_ids:
            ...
            updated_count += 1
            logger.info(f"Updated asset {asset_id}: {asset.get('hostname', 'Unknown')} with {backend_updates}")
    
    logger.info(f"Found {len(found_asset_ids)} assets out of {len(asset_ids)} requested")
...
def bulk_delete_assets(asset_ids: List[str]) -> int:
    ...
    def should_keep_asset(asset):
        ...
        asset_id = asset.get('id') or asset.get('ci_id') or asset.get('asset_id')
        if asset_id and str(asset_id) in asset_ids:
            logger.debug(f"Marking for deletion: {asset_id} - {asset.get('hostname', 'Unknown')}")
            return False
        return True
    
    processed_assets_store = [asset for asset in processed_assets_store if should_keep_asset(asset)]
    ...
    if deleted_count > 0:
        ...
        logger.warning(f"No assets were deleted. Checked {len(asset_ids)} IDs against {initial_count} assets")
        # Log some sample IDs for debugging
        sample_assets = processed_assets_store[:3]
        for asset in sample_assets:
            asset_id = asset.get('id') or asset.get('ci_id') or asset.get('asset_id')
            logger.debug(f"Sample asset ID: {asset_id}, hostname: {asset.get('hostname', 'Unknown')}")
...
def find_duplicate_assets() -> List[Dict[str, Any]]:
    ...
    for name_type_key, asset_list in name_type_groups.items():
        if len(asset_list) > 1:
            ...
            filtered_list = [(i, asset) for i, asset in asset_list if i not in hostname_indices]
...
def cleanup_duplicates() -> int:
    ...
    for idx in sorted_indices:
        if idx < len(processed_assets_store):
            ...
            logger.debug(f"Removed duplicate asset at index {idx}: {removed_asset.get('hostname', 'Unknown')}")
    
    removed_count = initial_count - len(processed_assets_store)
    
    if removed_count > 0:
        backup_processed_assets()
        logger.info(f"Cleaned up {removed_count} duplicate assets out of {initial_count} total assets")
