...
async def get_security_summary(
    ...
    try:
        ...
        admin_accesses = len([e for e in all_events if e.get("event_type") == "ADMIN_ACCESS"])
        role_changes = len([e for e in all_events if e.get("event_type") == "ROLE_CHANGE"])
        
        # Get recent critical events
        recent_critical = [e for e in all_events if e.get("severity") in ["CRITICAL", "WARNING"]][:10]
...
async def get_user_security_activity(
    ...
    if not AUDIT_AVAILABLE:
        return {"events": [], "summary": {"total": 0, "admin_accesses": 0, "role_changes": 0}}
    ...
    try:
        ...
        admin_accesses = len([e for e in events if e.get("event_type") == "ADMIN_ACCESS"])
