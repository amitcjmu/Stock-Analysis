...
class ModelPricingCreate(BaseModel):
    ...
    input_cost_per_1k_tokens: float = Field(
        ..., gt=0, description="Cost per 1K input tokens"
    )
    output_cost_per_1k_tokens: float = Field(
        ..., gt=0, description="Cost per 1K output tokens"
    )
...
async def get_usage_report(
    client_account_id: Optional[int] = Query(
        None, description="Filter by client account"
    ),
    ...
    feature_context: Optional[str] = Query(
        None, description="Filter by feature context"
    ),
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to generate usage report: {str(e)}"
        )
...
async def get_model_pricing(
    ...
    try:
            ...
            pricing_data.append(
                {
                    ...
                    "output_cost_per_1k_tokens": float(
                        record.output_cost_per_1k_tokens
                    ),
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get model pricing: {str(e)}"
        )


@router.post("/pricing/models")
async def create_model_pricing(
    pricing: ModelPricingCreate, db: AsyncSession = Depends(get_db)
):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to create model pricing: {str(e)}"
        )
...
async def get_real_time_usage(
    ...
    try:
        ...
        query = select(
            ...
            func.coalesce(func.avg(LLMUsageLog.response_time_ms), 0).label(
                "avg_response_time"
            ),
        ...
        if client_account_id:
            hourly_query = hourly_query.where(
                LLMUsageLog.client_account_id == client_account_id
            )

        hourly_query = hourly_query.group_by(
            func.date_trunc("hour", LLMUsageLog.created_at)
        )
        hourly_query = hourly_query.order_by(
            func.date_trunc("hour", LLMUsageLog.created_at)
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get real-time usage: {str(e)}"
        )
...
async def initialize_pricing(db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        return {"success": True, "message": "Default pricing initialized successfully"}

    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"Failed to initialize pricing: {str(e)}"
        )
