...
async def get_flows_for_comparison(
    ...
    try:
                ...
                comparison_flows.append(
                    {
                        ...
                        "quality_score": flow.get("metadata", {}).get(
                            "quality_score", 0
                        ),
                        "estimated_cost_savings": flow.get("metadata", {}).get(
                            "estimated_cost_savings", 0
                        ),
                        "processing_time_hours": flow.get("metadata", {}).get(
        ...
        return {"success": True, "data": {"flows": comparison_flows}}
...
async def perform_flow_comparison(
    ...
    try:
        ...
        if not source_flow_id or not target_flow_id:
            raise HTTPException(
                status_code=400, detail="Both source and target flow IDs are required"
            )
        ...
        comparison_result = {
            ...
            "key_metrics_diff": {
                "quality_score": {
                    "source_value": source_flow.get("metadata", {}).get(
                        "quality_score", 0
                    ),
                    "target_value": target_flow.get("metadata", {}).get(
                        "quality_score", 0
                    ),
                    "difference": 6.7,
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to perform comparison: {str(e)}"
        )
...
async def get_comparison_history(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get comparison history: {str(e)}"
        )
...
async def get_detailed_comparison(
    ...
    try:
        ...
        return {"success": True, "data": detailed_comparison}
