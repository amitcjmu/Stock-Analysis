...
async def seed_llm_usage(session, force: bool):
    if force:
        ...
        await session.execute(
            text("DELETE FROM llm_usage_summary WHERE is_mock = TRUE")
        )
        await session.execute(
            text("DELETE FROM llm_model_pricing WHERE is_mock = TRUE")
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Seed LLM model pricing and usage data."
    )
