    ...
    async with AsyncSessionLocal() as session:
        try:
            ...
            if not clients_to_update:
                print(
                    "✅ All client accounts already have proper industry and company_size values"
                )
                return

            print(
                f"🔧 Found {len(clients_to_update)} client accounts that need industry/company_size updates"
            )
            ...
            for client in clients_to_update:
                ...
                await session.execute(
                    ...
                    .values(industry=industry, company_size=company_size)
                )

                print(
                    f"  ✅ Updated '{client.name}': industry='{industry}', company_size='{company_size}'"
                )
