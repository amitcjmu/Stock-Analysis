...
DATABASE_URL = os.getenv("DATABASE_URL", "postgresql+asyncpg://postgres:postgres@postgres:5432/migration_db")
...
async def fix_null_asset_ids():
    ...
    async with AsyncSessionLocal() as session:
        try:
            ...
            print(f"Found {len(assets_to_fix)} assets with NULL IDs. Proceeding to fix them...")
            ...
            rows_to_update_query = text("SELECT hostname, created_at FROM assets WHERE id IS NULL")
