...
async def seed_migration_workflow(session, force: bool):
    if force:
        ...
        await session.execute(text("DELETE FROM workflow_progress WHERE is_mock = TRUE"))
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Seed migration logs & workflow progress.")
