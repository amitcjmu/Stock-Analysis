...
class DatabaseAuthBackend(AuthenticationBackend):
    ...
    async def authenticate(
        ...
        **kwargs,
        ...
        if not password:
            logger.warning(
                f"Authentication failed for {username}: no password provided"
            )
            return None
        ...
        if not user:
            ...
            if demo_password and self._hash_password(password) == self._hash_password(
                demo_password
            ):
                user = {
                    "id": f"demo_{username}",
    ...
    async def refresh_session(self, session_token: str) -> Optional[str]:
        ...
        new_token = await self.create_session(
            session["user_id"], session.get("metadata")
        )
...
class SSOAuthBackend(AuthenticationBackend):
    ...
    def __init__(
        self, provider: str = "oauth2", config: Optional[Dict[str, Any]] = None
    ):
        """
        ...
        self.redirect_uri = self.config.get("redirect_uri") or os.getenv(
            "SSO_REDIRECT_URI"
        )
        self.authorize_url = self.config.get("authorize_url") or os.getenv(
            "SSO_AUTHORIZE_URL"
        )
