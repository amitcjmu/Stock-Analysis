...
async def run_deployment_mode_test(mode: str) -> Dict[str, Any]:
    ...
    await telemetry.record_metric(
        "test_metric", 42.0, MetricType.GAUGE, tags={"mode": mode}
    )
    print("✅ Recorded metric")

    # Record event
    await telemetry.record_event("test_event", properties={"mode": mode, "test": True})
...
async def main():
    ...
    for result in results:
        ...
        print(
            f"  - Features enabled: {sum(1 for v in result['features'].values() if v)}"
        )
        print(
            f"  - Services healthy: {sum(1 for v in result['health'].values() if v)}/{len(result['health'])}"
        )
