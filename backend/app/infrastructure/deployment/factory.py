...
class ServiceFactory:
    ...
    def __init__(self, deployment_config: DeploymentConfig):
        ...
        logger.info(
            f"Initialized ServiceFactory for deployment mode: {deployment_config.mode.value}"
        )
    ...
    async def _create_service(
        self, service_name: str, service_config: ServiceConfig, **kwargs
    ) -> Any:
        """
        ...
        if not service_config.enabled:
            logger.info(
                f"Service {service_name} is disabled, using no-op implementation"
            )
        ...
        try:
            ...
            if service_class:
                ...
                if await self._check_service_availability(service_name, implementation):
                    logger.info(
                        f"Creating {implementation} implementation for {service_name}"
                    )
                    return service_class(**service_config.config, **kwargs)
        except Exception as e:
            logger.error(
                f"Failed to create {service_config.implementation} for {service_name}: {e}"
            )

        # Try fallback implementation
        if service_config.fallback:
            try:
                fallback_class = self._get_service_class(
                    service_name, service_config.fallback
                )
                if fallback_class:
                    logger.warning(
                        f"Using fallback {service_config.fallback} for {service_name}"
                    )
                    return fallback_class(**kwargs)
            except Exception as e:
                logger.error(
                    f"Failed to create fallback {service_config.fallback} for {service_name}: {e}"
                )
    ...
    def _get_service_class(
        self, service_name: str, implementation: str
    ) -> Optional[Type]:
        """
