...
class NoOpNotificationService:
    ...
    async def send_push(
        ...
        self._notifications.append(notification)
        logger.info(
            f"[NoOp] Push notification logged: user_id={user_id}, title={title}"
        )
...
class NoOpQueueService:
    ...
    async def _process_message(self, queue_name: str, message: Dict[str, Any]) -> None:
...
class NoOpCacheService:
    ...
    async def set(
        ...
        """
        item = {"value": value, "timestamp": datetime.utcnow().isoformat()}
...
class NoOpSearchService:
    ...
    async def index_document(
        ...
        self._documents[index_name] = [
            doc for doc in self._documents[index_name] if doc.get("_id") != document_id
        ]
