...
class FlowConfiguration:
    ...
    def validate(self) -> List[str]:
        ...
        for phase_config in self.phases:
            ...
            errors.extend(
                [f"Phase {phase_config.phase.value}: {error}" for error in phase_errors]
            )
        ...
        for phase_config in self.phases:
            for dep in phase_config.dependencies:
                if dep not in phase_set:
                    errors.append(
                        f"Phase {phase_config.phase.value} depends on non-existent phase {dep.value}"
                    )
    ...
    FlowType.PLANNING: FlowConfiguration(
        ...
        phases=[
            ...
            PhaseConfiguration(
                ...
                agent_config={
                    "type": "planning_agent",
                    "strategy_types": ["rehost", "replatform", "refactor"],
                },
...
def get_phase_configuration(
    flow_type: FlowType, phase: FlowPhase
) -> Optional[PhaseConfiguration]:
...
def merge_flow_configurations(
    base_config: FlowConfiguration, override_config: Dict[str, Any]
) -> FlowConfiguration:
    ...
    merged_config = FlowConfiguration(
        ...
        priority=FlowPriority(
            override_config.get("priority", base_config.priority.value)
        ),
        phases=base_config.phases.copy(),
        global_timeout_seconds=override_config.get(
            "global_timeout_seconds", base_config.global_timeout_seconds
        ),
        max_concurrent_phases=override_config.get(
            "max_concurrent_phases", base_config.max_concurrent_phases
        ),
        error_handling_strategy=override_config.get(
            "error_handling_strategy", base_config.error_handling_strategy
        ),
        notification_settings={
            **base_config.notification_settings,
            **override_config.get("notification_settings", {}),
        },
        resource_limits={
            **base_config.resource_limits,
            **override_config.get("resource_limits", {}),
        },
...
def create_custom_flow_configuration(
    flow_type: FlowType, name: str, phases: List[Dict[str, Any]], **kwargs
) -> FlowConfiguration:
