...
def get_performance_threshold(flow_type: str, phase: str) -> Dict[str, Any]:
    ...
    return flow_thresholds.get(phase, {
        "max_execution_time": 300,
        "max_memory_usage": 500,
        "max_cpu_usage": 70,
        "max_error_rate": 5
...
def check_performance_threshold(metrics: FlowPerformanceMetrics, flow_type: str, phase: str) -> bool:
...
def calculate_backoff_delay(retry_count: int, base_delay: int, max_delay: int, 
                          backoff_factor: int, exponential: bool = True) -> int:
...
def get_performance_recommendations(metrics: FlowPerformanceMetrics) -> Dict[str, str]:
    ...
    if metrics.execution_time_seconds > 300:
        recommendations["execution_time"] = "Consider optimizing algorithms or adding parallel processing"
    
    if metrics.memory_usage_mb > 1000:
        recommendations["memory"] = "Consider implementing memory optimization or data streaming"
    
    if metrics.cpu_usage_percent > 80:
        recommendations["cpu"] = "Consider load balancing or reducing computational complexity"
    
    if metrics.error_rate_percent > 5:
        recommendations["error_rate"] = "Review error handling and implement better validation"
    
    if metrics.database_queries > 100:
        recommendations["database"] = "Consider query optimization or caching strategies"
