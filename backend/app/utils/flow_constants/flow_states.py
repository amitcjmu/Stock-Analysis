...
VALID_TRANSITIONS: Dict[FlowStatus, Set[FlowStatus]] = {
    ...
    FlowStatus.INITIALIZING: {FlowStatus.RUNNING, FlowStatus.FAILED, FlowStatus.CANCELLED},
    FlowStatus.RUNNING: {FlowStatus.PAUSED, FlowStatus.WAITING, FlowStatus.COMPLETED, FlowStatus.FAILED, FlowStatus.TIMEOUT, FlowStatus.CANCELLED},
...
def get_previous_phase(flow_type: FlowType, current_phase: FlowPhase) -> Optional[FlowPhase]:
...
def calculate_progress_percentage(flow_type: FlowType, current_phase: FlowPhase, 
                                phase_progress: float = 0.0) -> float:
