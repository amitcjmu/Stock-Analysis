...
class URLValidator(BaseValidator):
    ...
    def __init__(self, require_https: bool = False, error_message: Optional[str] = None):
...
class DateValidator(BaseValidator):
    ...
    def __init__(self, date_format: str = "%Y-%m-%d", 
                 ...
                 error_message: Optional[str] = None):
        super().__init__(error_message or f"Invalid date format (expected: {date_format})")
    ...
    def validate(self, value: Any) -> ValidationResult:
        ...
        try:
            if isinstance(value, str):
                ...
                numeric_value = value
                if not self.allow_decimal and isinstance(value, float) and value != int(value):
                    return ValidationResult(
                        is_valid=False,
...
class PhoneNumberValidator(BaseValidator):
    ...
    def __init__(self, require_country_code: bool = False,
                 error_message: Optional[str] = None):
...
def validate_numeric(value: Any, min_value: Optional[Union[int, float]] = None,
...
def validate_file(file_path: Any, allowed_extensions: Optional[List[str]] = None,
                 max_size_mb: Optional[float] = None) -> ValidationResult:
    """Validate file."""
    return FileValidator(allowed_extensions=allowed_extensions, max_size_mb=max_size_mb).validate(file_path)
...
def validate_phone_number(phone: str, require_country_code: bool = False) -> ValidationResult:
    """Validate phone number."""
    return PhoneNumberValidator(require_country_code=require_country_code).validate(phone)
