...
class TenantContext:
    ...
    def is_valid(self) -> bool:
        ...
        return bool(
            self.client_account_id or self.engagement_id or self.is_platform_admin
        )
...
class MultiTenantHelper:
    ...
    def apply_tenant_filter(
        query: Select, model_class: Type, tenant_context: TenantContext
    ) -> Select:
        """
        ...
        if tenant_context.client_account_id and hasattr(
            model_class, "client_account_id"
        ):
            filters.append(
                model_class.client_account_id == tenant_context.client_account_id
            )
            logger.debug(
                f"Applied client_account_id filter: {tenant_context.client_account_id}"
            )

        # Apply engagement filter
        if tenant_context.engagement_id and hasattr(model_class, "engagement_id"):
            filters.append(model_class.engagement_id == tenant_context.engagement_id)
            logger.debug(
                f"Applied engagement_id filter: {tenant_context.engagement_id}"
            )

        # Handle demo/mock data access
        if (
            hasattr(model_class, "is_mock")
            and not tenant_context.client_account_id
            and not tenant_context.engagement_id
        ):
    ...
    def validate_tenant_access(
        ...
        if (
            required_client_id
            and tenant_context.client_account_id != required_client_id
        ):
            logger.warning(
                f"Client access denied: required={required_client_id}, "
        ...
        if (
            required_engagement_id
            and tenant_context.engagement_id != required_engagement_id
        ):
            logger.warning(
                f"Engagement access denied: required={required_engagement_id}, "
    ...
    def set_tenant_fields(
        data: Dict[str, Any], tenant_context: TenantContext
    ) -> Dict[str, Any]:
        """
    ...
    @staticmethod
    async def check_admin_status(session: AsyncSession, user_id: str) -> bool:
        """
...
class TenantAwareMixin:
    ...
    def apply_tenant_filter(self, query: Select, model_class: Type) -> Select:
        ...
        return MultiTenantHelper.apply_tenant_filter(
            query, model_class, self.tenant_context
        )
