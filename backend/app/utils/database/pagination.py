...
class PaginationResult(Generic[T]):
    ...
    @classmethod
    def create(
        cls, data: List[T], total_items: int, page: int, page_size: int
    ) -> "PaginationResult[T]":
...
class PaginationHelper:
    ...
    async def paginate_query(
        session: AsyncSession, query: Select, page: int = 1, page_size: int = 50
        ...
        logger.debug(
            f"Paginated query: page={page}, page_size={page_size}, total_items={total_items}"
        )
...
def create_pagination_metadata(
    total_items: int, page: int, page_size: int
) -> Dict[str, Any]:
...
def calculate_pagination_range(
    current_page: int, total_pages: int, max_visible: int = 10
) -> List[int]:
