...
class QueryFilter:
    ...
    def apply(self, query: Select, model_class: Type) -> Select:
        ...
        if not hasattr(model_class, self.field):
            logger.warning(f"Model {model_class.__name__} does not have field {self.field}")
            return query
...
class QueryBuilder:
    ...
    def filter(self, field: str, operator: FilterOperator, value: Any = None) -> 'QueryBuilder':
    ...
    def with_tenant_context(self, client_account_id: Optional[str] = None, 
                          engagement_id: Optional[str] = None) -> 'QueryBuilder':
    ...
    def _apply_tenant_filters(self, query: Select) -> Select:
        ...
        if self._multi_tenant_context.get("client_account_id") and hasattr(self.model_class, "client_account_id"):
            filters.append(self.model_class.client_account_id == self._multi_tenant_context["client_account_id"])
        
        if self._multi_tenant_context.get("engagement_id") and hasattr(self.model_class, "engagement_id"):
            filters.append(self.model_class.engagement_id == self._multi_tenant_context["engagement_id"])
...
def build_context_query(model_class: Type, client_account_id: Optional[str] = None, 
                       engagement_id: Optional[str] = None) -> QueryBuilder:
    """Build query with multi-tenant context."""
    return QueryBuilder(model_class).with_tenant_context(client_account_id, engagement_id)

def build_paginated_query(model_class: Type, page: int = 1, page_size: int = 50) -> QueryBuilder:
...
def apply_multi_tenant_filter(query: Select, model_class: Type, 
                            client_account_id: Optional[str] = None,
                            engagement_id: Optional[str] = None) -> Select:
