...
class DatabaseTransaction:
    ...
    def __init__(self, session: AsyncSession, config: Optional[TransactionConfig] = None):
        ...
        try:
            if exc_type is not None:
                ...
                if self.config.rollback_on_error:
                    ...
                    if self.config.log_transactions:
                        logger.error(f"Transaction rolled back due to error: {self.error_message} "
                                   f"(duration: {duration:.2f}s)")
                else:
                    if self.config.log_transactions:
                        logger.warning(f"Transaction completed with error but not rolled back: {self.error_message}")
            else:
                # No exception, commit if configured
                if self.config.auto_commit:
                    await self.session.commit()
                    if self.config.log_transactions:
                        logger.debug(f"Transaction committed successfully (duration: {duration:.2f}s)")
                else:
                    if self.config.log_transactions:
                        logger.debug(f"Transaction completed without auto-commit (duration: {duration:.2f}s)")
...
async def rollback_on_error(session: AsyncSession, operation: Callable[[AsyncSession], T]) -> T:
    """
...
async def commit_or_rollback(session: AsyncSession, operation: Callable[[AsyncSession], T]) -> T:
    """
    ...
    for i in range(0, len(operations), batch_size):
        ...
        try:
            async with transaction_scope(session) as tx:
                ...
                logger.debug(f"Batch {i//batch_size + 1} completed: {len(batch)} operations")
