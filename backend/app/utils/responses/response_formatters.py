...
class ResponseFormatter:
    ...
    @staticmethod
    def format_datetime(
        dt: Union[datetime, date, str], format_str: str = "%Y-%m-%d %H:%M:%S"
    ) -> Optional[str]:
        """
    ...
    @staticmethod
    def format_currency(
        amount: Union[float, Decimal, int], currency: str = "USD", locale: str = "en_US"
    ) -> str:
        """
    ...
    @staticmethod
    def format_number(
        value: Union[int, float, Decimal], decimals: Optional[int] = None
    ) -> str:
        """
...
def sanitize_response_data(data: Any, max_string_length: int = 10000) -> Any:
    ...
    if isinstance(data, str):
        ...
        return {
            key: sanitize_response_data(value, max_string_length)
            for key, value in data.items()
        }

    elif isinstance(data, list):
        return [sanitize_response_data(item, max_string_length) for item in data]
