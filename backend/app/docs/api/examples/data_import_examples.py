...
class DataImportClient:
    ...
    def import_data_direct(
        self, data: List[Dict[str, Any]], filename: str, import_type: str
    ) -> Dict[str, Any]:
        """
    ...
    def wait_for_import_completion(
        self, import_id: str, timeout: int = 300
    ) -> Dict[str, Any]:
        """
...
def main():
    ...
    try:
        result = client.import_data_direct(
            data=server_data, filename="servers_inventory.csv", import_type="servers"
        )
        ...
        print(
            f"Import completed with status: {final_status['import_status']['status']}"
        )
        ...
        if e.response.status_code == 409:
            ...
            print(
                f"Existing flow: {error_detail.get('existing_flow', {}).get('flow_id')}"
            )
...
PaymentGateway,2.1.0,prod-payment-01,Java Spring,Critical"""
    ...
    try:
        result = client.import_csv_file(
            file_path="applications.csv", import_type="applications"
        )
    ...
    try:
        ...
        if latest["success"] and latest["import_metadata"]:
            ...
            print(
                f"Current Phase: {status['import_status'].get('current_phase', 'Unknown')}"
            )
    except Exception as e:
    ...
    try:
        result = client.import_data_direct(
            data=invalid_data, filename="invalid_servers.csv", import_type="servers"
        )
    except requests.exceptions.HTTPError as e:
