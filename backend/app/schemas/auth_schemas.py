...
class UserRegistrationRequest(BaseModel):
    ...
    username: Optional[str] = Field(
        None, min_length=2, max_length=50, description="Optional username for the user"
    )
    ...
    role_name: Optional[str] = Field(
        None, max_length=100, description="Role name for admin-created users"
    )
    is_active: Optional[bool] = Field(
        None, description="Whether user should be immediately active"
    )
    notes: Optional[str] = Field(
        None, max_length=1000, description="Admin notes about the user"
    )
    default_client_id: Optional[str] = Field(
        None, description="Default client account for the user"
    )
    default_engagement_id: Optional[str] = Field(
        None, description="Default engagement for the user"
    )
...
class AccessValidationRequest(BaseModel):
    ...
    resource_type: str = Field(
        ..., pattern=r"^(client|engagement|admin_console|data|session)$"
    )
