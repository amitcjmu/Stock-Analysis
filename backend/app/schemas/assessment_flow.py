...
class AssessmentFlowCreateRequest(BaseModel):
    ...
    selected_application_ids: List[str] = Field(..., min_items=1, max_items=100, description="Application IDs to include in assessment")
...
class NavigateToPhaseRequest(BaseModel):
    ...
    target_phase: AssessmentPhase = Field(..., description="Target phase to navigate to")
    force_navigation: bool = Field(default=False, description="Force navigation even if prerequisites not met")
...
class ArchitectureStandardCreate(BaseModel):
    ...
    standard_type: str = Field(..., description="Type of standard (e.g., cloud_provider, framework)")
    domain: Optional[str] = Field(None, description="Domain area (e.g., infrastructure, application)")
    standard_definition: Dict[str, Any] = Field(..., description="The actual standard definition")
    enforcement_level: str = Field(default="recommended", description="Enforcement level")
    is_template: bool = Field(default=False, description="Whether this is a reusable template")
    customizable_fields: List[str] = Field(default=[], description="Fields that can be customized")
...
class ApplicationOverrideCreate(BaseModel):
    ...
    requires_approval: bool = Field(default=False, description="Whether override requires approval")
...
class ArchitectureStandardsUpdateRequest(BaseModel):
    ...
    engagement_standards: Optional[List[ArchitectureStandardCreate]] = Field(None, description="Engagement-level standards")
    application_overrides: Optional[List[ApplicationOverrideCreate]] = Field(None, description="Application-specific overrides")
...
class ComponentUpdate(BaseModel):
    ...
    components: List[ComponentStructure] = Field(..., description="Updated component list")
    user_verified: bool = Field(default=False, description="Whether user has verified components")
    verification_comments: Optional[str] = Field(None, description="User verification comments")
...
class TechDebtItem(BaseModel):
    ...
    estimated_effort_hours: Optional[int] = Field(None, description="Estimated effort to resolve")
    impact_score: Optional[float] = Field(None, ge=0, le=100, description="Impact score")
    remediation_suggestions: List[str] = Field(default=[], description="Remediation suggestions")


class TechDebtAnalysis(BaseModel):
    """Schema for technical debt analysis."""
    overall_debt_score: float = Field(..., ge=0, le=100, description="Overall debt score")
    ...
    recommendations: List[str] = Field(default=[], description="High-level recommendations")
...
class TechDebtUpdates(BaseModel):
    ...
    accepted_recommendations: List[str] = Field(default=[], description="User-accepted recommendations")
...
class SixRDecision(BaseModel):
    ...
    component_id: Optional[str] = Field(None, description="Component ID (if component-level decision)")
    recommended_strategy: str = Field(..., description="Primary 6R recommendation")
    alternative_strategies: List[Dict[str, Any]] = Field(default=[], description="Alternative strategies with scores")
    ...
    dependencies: List[str] = Field(default=[], description="Dependencies affecting decision")
...
class SixRDecisionUpdate(BaseModel):
    ...
    user_overrides: Dict[str, Any] = Field(default={}, description="User overrides to AI recommendations")
...
class AppOnPageData(BaseModel):
    ...
    component_breakdown: List[ComponentStructure] = Field(..., description="Component breakdown")
    tech_debt_summary: TechDebtAnalysis = Field(..., description="Tech debt summary")
    migration_recommendations: List[SixRDecision] = Field(..., description="Migration recommendations")
    estimated_timeline: Dict[str, Any] = Field(..., description="Estimated migration timeline")
...
class AssessmentFinalization(BaseModel):
    ...
    apps_to_finalize: List[str] = Field(..., description="Application IDs to mark as ready for planning")
    finalization_notes: Optional[str] = Field(None, description="Notes about finalization")
    export_to_planning: bool = Field(default=True, description="Whether to export to Planning Flow")
