...
class DatabaseInitializer:
    ...
    async def ensure_demo_data(self):
        ...
        if not client:
            ...
            if existing_client:
                ...
                await self.db.execute(
                    text("DELETE FROM user_roles WHERE scope_client_id = :client_id"),
                    {"client_id": existing_client.id},
                )
                await self.db.execute(
                    text(
    ...
    async def ensure_primary_demo_user(
        ...
        profile_result = await self.db.execute(
            select(UserProfile).where(
                UserProfile.user_id == self.requirements.DEMO_USER_ID
            )
        )
        profile = profile_result.scalar_one_or_none()
        ...
        try:
            ...
            granted_by_id = (
                platform_admin.id if platform_admin else self.requirements.DEMO_USER_ID
            )
    ...
    async def ensure_demo_user(
        ...
        if not association:
            ...
            self.db.add(association)
            logger.info(
                f"Created user association for {user.email} with role {user_data['role'].value}"
            )
        else:
        ...
        try:
            ...
            platform_admin = admin_result.scalar_one_or_none()
            granted_by_id = (
                platform_admin.id if platform_admin else user_id
            )  # Use self if no admin found
            ...
            if not client_access:
                ...
                access_level = access_level_map.get(
                    user_data["role"].value.lower(), AccessLevel.READ_ONLY
                )
                ...
                client_access = ClientAccess(
                    ...
                    permissions={
                        ...
                        "can_configure_client_settings": access_level
                        == AccessLevel.ADMIN,
                ...
                self.db.add(client_access)
                logger.info(
                    f"Created ClientAccess for {user.email} with {access_level} access"
                )
            ...
            if not engagement_access:
                ...
                self.db.add(engagement_access)
                logger.info(
                    f"Created EngagementAccess for {user.email} to engagement {engagement_id}"
                )
