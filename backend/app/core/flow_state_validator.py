...
class FlowStateValidator:
    ...
    def validate_state_structure(state: Dict[str, Any]) -> List[str]:
        ...
        if 'current_phase' in state:
            ...
            if phase not in FlowStateValidator.VALID_PHASES:
                errors.append(f"Invalid current_phase: {phase}. Must be one of {FlowStateValidator.VALID_PHASES}")
        
        # Validate status
        if 'status' in state:
            status = state['status']
            if status not in FlowStateValidator.VALID_STATUSES:
                errors.append(f"Invalid status: {status}. Must be one of {FlowStateValidator.VALID_STATUSES}")
    ...
    def validate_data_consistency(state: Dict[str, Any]) -> List[str]:
        ...
        if current_phase in ['data_import', 'field_mapping'] and not state.get('raw_data'):
            errors.append(f"raw_data required for phase {current_phase}")
        
        # Validate field_mappings for processing phases
        if current_phase in ['data_cleansing', 'asset_creation'] and not state.get('field_mappings'):
            errors.append(f"field_mappings required for phase {current_phase}")
        
        # Validate cleaned_data for asset phases
        if current_phase in ['asset_inventory', 'dependency_analysis'] and not state.get('cleaned_data'):
        ...
        if state.get('status') == 'completed':
            required_phases = ['data_import', 'field_mapping', 'data_cleansing', 'asset_inventory']
            ...
            if isinstance(confidences, dict):
                for agent, confidence in confidences.items():
                    if not isinstance(confidence, (int, float)) or confidence < 0 or confidence > 1:
                        errors.append(f"Invalid confidence for {agent}: must be between 0 and 1")
    ...
    def validate_enterprise_data(state: Dict[str, Any]) -> List[str]:
        ...
        if 'learning_scope' in state:
            ...
            if scope not in valid_scopes:
                errors.append(f"Invalid learning_scope: {scope}. Must be one of {valid_scopes}")
        ...
        if 'memory_isolation_level' in state:
            ...
            if level not in valid_levels:
                errors.append(f"Invalid memory_isolation_level: {level}. Must be one of {valid_levels}")
    ...
    def validate_complete_state(cls, state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        all_errors = structure_errors + consistency_errors + agent_errors + enterprise_errors
        ...
        for field in optional_collections:
            if field in state and isinstance(state[field], list) and len(state[field]) == 0:
        ...
        if not result['valid']:
            logger.warning(f"State validation failed for flow {state.get('flow_id')}: {len(all_errors)} errors")
        else:
...
def validate_state_for_phase_transition(state: Dict[str, Any], target_phase: str) -> bool:
