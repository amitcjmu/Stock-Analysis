...
SAFE_ATTRIBUTES = {
    'name', 'description', 'type', 'status', 'created_at', 'updated_at',
    'id', 'flow_type', 'flow_name', 'phase', 'progress', 'metadata',
    'retry_count', 'max_retries', 'timeout', 'enabled', 'priority'
}
...
def secure_setattr(obj: Any, attr_name: str, value: Any, 
    ...
    try:
        ...
        if _is_sensitive_value(value):
            logger.warning(f"Blocked setting attribute {attr_name} with sensitive value")
            return False
...
def secure_bulk_setattr(obj: Any, attributes: Dict[str, Any], 
                        allowed_attrs: Set[str] = None,
...
def _is_sensitive_value(value: Any) -> bool:
    ...
    if isinstance(value, str):
        ...
        if len(value) > 20 and any(pattern in value_lower for pattern in SENSITIVE_ATTRIBUTES):
            return True
        # Check for JWT pattern
        if value.count('.') == 2 and len(value) > 50:
            return True
        # Check for common secret patterns
        if any(pattern in value_lower for pattern in ['bearer ', 'token ', 'key ', 'secret ']):
            return True
