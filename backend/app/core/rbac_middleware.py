...
try:
    from app.services.rbac_service import create_rbac_service, RBAC_MODELS_AVAILABLE
...
class RBACMiddleware:
    ...
    def __init__(self):
        ...
        if not self.rbac_enabled:
            logger.warning("RBAC Middleware initialized but RBAC service is not available")
    ...
    async def __call__(self, request: Request, call_next):
        ...
        if not access_result["has_access"]:
            logger.warning(f"Access denied for user {user_id} to {path}: {access_result['reason']}")
    ...
    async def _validate_access(self, user_id: str, path: str, method: str, request: Request) -> Dict[str, Any]:
        ...
        try:
            ...
            async with AsyncSessionLocal() as db:
                ...
                resource_type, action = self._determine_resource_and_action(path, method)
