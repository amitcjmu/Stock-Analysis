...
async def get_db_with_timeout(
    timeout_seconds: Optional[int] = None, operation_type: str = "default"
    ...
    if timeout_seconds is None:
        logger.info(
            f"Creating DB session for {operation_type} with no timeout (agentic activity)"
        )
    else:
        logger.info(
            f"Creating DB session for {operation_type} with {timeout_seconds}s timeout"
        )
    ...
    try:
        ...
        timeout_msg = (
            f"{timeout_seconds}s" if timeout_seconds is not None else "unlimited"
        )
        logger.error(
            f"Database session timeout for {operation_type} after {timeout_msg}"
        )
