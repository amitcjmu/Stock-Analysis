...
try:
    ...
    from ..crewai_flows.crews.dependency_analysis_crew import create_dependency_analysis_crew
...
class CrewEscalationManager:
    ...
    def __init__(self):
        ...
        logger.info("🚀 Crew Escalation Manager initialized with strategic crew integration")
    ...
    async def _execute_crew_thinking(self, escalation_id: str, crew_type: str, 
        ...
        try:
            ...
            await self._update_escalation_progress(escalation_id, 30, "strategic_analysis",
            ...
            if STRATEGIC_CREWS_AVAILABLE and crew_type in self.strategic_crews:
                try:
                    ...
                    if crew_type == "asset_intelligence_crew":
                        crew_results = await strategic_crew.analyze_assets(assets_data, context)
                    elif crew_type == "dependency_analysis_crew":
                        crew_results = await strategic_crew.analyze_dependencies(assets_data, context)
                    elif crew_type == "tech_debt_analysis_crew":
                        crew_results = await strategic_crew.analyze_tech_debt(assets_data, context)
            ...
            if crew_results:
                preliminary_insights = self._extract_strategic_insights(crew_type, crew_results, context)
            else:
            ...
            if crew_results:
                results = await self._generate_strategic_crew_results(crew_type, context, crew_results, preliminary_insights)
            else:
            ...
            logger.info(f"✅ Strategic crew thinking completed for escalation {escalation_id}")
    ...
    async def _execute_crew_collaboration(self, escalation_id: str, 
        ...
        try:
            ...
            await self._update_escalation_progress(escalation_id, 10, "collaboration_setup",
            ...
            await self._update_escalation_progress(escalation_id, 30, "primary_analysis",
            ...
            if STRATEGIC_CREWS_AVAILABLE and primary_crew in self.strategic_crews:
                primary_results = await self._execute_strategic_crew_analysis(primary_crew, context)
            
            await asyncio.sleep(3)
            
            # Phase 3: Delegation and Parallel Analysis
            await self._update_escalation_progress(escalation_id, 50, "delegation_phase",
            ...
            if pattern == "parallel_delegation" and additional_crews:
                ...
                delegation_results = await self._execute_parallel_delegation(additional_crews, context, escalation_id)
            elif pattern == "sequential_delegation" and additional_crews:
                # Execute sequential delegation
                delegation_results = await self._execute_sequential_delegation(additional_crews, context, escalation_id)
            elif pattern == "hierarchical_delegation":
                # Execute hierarchical delegation
                delegation_results = await self._execute_hierarchical_delegation(primary_crew, additional_crews, context, escalation_id)
            
            await asyncio.sleep(3)
            
            # Phase 4: Collaborative Synthesis
            await self._update_escalation_progress(escalation_id, 75, "collaborative_synthesis",
            ...
            comprehensive_insights = self._synthesize_multi_crew_insights(all_crew_results, collaboration_strategy, context)
            ...
            await self._update_escalation_progress(escalation_id, 95, "final_synthesis",
                                                 "Generating final strategic recommendations")
            
            await asyncio.sleep(2)
            
            # Generate final collaborative results
            results = await self._generate_collaborative_results(collaboration_strategy, context, comprehensive_insights)
            results["crew_collaboration_details"] = {
                "primary_crew": primary_crew,
                "additional_crews": additional_crews,
                "collaboration_pattern": pattern,
                "total_crews_involved": len(all_crew_results),
                "strategic_crews_executed": sum(1 for r in all_crew_results.values() if r is not None)
            }
            ...
            logger.info(f"✅ Strategic crew collaboration completed for escalation {escalation_id}")
            
        except Exception as e:
            logger.error(f"❌ Error in strategic crew collaboration {escalation_id}: {e}")
            await self._handle_escalation_error(escalation_id, str(e))
    
    async def _execute_strategic_crew_analysis(self, crew_type: str, context: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    async def _execute_parallel_delegation(self, crews: List[str], context: Dict[str, Any], escalation_id: str) -> Dict[str, Any]:
        ...
        for crew_type in crews:
            if STRATEGIC_CREWS_AVAILABLE and crew_type in self.strategic_crews:
                task = asyncio.create_task(self._execute_strategic_crew_analysis(crew_type, context))
    ...
    async def _execute_sequential_delegation(self, crews: List[str], context: Dict[str, Any], escalation_id: str) -> Dict[str, Any]:
    ...
    async def _execute_hierarchical_delegation(self, primary_crew: str, additional_crews: List[str], 
                                             context: Dict[str, Any], escalation_id: str) -> Dict[str, Any]:
        """Execute hierarchical delegation with specialist review."""
        results = {}
        
        # Level 1: Specialist analysis (additional crews)
        specialist_results = await self._execute_parallel_delegation(additional_crews[:2], context, escalation_id)
        ...
        senior_result = await self._execute_strategic_crew_analysis(primary_crew, enhanced_context)
        ...
        if len(additional_crews) > 2:
            ...
            executive_result = await self._execute_strategic_crew_analysis(additional_crews[2], executive_context)
            results["executive_synthesis"] = executive_result
        
        return results

    def _extract_strategic_insights(self, crew_type: str, crew_results: Dict[str, Any], context: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        try:
            if crew_results.get("success") and crew_results.get("analysis_results"):
                ...
                if crew_type == "asset_intelligence_crew":
                    insights.extend(self._extract_asset_intelligence_insights(analysis_results, context))
                elif crew_type == "dependency_analysis_crew":
                    insights.extend(self._extract_dependency_insights(analysis_results, context))
                elif crew_type == "tech_debt_analysis_crew":
                    insights.extend(self._extract_tech_debt_insights(analysis_results, context))
    ...
    def _extract_asset_intelligence_insights(self, analysis_results: List[Any], context: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        for result in analysis_results:
            ...
            if hasattr(result, 'migration_priority') and result.migration_priority == "high":
                insights.append({
                    ...
                    "business_value": result.business_context.get("business_value_score", 0)
                })
        
        return insights

    def _extract_dependency_insights(self, analysis_results: List[Any], context: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    def _extract_tech_debt_insights(self, analysis_results: List[Any], context: Dict[str, Any]) -> List[Dict[str, Any]]:
            ...
            if hasattr(result, 'legacy_assessment'):
                ...
                if legacy_level in ["high", "critical"]:
                    insights.append({
                        ...
                        "modernization_urgency": result.legacy_assessment.get("modernization_urgency", {})
                    })
        
        return insights

    def _synthesize_multi_crew_insights(self, crew_results: Dict[str, Any], 
                                      collaboration_strategy: Dict[str, Any], 
        ...
        synthesized_insights.append({
            ...
            "crews_executed": len([r for r in crew_results.values() if r is not None]),
            ...
            "collaboration_effectiveness": len([r for r in crew_results.values() if r is not None]) / len(crew_results)
        })
        
        return synthesized_insights

    async def _generate_strategic_crew_results(self, crew_type: str, context: Dict[str, Any], 
        ...
        base_results = await self._generate_crew_results(crew_type, context, preliminary_insights)
        ...
        if crew_results and crew_results.get("success"):
            base_results.update({
                ...
                "strategic_recommendations": self._extract_strategic_recommendations(crew_type, crew_results),
                "confidence_improvements": {
                    "strategic_analysis_applied": True,
                    "analysis_depth": "comprehensive",
                    "crew_confidence": crew_results.get("metadata", {}).get("average_confidence", 0.8)
                }
            })
        else:
            base_results.update({
    ...
    def _extract_strategic_recommendations(self, crew_type: str, crew_results: Dict[str, Any]) -> List[str]:
        ...
        try:
            ...
            for result in analysis_results:
                ...
                if crew_type == "asset_intelligence_crew" and hasattr(result, 'migration_priority'):
                    if result.migration_priority == "high":
                        recommendations.append(f"Prioritize {result.asset_name} for early migration due to high business value")
                elif crew_type == "dependency_analysis_crew" and hasattr(result, 'critical_path_analysis'):
                    recommendations.append(f"Review dependency paths for {result.asset_name} before migration")
                elif crew_type == "tech_debt_analysis_crew" and hasattr(result, 'sixr_recommendations'):
                    sixr_strategy = result.sixr_recommendations.get("recommended_strategy", "rehost")
                    recommendations.append(f"Apply {sixr_strategy} strategy for {result.asset_name}")
        
        except Exception as e:
            logger.error(f"❌ Error extracting strategic recommendations: {e}")
            recommendations.append("Review strategic analysis results for detailed recommendations")
