...
class AgentPerformanceMonitor:
    ...
    def __init__(self, max_events: int = 10000, analysis_window_hours: int = 24):
        ...
        self.performance_events: Dict[str, deque] = defaultdict(lambda: deque(maxlen=max_events))
        ...
        self.performance_baselines = {
            ...
            "dependency_analysis": {"target_duration": 90.0, "target_success_rate": 0.88},
            "tech_debt_analysis": {"target_duration": 120.0, "target_success_rate": 0.85}
        }
    ...
    def end_operation(
        ...
        self.performance_trends[event.operation_type].append((event.end_time, event.duration))
        ...
        self.performance_trends[event.operation_type] = [
            (ts, dur) for ts, dur in self.performance_trends[event.operation_type]
            if ts > cutoff_time
        ]
    ...
    async def _analyze_real_time_performance(self, event: PerformanceEvent):
        ...
        try:
            ...
            if event.duration and event.duration > target_duration * 2:
                logger.warning(f"🚨 Performance alert: {event.operation_type} took {event.duration:.2f}s (target: {target_duration}s)")
                ...
                await enhanced_agent_memory.store_memory(
                    {
                        ...
                        "severity": "high" if event.duration > target_duration * 3 else "medium",
    ...
    def get_performance_metrics(
        ...
        for op_type in operation_types:
            events = [
                event for event in self.performance_events[op_type]
            ...
            if durations:
                metrics[op_type] = PerformanceMetrics(
                    ...
                    throughput_per_minute=len(events) / (time_window.total_seconds() / 60),
    ...
    def _calculate_memory_efficiency(self, events: List[PerformanceEvent]) -> float:
        ...
        try:
            memory_usages = [e.memory_usage for e in events if e.memory_usage is not None]
    ...
    async def analyze_performance_trends(
        ...
        try:
            ...
            historical_avg = statistics.mean(durations[:-10]) if len(durations) > 10 else recent_avg
            
            trend_direction = "improving" if recent_avg < historical_avg else "degrading"
            trend_magnitude = abs(recent_avg - historical_avg) / historical_avg if historical_avg > 0 else 0
    ...
    async def _detect_performance_patterns(
        ...
        try:
            ...
            earlier_chunk = durations[-20:-10] if len(durations) >= 20 else durations[:-10]
            ...
            recent_variance = statistics.variance(recent_chunk) if len(recent_chunk) > 1 else 0
    ...
    async def _generate_performance_recommendations(
        ...
        try:
            ...
            if avg_recent > target_duration * 1.5:
                recommendations.append(f"Consider enabling response caching - current avg ({avg_recent:.1f}s) significantly exceeds target ({target_duration}s)")
            
            if avg_recent > target_duration * 1.2:
                recommendations.append("Review agent configuration - consider reducing max_iter or enabling parallel processing")
            
            # Pattern-based recommendations
            for pattern in patterns:
                if pattern["type"] == "frequent_outliers":
                    recommendations.append("Investigate outlier causes - consider implementing timeout controls")
                elif pattern["type"] == "performance_degradation":
                    recommendations.append("Performance degradation detected - review recent configuration changes")
                elif pattern["type"] == "improved_consistency":
                    recommendations.append("Performance consistency improved - current optimization strategies are effective")
            ...
            if context:
                ...
                if past_optimizations:
                    successful_optimizations = [
                        opt for opt in past_optimizations
                    ...
                    for opt in successful_optimizations:
                        ...
                        if improvement > 1.2:  # 20% improvement
                            recommendations.append(f"Consider re-applying {optimization_type} - previously achieved {improvement:.1f}x improvement")
            ...
            if cache_hit_rate < 0.3:
                recommendations.append("Enable more aggressive caching - current cache hit rate is low")
    ...
    async def optimize_based_on_performance(
        ...
        try:
            ...
            cache_key = f"{operation_type}_{context.context_hash if context else 'global'}"
            ...
            if current_metrics.cache_hit_rate < 0.5:
                # Increase cache TTL
                response_optimizer.cache.ttl_seconds = min(7200, response_optimizer.cache.ttl_seconds * 1.5)
                optimization_result["optimizations_applied"].append("increased_cache_ttl")
                optimization_result["expected_improvements"]["cache_hit_rate"] = 0.2
            
            # Apply memory optimizations
            if current_metrics.avg_duration > self.performance_baselines.get(operation_type, {}).get("target_duration", 60) * 1.5:
                # Trigger memory cleanup
                memory_cleanup = await enhanced_agent_memory.optimize_memory_performance()
                optimization_result["optimizations_applied"].append("memory_optimization")
                optimization_result["expected_improvements"]["avg_duration"] = -0.3  # 30% improvement expected
    ...
    def get_comprehensive_report(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for op_type, metrics in all_metrics.items():
                ...
                duration_score = min(1.0, target_duration / metrics.avg_duration) if metrics.avg_duration > 0 else 1.0
                success_score = (1 - metrics.error_rate) / target_success_rate if target_success_rate > 0 else 1.0
            ...
            report = {
                ...
                "total_operations_monitored": sum(len(events) for events in self.performance_events.values()),
...
def monitor_performance(operation_type: str):
    ...
    def decorator(func):
        async def async_wrapper(*args, **kwargs):
            ...
            event_id = agent_performance_monitor.start_operation(agent_id, operation_type)
            ...
            try:
                ...
                agent_performance_monitor.end_operation(event_id, success=False, error_message=str(e))
                raise
