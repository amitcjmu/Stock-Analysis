...
class CredentialAuditService:
    ...
    async def detect_suspicious_patterns(
        self, user_id: uuid.UUID, time_window_minutes: int = 60
    ) -> List[Dict[str, Any]]:
        """
        ...
        rapid_access = await self.session.execute(
            select(func.count(CredentialAccessLog.id)).where(
                and_(
        ...
        client_access = await self.session.execute(
            select(
                func.count(func.distinct(SecurityAuditLog.details["client_account_id"]))
            ).where(
                and_(
    ...
    async def get_credential_access_history(
        self, credential_id: uuid.UUID, days: int = 30, include_failed: bool = True
    ...
    async def get_user_credential_activity(
        ...
        return {
            ...
            "risk_level": self._calculate_risk_level(
                access_by_type, violation_count, len(suspicious_patterns)
            ),
        }
    ...
    async def generate_compliance_report(
        ...
        if client_account_id:
            base_conditions.append(
                SecurityAuditLog.details["client_account_id"].astext
    ...
    def _calculate_risk_level(
        self, access_counts: Dict[str, int], violations: int, suspicious_patterns: int
    ) -> str:
        """
