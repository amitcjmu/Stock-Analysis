    ...
    async def create_discovery_flow(
        ...
        try:
            ...
            if master_flow_id:
                ...
                logger.info(
                    "🔧 Creating new master flow for discovery flow coordination"
                )
            ...
            logger.info(
                f"🔧 Creating discovery flow: {flow_id} with master_flow_id: {master_flow_id}"
            )
            ...
            logger.info(
                f"   Master flow: crewai_flow_state_extensions.flow_id = {master_flow_id}"
            )
            logger.info(f"   Discovery flow: discovery_flows.flow_id = {flow_id}")
            logger.info(
                f"   Discovery flow: discovery_flows.master_flow_id = {master_flow_id}"
            )
