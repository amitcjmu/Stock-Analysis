...
class ImportStorageHandler:
    ...
    async def handle_import(
        ...
        try:
            ...
            logger.info(
                f"🔄 Starting data import handling for session: {import_validation_id}"
            )
            ...
            async with self.transaction_manager.transaction():
                ...
                await self.storage_manager.create_field_mappings(
                    data_import=data_import, file_data=file_data
                )
                ...
                try:
                    ...
                    if flow_id:
                        ...
                        logger.error(
                            "❌ Discovery flow creation failed - no flow_id returned"
                        )
                ...
                if flow_success and flow_id:
                    await self.storage_manager.link_master_flow_to_import(
                        data_import_id=data_import.id, master_flow_id=flow_id
                    )
                    await self.storage_manager.update_import_status(
                        data_import=data_import, status="discovery_initiated"
                    )
                else:
                    await self.storage_manager.update_import_status(
                        data_import=data_import,
            ...
            if flow_success and flow_id:
                ...
                await self.background_service.start_background_flow_execution(
                    flow_id=flow_id, file_data=file_data, context=context
                )
    ...
    async def get_import_status(self, import_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            return {
                ...
                "completed_at": (
                    data_import.completed_at.isoformat()
                    if data_import.completed_at
                    else None
                ),
            }
    ...
    async def get_import_data(
        self, import_id: str, limit: int = 1000
        ...
        try:
            ...
            raw_records = await self.storage_manager.get_raw_records(
                data_import_id=data_import.id, limit=limit
            )

            return {
                "import_metadata": self.response_builder.format_import_metadata(
                    data_import=data_import, records_count=len(raw_records)
                ),
    ...
    async def cancel_import(self, import_id: str, context: RequestContext) -> bool:
        ...
        try:
            ...
            await self.storage_manager.update_import_status(
                data_import=data_import, status="cancelled"
            )
        ...
        try:
            ...
            try:
                ...
                if flow_id:
                    ...
                    try:
                        logger.info(
                            f"🔗 Linking master flow {flow_id} to data import {data_import.id} (retry)"
                        )
                        await self.storage_manager.link_master_flow_to_import(
                            data_import_id=data_import.id, master_flow_id=flow_id
                        )
                        logger.info(
                            f"✅ Master flow linkage completed successfully for retry flow {flow_id}"
                        )
                    except Exception as e:
                        logger.error(
                            f"💥 Master flow linkage failed for retry flow {flow_id}: {e}",
                            exc_info=True,
                        )
    ...
    async def get_latest_import_data(
        ...
        try:
            ...
            raw_records = await self.storage_manager.get_raw_records(
                data_import_id=latest_import.id, limit=1000  # Limit for performance
            )

            return {
                "import_metadata": self.response_builder.format_import_metadata(
                    data_import=latest_import, records_count=len(raw_records)
                ),
                "data": [record.raw_data for record in raw_records],
