...
class AssetCreationBridgeService:
    ...
    async def create_assets_from_discovery(
        ...
        """
        try:
            logger.info(f"🏗️ Starting asset creation from discovery flow: {discovery_flow_id}")
            ...
            for discovery_asset in discovery_assets:
                try:
                    ...
                    if existing_asset:
                        logger.info(f"⚠️ Skipping duplicate asset: {discovery_asset.asset_name}")
                    ...
                    if new_asset:
                        ...
                        logger.info(f"✅ Created asset: {new_asset.name} (ID: {new_asset.id})")
                    
                except Exception as e:
                    logger.error(f"❌ Failed to process discovery asset {discovery_asset.id}: {e}")
                    errors.append({
                        "discovery_asset_id": str(discovery_asset.id),
                        "asset_name": discovery_asset.asset_name,
                        "error": str(e)
    ...
    async def _create_asset_from_discovery(
        ...
        new_asset = Asset(
            ...
            description=normalized_data.get('description') or f"Discovered via {discovery_asset.discovery_method}",
            ...
            migration_status=AssetStatus.ASSESSED if discovery_asset.migration_ready else AssetStatus.DISCOVERED,
    ...
    async def _update_discovery_flow_completion(
        ...
        state_data["next_phase"] = "dependencies" if len(created_assets) > 0 else "completed"
        
        discovery_flow.crewai_state_data = state_data
        
        await self.db.commit()
        
        logger.info(f"📊 Updated discovery flow {discovery_flow.flow_id} with asset creation results") 