...
class PerformanceMonitor:
    ...
    def end_operation(
        ...
        logger.info(
            f"{status} {operation_data['operation_type']} {operation_id} completed in {duration:.2f}s"
        )
    ...
    def get_performance_dashboard(self) -> Dict[str, Any]:
        ...
        for metric_type, type_metrics in metrics_by_type.items():
            dashboard["metrics_by_type"][metric_type] = self._analyze_metrics(
                type_metrics
            )
    ...
    def _analyze_metrics(self, metrics: List[PerformanceMetric]) -> Dict[str, Any]:
        ...
        return {
            ...
            "recent_operations": len(
                [m for m in metrics if m.start_time > time.time() - 3600]
            ),
        }
...
def start_operation_tracking(
    operation_type: str, operation_id: str, metadata: Optional[Dict[str, Any]] = None
):
    """Start tracking an operation"""
...
def end_operation_tracking(
    operation_id: str, success: bool = True, metadata: Optional[Dict[str, Any]] = None
):
    """End tracking an operation"""
