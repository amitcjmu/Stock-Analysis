...
class AdapterRegistry:
    ...
    def register(
        self, adapter_class: Type[BaseAdapter], metadata: AdapterMetadata
    ) -> None:
        """
        ...
        if adapter_key in self._adapters:
            self.logger.warning(
                f"Adapter {adapter_key} already registered, overwriting"
            )
        ...
        self.logger.info(
            f"Registered adapter {adapter_key} for platforms: {metadata.supported_platforms}"
        )
    ...
    def get_adapter(
        self, name: str, version: str, db: AsyncSession
