...
class TierDetectionService:
    ...
    TIER_4_PLATFORMS = {
        "servicenow": {
            "api": True,
            "categories": [PlatformCategory.CMDB, PlatformCategory.ORCHESTRATION],
        },
        "vmware_vcenter": {
            "api": True,
            "categories": [PlatformCategory.VIRTUALIZATION],
        },
        ...
        "splunk": {
            "api": True,
            "categories": [PlatformCategory.MONITORING, PlatformCategory.SECURITY],
        },
    }
    ...
    TIER_3_PLATFORMS = {
        ...
        "nagios": {
            "api": False,
            "script": True,
            "categories": [PlatformCategory.MONITORING],
        },
        ...
        "oracle_db": {
            "api": False,
            "script": True,
            "categories": [PlatformCategory.DATABASE],
        },
    }
    ...
    TIER_2_PLATFORMS = {
        ...
        "bash_scripts": {
            "script": True,
            "categories": [PlatformCategory.ORCHESTRATION],
        },
    ...
    async def detect_environment_tier(
        ...
        try:
            ...
            capabilities = await self._analyze_capabilities(
                platforms, requested_capabilities
            )
            ...
            assessment = TierAssessment(
                ...
                assessment_metadata={
                    ...
                    "tier_rationale": self._explain_tier_selection(
    ...
    async def _detect_platforms(
        ...
        for platform_data in declared_platforms:
            ...
            platform_info = await self._create_platform_info(
    ...
    async def _detect_from_tool(self, tool: Dict[str, Any]) -> Optional[PlatformInfo]:
    ...
    async def _analyze_capabilities(
        ...
        if requested_capabilities:
            available_capabilities = available_capabilities.intersection(
    ...
    def _determine_tier(
        self, platforms: List[PlatformInfo], capabilities: List[AdapterCapability]
    ...
    def _identify_limitations(
        self, platforms: List[PlatformInfo], tier: AutomationTier
    ) -> List[str]:
        ...
        has_monitoring = any(
            p.category == PlatformCategory.MONITORING for p in platforms
        )

        if not has_cmdb:
            limitations.append(
                "No CMDB platform detected - inventory data may be incomplete"
            )

        if not has_monitoring:
            limitations.append(
                "No monitoring platform detected - performance metrics unavailable"
            )
        ...
        if tier == AutomationTier.TIER_1:
            limitations.append(
                "Manual data collection required - increased time and effort"
            )
            limitations.append("Limited validation capabilities")

        elif tier == AutomationTier.TIER_2:
            limitations.append(
                "Script execution required - potential security constraints"
            )
    ...
    def _generate_recommendations(
        ...
        if tier == AutomationTier.TIER_1:
            ...
            api_capable = [
                p
                for p in platforms
                if p.name in {**self.TIER_3_PLATFORMS, **self.TIER_4_PLATFORMS}
            ]
            if api_capable:
                recommendations.append(
        ...
        if not any(p.category == PlatformCategory.CMDB for p in platforms):
            recommendations.append(
                "Implement a CMDB solution for centralized asset management"
            )

        if not any(p.category == PlatformCategory.MONITORING for p in platforms):
            recommendations.append(
                "Deploy monitoring tools for performance and dependency data"
            )
    ...
    def _create_environment_summary(
        ...
        return {
            ...
            "average_confidence": (
                sum(p.confidence_score for p in platforms) / len(platforms)
                if platforms
                else 0
            ),
        }
    ...
    def _explain_tier_selection(
        ...
        return explanations.get(
            tier, "Tier selection based on available platforms and capabilities"
        )
