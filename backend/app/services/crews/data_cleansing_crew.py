        ...
        try:
            ...
            return {
                ...
                "issues": {
                    ...
                    "high_severity": sum(
                        1 for issue in issues_log if issue.get("severity") == "high"
                    ),
                    "medium_severity": sum(
                        1 for issue in issues_log if issue.get("severity") == "medium"
                    ),
                    "low_severity": sum(
                        1 for issue in issues_log if issue.get("severity") == "low"
                    ),
        ...
        for line in lines:
            ...
            if "records processed" in line or "total records" in line:
                ...
                try:
                    ...
                    if scores:
                        result["quality_metrics"]["overall_quality_score"] = float(
                            scores[0]
                        )
                except Exception:
                    pass
