...
class AssetInventoryCrew(BaseDiscoveryCrew):
    ...
    def create_tasks(self, inputs: Dict[str, Any]) -> List[Task]:
        ...
        criticality_criteria = inputs.get(
            "criticality_criteria", self._get_default_criticality_criteria()
        )
    ...
    def process_results(self, raw_results: Any) -> Dict[str, Any]:
        ...
        try:
            ...
            critical_count = sum(
                1
                for asset in asset_criticality
                if asset.get("criticality_level") == "critical"
            )
            high_count = sum(
                1
                for asset in asset_criticality
                if asset.get("criticality_level") == "high"
            )
            medium_count = sum(
                1
                for asset in asset_criticality
                if asset.get("criticality_level") == "medium"
            )
            low_count = sum(
                1
                for asset in asset_criticality
                if asset.get("criticality_level") == "low"
            )
            ...
            return {
                ...
                "criticality_distribution": {
                    ...
                    "low": low_count,
                    "risk_percentage": (
                        ((critical_count + high_count) / total_assets * 100)
                        if total_assets > 0
                        else 0.0
                    ),
                },
                "migration_readiness": {
                    "wave_1_ready": sum(
                        1
                        for asset in asset_criticality
                        if asset.get("migration_priority") == 1
                    ),
                    "wave_2_ready": sum(
                        1
                        for asset in asset_criticality
                        if asset.get("migration_priority") == 2
                    ),
                    "wave_3_ready": sum(
                        1
                        for asset in asset_criticality
                        if asset.get("migration_priority") == 3
                    ),
                    "requires_planning": sum(
                        1
                        for asset in asset_criticality
                        if asset.get("migration_priority") == 4
                    ),
    ...
    def _calculate_readiness_score(
        self, asset_criticality: List[Dict[str, Any]]
    ) -> float:
        ...
        total_assets = len(asset_criticality)
        ready_assets = sum(
            1 for asset in asset_criticality if asset.get("migration_priority", 4) <= 2
        )
