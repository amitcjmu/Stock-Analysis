...
class StockFinancialsAgent:
    ...
    def _create_financials_prompt(self, stock_data: Dict[str, Any]) -> str:
        ...
        debt_to_assets = (
            stock_data.get("debtToAssets")
            or stock_data.get("debt_to_assets")
        )
        price_to_sales = (
...
"""
        ...
        if all_financial_data:
            ...
            for key, value in sorted(all_financial_data.items())[:30]:  # Limit to 30 fields
...
"""
    ...
    def _parse_llm_response(
        ...
        try:
            ...
            if not profitability.get("net_margin") or profitability.get("net_margin") in ["null", "N/A", "undeterminable"]:
                net_margin = stock_data.get("netProfitMargin") or stock_data.get("profitMargins")
                if net_margin:
                    profitability["net_margin"] = f"{net_margin:.2%}" if isinstance(net_margin, float) else net_margin
            
            if not profitability.get("roe") or profitability.get("roe") in ["null", "N/A", "undeterminable"]:
                roe = stock_data.get("returnOnEquity")
                if roe:
                    profitability["roe"] = f"{roe:.2%}" if isinstance(roe, float) else roe
            
            if not profitability.get("roa") or profitability.get("roa") in ["null", "N/A", "undeterminable"]:
                roa = stock_data.get("returnOnAssets")
                if roa:
                    profitability["roa"] = f"{roa:.2%}" if isinstance(roa, float) else roa
            
            if not liquidity.get("current_ratio") or liquidity.get("current_ratio") in ["null", "N/A", "undeterminable"]:
                current_ratio = stock_data.get("currentRatio")
                if current_ratio:
                    liquidity["current_ratio"] = f"{current_ratio:.2f}" if isinstance(current_ratio, float) else current_ratio
            
            if not liquidity.get("quick_ratio") or liquidity.get("quick_ratio") in ["null", "N/A", "undeterminable"]:
                quick_ratio = stock_data.get("quickRatio")
                if quick_ratio:
                    liquidity["quick_ratio"] = f"{quick_ratio:.2f}" if isinstance(quick_ratio, float) else quick_ratio
            
            if not leverage.get("debt_to_equity") or leverage.get("debt_to_equity") in ["null", "N/A", "undeterminable"]:
                debt_to_equity = stock_data.get("debtToEquity")
                if debt_to_equity:
                    leverage["debt_to_equity"] = f"{debt_to_equity:.2f}" if isinstance(debt_to_equity, float) else debt_to_equity
            
            if not leverage.get("debt_to_assets") or leverage.get("debt_to_assets") in ["null", "N/A", "undeterminable"]:
                debt_to_assets = stock_data.get("debtToAssets")
                if debt_to_assets:
                    leverage["debt_to_assets"] = f"{debt_to_assets:.2f}" if isinstance(debt_to_assets, float) else debt_to_assets
            
            if not valuation.get("pe_ratio") or valuation.get("pe_ratio") in ["null", "N/A", "undeterminable"]:
                pe_ratio = stock_data.get("trailingPE") or stock_data.get("forwardPE")
                if pe_ratio:
                    valuation["pe_ratio"] = f"{pe_ratio:.2f}" if isinstance(pe_ratio, float) else pe_ratio
            
            if not valuation.get("price_to_book") or valuation.get("price_to_book") in ["null", "N/A", "undeterminable"]:
                price_to_book = stock_data.get("priceToBook")
                if price_to_book:
                    valuation["price_to_book"] = f"{price_to_book:.2f}" if isinstance(price_to_book, float) else price_to_book
            
            if not valuation.get("price_to_sales") or valuation.get("price_to_sales") in ["null", "N/A", "undeterminable"]:
                price_to_sales = stock_data.get("priceToSalesTrailing12Months") or stock_data.get("priceToSales")
                if price_to_sales:
                    valuation["price_to_sales"] = f"{price_to_sales:.2f}" if isinstance(price_to_sales, float) else price_to_sales
            ...
            if not valuation_analysis.get("valuation") or valuation_analysis.get("valuation") in [None, "null", "N/A", "undeterminable"]:
                # Determine valuation based on P/E ratio
                pe_ratio_val = stock_data.get("trailingPE") or stock_data.get("forwardPE")
                if pe_ratio_val:
                    if pe_ratio_val > 25:
            ...
            if not valuation_analysis.get("valuation_basis") or valuation_analysis.get("valuation_basis") in [None, "null", "N/A"]:
                pe_ratio_val = stock_data.get("trailingPE") or stock_data.get("forwardPE")
                if pe_ratio_val:
                    valuation_analysis["valuation_basis"] = f"Based on P/E ratio of {pe_ratio_val:.2f}"
                else:
                    valuation_analysis["valuation_basis"] = "Analysis based on available financial metrics"
            
            if not valuation_analysis.get("price_target") or valuation_analysis.get("price_target") in [None, "null", "N/A"]:
                ...
                if current_price and pe_ratio_val and eps_val:
                    ...
                    valuation_analysis["price_target"] = round(pe_ratio_val * eps_val, 2)
                    if current_price:
                        upside = ((valuation_analysis["price_target"] - current_price) / current_price) * 100
            ...
            if not financial_health or len(financial_health) == 0:
                ...
                if roe:
                    financial_health["profitability"] = "strong" if roe > 0.15 else "moderate" if roe > 0.10 else "weak"
                else:
                    financial_health["profitability"] = "moderate"
                
                if current_ratio:
                    financial_health["liquidity"] = "strong" if current_ratio > 1.5 else "moderate" if current_ratio > 1.0 else "weak"
                else:
                    financial_health["liquidity"] = "moderate"
                
                if debt_to_equity:
                    financial_health["solvency"] = "strong" if debt_to_equity < 50 else "moderate" if debt_to_equity < 100 else "weak"
                else:
            ...
            if not growth_analysis.get("revenue_growth") or growth_analysis.get("revenue_growth") in ["null", "N/A", "undeterminable"]:
                revenue_growth = stock_data.get("revenueGrowth")
                if revenue_growth is not None:
                    growth_analysis["revenue_growth"] = "positive" if revenue_growth > 0 else "negative" if revenue_growth < 0 else "stable"
                else:
                    growth_analysis["revenue_growth"] = "stable"
            
            if not growth_analysis.get("earnings_growth") or growth_analysis.get("earnings_growth") in ["null", "N/A", "undeterminable"]:
                earnings_growth = stock_data.get("earningsGrowth") or stock_data.get("earningsQuarterlyGrowth")
                if earnings_growth is not None:
                    growth_analysis["earnings_growth"] = "positive" if earnings_growth > 0 else "negative" if earnings_growth < 0 else "stable"
                else:
                    growth_analysis["earnings_growth"] = "stable"
            
            if not growth_analysis.get("growth_prospects") or growth_analysis.get("growth_prospects") in ["null", "N/A", "undeterminable"]:
                revenue_growth = stock_data.get("revenueGrowth")
                earnings_growth = stock_data.get("earningsGrowth") or stock_data.get("earningsQuarterlyGrowth")
                if (revenue_growth and revenue_growth > 0.1) or (earnings_growth and earnings_growth > 0.1):
                    growth_analysis["growth_prospects"] = "strong"
                elif (revenue_growth and revenue_growth > 0) or (earnings_growth and earnings_growth > 0):
