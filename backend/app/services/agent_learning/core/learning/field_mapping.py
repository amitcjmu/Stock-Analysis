...
class FieldMappingLearning:
    ...
    async def learn_field_mapping_pattern(
        self, learning_data: Dict[str, Any], context: Optional[LearningContext] = None
    ):
    ...
    async def suggest_field_mapping(
        ...
        for pattern in patterns:
            if pattern.pattern_type == "field_mapping":
                ...
                if (
                    field_name.lower() in original_field
                    or original_field in field_name.lower()
                ):
        ...
        if not matching_patterns:
            ...
            for ctx_patterns in self.learning_patterns.values():
                for pattern in ctx_patterns:
                    if pattern.pattern_type == "field_mapping":
                        original_field = pattern.pattern_data.get(
                            "original_field", ""
                        ).lower()
                        if (
                            field_name.lower() in original_field
        ...
        if matching_patterns:
            ...
            return {
                ...
                "pattern_source": (
                    "context_specific"
                    if context_key in self.learning_patterns
                    else "global"
                ),
    ...
    async def _update_field_mappings_from_feedback(
        self, patterns: List[str], context: LearningContext
    ):
