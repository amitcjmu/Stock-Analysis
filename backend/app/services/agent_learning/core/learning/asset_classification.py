...
class AssetClassificationLearning:
    ...
    async def learn_from_asset_classification(
        ...
        try:
            pattern_data = {
                ...
                "metadata_patterns": {"original_metadata": asset_metadata},
                "predicted_asset_type": classification_result.get(
                    "asset_type", "unknown"
                ),
                "confidence_score": 0.9 if user_confirmed else 0.3,
                "learning_source": (
                    "user_feedback" if user_confirmed else "ai_inference"
                ),
    ...
    async def classify_asset_automatically(
        self, asset_data: Dict[str, Any], context: Optional[LearningContext] = None
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            similar_patterns = await self.vector_utils.find_similar_asset_patterns(
                asset_name, context.client_account_id, limit=1, similarity_threshold=0.7
            )
    ...
    async def _store_classification_pattern(self, pattern_data: Dict[str, Any]) -> str:
        ...
        logger.warning(
            "AssetClassificationPattern model not available - storing in memory only"
        )
