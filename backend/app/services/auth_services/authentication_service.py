...
class AuthenticationService:
    ...
    async def authenticate_user(self, login_request: LoginRequest) -> LoginResponse:
        ...
        try:
            ...
            if not user.password_hash:
                logger.warning(
                    f"Authentication attempt for user {user.email} without password hash - BLOCKED"
                )
                raise HTTPException(status_code=401, detail="Invalid credentials")
    ...
    async def change_user_password(
        self, user_id: uuid.UUID, password_change: PasswordChangeRequest
        ...
        try:
            ...
            new_password_hash = bcrypt.hashpw(
                password_change.new_password.encode("utf-8"), bcrypt.gensalt()
            ).decode("utf-8")
            ...
            raise HTTPException(
                status_code=500, detail=f"Password change failed: {str(e)}"
            )
    ...
    async def get_dashboard_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Users by role
            roles_query = select(UserRole.role_type, func.count()).group_by(
                UserRole.role_type
            )
