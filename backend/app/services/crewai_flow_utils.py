...
class CrewAIFlowUtils:
    ...
    def validate_context(context: Dict[str, Any]) -> bool:
        ...
        for field in required_fields:
            if not context.get(field):
                logger.warning(f"⚠️ Missing required context field: {field}")
                return False
    ...
    def log_operation_start(operation: str, flow_id: str, **kwargs) -> None:
        ...
        """
        logger.info(f"🚀 Starting {operation} for flow: {flow_id}")
        if kwargs:
    ...
    def log_operation_success(operation: str, flow_id: str, **kwargs) -> None:
        ...
        Args:
            ...
            **kwargs: Additional context to log
        """
        logger.info(f"✅ {operation} completed successfully for flow: {flow_id}")
        if kwargs:
    ...
    def log_operation_error(
        ...
        Args:
            ...
            **kwargs: Additional context to log
        """
        logger.error(f"❌ {operation} failed for flow: {flow_id}")
        ...
        try:
            ...
            for pattern, replacement in sensitive_patterns:
                if pattern in data_lower:
                    ...
                    logger.warning(
                        f"⚠️ Potential sensitive data detected in logs, pattern: {pattern}"
                    )

            return data_str

        except Exception as e:
            logger.warning(f"⚠️ Failed to sanitize data for logging: {e}")
    ...
    def measure_execution_time(func):
        ...
        async def async_wrapper(*args, **kwargs):
            ...
            try:
                ...
                execution_time = time.time() - start_time
                logger.error(
                    f"⏱️ {func.__name__} failed after {execution_time:.2f}s: {e}"
                )
                raise
