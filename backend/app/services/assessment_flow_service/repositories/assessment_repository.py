...
class AssessmentRepository:
    ...
    async def get_flow_assets(self, flow: DiscoveryFlow, filters: Optional[Dict[str, Any]] = None) -> List[DiscoveryAsset]:
        """
    ...
    async def get_assessment_ready_assets(self, flow_id: str) -> List[DiscoveryAsset]:
        ...
        try:
            ...
            logger.error(f"❌ Error retrieving assessment-ready assets for flow {flow_id}: {e}")
            raise
    ...
    async def update_flow_completion_status(
        ...
        try:
            ...
            if "assessment_package" in completion_data:
                update_values["assessment_package"] = completion_data["assessment_package"]
            ...
            if "migration_readiness_score" in completion_data:
                update_values["migration_readiness_score"] = completion_data["migration_readiness_score"]
    ...
    async def get_flow_statistics(self, flow_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            asset_types = {row.asset_type or "unknown": row.count for row in type_result}
            ...
            complexity_distribution = {row.migration_complexity or "unknown": row.count for row in complexity_result}
            
            # Calculate percentages
            ready_percentage = (migration_ready / total_assets * 100) if total_assets > 0 else 0
            validated_percentage = (validated_assets / total_assets * 100) if total_assets > 0 else 0
    ...
    async def get_assets_by_criteria(
        ...
        try:
            ...
            query = query.order_by(
                DiscoveryAsset.migration_priority.asc(),
                DiscoveryAsset.asset_name.asc()
            )
    ...
    def _apply_asset_filters(self, query, filters: Dict[str, Any]):
        ...
        if filters.get("migration_ready") is not None:
            query = query.where(DiscoveryAsset.migration_ready == filters["migration_ready"])
        
        if filters.get("asset_type"):
            query = query.where(DiscoveryAsset.asset_type == filters["asset_type"])
        
        if filters.get("asset_subtype"):
            query = query.where(DiscoveryAsset.asset_subtype == filters["asset_subtype"])
        ...
        if filters.get("migration_priority"):
            query = query.where(DiscoveryAsset.migration_priority == filters["migration_priority"])
        
        if filters.get("min_confidence"):
            query = query.where(DiscoveryAsset.confidence_score >= filters["min_confidence"])
        
        if filters.get("max_confidence"):
            query = query.where(DiscoveryAsset.confidence_score <= filters["max_confidence"])
        
        if filters.get("discovered_in_phase"):
            query = query.where(DiscoveryAsset.discovered_in_phase == filters["discovered_in_phase"])
        
        if filters.get("discovery_method"):
            query = query.where(DiscoveryAsset.discovery_method == filters["discovery_method"])
