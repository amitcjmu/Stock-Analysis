...
class AssessmentRequest(BaseModel):
    ...
    assessment_type: str = Field("full", description="Type of assessment (full, partial, validation)")
    selected_asset_ids: Optional[List[str]] = Field(None, description="Specific asset IDs for partial assessment")
    include_recommendations: bool = Field(True, description="Include AI-generated recommendations")
    assessment_options: Optional[Dict[str, Any]] = Field(None, description="Additional assessment configuration")
...
class FlowCompletionRequest(BaseModel):
    ...
    completion_options: Optional[Dict[str, Any]] = Field(None, description="Completion configuration options")
    generate_assessment_package: bool = Field(True, description="Generate assessment package during completion")
...
class AssetReadinessResponse(BaseModel):
    ...
    readiness_factors: Dict[str, Any] = Field(..., description="Detailed readiness factor scores")
    blocking_issues: List[str] = Field(..., description="Issues preventing readiness")
    recommendations: List[str] = Field(..., description="Recommendations for improving readiness")
...
class RiskAssessmentResponse(BaseModel):
    ...
    risk_distribution: Dict[str, Dict[str, Any]] = Field(..., description="Risk level distribution")
...
class ComplexityAssessmentResponse(BaseModel):
    ...
    overall_complexity_score: float = Field(..., description="Overall complexity score (0-1)")
    overall_complexity_level: str = Field(..., description="Categorical complexity level")
    total_assets: int = Field(..., description="Total number of assets assessed")
    complexity_distribution: Dict[str, Dict[str, Any]] = Field(..., description="Complexity level distribution")
    effort_estimates: Dict[str, Any] = Field(..., description="Effort estimation details")
    complexity_factors: List[Dict[str, Any]] = Field(..., description="Identified complexity factors")
    recommendations: List[str] = Field(..., description="Complexity management recommendations")
...
class ValidationResult(BaseModel):
    ...
    validation_checks: Dict[str, Any] = Field(..., description="Detailed validation check results")
    ...
    recommendations: List[str] = Field(..., description="Recommendations for passing validation")
...
class AssessmentPackage(BaseModel):
    ...
    risk_assessment: RiskAssessmentResponse = Field(..., description="Migration risk assessment")
    complexity_assessment: ComplexityAssessmentResponse = Field(..., description="Migration complexity assessment")
    migration_waves: List[Dict[str, Any]] = Field(..., description="Recommended migration waves")
    recommendations: Dict[str, Any] = Field(..., description="Migration recommendations")
...
class FlowCompletionResponse(BaseModel):
    ...
    assessment_ready: bool = Field(..., description="Whether flow is ready for assessment")
    assessment_package: Optional[AssessmentPackage] = Field(None, description="Generated assessment package")
    next_steps: Dict[str, Any] = Field(..., description="Recommended next steps")
    completion_summary: Dict[str, Any] = Field(..., description="Completion summary and metrics")


class AssessmentResponse(BaseModel):
    """Generic response model for assessment operations."""
    
    operation_type: str = Field(..., description="Type of assessment operation performed")
    flow_id: str = Field(..., description="Discovery flow identifier")
...
class PartialAssessmentRequest(BaseModel):
    ...
    flow_id: str = Field(..., description="Discovery flow identifier")
    selected_asset_ids: List[str] = Field(..., description="Asset IDs to include in assessment")
    assessment_scope: str = Field("selected_assets", description="Scope of partial assessment")
    assessment_options: Optional[Dict[str, Any]] = Field(None, description="Assessment configuration")
...
class ValidationRequest(BaseModel):
    ...
    flow_id: str = Field(..., description="Discovery flow identifier")
    validation_type: str = Field("comprehensive", description="Type of validation to perform")
    validation_options: Optional[Dict[str, Any]] = Field(None, description="Validation configuration")
...
class CoordinationResponse(BaseModel):
    ...
    timestamp: datetime = Field(..., description="Operation timestamp")
    coordination_results: Dict[str, Any] = Field(..., description="Coordination operation results")
    orchestration_summary: Dict[str, Any] = Field(..., description="High-level orchestration summary")
    next_phase: Optional[Dict[str, Any]] = Field(None, description="Next phase information")
...
class BulkAssetUpdateRequest(BaseModel):
    ...
    update_data: Dict[str, Any] = Field(..., description="Data to update for all assets")
...
class FlowStatisticsResponse(BaseModel):
    ...
    migration_ready_percentage: float = Field(..., description="Percentage of migration-ready assets")
    validated_assets: int = Field(..., description="Number of validated assets")
    validated_percentage: float = Field(..., description="Percentage of validated assets")
    asset_types: Dict[str, int] = Field(..., description="Asset count by type")
    complexity_distribution: Dict[str, int] = Field(..., description="Asset count by complexity")
    flow_status: str = Field(..., description="Current flow status")
    progress_percentage: Optional[float] = Field(None, description="Flow progress percentage")
...
class ResetFlowRequest(BaseModel):
    ...
    reset_options: Optional[Dict[str, Any]] = Field(None, description="Reset configuration options")
