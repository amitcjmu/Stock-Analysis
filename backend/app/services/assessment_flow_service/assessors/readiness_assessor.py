...
class ReadinessAssessor:
    ...
    async def assess_flow_readiness(
        self, flow: DiscoveryFlow, assets: List[DiscoveryAsset]
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            overall_readiness = self._calculate_overall_readiness(
                flow_readiness, asset_readiness
            )

            # Identify readiness gaps
            readiness_gaps = self._identify_readiness_gaps(
                flow_readiness, asset_readiness
            )

            # Generate readiness recommendations
            recommendations = self._generate_readiness_recommendations(
                overall_readiness, readiness_gaps
            )
            ...
            logger.info(
                f"✅ Flow readiness assessment completed - Ready: {readiness_assessment['is_ready']}"
            )
    ...
    async def assess_asset_migration_readiness(
        self, asset: DiscoveryAsset
    ) -> Dict[str, Any]:
        """
    ...
    def _assess_flow_level_readiness(self, flow: DiscoveryFlow) -> Dict[str, Any]:
        ...
        total_score = sum(
            check["score"] * check["weight"] for check in readiness_checks.values()
        )
    ...
    async def _assess_assets_readiness(
        ...
        high_readiness = [
            a for a in asset_assessments if a["readiness_level"] == "high"
        ]
        medium_readiness = [
            a for a in asset_assessments if a["readiness_level"] == "medium"
        ]
        low_readiness = [a for a in asset_assessments if a["readiness_level"] == "low"]

        # Calculate average readiness score
        avg_readiness_score = (
            sum(a["readiness_score"] for a in asset_assessments) / total_assets
        )
        ...
        return {
            ...
            "common_blocking_issues": [
                ...
                for issue, count in sorted(
                    blocking_issue_counts.items(), key=lambda x: x[1], reverse=True
                )
            ],
        }
            ...
            if isinstance(requirements, list):
                if len(requirements) == 0:
                    ...
                    if "compliance_assessment_complete" in normalized_data:
                        score = (
                            1.0
                            if normalized_data["compliance_assessment_complete"]
                            else 0.3
                        )
                    else:
    ...
    def _calculate_asset_readiness_score(
        self, readiness_factors: Dict[str, Any]
    ) -> float:
        """Calculate weighted readiness score for an asset."""

        total_score = sum(
            factor["score"] * factor["weight"] for factor in readiness_factors.values()
        )
            ...
            if factor_data["score"] < 0.7:
                if factor_name == "data_completeness":
                    ...
                    recommendations.append(
                        "Complete asset validation and approval process"
                    )
                elif factor_name == "confidence_level":
                    recommendations.append(
                        "Perform additional discovery to improve confidence"
                    )
                elif factor_name == "migration_classification":
                    recommendations.append(
                        "Complete migration complexity and priority assessment"
                    )
                    ...
                    recommendations.append(
                        "Complete compliance and security assessment"
                    )
    ...
    def _calculate_phase_completion_score(self, flow: DiscoveryFlow) -> float:
        ...
        completed_count = sum(
            1 for phase in required_phases if getattr(flow, phase, False)
        )
        return completed_count / len(required_phases)

    def _calculate_overall_readiness(
        self, flow_readiness: Dict[str, Any], asset_readiness: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        is_ready = (
            flow_readiness["is_ready"]
            and asset_readiness["readiness_percentage"] >= 80
                ...
                gaps.append(
                    {
                        ...
                        "severity": (
                            "high" if issue_data["percentage"] > 40 else "medium"
                        ),
    ...
    def _generate_readiness_recommendations(
        self, overall_readiness: Dict[str, Any], readiness_gaps: List[Dict[str, Any]]
    ) -> List[str]:
        ...
        for gap in readiness_gaps:
            if gap["type"] == "flow":
                ...
                if "validation" in gap["category"].lower():
                    ...
                    recommendations.append(
                        "Improve asset discovery confidence through additional validation"
                    )

        # Overall readiness recommendations
        if overall_readiness["readiness_score"] < 0.6:
            recommendations.append(
                "Conduct comprehensive readiness review before proceeding"
            )
        elif overall_readiness["readiness_score"] < 0.75:
            recommendations.append(
                "Address critical readiness gaps before migration assessment"
            )
