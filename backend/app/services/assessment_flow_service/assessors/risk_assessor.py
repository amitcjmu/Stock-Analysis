...
class RiskAssessor:
    ...
    async def assess_migration_risks(self, assets: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            medium_risk_assets = [ar for ar in asset_risks if ar["risk_level"] == "medium"]
            ...
            mitigation_strategies = self._generate_mitigation_strategies(risk_factors, asset_risks)
            ...
            risk_assessment = {
                ...
                "risk_distribution": {
                    "high": {
                        ...
                        "percentage": round(len(high_risk_assets) / len(assets) * 100, 1)
                    },
                    "medium": {
                        "count": len(medium_risk_assets),
                        "percentage": round(len(medium_risk_assets) / len(assets) * 100, 1)
                    },
                    "low": {
                        "count": len(low_risk_assets),
                        "percentage": round(len(low_risk_assets) / len(assets) * 100, 1)
                    }
                },
            ...
            logger.info(f"✅ Risk assessment completed - Overall risk: {risk_assessment['overall_risk_level']}")
    ...
    async def assess_flow_risk(self, flow_id: str, assets: List[DiscoveryAsset]) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            risk_assessment["flow_risk_factors"] = self._assess_flow_specific_risks(assets)
    ...
    def _assess_asset_risk_detailed(self, asset: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Base risk factors
        complexity_risk = self._assess_complexity_risk(asset.get("migration_complexity", "medium"))
        confidence_risk = self._assess_confidence_risk(asset.get("confidence_score", 0.5))
        validation_risk = self._assess_validation_risk(asset.get("validation_status", "pending"))
    ...
    def _assess_complexity_risk(self, complexity: str) -> float:
        ...
        return complexity_risks.get(complexity.lower() if complexity else "unknown", 0.7)
    ...
    def _assess_validation_risk(self, validation_status: str) -> float:
        ...
        return validation_risks.get(validation_status.lower() if validation_status else "unknown", 0.7)
    ...
    def _identify_risk_factors(self, asset_risks: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        high_complexity_count = sum(1 for ar in asset_risks if ar["component_risks"]["complexity"] > 0.7)
        if high_complexity_count > 0:
            risk_factors.append({
                "factor": "High Complexity Assets",
                "count": high_complexity_count,
                "percentage": round(high_complexity_count / total_assets * 100, 1),
                "severity": "high" if high_complexity_count / total_assets > 0.3 else "medium",
                "description": f"{high_complexity_count} assets have high migration complexity"
            })
        
        # Low confidence assets
        low_confidence_count = sum(1 for ar in asset_risks if ar["component_risks"]["confidence"] > 0.6)
        if low_confidence_count > 0:
            risk_factors.append({
                "factor": "Low Confidence Data",
                "count": low_confidence_count,
                "percentage": round(low_confidence_count / total_assets * 100, 1),
                "severity": "medium" if low_confidence_count / total_assets > 0.2 else "low",
                "description": f"{low_confidence_count} assets have low confidence scores"
            })
        
        # Validation issues
        validation_issues_count = sum(1 for ar in asset_risks if ar["component_risks"]["validation"] > 0.5)
        if validation_issues_count > 0:
            risk_factors.append({
                "factor": "Validation Issues",
                "count": validation_issues_count,
                "percentage": round(validation_issues_count / total_assets * 100, 1),
                "severity": "high" if validation_issues_count / total_assets > 0.4 else "medium",
                "description": f"{validation_issues_count} assets have validation concerns"
            })
        
        return risk_factors
    
    def _generate_mitigation_strategies(self, risk_factors: List[Dict[str, Any]], asset_risks: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """Generate mitigation strategies based on identified risks."""
        
        strategies = []
        
        # Strategy for high complexity assets
        high_complexity_assets = [ar for ar in asset_risks if ar["component_risks"]["complexity"] > 0.7]
        ...
        low_confidence_assets = [ar for ar in asset_risks if ar["component_risks"]["confidence"] > 0.6]
        ...
        validation_issues_assets = [ar for ar in asset_risks if ar["component_risks"]["validation"] > 0.5]
    ...
    def _generate_risk_recommendations(self, overall_risk_score: float, risk_factors: List[Dict[str, Any]]) -> List[str]:
        ...
        for factor in risk_factors:
            if factor["factor"] == "High Complexity Assets" and factor["severity"] == "high":
                recommendations.append("Prioritize architecture reviews for complex systems")
            elif factor["factor"] == "Low Confidence Data" and factor["percentage"] > 30:
                recommendations.append("Invest in additional discovery tools and processes")
            elif factor["factor"] == "Validation Issues" and factor["severity"] == "high":
                recommendations.append("Establish clear validation criteria and approval workflows")
        
        return list(set(recommendations))  # Remove duplicates
    
    def _assess_flow_specific_risks(self, assets: List[DiscoveryAsset]) -> List[Dict[str, Any]]:
        ...
        unvalidated_count = sum(1 for asset in assets if asset.validation_status != "approved")
        if unvalidated_count > total_assets * 0.2:
            flow_risks.append({
                ...
                "severity": "high" if unvalidated_count > total_assets * 0.4 else "medium"
            })
    ...
    def _create_empty_risk_assessment(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "recommendations": ["Complete asset discovery before proceeding with risk assessment"]
        }