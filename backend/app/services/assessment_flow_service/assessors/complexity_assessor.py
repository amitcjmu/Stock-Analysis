...
class ComplexityAssessor:
    ...
    async def assess_migration_complexity(self, assets: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            overall_complexity_score = sum(complexity_scores) / len(complexity_scores) if complexity_scores else 0.0
            
            # Categorize complexities
            high_complexity_assets = [ac for ac in asset_complexities if ac["complexity_level"] == "high"]
            medium_complexity_assets = [ac for ac in asset_complexities if ac["complexity_level"] == "medium"]
            low_complexity_assets = [ac for ac in asset_complexities if ac["complexity_level"] == "low"]
            ...
            recommendations = self._generate_complexity_recommendations(overall_complexity_score, complexity_factors)
            
            complexity_assessment = {
                "overall_complexity_score": round(overall_complexity_score, 2),
                "overall_complexity_level": self._determine_complexity_level(overall_complexity_score),
                "total_assets": len(assets),
                "complexity_distribution": {
                    "high": {
                        "count": len(high_complexity_assets),
                        "percentage": round(len(high_complexity_assets) / len(assets) * 100, 1)
                    },
                    "medium": {
                        "count": len(medium_complexity_assets),
                        "percentage": round(len(medium_complexity_assets) / len(assets) * 100, 1)
                    },
                    "low": {
                        "count": len(low_complexity_assets),
                        "percentage": round(len(low_complexity_assets) / len(assets) * 100, 1)
                    }
                },
            ...
            logger.info(f"✅ Complexity assessment completed - Overall complexity: {complexity_assessment['overall_complexity_level']}")
    ...
    async def assess_flow_complexity(self, flow_id: str, assets: List[DiscoveryAsset]) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            complexity_assessment["flow_complexity_factors"] = self._assess_flow_specific_complexity(assets)
    ...
    def _assess_asset_complexity_detailed(self, asset: Dict[str, Any]) -> Dict[str, Any]:
        """Perform detailed complexity assessment for a single asset."""
        
        # Base complexity factors
        type_complexity = self._assess_type_complexity(asset.get("asset_type"), asset.get("asset_subtype"))
        data_complexity = self._assess_data_complexity(asset.get("normalized_data", {}))
        dependency_complexity = self._assess_dependency_complexity(asset.get("normalized_data", {}))
        configuration_complexity = self._assess_configuration_complexity(asset.get("raw_data", {}))
        modernization_complexity = self._assess_modernization_complexity(asset.get("asset_type"))
        ...
        if type_complexity > 0.7:
            complexity_factors.append("Complex asset type requiring specialized migration approach")
        ...
        if dependency_complexity > 0.7:
            complexity_factors.append("High dependency complexity with multiple interconnections")
        if configuration_complexity > 0.6:
            complexity_factors.append("Complex configuration requiring manual intervention")
        if modernization_complexity > 0.7:
            complexity_factors.append("High modernization potential increasing migration options")
        
        # Estimate effort
        effort_weeks = self._estimate_asset_effort(complexity_score, asset.get("asset_type"))
    ...
    def _assess_type_complexity(self, asset_type: Optional[str], asset_subtype: Optional[str] = None) -> float:
        ...
        base_complexity = type_complexities.get(asset_type.lower() if asset_type else "unknown", 0.8)
    ...
    def _assess_dependency_complexity(self, normalized_data: Dict[str, Any]) -> float:
        ...
        if "dependency_types" in normalized_data:
            ...
            critical_count = sum(1 for dep_type in dependency_types if dep_type in critical_types)
    ...
    def _assess_configuration_complexity(self, raw_data: Dict[str, Any]) -> float:
        ...
        security_indicators = ["ssl", "tls", "certificate", "encryption", "authentication"]
    ...
    def _estimate_asset_effort(self, complexity_score: float, asset_type: Optional[str]) -> int:
        ...
        base_effort = base_efforts.get(asset_type.lower() if asset_type else "unknown", 4)
    ...
    def _calculate_effort_estimates(self, asset_complexities: List[Dict[str, Any]]) -> Dict[str, Any]:
        """Calculate overall effort estimates."""
        
        total_effort_weeks = sum(ac["estimated_effort_weeks"] for ac in asset_complexities)
        
        # Categorize by effort
        low_effort = [ac for ac in asset_complexities if ac["estimated_effort_weeks"] <= 2]
        medium_effort = [ac for ac in asset_complexities if 3 <= ac["estimated_effort_weeks"] <= 6]
        high_effort = [ac for ac in asset_complexities if ac["estimated_effort_weeks"] > 6]
        ...
        return {
            ...
            "parallel_execution_potential": self._assess_parallel_potential(asset_complexities),
            "critical_path_weeks": self._estimate_critical_path(asset_complexities)
        }
    
    def _identify_complexity_factors(self, asset_complexities: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        high_complexity_count = sum(1 for ac in asset_complexities if ac["complexity_level"] == "high")
        if high_complexity_count > 0:
            complexity_factors.append({
                ...
                "impact": "high" if high_complexity_count / total_assets > 0.3 else "medium",
                "description": f"{high_complexity_count} assets require specialized migration approaches"
            })
        
        # Data complexity issues
        data_complex_count = sum(1 for ac in asset_complexities if ac["component_complexities"]["data"] > 0.6)
        ...
        dependency_complex_count = sum(1 for ac in asset_complexities if ac["component_complexities"]["dependency"] > 0.7)
        if dependency_complex_count > 0:
            complexity_factors.append({
                "factor": "Dependency Complexity",
                "count": dependency_complex_count,
                "percentage": round(dependency_complex_count / total_assets * 100, 1),
                "impact": "high",
    ...
    def _generate_complexity_recommendations(self, overall_complexity_score: float, complexity_factors: List[Dict[str, Any]]) -> List[str]:
        ...
        for factor in complexity_factors:
            if factor["factor"] == "High Complexity Assets" and factor["impact"] == "high":
                recommendations.append("Prioritize architecture reviews for complex systems")
            elif factor["factor"] == "Data Complexity" and factor["percentage"] > 30:
                recommendations.append("Invest in data migration tools and validation processes")
            elif factor["factor"] == "Dependency Complexity" and factor["impact"] == "high":
                recommendations.append("Create detailed dependency maps and migration sequencing")
        
        return list(set(recommendations))  # Remove duplicates
    
    def _assess_parallel_potential(self, asset_complexities: List[Dict[str, Any]]) -> float:
        ...
        low_dependency_count = sum(1 for ac in asset_complexities 
    ...
    def _estimate_critical_path(self, asset_complexities: List[Dict[str, Any]]) -> int:
        ...
        sorted_assets = sorted(asset_complexities, 
        ...
        critical_path_weeks = sum(ac["estimated_effort_weeks"] for ac in critical_path_assets)
        
        return critical_path_weeks
    
    def _assess_flow_specific_complexity(self, assets: List[DiscoveryAsset]) -> List[Dict[str, Any]]:
    ...
    def _create_empty_complexity_assessment(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "complexity_factors": [],
            "recommendations": ["Complete asset discovery before proceeding with complexity assessment"],
