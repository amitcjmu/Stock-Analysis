...
class AssessmentManager:
    ...
    async def validate_flow_completion_readiness(
        ...
        try:
            ...
            readiness_result = await self.readiness_assessor.assess_flow_readiness(flow, assets)
            
            logger.info(f"✅ Flow readiness validation completed for {discovery_flow_id}")
    ...
    async def get_assessment_ready_assets(
        ...
        try:
            ...
            result = {
                ...
                "readiness_percentage": (len(assessment_ready) / len(assets) * 100) if assets else 0
            }
    ...
    async def generate_assessment_package(
        ...
        try:
            ...
            risk_assessment = await self.risk_assessor.assess_migration_risks(ready_assets)
            
            # Generate complexity assessment
            complexity_assessment = await self.complexity_assessor.assess_migration_complexity(ready_assets)
            ...
            assessment_package = {
                ...
                "metadata": {
                    ...
                    "discovery_completed_at": flow.completed_at.isoformat() if flow.completed_at else None
                },
                ...
                "summary": {
                    ...
                    "overall_complexity_score": complexity_assessment.get("overall_complexity_score", 0.0),
                    "estimated_effort": complexity_assessment.get("estimated_effort", "Unknown")
                }
            }
            
            # Add recommendations if requested
            if include_recommendations:
                assessment_package["recommendations"] = await self._generate_recommendations(
    ...
    async def complete_discovery_flow(
        ...
        try:
            ...
            # Validate readiness
            readiness_result = await self.validate_flow_completion_readiness(discovery_flow_id)
            if not readiness_result["is_ready"]:
                raise ValueError(f"Flow not ready for completion: {readiness_result['errors']}")
            
            # Generate assessment package
            assessment_package = await self.generate_assessment_package(discovery_flow_id)
            ...
            completion_result = {
                ...
                "next_steps": {
                    ...
                    "ready_assets": assessment_package["asset_inventory"]["assessment_ready"]["count"],
                    "estimated_duration": self._estimate_assessment_duration(assessment_package),
                    "recommended_actions": [
    ...
    async def _assess_asset_readiness(self, asset: DiscoveryAsset) -> Dict[str, Any]:
        ...
        is_ready = (
            has_required_data and
            (asset.confidence_score or 0) >= confidence_threshold and
            is_validated
        )
        ...
        return {
            ...
            "readiness_factors": {
                ...
                "meets_confidence_threshold": (asset.confidence_score or 0) >= confidence_threshold,
    ...
    def _estimate_assessment_duration(self, assessment_package: Dict[str, Any]) -> str:
        ...
        ready_assets = assessment_package["asset_inventory"]["assessment_ready"]["count"]
        complexity_score = assessment_package["complexity_assessment"].get("overall_complexity_score", 0.5)
        
        # Base duration calculation
        base_days = max(5, ready_assets * 0.5)  # Minimum 5 days, 0.5 days per asset
        
        # Adjust for complexity
        complexity_multiplier = 1 + (complexity_score * 0.5)  # Up to 50% increase for high complexity
