...
class FlowCoordinator:
    ...
    async def orchestrate_flow_completion(
        ...
        try:
            ...
            readiness_result = (
                await self.assessment_manager.validate_flow_completion_readiness(
                    discovery_flow_id
                )
            )
            ...
            assets_result = await self.assessment_manager.get_assessment_ready_assets(
                discovery_flow_id
            )

            # Step 3: Generate comprehensive assessment package
            include_recommendations = completion_options.get(
                "include_recommendations", True
            )
            assessment_package = (
                await self.assessment_manager.generate_assessment_package(
            ...
            orchestration_result = {
                ...
                "next_phase": {
                    ...
                    "estimated_duration": completion_result["next_steps"][
                        "estimated_duration"
                    ],
                    "ready_assets": completion_result["next_steps"]["ready_assets"],
                },
                "workflow_recommendations": completion_result["next_steps"][
                    "recommended_actions"
                ],
            }

            logger.info(
                f"✅ Flow completion orchestration successful: {discovery_flow_id}"
            )
    ...
    async def coordinate_partial_assessment(
        ...
        try:
            logger.info(
                f"🎯 Coordinating partial assessment for {len(selected_asset_ids)} assets"
            )
            ...
            for asset_id in selected_asset_ids:
                ...
                if asset and asset.discovery_flow_id == flow.id:
                    ...
                    logger.warning(
                        f"Asset {asset_id} not found or doesn't belong to flow"
                    )
            ...
            logger.info(
                f"✅ Partial assessment coordination completed for {len(selected_assets)} assets"
            )
    ...
    async def coordinate_assessment_validation(
        ...
        try:
            ...
            validation_results = {
                "flow_validation": await self.assessment_manager.validate_flow_completion_readiness(
                    discovery_flow_id
                ),
                "asset_statistics": await self.repository.get_flow_statistics(
                    str(discovery_flow_id)
                ),
                "readiness_assessment": await self.assessment_manager.readiness_assessor.assess_flow_readiness(
                    flow, assets
                ),
                "risk_assessment": await self.assessment_manager.risk_assessor.assess_flow_risk(
                    str(discovery_flow_id), assets
                ),
                "complexity_assessment": await self.assessment_manager.complexity_assessor.assess_flow_complexity(
                    str(discovery_flow_id), assets
                ),
            }

            # Generate validation summary
            overall_validation = self._generate_overall_validation_summary(
                validation_results
            )
            ...
            logger.info(
                f"✅ Assessment validation coordination completed: {discovery_flow_id}"
            )
            return coordination_result
    ...
    async def coordinate_flow_reset(
        ...
        try:
            ...
            reset_successful = await self.repository.delete_assessment_data(
                str(discovery_flow_id)
            )
            ...
            updated_statistics = await self.repository.get_flow_statistics(
                str(discovery_flow_id)
            )
    ...
    async def _generate_partial_assessment_package(
        ...
        risk_assessment = (
            await self.assessment_manager.risk_assessor.assess_migration_risks(
                asset_dicts
            )
        )

        # Generate complexity assessment for selected assets
        complexity_assessment = await self.assessment_manager.complexity_assessor.assess_migration_complexity(
            asset_dicts
        )
    ...
    def _generate_overall_validation_summary(
        ...
        overall_score = (
            ...
            * 0.2  # Lower risk = higher readiness
            + (1 - complexity_assessment["overall_complexity_score"])
            * 0.1  # Lower complexity = higher readiness
        )

        # Determine overall status
        is_ready = (
            flow_validation["is_ready"]
            and readiness_assessment["is_ready"]
            and risk_assessment["overall_risk_level"] in ["low", "medium"]
            and complexity_assessment["overall_complexity_level"] in ["low", "medium"]
        )
    ...
    def _identify_critical_validation_issues(
        self, validation_results: Dict[str, Any]
    ) -> List[str]:
        ...
        # Complexity issues
        if (
            validation_results["complexity_assessment"]["overall_complexity_level"]
            == "high"
        ):
    ...
    def _generate_validation_recommendations(
        self, overall_validation: Dict[str, Any], validation_results: Dict[str, Any]
    ) -> List[str]:
        ...
        if overall_validation["is_ready"]:
            ...
            recommendations.append(
                "Address critical validation issues before proceeding"
            )

            # Add specific recommendations from each assessment
            recommendations.extend(
                validation_results["readiness_assessment"].get("recommendations", [])
            )
            recommendations.extend(
                validation_results["risk_assessment"].get("recommendations", [])
            )
            recommendations.extend(
                validation_results["complexity_assessment"].get("recommendations", [])
            )

        return list(set(recommendations))  # Remove duplicates

    def _generate_validation_next_steps(
