...
class FieldMappingService(ServiceBase):
    ...
    BASE_MAPPINGS = {
        "hostname": ["host_name", "server_name", "machine_name", "computer_name", "fqdn"],
        ...
        "business_owner": ["owner", "business_contact", "responsible_party", "app_owner"],
    ...
    REQUIRED_FIELDS = {
        "server": ["hostname", "asset_name", "asset_type", "environment", "operating_system"],
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "cached_learned_mappings": len(self._learned_mappings_cache) if self._learned_mappings_cache else 0,
            }
    ...
    async def analyze_columns(
        ...
        """
        try:
            self.logger.debug(f"Analyzing {len(columns)} columns for {asset_type} assets")
            ...
            missing_required = [
                ...
                if field not in mapped_field_values
            ]
        ...
        try:
            ...
            if existing_mapping:
                ...
                if confidence > existing_mapping.confidence_score:
                    ...
                    if context:
                        existing_mapping.transformation_rules = existing_mapping.transformation_rules or {}
                    ...
                    # Update cache
                    self._update_cache(normalized_source, normalized_target, confidence, source)
                ...
                new_mapping = FieldMapping(
                    ...
                    transformation_rules={"context": context} if context else {},  # Use transformation_rules instead of metadata
    ...
    async def learn_negative_mapping(
        ...
        try:
            ...
            negative_mapping = FieldMapping(
                ...
                transformation_rules={"rejection_reason": reason} if reason else {},  # Use transformation_rules instead of metadata
    ...
    async def validate_mapping(
        ...
        try:
            ...
            if normalized_target in self.BASE_MAPPINGS:
                variations = [self._normalize_field_name(v) for v in self.BASE_MAPPINGS[normalized_target]]
            ...
            existing = await self._get_existing_mapping(normalized_source, normalized_target)
        ...
        try:
            query = select(FieldMapping).where(
                and_(
                    FieldMapping.client_account_id == self.context.client_account_id,
                    FieldMapping.engagement_id == self.context.engagement_id,
                    FieldMapping.status == "approved",  # Use status instead of is_active
            ...
            for mapping in mappings:
                ...
                rule = MappingRule(
                    ...
                    source=mapping.suggested_by or "learned",  # Use suggested_by instead of source
                    context=mapping.transformation_rules.get("context") if mapping.transformation_rules else None,  # Use transformation_rules instead of metadata
    ...
    async def _get_existing_mapping(
        ...
        query = select(FieldMapping).where(
            and_(
                ...
                FieldMapping.status.in_(["approved", "suggested"])  # Use status instead of is_active
            )
        )
