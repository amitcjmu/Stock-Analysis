...
class AccessValidationHandler(BaseRBACHandler):
    ...
    async def validate_user_access(self, user_id: str, resource_type: str, 
        ...
        if not self.is_available:
            return {"status": "error", "message": "RBAC models not available", "has_access": False}
        ...
        try:
            ...
            if not user_profile:
                return {"status": "error", "message": "User not found or inactive", "has_access": False}
            
            # Route to specific validation based on resource type
            if resource_type == "client":
                has_access = await self._validate_client_access(user_id, resource_id, action)
            elif resource_type == "engagement":
                has_access = await self._validate_engagement_access(user_id, resource_id, action)
            ...
            return {"status": "error", "message": f"Access validation failed: {str(e)}", "has_access": False}
    ...
    async def _validate_engagement_access(self, user_id: str, engagement_id: str, action: str) -> bool:
        ...
        try:
            ...
            if not engagement_access:
                ...
                if engagement:
                    return await self._validate_client_access(user_id, str(engagement.client_account_id), action)
                return False
