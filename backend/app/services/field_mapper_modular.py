...
class FieldMapperService:
    ...
    def learn_field_mapping(
        ...
        if not self.is_available():
            return {
                "status": "unavailable",
                "message": "Field mapper service not available",
            }

        try:
            # Delegate to mapping engine handler
            return self.mapping_engine.learn_field_mapping(
                canonical_field, field_variations, source, context
            )
        except Exception as e:
            logger.error(f"Error in learn_field_mapping: {e}")
            return {"status": "error", "message": str(e)}

    def learn_field_mapping_rejection(
        self, source_field: str, target_field: str, rejection_reason: str = ""
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if hasattr(self.mapping_engine, "learn_negative_mapping"):
                ...
                logger.info(
                    f"📚 Learning negative mapping: {source_field} should NOT map to {target_field}"
                )
                return {
                    "status": "learned",
    ...
    def find_matching_fields(
        self, available_columns: List[str], required_field: str
    ) -> List[str]:
        """Find matching columns for a required field."""
        return self.mapping_engine.find_matching_fields(
            available_columns, required_field
        )
    ...
    def identify_missing_fields(
        ...
        return self.validation_handler.identify_missing_fields(
            available_columns, asset_type, mapped_fields
        )
    ...
    def validate_field_format(
        ...
        return self.validation_handler.validate_field_format(
            field_name, value, canonical_field
        )
    ...
    def agent_learn_field_mapping(
        ...
        return self.agent_interface.agent_learn_field_mapping(
    ...
    def learn_content_patterns(
        self, field_name: str, canonical_field: str, sample_values: List[str]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"Learned content patterns for {field_name} -> {canonical_field}"
            )
    ...
    def _analyze_content_patterns(
        ...
        try:
            patterns = {"data_types": [], "value_patterns": [], "semantic_hints": []}
            ...
            for value in sample_values[:10]:  # Analyze first 10 values
                ...
                if value_str.replace(".", "").replace("-", "").isdigit():
                    ...
                    term in value_str for term in ["prod", "dev", "test", "staging"]
                ):
            ...
            # Summarize patterns
            patterns["dominant_type"] = (
                max(set(patterns["data_types"]), key=patterns["data_types"].count)
                if patterns["data_types"]
                else "unknown"
            )
            patterns["semantic_category"] = (
                max(
                    set(patterns["semantic_hints"]),
                    key=patterns["semantic_hints"].count,
                )
                if patterns["semantic_hints"]
                else "general"
            )
    ...
    async def analyze_field_mappings(
        self, data_source: Dict[str, Any], page_context: str = "attribute-mapping"
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if "columns" in data_source:
                ...
                first_row = (
                    data_source["file_data"][0]
                    if isinstance(data_source["file_data"], list)
                    else data_source["file_data"]
                )
            ...
            enhanced_analysis = self.mapping_engine.analyze_columns(
                columns, "server", sample_data_for_analysis
            )
    ...
    async def process_field_mapping_batch(
        ...
        try:
            ...
            try:
                ...
                async with AsyncSessionLocal() as db:
                    ...
                    for asset in asset_data:
                        try:
                            ...
                            completeness = self._calculate_mapping_completeness(
                                asset, mapping_analysis
                            )
                            mapping_results["mapping_completeness"][
                            ...
                            if (
                                completeness >= 80.0
                            ):  # 80% threshold for mapping completion
                                asset_id = asset.get("id")
                                if asset_id:
                                    workflow_update = await workflow_service.advance_asset_workflow(
                                        asset_id,
                                        "mapping",
                                        f"Field mapping completed ({completeness:.1f}% complete)",
                                    )
                            ...
                            logger.error(
                                f"Error processing asset {asset.get('id', 'unknown')}: {e}"
                            )
                ...
                logger.warning(
                    "Workflow service not available, proceeding without workflow updates"
                )
    ...
    def _calculate_mapping_completeness(
        self, asset: Dict[str, Any], mapping_analysis: Dict[str, Any]
    ) -> float:
        ...
        try:
            ...
            for field in critical_fields:
                ...
                if (
                    value
                    and str(value).strip()
                    and str(value).lower() not in ["unknown", "null", "none", ""]
                ):
                    populated_count += 1
            ...
            bonus_completeness = min(
                20, (total_fields - len(critical_fields)) * 2
            )  # Max 20% bonus
    ...
    async def update_field_mapping_from_user_input(
        self,
        ...
        try:
            ...
            for source_field, target_field in mapping_updates.items():
                learn_result = self.learn_field_mapping(
                    target_field, [source_field], "user_mapping"
                )
            ...
            try:
                ...
                async with AsyncSessionLocal() as db:
                    ...
                    if asset:
                        ...
                        await asset_repo.update(
                            asset_id, completeness_score=completeness
                        )
                        ...
                        if completeness >= 80.0:
                            workflow_result = (
                                await workflow_service.update_asset_workflow_status(
                ...
                logger.warning(
                    "Workflow service not available for field mapping updates"
                )
                return {
                    "success": True,
                    "mappings_learned": len(learning_results),
    ...
    def assess_mapping_readiness(self, assets: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"readiness": "error", "error": str(e)}
    ...
    def _generate_mapping_recommendations(
        ...
        if average_completeness < 80.0:
            recommendations.append(
                f"Increase overall mapping completeness from {average_completeness:.1f}% to 80%"
            )
        ...
        if field_coverage.get("asset_type", 0) < 90.0:
            recommendations.insert(
                0,
                "Prioritize asset_type classification - critical for migration planning",
            )

        if field_coverage.get("environment", 0) < 80.0:
            recommendations.insert(
                0 if not recommendations else 1,
                "Complete environment classification - required for wave planning",
            )

        if not recommendations:
            recommendations.append(
                "Mapping is complete! Ready to proceed to cleanup phase."
            )
    ...
    def _calculate_match_confidence(
        self, matches: List[str], target_field: str
    ) -> float:
        ...
        exact_matches = sum(
            1 for match in matches if match.lower() == target_field.lower()
        )
