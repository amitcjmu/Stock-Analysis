...
class FlowStatusSyncService:
    ...
    async def update_flow_status_atomically(
        ...
        """
        logger.info(f"🔄 [FlowStatusSync] Atomic update: {flow_id} -> child:{child_status}, master:{master_status}")
    ...
    async def publish_status_event(self, event: FlowStatusEvent):
        ...
        """
        logger.info(f"📢 [FlowStatusSync] Publishing event: {event.flow_id} -> {event.child_status}")
    ...
    async def _process_status_event(self, event: FlowStatusEvent):
        ...
        try:
            ...
            logger.error(f"❌ [FlowStatusSync] Event processing failed: {event.flow_id} - {e}")
    ...
    async def start_flow(self, flow_id: str, flow_type: str) -> Dict[str, Any]:
        ...
        return await self.update_flow_status_atomically(
            ...
            metadata={"operation": "start", "started_at": datetime.utcnow().isoformat()}
        )
    
    async def pause_flow(self, flow_id: str, flow_type: str, reason: str = "user_requested") -> Dict[str, Any]:
        ...
        return await self.update_flow_status_atomically(
            ...
            metadata={"operation": "pause", "reason": reason, "paused_at": datetime.utcnow().isoformat()}
        )
    ...
    async def resume_flow(self, flow_id: str, flow_type: str) -> Dict[str, Any]:
        ...
        return await self.update_flow_status_atomically(
            ...
            child_status="active",
            master_status="running",
            metadata={"operation": "resume", "resumed_at": datetime.utcnow().isoformat()}
        )
    
    async def update_operational_status(self, flow_id: str, flow_type: str, status: str, metadata: Optional[Dict[str, Any]] = None):
    ...
    async def validate_flow_consistency(self, flow_id: str, flow_type: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if master_flow.flow_status == "deleted":
                return {"consistent": True, "note": "Master flow deleted - child flow ignored"}
            
            # Rule 2: If master is completed, child should be completed
            if master_flow.flow_status == "completed" and child_flow.status != "completed":
            ...
            if master_flow.flow_status == "running" and child_flow.status in ["failed", "deleted"]:
            ...
            logger.error(f"❌ [FlowStatusSync] Consistency validation failed: {flow_id} - {e}")
            return {"consistent": False, "error": str(e)}
    
    async def recover_from_partial_update(self, flow_id: str, flow_type: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if child_flow.status in ["active", "paused"]:
                ...
                await self.master_repo.update_flow_status(
                    flow_id=flow_id,
                    status=master_status,
                    metadata={"operation": "recovery", "recovered_at": datetime.utcnow().isoformat()}
                )
...
async def get_flow_status_sync_service(db: AsyncSession, context: RequestContext) -> FlowStatusSyncService:
