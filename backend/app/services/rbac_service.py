...
class RBACService:
    ...
    def __init__(self, db: AsyncSession):
        ...
        self.is_available = (
            self.user_management.is_available
            and self.access_validation.is_available
            and self.admin_operations.is_available
        )

        if not self.is_available:
            logger.warning(
                "Modular RBAC Service initialized with limited functionality"
            )
    ...
    async def approve_user(
        ...
        return await self.user_management.approve_user(
    ...
    async def reject_user(
        ...
        return await self.user_management.reject_user(
    ...
    async def deactivate_user(
        ...
        return await self.user_management.deactivate_user(
    ...
    async def validate_user_access(
        ...
        return await self.access_validation.validate_user_access(
    ...
    async def grant_client_access(
        ...
        return await self.access_validation.grant_client_access(
            user_id, client_id, access_data, granted_by
        )
    ...
    async def bulk_user_operation(
        ...
        return await self.admin_operations.bulk_user_operation(
            operation, user_ids, admin_user_id, operation_data
        )
    ...
    async def validate_client_access(
        self, user_id: str, client_id: str, action: str = "read"
    ) -> bool:
        """Quick check if user has client access."""
        result = await self.access_validation._validate_client_access(
            user_id, client_id, action
        )
        return result

    async def validate_engagement_access(
        self, user_id: str, engagement_id: str, action: str = "read"
    ) -> bool:
        """Quick check if user has engagement access."""
        result = await self.access_validation._validate_engagement_access(
            user_id, engagement_id, action
        )
        return result
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                handler_health["user_management"] = (
                    "healthy" if self.user_management.is_available else "unavailable"
                )
            except Exception as e:
            ...
            try:
                handler_health["access_validation"] = (
                    "healthy" if self.access_validation.is_available else "unavailable"
                )
            except Exception as e:
            ...
            try:
                handler_health["admin_operations"] = (
                    "healthy" if self.admin_operations.is_available else "unavailable"
                )
            except Exception as e:
            ...
            all_healthy = status["database_health"] == "healthy" and all(
                health == "healthy" for health in handler_health.values()
            )

            status["overall_health"] = "healthy" if all_healthy else "degraded"

            return {"status": "success", "service_health": status}
