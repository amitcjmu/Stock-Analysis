...
class AgentMemory:
    ...
    def add_experience(self, experience_type: str, data: Dict[str, Any]):
        ...
        if len(self.experiences[experience_type]) > max_experiences:
            ...
            self.experiences[experience_type] = self.experiences[experience_type][-max_experiences:]
    ...
    def _is_relevant(self, experience: Dict, filename: str) -> bool:
        ...
        if exp_filename and filename:
            ...
            if any(pattern in exp_base and pattern in curr_base for pattern in cmdb_patterns):
                return True
    ...
    def update_learning_metrics(self, metric: str, value: Any):
        ...
        if metric in self.learning_metrics:
            if isinstance(self.learning_metrics[metric], (int, float)) and isinstance(value, (int, float)):
    ...
    def _get_last_activity(self) -> Optional[str]:
        ...
        for exp_list in self.experiences.values():
            if exp_list:
                ...
                if exp_timestamp and (not last_timestamp or exp_timestamp > last_timestamp):
    ...
    def load_memory(self):
        ...
        try:
            if self.memory_file.exists():
                ...
                logger.info(f"Loaded {sum(len(exp_list) for exp_list in self.experiences.values())} experiences")
            else:
    ...
    def cleanup_old_experiences(self, days_to_keep: int = 90):
        ...
        for exp_type, exp_list in self.experiences.items():
            ...
            self.experiences[exp_type] = [
                ...
                if exp.get('timestamp', '') > cutoff_iso
            ]
            
            cleaned_count += original_count - len(self.experiences[exp_type])
        
        if cleaned_count > 0:
            logger.info(f"Cleaned up {cleaned_count} old experiences (older than {days_to_keep} days)")
            self.save_memory()
