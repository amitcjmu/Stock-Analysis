...
class AgentLifecycleManager:
    ...
    def record_task_completion(
        self, agent_name: str, crew_name: str, task_info: Dict[str, Any]
    ) -> None:
        ...
        if agent_id:
            ...
            logger.info(
                f"Recorded task completion for {agent_name} in {crew_name}: {duration:.2f}s, success={success}"
            )
        else:
    ...
    def mark_agent_busy(self, agent_id: str, task_info: Dict[str, Any] = None) -> None:
        ...
        if task_info:
            logger.info(
                f"Agent {agent_id} marked as busy: {task_info.get('task_name', 'unknown task')}"
            )
    ...
    def mark_agent_error(
        self, agent_id: str, error_details: Dict[str, Any] = None
    ) -> None:
        """Mark agent as in error state"""
        self.registry_core.update_agent_status(agent_id, AgentStatus.ERROR)
        if error_details:
            logger.error(
                f"Agent {agent_id} marked as error: {error_details.get('error_message', 'unknown error')}"
            )
    ...
    def get_agent_health_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "health_status": self._calculate_overall_health(
                avg_success_rate, status_counts
            ),
        }

    def _calculate_overall_health(
        self, avg_success_rate: float, status_counts: Dict[str, int]
    ) -> str:
        ...
        if error_agents > total_agents * 0.1:  # More than 10% in error
            ...
            status_counts.get("active", 0) < total_agents * 0.5
        ):  # Less than 50% active
            return "limited"
        else:
            return "healthy"
