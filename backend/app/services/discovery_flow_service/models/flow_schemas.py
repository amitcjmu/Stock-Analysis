...
class FlowCreationRequest(BaseModel):
    ...
    data_import_id: Optional[str] = Field(
        None, description="Data import identifier for linking"
    )
...
class PhaseCompletionRequest(BaseModel):
    ...
    crew_status: Optional[Dict[str, Any]] = Field(
        None, description="CrewAI crew status"
    )
    agent_insights: Optional[List[Dict[str, Any]]] = Field(
        None, description="Agent insights from phase"
    )
...
class AssetValidationRequest(BaseModel):
    ...
    validation_results: Optional[Dict[str, Any]] = Field(
        None, description="Validation results data"
    )
...
class BulkAssetValidationRequest(BaseModel):
    ...
    validation_results: Optional[Dict[str, Any]] = Field(
        None, description="Common validation results"
    )
...
class FlowSummaryResponse(BaseModel):
    ...
    crewai_integration: Dict[str, Any] = Field(
...
class MultiFlowSummaryResponse(BaseModel):
    ...
    flow_status_distribution: Dict[str, int] = Field(
        ..., description="Flow status counts"
    )
    phase_completion: Dict[str, Any] = Field(
        ..., description="Phase completion statistics"
    )
    quality_metrics: Dict[str, Any] = Field(
        ..., description="Quality metrics across flows"
    )
...
class FlowHealthReportResponse(BaseModel):
    ...
    health_indicators: List[Dict[str, Any]] = Field(
        ..., description="Health indicators found"
    )
...
class AssetFilterCriteria(BaseModel):
    ...
    max_quality_score: Optional[float] = Field(
        None, description="Maximum quality score"
    )
    min_confidence_score: Optional[float] = Field(
        None, description="Minimum confidence score"
    )
    max_confidence_score: Optional[float] = Field(
...
class CrewAIStateSync(BaseModel):
    ...
    force_sync: bool = Field(
        False, description="Force synchronization even if conflicts exist"
    )
...
class ValidationReport(BaseModel):
    ...
    last_database_update: Optional[str] = Field(
...
class AssetStatistics(BaseModel):
    ...
    validation_status_distribution: Dict[str, int] = Field(
        ..., description="Asset count by validation status"
    )
...
class FlowListResponse(BaseModel):
    ...
    summary_statistics: Dict[str, Any] = Field(
        ..., description="Summary statistics across flows"
    )
...
class FlowPhaseData(BaseModel):
    ...
    phase_data: Dict[str, Any] = Field(..., description="Phase-specific data")
    crew_status: Optional[Dict[str, Any]] = Field(
        None, description="Crew status for phase"
    )
    agent_insights: Optional[List[Dict[str, Any]]] = Field(
        None, description="Agent insights from phase"
    )
