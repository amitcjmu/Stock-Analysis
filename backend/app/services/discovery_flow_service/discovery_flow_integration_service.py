...
class DiscoveryFlowIntegrationService:
    ...
    async def sync_crewai_state(
        ...
        return await self.crewai_integration.sync_crewai_state(
            flow_id=flow_id, crewai_state=crewai_state, phase=phase
        )
    ...
    async def validate_crewai_state_sync(
        ...
        return await self.crewai_integration.validate_crewai_state_sync(
            flow_id=flow_id, crewai_state=crewai_state
        )

    async def synchronize_with_crewai_flow(
        self, flow_id: str, crewai_flow_state: Dict[str, Any], force_sync: bool = False
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            validation_result = await self.validate_crewai_state_sync(
                flow_id, crewai_flow_state
            )
            ...
            await self.sync_crewai_state(
                flow_id=flow_id, crewai_state=crewai_flow_state
            )
            actions_taken.append("state_synchronized")

            # Re-validate after sync
            post_sync_validation = await self.validate_crewai_state_sync(
                flow_id, crewai_flow_state
            )
            ...
            sync_result = {
                ...
                "improvement": {
                    "discrepancies_resolved": len(validation_result["discrepancies"])
                    - len(post_sync_validation["discrepancies"]),
                    "sync_quality_improved": post_sync_validation["sync_quality"]
                    != validation_result["sync_quality"],
                },
            }
    ...
    async def get_crewai_integration_status(self, flow_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            has_crew_status = bool(
                flow.crewai_state_data and flow.crewai_state_data.get("crew_status")
            )
            has_agent_insights = bool(
                flow.crewai_state_data and flow.crewai_state_data.get("agent_insights")
            )
            ...
            integration_status = {
                ...
                "integration_level": (
                    "full"
                    if all([has_crewai_state, has_crew_status, has_agent_insights])
                    else ("partial" if has_crewai_state else "minimal")
                ),
                "features": {
                ...
                "data_completeness": {
                    ...
                    "phase_data_available": any(
                        [
                            getattr(flow, f"{phase}_data", None)
                            for phase in [
                                "data_import",
                                "attribute_mapping",
                                "data_cleansing",
                ...
                "recommendations": self._generate_integration_recommendations(
                    flow, has_crewai_state, has_crew_status
                ),
            }
            ...
            logger.error(
                f"❌ Failed to get CrewAI integration status for {flow_id}: {e}"
            )
            raise

    def _generate_integration_recommendations(
        self, flow: DiscoveryFlow, has_crewai_state: bool, has_crew_status: bool
    ) -> List[str]:
        ...
        if not has_crew_status:
            recommendations.append(
                "Capture crew status information for better tracking"
            )

        if not flow.crewai_state_data or not flow.crewai_state_data.get(
            "agent_insights"
        ):
            recommendations.append(
                "Enable agent insights collection for enhanced analysis"
            )
