...
class CrewAIIntegrationService:
    ...
    async def export_flow_to_crewai(self, flow_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            crewai_export = {
                ...
                "crew_status": flow.crewai_state_data.get("crew_status", {}) if flow.crewai_state_data else {},
    ...
    async def validate_crewai_state_sync(self, flow_id: str, crewai_state: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            validation_result = {
                ...
                "sync_quality": "excellent" if len(discrepancies) == 0 else (
                    "good" if len([d for d in discrepancies if d["severity"] == "high"]) == 0 else "poor"
                ),
                "last_database_update": flow.updated_at.isoformat() if flow.updated_at else None,
                "validation_timestamp": logger.info.__self__.time() if hasattr(logger.info.__self__, 'time') else "unknown"
            }
            
            logger.info(f"✅ CrewAI state validation completed for flow: {flow_id} - Synced: {validation_result['is_synced']}")
    ...
    def _extract_data_import_id(self, metadata: Dict[str, Any], crewai_state: Dict[str, Any]) -> str:
    ...
    async def _store_crewai_state(self, flow_id: str, crewai_state: Dict[str, Any]) -> None:
