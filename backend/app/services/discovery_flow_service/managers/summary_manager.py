...
class SummaryManager:
    ...
    async def get_flow_summary(self, flow: DiscoveryFlow) -> Dict[str, Any]:
        ...
        try:
            ...
            asset_statistics = await self.asset_manager.get_asset_summary_statistics(
            ...
            completed_phases = sum(
                1 for completed in phase_completion.values() if completed
            )
    ...
    async def get_multi_flow_summary(
        ...
        try:
            ...
            for flow in flows:
                ...
                status_distribution[flow.status] = (
                    status_distribution.get(flow.status, 0) + 1
                )
                ...
                try:
                    assets = await self.asset_manager.get_flow_assets(
                        flow.flow_id, flow.id
                    )
                    ...
                    for asset in assets:
                        ...
                        quality_aggregates["total_quality"] += (
                            asset.quality_score or 0.0
                        )
                        quality_aggregates["total_confidence"] += (
                            asset.confidence_score or 0.0
                        )
                except Exception as e:
                    logger.warning(
                        f"⚠️ Failed to get assets for flow {flow.flow_id}: {e}"
                    )
                    continue

            # Calculate averages
            avg_quality = (
                quality_aggregates["total_quality"] / quality_aggregates["asset_count"]
                if quality_aggregates["asset_count"] > 0
                else 0.0
            )
            avg_confidence = (
                quality_aggregates["total_confidence"]
                / quality_aggregates["asset_count"]
                if quality_aggregates["asset_count"] > 0
                else 0.0
            )
            ...
            for phase, stats in phase_completion_stats.items():
                phase_completion_percentages[phase] = (
            ...
            summary = {
                "overview": {
                    ...
                    "avg_assets_per_flow": (
                        round(total_assets / total_flows, 1) if total_flows > 0 else 0
                    ),
                },
                ...
                "performance_indicators": self._calculate_performance_indicators(
                    flows, phase_completion_percentages
                ),
            }
    ...
    async def get_flow_health_report(self, flow: DiscoveryFlow) -> Dict[str, Any]:
        ...
        try:
            ...
            asset_stats = await self.asset_manager.get_asset_summary_statistics(
                flow.flow_id, flow.id
            )
            ...
            health_report = {
                ...
                "recommendations": [
                    indicator["recommendation"] for indicator in health_indicators
                ],
                "metrics": {
                    "phase_completion_rate": round((completed_phases / 6) * 100, 1),
                    "asset_quality_rate": round(
                        asset_stats["avg_quality_score"] * 100, 1
                    ),
                    "asset_confidence_rate": round(
                        asset_stats["avg_confidence_score"] * 100, 1
                    ),
            ...
            logger.info(
                f"✅ Health report generated for flow: {flow.flow_id} - Status: {overall_health}"
            )
    ...
    def _analyze_flow_progress(
        self, flow: DiscoveryFlow, asset_stats: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        if (
            asset_stats["quality_metrics"]["unvalidated_assets"]
            > asset_stats["total_count"] * 0.5
        ):
    ...
    def _assess_flow_quality(
        ...
        overall_quality = sum(
            score * weights[factor] for factor, score in quality_factors.items()
        )
        ...
        quality_level = (
            ...
            if overall_quality >= 0.9
                ...
                else ("fair" if overall_quality >= 0.5 else "poor")
            )
        )
    ...
    def _calculate_performance_indicators(
        self, flows: List[DiscoveryFlow], phase_percentages: Dict[str, float]
    ) -> Dict[str, Any]:
        ...
        return {
            "completion_rate": (
                round((len(completed_flows) / len(flows)) * 100, 1) if flows else 0
            ),
            "average_phase_completion": (
                round(sum(phase_percentages.values()) / len(phase_percentages), 1)
                if phase_percentages
                else 0
            ),
            "active_flows_ratio": (
                round((len(active_flows) / len(flows)) * 100, 1) if flows else 0
            ),
            "flow_efficiency": (
                "high" if len(completed_flows) > len(active_flows) else "medium"
            ),
        }
    ...
    def _estimate_completion_time(
        self, flow: DiscoveryFlow, completed_phases: int
    ) -> str:
    ...
    def _create_empty_multi_flow_summary(self) -> Dict[str, Any]:
        ...
        return {
            "overview": {"total_flows": 0, "total_assets": 0, "avg_assets_per_flow": 0},
            "flow_status_distribution": {},
            "phase_completion": {"statistics": {}, "completion_percentages": {}},
            "quality_metrics": {
