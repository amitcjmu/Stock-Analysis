...
class AssetManager:
    ...
    async def get_flow_assets(
        self, flow_id: str, flow_internal_id: uuid.UUID
    ) -> List[Asset]:
        """Get all assets for a discovery flow"""
        try:
            assets = await self.flow_repo.asset_queries.get_assets_by_flow_id(
                flow_internal_id
            )
    ...
    async def create_assets_from_discovery(
        ...
        try:
            logger.info(
                f"📦 Creating {len(asset_data_list)} assets from {discovered_in_phase} phase"
            )
    ...
    async def update_asset_quality_scores(
        ...
        try:
            ...
            if not (0.0 <= quality_score <= 1.0):
                raise ValueError(
                    f"Quality score must be between 0.0 and 1.0: {quality_score}"
                )

            if not (0.0 <= confidence_score <= 1.0):
                raise ValueError(
                    f"Confidence score must be between 0.0 and 1.0: {confidence_score}"
                )
    ...
    async def get_assets_by_validation_status(
        ...
        try:
            assets = await self.flow_repo.asset_queries.get_assets_by_validation_status(
                validation_status
            )
            logger.info(
                f"✅ Found {len(assets)} assets with validation status: {validation_status}"
            )
            return assets

        except Exception as e:
            logger.error(
                f"❌ Failed to get assets by validation status {validation_status}: {e}"
            )
            raise
    ...
    async def get_assets_requiring_review(
        ...
        try:
            assets = await self.flow_repo.asset_queries.get_assets_below_confidence_threshold(
                min_confidence_threshold
            )
            logger.info(
                f"✅ Found {len(assets)} assets requiring review (confidence < {min_confidence_threshold})"
            )
            return assets
    ...
    async def bulk_validate_assets(
        ...
        try:
            logger.info(
                f"🔍 Bulk validating {len(asset_ids)} assets with status: {validation_status}"
            )
            ...
            for asset_id in asset_ids:
                try:
                    asset = await self.validate_asset(
                        asset_id, validation_status, validation_results
                    )
            ...
            logger.info(
                f"✅ Bulk validation completed: {len(updated_assets)}/{len(asset_ids)} assets updated"
            )
    ...
    async def get_asset_summary_statistics(
        ...
        try:
            ...
            for asset in assets:
                ...
                validation_status_counts[asset.validation_status] = (
                    validation_status_counts.get(asset.validation_status, 0) + 1
                )
            ...
            avg_confidence_score = (
                total_confidence_score / len(assets) if assets else 0.0
            )
            ...
            statistics = {
                ...
                "quality_metrics": {
                    "high_quality_assets": sum(
                        1 for a in assets if (a.quality_score or 0) >= 0.8
                    ),
                    "low_confidence_assets": sum(
                        1 for a in assets if (a.confidence_score or 0) < 0.7
                    ),
                    "unvalidated_assets": sum(
                        1 for a in assets if a.validation_status == "pending"
                    ),
                },
            }
            ...
            logger.error(
                f"❌ Failed to generate asset summary statistics for flow {flow_id}: {e}"
            )
            raise

    async def filter_assets_by_criteria(
        self, flow_internal_id: uuid.UUID, criteria: Dict[str, Any]
    ) -> List[Asset]:
    ...
    def _matches_criteria(self, asset: Asset, criteria: Dict[str, Any]) -> bool:
        ...
        if (
            "validation_status" in criteria
            and asset.validation_status != criteria["validation_status"]
        ):
            return False

        # Quality score filter
        if (
            "min_quality_score" in criteria
            and (asset.quality_score or 0) < criteria["min_quality_score"]
        ):
            return False

        if (
            "max_quality_score" in criteria
            and (asset.quality_score or 0) > criteria["max_quality_score"]
        ):
            return False

        # Confidence score filter
        if (
            "min_confidence_score" in criteria
            and (asset.confidence_score or 0) < criteria["min_confidence_score"]
        ):
            return False

        if (
            "max_confidence_score" in criteria
            and (asset.confidence_score or 0) > criteria["max_confidence_score"]
        ):
            return False
        ...
        if (
            "created_before" in criteria
            and asset.created_at > criteria["created_before"]
        ):
            return False
