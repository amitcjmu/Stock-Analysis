...
def validate_asset_data_quality(asset_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    validity_score = 1.0
    if "asset_type" in asset_data:
        valid_types = ["server", "database", "application", "network", "storage", "unknown"]
...
def generate_flow_summary_key_metrics(
    ...
    return {
        ...
        "is_assessment_ready": completed_phases == len(FlowPhase) and overall_health >= 0.7
    }


def estimate_remaining_effort(
    current_phase_completion: Dict[str, bool],
    asset_count: int
) -> Dict[str, Any]:
    """
    ...
    """
    remaining_phases = [
        phase.value for phase in FlowPhase
        if not current_phase_completion.get(phase.value, False)
    ]
    ...
    asset_multiplier = max(1.0, min(3.0, asset_count / 50))  # 1x to 3x based on asset count
