...
class CoreAnalysisHandler:
    ...
    def __init__(self, llm=None):
        ...
        logger.info(
            f"Core analysis handler initialized (CrewAI: {self.service_available})"
        )

    def _initialize_agents(self):
        """Initialize persistent agents for core analysis."""
        try:
            if AGENTS_AVAILABLE:
                ...
                logger.info("Persistent agents available for core analysis")
    ...
    def calculate_data_completeness(self, sample_data: List[Dict]) -> float:
        ...
        try:
            ...
            if "data_cleansing" in self.crews:
                try:
                    crew_result = self.crews["data_cleansing"].kickoff(
                        {"sample_data": sample_data, "task": "completeness_analysis"}
                    )
                    ...
                    logger.error(
                        f"Data Cleansing Crew completeness analysis failed: {e}"
                    )
            ...
            if "field_mapping" in self.crews:
                try:
                    crew_result = self.crews["field_mapping"].kickoff(
                        {"columns": columns, "task": "type_indicator_analysis"}
                    )
                    ...
                    logger.error(
                        f"Field Mapping Crew type indicator analysis failed: {e}"
                    )
            ...
            if "inventory_building" in self.crews:
                try:
                    crew_result = self.crews["inventory_building"].kickoff(
                        {"cmdb_data": cmdb_data, "task": "asset_type_detection"}
                    )
                    ...
                    logger.error(
                        f"Inventory Building Crew asset type detection failed: {e}"
                    )
    ...
    def _fallback_asset_type(self, cmdb_data: Dict[str, Any]) -> str:
        ...
        if any(indicator in col_text for indicator in ["server", "host", "compute"]):
            ...
            indicator in col_text for indicator in ["application", "app", "service"]
        ):
            return "application"
        ...
        try:
            ...
            if "field_mapping" in self.crews:
                try:
                    ...
                    logger.error(
                        f"Field Mapping Crew missing field analysis failed: {e}"
                    )
    ...
    def identify_issues(self, cmdb_data: Dict) -> List[str]:
        ...
        try:
            ...
            if "data_cleansing" in self.crews:
                try:
                    crew_result = self.crews["data_cleansing"].kickoff(
                        {"cmdb_data": cmdb_data, "task": "issue_identification"}
                    )
                    ...
                    logger.error(
                        f"Data Cleansing Crew issue identification failed: {e}"
                    )
    ...
    def generate_basic_recommendations(
        self, cmdb_data: Dict, asset_type: str
    ) -> List[str]:
        ...
        try:
            ...
            if "data_cleansing" in self.crews:
                try:
                    ...
                    logger.error(
                        f"Data Cleansing Crew recommendation generation failed: {e}"
                    )
    ...
    def assess_migration_readiness(
        self, cmdb_data: Dict, asset_type: str, quality_score: int
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if "data_cleansing" in self.crews:
                try:
                    ...
                    logger.error(
                        f"Data Cleansing Crew readiness assessment failed: {e}"
                    )
    ...
    def analyze_data_patterns(self, sample_data: List[Dict]) -> Dict[str, Any]:
        ...
        try:
            ...
            if "data_cleansing" in self.crews:
                try:
                    crew_result = self.crews["data_cleansing"].kickoff(
                        {"sample_data": sample_data, "task": "pattern_analysis"}
                    )
