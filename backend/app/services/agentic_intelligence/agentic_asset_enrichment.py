...
class AgenticAssetEnrichment:
    ...
    def __init__(self, crewai_service, client_account_id: uuid.UUID, engagement_id: uuid.UUID, flow_id: Optional[uuid.UUID] = None):
        ...
        logger.info("✅ Agentic Asset Enrichment orchestrator initialized with 3 intelligence agents")
    
    async def enrich_asset_complete(self, asset_data: Dict[str, Any], enable_parallel: bool = True) -> Dict[str, Any]:
        """
        ...
        try:
            if enable_parallel:
                ...
                business_analysis, risk_analysis, modernization_analysis = await self._run_agents_parallel(asset_data)
            else:
                # Run agents sequentially (safer but slower)
                business_analysis, risk_analysis, modernization_analysis = await self._run_agents_sequential(asset_data)
            ...
            enriched_asset.update({
                ...
                "enrichment_agents": ["BusinessValueAgent", "RiskAssessmentAgent", "ModernizationAgent"],
            ...
            logger.info(f"   - Business Value: {business_analysis.get('business_value_score', 'N/A')}/10")
            logger.info(f"   - Risk Level: {risk_analysis.get('risk_assessment', 'N/A')}")
            logger.info(f"   - Cloud Readiness: {modernization_analysis.get('cloud_readiness_score', 'N/A')}/100")
    ...
    async def enrich_assets_batch(self, assets: List[Dict[str, Any]], 
                                 batch_size: int = 5, 
        ...
        logger.info(f"🧠 Starting batch agentic enrichment for {len(assets)} assets (batch size: {batch_size})")
        ...
        for i in range(0, len(assets), batch_size):
            ...
            logger.info(f"📦 Processing batch {batch_number}/{total_batches} ({len(batch)} assets)")
            
            # Process batch concurrently
            batch_tasks = [
                self.enrich_asset_complete(asset, enable_parallel) 
                for asset in batch
            ]
            ...
            try:
                ...
                for asset, result in zip(batch, batch_results):
                    if isinstance(result, Exception):
                        logger.error(f"❌ Asset enrichment failed for {asset.get('name')}: {result}")
                        enriched_assets.append(self._create_fallback_enrichment(asset, str(result)))
                    else:
                ...
                for asset in batch:
                    enriched_assets.append(self._create_fallback_enrichment(asset, str(e)))
        
        success_count = sum(1 for asset in enriched_assets if asset.get("enrichment_status") == "agentic_complete")
        logger.info(f"✅ Batch enrichment completed: {success_count}/{len(assets)} assets successfully enriched")
        
        return enriched_assets
    
    async def _run_agents_parallel(self, asset_data: Dict[str, Any]) -> Tuple[Dict[str, Any], Dict[str, Any], Dict[str, Any]]:
        """Run all three agents in parallel for performance"""
        
        logger.debug(f"🔀 Running agents in parallel for {asset_data.get('name')}")
        
        # Create tasks for all three agents
        business_task = self.business_value_agent.analyze_asset_business_value(asset_data)
        risk_task = self.risk_assessment_agent.analyze_asset_risk(asset_data) 
        modernization_task = self.modernization_agent.analyze_modernization_potential(asset_data)
        ...
        business_analysis = results[0] if not isinstance(results[0], Exception) else self._create_fallback_business_analysis(asset_data)
        risk_analysis = results[1] if not isinstance(results[1], Exception) else self._create_fallback_risk_analysis(asset_data)
        modernization_analysis = results[2] if not isinstance(results[2], Exception) else self._create_fallback_modernization_analysis(asset_data)
        
        return business_analysis, risk_analysis, modernization_analysis
    
    async def _run_agents_sequential(self, asset_data: Dict[str, Any]) -> Tuple[Dict[str, Any], Dict[str, Any], Dict[str, Any]]:
        """Run agents sequentially (safer but slower)"""
        ...
        try:
            business_analysis = await self.business_value_agent.analyze_asset_business_value(asset_data)
        except Exception as e:
        ...
        # Risk Assessment  
        try:
            risk_analysis = await self.risk_assessment_agent.analyze_asset_risk(asset_data)
        except Exception as e:
        ...
        try:
            modernization_analysis = await self.modernization_agent.analyze_modernization_potential(asset_data)
        except Exception as e:
            logger.warning(f"Modernization analysis failed: {e}")
            modernization_analysis = self._create_fallback_modernization_analysis(asset_data)
        
        return business_analysis, risk_analysis, modernization_analysis
    
    def _consolidate_agent_insights(self, asset_data: Dict[str, Any], 
                                   business_analysis: Dict[str, Any],
                                   risk_analysis: Dict[str, Any], 
                                   modernization_analysis: Dict[str, Any]) -> Dict[str, Any]:
        ...
        enriched_asset.update({
            ...
            "business_value_recommendations": business_analysis.get("recommendations", [])
        })
        ...
        enriched_asset.update({
            ...
            "risk_recommendations": risk_analysis.get("longterm_recommendations", [])
        })
        
        # Modernization Intelligence
        enriched_asset.update({
            "cloud_readiness_score": modernization_analysis.get("cloud_readiness_score"),
            "modernization_potential": modernization_analysis.get("modernization_potential"),
            "recommended_migration_strategy": modernization_analysis.get("recommended_strategy"),
            "migration_effort_estimate": modernization_analysis.get("migration_effort"),
            "architecture_assessment": modernization_analysis.get("architecture_assessment"),
            "containerization_readiness": modernization_analysis.get("containerization_readiness"),
            "modernization_recommendations": modernization_analysis.get("immediate_steps", []),
            "migration_roadmap": modernization_analysis.get("migration_strategy"),
            "expected_modernization_benefits": modernization_analysis.get("expected_benefits", [])
        })
        
        # Generate comprehensive summary
        enriched_asset["agentic_intelligence_summary"] = self._generate_intelligence_summary(
            business_analysis, risk_analysis, modernization_analysis
        )
        
        return enriched_asset
    
    def _calculate_overall_score(self, business_analysis: Dict[str, Any],
                               risk_analysis: Dict[str, Any], 
                               modernization_analysis: Dict[str, Any]) -> float:
        ...
        overall_score = (business_score * 0.4) + (risk_score * 0.3) + (modernization_score * 0.3)
        
        return round(overall_score, 1)
    
    def _count_patterns_discovered(self, business_analysis: Dict[str, Any],
                                 risk_analysis: Dict[str, Any], 
                                 modernization_analysis: Dict[str, Any]) -> int:
        """Count total patterns discovered across all agents"""
        
        count = 0
        count += business_analysis.get("patterns_discovered", 0) if isinstance(business_analysis.get("patterns_discovered"), int) else 0
        count += risk_analysis.get("patterns_discovered", 0) if isinstance(risk_analysis.get("patterns_discovered"), int) else 0
        count += modernization_analysis.get("patterns_discovered", 0) if isinstance(modernization_analysis.get("patterns_discovered"), int) else 0
        
        return count
    
    def _calculate_confidence_score(self, business_analysis: Dict[str, Any],
                                  risk_analysis: Dict[str, Any], 
                                  modernization_analysis: Dict[str, Any]) -> float:
        ...
        business_conf = confidence_map.get(business_analysis.get("confidence_level", "medium"), 0.6)
        risk_conf = confidence_map.get(risk_analysis.get("confidence_level", "medium"), 0.6)
        modernization_conf = confidence_map.get(modernization_analysis.get("technical_confidence", "medium"), 0.6)
        
        confidence_scores = [business_conf, risk_conf, modernization_conf]
        
        return round(sum(confidence_scores) / len(confidence_scores), 2)
    
    def _generate_intelligence_summary(self, business_analysis: Dict[str, Any],
                                     risk_analysis: Dict[str, Any], 
                                     modernization_analysis: Dict[str, Any]) -> str:
        ...
        # Add key insights
        if business_score >= 8:
            summary += "High business value asset requiring careful migration planning. "
        ...
        if cloud_readiness >= 80:
            ...
            summary += "Limited modernization readiness - consider lift-and-shift approach."
        
        return summary
    
    # Fallback Methods
    
    def _create_fallback_enrichment(self, asset_data: Dict[str, Any], error_message: str) -> Dict[str, Any]:
    ...
    def _create_fallback_business_analysis(self, asset_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _create_fallback_risk_analysis(self, asset_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _create_fallback_modernization_analysis(self, asset_data: Dict[str, Any]) -> Dict[str, Any]:
...
async def enrich_assets_with_agentic_intelligence(
    ...
    logger.info(f"🧠 Starting complete agentic asset enrichment for {len(assets)} assets")
    ...
    enriched_assets = await enrichment_orchestrator.enrich_assets_batch(
        ...
        enable_parallel=enable_parallel_agents
    )
