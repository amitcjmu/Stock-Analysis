...
class BaseDiscoveryFlow(Flow):
    ...
    async def save_state(self, state: BaseFlowState) -> None:
        ...
        try:
            ...
            await self.state_store.save_state(
                flow_id=state.flow_id, state=state_dict, phase=state.current_phase
            )
            logger.info(
                f"Saved state for flow {state.flow_id} in phase {state.current_phase}"
            )
    ...
    def emit_event(self, event_type: str, data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            try:
                ...
                loop.create_task(
                    publish_flow_event(flow_id, event_type, phase, data, context)
                )
            except RuntimeError:
    ...
    def handle_error(self, error: Exception, phase: str) -> None:
        ...
        self.emit_event(
            "error", {"phase": phase, "error": str(error), "type": type(error).__name__}
        )
