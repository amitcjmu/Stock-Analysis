...
from .discovery_validators import dependency_validation as discovery_dependency_validation
...
class FlowConfigurationManager:
    ...
    def initialize_all_flows(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"✅ Flow configuration complete: {len(results['flows_registered'])} flows registered")
        ...
        for flow_name in registered_flows:
            try:
                ...
                if len(config.phases) == 0:
                    verification_results["issues"].append(f"{flow_name}: No phases defined")
                
                if not config.initialization_handler:
                    verification_results["issues"].append(f"{flow_name}: No initialization handler")
