...
async def assessment_validation(
    ...
    try:
        ...
        if not asset_inventory:
            ...
            missing_sections = [
                s for s in required_sections if s not in asset_inventory
            ]
            if missing_sections:
                warnings.append(
                    f"Missing inventory sections: {', '.join(missing_sections)}"
                )
        ...
        if not assessment_criteria:
            ...
            missing_criteria = [
                c for c in required_criteria if c not in assessment_criteria
            ]
            if missing_criteria:
                warnings.append(
                    f"Missing assessment criteria: {', '.join(missing_criteria)}"
                )
        ...
        if not business_priorities:
            warnings.append(
                "No business priorities provided - using default priorities"
            )
        ...
        metadata = {
            ...
            "criteria_count": (
                len(assessment_criteria) if isinstance(assessment_criteria, dict) else 0
            ),
        }
...
async def complexity_validation(
    ...
    try:
            ...
            for asset_id, scores in readiness_scores.items():
                ...
                for dimension, score in scores.items():
                    if not isinstance(score, (int, float)):
                        ...
                        errors.append(
                            f"Score out of range for {asset_id}.{dimension}: {score}"
                        )
        ...
        metadata = {
            "validated_at": datetime.utcnow().isoformat(),
            "asset_count": (
                len(readiness_scores) if isinstance(readiness_scores, dict) else 0
            ),
            "rule_count": (
                len(complexity_rules) if isinstance(complexity_rules, dict) else 0
            ),
        }
    ...
    try:
        ...
        if not complexity_scores:
            ...
            for asset_id, score_data in complexity_scores.items():
                if not isinstance(score_data, dict):
                    ...
                    errors.append(
                        f"Invalid complexity level for asset {asset_id}: {score_data['complexity_level']}"
                    )
        ...
        if not risk_matrix:
            ...
            if "categories" in risk_matrix:
                ...
                missing_categories = [
                    c for c in required_categories if c not in categories
                ]
                if missing_categories:
                    warnings.append(
                        f"Missing risk categories: {', '.join(missing_categories)}"
                    )
        ...
        metadata = {
            "validated_at": datetime.utcnow().isoformat(),
            "asset_count": (
                len(complexity_scores) if isinstance(complexity_scores, dict) else 0
            ),
            "risk_categories": (
                len(risk_matrix.get("categories", {}))
    ...
    try:
        ...
        if not business_priorities:
            ...
            for priority in required_priorities:
                if priority not in business_priorities:
                    ...
                    errors.append(
                        f"Priority value out of range for {priority}: {business_priorities[priority]}"
                    )
        ...
        if cost_constraints:
            if not isinstance(cost_constraints, dict):
                ...
                if (
                    not isinstance(cost_constraints["budget_limit"], (int, float))
                    or cost_constraints["budget_limit"] <= 0
                ):
        ...
        metadata = {
            ...
            "priority_count": (
                len(business_priorities) if isinstance(business_priorities, dict) else 0
            ),
...
async def inventory_completeness(
    ...
    try:
        ...
        if len(asset_quality_issues) > 10:
            warnings.append(
                f"Found {len(asset_quality_issues)} asset quality issues (showing first 10)"
            )
        ...
        if total_assets == 0:
            ...
            warnings.append(
                f"Low asset count ({total_assets}) may affect assessment accuracy"
            )
            ...
            if isinstance(scores, dict):
                for key, value in scores.items():
                    if isinstance(value, dict) and any(
                        "score" in k for k in value.keys()
                    ):
                        ...
                        for score_key, score_value in value.items():
                            if "score" in score_key:
                                if not isinstance(score_value, (int, float)):
                                    errors.append(
                                        f"Invalid score type in {field}.{key}.{score_key}"
                                    )
                                elif not 0 <= score_value <= 1:
                                    warnings.append(
                                        f"Score out of typical range in {field}.{key}.{score_key}: {score_value}"
                                    )
                    elif isinstance(value, (int, float)):
                        # Direct score value
                        if not 0 <= value <= 1:
                            warnings.append(
                                f"Score out of typical range in {field}.{key}: {value}"
                            )
    ...
    try:
        ...
        for asset_id, risk_data in risk_scores.items():
            ...
            for strategy in mitigation_strategies:
                if not isinstance(strategy, dict):
                    warnings.append(
                        f"Invalid mitigation strategy format for {asset_id}"
                    )
        ...
        if high_risk_without_mitigation:
            errors.append(
                ...
                + (
                    f" and {len(high_risk_without_mitigation) - 5} more"
                    if len(high_risk_without_mitigation) > 5
                    else ""
                )
            )
...
async def roadmap_validation(
    ...
    try:
        ...
        if not roadmap:
            ...
            if missing_elements:
                errors.append(
                    f"Roadmap missing required elements: {', '.join(missing_elements)}"
                )
                ...
                if not isinstance(phases, list) or len(phases) == 0:
                    ...
                    for idx, phase in enumerate(phases):
                        if not isinstance(phase, dict):
                            ...
                            k in phase for k in ["name", "duration", "assets"]
                        ):
                            warnings.append(
                                f"Incomplete phase definition at index {idx}"
                            )
        ...
        metadata = {
            ...
            "phase_count": (
                len(roadmap.get("phases", [])) if isinstance(roadmap, dict) else 0
            ),
        }
