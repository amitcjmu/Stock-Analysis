...
async def platform_validation(
    state: Dict[str, Any], phase_input: Dict[str, Any]
    ...
    try:
        ...
        if (
            not isinstance(env_config["platforms"], list)
            or len(env_config["platforms"]) == 0
        ):
            return False, "environment_config.platforms must be a non-empty list"

        logger.info(
            f"✅ Platform validation passed for {len(env_config['platforms'])} platforms"
        )
        return True, None
...
async def credential_validation(
    ...
    try:
        ...
        for platform in platforms:
            ...
            if platform_id not in credentials:
                ...
                if phase_input.get("automation_tier") in ["tier_3", "tier_4"]:
                    logger.warning(
                        f"Missing credentials for {platform_id}, will use manual collection"
                    )
                else:
        ...
        for platform_id, cred in credentials.items():
            ...
            if not any(
                key in cred
                for key in ["api_key", "username", "token", "service_account"]
            ):
...
async def collection_validation(
    state: Dict[str, Any], phase_input: Dict[str, Any]
) -> Tuple[bool, Optional[str]]:
    ...
    try:
        ...
        if platforms_without_adapters:
            ...
            if state.get("automation_tier") in ["tier_3", "tier_4"]:
                logger.warning(
                    f"No adapters for platforms: {platforms_without_adapters}"
                )
            else:
                return (
                    False,
                    f"Missing adapter configurations for: {platforms_without_adapters}",
                )

        logger.info(
            f"✅ Collection validation passed for {len(detected_platforms)} platforms"
        )
        return True, None
    ...
    try:
        ...
        if low_quality_platforms:
            ...
            if automation_tier in ["tier_3", "tier_4"]:
                ...
                return (
                    False,
                    f"Data quality below threshold for: {low_quality_platforms}",
                )
...
async def sixr_impact_validation(
    state: Dict[str, Any], phase_input: Dict[str, Any]
) -> Tuple[bool, Optional[str]]:
        ...
        for gap_id, validation in validation_results.items():
            if not validation.get("is_valid", False):
                failed_validations.append(
                    f"{gap_id}: {validation.get('reason', 'unknown')}"
                )
...
async def completeness_validation(
    state: Dict[str, Any], phase_input: Dict[str, Any]
) -> Tuple[bool, Optional[str]]:
    ...
    try:
        ...
        response_rate = (
            len(responses) / len(identified_gaps) if identified_gaps else 1.0
        )
        ...
        if response_rate < min_rate:
            return (
                False,
                f"Response rate {response_rate:.2%} below minimum {min_rate:.2%}",
            )
        ...
        logger.info(
            f"✅ Completeness validation passed with {response_rate:.2%} response rate"
        )
        return True, None
...
async def final_validation(
    ...
    try:
        ...
        if completeness < min_completeness:
            return (
                False,
                f"Data completeness {completeness:.2%} below threshold {min_completeness:.2%}",
            )
        ...
        for platform_id, platform_data in final_data.items():
            ...
            if "metadata" not in platform_data or "resources" not in platform_data:
                return (
                    False,
                    f"Missing required fields in synthesized data for {platform_id}",
                )
...
async def sixr_readiness_validation(
    state: Dict[str, Any], phase_input: Dict[str, Any]
) -> Tuple[bool, Optional[str]]:
    ...
    try:
        ...
        if sixr_readiness < min_readiness:
            return (
                False,
                f"6R readiness score {sixr_readiness:.2f} below threshold {min_readiness:.2f}",
            )
