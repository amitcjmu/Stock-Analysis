    ...
    try:
        ...
        if mapping_rules:
            ...
            for target_field, source_field in mapping_rules.items():
                if not source_field:
                    errors.append(
                        f"Empty source field for target field: {target_field}"
                    )

                # Check if source field exists in data (sample first record)
                if (
                    imported_data
                    and isinstance(imported_data, list)
                    and len(imported_data) > 0
                ):
                    sample_record = imported_data[0]
                    if (
                        isinstance(sample_record, dict)
                        and source_field not in sample_record
                    ):
                        warnings.append(
                            f"Source field '{source_field}' not found in imported data"
                        )
        ...
        if imported_data and isinstance(imported_data, list):
            ...
            if len(imported_data) == 0:
                ...
                warnings.append(
                    f"Large dataset detected: {len(imported_data)} records. Performance may be impacted."
                )
        ...
        return ValidationResult(
            valid=False, errors=[f"Validation exception: {str(e)}"], warnings=[]
        )
            ...
            if asset_type == "server":
                ...
                for field in server_fields:
                    if field not in asset:
                        warnings.append(
                            f"Server asset {asset_id} missing recommended field: {field}"
                        )
                ...
                for field in app_fields:
                    if field not in asset:
                        warnings.append(
                            f"Application asset {asset_id} missing recommended field: {field}"
                        )

        # Business rule validations
        if len(assets) > 50000:
            warnings.append(
                f"Large number of assets created: {len(assets)}. Consider batching for performance."
            )
...
async def inventory_validation(
    ...
    try:
        ...
        if assets and inventory:
            asset_ids = {
                asset.get("asset_id") for asset in assets if asset.get("asset_id")
            }
            inventory_ids = set()
            ...
            if missing_in_inventory:
                errors.append(
                    f"Assets not in inventory: {len(missing_in_inventory)} assets"
                )
            ...
            if orphaned_in_inventory:
                warnings.append(
                    f"Orphaned inventory entries: {len(orphaned_in_inventory)} entries"
                )
...
async def dependency_validation(
    ...
    try:
        ...
        for asset_id, deps in dependencies.items():
            ...
            if len(deps) > 50:
                warnings.append(
                    f"Asset {asset_id} has excessive dependencies: {len(deps)}"
                )
...
async def mapping_completeness(
    ...
    try:
        ...
        if (
            not imported_data
            or not isinstance(imported_data, list)
            or len(imported_data) == 0
        ):
        ...
        if critical_unmapped:
            warnings.append(
                f"Potentially important unmapped fields: {', '.join(critical_unmapped[:10])}"
            )

        completeness_ratio = (
            len(mapped_source_fields) / len(all_source_fields)
            if all_source_fields
            else 0
        )

        if completeness_ratio < 0.5:
            warnings.append(
                f"Low mapping coverage: {completeness_ratio:.1%} of fields mapped"
            )
...
async def cleansing_validation(
    ...
    try:
        ...
        metadata = {
            "validated_at": datetime.utcnow().isoformat(),
            "cleansed_record_count": (
                len(cleansed_data) if isinstance(cleansed_data, list) else 0
            ),
