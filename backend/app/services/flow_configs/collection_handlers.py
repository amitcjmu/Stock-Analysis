...
async def collection_finalization(
    db: AsyncSession, flow_id: str, final_state: Dict[str, Any], context: Dict[str, Any]
) -> Dict[str, Any]:
...
async def collection_rollback_handler(
    db: AsyncSession, flow_id: str, rollback_to_phase: str, context: Dict[str, Any]
) -> Dict[str, Any]:
...
async def adapter_preparation(
    db: AsyncSession, flow_id: str, phase_input: Dict[str, Any], context: Dict[str, Any]
) -> Dict[str, Any]:
...
async def gap_analysis_preparation(
    ...
    try:
        ...
        analysis_data = {"platforms": {}, "total_resources": 0, "average_quality": 0.0}
        ...
        for gap_id, response in responses.items():
            ...
            await db.execute(
                update_query,
                {
                    ...
                    "validation_status": (
                        "validated" if response.get("is_valid", False) else "pending"
                    ),
...
async def _get_collection_flow_by_master_id(
    db: AsyncSession, master_flow_id: str
...
async def _get_adapter_by_name(
...
async def _clear_collected_data(
...
async def _clear_questionnaire_responses(
