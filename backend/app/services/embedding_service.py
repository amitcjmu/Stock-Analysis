...
class EmbeddingService:
    ...
    def __init__(self):
        ...
        if EMBEDDING_LLM_AVAILABLE and settings.DEEPINFRA_API_KEY:
            try:
                ...
                logger.info(
                    "✅ AI embedding service initialized with DeepInfra thenlper/gte-large model via LLM config"
                )
            ...
            logger.info(
                "AI embedding service using mock mode - LLM config not available or DEEPINFRA_API_KEY not configured"
            )
    ...
    async def embed_text(self, text: str) -> List[float]:
        ...
        try:
            ...
            logger.debug(
                f"Using mock embedding for text (length: {len(text)}) - CrewAI embedding support pending"
            )
            return self._generate_mock_embedding(text)
    ...
    async def embed_texts(self, texts: List[str]) -> List[List[float]]:
        ...
        try:
            ...
            logger.debug(
                f"Using mock embeddings for batch of {len(texts)} texts - CrewAI embedding support pending"
            )
            return [self._generate_mock_embedding(text) for text in texts]
    ...
    async def search_similar_assets(
        self, query_text: str, limit: int = 10, is_mock_only: bool = True
    ) -> List[Asset]:
    ...
    async def find_similar_assets(
        self, asset_id: int, limit: int = 5, is_mock_only: bool = True
    ) -> List[Asset]:
    ...
    async def suggest_tags_for_asset(
        self, asset_id: int, is_mock_only: bool = True
    ) -> List[Tag]:
    ...
    async def _mock_text_search(
        self, query_text: str, limit: int, is_mock_only: bool
    ) -> List[Asset]:
    ...
    async def _mock_similarity_search(
        self, asset_id: int, limit: int, is_mock_only: bool
    ) -> List[Asset]:
        ...
        try:
            ...
            for asset in assets:
                ...
                if (
                    reference_asset.cpu_cores
                    and asset.cpu_cores
                    and abs(asset.cpu_cores - reference_asset.cpu_cores)
                    <= max(1, reference_asset.cpu_cores * 0.2)
                ):
                    score += 1

                # Similar memory (within 30%)
                if (
                    reference_asset.memory_gb
                    and asset.memory_gb
                    and abs(asset.memory_gb - reference_asset.memory_gb)
                    <= reference_asset.memory_gb * 0.3
                ):
                    score += 1
    ...
    async def _mock_tag_suggestions(
        self, asset_id: int, is_mock_only: bool
    ) -> List[Tag]:
        ...
        try:
            ...
            for tag in available_tags:
                ...
                if (
                    ...
                    and asset.business_criticality.lower() in tag_name_lower
                ):
                    suggested_tags.append(tag)
                    continue
    ...
    async def _ai_text_search(
        ...
        logger.info(
            "AI text search with embeddings not yet fully implemented, falling back to mock search"
        )
        return await self._mock_text_search(query_text, limit, is_mock_only)

    async def _ai_similarity_search(
        self, asset_id: int, limit: int, is_mock_only: bool
    ) -> List[Asset]:
        ...
        # For now, fall back to mock search
        logger.info(
            "AI similarity search with embeddings not yet fully implemented, falling back to mock search"
        )
        return await self._mock_similarity_search(asset_id, limit, is_mock_only)
    ...
    async def _ai_tag_suggestions(self, asset_id: int, is_mock_only: bool) -> List[Tag]:
        ...
        logger.info(
            "AI tag suggestions with embeddings not yet fully implemented, falling back to mock suggestions"
        )
        return await self._mock_tag_suggestions(asset_id, is_mock_only)
