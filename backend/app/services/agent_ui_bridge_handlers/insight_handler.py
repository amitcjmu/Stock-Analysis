...
class InsightHandler:
    ...
    def update_insight_actionability(self, insight_id: str, actionable: bool,
        ...
        logger.info(f"Updated actionability for insight {insight_id}: {old_actionable} -> {actionable}")
    ...
    def get_insight_statistics(self, page: str = None) -> Dict[str, Any]:
        ...
        actionability_rate = actionable_insights / total_insights if total_insights > 0 else 0.0
    ...
    def filter_insights(self, filters: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        if filters.get("insight_type"):
            insights = [i for i in insights if i.insight_type == filters["insight_type"]]
        
        if filters.get("confidence"):
            insights = [i for i in insights if i.confidence.value == filters["confidence"]]
        ...
        if filters.get("min_confidence"):
            ...
            insights = [i for i in insights if confidence_order.get(i.confidence.value, 0) >= min_level]
    ...
    def clear_insights(self, page: str = None) -> int:
        ...
        if page:
            self.agent_insights = {
                insight_id: insight for insight_id, insight in self.agent_insights.items()
                if insight.page != page
            }
        else:
