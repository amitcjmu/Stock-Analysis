...
class QuestionHandler:
    ...
    def add_agent_question(
        ...
        existing_questions = [
            existing
            for existing in self.agent_questions.values()
            if (
                existing.page == page
                ...
                (
                    ...
                    or (
                        ...
                        and (datetime.utcnow() - existing.answered_at).total_seconds()
                        < 3600
                    )
                )
            )
        ...
        if existing_questions:
            logger.info(
                f"Duplicate question detected and filtered out: {title} from {agent_name}"
            )
    ...
    def answer_agent_question(self, question_id: str, response: Any) -> Dict[str, Any]:
        ...
        return {"success": True, "question": asdict(question), "learning_stored": True}
    ...
    def clear_resolved_questions(self, page: str = None) -> int:
        ...
        if page:
            ...
            self.agent_questions = {
                qid: q
                for qid, q in self.agent_questions.items()
                if not (q.is_resolved and q.page == page)
            }
        else:
            # Clear all resolved questions
            self.agent_questions = {
                qid: q for qid, q in self.agent_questions.items() if not q.is_resolved
            }
    ...
    def get_question_statistics(self) -> Dict[str, Any]:
        ...
        resolved_questions = sum(
            1 for q in self.agent_questions.values() if q.is_resolved
        )
        ...
        return {
            ...
            "resolution_rate": (
                resolved_questions / total_questions if total_questions > 0 else 0
            ),
        }
    ...
    def load_questions(self, questions_data: Dict[str, Any]) -> None:
        ...
        for qid, q_data in questions_data.items():
            ...
            question = AgentQuestion(
                ...
                confidence=(
                    ConfidenceLevel(q_data["confidence"])
