...
class DiscoveryFlowMonitor:
    ...
    def monitor_field_mappings_request(
        ...
        try:
            ...
            if low_confidence_mappings:
                ...
                for mapping in low_confidence_mappings[:2]:  # Limit to 2 questions
                    ...
                    logger.info(
                        f"Created field mapping question {question_id} for {mapping['sourceField']}"
                    )

            # Add insight about overall mapping quality
            total_mappings = len(mappings)
            high_confidence = len(
                [m for m in mappings if m.get("confidence", 0) >= 0.85]
            )
            ...
            if total_mappings > 0:
                ...
                insight_id = self.agent_ui_bridge.add_agent_insight(
                    ...
                    + (
                        ...
                        if low_confidence_mappings
                        else "All mappings have high confidence."
                    ),
                    ...
                    supporting_data={
                        ...
                        "low_confidence_fields": [
                            m["sourceField"] for m in low_confidence_mappings
                        ],
                    },
                    page="attribute-mapping",
    ...
    def monitor_discovery_request(
        self, endpoint: str, request_data: Dict[str, Any], response_data: Dict[str, Any]
    ) -> None:
