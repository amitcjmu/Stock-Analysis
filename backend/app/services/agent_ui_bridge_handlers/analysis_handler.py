...
class AnalysisHandler:
    ...
    async def analyze_with_agents(
        ...
        try:
            ...
            quality_analysis = await self._analyze_data_quality_with_agents(
                analysis_request
            )
            analysis_result["data_classifications"] = quality_analysis.get(
                "classifications", {}
            )
            analysis_result["insights"].extend(quality_analysis.get("insights", []))

            # Data source analysis
            source_analysis = await self._analyze_data_source_with_agents(
                analysis_request
            )
            analysis_result["insights"].extend(source_analysis.get("insights", []))
            analysis_result["agent_recommendations"].extend(
                source_analysis.get("recommendations", [])
            )
            ...
            logger.info(
                f"Agent analysis completed: {len(analysis_result['insights'])} insights generated"
            )
    ...
    async def process_with_agents(
        ...
        try:
            ...
            logger.info(
                f"Starting agent processing: {operation_type} for {len(data_items)} items"
            )
            ...
            for item in data_items:
                processed_item = await self._process_single_item_with_agents(
                    item, operation_type, processing_context
                )
                ...
                if processed_item.get("quality_improved", False):
                    ...
                    processing_result["quality_improvements"].append(
                        processed_item.get("improvements", [])
                    )
            ...
            item_confidences = [
                ...
                for item in processing_result["processed_items"]
            ]
            processing_result["processing_summary"]["confidence_score"] = (
    ...
    async def _analyze_data_quality_with_agents(
        ...
        for record in sample_records:
            ...
            if quality_score >= 0.8:
                ...
                quality_analysis["classifications"]["needs_clarification"].append(
                    record
                )
            else:
        ...
        overall_quality = (
            total_quality_score / classified_count if classified_count > 0 else 0.0
        )
                ...
                "supporting_data": {
                    ...
                    "quality_distribution": {
                        ...
                        "needs_clarification": len(
                            quality_analysis["classifications"]["needs_clarification"]
                        ),
                        "unusable": len(
                            quality_analysis["classifications"]["unusable"]
                        ),
                    },
                },
            }
    ...
    async def _analyze_data_source_with_agents(
        self, request: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        source_analysis = {"insights": [], "recommendations": []}
    ...
    async def _process_single_item_with_agents(
        self, item: Dict[str, Any], operation_type: str, context: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        if operation_type == "cleanup":
            processed_item["processed_item"] = self._standardize_asset_type(
                processed_item["processed_item"]
            )
            processed_item["processed_item"] = self._normalize_environment(
                processed_item["processed_item"]
            )
            processed_item["processed_item"] = self._fix_hostname(
                processed_item["processed_item"]
            )
            processed_item["processed_item"] = self._complete_missing_data(
                processed_item["processed_item"]
            )
    ...
    def _standardize_asset_type(self, asset: Dict[str, Any]) -> Dict[str, Any]:
        ...
        standardized_type = type_mappings.get(
    ...
    def _normalize_environment(self, asset: Dict[str, Any]) -> Dict[str, Any]:
        ...
        normalized_env = env_mappings.get(
    ...
    def _analyze_field_patterns(self, records: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        return {
            ...
            "field_completeness": {
                k: v / total_records for k, v in field_completeness.items()
            },
