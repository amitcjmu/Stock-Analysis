...
class StorageManager:
    ...
    def load_classifications(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.warning(
                f"Could not parse data_classifications.json: {e}. Returning empty classifications."
            )
            # Optionally, move or rename the corrupted file
            try:
                corrupted_path = (
                    f"{file_path}.corrupted.{datetime.now().strftime('%Y%m%d%H%M%S')}"
                )
                os.rename(file_path, corrupted_path)
                logger.info(
                    f"Renamed corrupted classifications file to {corrupted_path}"
                )
            except Exception as rename_e:
                logger.error(
                    f"Could not rename corrupted classifications file: {rename_e}"
                )
            return {}
        except Exception as e:
            ...
            for insight_id, insight in insights.items():
                if hasattr(insight, "__dict__"):
                    ...
                    if insight_dict.get("created_at"):
                        insight_dict["created_at"] = insight_dict[
                            "created_at"
                        ].isoformat()
                    # Convert enums to strings
    ...
    def load_learning_experiences(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if os.path.exists(file_path):
                ...
                logger.info(
                    f"Loaded {len(experiences)} learning experiences from storage"
                )
                return experiences
        except Exception as e:
