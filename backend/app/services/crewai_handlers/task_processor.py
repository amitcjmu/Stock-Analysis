...
class TaskProcessor:
    ...
    async def process_cmdb_data(self, processing_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.field_mapping_tool and cmdb_data.get('structure', {}).get('columns'):
            ...
            asset_type_detected = self._detect_asset_type_intelligently(sample_data, enhanced_mappings)
            quality_score = self._calculate_intelligent_quality_score(sample_data, enhanced_mappings)
            issues = self._identify_intelligent_issues(sample_data, enhanced_mappings)
            recommendations = self._generate_intelligent_recommendations(sample_data, enhanced_mappings, issues)
            missing_fields = self._identify_truly_missing_fields(enhanced_mappings, field_analysis)
            ...
            return {
                ...
                "migration_readiness": "ready" if quality_score >= 80 else "requires_cleansing",
    ...
    def _detect_asset_type_intelligently(self, sample_data: List[Dict], enhanced_mappings: Dict) -> str:
        ...
        try:
            ...
            if enhanced_mappings:
                ...
                app_indicators = ["application", "app_name", "service", "business_service"]
            ...
            if sample_data and isinstance(sample_data[0], dict):
                ...
                if any("app" in field or "application" in field for field in field_names_lower):
                    return "application"
                elif any("db" in field or "database" in field for field in field_names_lower):
                    return "database"
    ...
    def _calculate_intelligent_quality_score(self, sample_data: List[Dict], enhanced_mappings: Dict) -> int:
        ...
        try:
            ...
            if enhanced_mappings:
                mapping_quality = len(enhanced_mappings) / max(len(sample_data[0].keys()), 1) * 40
            ...
            for record in sample_data[:10]:  # Sample first 10 records
                filled_fields = sum(1 for value in record.values() if value and str(value).strip())
    ...
    def _identify_intelligent_issues(self, sample_data: List[Dict], enhanced_mappings: Dict) -> List[str]:
        ...
        try:
            ...
            if enhanced_mappings:
                for original_field, mapped_field in enhanced_mappings.items():
                    if any(critical in mapped_field.lower() for critical in critical_fields):
                        mapped_critical.append(mapped_field)
            
            unmapped_critical = [field for field in critical_fields if field not in [mf.lower().replace(' ', '_') for mf in mapped_critical]]
            if unmapped_critical:
                issues.append(f"Missing critical fields: {', '.join(unmapped_critical)}")
            
            # Check for empty values in mapped fields
            empty_count = 0
            for record in sample_data[:5]:
                for field, value in record.items():
                    if not value or str(value).strip() in ['', 'null', 'none', 'unknown']:
                        empty_count += 1
            
            if empty_count > len(sample_data) * 2:  # More than 2 empty fields per record on average
                issues.append(f"Found {empty_count} empty or null values requiring cleanup")
            
            # Check for potential duplicates
            if len(sample_data) > 1:
                potential_duplicates = 0
                hostnames = [record.get('hostname', record.get('name', '')) for record in sample_data]
                ...
                if len(hostnames) - len(unique_hostnames) > 0:
                    ...
                    issues.append(f"Found {potential_duplicates} potential duplicate hostnames")
    ...
    def _generate_intelligent_recommendations(self, sample_data: List[Dict], enhanced_mappings: Dict, issues: List[str]) -> List[str]:
        """Generate recommendations using enhanced field mapping intelligence."""
        recommendations = []
        try:
            # Recommendations based on enhanced mappings
            if enhanced_mappings:
                recommendations.append("Use enhanced field mappings to improve data consistency")
                
                # Specific recommendations based on mapped fields
                if any("environment" in mapped_field.lower() for mapped_field in enhanced_mappings.values()):
                    recommendations.append("Standardize environment values using mapped field intelligence")
                
                if any("memory" in mapped_field.lower() or "ram" in mapped_field.lower() for mapped_field in enhanced_mappings.values()):
                    recommendations.append("Validate memory/RAM values are in correct units (GB)")
            
            # Recommendations based on issues
            if any("missing" in issue.lower() for issue in issues):
                recommendations.append("Complete missing field mappings using content analysis")
            
            if any("duplicate" in issue.lower() for issue in issues):
                recommendations.append("Resolve duplicate entries by adding unique identifiers")
            
            if any("empty" in issue.lower() or "null" in issue.lower() for issue in issues):
                recommendations.append("Fill empty values using intelligent pattern matching")
            
            return recommendations if recommendations else ["Data is ready for migration analysis"]
            
        except Exception as e:
            logger.warning(f"Error generating recommendations: {e}")
            return ["Use AI-enhanced field mapping for better analysis"]
    
    def _identify_truly_missing_fields(self, enhanced_mappings: Dict, field_analysis: Dict) -> List[str]:
        """Identify truly missing fields after applying enhanced field mapping intelligence."""
        try:
            critical_fields = ["Business Owner", "Criticality", "Department", "Environment"]
            ...
            if enhanced_mappings:
                for mapped_field in enhanced_mappings.values():
                    for critical_field in critical_fields:
                        if critical_field.lower().replace(' ', '_') in mapped_field.lower().replace(' ', '_'):
                            mapped_critical_fields.append(critical_field)
            
            # Return fields that are truly missing (not mapped)
            truly_missing = [field for field in critical_fields if field not in mapped_critical_fields]
