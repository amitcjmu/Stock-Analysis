...
class DiscoveryFlowIntegration:
        ...
        try:
            ...
            for asset in assets:
                application_data = {
                    ...
                    "discovery_completed_at": asset.discovery_completed_at.isoformat() if asset.discovery_completed_at else None,
                    "metadata": {
            ...
            self.logger.info(f"Found {len(applications)} applications ready for assessment in engagement {engagement_id}")
            return applications
            
        except Exception as e:
            self.logger.error(f"Failed to get applications ready for assessment: {str(e)}")
            raise
    ...
    async def verify_applications_ready_for_assessment(
        ...
        try:
            ...
            if not_ready:
                raise ValueError(f"Applications not ready for assessment: {'; '.join(not_ready)}")
            
            self.logger.info(f"Verified {len(application_ids)} applications are ready for assessment")
            return True
        ...
        try:
            ...
            metadata = {
                ...
                "discovery_info": {
                    ...
                    "completed_at": asset.discovery_completed_at.isoformat() if asset.discovery_completed_at else None,
                ...
                "performance_metrics": {
                    ...
                    "performance_characteristics": asset.performance_characteristics or {}
                },
    ...
    async def update_application_assessment_status(
        ...
        try:
            ...
            if success:
                self.logger.info(f"Updated assessment status for application {application_id} to {status}")
            else:
            ...
            await db.rollback()
            self.logger.error(f"Failed to update application assessment status: {str(e)}")
            raise
    ...
    async def update_application_readiness_status(
        ...
        try:
            ...
            success = result.rowcount > 0
            if success:
                self.logger.info(f"Updated readiness status for {result.rowcount} applications to {status}")
            
            return success
            
        except Exception as e:
            await db.rollback()
            self.logger.error(f"Failed to update application readiness status: {str(e)}")
            raise
    ...
    async def get_discovery_flow_summary(
        ...
        try:
            ...
            discovery_completed = sum(1 for asset in assets if asset.discovery_status == "completed")
            ready_for_assessment = sum(1 for asset in assets if asset.assessment_readiness == "ready")
            assessment_completed = sum(1 for asset in assets if asset.assessment_status == "completed")
            ready_for_planning = sum(1 for asset in assets if asset.planning_readiness == "ready")
            
            summary = {
                "engagement_id": engagement_id,
                "discovery_flows": {
                    "total_flows": len(discovery_flows),
                    "active_flows": sum(1 for flow in discovery_flows if flow.status == "active"),
                    "completed_flows": sum(1 for flow in discovery_flows if flow.status == "completed")
                },
                "assets": {
                ...
                "readiness_percentages": {
                    "discovery_completion": (discovery_completed / total_assets * 100) if total_assets > 0 else 0,
                    "assessment_readiness": (ready_for_assessment / total_assets * 100) if total_assets > 0 else 0,
                    "assessment_completion": (assessment_completed / total_assets * 100) if total_assets > 0 else 0,
                    "planning_readiness": (ready_for_planning / total_assets * 100) if total_assets > 0 else 0
                }
            }
            
            self.logger.info(f"Generated discovery flow summary for engagement {engagement_id}")
            return summary
        ...
        try:
            ...
            for asset in assets:
                ...
                if asset.assessment_readiness != "ready":
                    app_issues.append(f"Assessment readiness: {asset.assessment_readiness}")
                ...
                validation_results["issues"].extend([f"{asset.name}: {issue}" for issue in app_issues])
                validation_results["warnings"].extend([f"{asset.name}: {warning}" for warning in app_warnings])
            
            validation_results["valid"] = len(validation_results["issues"]) == 0
            
            self.logger.info(f"Validated prerequisites for {len(application_ids)} applications")
