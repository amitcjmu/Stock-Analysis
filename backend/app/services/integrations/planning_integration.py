...
class PlanningFlowIntegration:
    ...
    async def export_assessment_results(
        self, db: AsyncSession, flow_id: str, client_account_id: int
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            export_package = {
                "export_metadata": {
                    ...
                    "readiness_score": self._calculate_export_readiness_score(
                        flow_state
                    ),
                },
                "assessment_summary": {
                    ...
                    "phases_completed": (
                        list(flow_state.phase_results.keys())
                        if flow_state.phase_results
                        else []
                    ),
                    "architecture_captured": flow_state.architecture_captured,
                    "finalized_at": (
                        flow_state.finalized_at.isoformat()
                        if flow_state.finalized_at
                        else None
                    ),
                },
                ...
                "applications": await self._export_application_assessments(
                    db, flow_state
                ),
                "move_groups": await self._generate_move_group_hints(db, flow_state),
                "planning_recommendations": await self._generate_planning_recommendations(
                    db, flow_state
                ),
            ...
            self.logger.info(
                f"Exported assessment results for flow {flow_id} to Planning Flow"
            )
    ...
    async def handle_reassessment_request(
        ...
        try:
            ...
            reassessment_request = {
                ...
                "status": "pending",
                "reassessment_scope": self._determine_reassessment_scope(
                    planning_context
                ),
            ...
            response = {
                ...
                "estimated_effort": self._estimate_reassessment_effort(
                    assets, planning_context
                ),
                "next_steps": [
            ...
            self.logger.info(
                f"Created re-assessment request for {len(application_ids)} applications"
            )
            return response
        ...
        try:
            ...
            handoff_package = {
                ...
                "validation_requirements": {
                    ...
                    "architecture_standards_applied": export_package[
                        "assessment_summary"
            ...
            self.logger.info(
                f"Created Planning Flow handoff package for assessment {assessment_flow_id}"
            )
    ...
    async def sync_planning_feedback(
        ...
        try:
            ...
            feedback_processed = {
                ...
                "application_updates": {},
                "recommendations_accepted": planning_feedback.get(
                    "recommendations_accepted", []
                ),
                "recommendations_rejected": planning_feedback.get(
            ...
            self.logger.info(
                f"Synchronized planning feedback for assessment flow {assessment_flow_id}"
            )
            return True
    ...
    def _calculate_export_readiness_score(
        self, flow_state: AssessmentFlowState
    ) -> float:
        ...
        if flow_state.apps_ready_for_planning:
            ready_ratio = len(flow_state.apps_ready_for_planning) / len(
    ...
    async def _extract_compliance_requirements(
        self, db: AsyncSession, flow_state: AssessmentFlowState
    ) -> Dict[str, Any]:
    ...
    async def _generate_planning_recommendations(
        ...
        for app_id in flow_state.selected_application_ids:
            ...
            if (
                tech_debt
                and tech_debt.get("analysis", {}).get("overall_debt_score", 0) > 70
            ):
    ...
    def _calculate_assessment_confidence(
        self, flow_state: AssessmentFlowState, app_id: str
    ) -> float:
    ...
    def _determine_reassessment_scope(
    ...
    async def _mark_asset_for_reassessment(
        self, db: AsyncSession, asset: Asset, reassessment_request: Dict[str, Any]
    ):
    ...
    async def _generate_reassessment_recommendations(
        self, db: AsyncSession, assets: List[Asset], planning_context: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        """Generate recommendations for re-assessment approach."""
        recommendations = []
    ...
    def _estimate_reassessment_effort(
        self, assets: List[Asset], planning_context: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        scope_multipliers = {"full": 1.0, "partial": 0.6, "focused": 0.3}
    ...
    async def _apply_planning_feedback_to_application(
        ...
        stmt = select(Asset).where(
            and_(Asset.id == app_id, Asset.client_account_id == client_account_id)
        )
        result = await db.execute(stmt)
