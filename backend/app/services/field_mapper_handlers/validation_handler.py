...
class ValidationHandler:
    ...
    def __init__(self):
        ...
        self.required_fields_by_type = {
            'server': [
                'Asset Name', 'IP Address', 'Operating System', 'Environment',
                'CPU Cores', 'Memory (GB)', 'Business Owner'
            ],
            'application': [
                'Asset Name', 'Business Owner', 'Environment', 'Version',
                'Criticality', 'Application Service'
            ],
            'database': [
                'Asset Name', 'Business Owner', 'Environment', 'Version',
                'Storage (GB)', 'Criticality'
            ],
            'network': [
                'Asset Name', 'IP Address', 'Location', 'Vendor', 'Model'
            ],
            'storage': [
                'Asset Name', 'Storage (GB)', 'Location', 'Vendor', 'Model'
            ],
            'generic': [
    ...
    def identify_missing_fields(self, available_columns: List[str], 
        ...
        try:
            required_fields = self.required_fields_by_type.get(asset_type, 
                                                              self.required_fields_by_type['generic'])
            ...
            if mapped_fields:
                ...
                available_lower = [col.lower().replace(' ', '_') for col in available_columns]
                
                for required_field in required_fields:
                    required_lower = required_field.lower().replace(' ', '_')
                    for available_field in available_lower:
                        if required_lower in available_field or available_field in required_lower:
    ...
    def validate_field_format(self, field_name: str, value: Any, 
        ...
        try:
            ...
            # Environment validation
                ...
                valid_envs = ['production', 'prod', 'staging', 'stage', 'development', 'dev', 'test', 'qa']
    ...
    def validate_data_completeness(self, data: List[Dict[str, Any]], 
        ...
        try:
            ...
            for field in all_fields:
                filled_count = sum(1 for record in data if record.get(field) and str(record[field]).strip())
                field_stats[field] = {
                    "filled_count": filled_count,
                    "empty_count": total_records - filled_count,
                    "completeness_ratio": filled_count / total_records if total_records > 0 else 0
                }
            
            # Calculate overall completeness
            total_possible_values = len(all_fields) * total_records
            total_filled_values = sum(stats["filled_count"] for stats in field_stats.values())
            overall_completeness = total_filled_values / total_possible_values if total_possible_values > 0 else 0
            ...
            if low_completeness_fields:
                recommendations.append(f"Improve data collection for fields with low completeness: {', '.join(low_completeness_fields[:5])}")
            
            if overall_completeness < 0.7:
                recommendations.append("Overall data completeness is below 70% - consider data cleanup efforts")
            
            if overall_completeness >= 0.9:
                recommendations.append("Excellent data completeness - ready for migration planning")
