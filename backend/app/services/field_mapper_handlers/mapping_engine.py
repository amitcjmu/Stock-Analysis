...
class MappingEngineHandler:
        ...
        try:
            ...
            for field, variations in self.learned_mappings.items():
                if field in all_mappings:
                    ...
                    all_mappings[field].extend(
                        v for v in variations if v not in all_mappings[field]
                    )
                else:
                    all_mappings[field] = list(variations)
    ...
    def learn_field_mapping(
        self, canonical_field: str, field_variations: List[str], source: str = "manual"
    ) -> Dict[str, Any]:
        """Learn new field mapping variations."""
        try:
            # Normalize variations
            normalized_variations = [
                v.lower().strip().replace(" ", "_") for v in field_variations
            ]
            ...
            logger.info(
                f"Learned {added_count} new variations for '{canonical_field}' from {source}"
            )
            ...
            for column in available_columns:
                ...
                for variation in field_variations:
                    ...
                    # Contains match
                        ...
                        variation_clean in column_lower
                        or column_lower in variation_clean
                    ):
                        matches.append(column)
                        break
                ...
                for canonical_field, variations in all_mappings.items():
                    for variation in variations:
                        ...
                        if sample_data and confidence > 0 and i < len(sample_data[0]):
                            content_boost = self._analyze_content_match(
                                canonical_field, sample_data, i
                            )
    ...
    def get_mapping_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            total_variations = sum(
                len(variations) for variations in self.learned_mappings.values()
            )
            ...
            return {
                ...
                "learning_effectiveness": (
                    learned_fields / (base_fields + learned_fields)
                    if base_fields + learned_fields > 0
                    else 0
                ),
    ...
    def _analyze_content_match(
        self, canonical_field: str, sample_data: List[List[str]], column_index: int
    ) -> float:
            ...
            if sample_data and column_index < len(sample_data[0]):
                ...
                if column_values:
                    ...
                    non_empty_values = [
                        ...
                        if str(v).strip() not in ["", "N/A", "NULL", "None"]
                    ]
