...
class DataFlowValidator:
    ...
    async def validate_end_to_end_data_flow(
        ...
        logger.info(
            ...
            extra={
                "engagement_id": str(engagement_id),
                "validation_scope": (
                    list(validation_scope) if validation_scope else "all"
                ),
            },
        )

        validation_id = (
            f"validation_{engagement_id}_{int(datetime.utcnow().timestamp())}"
        )
        issues: List[ValidationIssue] = []
        phase_scores: Dict[str, float] = {}
    ...
    async def _get_assets_with_dependencies(
        self, session: AsyncSession, engagement_id: UUID
    ) -> List[Asset]:
    ...
    async def _validate_collection_phase(
        self, session: AsyncSession, flows_data: Dict[str, Any], assets: List[Asset]
            ...
            issues.append(
                ValidationIssue(
                    ...
                    remediation_suggestions=[
                        "Initiate collection flow for this engagement"
                    ],
        ...
        required_fields = self.validation_rules["collection_validation"][
            "required_fields"
        ]
        ...
        min_confidence = self.validation_rules["collection_validation"][
            ...
            issues.append(
                ValidationIssue(
                    ...
                    metadata={
                        ...
                        "affected_count": len(low_confidence_assets),
                    },
                    remediation_suggestions=[
                        "Review and enhance data collection for low-confidence assets"
                    ],
                    created_at=datetime.utcnow(),
                )
            )
        ...
        avg_completion = (
            sum(field_completion_scores) / len(field_completion_scores)
            if field_completion_scores
            else 0.0
        )
    ...
    async def _validate_discovery_phase(
        ...
        min_dependency_coverage = self.validation_rules["discovery_validation"][
            "min_dependency_coverage"
        ]
        assets_with_dependencies = [asset for asset in assets if asset.dependencies]
        dependency_coverage = (
            len(assets_with_dependencies) / len(assets) if assets else 0.0
        )
        ...
        if dependency_coverage < min_dependency_coverage:
            issues.append(
                ValidationIssue(
                    ...
                    metadata={
                        "current_coverage": dependency_coverage,
                        "target_coverage": min_dependency_coverage,
                    },
                    remediation_suggestions=[
                        "Run dependency discovery analysis",
        ...
        discovery_score = (
            dependency_coverage * 0.7
            + (1.0 - len(assets_missing_details) / len(assets)) * 0.3
        )
    ...
    async def _validate_assessment_phase(
        ...
        sixr_coverage = (
            1.0 - (len(assets_without_sixr) / len(assets)) if assets else 1.0
        )
        value_coverage = (
            1.0 - (len(assets_without_value) / len(assets)) if assets else 1.0
        )
            ...
            if abs(collection_count - discovery_count) > 2:  # Allow small variance
                issues.append(
                    ValidationIssue(
                        ...
                        remediation_suggestions=[
                            "Verify asset creation and deletion logs",
                            "Resync asset counts",
                        ],
                        created_at=datetime.utcnow(),
                    )
    ...
    def _generate_validation_summary(
        ...
        return {
            ...
            "workflow_health": (
                "EXCELLENT"
                if sum(phase_scores.values()) / len(phase_scores) > 0.9
                else (
                    "GOOD"
                    if sum(phase_scores.values()) / len(phase_scores) > 0.7
    ...
    def _generate_recommendations(
        self, issues: List[ValidationIssue], phase_scores: Dict[str, float]
    ) -> List[str]:
        ...
        if phase_scores.get("collection", 1.0) < 0.7:
            recommendations.append(
                "📊 Improve collection data quality before proceeding to discovery"
            )

        if phase_scores.get("discovery", 1.0) < 0.7:
            recommendations.append(
                "🔍 Complete discovery analysis before proceeding to assessment"
            )
        ...
        if warning_count > 5:
            recommendations.append(
                "⚠️ Review and address warning-level issues to improve workflow quality"
            )
