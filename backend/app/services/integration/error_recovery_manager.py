...
class ErrorRecoveryManager:
    ...
    async def _create_error_context(
        ...
        # Determine severity
        severity = await self._determine_error_severity(
            error, error_type, flow_type, phase
        )
    ...
    async def _determine_error_severity(
        ...
        if any(
            ...
            for keyword in ["critical", "fatal", "system", "database", "security"]
        ):
        ...
        if any(
            keyword in error_type.lower()
            for keyword in ["validation", "integrity", "constraint", "timeout"]
        ):
    ...
    async def _determine_error_category(
        self, error: Exception, error_type: str, operation: str = None
    ) -> ErrorCategory:
    ...
    async def _get_related_assets(
        self, engagement_id: UUID, flow_id: UUID = None, phase: str = None
    ) -> List[UUID]:
    ...
    async def _create_recovery_plan(self, error_context: ErrorContext) -> RecoveryPlan:
        ...
        if strategy_config.get("fallback"):
            ...
            plan.fallback_plan = RecoveryPlan(
                ...
                actions=await self._create_recovery_actions(
                    error_context, fallback_config
                ),
    ...
    async def _select_recovery_strategy(
        ...
        category_strategies = {
            ...
            ErrorCategory.VALIDATION_ERROR: self.recovery_strategies.get(
                "data_validation_error"
            ),
            ErrorCategory.INTEGRATION_ERROR: self.recovery_strategies.get(
                "integration_error"
            ),
            ErrorCategory.RESOURCE_ERROR: self.recovery_strategies.get(
                "resource_exhaustion"
            ),
            ErrorCategory.SYSTEM_ERROR: self.recovery_strategies.get(
                "critical_system_error"
            ),
        }
    ...
    async def _create_recovery_actions(
        self, error_context: ErrorContext, strategy_config: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
    ...
    async def _execute_recovery_action(
        self, action: Dict[str, Any], error_context: ErrorContext
    ) -> bool:
    ...
    async def get_error_recovery_status(self, engagement_id: UUID) -> Dict[str, Any]:
        ...
        active_plans = [
            ...
            if plan.error_context.engagement_id == engagement_id
            and plan.status in ["pending", "executing"]
        ]
