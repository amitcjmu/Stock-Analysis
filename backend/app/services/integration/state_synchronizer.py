...
class StateSynchronizer:
    ...
    def __init__(self):
        ...
        self.sync_rules = {
            ...
            "discovery_to_assessment": {
                ...
                "actions": [
                    "propagate_enriched_data",
                    "trigger_analysis",
                    "update_readiness",
                ],
                "required_conditions": [
                    "min_discovery_completion",
    ...
    async def synchronize_engagement_state(
        self, engagement_id: UUID, force_full_sync: bool = False
    ) -> SyncContext:
        """
        ...
        async with self.processing_lock:
            try:
                ...
                logger.error(
                    ...
                    extra={"engagement_id": str(engagement_id), "error": str(e)},
                )
                raise
        ...
        async with AsyncSessionLocal() as session:
            ...
            if collection_flow:
                context.flows["collection"] = FlowState(
                    ...
                    last_updated=collection_flow.updated_at
                    or collection_flow.created_at,
            ...
            if assessment_flow:
                context.flows["assessment"] = FlowState(
                    ...
                    last_updated=assessment_flow.updated_at
                    or assessment_flow.created_at,
        ...
        async with AsyncSessionLocal() as session:
            ...
            for asset in assets:
                context.assets_snapshot[asset.id] = {
                    ...
                    "dependency_count": (
                        len(asset.dependencies) if asset.dependencies else 0
                    ),
    ...
    async def _detect_state_conflicts(
        ...
        flow_statuses = {
            flow_type: flow.status for flow_type, flow in context.flows.items()
        }

        # Collection completed but discovery not started
        if (
            flow_statuses.get("collection") == "completed"
            and "discovery" not in context.flows
            and len(context.assets_snapshot) > 0
        ):
            conflicts.append(
                {
        ...
        if (
            flow_statuses.get("discovery") == "completed"
            and "assessment" not in context.flows
        ):
            conflicts.append(
                {
        ...
        collection_asset_count = context.flows.get("collection", {}).metadata.get(
            "asset_count", 0
        )
    ...
    async def _process_sync_queue(self, context: SyncContext) -> None:
        ...
        engagement_events = [
            event
            for event in self.sync_queue
            if event.engagement_id == context.engagement_id
    ...
    async def _handle_flow_completion(
        self, context: SyncContext, event: SyncEvent
    ) -> None:
    ...
    async def _apply_sync_rules(self, context: SyncContext) -> None:
        ...
        if (
            "collection" in context.flows
            and context.flows["collection"].status == "completed"
            and "discovery" not in context.flows
        ):
        ...
        if (
            "discovery" in context.flows
            and context.flows["discovery"].status == "completed"
            and "assessment" not in context.flows
        ):
    ...
    async def _sync_asset_counts(
        self, context: SyncContext, conflict: Dict[str, Any]
    ) -> None:
    ...
    async def _fix_progress_percentages(
        self, context: SyncContext, conflict: Dict[str, Any]
    ) -> None:
    ...
    async def get_sync_status(self, engagement_id: UUID) -> Dict[str, Any]:
        ...
        return {
            "engagement_id": str(engagement_id),
            "sync_status": (
                "synchronized" if not context.conflicts else "conflicts_detected"
            ),
            "flows": {
                flow_type: {
                    "status": flow.status,
            ...
            "last_sync": max(
                [
                    event.processed_at
                    for event in context.sync_history
