...
class SmartWorkflowContext:
    ...
    def add_phase_entry(self, phase: WorkflowPhase, status: str, metadata: Dict[str, Any] = None):
...
class SmartWorkflowOrchestrator:
    ...
    async def _handle_collection_phase(self, context: SmartWorkflowContext) -> SmartWorkflowContext:
        ...
        async with AsyncSessionLocal() as session:
            ...
            collection_flow = await self._get_collection_flow(session, context.engagement_id)
            
            if collection_flow and collection_flow.status == "completed":
                # Collection already completed, analyze quality
                quality_metrics = await self._analyze_collection_quality(session, collection_flow)
    ...
    async def _transition_to_discovery(self, context: SmartWorkflowContext) -> SmartWorkflowContext:
        ...
        if not await self._validate_quality_gates(context, "collection_to_discovery"):
            raise ValueError("Collection quality gates not met for discovery transition")
    ...
    async def _handle_discovery_phase(self, context: SmartWorkflowContext) -> SmartWorkflowContext:
        ...
        async with AsyncSessionLocal() as session:
            ...
            if discovery_flow:
                ...
                if discovery_flow.status == "completed":
                    ...
                    quality_metrics = await self._analyze_discovery_quality(session, discovery_flow)
                    context.data_quality_metrics.update(quality_metrics)
                    context.add_phase_entry(
                        WorkflowPhase.DISCOVERY,
                        "completed",
    ...
    async def _transition_to_assessment(self, context: SmartWorkflowContext) -> SmartWorkflowContext:
        ...
        if not await self._validate_quality_gates(context, "discovery_to_assessment"):
            raise ValueError("Discovery quality gates not met for assessment transition")
            
        # Initialize assessment flow if needed
        async with AsyncSessionLocal() as session:
            assessment_flow = await self._get_or_create_assessment_flow(session, context)
            context.add_phase_entry(
    ...
    async def _handle_assessment_phase(self, context: SmartWorkflowContext) -> SmartWorkflowContext:
        ...
        async with AsyncSessionLocal() as session:
            assessment_flow = await self._get_assessment_flow(session, context.engagement_id)
            
            if assessment_flow:
                if assessment_flow.status == "completed":
                    # Assessment completed
                    quality_metrics = await self._analyze_assessment_quality(session, assessment_flow)
                    context.data_quality_metrics.update(quality_metrics)
                    context.add_phase_entry(
                        WorkflowPhase.ASSESSMENT,
                        "completed",
    ...
    async def _complete_workflow(self, context: SmartWorkflowContext) -> SmartWorkflowContext:
    ...
    async def _validate_quality_gates(self, context: SmartWorkflowContext, transition: str) -> bool:
    ...
    async def _get_collection_flow(self, session: AsyncSession, engagement_id: UUID) -> Optional[CollectionFlow]:
    ...
    async def _get_discovery_flow(self, session: AsyncSession, engagement_id: UUID) -> Optional[DiscoveryFlow]:
    ...
    async def _get_assessment_flow(self, session: AsyncSession, engagement_id: UUID) -> Optional[AssessmentFlow]:
    ...
    async def _get_or_create_discovery_flow(self, session: AsyncSession, context: SmartWorkflowContext) -> DiscoveryFlow:
    ...
    async def _get_or_create_assessment_flow(self, session: AsyncSession, context: SmartWorkflowContext) -> AssessmentFlow:
    ...
    async def _analyze_collection_quality(self, session: AsyncSession, collection_flow: CollectionFlow) -> Dict[str, float]:
    ...
    async def _analyze_discovery_quality(self, session: AsyncSession, discovery_flow: DiscoveryFlow) -> Dict[str, float]:
        ...
        return {
            ...
            "data_completeness": data_completeness,
            "critical_gaps_count": critical_gaps,
            "discovery_enrichment": overall_confidence * 1.1  # Discovery should improve confidence
        }
        
    async def _analyze_assessment_quality(self, session: AsyncSession, assessment_flow: AssessmentFlow) -> Dict[str, float]:
    ...
    async def _generate_workflow_summary(self, context: SmartWorkflowContext) -> Dict[str, Any]:
        """Generate comprehensive workflow summary"""
        return {
            "engagement_id": str(context.engagement_id),
            "total_duration": (datetime.utcnow() - context.created_at).total_seconds(),
            "phases_completed": len([entry for entry in context.phase_history if entry["status"] == "completed"]),
            "final_confidence": context.data_quality_metrics.get("overall_confidence", 0.0),
            "final_completeness": context.data_quality_metrics.get("data_completeness", 0.0),
            "workflow_success": True
        }
        
    async def _handle_workflow_error(self, context: SmartWorkflowContext, error: Exception):
