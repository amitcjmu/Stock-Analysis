...
class OptimizationManager:
    ...
    async def apply_optimizations(
        ...
        for opt_id in optimization_ids:
            try:
                # Find recommendation
                recommendation = next((r for r in cached_recommendations if r.id == opt_id), None)
                if not recommendation:
                    failed_optimizations.append({"id": opt_id, "reason": "Recommendation not found"})
                    continue
                    
                # Apply optimization based on area
                success = await self._apply_single_optimization(engagement_id, user_id, recommendation)
    ...
    def get_optimization_stats(self) -> Dict[str, Any]:
        ...
        total_optimizations = sum(len(prefs) for prefs in self.user_preferences.values())
        ...
        return {
            ...
            "average_optimizations_per_user": total_optimizations / total_users if total_users > 0 else 0,
