...
class UXAnalyzer:
    ...
    async def create_optimization_context(
        ...
        async with AsyncSessionLocal() as session:
            ...
            user_behavior = await self._get_user_behavior_data(
                session, user_id, engagement_id
            )
    ...
    async def _get_performance_metrics(
        ...
        if assets:
            avg_confidence = sum(a.confidence_score or 0.0 for a in assets) / len(
                assets
            )
        ...
        for flow_type, flow in [
            ...
            if flow and flow.created_at:
                if (
                    ...
                    and hasattr(flow, "completed_at")
                    and flow.completed_at
                ):
    ...
    async def _get_historical_data(
        ...
        if discoveries:
            ...
            avg_discovery_time = (
                sum(discovery_times) / len(discovery_times) if discovery_times else 0
            )
        ...
        return {
            ...
            "user_experience_level": (
                "experienced" if len(collections) > 5 else "novice"
            ),
        }
    ...
    def _get_completed_phases(self, flows_data: Dict[str, Any]) -> List[str]:
        ...
        if (
            flows_data.get("collection_flow")
            and flows_data["collection_flow"].status == "completed"
        ):
            phases_completed.append("collection")
        if (
            flows_data.get("discovery_flow")
            and flows_data["discovery_flow"].status == "completed"
        ):
            phases_completed.append("discovery")
        if (
            flows_data.get("assessment_flow")
            and flows_data["assessment_flow"].status == "completed"
        ):
        ...
        for phase, flow in [
            ...
            if flow:
                if (
                    ...
                    time_per_phase[phase] = (
                        flow.completed_at - flow.created_at
                    ).total_seconds()
                else:
                    time_per_phase[phase] = (
                        datetime.utcnow() - flow.created_at
                    ).total_seconds()
        return time_per_phase

    def _calculate_automation_efficiency(
        self, flows_data: Dict[str, Any], user_behavior: Dict[str, Any]
    ) -> float:
            ...
            user_actions.append(
                {
                    ...
                    "duration": (
                        flow_data["last_accessed"] - flow_data["created_at"]
                    ).total_seconds(),
                }
            )
        return user_actions
