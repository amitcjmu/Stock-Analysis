...
class DiscoveryFlowCompletionService:
    ...
    async def validate_flow_completion_readiness(
        ...
        try:
            ...
            validation_results["validation_checks"]["phases_completed"] = {
                ...
                "missing": [
                    p for p in required_phases if not getattr(flow, phase, False)
                ],
            ...
            validated_count = sum(
                1 for a in assets if a.validation_status == "approved"
            )
            ...
            validation_results["validation_checks"]["asset_quality"] = {
                "migration_ready": {
                    ...
                    "percentage": (
                        (migration_ready_count / asset_count * 100)
                        if asset_count > 0
                        else 0
                    ),
                    "minimum_percentage": 50,
                    "passed": (
                        (migration_ready_count / asset_count * 100) >= 50
                        if asset_count > 0
                        else False
                    ),
                },
                "high_confidence": {
                    ...
                    "percentage": (
                        (high_confidence_count / asset_count * 100)
                        if asset_count > 0
                        else 0
                    ),
                    "minimum_percentage": 70,
                    "passed": (
                        (high_confidence_count / asset_count * 100) >= 70
                        if asset_count > 0
                        else False
                    ),
                },
                "validated": {
                    ...
                    "percentage": (
                        (validated_count / asset_count * 100) if asset_count > 0 else 0
                    ),
                    "minimum_percentage": 80,
                    "passed": (
                        (validated_count / asset_count * 100) >= 80
                        if asset_count > 0
                        else False
                    ),
                },
            ...
            if not validation_results["validation_checks"]["asset_quality"][
                "migration_ready"
            ]["passed"]:
            ...
            validation_results["validation_checks"]["data_completeness"] = {
                "assets_with_names": {
                    ...
                    "percentage": (
                        (assets_with_names / asset_count * 100)
                        if asset_count > 0
                        else 0
                    ),
                ...
                "assets_with_types": {
                    ...
                    "percentage": (
                        (assets_with_types / asset_count * 100)
                        if asset_count > 0
                        else 0
                    ),
        ...
        try:
            ...
            if filters:
                ...
                if filters.get("asset_type"):
                    query = query.where(
                        DiscoveryAsset.asset_type == filters["asset_type"]
                    )
            ...
            response = {
                ...
                "summary": {
                    ...
                    "migration_ready_percentage": (
                        (migration_ready_count / total_count * 100)
                        if total_count > 0
                        else 0
                    ),
                    "by_type": asset_types,
                    ...
                    "average_confidence": (
                        sum(a["confidence_score"] or 0 for a in asset_list)
                        / total_count
                        if total_count > 0
                        else 0
                    ),
            ...
            logger.info(
                f"✅ Retrieved {total_count} assessment-ready assets for flow: {discovery_flow_id}"
            )
            return response
    ...
    async def generate_assessment_package(
        ...
        try:
            ...
            validation_results = await self.validate_flow_completion_readiness(
                discovery_flow_id
            )
            ...
            for asset in discovery_assets:
                asset_data = {
                    ...
                    "modernization_potential": self._assess_modernization_potential(
                        asset
                    ),
                }
                ...
                assessment_package["summary"]["by_complexity"][complexity] = (
                    assessment_package["summary"]["by_complexity"].get(complexity, 0)
                    + 1
                )

                priority = asset.migration_priority or 3
                assessment_package["summary"]["by_priority"][str(priority)] = (
                    assessment_package["summary"]["by_priority"].get(str(priority), 0)
                    + 1
                )

                total_confidence += asset.confidence_score or 0.0
                assessment_package["summary"]["total_estimated_effort"] += asset_data[
            ...
            assessment_package["risk_assessment"] = self._generate_risk_assessment(
                discovery_assets
            )

            # Generate recommendations
            assessment_package["recommendations"] = self._generate_recommendations(
                discovery_assets
            )
