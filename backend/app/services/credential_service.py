...
class CredentialService:
    ...
    async def create_credential(
        ...
        try:
            ...
            encrypted_data, encryption_metadata = encrypt_credential(
                credential_data, credential_type.value
            )
            ...
            logger.info(
                f"Created credential {credential.id} for client {client_account_id}"
            )
            return credential
    ...
    async def update_credential(
        ...
        if credential_data:
            ...
            # Encrypt new data
            encrypted_data, encryption_metadata = encrypt_credential(
                credential_data, credential.credential_type.value
            )
    ...
    async def validate_credential(
        self, credential_id: uuid.UUID, user_id: uuid.UUID, platform_adapter: Any
        ...
        credential, decrypted_data = await self.get_credential(
            credential_id, user_id, decrypt_data=True, purpose="validation"
        )
        ...
        if credential.expires_at and credential.expires_at < datetime.utcnow():
            ...
            try:
                is_valid = await platform_adapter.validate_credential(
                    credential.credential_type.value, decrypted_data
                )
                validation_errors = (
                    None if is_valid else {"invalid": "Credential validation failed"}
                )
            except Exception as e:
                is_valid = False
        ...
        if not is_valid and credential.status == CredentialStatus.ACTIVE:
            credential.status = (
                ...
                if "expired" in (validation_errors or {})
    ...
    async def revoke_permission(
        self, permission_id: uuid.UUID, revoking_user_id: uuid.UUID
    ) -> None:
        """
    ...
    async def list_credentials(
        ...
        if platform_adapter_id:
            query = query.where(
                PlatformCredential.platform_adapter_id == platform_adapter_id
            )
    ...
    async def get_expiring_credentials(
        self, days_before_expiry: int = 30
    ) -> List[PlatformCredential]:
        """
    ...
    async def _check_permission(
        self, credential: PlatformCredential, user_id: uuid.UUID, permission_type: str
    ) -> bool:
        """
        ...
        for permission in credential.permissions:
            if (
                ...
                and permission.permission_type == permission_type
    ...
    def _validate_credential_data(
        self, credential_type: CredentialType, credential_data: Dict[str, Any]
    ) -> None:
        """
        Validate credential data based on type
