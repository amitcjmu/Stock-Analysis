...
class PerformanceTracker:
    ...
    def get_operation_stats(
        ...
        stats = PerformanceStats(
            ...
            avg_memory_delta_mb=statistics.mean(memory_deltas) if memory_deltas else 0.0,
    ...
    def get_performance_report(
        ...
        report = {
            ...
            "resource_usage": {
                "current_cpu_percent": self._process.cpu_percent(interval=0.1) if self._process else "N/A",
                "current_memory_mb": self._process.memory_info().rss / 1024 / 1024 if self._process else "N/A"
            }
        }
        ...
        for op_type in self._history.keys():
            ...
            recent_ops = [
                op for op in self._history[op_type]
        ...
        for op_type, thresholds in self._thresholds.items():
            recent_ops = [
                op for op in self._history.get(op_type, [])
                if op.start_time > cutoff_time
            ]
            
            for op in recent_ops:
                if op.duration_ms and op.duration_ms > thresholds.get("duration_ms", float('inf')):
    ...
    def _check_thresholds(self, operation: OperationMetrics):
        ...
        if duration_threshold and operation.duration_ms and operation.duration_ms > duration_threshold:
            violations.append(
        ...
        if cpu_threshold and operation.cpu_percent_end and operation.cpu_percent_end > cpu_threshold:
            violations.append(
