...
class BulkDataService:
    ...
    async def upload_bulk_data(
        ...
        try:
            ...
            transformed_data = await self._transform_bulk_data(
                raw_data, validation_issues
            )
            ...
            successful_rows = len(
                [
                    ...
                    if row.get("_validation_status") == "valid"
                ]
            )
            failed_rows = len(
                [
            ...
            quality_score = await self._calculate_bulk_quality_score(
            ...
            self.logger.info(
                f"Bulk upload {upload_id} completed - {successful_rows}/{total_rows} rows successful"
            )
            return result
    ...
    async def process_grid_data(
        self, grid_entries: List[GridDataEntry], form_id: str
    ) -> BulkDataProcessingResult:
        """
        ...
        try:
            self.logger.info(
                f"Processing grid data {upload_id} - {len(grid_entries)} entries"
            )
    ...
    async def generate_bulk_template(
        ...
        for attr in critical_attributes:
            ...
            validation_rules[f"field_{attr}"] = self._get_attribute_validation_rules(
                attr
            )
        ...
        if include_examples:
            example_data = self._generate_example_data(
                critical_attributes, application_types or ["web_application"]
            )
        ...
        self.logger.info(
            f"Generated bulk template {template_id} for attributes: {critical_attributes}"
        )
        return template
    ...
    def _detect_format(self, filename: str) -> BulkDataFormat:
        ...
        return self.SUPPORTED_EXTENSIONS.get(
            file_path.suffix.lower(), BulkDataFormat.CSV
        )

    async def _parse_file_content(
        self, file_content: bytes, data_format: BulkDataFormat
    ) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if len(df) > self.MAX_ROWS:
                raise ValueError(
                    f"File contains {len(df)} rows, maximum allowed is {self.MAX_ROWS}"
                )
    ...
    def _apply_template_mapping(
        self, raw_data: List[Dict[str, Any]], template: BulkTemplate
    ) -> List[Dict[str, Any]]:
    ...
    async def _validate_bulk_data(
        self, data: List[Dict[str, Any]], form_id: str
        ...
        for row_idx, row in enumerate(data, 1):
            ...
            for field_id, value in row.items():
                if field_id.startswith("field_"):
                    ...
                    issues = self._validate_attribute_value(
                        attribute_name, value, row_idx, field_id
                    )
    ...
    def _validate_attribute_value(
        self, attribute_name: str, value: Any, row_number: int, field_id: str
    ) -> List[BulkDataValidationIssue]:
    ...
    async def _calculate_bulk_quality_score(
        ...
        validation_score = max(
            0.0, 1.0 - (error_count * 0.5 + warning_count * 0.1) / total_rows
        )
    ...
    def _get_attribute_validation_rules(self, attribute_name: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "type": "string",
            "min_length": (
                1 if attribute_name in ["os_version", "business_criticality"] else 10
            ),
        }

    def _generate_example_data(
        self, critical_attributes: List[str], application_types: List[str]
    ) -> List[Dict[str, Any]]:
        ...
        for i in range(3):
            ...
            for attr in critical_attributes:
                if attr == "os_version":
                    ...
                    example[f"field_{attr}"] = ["monolith", "microservices", "layered"][
                        i
                    ]
                else:
                    example[f"field_{attr}"] = (
