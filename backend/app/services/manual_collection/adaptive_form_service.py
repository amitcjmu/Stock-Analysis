...
class AdaptiveFormService:
    ...
    async def create_adaptive_form(
        ...
        try:
            ...
            gap_lookup = {gap.get("field_name"): gap for gap in gap_context if gap.get("field_name")}
            
            # Extract sections from questionnaire data
            sections = []
            if isinstance(questionnaire_data, dict):
                # Check for nested structure
                if "questionnaire" in questionnaire_data and isinstance(questionnaire_data["questionnaire"], dict):
                ...
                # Check for flat structure with questions
                    ...
                    sections = [{
                        "section_id": "default",
                        "section_title": questionnaire_data.get("title", "Data Collection"),
            ...
            form_config = {
                ...
                "description": questionnaire_data.get("description", "Adaptive questionnaire for data collection"),
                "sections": [],
            ...
            for section in sections:
                ...
                for question in questions:
                    field_config = self._create_field_from_question(question, gap_lookup)
                    if field_config:
            ...
            self.logger.info(f"Created adaptive form with {len(form_config['sections'])} sections")
            return form_config
