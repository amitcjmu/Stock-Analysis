...
class TemplateService:
    ...
    async def create_template_from_applications(
        ...
        self.logger.info(
            f"Creating template {template_id} from {len(source_applications)} applications"
        )
        ...
        template_fields = await self._generate_template_fields(
            common_patterns, source_applications
        )

        # Calculate default values from source applications
        default_values = await self._calculate_default_values(
            source_applications, template_fields
        )
        ...
        similarity_criteria = await self._calculate_similarity_criteria(
            source_applications, template_type
        )

        # Calculate effectiveness score based on source application quality
        effectiveness_score = await self._calculate_template_effectiveness(
            source_applications
        )
        ...
        self.logger.info(
            f"Created template {template_id} with {len(template_fields)} fields"
        )
        return template
    ...
    async def find_similar_applications(
        ...
        """
        self.logger.info(
            f"Finding similar applications for {target_application.application_id}"
        )
        ...
        confidence_score = self._calculate_similarity_confidence(
            similar_apps, target_application
        )
    ...
    async def apply_template_to_application(
        ...
        self.logger.info(
            f"Applying template {template_id} to application {application_id}"
        )

        # Generate form sections from template
        sections = await self._generate_form_sections_from_template(
            ...
            required_fields=sum(
                len(
                    [
                        ...
                        if f.validation
                        and any(rule.name == "REQUIRED" for rule in f.validation.rules)
                    ]
                )
    ...
    async def get_template_recommendations(
        self, application_profile: ApplicationProfile, max_recommendations: int = 5
    ...
    async def update_template_effectiveness(
        ...
        if applications:
            avg_completion = sum(
                app.completion_percentage for app in applications
            ) / len(applications)
            avg_feedback = sum(
                app.user_feedback_score or 0.7 for app in applications
            ) / len(applications)
            template.effectiveness_score = (
                avg_completion / 100.0 * 0.7 + avg_feedback * 0.3
            )
            template.updated_at = datetime.now()
    ...
    async def _generate_template_fields(
        self, patterns: Dict[str, Any], applications: List[ApplicationProfile]
    ) -> List[TemplateField]:
    ...
    async def _calculate_default_values(
        self, applications: List[ApplicationProfile], fields: List[TemplateField]
    ) -> Dict[str, Any]:
    ...
    def _calculate_technology_similarity(
        self, app1: ApplicationProfile, app2: ApplicationProfile
    ) -> float:
    ...
    def _calculate_business_similarity(
        self, app1: ApplicationProfile, app2: ApplicationProfile
    ) -> float:
        """Calculate business context similarity"""

        criticality_sim = (
            1.0 if app1.business_criticality == app2.business_criticality else 0.5
        )

        compliance_set1 = set(app1.compliance_requirements)
        compliance_set2 = set(app2.compliance_requirements)
        compliance_sim = len(compliance_set1.intersection(compliance_set2)) / max(
            len(compliance_set1.union(compliance_set2)), 1
        )
    ...
    async def _calculate_template_relevance(
        self, application: ApplicationProfile, template: ApplicationTemplate
    ) -> float:
        ...
        if "technology_stack" in criteria:
            ...
            tech_overlap = len(template_techs.intersection(app_techs)) / max(
                len(template_techs), 1
            )
        ...
        if "compliance_requirements" in criteria:
            ...
            compliance_overlap = len(
                template_compliance.intersection(app_compliance)
            ) / max(len(template_compliance), 1)
    ...
    async def _generate_form_sections_from_template(
        self, template: ApplicationTemplate, application_id: UUID
    ) -> List[FormSection]:
        """Generate form sections from template"""
        ...
        for template_field in template.fields:
            ...
            if template_field.critical_attribute in [
                "os_version",
                "specifications",
                "network_config",
                "virtualization",
            ...
            elif template_field.critical_attribute in [
    ...
    async def _apply_user_overrides(
        self, sections: List[FormSection], overrides: Dict[str, Any]
    ) -> List[FormSection]:
    ...
    async def _generate_template_validation_rules(
        self, applications: List[ApplicationProfile]
    ) -> Dict[str, Any]:
        """Generate validation rules for template"""
        return {}

    async def _calculate_template_effectiveness(
        self, applications: List[ApplicationProfile]
    ) -> float:
        ...
        avg_quality = sum(app.collection_quality_score for app in applications) / len(
            applications
        )
        return min(1.0, avg_quality + 0.1)  # Slight boost for being template-based

    def _calculate_similarity_confidence(
        self, similar_apps: List[Tuple[UUID, float]], target_app: ApplicationProfile
    ) -> float:
        ...
        count_factor = min(
            1.0, len(similar_apps) / 5.0
        )  # Max confidence with 5+ similar apps
