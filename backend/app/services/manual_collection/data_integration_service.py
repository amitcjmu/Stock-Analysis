...
class DataIntegrationService:
    ...
    async def integrate_data_sources(
        ...
        resolved_data = await self._resolve_conflicts(conflicts, conflict_resolution_strategy)
        ...
        critical_coverage = await self._calculate_critical_attributes_coverage(resolved_data)
        
        # Determine data quality level
        quality_level = self._determine_quality_level(overall_confidence, completeness_score)
        
        # Assess readiness for 6R analysis
        readiness = await self._assess_6r_readiness(resolved_data, overall_confidence, completeness_score)
        ...
        dataset = IntegratedDataset(
            ...
            integration_metadata={
                'integration_duration_seconds': (datetime.now() - start_time).total_seconds(),
    ...
    async def get_integration_report(self, application_id: UUID) -> Optional[IntegrationReport]:
        ...
        critical_coverage = sum(
            1 for attr_name in dataset.data_points.keys()
        ...
        for source_type, source_ids in dataset.data_sources.items():
            for source_id in source_ids:
                contributed_attrs = [
                    attr_name for attr_name, data_point in dataset.data_points.items()
                    if data_point.source == source_type and data_point.source_id == source_id
                ]
        ...
        return IntegrationReport(
            ...
            integration_duration_seconds=dataset.integration_metadata.get('integration_duration_seconds', 0.0)
        )
        ...
        for attr_name, attr_config in self.CRITICAL_ATTRIBUTES.items():
            if attr_name not in collected_attributes:
                gaps.append({
                    ...
                    'priority': 'high' if attr_config['weight'] >= 0.05 else 'medium',
                    'collection_methods': self._suggest_collection_methods(attr_name)
                })
    ...
    async def _extract_data_points(self, data_sources: Dict[DataSource, List[Dict[str, Any]]]) -> List[DataPoint]:
        """Extract and normalize data points from all sources"""
        ...
        for source_type, source_data_list in data_sources.items():
            for idx, source_data in enumerate(source_data_list):
                ...
                for field_name, value in source_data.items():
                    ...
                    attr_name = field_name.replace('field_', '') if field_name.startswith('field_') else field_name
                    
                    # Calculate confidence score
                    confidence_score = self._calculate_data_point_confidence(
                        source_type, value, validation_metadata.get('field_validations', {}).get(field_name, {})
                    )
                    
                    # Normalize value
                    normalized_value = await self.data_transformation_service.normalize_collected_data({attr_name: value})
    ...
    def _group_data_by_attribute(self, data_points: List[DataPoint]) -> Dict[str, List[DataPoint]]:
    ...
    async def _detect_conflicts(self, grouped_data: Dict[str, List[DataPoint]]) -> List[DataConflict]:
        ...
        for attr_name, data_points in grouped_data.items():
            ...
            unique_values = set(str(point.normalized_value or point.value).lower().strip() for point in data_points)
            ...
            if len(unique_values) > 1:
                ...
                conflict = DataConflict(
                    ...
                    requires_user_review=self._requires_user_review(attr_name, data_points)
                )
    ...
    async def _resolve_conflicts(
        ...
        for conflict in conflicts:
            if strategy == ConflictResolutionStrategy.HIGHEST_CONFIDENCE:
                resolved_point = max(conflict.conflicting_values, key=lambda p: p.confidence_score)
            elif strategy == ConflictResolutionStrategy.MOST_RECENT:
                resolved_point = max(conflict.conflicting_values, key=lambda p: p.collected_at)
            elif strategy == ConflictResolutionStrategy.AUTOMATED_PRIORITY:
                # Prefer automated sources
                automated_points = [p for p in conflict.conflicting_values if p.source == DataSource.AUTOMATED_ADAPTER]
                if automated_points:
                    resolved_point = max(automated_points, key=lambda p: p.confidence_score)
                else:
                    resolved_point = max(conflict.conflicting_values, key=lambda p: p.confidence_score)
            elif strategy == ConflictResolutionStrategy.MANUAL_OVERRIDE:
                # Prefer manual sources
                manual_points = [p for p in conflict.conflicting_values if p.source == DataSource.MANUAL_FORM]
                if manual_points:
                    resolved_point = max(manual_points, key=lambda p: p.confidence_score)
                else:
                    resolved_point = max(conflict.conflicting_values, key=lambda p: p.confidence_score)
            else:
    ...
    async def _calculate_overall_confidence(self, data_points: Dict[str, DataPoint]) -> float:
    ...
    async def _calculate_completeness_score(self, data_points: Dict[str, DataPoint]) -> float:
        """Calculate completeness score based on critical attributes coverage"""
        
        critical_attributes_collected = sum(
            1 for attr_name in data_points.keys()
            if attr_name in self.CRITICAL_ATTRIBUTES
        )
    ...
    async def _calculate_critical_attributes_coverage(self, data_points: Dict[str, DataPoint]) -> float:
    ...
    def _determine_quality_level(self, confidence_score: float, completeness_score: float) -> DataQualityLevel:
    ...
    async def _assess_6r_readiness(
        ...
        high_impact_collected = sum(
            1 for attr_name in data_points.keys()
            if attr_name in self.CRITICAL_ATTRIBUTES and self.CRITICAL_ATTRIBUTES[attr_name]['weight'] >= 0.05
        )
        
        return (
            confidence_score >= min_confidence and
            completeness_score >= min_completeness and
            high_impact_collected >= min_high_impact_attributes
        )
    ...
    def _requires_user_review(self, attr_name: str, data_points: List[DataPoint]) -> bool:
        """Determine if conflict requires user review"""
        
        # High-impact attributes always require review
        if attr_name in self.CRITICAL_ATTRIBUTES and self.CRITICAL_ATTRIBUTES[attr_name]['weight'] >= 0.06:
            return True
        ...
        if DataSource.MANUAL_FORM in sources and DataSource.AUTOMATED_ADAPTER in sources:
            return True  # Manual vs automated always requires review
        
        return False

    def _summarize_data_sources(self, data_sources: Dict[DataSource, List[Dict[str, Any]]]) -> Dict[DataSource, List[str]]:
        """Summarize data sources"""
        summary = {}
        for source_type, source_data_list in data_sources.items():
            source_ids = [f"{source_type.value}_{idx}" for idx in range(len(source_data_list))]
    ...
    async def _generate_recommendations(self, dataset: IntegratedDataset) -> List[str]:
        ...
        if dataset.overall_confidence_score < 0.7:
            recommendations.append("Consider collecting additional manual data to improve confidence scores")
        
        if dataset.completeness_score < 70:
            recommendations.append("Focus on collecting missing critical attributes for better 6R analysis")
        
        if len(dataset.conflicts) > 0:
            unresolved_conflicts = [c for c in dataset.conflicts if not c.resolved_value]
            if unresolved_conflicts:
                recommendations.append(f"Review and resolve {len(unresolved_conflicts)} data conflicts")
        
        if not dataset.readiness_for_6r_analysis:
            recommendations.append("Data quality needs improvement before proceeding with 6R analysis")
    ...
    async def _generate_next_actions(self, dataset: IntegratedDataset) -> List[str]:
        ...
        if critical_gaps:
            actions.append(f"Collect data for high-impact attributes: {', '.join(critical_gaps[:3])}")
