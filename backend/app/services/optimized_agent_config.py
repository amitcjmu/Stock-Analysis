...
class OptimizedAgentConfigurator:
    ...
    async def get_optimized_config(
        ...
        try:
            ...
            config = await self._apply_performance_optimizations(config, operation_type, context)
            
            # Apply learning-based optimizations
            config = await self._apply_learning_optimizations(config, operation_type, context)
            ...
            logger.info(f"Generated optimized config for {operation_type} with {performance_priority} priority")
            return config
    ...
    async def _apply_performance_optimizations(
        ...
        try:
            ...
            if current_metrics.avg_duration > target_duration * 1.5:
                ...
                logger.info(f"Applied speed optimizations for {operation_type} (avg: {current_metrics.avg_duration:.1f}s)")
            ...
            if current_metrics.error_rate > 0.1:  # More than 10% error rate
                ...
                logger.info(f"Applied reliability optimizations for {operation_type} (error rate: {current_metrics.error_rate:.2f})")
    ...
    async def _apply_learning_optimizations(
        ...
        try:
            ...
            successful_patterns = [p for p in past_patterns if p.content.get("success", False)]
            ...
            if successful_patterns:
                ...
                for pattern in successful_patterns:
                    ...
                    if improvement_factor > 1.2:  # Significant improvement
                        if optimization_type == "reduced_iterations":
                            ...
                            config.cache_ttl_seconds = min(14400, int(config.cache_ttl_seconds * 1.5))
                ...
                logger.info(f"Applied {len(successful_patterns)} learned optimizations for {operation_type}")
            ...
            if learning_stats.get("field_mapping_patterns", 0) > 50:
                ...
                config.confidence_threshold = max(0.7, config.confidence_threshold - 0.1)
    ...
    def _validate_configuration(
        ...
        config.memory_similarity_threshold = max(0.5, min(0.95, config.memory_similarity_threshold))
    ...
    def get_crew_configuration(
        self,
        operation_type: str,
        agent_configs: List[AgentOptimizationConfig]
    ) -> Dict[str, Any]:
        ...
        crew_config = {
            ...
            } if any(config.enable_long_term_memory for config in agent_configs) else None
        }
    ...
    def _calculate_optimal_rpm(
        ...
        avg_iterations = sum(config.max_iterations for config in agent_configs) / len(agent_configs)
        avg_tokens = sum(config.max_tokens for config in agent_configs) / len(agent_configs)
        ...
        try:
            ...
            analysis = {
                "performance_vs_target": {
                    ...
                    "accuracy_ratio": actual_accuracy / target_accuracy if target_accuracy > 0 else 1.0,
            ...
            if actual_accuracy < target_accuracy * 0.9:
                ...
                analysis["configuration_effectiveness"]["accuracy"] = "needs_improvement"
            else:
            ...
            if actual_error_rate > 0.1:
                ...
                analysis["configuration_effectiveness"]["reliability"] = "needs_improvement"
            else:
