...
class MFOSyncAgent:
    ...
    def __init__(self, db: AsyncSession, context: RequestContext):
        ...
        self.master_repo = CrewAIFlowStateExtensionsRepository(db, context.client_account_id, context.engagement_id, context.user_id)
        self._child_repos = {}
    ...
    async def reconcile_master_status(self, master_flow_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if not child_flow:
                logger.warning(f"⚠️ [MFOSyncAgent] Child flow not found: {master_flow_id}")
                return {"success": False, "error": "Child flow not found"}
            
            # Determine master status based on child flow and database state
            new_master_status = await self._determine_master_status(master_flow, child_flow)
            ...
            if new_master_status != master_flow.flow_status:
                ...
                logger.info(f"✅ [MFOSyncAgent] Master status updated: {master_flow_id} -> {new_master_status}")
                ...
                logger.info(f"ℹ️ [MFOSyncAgent] No status change needed: {master_flow_id}")
                return {
                    "success": True,
            ...
            logger.error(f"❌ [MFOSyncAgent] Reconciliation failed: {master_flow_id} - {e}")
    ...
    async def _determine_master_status(self, master_flow, child_flow) -> str:
        ...
        db_state_complete = await self._verify_db_state_complete(master_flow.flow_id, master_flow.flow_type)
        ...
        if child_flow.status == "completed":
            if db_state_complete:
                ...
                logger.warning(f"⚠️ [MFOSyncAgent] Child completed but DB state incomplete: {master_flow.flow_id}")
            ...
            logger.warning(f"⚠️ [MFOSyncAgent] Unknown child status: {child_flow.status}")
    ...
    async def _verify_db_state_complete(self, flow_id: str, flow_type: str) -> bool:
        ...
        try:
            if flow_type == "discovery":
                ...
                logger.warning(f"⚠️ [MFOSyncAgent] Unknown flow type for DB verification: {flow_type}")
                return True  # Assume complete if we don't know how to verify
                
        except Exception as e:
            logger.error(f"❌ [MFOSyncAgent] DB state verification failed: {flow_id} - {e}")
            return False
    ...
    async def _verify_discovery_db_state(self, flow_id: str) -> bool:
        ...
        try:
            ...
            if discovery_flow.asset_inventory_completed:
                ...
                if assets_count == 0:
                    logger.warning(f"⚠️ [MFOSyncAgent] Asset inventory marked complete but no assets found: {flow_id}")
                    return False
            ...
            logger.error(f"❌ [MFOSyncAgent] Discovery DB state verification failed: {flow_id} - {e}")
            return False
    ...
    async def _verify_assessment_db_state(self, flow_id: str) -> bool:
        ...
        try:
            ...
            logger.error(f"❌ [MFOSyncAgent] Assessment DB state verification failed: {flow_id} - {e}")
            return False
    
    async def handle_child_flow_completion(self, flow_id: str, child_status: str) -> Dict[str, Any]:
        """
        Handle child flow completion events
        
        Called when a child flow reports completion or failure
        """
        logger.info(f"📢 [MFOSyncAgent] Handling child flow completion: {flow_id} -> {child_status}")
    ...
    async def monitor_flows_health(self) -> Dict[str, Any]:
        ...
        try:
            ...
            master_flows_query = select(CrewAIFlowStateExtensions).where(
                ...
                CrewAIFlowStateExtensions.flow_status.in_(["running", "paused", "completed", "failed"])
            )
            ...
            for master_flow in master_flows:
                try:
                    ...
                    expected_status = await self._determine_master_status(master_flow, child_flow)
                    if expected_status != master_flow.flow_status:
                        # Reconcile the flow
                        reconcile_result = await self.reconcile_master_status(master_flow.flow_id)
