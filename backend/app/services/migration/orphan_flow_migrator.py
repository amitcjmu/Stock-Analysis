...
class OrphanFlowMigrator:
    ...
    async def analyze_orphan_situation(self) -> Dict[str, Any]:
        ...
        try:
            async with AsyncSessionLocal() as db_session:
                ...
                client_distribution = {
                    str(row.client_account_id): row.count for row in client_result
                }
                ...
                analysis = {
                    ...
                    "orphaned_percentage": (
                        (len(orphaned_flows) / total_discovery_flows * 100)
                        if total_discovery_flows > 0
                        else 0
                    ),
                    "client_distribution": client_distribution,
                    "critical_issue": total_master_flows == 0
                    and total_discovery_flows > 0,
                    "analysis_timestamp": datetime.utcnow().isoformat(),
                }

                logger.info("📊 Orphan Analysis Results:")
                logger.info(
                    f"   Total Discovery Flows: {analysis['total_discovery_flows']}"
                )
                logger.info(f"   Total Master Flows: {analysis['total_master_flows']}")
                logger.info(
                    f"   Orphaned Flows: {analysis['orphaned_flows_count']} ({analysis['orphaned_percentage']:.1f}%)"
                )
            ...
            async with AsyncSessionLocal() as db_session:
                ...
                for flow in orphaned_flows:
                    try:
                        ...
                        error_msg = (
                            f"Failed to migrate flow {flow.flow_id}: {flow_error}"
                        )
                ...
                logger.info(
                    f"   Total Orphaned Flows: {self.migration_stats['total_orphaned_flows']}"
                )
                logger.info(
                    f"   Master Flows Created: {self.migration_stats['master_flows_created']}"
                )
    ...
    async def _migrate_single_flow(
        self, db_session: AsyncSession, flow: DiscoveryFlow, dry_run: bool
    ):
        """
        ...
        try:
            ...
            existing_extensions = await extensions_repo.get_by_flow_id_global(
                str(flow.flow_id)
            )
            if existing_extensions:
                logger.info(
                    f"   Extensions record already exists for {flow.flow_id}, skipping"
                )
                return
            ...
            if not dry_run:
                ...
                extensions_record = await extensions_repo.create_master_flow(
                    ...
                    or f"Discovery Flow {str(flow.flow_id)[:8]}",
                    flow_configuration={
                        "data_import_id": (
                ...
                logger.info(
                    f"   ✅ Created extensions record: {extensions_record.flow_id}"
                )
                self.migration_stats["master_flows_created"] += 1
            else:
                logger.info(
                    f"   [DRY RUN] Would create extensions record for: {flow.flow_id}"
                )
                self.migration_stats["master_flows_created"] += 1
            ...
            async with AsyncSessionLocal() as db_session:
                ...
                validation = {
                    ...
                    "coordination_success_rate": (
                        (linked_flows / total_discovery_flows * 100)
                        if total_discovery_flows > 0
                        else 0
                    ),
                    "architecture_healthy": remaining_orphans == 0
                    and total_master_flows == total_discovery_flows,
                    "validation_timestamp": datetime.utcnow().isoformat(),
                }

                logger.info("📊 Migration Validation Results:")
                logger.info(
                    f"   Total Discovery Flows: {validation['total_discovery_flows']}"
                )
                logger.info(
                    f"   Total Master Flows: {validation['total_master_flows']}"
                )
                logger.info(f"   Linked Flows: {validation['linked_flows']}")
                logger.info(f"   Remaining Orphans: {validation['remaining_orphans']}")
                logger.info(
                    f"   Coordination Success Rate: {validation['coordination_success_rate']:.1f}%"
                )
                logger.info(
                    f"   Architecture Healthy: {validation['architecture_healthy']}"
                )
