...
class ConfidenceManager:
    ...
    def calculate_agent_confidence(
        self, agent_id: str, factors: Dict[str, Any]
    ) -> float:
        ...
        logger.debug(
            f"🎯 {agent_id} confidence: {final_confidence:.1f}% (base: {base_confidence:.1f}%)"
        )
        return final_confidence

    def aggregate_flow_confidence(
        ...
        return {
            ...
            "confidence_statistics": {
                ...
                "std_dev": (
                    statistics.stdev(agent_confidences.values())
    ...
    def determine_escalation_triggers(
        self, agent_confidences: Dict[str, float], flow_context: Dict[str, Any] = None
    ) -> Dict[str, Any]:
    ...
    def track_confidence_improvement(
        self,
        agent_id: str,
        ...
        improvement_record = {
            ...
            "timestamp": datetime.utcnow().isoformat(),
            "effectiveness": (
                "high" if improvement > 10 else "medium" if improvement > 5 else "low"
            ),
        }
        ...
        logger.info(
            f"📈 {agent_id} confidence improved by {improvement:.1f}% after {intervention}"
        )
    ...
    async def get_page_confidence_scores(self, page: str) -> Dict[str, Any]:
        ...
        relevant_agents = page_agent_mapping.get(
            page, ["data_validation", "attribute_mapping"]
        )
        ...
        return {
            ...
            "page_specific_insights": self._get_page_specific_insights(
                page, agent_confidences
            ),
        }

    def _get_page_specific_insights(
        self, page: str, agent_confidences: Dict[str, float]
    ) -> List[Dict[str, Any]]:
