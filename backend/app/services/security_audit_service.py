...
class SecurityAuditService:
    ...
    async def log_role_change(
        ...
        if not self.is_available:
            audit_logger.warning(
                f"Role change not audited - models unavailable: {old_role} -> {new_role}"
            )
            return True
    ...
    async def log_security_violation(
        ...
        if not self.is_available:
            audit_logger.critical(
                f"SECURITY_VIOLATION not audited: {violation_type} by {user_id}"
            )
            return True
        ...
        try:
            ...
            audit_logger.critical(
                f"SECURITY_VIOLATION: {violation_type} by {user_id} - {details}"
            )
            return True
        ...
        try:
            ...
            query = select(func.count(SecurityAuditLog.id)).where(
                and_(
                    ...
                    SecurityAuditLog.details.op("->>")(text("new_role"))
                    == "platform_admin",
            ...
            if (
                admin_creations_today > 1
            ):  # More than 1 admin creation in 24h is suspicious
    ...
    async def get_security_events(
        self, user_id: str = None, event_type: str = None, hours: int = 24
    ) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            query = select(SecurityAuditLog).where(SecurityAuditLog.created_at >= since)
