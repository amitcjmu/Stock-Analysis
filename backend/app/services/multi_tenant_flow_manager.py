...
class MultiTenantFlowManager:
    ...
    def __init__(
        ...
        logger.info(
            f"✅ Multi-tenant flow manager initialized with {isolation_level.value} isolation"
        )
    ...
    async def create_tenant_flow(
        ...
        try:
            ...
            logger.info(
                f"✅ Created tenant flow {flow_id} for client {context.client_account_id}"
            )
    ...
    async def execute_tenant_flow_phase(
        ...
        try:
            ...
            # Update metrics
            await self._update_tenant_metrics(
                context.client_account_id, "phase_executed"
            )
    ...
    async def get_tenant_flows(
        ...
        try:
            ...
            tenant_flows = []
            for flow in flows:
                if await self._validate_flow_ownership(
                    context.client_account_id, flow["flow_id"]
                ):
            ...
            logger.info(
                f"✅ Retrieved {len(tenant_flows)} flows for tenant {context.client_account_id}"
            )
            return tenant_flows
    ...
    async def delete_tenant_flow(
        ...
        try:
            ...
            result = await orchestrator.delete_flow(
                flow_id=flow_id, soft_delete=soft_delete, reason=reason
            )
            ...
            logger.info(
                f"✅ Deleted tenant flow {flow_id} for client {context.client_account_id}"
            )
            return result
    ...
    async def get_tenant_metrics(self, context: RequestContext) -> Dict[str, Any]:
        ...
        try:
            ...
            quota_usage = {
                "concurrent_flows": (metrics.current_flows / quota.max_concurrent_flows)
                * 100,
                "daily_flows": (
                    metrics.total_flows_today / quota.max_total_flows_per_day
                )
                * 100,
    ...
    async def _validate_tenant_access(self, context: RequestContext) -> None:
        ...
        if not await self._is_tenant_active(context.client_account_id):
            raise TenantIsolationError(
                f"Tenant {context.client_account_id} is not active"
            )

    async def _validate_flow_access(
        self, context: RequestContext, flow_id: str
    ) -> None:
        ...
        if not await self._validate_flow_ownership(context.client_account_id, flow_id):
            raise TenantIsolationError(
                f"Tenant {context.client_account_id} does not own flow {flow_id}"
            )

    async def _validate_flow_ownership(
        self, client_account_id: int, flow_id: str
    ) -> bool:
    ...
    async def _validate_admin_access(self, context: RequestContext) -> None:
        ...
        if self.isolation_level == TenantIsolationLevel.STRICT:
            ...
            if not await self._is_platform_admin(context.user_id):
                raise TenantIsolationError(
                    "Admin access required for cross-tenant operations"
                )
    ...
    async def _check_tenant_quotas(
        self, context: RequestContext, operation: str
    ) -> None:
        ...
        if (
            ...
            and metrics.current_flows >= quota.max_concurrent_flows
        ):
        ...
        if (
            operation == "create_flow"
            and metrics.total_flows_today >= quota.max_total_flows_per_day
        ):
            raise TenantQuotaExceededError(
    ...
    async def _track_tenant_flow(
        self, client_account_id: int, flow_id: str, action: str
    ) -> None:
        ...
        async with self._metrics_lock:
            if client_account_id not in self._tenant_metrics:
                self._tenant_metrics[client_account_id] = TenantMetrics(
                    client_account_id
                )
    ...
    async def _update_tenant_metrics(
        self, client_account_id: int, operation: str
    ) -> None:
    ...
    async def _refresh_tenant_metrics_from_db(self, client_account_id: int) -> None:
        ...
        try:
            ...
            result = await self.db.execute(
                flow_count_query, {"client_account_id": client_account_id}
            )
            ...
            result = await self.db.execute(
                daily_query, {"client_account_id": client_account_id}
            )
...
async def validate_tenant_flow_access(
    context: RequestContext, flow_id: str, db: AsyncSession
) -> bool:
    """Validate tenant has access to specific flow"""
