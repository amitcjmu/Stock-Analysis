...
from .business_context_analyzer.domain_configurations import \
...
from .business_context_analyzer.enums import (BusinessDomain,
                                              MigrationDriverType,
                                              OrganizationSize,
                                              StakeholderRole)
...
class BusinessContextAnalyzer:
    ...
    def __init__(self):
        ...
        self.domain_configurations = (
            DomainConfigurationManager.get_domain_configurations()
        )
        self.stakeholder_expertise = (
            DomainConfigurationManager.get_stakeholder_expertise()
        )
        self.complexity_thresholds = (
            DomainConfigurationManager.get_complexity_thresholds()
        )
    ...
    def analyze_business_context(
        ...
        try:
            ...
            org_profile = BusinessAnalyzers.analyze_organization_profile(
                organization_info
            )
            ...
            stakeholder_landscape = BusinessAnalyzers.analyze_stakeholder_landscape(
                stakeholder_info
            )
            ...
            tech_maturity = BusinessAnalyzers.evaluate_technical_maturity(
                organization_info
            )

            # Assess cultural factors
            cultural_factors = BusinessAnalyzers.assess_cultural_factors(
            ...
            success_criteria = BusinessAnalyzers.define_success_criteria(
                migration_objectives, org_profile
            )
        ...
        try:
            ...
            for gap in prioritized_gaps:
                if gap.get("priority", 4) <= 2:  # Critical and high priority only
                    target = self.optimizer.create_questionnaire_target(
                        gap, business_context, available_stakeholders
                    )
                    if target:
                        questionnaire_targets.append(target)

            # Optimize delivery sequence
            delivery_sequence = self.optimizer.optimize_delivery_sequence(
            ...
            effort_estimates = self.optimizer.calculate_effort_estimates(
                questionnaire_targets, business_context
            )
            ...
            return {
                "targeting_strategy": {
                    "questionnaire_targets": [
                        self.optimizer.target_to_dict(target)
                        for target in questionnaire_targets
                    ],
                ...
                "stakeholder_optimization": {
                    "primary_stakeholders": self.optimizer.identify_primary_stakeholders(
                        business_context
                    ),
                    "stakeholder_workload": effort_estimates["stakeholder_workload"],
                    "capacity_constraints": self.optimizer.assess_capacity_constraints(
                        business_context
                    ),
                    "escalation_paths": self.optimizer.define_escalation_paths(
                        business_context
                    ),
                },
                "communication_strategy": communication_strategy,
                "success_metrics": {
                    "target_response_rate": self.optimizer.calculate_target_response_rate(
                        business_context
                    ),
                    "quality_thresholds": self.optimizer.define_quality_thresholds(
                        business_context
                    ),
                    "timeline_milestones": effort_estimates["milestones"],
                    "risk_mitigation": self.optimizer.identify_targeting_risks(
                        business_context
                    ),
                },
                "optimization_metadata": {
                    "analysis_timestamp": datetime.now(timezone.utc).isoformat(),
                    "business_domain": business_context.organization_profile.get(
                        "domain"
                    ),
                    "organization_size": business_context.organization_profile.get(
                        "size"
                    ),
                    "complexity_level": self.optimizer.assess_overall_complexity(
                        business_context
                    ),
                },
            }
...
def analyze_and_optimize_targeting(
    ...
    try:
        ...
        targeting_optimization = analyzer.optimize_questionnaire_targeting(
            business_context, gap_analysis, available_stakeholders
        )
        ...
        return {
            "business_context_analysis": {
                ...
                "migration_drivers": [
                    driver.value for driver in business_context.migration_drivers
                ],
        ...
        logger.error(
            f"Error in business context analysis and targeting optimization: {e}"
        )
        return {
            "error": str(e),
            "business_context_analysis": {},
