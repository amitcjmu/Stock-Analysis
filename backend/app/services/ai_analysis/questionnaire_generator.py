...
class AdaptiveQuestionnaireGenerator(BaseDiscoveryCrew):
    ...
    def create_tasks(self, inputs: Dict[str, Any]) -> List[Task]:
        ...
        if len(self.agents) > 2:
            quality_validation_task = Task(
                ...
                context=[questionnaire_design_task, business_context_task] if len(tasks) > 1 else [questionnaire_design_task]
            )
        ...
        try:
            ...
            if isinstance(raw_results, str):
                try:
                    ...
                    logger.warning(f"Could not parse JSON from questionnaire generation results: {e}")
                    parsed_results = self._parse_text_results(raw_results)
            else:
            ...
            return {
                ...
                "generation_metrics": {
                    ...
                    "estimated_duration": metadata.get("estimated_duration_minutes", 20),
                    "adaptive_branches": len(adaptive_logic.get("branching_rules", [])),
                    "dynamic_questions": len(adaptive_logic.get("dynamic_questions", []))
                },
                "quality_assessment": {
                    "question_quality_score": self._assess_question_quality(sections),
                    "user_experience_score": self._assess_user_experience(metadata, sections),
                    "data_quality_score": self._assess_data_quality(sections),
                    "business_alignment_score": self._assess_business_alignment(sections),
                    "overall_quality_score": 0  # Will be calculated
                },
                "recommendations": {
                    "deployment_readiness": self._assess_deployment_readiness(metadata, sections),
                    "optimization_suggestions": self._generate_optimization_suggestions(sections),
                    "integration_requirements": self._identify_integration_requirements(sections),
    ...
    def _parse_text_results(self, text_result: str) -> Dict[str, Any]:
        ...
        result["questionnaire_metadata"]["estimated_duration_minutes"] = max(10, question_count * 2)
        
        return result
    
    def _create_deployment_package(self, metadata: Dict, sections: List, adaptive_logic: Dict, completion_criteria: Dict) -> Dict[str, Any]:
        ...
        return {
            ...
            "integration_points": {
                "database_tables": ["collection_questionnaire_responses", "collection_data_gaps"],
        ...
        for section in sections:
            for question in section.get("questions", []):
                ...
                if question.get("validation_rules"):
                    quality_indicators += 1
                if question.get("question_type") in ["single_select", "multi_select"] and question.get("options"):
                    quality_indicators += 1
                if question.get("gap_resolution"):
                    quality_indicators += 1
        
        max_possible_indicators = total_questions * 4
        return (quality_indicators / max_possible_indicators) * 100 if max_possible_indicators > 0 else 0
    ...
    def _generate_optimization_suggestions(self, sections: List[Dict]) -> List[str]:
        ...
        if total_questions > 30:
            suggestions.append("Consider reducing total question count to improve completion rates")
        ...
        if questions_without_help > total_questions * 0.3:
            suggestions.append("Add help text to more questions to improve user experience")
        ...
        if critical_questions < total_questions * 0.2:
            suggestions.append("Consider marking more questions as critical to ensure essential data collection")
...
async def generate_adaptive_questionnaire(
    ...
    try:
        ...
        logger.info(f"Starting questionnaire generation for collection flow: {collection_flow_id}")
        results = await questionnaire_generator.kickoff_async(generation_inputs)
        
        logger.info(f"Questionnaire generation completed for collection flow: {collection_flow_id}")
