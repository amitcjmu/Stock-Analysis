...
class ConfidenceScorer:
    ...
    def _initialize_strategy_requirements(self) -> Dict[SixRStrategy, Dict[str, Any]]:
        ...
        return {
            SixRStrategy.REHOST: {
                "critical_attributes": [
                    "hostname", "os_type", "os_version", "cpu_cores", "memory_gb",
                    "storage_gb", "network_zone", "environment", "dependencies"
                ],
                "importance_weights": {
                    "infrastructure": 0.6,
                    "operational": 0.25,
            ...
            SixRStrategy.REPLATFORM: {
                "critical_attributes": [
                    "application_type", "technology_stack", "os_type", "database_type",
                    "integration_points", "data_flows", "performance_requirements"
                ],
                "importance_weights": {
                    "application": 0.4,
                    "infrastructure": 0.3,
            ...
            SixRStrategy.REFACTOR: {
                "critical_attributes": [
                    "application_name", "technology_stack", "code_complexity", 
                    "application_dependencies", "data_classification", "integration_points",
                    "business_logic_complexity", "technical_debt_score"
                ],
                "importance_weights": {
                    "application": 0.5,
                    "dependencies": 0.3,
            ...
            SixRStrategy.REPURCHASE: {
                "critical_attributes": [
                    "business_function", "user_count", "business_criticality",
                    "compliance_scope", "cost_center", "vendor_relationship",
                    "licensing_model", "integration_complexity"
                ],
                "importance_weights": {
                    "application": 0.3,
                    "operational": 0.4,
            ...
            SixRStrategy.RETIRE: {
                "critical_attributes": [
                    "business_value", "user_activity", "last_access_date",
                    "business_criticality", "dependencies", "data_retention_requirements"
                ],
                "importance_weights": {
                    "operational": 0.5,
                    "application": 0.25,
            ...
            SixRStrategy.RETAIN: {
                "critical_attributes": [
                    "business_justification", "cost_analysis", "risk_assessment",
                    "compliance_requirements", "operational_metrics", "maintenance_cost"
                ],
                "importance_weights": {
                    "operational": 0.6,
                    "application": 0.2,
    ...
    def calculate_overall_confidence(
        ...
        try:
            ...
            # Data Quality Factor
            quality_factor = self._calculate_quality_factor(asset_data, collection_metadata)
            ...
            validation_factor = self._calculate_validation_factor(asset_data, collection_metadata)
            factors.append(validation_factor)
            
            # Business Context Factor
            if business_context:
                context_factor = self._calculate_business_context_factor(asset_data, business_context)
            ...
            cross_val_factor = self._calculate_cross_validation_factor(asset_data, collection_metadata)
            ...
            recommendations = self._generate_confidence_recommendations(factors, strategy_scores, critical_gaps)
    ...
    def _calculate_completeness_factor(self, asset_data: Dict[str, Any]) -> ConfidenceFactor:
        ...
        try:
            ...
            available_attributes = set(key for key, value in asset_data.items() 
                                     if value is not None and value != "" and value != [])
            
            critical_available = available_attributes.intersection(all_critical_attributes)
            completeness_percentage = (len(critical_available) / len(all_critical_attributes)) * 100
            ...
            return ConfidenceFactor(
                ...
                evidence={
                    ...
                    "missing_critical": list(all_critical_attributes - critical_available)
                },
            ...
            return self._create_error_factor(ConfidenceFactorType.DATA_COMPLETENESS, str(e))
    
    def _calculate_quality_factor(self, asset_data: Dict[str, Any], collection_metadata: Dict[str, Any]) -> ConfidenceFactor:
        ...
        try:
            ...
            weighted_score = sum(score for _, score in quality_indicators) / total_weight if total_weight > 0 else 0
    ...
    def _calculate_reliability_factor(self, collection_metadata: Dict[str, Any]) -> ConfidenceFactor:
        ...
        try:
            ...
            collection_method = collection_metadata.get("collection_method", "manual_survey")
            ...
            reliability_score = base_reliability * tier_multipliers.get(automation_tier, 1.0)
            ...
            return self._create_error_factor(ConfidenceFactorType.SOURCE_RELIABILITY, str(e))
    
    def _calculate_validation_factor(self, asset_data: Dict[str, Any], collection_metadata: Dict[str, Any]) -> ConfidenceFactor:
        ...
        try:
            ...
            return self._create_error_factor(ConfidenceFactorType.VALIDATION_STATUS, str(e))
    
    def _calculate_business_context_factor(self, asset_data: Dict[str, Any], business_context: Dict[str, Any]) -> ConfidenceFactor:
        ...
        try:
            ...
            if business_context.get("business_criticality"):
                context_score += 25
                context_evidence["business_criticality"] = business_context["business_criticality"]
            ...
            if business_context.get("compliance_requirements") or asset_data.get("compliance_scope"):
                context_score += 20
                context_evidence["compliance_context"] = True
            
            # Business function and purpose
            if business_context.get("business_function") or asset_data.get("business_function"):
                context_score += 15
                context_evidence["business_function"] = True
            
            # Stakeholder information
            if business_context.get("stakeholders") or business_context.get("business_owner"):
                context_score += 10
            ...
            if business_context.get("strategic_importance"):
                context_score += 10
                context_evidence["strategic_importance"] = business_context["strategic_importance"]
            ...
            return self._create_error_factor(ConfidenceFactorType.BUSINESS_CONTEXT, str(e))
    
    def _calculate_freshness_factor(self, collection_metadata: Dict[str, Any]) -> ConfidenceFactor:
        ...
        try:
            ...
            return self._create_error_factor(ConfidenceFactorType.TEMPORAL_FRESHNESS, str(e))
    
    def _calculate_cross_validation_factor(self, asset_data: Dict[str, Any], collection_metadata: Dict[str, Any]) -> ConfidenceFactor:
        ...
        try:
            ...
            return self._create_error_factor(ConfidenceFactorType.CROSS_VALIDATION, str(e))
    ...
    def _calculate_strategy_scores(self, asset_data: Dict[str, Any], factors: List[ConfidenceFactor]) -> Dict[SixRStrategy, float]:
        ...
        try:
            ...
            for strategy, requirements in self.strategy_requirements.items():
                ...
                available_attrs = [attr for attr in critical_attrs 
                                 if attr in asset_data and asset_data[attr] is not None and asset_data[attr] != ""]
                
                # Calculate attribute completeness for this strategy
                attr_completeness = len(available_attrs) / len(critical_attrs) if critical_attrs else 0
                
                # Apply strategy-specific adjustments
                strategy_confidence = base_confidence * 0.7 + (attr_completeness * 100) * 0.3
    ...
    def _identify_critical_gaps(self, asset_data: Dict[str, Any], factors: List[ConfidenceFactor]) -> List[str]:
        ...
        try:
            ...
            missing_attrs = [attr for attr in all_critical_attrs 
                           if attr not in asset_data or not asset_data[attr]]
            
            if missing_attrs:
                gaps.append(f"Missing critical attributes: {', '.join(missing_attrs[:5])}")
                if len(missing_attrs) > 5:
                    gaps.append(f"... and {len(missing_attrs) - 5} more missing attributes")
            ...
            for factor in factors:
                if factor.score < 70:
                    if factor.factor_type == ConfidenceFactorType.DATA_COMPLETENESS:
                        recommendations.append("Collect missing critical attributes through targeted questionnaires")
                    elif factor.factor_type == ConfidenceFactorType.DATA_QUALITY:
                        recommendations.append("Improve data quality through validation and standardization")
                    elif factor.factor_type == ConfidenceFactorType.SOURCE_RELIABILITY:
                        recommendations.append("Verify data with more reliable sources or additional collection methods")
                    elif factor.factor_type == ConfidenceFactorType.VALIDATION_STATUS:
                        recommendations.append("Implement comprehensive data validation processes")
            
            # Strategy-specific recommendations
            low_confidence_strategies = [strategy for strategy, score in strategy_scores.items() if score < 75]
            if low_confidence_strategies:
                recommendations.append(f"Focus data collection on requirements for: {', '.join(low_confidence_strategies)}")
            
            # Gap-based recommendations
            if len(critical_gaps) > 3:
                recommendations.append("Prioritize filling critical data gaps before proceeding with migration planning")
            elif critical_gaps:
                recommendations.append("Address identified data gaps to improve migration confidence")
            
            # General recommendations
            overall_score = self._calculate_weighted_score(factors)
            if overall_score < 60:
                recommendations.append("Consider additional data collection phases before finalizing migration strategy")
            elif overall_score < 80:
                recommendations.append("Validate current data with subject matter experts")
            
        except Exception as e:
            logger.error(f"Error generating recommendations: {e}")
            recommendations.append("Review data collection process and consider additional validation")
    ...
    def _assess_data_consistency(self, asset_data: Dict[str, Any]) -> float:
        ...
        if asset_data.get("environment") == "production" and asset_data.get("business_criticality") == "low":
    ...
    def _assess_data_richness(self, asset_data: Dict[str, Any]) -> float:
        ...
        populated_fields = sum(1 for value in asset_data.values() 
                             if value is not None and value != "" and value != [])
    ...
    def _create_error_factor(self, factor_type: ConfidenceFactorType, error_msg: str) -> ConfidenceFactor:
    ...
    def _create_error_assessment(self, error_msg: str) -> ConfidenceAssessment:
        ...
        return ConfidenceAssessment(
            ...
            recommendations=["Review data collection process", "Retry confidence assessment"],
            last_updated=datetime.now(timezone.utc),
...
def calculate_collection_confidence(
    ...
    try:
        ...
        for asset in collected_assets:
            assessment = scorer.calculate_overall_confidence(asset, collection_metadata, business_context)
        ...
        for strategy in SixRStrategy:
            strategy_scores = [a.strategy_scores.get(strategy, 0) for a in asset_assessments]
            aggregate_strategy_scores[strategy] = sum(strategy_scores) / len(strategy_scores)
        ...
        common_gaps = sorted(gap_frequency.items(), key=lambda x: x[1], reverse=True)[:10]
        
        return {
            "collection_confidence": {
                "overall_score": round(avg_overall_score, 2),
                "strategy_scores": {k.value: round(v, 2) for k, v in aggregate_strategy_scores.items()},
            ...
            "common_gaps": [{"gap": gap, "frequency": freq} for gap, freq in common_gaps],
            "recommendations": [
