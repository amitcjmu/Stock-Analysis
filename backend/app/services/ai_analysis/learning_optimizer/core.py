...
class LearningOptimizer:
        ...
        try:
            ...
            for pattern_type in pattern_types:
                ...
                pattern_events = self._get_pattern_events(
                    pattern_type, analysis_window_days
                )

                if (
                    len(pattern_events)
                    < self.pattern_analyzers[pattern_type]["min_events"]
                ):
                    logger.warning(
                        f"Insufficient events for {pattern_type.value}: {len(pattern_events)}"
                    )
                    continue

                # Analyze specific pattern
                pattern_analysis = self._analyze_specific_pattern(
                    pattern_type, pattern_events
                )
                analysis_results[pattern_type.value] = pattern_analysis

                # Generate insights
                insights = self._generate_pattern_insights(
                    pattern_type, pattern_analysis, pattern_events
                )
                learning_insights.extend(insights)

            # Cross-pattern correlation analysis
            correlation_analysis = self._analyze_cross_pattern_correlations(
                analysis_results
            )
            ...
            return {
                ...
                "learning_insights": [
                    self._insight_to_dict(insight) for insight in learning_insights
                ],
    ...
    def _get_pattern_events(
        self, pattern_type: LearningPattern, analysis_window_days: Optional[int]
    ) -> List[LearningEvent]:
        """Get events for a specific pattern within the analysis window"""
        if analysis_window_days is None:
            analysis_window_days = self.pattern_analyzers[pattern_type][
    ...
    def _analyze_specific_pattern(
        self, pattern_type: LearningPattern, events: List[LearningEvent]
    ) -> Dict[str, Any]:
    ...
    def _analyze_cross_pattern_correlations(
            ...
            for i, pattern1 in enumerate(patterns):
                for j, pattern2 in enumerate(patterns[i + 1 :], i + 1):
                    correlation = calculate_pattern_correlation(
                        analysis_results[pattern1], analysis_results[pattern2]
                    )
    ...
    def _generate_optimization_recommendations(
        ...
        try:
            ...
            correlation_recommendations = generate_correlation_recommendations(
            ...
            optimized_recommendations = optimize_recommendation_set(
    ...
    def _calculate_learning_effectiveness(self) -> float:
        ...
        return min(
            1.0, (pattern_diversity * 0.6 + min(learning_velocity / 2, 1.0) * 0.4)
        )
    ...
    def _calculate_improvement_score(self) -> float:
        ...
        for pattern_type in LearningPattern:
            pattern_events = [
                e for e in self.learning_events if e.event_type == pattern_type
            ]
            if len(pattern_events) >= 3:
                ...
                if len(quality_trend) >= 2:
                    ...
                    improvement = (quality_trend[-1] - quality_trend[0]) / (
                        quality_trend[0] if quality_trend[0] > 0 else 1
                    )
    ...
    def _insight_to_dict(self, insight: LearningInsight) -> Dict[str, Any]:
        ...
        return {
            ...
            "recommendations": [
                self._recommendation_to_dict(rec) for rec in insight.recommendations
            ],
        }
...
def optimize_questionnaire_learning(
    ...
    try:
        ...
        for event_data in learning_events_data:
            try:
                pattern_type = LearningPattern(
                    event_data.get("event_type", "response_quality")
                )
        ...
        analysis_results = optimizer.analyze_learning_patterns(
            pattern_types=pattern_type_enums, analysis_window_days=analysis_window_days
        )
