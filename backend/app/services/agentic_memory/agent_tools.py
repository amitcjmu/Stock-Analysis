...
Example: {"asset_type": "database", "environment": "production", "limit": 5, "include_fields": ["name", "technology_stack", "business_criticality"]}"""
    ...
    def _run(self, input_data: str) -> str:
        ...
        try:
            ...
            import asyncio
            return asyncio.run(self._query_assets_async(
    ...
    async def _query_assets_async(self, asset_type, technology_stack, environment, business_criticality, limit, include_fields):
...
class PatternRecordingTool(BaseTool):
    ...
    def __init__(self, client_account_id: uuid.UUID, engagement_id: uuid.UUID, agent_name: str, flow_id: Optional[uuid.UUID] = None):
        super().__init__(
...
Example: {"pattern_type": "business_value_indicator", "pattern_name": "Production Database Critical Business Value", "pattern_description": "Production databases with high utilization indicate critical business value", "pattern_logic": {"environment": "production", "asset_type": "database", "cpu_utilization_percent": ">= 70"}, "confidence_score": 0.85}"""
    ...
    def _run(self, input_data: str) -> str:
        ...
        try:
            ...
            # Store pattern
            import asyncio
            pattern = asyncio.run(self.memory_manager.store_pattern_discovery(
...
class AssetEnrichmentTool(BaseTool):
    ...
    def __init__(self, client_account_id: uuid.UUID, engagement_id: uuid.UUID, agent_name: str):
        super().__init__(
...
Example: {"asset_id": "123e4567-e89b-12d3-a456-426614174000", "business_value_score": 8, "risk_assessment": "medium", "modernization_potential": "high", "reasoning": "Production database with high utilization serving critical customer applications"}"""
    ...
    def _run(self, input_data: str) -> str:
        ...
        try:
            ...
            # Perform enrichment
            import asyncio
            result = asyncio.run(self._enrich_asset_async(
    ...
    async def _enrich_asset_async(self, asset_id, business_value_score, risk_assessment, modernization_potential, cloud_readiness_score, reasoning):
        ...
        async with AsyncSessionLocal() as session:
            ...
            if asset.client_account_id != self.client_account_id or asset.engagement_id != self.engagement_id:
...
def create_agent_tools(client_account_id: uuid.UUID, engagement_id: uuid.UUID, agent_name: str, flow_id: Optional[uuid.UUID] = None) -> List[BaseTool]:
    """
    ...
    if CREWAI_TOOLS_AVAILABLE and DATABASE_AVAILABLE:
        try:
            tools.extend([
                ...
                PatternRecordingTool(client_account_id, engagement_id, agent_name, flow_id),
