            ...
            try:
                ...
                with concurrent.futures.ThreadPoolExecutor() as executor:
                    future = executor.submit(
                        asyncio.run, memory_manager.query_memory(memory_query)
                    )
...
def create_pattern_recording_tool(
        ...
        try:
            ...
            try:
                ...
                with concurrent.futures.ThreadPoolExecutor() as executor:
                    future = executor.submit(
                        asyncio.run,
                        memory_manager.store_pattern_discovery(
...
def create_asset_enrichment_tool(
    client_account_id: uuid.UUID, engagement_id: uuid.UUID, agent_name: str
):
...
async def _enrich_asset_async(
    ...
    async with AsyncSessionLocal() as session:
        ...
        if (
            asset.client_account_id != client_account_id
            or asset.engagement_id != engagement_id
        ):
...
def create_functional_agent_tools(
    client_account_id: uuid.UUID,
    engagement_id: uuid.UUID,
    agent_name: str,
    flow_id: Optional[uuid.UUID] = None,
        ...
        try:
            tools.extend(
                [
                    ...
                    create_pattern_recording_tool(
                        client_account_id, engagement_id, agent_name, flow_id
                    ),
                    create_asset_enrichment_tool(
                        client_account_id, engagement_id, agent_name
                    ),
                ]
            )
            logger.info(
                f"✅ Created {len(tools)} functional agent tools for {agent_name}"
            )
        except Exception as e:
