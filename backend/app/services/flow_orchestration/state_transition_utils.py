...
class FlowStateTransitionValidator:
    ...
    @classmethod
    def can_resume_flow(
        cls, master_flow: CrewAIFlowStateExtensions, debug_logging: bool = True
    ) -> Dict[str, Any]:
        """
        ...
        if flow_status in cls.RESUMABLE_STATUSES:
            return {
                "can_resume": True,
                "reason": f"Flow status '{flow_status}' is resumable",
            }
        ...
        if persistence_status == "reset":
            return {
                "can_resume": True,
                "reason": "Flow has been reset and can be restarted",
            }
        ...
        if flow_status in cls.TERMINAL_STATUSES:
            return {
                "can_resume": False,
                "reason": f"Flow status '{flow_status}' is terminal",
            }
        ...
        if flow_status in ["active", "processing"]:
            return {
                "can_resume": True,
                "reason": f"Flow status '{flow_status}' can continue execution",
            }

        # Default: Unknown status, be permissive but log warning
        logger.warning(
            f"⚠️ Unknown flow status '{flow_status}' for flow {flow_id}, allowing resumption"
        )
        return {
            "can_resume": True,
            "reason": f"Unknown status '{flow_status}' - allowing resumption",
        }
    ...
    def validate_status_transition(
        ...
        if target_status not in cls.VALID_FLOW_STATUSES:
            return {"valid": False, "error": f"Invalid target status: {target_status}"}
        ...
        if context.get("is_resumption", False):
            logger.info(
                f"⚡ Allowing transition during resumption: {current_status} -> {target_status}"
            )
            return {"valid": True, "error": None}
    ...
    def safe_resume_flow(
        cls, master_flow: CrewAIFlowStateExtensions, target_status: str = "active"
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            logger.info(
                f"✅ Flow {flow_id} can be safely resumed: {resume_check['reason']}"
            )
...
def safe_flow_resume_wrapper(resume_func):
    ...
    async def wrapper(*args, **kwargs):
        try:
            ...
            logger.error(
                f"   Current state: {e.details.get('current_state', 'unknown')}"
            )
