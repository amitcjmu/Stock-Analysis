...
class FlowExecutionCore:
    ...
    def __init__(
        ...
        logger.info(
            f"✅ Flow Execution Core initialized with decision agents for client {context.client_account_id}"
        )
    ...
    async def execute_phase(
        ...
        try:
            ...
            if has_crew_config:
                ...
                handler_name = (
                    phase_config.completion_handler or f"{phase_name}_handler"
                )
                handler = self.handler_registry.get_handler(handler_name)
                if not handler:
                    # If no specific handler found, create a simple successful result
                    logger.warning(
                        f"⚠️ No handler found for {handler_name}, attempting generic execution"
                    )
                    phase_result = {
                        "phase": phase_name,
                        "status": "completed",
            ...
            if post_execution_decision.next_phase != self._get_default_next_phase(
                phase_name
            ):
                logger.info(
                    f"🤖 Agent override: Proceeding to {post_execution_decision.next_phase} instead of default"
                )
            ...
            execution_time = (datetime.utcnow() - start_time).total_seconds()
            logger.error(
                f"❌ Phase execution failed for {phase_name} in flow {flow_id}: {e}"
            )
            ...
            await self.master_repo.update_flow_status(
                flow_id=flow_id, status="failed", phase_data=error_details
            )
    ...
    async def _get_flow_state(self, flow_id: str) -> Optional[Dict[str, Any]]:
