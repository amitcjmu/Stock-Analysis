...
class FlowPerformanceMonitor:
    ...
    def end_operation(
        ...
        if "initial" in metric.resource_usage:
            metric.resource_usage["delta"] = self._calculate_resource_delta(
                metric.resource_usage["initial"], final_resources
            )
        ...
        logger.debug(
            f"📊 Completed tracking {metric.operation_type} for flow {metric.flow_id}: {metric.duration_ms:.2f}ms"
        )
        return metric
    ...
    def _take_resource_snapshot(self, flow_id: str):
        ...
        try:
            ...
            if len(self.resource_snapshots[flow_id]) > 20:
                self.resource_snapshots[flow_id] = self.resource_snapshots[flow_id][
                    -20:
                ]

        except Exception as e:
            logger.warning(
                f"⚠️ Failed to take resource snapshot for flow {flow_id}: {e}"
            )

    def _calculate_resource_delta(
        self, initial_resources: Dict[str, Any], final_resources: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if "cpu_percent" in initial_resources and "cpu_percent" in final_resources:
                delta["cpu_percent_delta"] = (
                    final_resources["cpu_percent"] - initial_resources["cpu_percent"]
                )

            # Calculate memory delta
            if "memory_mb" in initial_resources and "memory_mb" in final_resources:
                delta["memory_mb_delta"] = (
                    final_resources["memory_mb"] - initial_resources["memory_mb"]
                )

            # Calculate network I/O delta
            if (
                "network_io" in initial_resources
                and "network_io" in final_resources
                and initial_resources["network_io"]
                and final_resources["network_io"]
            ):
                initial_net = initial_resources["network_io"]
                final_net = final_resources["network_io"]

                delta["network_io_delta"] = {
                    "bytes_sent_delta": final_net.get("bytes_sent", 0)
                    - initial_net.get("bytes_sent", 0),
                    "bytes_recv_delta": final_net.get("bytes_recv", 0)
                    - initial_net.get("bytes_recv", 0),
                    "packets_sent_delta": final_net.get("packets_sent", 0)
                    - initial_net.get("packets_sent", 0),
                    "packets_recv_delta": final_net.get("packets_recv", 0)
                    - initial_net.get("packets_recv", 0),
                }
    ...
    def _check_performance_thresholds(self, metric: PerformanceMetric):
        ...
        if threshold_key in self.performance_thresholds:
            ...
            if metric.duration_ms > threshold:
                warnings.append(
                    f"Operation {metric.operation_type} took {metric.duration_ms:.2f}ms (threshold: {threshold}ms)"
                )
    ...
    def get_flow_performance_summary(self, flow_id: str) -> Dict[str, Any]:
        ...
        active_metrics = [
            m for m in self.active_metrics.values() if m.flow_id == flow_id
        ]
        ...
        # Duration statistics
        durations = [
            m.duration_ms for m in completed_metrics if m.duration_ms is not None
        ]
        duration_stats = {}
        if durations:
            duration_stats = {
    ...
    def get_system_performance_overview(self) -> Dict[str, Any]:
        ...
        # Duration statistics
        durations = [
            m.duration_ms for m in all_completed_metrics if m.duration_ms is not None
        ]
        duration_stats = {}
        if durations:
            duration_stats = {
    ...
    def set_performance_threshold(self, threshold_name: str, threshold_value: float):
        ...
        logger.info(
            f"📊 Set performance threshold {threshold_name} to {threshold_value}"
        )
    ...
    def clear_flow_metrics(self, flow_id: str):
        ...
        active_to_remove = [
            tid
            for tid, metric in self.active_metrics.items()
    ...
    def record_audit_event(self, audit_entry: Dict[str, Any]):
        ...
        if audit_entry.get("operation") in ["create", "execute", "delete"]:
            logger.debug(
                f"📊 Audit event recorded: {audit_entry.get('operation')} for flow {audit_entry.get('flow_id')}"
            )
