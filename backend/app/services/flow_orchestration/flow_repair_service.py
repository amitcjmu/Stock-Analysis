...
class FlowRepairService:
    ...
    async def repair_orphaned_data(
        ...
        try:
            ...
            logger.info(f"Repair type: {repair_type}, Target items: {len(target_items)}")
            ...
            if repair_type == "link_discovery_flow":
                result = await self._repair_orphaned_discovery_flows(flow_id, target_items)
            ...
            logger.info(f"✅ Repair completed: {repair_result['items_repaired']}/{repair_result['items_processed']} items repaired")
    ...
    async def _repair_orphaned_discovery_flows(self, flow_id: str, target_items: List[str]) -> Dict[str, Any]:
            ...
            for item_id in target_items:
                try:
                    ...
                    if result.rowcount > 0:
                        items_repaired += 1
                        logger.info(f"✅ Linked discovery flow {item_id} to master flow {flow_id}")
                    else:
                        errors.append(f"Discovery flow {item_id} not found or already linked")
    ...
    async def _repair_orphaned_data_imports(self, flow_id: str, target_items: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            for item_id in target_items:
                try:
                    ...
                    if result.rowcount > 0:
                        items_repaired += 1
                        logger.info(f"✅ Linked data import {item_id} to master flow {flow_id}")
                    else:
                        errors.append(f"Data import {item_id} not found or already linked")
    ...
    async def _repair_orphaned_assets(self, flow_id: str, target_items: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            for item_id in target_items:
                try:
                    ...
                    if result.rowcount > 0:
                        items_repaired += 1
                        logger.info(f"✅ Linked asset {item_id} to master flow {flow_id}")
                    else:
