...
class FlowErrorHandler:
    ...
    async def handle_error(
        ...
        try:
            ...
            base_result = await self.base_handler.handle_error(error, context, retry_config)
            ...
            logger.info(f"🔧 Error handled: {error_classification['category']} - {error_classification['severity']}")
    ...
    def _classify_error(self, error: Exception) -> Dict[str, Any]:
        ...
        # Database errors
        if any(db_term in error_message for db_term in ["database", "sql", "connection", "session", "integrity"]):
            return {
        ...
        # Network errors
        if any(net_term in error_message for net_term in ["network", "connection", "timeout", "unreachable"]):
            return {
        ...
        # Validation errors
        if any(val_term in error_message for val_term in ["validation", "invalid", "required", "missing"]):
            return {
        ...
        # Permission errors
        if any(perm_term in error_message for perm_term in ["permission", "unauthorized", "forbidden", "access"]):
            return {
        ...
        # Timeout errors
        if any(timeout_term in error_message for timeout_term in ["timeout", "deadline", "expired"]):
            return {
        ...
        # Resource errors
        if any(res_term in error_message for res_term in ["memory", "disk", "resource", "limit", "quota"]):
            return {
    ...
    async def _apply_recovery_strategy(
        ...
        if not strategy:
            return {"status": "no_strategy", "message": f"No recovery strategy for {category}"}
    ...
    def get_error_statistics(self, flow_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        return {
            ...
            "most_common_severity": max(severities, key=severities.get) if severities else None
        }
    ...
    def clear_error_history(self, flow_id: Optional[str] = None):
        ...
        logger.info(f"🧹 Cleared error history for {'flow ' + flow_id if flow_id else 'all flows'}")