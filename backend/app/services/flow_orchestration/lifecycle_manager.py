...
class FlowLifecycleManager:
    ...
    def __init__(self, 
        ...
        logger.info(f"✅ Flow Lifecycle Manager initialized for client {context.client_account_id}")
    ...
    async def transition_flow_state(
        ...
        try:
            ...
            if master_flow.flow_status != from_state:
                logger.error(f"State mismatch for flow {flow_id}: expected {from_state}, got {master_flow.flow_status}")
                return False
            ...
            if success:
                logger.info(f"✅ State transition successful for flow {flow_id}: {from_state} -> {to_state}")
        ...
        try:
            ...
            if expected_state:
                ...
                return {
                    ...
                    "error": None if state_matches else f"State mismatch: expected {expected_state}, got {current_state}"
                }
            ...
            return {
                "valid": True,
                "current_state": current_state,
                "last_updated": master_flow.updated_at.isoformat() if master_flow.updated_at else None,
                "error": None
            }
