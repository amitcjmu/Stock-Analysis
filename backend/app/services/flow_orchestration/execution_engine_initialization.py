...
class FlowInitializer:
    ...
    async def initialize_flow_execution(
        ...
        try:
            logger.info(
                f"🚀 Initializing flow execution for {flow_type} flow: {flow_id}"
            )
            ...
            if flow_config.initialization_handler:
                handler = self.handler_registry.get_handler(
                    flow_config.initialization_handler
                )
                if handler:
                    ...
                    await self.master_repo.update_flow_status(
                        ...
                        status="initialized",
                        phase_data={
                            "initialization": self._ensure_json_serializable(
                                init_result
                            )
                        },
                    )
            ...
            if flow_type == "discovery":
                ...
                logger.warning(
                    f"⚠️ No specific initialization for flow type: {flow_type}"
                )
                return {
                    "status": "initialized",
                    "message": f"Generic initialization for {flow_type} flow",
                }
    ...
    async def _initialize_discovery_flow(
        ...
        try:
            ...
            await self._start_discovery_flow_background(
                discovery_flow, flow_id, crewai_service
            )
    ...
    async def _initialize_assessment_flow(
        ...
        try:
            ...
            assessment_config = (
                initial_state.get("assessment_config", {}) if initial_state else {}
            )
    ...
    async def _initialize_collection_flow(
        ...
        try:
            ...
            automation_tier = (
                initial_state.get("automation_tier", "tier_2")
                if initial_state
                else "tier_2"
            )
            collection_config = (
                initial_state.get("collection_config", {}) if initial_state else {}
            )
            ...
            await self._start_collection_flow_background(
                collection_flow, flow_id, crewai_service
            )
    ...
    async def _create_discovery_flow_record(
        ...
        try:
            ...
            discovery_state = UnifiedDiscoveryFlowState(
                ...
                engagement_id=(
                    str(self.context.engagement_id)
                    if self.context.engagement_id
                    else ""
                ),
    ...
    async def _start_discovery_flow_background(
        self, discovery_flow, flow_id: str, crewai_service
    ):
    ...
    async def _update_discovery_flow_status_from_result(
        self, flow_id: str, flow_result: Dict[str, Any]
    ):
    ...
    async def _update_discovery_flow_status_failed(
        self, flow_id: str, error_message: str
    ):
    ...
    async def _update_collection_flow_status_from_result(
        ...
        try:
            ...
            logger.error(
                f"❌ Failed to update collection flow status for {flow_id}: {e}"
            )

    async def _update_collection_flow_status_failed(self, flow_id: str, error: str):
        """Update collection flow status to failed"""
        try:
            phase_data = {"error": error, "failed_at": datetime.utcnow().isoformat()}
            ...
            logger.error(
                f"❌ Failed to update failed status for collection flow {flow_id}: {e}"
            )
