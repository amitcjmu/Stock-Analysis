...
class FlowAuditLogger:
    ...
    async def log_audit_event(
        ...
        try:
            ...
            event_id = f"audit_{flow_id}_{datetime.utcnow().strftime('%Y%m%d_%H%M%S_%f')}"
    ...
    def _log_to_system(self, event: AuditEvent):
        ...
        log_data = {
            ...
            "client_account_id": str(event.client_account_id) if event.client_account_id else None,
        ...
        for rule_name, rule_func in self.compliance_rules.items():
            try:
                ...
                if not compliance_result.get("compliant", True):
                    await self._handle_compliance_violation(event, rule_name, compliance_result)
            except Exception as e:
        ...
        if event.details and "configuration" in event.details:
            ...
            if isinstance(config, dict):
                for key in list(config.keys()):
                    if any(cred_key in key.lower() for cred_key in ["api_key", "secret", "password", "token"]):
                        config[key] = "***CREDENTIAL_FILTERED***"
        
        return event
    
    # Built-in compliance rules
    async def _check_data_retention_compliance(self, event: AuditEvent) -> Dict[str, Any]:
    ...
    async def _check_access_control_compliance(self, event: AuditEvent) -> Dict[str, Any]:
    ...
    async def _check_audit_completeness_compliance(self, event: AuditEvent) -> Dict[str, Any]:
    ...
    async def _check_flow_approval_compliance(self, event: AuditEvent) -> Dict[str, Any]:
        ...
        if event.operation in approval_required_operations:
            ...
            approval_recorded = event.details and event.details.get("approval_recorded", False)
    ...
    async def _check_suspicious_activity(self, event: AuditEvent) -> Dict[str, Any]:
        ...
        if event.details and any(pattern in str(event.details) for pattern in suspicious_patterns):
            return {
                "alert": True,
                "severity": "medium",
    ...
    def get_compliance_report(self, flow_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        compliance_events = [e for e in events if e.category == AuditCategory.COMPLIANCE_EVENT]
        ...
        alert_count = len([e for e in security_events if e.level == AuditLevel.CRITICAL])
        ...
        return {
            ...
            "compliance_score": (1 - violation_count / len(events)) * 100 if events else 100,
    ...
    def export_audit_log(
        ...
        if format.lower() == "json":
            ...
            csv_lines = ["timestamp,category,level,flow_id,operation,user_id,success,error_message"]
            for event in events:
                csv_lines.append(
