...
class SmartDiscoveryService:
    ...
    async def _find_related_data_by_timestamp(
        self, flow_id: str
    ...
    async def _find_related_data_by_context(
        self, flow_id: str
    ) -> Optional[Dict[str, Any]]:
    ...
    def _calculate_context_confidence(self, flow_id: str, row) -> float:
        ...
        if row.import_created and row.import_created >= datetime.now(
            timezone.utc
    ...
    async def build_status_from_discovered_data(
        ...
        try:
                ...
                status["data_sources"].append(
                    {
                        ...
                        "data_import_completed": df_data.get(
                            "data_import_completed", False
                        ),
                        "asset_inventory_completed": df_data.get(
            ...
            field_mappings = await self._retrieve_field_mappings_from_discovered_data(
                discovered_data
            )
            if field_mappings:
            ...
            return {"flow_id": flow_id, "status": "discovery_error", "error": str(e)}
    ...
    async def _find_orphaned_discovery_flows(
        self, flow_id: str
    ) -> Optional[List[Dict[str, Any]]]:
    ...
    async def _find_orphaned_data_imports(
        self, flow_id: str
    ) -> Optional[List[Dict[str, Any]]]:
    ...
    async def _find_orphaned_assets(
        self, flow_id: str
    ) -> Optional[List[Dict[str, Any]]]:
