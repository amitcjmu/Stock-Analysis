...
class DataQualityAnalyzerTool(AsyncBaseDiscoveryTool):
    ...
    description: str = "Analyze data quality metrics including completeness, accuracy, consistency"
    ...
    async def arun(
        ...
        try:
            ...
            if not analysis_data and import_id:
                analysis_data = await self._load_data_from_import(import_id, sample_size)
            ...
            quality_analysis["dimensions"]["completeness"] = self._analyze_completeness(analysis_data)
            quality_analysis["dimensions"]["consistency"] = self._analyze_consistency(analysis_data)
            quality_analysis["dimensions"]["accuracy"] = self._analyze_accuracy(analysis_data)
            quality_analysis["dimensions"]["validity"] = self._analyze_validity(analysis_data)
            quality_analysis["dimensions"]["uniqueness"] = self._analyze_uniqueness(analysis_data)
            quality_analysis["dimensions"]["conformity"] = self._analyze_conformity(analysis_data)
    ...
    async def _load_data_from_import(self, import_id: str, sample_size: int) -> List[Dict[str, Any]]:
        ...
        async with get_context_db() as db:
            ...
            return [
                json.loads(record.raw_data) if isinstance(record.raw_data, str) else record.raw_data
    ...
    def _analyze_completeness(self, data: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        overall_completeness = (total_cells - missing_cells) / total_cells if total_cells > 0 else 0
        
        # Identify problematic fields
        issues = []
        for field, stats in field_completeness.items():
            if stats["completeness_rate"] < 0.5:
                issues.append(f"Field '{field}' has high missing rate: {(1 - stats['completeness_rate']) * 100:.1f}%")
    ...
    def _analyze_consistency(self, data: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for field in data[0].keys():
            ...
            if not type_consistency:
                consistency_issues.append(f"Field '{field}' has mixed data types: {data_types}")
        ...
        passed_checks = sum(
            sum([stats["type_consistency"], stats["format_consistency"], stats["case_consistency"]])
    ...
    def _analyze_accuracy(self, data: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for field in data[0].keys():
            values = [str(record.get(field)) for record in data if record.get(field) is not None]
    ...
    def _analyze_validity(self, data: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        valid_fields = sum(
            1 for stats in field_validity.values() 
            if stats["has_values"] and stats["no_extreme_outliers"] and stats["reasonable_range"]
        )
    ...
    def _analyze_conformity(self, data: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        dominant_pattern = max(naming_patterns.items(), key=lambda x: x[1])[0] if naming_patterns else None
        
        # Check conformity for each field
        for field in field_names:
            conformity_checks = {
                "naming_standard": dominant_pattern and re.match(naming_standards[dominant_pattern], field),
                "no_special_chars": not re.search(r'[^a-zA-Z0-9_-]', field),
                "reasonable_length": 3 <= len(field) <= 50,
                "not_reserved_word": field.lower() not in ['select', 'from', 'where', 'table', 'index']
            }
            ...
            if not conformity_checks["naming_standard"]:
                conformity_issues.append(f"Field '{field}' doesn't follow standard naming convention")
        ...
        conformity_score = (passed_checks / total_checks * 100) if total_checks > 0 else 0
        ...
        for dimension, score in worst_dimensions:
            if score < 70:
                if dimension == "completeness":
                    recommendations.append("Address missing data issues before proceeding")
    ...
    def _create_summary(self, analysis: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            "overall_quality": "Excellent" if analysis["overall_score"] >= 90 else
                             "Good" if analysis["overall_score"] >= 75 else
                             "Fair" if analysis["overall_score"] >= 60 else
                             "Poor",
            ...
            "best_dimension": max(dimensions.items(), key=lambda x: x[1].get("score", 0))[0],
            "worst_dimension": min(dimensions.items(), key=lambda x: x[1].get("score", 0))[0],
    ...
    def _apply_custom_rules(self, data: List[Dict[str, Any]], rules: Dict[str, Any]) -> Dict[str, Any]:
            ...
            if rule_type == "required_field":
                ...
                if numeric_values:
                    ...
                    results["rule_results"][rule_name] = {
                        ...
                        "type": rule_type,
                        "field": field,
                        "out_of_range_count": sum(1 for v in numeric_values if not (min_val <= v <= max_val))
                    }
