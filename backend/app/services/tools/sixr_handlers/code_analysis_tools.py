...
class CodeAnalysisToolsHandler:
    ...
    def analyze_code(self, file_content: str, file_type: str, analysis_type: str = "complexity") -> Dict[str, Any]:
    ...
    def _analyze_java_code(self, content: str) -> Dict[str, Any]:
        ...
        method_count = len(re.findall(r'\b(public|private|protected).*?\w+\s*\(', content))
        ...
        complexity = 1
        if class_count > 5: complexity += 1
        if method_count > 20: complexity += 1  
        if line_count > 500: complexity += 1
        if 'throw' in content: complexity += 1
    ...
    def _analyze_python_code(self, content: str) -> Dict[str, Any]:
        ...
        # Calculate complexity
        complexity = 1
        if class_count > 3: complexity += 1
        if function_count > 15: complexity += 1
        if line_count > 300: complexity += 1
        if import_count > 15: complexity += 1
    ...
    def _analyze_dotnet_code(self, content: str) -> Dict[str, Any]:
        ...
        # Calculate complexity
        complexity = 1
        if namespace_count > 2: complexity += 1
        if class_count > 5: complexity += 1
        if method_count > 20: complexity += 1
        if line_count > 500: complexity += 1
    ...
    def _analyze_javascript_code(self, content: str) -> Dict[str, Any]:
        ...
        # Calculate complexity
        complexity = 1
        if function_count > 10: complexity += 1
        if class_count > 3: complexity += 1
        if line_count > 300: complexity += 1
        if import_count > 10: complexity += 1
