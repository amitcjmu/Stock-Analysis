...
class MappingValidatorTool(AsyncBaseDiscoveryTool):
    ...
    async def arun(
        ...
        try:
            ...
            for i, mapping in enumerate(mappings):
                ...
                validation_results["issues"].extend(mapping_validation.get("issues", []))
    ...
    async def _validate_single_mapping(
        ...
        if confidence < 0.3:
            ...
            result["warnings"].append(f"Medium confidence mapping: {confidence:.2f} - review recommended")
    ...
    def _check_type_compatibility(self, source_type: str, target_type: str) -> tuple[bool, str]:
        ...
        for base_type, compatible in compatible_types.items():
            if source_lower in compatible and target_lower in compatible:
                ...
                if (source_lower in ["float", "decimal"] and target_lower in ["int", "integer"]):
                    return True, "high"
    ...
    def _validate_field_names(self, source_field: str, target_field: str) -> List[str]:
        ...
        if any(term in source_lower for term in ["date", "time", "created", "updated"]):
            if not any(term in target_lower for term in ["date", "time", "created", "updated"]):
        ...
        if any(term in source_lower for term in critical_sources):
            if not any(term in target_lower for term in critical_targets):
                warnings.append("Critical asset field may not be mapped to appropriate target")
        
        return warnings
    
    def _check_duplicate_targets(self, mappings: List[Dict[str, Any]], results: Dict[str, Any]):
    ...
    def _check_missing_critical_fields(self, mappings: List[Dict[str, Any]], results: Dict[str, Any]):
        ...
        missing_critical = [field for field in critical_fields if field not in mapped_targets]
    ...
    def _generate_recommendations(self, results: Dict[str, Any]) -> List[str]:
        ...
        if quality_score < 60:
            recommendations.append("Consider reviewing all mappings - quality score is low")
        elif quality_score < 80:
            recommendations.append("Review mappings with warnings - moderate quality score")
        ...
        if len(results["warnings"]) > results["total_mappings"] * 0.5:
            recommendations.append("High number of warnings - consider additional field analysis")
        
        # Validation rate recommendations
        valid_rate = results["valid_mappings"] / results["total_mappings"] if results["total_mappings"] > 0 else 0
        if valid_rate < 0.8:
