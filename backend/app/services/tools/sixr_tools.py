...
class CMDBAnalysisInput(BaseModel):
    ...
    application_data: Dict[str, Any] = Field(..., description="Application data from CMDB")
    analysis_focus: str = Field(default="all", description="Focus area: technical, business, compliance, or all")
...
class CMDBAnalysisTool(BaseTool):
    ...
    def _run(self, application_data: Dict[str, Any], analysis_focus: str = "all") -> str:
    ...
    def _analyze_technical_aspects(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        technical_insights["complexity_score"] = min(10.0, technical_insights["complexity_score"])
    ...
    def _analyze_business_aspects(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "department" in data or "business_unit" in data:
            ...
            if any(critical_dept in dept for critical_dept in ["finance", "trading", "core", "customer"]):
                business_insights["strategic_importance"] = "high"
                business_insights["business_value_score"] += 1.0
        
        # Cap business value score
        business_insights["business_value_score"] = max(1.0, min(10.0, business_insights["business_value_score"]))
    ...
    def _analyze_compliance_aspects(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        compliance_insights["compliance_score"] = max(1.0, min(10.0, compliance_insights["compliance_score"]))
    ...
    def _identify_risk_indicators(self, data: Dict[str, Any]) -> List[str]:
        ...
        if "operating_system" in data:
            ...
            if any(legacy_os in os_info for legacy_os in ["windows 2008", "windows 2003", "aix", "solaris"]):
    ...
    def _recommend_initial_parameters(self, insights: Dict[str, Any]) -> Dict[str, float]:
        ...
        if "business_insights" in insights:
            params["business_value"] = insights["business_insights"].get("business_value_score", 5.0)
        
        # Set technical complexity from analysis
        if "technical_insights" in insights:
            params["technical_complexity"] = insights["technical_insights"].get("complexity_score", 5.0)
        
        # Set compliance requirements from analysis
        if "compliance_insights" in insights:
            params["compliance_requirements"] = insights["compliance_insights"].get("compliance_score", 5.0)
...
class QuestionGenerationInput(BaseModel):
    ...
    information_gaps: List[str] = Field(..., description="List of information gaps to address")
...
class QuestionGenerationTool(BaseTool):
    ...
    description: str = "Generate targeted qualifying questions to address information gaps"
    args_schema: type[BaseModel] = QuestionGenerationInput
    
    def _run(self, information_gaps: List[str], application_context: Dict[str, Any], 
             priority_focus: str = "all") -> str:
        ...
        try:
            ...
            question_templates = {
                "application_type": {
                    ...
                    "type": "select",
                    "options": [
                        {"value": "custom", "label": "Custom-built application (developed in-house)"},
                        {"value": "cots", "label": "Commercial Off-The-Shelf (COTS) application"},
                        {"value": "hybrid", "label": "Hybrid (mix of custom and COTS components)"}
                    ],
                ...
                "business_impact": {
                    ...
                    "type": "select",
                    "options": [
                        {"value": "low", "label": "Minimal impact - can be down for days"},
                        {"value": "medium", "label": "Moderate impact - can be down for hours"},
                        {"value": "high", "label": "High impact - can be down for minutes"},
                        {"value": "critical", "label": "Critical - must be always available"}
                    ],
                ...
                "technical_debt": {
                    ...
                    "options": [
                        ...
                        {"value": "very_high", "label": "Very High - mostly legacy, hard to maintain"}
                    ],
            ...
            for gap in information_gaps:
                ...
                for template_key, template in question_templates.items():
                    if template_key in gap_lower or any(keyword in gap_lower for keyword in template_key.split("_")):
                        question = {
            ...
            return json.dumps({"questions": questions, "total_count": len(questions)}, indent=2)
...
class CodeAnalysisInput(BaseModel):
    ...
    analysis_type: str = Field(default="complexity", description="Type of analysis to perform")
...
class CodeAnalysisTool(BaseTool):
    ...
    description: str = "Analyze uploaded code files to assess complexity and migration factors"
    args_schema: type[BaseModel] = CodeAnalysisInput
    
    def _run(self, file_content: str, file_type: str, analysis_type: str = "complexity") -> str:
    ...
    def _analyze_java_code(self, content: str) -> Dict[str, Any]:
        ...
        if "import java.sql" in content:
            ...
            analysis["migration_challenges"].append("Direct database access may need refactoring")
        
        if "import org.springframework" in content:
            analysis["technology_indicators"].append("Spring Framework")
            analysis["recommendations"].append("Spring applications often migrate well to cloud")
        ...
        if "@EJB" in content or "import javax.ejb" in content:
            ...
            analysis["migration_challenges"].append("EJB dependencies may require significant refactoring")
    ...
    def _analyze_python_code(self, content: str) -> Dict[str, Any]:
        ...
        if "from django" in content or "import django" in content:
            ...
            analysis["recommendations"].append("Django applications migrate well to cloud platforms")
        ...
        if any(lib in content for lib in ["pandas", "numpy", "scikit-learn", "tensorflow"]):
            analysis["technology_indicators"].append("Data Science Libraries")
            analysis["recommendations"].append("Consider cloud-native ML services for data science workloads")
    ...
    def _analyze_dotnet_code(self, content: str) -> Dict[str, Any]:
        ...
        if "using System.Web" in content:
            ...
            analysis["migration_challenges"].append(".NET Framework may require porting to .NET Core/.NET 5+")
        ...
        if "System.ServiceModel" in content:
            ...
            analysis["complexity_score"] += 2.0
            analysis["migration_challenges"].append("WCF services may need to be replaced with REST APIs")
    ...
    def _analyze_javascript_code(self, content: str) -> Dict[str, Any]:
        ...
        if "require(" in content or "import " in content:
            ...
            analysis["recommendations"].append("Node.js applications are highly cloud-compatible")
    ...
    def _analyze_generic_code(self, content: str) -> Dict[str, Any]:
        ...
        if len(content) > 10000:  # Large file
            analysis["complexity_score"] += 1.0
            analysis["migration_challenges"].append("Large file size may indicate complexity")
...
class RecommendationValidationInput(BaseModel):
    ...
    recommendation: Dict[str, Any] = Field(..., description="6R recommendation to validate")
    application_context: Dict[str, Any] = Field(..., description="Application context")
    validation_criteria: List[str] = Field(default=[], description="Specific validation criteria")
...
class RecommendationValidationTool(BaseTool):
    ...
    def _run(self, recommendation: Dict[str, Any], application_context: Dict[str, Any], 
        ...
        try:
            ...
            if confidence < 0.6:
                validation_result["warnings"].append("Low confidence score - consider gathering more information")
                validation_result["overall_status"] = "needs_review"
            
            # Strategy-specific validation
            if strategy == "retire":
                if application_context.get("business_criticality") == "high":
                    validation_result["warnings"].append("Retire recommendation for high-criticality application needs careful review")
            
            elif strategy == "rearchitect":
                if application_context.get("migration_urgency", 5) > 7:
                    validation_result["warnings"].append("Rearchitect strategy conflicts with high urgency requirements")
            
            elif strategy == "rewrite":
                if application_context.get("innovation_priority", 5) < 6:
                    validation_result["warnings"].append("Rewrite strategy requires high innovation priority and commitment")
            
            # Technical feasibility checks
            tech_complexity = application_context.get("technical_complexity", 5)
            if tech_complexity > 8 and strategy in ["refactor", "rearchitect", "rewrite"]:
                validation_result["warnings"].append("High technical complexity may increase implementation risk")
            ...
            if business_value > 7 and strategy == "retire":
                ...
                validation_result["warnings"].append("High business value conflicts with retire recommendation")
