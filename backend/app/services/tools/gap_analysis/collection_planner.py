...
class CollectionPlannerTool(AsyncBaseDiscoveryTool):
    ...
    async def arun(
        ...
        try:
            ...
            immediate_gaps = [
                g for g in prioritized_gaps if g.get("priority_score", 0) >= 80
            ]
            next_sprint_gaps = [
                g for g in prioritized_gaps if 60 <= g.get("priority_score", 0) < 80
            ]
            backlog_gaps = [
                g for g in prioritized_gaps if g.get("priority_score", 0) < 60
            ]
            ...
            collection_plan["collection_methods"] = self._define_collection_methods(
                prioritized_gaps
            )
            ...
            collection_plan["plan_summary"] = {
                ...
                "estimated_duration_weeks": sum(
                    p["duration_weeks"] for p in collection_plan["phases"]
                ),
    ...
    def _create_resource_schedule(
        self, phases: List[Dict[str, Any]], resources: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def _define_collection_methods(self, gaps: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        used_methods = set(
            g.get("recommended_source", "manual_collection") for g in gaps
        )
    ...
    def _identify_risks_and_mitigations(
        self, gaps: List[Dict[str, Any]], resources: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        ...
        # Complexity risks
        hard_gaps = [
            g for g in gaps if g.get("collection_difficulty") in ["hard", "very_hard"]
        ]
        if len(hard_gaps) > 5:
            risks.append(
                {
        ...
        # Stakeholder risks
        stakeholder_gaps = [
            g for g in gaps if "stakeholder" in g.get("recommended_source", "")
        ]
        if stakeholder_gaps:
            risks.append(
                {
    ...
    def _calculate_automation_percentage(self, gaps: List[Dict[str, Any]]) -> float:
        ...
        automated_gaps = [
            g for g in gaps if g.get("recommended_source") in automated_sources
        ]
