...
class GapIdentifierTool(AsyncBaseDiscoveryTool):
            ...
            for category, attributes in ATTRIBUTE_CATEGORIES.items():
                for attribute in attributes:
                    if attribute not in attribute_mapping:
                        ...
                        gap_results["gaps_by_priority"][gap_info["priority"]].append(
                            gap_info
                        )
                        gap_results["total_gaps"] += 1

                    elif attribute in completeness_analysis.get(
                        "attribute_completeness", {}
                    ):
                        # Check if mapped but has poor completeness
                        attr_completeness = completeness_analysis[
                            "attribute_completeness"
                        ][attribute]
                        if attr_completeness["completeness_percentage"] < 50:
                            ...
                            gap_results["gaps_by_category"][category].append(gap_info)
                            gap_results["gaps_by_priority"][
                                gap_info["priority"]
                            ].append(gap_info)
                            gap_results["total_gaps"] += 1

            # Summarize business impact
            gap_results["business_impact_summary"] = self._summarize_business_impact(
                gap_results
            )

            # Assess collection difficulty
            gap_results["collection_difficulty"] = self._assess_collection_difficulty(
                gap_results
            )
    ...
    def _summarize_business_impact(self, gap_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if critical_count >= 3:
            ...
            recommendation = (
                "Prioritize critical and high-priority gaps in the next sprint"
            )
        elif high_count >= 1:
            overall_impact = "moderate"
            risk_level = "medium"
            recommendation = (
                "Plan gap resolution activities alongside migration assessment"
            )
        else:
            overall_impact = "minor"
            risk_level = "low"
            recommendation = (
                "Address gaps as part of normal migration discovery process"
            )
        ...
        return {
            ...
            "decision_readiness": (
                "blocked" if critical_count > 0 else "proceed_with_caution"
            ),
        }

    def _assess_collection_difficulty(
        ...
        return {
            ...
            "automated_collection_possible": difficulty_counts["easy"]
            + difficulty_counts["medium"],
            "manual_collection_required": difficulty_counts["hard"]
            + difficulty_counts["very_hard"],
            "recommended_approach": (
                "hybrid"
                if difficulty_counts["easy"] > 0 and difficulty_counts["hard"] > 0
                else "manual"
            ),
        }
