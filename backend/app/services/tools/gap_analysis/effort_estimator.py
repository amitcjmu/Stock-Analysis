...
class EffortEstimatorTool(AsyncBaseDiscoveryTool):
    ...
    description: str = (
        "Estimate time and resources needed to collect missing attributes"
    )
    ...
    async def arun(
        ...
        try:
            ...
            estimation_results["timeline_estimate"] = self._estimate_timeline(
                estimation_results["total_effort_hours"], resources
            )
    ...
    def _calculate_resource_allocation(
        self, gaps: List[Dict[str, Any]], resources: Dict[str, Any], total_hours: float
    ) -> Dict[str, Any]:
        ...
        if total_work > 0:
            for skill in skill_requirements:
                skill_requirements[skill] = (
                    skill_requirements[skill] / total_work
                ) * 100

        return {
            "recommended_team_composition": skill_requirements,
            "minimum_team_size": max(
                2, len([s for s in skill_requirements.values() if s > 20])
            ),
            "optimal_team_size": min(available_resources, 4),
            "parallel_work_possible": total_hours > 80,
            "skill_gaps": [
                skill for skill, pct in skill_requirements.items() if pct > 30
            ],
        }

    def _estimate_timeline(
        self, total_hours: float, resources: Dict[str, Any]
    ) -> Dict[str, Any]:
        """Estimate project timeline"""
        team_size = resources.get("available_team_members", 2)
        hours_per_week = resources.get("hours_per_week", 40)
        efficiency_factor = resources.get(
