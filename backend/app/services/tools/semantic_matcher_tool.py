...
class SemanticMatcherTool(AsyncBaseDiscoveryTool):
    ...
    description: str = (
        "Find semantic similarities between source and target field names using ML"
    )
    ...
    async def arun(
        ...
        try:
            ...
            result = {
                ...
                "average_confidence": (
                    np.mean([m["confidence"] for m in matches]) if matches else 0.0
                ),
            }
            ...
            return json.dumps({"status": "error", "error": str(e), "matches": []})
    ...
    def _calculate_similarity_matrix(
        self, source_embeddings: np.ndarray, target_embeddings: np.ndarray
    ) -> np.ndarray:
        """Calculate cosine similarity matrix"""
        # Normalize embeddings
        source_norm = source_embeddings / (
            np.linalg.norm(source_embeddings, axis=1, keepdims=True) + 1e-8
        )
    ...
    def _find_exact_matches(
        ...
        for source in source_fields:
            ...
            source_clean = (
                source_lower.replace("_", "").replace("-", "").replace(" ", "")
            )

            for target in target_fields:
                target_lower = target.lower()
                target_clean = (
                    target_lower.replace("_", "").replace("-", "").replace(" ", "")
                )
