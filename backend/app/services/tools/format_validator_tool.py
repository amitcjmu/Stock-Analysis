...
class FormatValidatorTool(AsyncBaseDiscoveryTool):
    ...
    async def arun(
        ...
        try:
            ...
            for field in all_fields:
                ...
                # Collect issues
                validation_results["format_issues"].extend(field_validation.get("issues", []))
            
            # Generate summary and recommendations
            validation_results["summary"] = self._create_validation_summary(validation_results)
            validation_results["recommendations"] = self._generate_format_recommendations(validation_results)
    ...
    async def _validate_field_format(
        ...
        if format_rules and field in format_rules:
            ...
            validation_result.update(
                self._validate_against_pattern(values, custom_pattern, f"custom_{field}")
            )
        else:
            ...
            if detected_format != "unknown":
                format_validation = self._validate_detected_format(values, detected_format, strict_mode)
        ...
        if validation_result["total_values"] > 0:
            validation_result["format_consistency"] = validation_result["valid_values"] / validation_result["total_values"]
            validation_result["valid"] = validation_result["format_consistency"] >= (0.95 if strict_mode else 0.8)
        else:
    ...
    def _verify_format_with_samples(self, sample_values: List[str], format_type: str) -> bool:
    ...
    def _detect_format_from_content(self, sample_values: List[str]) -> str:
        ...
        boolean_count = sum(1 for v in sample_values if v.lower() in ['true', 'false', '1', '0', 'yes', 'no'])
        if boolean_count / len(sample_values) >= 0.8:
            return "boolean"
        
        return "text"
    
    def _validate_detected_format(self, values: List[Any], format_type: str, strict_mode: bool) -> Dict[str, Any]:
    ...
    def _validate_against_pattern(self, values: List[Any], pattern: str, format_name: str) -> Dict[str, Any]:
        ...
        if invalid_values:
            issues.append(f"Found {len(invalid_values)} values that don't match {format_name} format")
    ...
    def _basic_format_validation(self, values: List[Any]) -> Dict[str, Any]:
        ...
        null_like_count = sum(1 for v in values if str(v).lower() in ['null', 'none', 'n/a', 'undefined'])
        ...
        return {
            ...
            "issues": issues,
            "sample_valid": [v for v in values if str(v).strip() and str(v).lower() not in ['null', 'none', 'n/a', 'undefined']][:3],
            "sample_invalid": [v for v in values if not str(v).strip() or str(v).lower() in ['null', 'none', 'n/a', 'undefined']][:3]
        }
    ...
    def _create_validation_summary(self, results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if field_validations:
            consistencies = [v["format_consistency"] for v in field_validations.values()]
    ...
    def _generate_format_recommendations(self, results: Dict[str, Any]) -> List[str]:
        ...
        if summary["validation_rate"] < 80:
            recommendations.append("Consider data cleansing - low format validation rate")
        ...
        for field, validation in field_validations.items():
            if not validation["valid"]:
                if validation["format_consistency"] < 0.5:
                    recommendations.append(f"Field '{field}': Major format inconsistencies detected")
                else:
                    recommendations.append(f"Field '{field}': Minor format issues need attention")
        
        # Format-specific recommendations
        detected_formats = summary["detected_formats"]
        if "unknown" in detected_formats and detected_formats["unknown"] > 2:
            recommendations.append("Multiple fields have unrecognized formats - review data standards")
