...
class PlatformAdapterManager(AsyncBaseDiscoveryTool, BaseCollectionTool):
    ...
    async def arun(
        ...
        try:
            if action == "initialize":
                ...
                await self._execute_collections(
                    platforms, adapter_configs, credentials, result
                )
            elif action == "status":
    ...
    async def _initialize_adapters(
        self, platforms: List[Dict[str, Any]], result: Dict[str, Any]
    ):
        ...
        for platform in platforms:
            ...
            if adapter:
                ...
                self._add_error(
                    result, f"No adapter found for platform: {platform_type}"
                )
    ...
    async def _execute_collections(
        ...
        for platform in platforms:
            ...
            if adapter:
                config = (
                    adapter_configs.get(platform_type, {}) if adapter_configs else {}
                )
        ...
        if collection_tasks:
            ...
            for i, collection_result in enumerate(collection_results):
                if isinstance(collection_result, Exception):
                    self._add_error(
                        result,
                        f"Collection failed for {platforms[i]['name']}: {str(collection_result)}",
                    )
                else:
                    result["adapters"].append(collection_result)

    async def _get_adapter_status(
        self, platforms: List[Dict[str, Any]], result: Dict[str, Any]
    ):
        ...
        for platform in platforms:
            ...
            if adapter and hasattr(adapter, "get_status"):
                ...
                result["adapters"].append({"platform": platform_type, "status": status})

    async def _cleanup_adapters(
        self, platforms: List[Dict[str, Any]], result: Dict[str, Any]
    ):
        ...
        for platform in platforms:
            ...
            if adapter and hasattr(adapter, "cleanup"):
                ...
                result["adapters"].append(
                    {"platform": platform_type, "status": "cleaned_up"}
                )
