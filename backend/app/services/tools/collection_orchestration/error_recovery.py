...
class ErrorRecoveryManager(AsyncBaseDiscoveryTool, BaseCollectionTool):
    ...
    async def arun(
        ...
        try:
            ...
            if recovery_action == "retry":
                await self._handle_retry_action(recovery_result, platform, error_type, error_details, retry_config, fallback_strategy)
            elif recovery_action == "skip":
                self._handle_skip_action(recovery_result, platform, error_type)
            elif recovery_action == "fallback":
                await self._handle_fallback_action(recovery_result, platform, error_type, fallback_strategy)
            elif recovery_action == "manual":
                self._handle_manual_action(recovery_result, platform, error_type, error_details)
            else:
                self._add_error(recovery_result, f"Unknown recovery action: {recovery_action}")
                return recovery_result
            
            # Add error-specific recommendations
            recovery_result["recommendations"].extend(
                self._generate_error_recommendations(error_type, recovery_result["success"])
            )
    ...
    async def _handle_retry_action(
        ...
        retry_result = await self._execute_retry(platform, error_type, error_details, retry_config)
        ...
        if not recovery_result["success"] and fallback_strategy:
            fallback_result = await self._apply_fallback_strategy(platform, error_type, fallback_strategy)
            recovery_result["actions_taken"].append(fallback_result)
            recovery_result["success"] = fallback_result.get("success", False)
    
    def _handle_skip_action(self, recovery_result: Dict[str, Any], platform: str, error_type: str):
    ...
    def _handle_manual_action(
        ...
        recovery_result["actions_taken"].append({
            ...
            "platform": platform,
            "priority": priority,
            "estimated_resolution_time": self._estimate_resolution_time(error_type, priority)
        })
        recovery_result["success"] = True
    ...
    async def _execute_retry(
        ...
        for attempt in range(max_retries):
            ...
            # Calculate delay
            delay = self._calculate_backoff_delay(attempt, backoff, initial_delay, max_delay)
    ...
    async def _apply_fallback_strategy(
        ...
        if fallback_strategy == "alternative_endpoint":
            ...
            fallback_result["details"] = f"Unknown fallback strategy: {fallback_strategy}"
            fallback_result["success"] = False
        
        return fallback_result
    
    def _calculate_backoff_delay(self, attempt: int, backoff: str, initial_delay: float, max_delay: float) -> float:
    ...
    def _determine_intervention_priority(self, error_type: str, error_details: Dict[str, Any]) -> str:
    ...
    def _generate_error_recommendations(self, error_type: str, recovery_success: bool) -> List[str]:
