...
class CollectionStrategyPlanner(BaseDiscoveryTool, BaseCollectionTool):
    ...
    def run(
        ...
        try:
            ...
            self._add_resource_allocation(
                strategy, automation_tier, len(platforms), constraints
            )
    ...
    def _plan_tier3_strategy(
        self, strategy: Dict[str, Any], platforms: List[Dict[str, Any]]
    ):
    ...
    def _add_quality_requirements(
        self, strategy: Dict[str, Any], requirements: Optional[Dict[str, Any]]
    ):
