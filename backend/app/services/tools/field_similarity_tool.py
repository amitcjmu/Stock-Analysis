...
class FieldSimilarityTool(AsyncBaseDiscoveryTool):
    ...
    description: str = (
        "Calculate field similarity using multiple algorithms and pattern matching"
    )
    ...
    async def arun(
        ...
        try:
            ...
            if "lexical" in algorithms:
                similarities["lexical"] = self._calculate_lexical_similarity(
                    field1, field2
                )

            # Semantic similarity (meaning-based)
            if "semantic" in algorithms:
                similarities["semantic"] = await self._calculate_semantic_similarity(
                    field1, field2
                )
            ...
            if "phonetic" in algorithms:
                similarities["phonetic"] = self._calculate_phonetic_similarity(
                    field1, field2
                )
            ...
            if "pattern" in algorithms:
                similarities["pattern"] = self._calculate_pattern_similarity(
                    field1, field2
                )
            ...
            result = {
                ...
                "match_recommendation": self._get_match_recommendation(
                    overall_similarity
                ),
    ...
    def _calculate_structural_similarity(self, field1: str, field2: str) -> float:
        ...
        len_similarity = 1.0 - abs(len(field1) - len(field2)) / max(
        ...
        type_similarity = len(char_types1.intersection(char_types2)) / len(
            char_types1.union(char_types2)
        )
    ...
    def _calculate_contextual_similarity(
        self, field1: str, field2: str, context_data: List[Dict[str, Any]]
    ) -> float:
        ...
        pattern_sim = len(patterns1.intersection(patterns2)) / len(
            patterns1.union(patterns2)
        )
    ...
    def _generate_analysis(
        self, field1: str, field2: str, similarities: Dict[str, float]
    ) -> str:
        ...
        if similarities:
            ...
            analysis_parts.append(
                f"Strongest similarity: {best_algo[0]} ({best_algo[1]:.3f})"
            )
        ...
        return (
            " | ".join(analysis_parts)
            if analysis_parts
            else "No significant similarities found"
        )
