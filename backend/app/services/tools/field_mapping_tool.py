...
class FieldMappingTool:
    ...
    def query_field_mapping(self, field_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"Agent queried field mapping for '{field_name}': {result['source']}"
            )
            return result
    ...
    def learn_field_mapping(
        self, source_field: str, target_field: str, context: str = "agent_discovery"
    ) -> Dict[str, Any]:
        """
        ...
        """
        try:
            result = self.field_mapper.agent_learn_field_mapping(
                source_field, target_field, context
            )
            if result["success"]:
                logger.info(
                    f"Agent learned new field mapping: {source_field} → {result['canonical_field']}"
                )
            return result
        except Exception as e:
    ...
    def analyze_data_columns(
        ...
        try:
            ...
            logger.info(
                f"Agent analyzed {len(columns)} columns for {asset_type} assets"
            )
            return result
        except Exception as e:
            logger.error(f"Column analysis failed: {e}")
            return {"error": str(e), "message": "Column analysis failed"}

    def analyze_data_and_learn(
        self,
        columns: List[str],
        ...
        try:
            ...
            result = self.field_mapper.agent_analyze_columns(columns, asset_type)
            logger.info(
                f"Agent analyzed data patterns for {len(columns)} columns with {len(sample_data)} sample rows"
            )
                ...
                if confidence > 0.85:  # Auto-learn high-confidence mappings
                    ...
                    if target_field:
                        learn_result = self.learn_field_mapping(
                            column, target_field, f"auto_pattern_analysis_{asset_type}"
                        )
                        if learn_result.get("success"):
                            learned_mappings.append(
                                f"{column} → {target_field} (confidence: {confidence:.2f})"
                            )
            ...
            logger.error(
                f"Data pattern analysis and learning failed: {e}", exc_info=True
            )
            return {
                "error": str(e),
    ...
    def get_mapping_context(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"error": str(e), "message": "Failed to retrieve mapping context"}
    ...
    def suggest_mappings_for_missing_fields(
        self, available_columns: List[str], missing_fields: List[str]
    ) -> List[Dict[str, Any]]:
        """
            ...
            for missing_field in missing_fields:
                ...
                if mapping_info["confidence"] > 0:
                    ...
                    for col in available_columns:
                        if col.lower() in [
                            v.lower() for v in mapping_info["variations"]
                        ]:
                            suggestions.append(
                                {
                    ...
                    for col in available_columns:
                        ...
                        if overlap:
                            confidence = len(overlap) / max(
                                len(missing_words), len(col_words)
                            )
                            suggestions.append(
                                {
                                    "available_column": col,
