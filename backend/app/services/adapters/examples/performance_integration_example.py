...
async def demonstrate_performance_monitoring():
    ...
    await adapter_manager.register_adapter(
        "enhanced_aws_adapter", ExampleEnhancedAWSAdapter, config
    )
    ...
    print(
        f"Active Adapters: {dashboard['active_adapters']}/{dashboard['total_adapters']}"
    )
    ...
    if adapter_instance._performance_monitor:
        trends = await adapter_instance._performance_monitor.analyze_performance_trends(
            "enhanced_aws_adapter", "aws", hours=1
        )
