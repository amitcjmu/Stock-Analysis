...
class ProtocolCollectors:
    ...
    async def perform_detailed_info_gathering(
        self,
            ...
            for host in discovered_hosts:
                ...
                if creds.snmp_community or creds.snmp_username:
                    try:
                        ...
                        self.logger.debug(
                            f"SNMP gathering failed for {host.ip_address}: {str(e)}"
                        )
                ...
                if creds.ssh_username and (creds.ssh_password or creds.ssh_private_key):
                    if 22 in (host.open_ports or []):
                        try:
                            ssh_info = await self.gather_ssh_info(
                                host.ip_address, creds
                            )
                            if ssh_info:
                            ...
                            self.logger.debug(
                                f"SSH gathering failed for {host.ip_address}: {str(e)}"
                            )
                ...
                if creds.wmi_username and creds.wmi_password:
                    if 135 in (host.open_ports or []) or 445 in (host.open_ports or []):
                        try:
                            wmi_info = await self.gather_wmi_info(
                                host.ip_address, creds
                            )
                            if wmi_info:
                            ...
                            self.logger.debug(
                                f"WMI gathering failed for {host.ip_address}: {str(e)}"
                            )
    ...
    async def gather_snmp_info(
        self, ip: str, creds: OnPremisesCredentials
