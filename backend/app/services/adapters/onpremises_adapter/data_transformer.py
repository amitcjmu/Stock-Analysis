...
class DataTransformer:
    ...
    def transform_data(self, raw_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if (
                "host_discovery" in raw_data
                and "discovered_hosts" in raw_data["host_discovery"]
            ):
            ...
            if (
                "service_discovery" in raw_data
                and "service_results" in raw_data["service_discovery"]
            ):
                dependencies = self._transform_services_to_dependencies(
                    raw_data["service_discovery"]["service_results"]
                )
                normalized_data["dependencies"].extend(dependencies)

            self.logger.info(
                f"Transformed {len(normalized_data['assets'])} on-premises assets to normalized format"
            )
    ...
    def _transform_host_to_asset(
        ...
        try:
            asset = {
                ...
                "ip_addresses": {"primary": host_data.get("ip_address")},
                "discovery_method": "network_scanning",
                "discovery_timestamp": host_data.get("discovery_timestamp")
    ...
    def _transform_network_to_asset(
        ...
        try:
            ...
            self.logger.warning(
                f"Failed to transform network segment to asset: {str(e)}"
            )
            return None

    def _transform_services_to_dependencies(
        ...
        try:
            ...
            self.logger.warning(
                f"Failed to transform services to dependencies: {str(e)}"
            )
