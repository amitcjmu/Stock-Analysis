    ...
    async def collect_data(self, request: CollectionRequest) -> CollectionResponse:
        ...
        try:
            ...
            if request.configuration.get("include_port_scanning", True):
                ...
                service_discovery_data = await self.service_discovery.perform_service_discovery(
            ...
            if request.configuration.get("include_detailed_info", True):
                ...
                detailed_info_data = await self.protocol_collectors.perform_detailed_info_gathering(
                    self._discovered_hosts, creds, request.configuration
                )
                ...
                for host in self._discovered_hosts:
                    try:
                        os_info = await self.service_discovery.detect_operating_system(host)
                        if os_info:
            ...
            if request.configuration.get("include_topology", False):
                ...
                topology_data = await self.topology_discovery.perform_topology_discovery(
                    self._network_ranges, self._discovered_hosts, request.configuration
                )
