...
class GCPResourceEnhancer:
    ...
    async def enhance_asset_data(
        self, asset_type: str, assets: List[Dict], config: Dict[str, Any]
    ) -> List[Dict]:
            ...
            for asset in assets:
                ...
                if asset_type == "compute.googleapis.com/Instance":
                    enhanced_asset.update(
                        await self.enhance_compute_instance_data(asset)
                    )
    ...
    async def enhance_compute_instance_data(
        self, instance_asset: Dict[str, Any]
    ) -> Dict[str, Any]:
            ...
            if len(parts) >= 6:
                ...
                return {
                        ...
                        [
                            {
                                "name": nic.name,
                                "network": (
                                    nic.network.split("/")[-1] if nic.network else None
                                ),
                                "subnetwork": (
                                    nic.subnetwork.split("/")[-1]
                                    if nic.subnetwork
                                    else None
                                ),
                    ...
                    "scheduling": (
                        {
                            "automatic_restart": (
                                instance.scheduling.automatic_restart
                    ...
                    "tags": (
                        {
                            ...
                            "fingerprint": (
                                instance.tags.fingerprint if instance.tags else None
                            ),
                        }
                        if instance.tags
                        else {}
    ...
    async def enhance_sql_instance_data(
        self, sql_asset: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            request = sql_v1.SqlInstancesGetRequest(
                project=self.auth_manager.project_id, instance=instance_name
            )
            ...
            return {
                ...
                "settings": (
                    {
                        "activation_policy": (
                            instance.settings.activation_policy
                            if instance.settings
                            else None
                        ),
                        "authorized_gae_applications": (
                            list(instance.settings.authorized_gae_applications)
                            if instance.settings
                            and instance.settings.authorized_gae_applications
                            else []
                        ),
                        "availability_type": (
                            instance.settings.availability_type
                            if instance.settings
                            else None
                        ),
                        ...
                        "crash_safe_replication_enabled": (
                            instance.settings.crash_safe_replication_enabled
                            if instance.settings
                            else None
                        ),
                        "data_disk_size_gb": (
                        ...
                        "data_disk_type": (
                            instance.settings.data_disk_type
                            if instance.settings
                            else None
                        ),
                        "database_flags": (
                        ...
                        "maintenance_window": (
                            {
                                "day": (
                                    instance.settings.maintenance_window.day
                                    if instance.settings
                                    and instance.settings.maintenance_window
                                    else None
                                ),
                                "hour": (
                                    instance.settings.maintenance_window.hour
                                    if instance.settings
                                    and instance.settings.maintenance_window
                                    else None
                                ),
                                "update_track": (
                                    instance.settings.maintenance_window.update_track
                                    if instance.settings
                                    and instance.settings.maintenance_window
                                    else None
                                ),
                        ...
                        "pricing_plan": (
                            instance.settings.pricing_plan
                            if instance.settings
                            else None
                        ),
                        "replication_type": (
                            instance.settings.replication_type
                            if instance.settings
                            else None
                        ),
                        "storage_auto_resize": (
                            instance.settings.storage_auto_resize
                            if instance.settings
                            else None
                        ),
                        "storage_auto_resize_limit": (
                            instance.settings.storage_auto_resize_limit
                            if instance.settings
                            else None
                        ),
                        "tier": instance.settings.tier if instance.settings else None,
                        "user_labels": (
                            dict(instance.settings.user_labels)
                            if instance.settings and instance.settings.user_labels
                            else {}
                        ),
                    }
                    if instance.settings
                ...
                "failover_replica": (
                    {
                        "available": (
                            instance.failover_replica.available
                            if instance.failover_replica
                            else None
                        ),
                        "name": (
                            instance.failover_replica.name
                            if instance.failover_replica
                            else None
                        ),
                    }
                ...
                "server_ca_cert": (
                    {
                        "cert": (
                            instance.server_ca_cert.cert
                            if instance.server_ca_cert
                            else None
                        ),
                        "common_name": (
                            instance.server_ca_cert.common_name
                            if instance.server_ca_cert
                            else None
                        ),
                        "create_time": (
                            instance.server_ca_cert.create_time
                            if instance.server_ca_cert
                            else None
                        ),
                        "expiration_time": (
                            instance.server_ca_cert.expiration_time
                            if instance.server_ca_cert
                            else None
                        ),
                        "instance": (
                            instance.server_ca_cert.instance
                            if instance.server_ca_cert
                            else None
                        ),
                        "sha1_fingerprint": (
                            instance.server_ca_cert.sha1_fingerprint
                            if instance.server_ca_cert
                            else None
                        ),
                    }
    ...
    async def enhance_storage_bucket_data(
        self, bucket_asset: Dict[str, Any]
    ) -> Dict[str, Any]:
                ...
                "encryption": (
                    {
                        "default_kms_key_name": (
                            bucket.encryption.default_kms_key_name
    ...
    async def enhance_gke_cluster_data(
        self, cluster_asset: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if len(parts) >= 6:
                ...
                cluster = self.auth_manager.container_client.get_cluster(
                    request=request
                )
                ...
                return {
                    ...
                    "enable_k8s_beta_apis": (
                        {
                            "enabled_apis": (
                                list(cluster.enable_k8s_beta_apis.enabled_apis)
                                if cluster.enable_k8s_beta_apis
                                else []
                            ),
                        }
                        if cluster.enable_k8s_beta_apis
                    ...
                    "resource_labels": (
                        dict(cluster.resource_labels) if cluster.resource_labels else {}
                    ),
                    ...
                    "legacy_abac": (
                        {
                            "enabled": (
                                cluster.legacy_abac.enabled
                                if cluster.legacy_abac
                                else None
                            ),
                        }
                        if cluster.legacy_abac
                    ...
                    "network_policy": (
                        {
                            "provider": (
                                cluster.network_policy.provider
                                if cluster.network_policy
                                else None
                            ),
                            "enabled": (
                                cluster.network_policy.enabled
                                if cluster.network_policy
                                else None
                            ),
                        }
                    ...
                    "ip_allocation_policy": (
                        {
                            "use_ip_aliases": (
                                cluster.ip_allocation_policy.use_ip_aliases
                                if cluster.ip_allocation_policy
                                else None
                            ),
                            "cluster_secondary_range_name": (
                                cluster.ip_allocation_policy.cluster_secondary_range_name
                                if cluster.ip_allocation_policy
                                else None
                            ),
                            "services_secondary_range_name": (
                                cluster.ip_allocation_policy.services_secondary_range_name
                                if cluster.ip_allocation_policy
                                else None
                            ),
                            "cluster_ipv4_cidr_block": (
                                cluster.ip_allocation_policy.cluster_ipv4_cidr_block
                                if cluster.ip_allocation_policy
                                else None
                            ),
                            "services_ipv4_cidr_block": (
                                cluster.ip_allocation_policy.services_ipv4_cidr_block
                                if cluster.ip_allocation_policy
                                else None
                            ),
                        }
                            ...
                            "client_certificate_config": (
                                {
                                    "issue_client_certificate": (
                                        ...
                                        if cluster.master_auth
                                        and cluster.master_auth.client_certificate_config
                                        else None
                                    ),
                                }
                                if cluster.master_auth
                    ...
                    "network_config": (
                        {
                            "network": (
                                ...
                                if cluster.network_config
                                else None
                            ),
                            "subnetwork": (
                                cluster.network_config.subnetwork
                                if cluster.network_config
                                else None
                            ),
                            "enable_intra_node_visibility": (
                                cluster.network_config.enable_intra_node_visibility
                                if cluster.network_config
                                else None
                            ),
                        }
    ...
    async def enhance_cloud_function_data(
        self, function_asset: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "update_time": (
                    function.update_time.isoformat() if function.update_time else None
                ),
                ...
                "source_repository": (
                    {
                        "url": (
                            function.source_repository.url
                            if function.source_repository
                            else None
                        ),
                        "deployed_url": (
                            function.source_repository.deployed_url
                            if function.source_repository
                            else None
                        ),
                    }
                ...
                "https_trigger": (
                    {
                        ...
                        "security_level": (
                            function.https_trigger.security_level
                            if function.https_trigger
                            else None
                        ),
                    }
                ...
                "build_environment_variables": (
                    dict(function.build_environment_variables)
                    if function.build_environment_variables
                    else {}
                ),
