...
class GCPAssetCollector:
    ...
    async def collect_assets_with_inventory(
        self, asset_types: Set[str], config: Dict[str, Any]
    ) -> Dict[str, List[Dict]]:
        ...
        try:
            ...
            for asset in page_result:
                ...
                asset_dict = {
                    ...
                    "update_time": (
                        asset.update_time.isoformat() if asset.update_time else None
                    ),
                }
            ...
            self.logger.info(
                f"Asset Inventory collected resources across {len(asset_data)} types"
            )
    ...
    async def check_asset_type_has_resources(self, asset_type: str) -> bool:
        ...
        try:
            ...
            request = asset_v1.ListAssetsRequest(
                parent=parent, asset_types=[asset_type], page_size=1
            )
    ...
    async def get_available_resources(self, configuration: Dict[str, Any]) -> List[str]:
        ...
        try:
            ...
            for asset_type in SUPPORTED_ASSET_TYPES:
                try:
                    has_resources = await self.check_asset_type_has_resources(
                        asset_type
                    )
                    if has_resources:
                    ...
                    self.logger.warning(
                        f"Failed to check resources for {asset_type}: {str(e)}"
                    )
