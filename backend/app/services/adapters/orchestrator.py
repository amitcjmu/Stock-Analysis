...
class AdapterExecutionResult:
    ...
    def success(self) -> bool:
        return self.status == AdapterStatus.COMPLETED and self.response and self.response.success
...
class AdapterOrchestrator:
    ...
    def __init__(self, db: AsyncSession, adapter_registry: AdapterRegistry, config: Optional[OrchestrationConfig] = None):
        """
        ...
        self._thread_pool = ThreadPoolExecutor(max_workers=self.config.max_parallel_adapters)
    ...
    async def orchestrate_collection(
        ...
        self.logger.info(f"Starting orchestration {orchestration_id} for {len(platform_requests)} platforms")
        ...
        try:
            ...
            adapter_tasks = await self._prepare_adapter_tasks(platform_requests, orchestration_id)
            
            if not adapter_tasks:
                result.status = OrchestrationStatus.FAILED
                result.error_summary = {"error": "No valid adapters found for execution"}
                return result
                
            # Execute adapters in parallel with resource management
            adapter_results = await self._execute_adapters_parallel(adapter_tasks, orchestration_id)
            ...
            if result.successful_adapters > 0:
                ...
                result.duplicate_resources = aggregated_data.get("duplicate_resources", 0)
            ...
            result.platforms_collected = [r.platform for r in adapter_results if r.success]
            result.collection_methods_used = list(set([
                r.response.collection_method.value for r in adapter_results 
    ...
    async def _prepare_adapter_tasks(
        ...
        for platform, request in platform_requests.items():
            try:
                ...
                adapter_metadata_list = self.adapter_registry.get_adapters_by_platform(platform)
                ...
                if not adapter:
                    self.logger.warning(f"Failed to instantiate adapter for platform: {platform}")
                    continue
                    
                # Validate adapter configuration
                if not await self._validate_adapter_request(adapter, request):
                    self.logger.warning(f"Invalid configuration for adapter: {best_metadata.name}")
                    continue
                    
                adapter_tasks.append((adapter, request, platform))
                
            except Exception as e:
                self.logger.error(f"Failed to prepare adapter for platform {platform}: {str(e)}")
                continue
                
        return adapter_tasks
        
    async def _validate_adapter_request(self, adapter: BaseAdapter, request: CollectionRequest) -> bool:
        ...
        try:
            ...
            if request.configuration.get("test_connectivity", True):
                if not await adapter.test_connectivity(request.configuration):
                    self.logger.warning(f"Connectivity test failed for adapter: {adapter.metadata.name}")
    ...
    async def _execute_single_adapter(
        ...
        async with self._execution_semaphore:
            try:
                self.logger.info(f"Starting adapter {adapter.metadata.name} for platform {platform}")
                ...
                if response and response.success:
                    ...
                    result.status = AdapterStatus.FAILED
                    result.error = response.error_message if response else "Unknown error"
                    self.logger.error(f"Adapter {adapter.metadata.name} failed: {result.error}")
                ...
                result.status = AdapterStatus.FAILED
                result.error = str(e)
                self.logger.error(f"Adapter {adapter.metadata.name} failed with exception: {str(e)}")
    ...
    async def _check_resource_limits(self) -> bool:
        ...
        try:
            ...
            # Check CPU limit
            if current_metrics.get("cpu_usage_percent", 0) > self.config.cpu_limit_percent:
                self.logger.warning("CPU limit exceeded, waiting before execution")
                await asyncio.sleep(5)  # Brief wait to allow CPU to settle
                
            # Check disk space
            if current_metrics.get("available_disk_mb", float('inf')) < self.config.disk_space_threshold_mb:
    ...
    async def _aggregate_adapter_results(self, adapter_results: List[AdapterExecutionResult]) -> Dict[str, Any]:
        ...
        try:
            aggregated_data = {
                ...
                "metadata": {
                    "orchestration_summary": {
                        ...
                        "successful_adapters": len([r for r in adapter_results if r.success]),
                        "platforms_collected": []
                    }
                }
            }
            ...
            for adapter_result in adapter_results:
                ...
                aggregated_data["metadata"]["orchestration_summary"]["platforms_collected"].append(adapter_result.platform)
                
                # Transform and collect assets
                transformed_data = self._get_adapter_instance(adapter_result).transform_data(response_data)
                ...
                if "performance_metrics" in transformed_data:
                    platform_metrics[adapter_result.platform] = transformed_data["performance_metrics"]
            ...
            if self.config.enable_result_deduplication and all_assets:
                ...
                aggregated_data["unique_resources"] = len(deduplicated_assets["unique_assets"])
                aggregated_data["duplicate_resources"] = len(deduplicated_assets["duplicates"])
            else:
            ...
            if self.config.enable_cross_platform_correlation:
                correlations = await self._correlate_cross_platform_assets(aggregated_data["assets"])
    ...
    def _get_adapter_instance(self, adapter_result: AdapterExecutionResult) -> BaseAdapter:
    ...
    def _calculate_asset_similarity(self, asset1: Dict[str, Any], asset2: Dict[str, Any]) -> float:
        ...
        try:
            ...
            if asset1.get("ip_addresses") and asset2.get("ip_addresses"):
                total_checks += 1
                ip1 = asset1["ip_addresses"].get("primary") or asset1["ip_addresses"].get("private")
                ip2 = asset2["ip_addresses"].get("primary") or asset2["ip_addresses"].get("private")
    ...
    def _merge_similar_assets(self, similar_assets: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        try:
            ...
            merged_asset["merged_from_platforms"] = [asset.get("platform") for asset in similar_assets]
    ...
    async def _correlate_cross_platform_assets(self, assets: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    def _find_network_correlations(self, assets1: List[Dict], assets2: List[Dict]) -> List[Dict]:
        ...
        try:
            ...
            for asset in assets:
                ...
                for service in services:
                    service_key = f"{service.get('service', 'unknown')}:{service.get('port', 0)}"
    ...
    def _extract_primary_ip(self, asset: Dict[str, Any]) -> Optional[str]:
        ...
        return (
            ip_addresses.get("primary") or
            ip_addresses.get("private") or
            ip_addresses.get("public") or
            asset.get("unique_id") if self._is_ip_address(asset.get("unique_id")) else None
        )
...
class ResourceMonitor:
    ...
    async def stop_monitoring(self) -> Dict[str, Any]:
        ...
        return {
            "peak_memory_mb": max(m.get("memory_usage_mb", 0) for m in self._metrics_history),
            "peak_cpu_percent": max(m.get("cpu_usage_percent", 0) for m in self._metrics_history),
            "avg_memory_mb": sum(m.get("memory_usage_mb", 0) for m in self._metrics_history) / len(self._metrics_history),
            "avg_cpu_percent": sum(m.get("cpu_usage_percent", 0) for m in self._metrics_history) / len(self._metrics_history),
