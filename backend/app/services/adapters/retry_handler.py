...
class ErrorClassifier:
    ...
    def _initialize_error_patterns(self) -> List[ErrorPattern]:
        ...
        return [
            ...
            ErrorPattern(
                ...
                base_delay=0,
                max_delay=0,
                keywords=["authentication", "auth", "login", "credentials", "unauthorized", "401"],
            ...
            ErrorPattern(
                ...
                base_delay=0,
                max_delay=0,
                keywords=["authorization", "forbidden", "access denied", "permission", "403"],
                exception_types=[PermissionError]
            ),
            ...
            ErrorPattern(
                ...
                base_delay=2.0,
                max_delay=30.0,
                keywords=["connection", "network", "dns", "host", "unreachable", "timeout"],
            ...
            ErrorPattern(
                ...
                base_delay=5.0,
                max_delay=120.0,
                keywords=["rate limit", "throttle", "quota", "429", "too many requests"],
                exception_types=[]
            ),
    ...
    def classify_error(self, error: Exception, context: Dict[str, Any] = None) -> ErrorPattern:
...
class RetryHandler:
        ...
        **kwargs
        ...
        while retry_count <= config.max_retries:
            try:
                ...
                if config.circuit_breaker_enabled and error_pattern.should_circuit_break:
                    await self._record_failure(circuit_key)
                    
                # Check if we should retry
                if retry_count >= config.max_retries or error_pattern.retry_strategy == RetryStrategy.NO_RETRY:
                    self.logger.error(
        ...
        raise MaxRetriesExceededError(f"Unexpected retry loop exit. Last error: {str(last_error)}")
    ...
    async def _check_circuit_breaker(self, circuit_key: str):
        ...
        if state.next_attempt_time and datetime.utcnow() >= state.next_attempt_time:
            ...
            state.is_open = False
            self.logger.info(f"Circuit breaker for {circuit_key} entering half-open state")
            return
    ...
    async def _record_success(self, circuit_key: str):
        ...
        if state.is_open:
            ...
            self.logger.info(f"Circuit breaker for {circuit_key} closed after successful execution")
    ...
    async def _record_failure(self, circuit_key: str):
        ...
        if state.failure_count >= self.config.circuit_breaker_threshold:
            state.is_open = True
            state.next_attempt_time = datetime.utcnow() + timedelta(seconds=self.config.circuit_breaker_timeout)
            self.logger.warning(
    ...
    def get_circuit_breaker_status(self, circuit_key: Optional[str] = None) -> Dict[str, Any]:
    ...
    def get_error_summary(self, hours: int = 24) -> Dict[str, Any]:
        ...
        for error in errors:
            ...
            summary["adapter_errors"][adapter] = summary["adapter_errors"].get(adapter, 0) + 1
            
            platform = error.platform
            summary["platform_errors"][platform] = summary["platform_errors"].get(platform, 0) + 1
...
class AdapterErrorHandler:
    ...
    async def handle_collection_request(
        ...
        try:
            ...
            if not isinstance(response, CollectionResponse):
                raise ValueError(f"Adapter returned invalid response type: {type(response)}")
                
            return response
            
        except CircuitBreakerOpenError as e:
            self.logger.error(f"Circuit breaker open for {adapter_name}:{platform}: {str(e)}")
            ...
            self.logger.error(f"Max retries exceeded for {adapter_name}:{platform}: {str(e)}")
            return CollectionResponse(
                success=False,
            ...
            self.logger.error(f"Unexpected error in {adapter_name}:{platform}: {str(e)}")
    ...
    def get_health_status(self) -> Dict[str, Any]:
        ...
        critical_errors = error_summary.get("severity_distribution", {}).get("critical", 0)
        open_circuits = sum(
            1 for state in circuit_status.get("circuit_breakers", {}).values()
        ...
        return {
            ...
            "recommendations": self._get_health_recommendations(error_summary, circuit_status)
        }
    ...
    def _get_health_recommendations(
        ...
        critical_count = error_summary.get("severity_distribution", {}).get("critical", 0)
        if critical_count > 0:
            recommendations.append("Critical errors detected - check adapter configurations and credentials")
            
        # Check for high error rates
        total_errors = error_summary.get("total_errors", 0)
        if total_errors > 50:
            recommendations.append("High error rate detected - consider adjusting retry strategies")
            
        # Check for open circuit breakers
        open_circuits = [
            key for key, state in circuit_status.get("circuit_breakers", {}).items()
            if state.get("is_open", False)
        ]
        if open_circuits:
            recommendations.append(f"Circuit breakers open for: {', '.join(open_circuits)} - check service availability")
            
        # Check for authentication errors
        auth_errors = error_summary.get("error_types", {}).get("authentication", 0)
        if auth_errors > 0:
            recommendations.append("Authentication errors detected - verify credentials and permissions")
            
        # Check for network errors
        network_errors = error_summary.get("error_types", {}).get("network_connectivity", 0)
        if network_errors > 5:
