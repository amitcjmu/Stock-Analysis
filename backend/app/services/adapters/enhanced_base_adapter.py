...
class EnhancedBaseAdapter(BaseAdapter, ABC):
    ...
    async def validate_credentials(self, credentials: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            await self._record_metric(
                MetricType.ERROR_RATE, 1.0, "errors", "validate_credentials"
            )
            raise
    ...
    async def test_connectivity(self, credentials: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            await self._record_metric(
                MetricType.ERROR_RATE, 1.0, "errors", "test_connectivity"
            )
            raise
    ...
    async def _collect_data_with_monitoring(
        ...
        try:
            ...
            if self.config.enable_caching:
                ...
                if cached_response:
                    await self._record_metric(
                        MetricType.CACHE_HIT_RATE, 1.0, "ratio", "collect_data"
                    )
                    return cached_response
                else:
                    await self._record_metric(
                        MetricType.CACHE_HIT_RATE, 0.0, "ratio", "collect_data"
                    )
            ...
            await self._record_metric(
                MetricType.LATENCY, duration_ms, "ms", "collect_data"
            )
            await self._record_metric(
                MetricType.THROUGHPUT, 1.0, "ops/sec", "collect_data"
            )

            if response.data:
                data_size = len(str(response.data))
                await self._record_metric(
                    MetricType.DATA_VOLUME, data_size, "bytes", "collect_data"
                )
            ...
            await self._record_metric(
                MetricType.ERROR_RATE, 1.0, "errors", "collect_data"
            )
            raise
        finally:
            self._active_operations -= 1
    ...
    async def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        snapshot = await self._performance_monitor.create_snapshot(
            self._adapter_name, self._platform
        )

        # Get optimization recommendations
        recommendations = (
            await self._performance_monitor.generate_optimization_recommendations(
                self._adapter_name, self._platform
            )
        )
    ...
    async def get_health_status(self) -> Dict[str, Any]:
        ...
        if self._performance_monitor:
            performance_data = (
                self._performance_monitor.get_performance_dashboard_data()
            )
    ...
    async def optimize_configuration(self) -> Dict[str, Any]:
        ...
        trends = await self._performance_monitor.analyze_performance_trends(
            self._adapter_name, self._platform, hours=24
        )
    ...
    def _generate_cache_key(self, request: CollectionRequest) -> str:
        ...
        key_parts = [
            ...
            (
                str(request.options.get("region", "default"))
                if request.options
                else "no_options"
            ),
        ]
    ...
    async def _record_metric(
        self, metric_type: MetricType, value: float, unit: str, operation: str
    ):
        """Record a performance metric"""
        if self._performance_monitor:
            await self._performance_monitor.record_metric(
                metric_type, value, unit, self._adapter_name, self._platform, operation
            )
