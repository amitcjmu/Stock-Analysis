...
class PerformanceProfiler:
    ...
    def get_operation_stats(
        ...
        # Filter operations
        filtered_ops = [
            op for op in self._operation_history if op["timestamp"] >= cutoff_time
        ]

        if adapter_name:
            filtered_ops = [
                op for op in filtered_ops if op["adapter_name"] == adapter_name
            ]
        if platform:
        ...
        stats = {
            ...
            "p95_duration_ms": (
                self._calculate_percentile(durations, 0.95) if durations else 0
            ),
            "p99_duration_ms": (
                self._calculate_percentile(durations, 0.99) if durations else 0
            ),
...
class PerformanceMonitor:
    ...
    def __init__(self, thresholds: Optional[PerformanceThresholds] = None):
        ...
        self._performance_trends: Dict[str, deque] = defaultdict(
    ...
    async def record_metric(
        ...
        self._performance_trends[trend_key].append(
            {"timestamp": metric.timestamp, "value": value}
        )
    ...
    async def create_snapshot(
        self, adapter_name: str, platform: str
    ) -> PerformanceSnapshot:
    ...
    async def _check_thresholds(self, metric: PerformanceMetric):
        ...
        if (
            ...
            and metric.value > self.thresholds.max_latency_ms
        ):
            violations.append(
                f"High latency: {metric.value:.1f}ms exceeds threshold {self.thresholds.max_latency_ms}ms"
            )

        elif (
            metric.metric_type == MetricType.ERROR_RATE
            and metric.value > self.thresholds.max_error_rate_percent
        ):
            violations.append(
                f"High error rate: {metric.value:.1f}% exceeds threshold {self.thresholds.max_error_rate_percent}%"
            )

        elif (
            metric.metric_type == MetricType.THROUGHPUT
    ...
    async def analyze_performance_trends(
        self, adapter_name: str, platform: str, hours: int = 24
    ) -> Dict[str, Any]:
        ...
        for metric_type, values in metrics_by_type.items():
            ...
            if len(recent_values) >= 2 and len(early_values) >= 2:
                ...
                trends[metric_type.value] = {
                    ...
                    "change_percent": (
                        ((recent_avg - early_avg) / early_avg * 100)
                        if early_avg > 0
                        else 0
                    ),
    ...
    async def generate_optimization_recommendations(
        ...
        operation_stats = self.profiler.get_operation_stats(
            adapter_name, platform, hours=24
        )
        ...
        if (
            latency_trend
            and latency_trend["current_average"] > self.thresholds.max_latency_ms
        ):
        ...
        if (
            error_rate_trend
            and error_rate_trend["current_average"]
        ...
        if (
            throughput_trend
            and throughput_trend["current_average"]
            < self.thresholds.min_throughput_ops_per_sec
        ):
            recommendations.append(
                OptimizationRecommendation(
        ...
        if (
            operation_stats.get("p95_duration_ms", 0)
            > operation_stats.get("average_duration_ms", 0) * 2
        ):
            recommendations.append(
                OptimizationRecommendation(
                    ...
                    platform=platform,
                    parameters={
                        "average_duration_ms": operation_stats.get(
                            "average_duration_ms", 0
                        ),
                        "p95_duration_ms": operation_stats.get("p95_duration_ms", 0),
                        "suggested_timeout_ms": operation_stats.get(
                            "average_duration_ms", 0
                        )
                        * 3,
                    },
    ...
    def get_performance_dashboard_data(self) -> Dict[str, Any]:
        ...
        recent_metrics = [
            m for m in self._metrics if m.timestamp >= current_time - timedelta(hours=1)
        ]
        ...
        top_recommendations = sorted(
            self._recommendations, key=lambda r: r.priority_score, reverse=True
        )[:5]
        ...
        health_score = (
            max(0, 100 - (health_issues / max(total_adapters, 1)) * 100)
            if total_adapters > 0
            else 100
        )
    ...
    def export_performance_report(
        ...
        if adapter_name:
            filtered_metrics = [
                m for m in filtered_metrics if m.adapter_name == adapter_name
            ]
        if platform:
        ...
        for adapter, platform in adapter_platforms:
            trends = asyncio.run(
                self.analyze_performance_trends(adapter, platform, hours)
            )
            recommendations = asyncio.run(
                self.generate_optimization_recommendations(adapter, platform)
            )
...
def monitor_performance(operation_type: str = "unknown", record_metrics: bool = True):
    """
    ...
    def decorator(func):
        async def wrapper(*args, **kwargs):
            ...
            if not (
                adapter_instance and hasattr(adapter_instance, "_performance_monitor")
            ):
            ...
            operation_id = (
                f"{adapter_name}_{platform}_{operation_type}_{int(time.time() * 1000)}"
            )
