...
class AzureAdapter(BaseAdapter):
    ...
    def _get_azure_credential(self, credentials: AzureCredentials) -> ClientSecretCredential:
    ...
    async def validate_credentials(self, credentials: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            resource_client = ResourceManagementClient(credential, azure_creds.subscription_id)
            
            # Test credentials by getting subscription info
            subscription = resource_client.subscriptions.get(azure_creds.subscription_id)
            
            self.logger.info(f"Azure credentials validated for subscription: {subscription.display_name}")
            return True
            ...
            self.logger.error(f"Unexpected error validating Azure credentials: {str(e)}")
            return False
    ...
    async def test_connectivity(self, configuration: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            self.logger.info(f"Azure connectivity tests: {successful_tests}/{total_tests} successful")
    ...
    async def collect_data(self, request: CollectionRequest) -> CollectionResponse:
        ...
        try:
            ...
            if not request.target_resources or "all" in request.target_resources:
                ...
                target_types = self._map_targets_to_resource_types(request.target_resources)
                
            # Use Resource Graph for efficient bulk collection
            resource_data = await self._collect_resources_with_graph(target_types, request.configuration)
            
            if resource_data:
                # Process and enhance resource data
                for resource_type, resources in resource_data.items():
                    try:
                        enhanced_resources = await self._enhance_resource_data(resource_type, resources, request.configuration)
                        ...
                        self.logger.error(f"Failed to enhance data for {resource_type}: {str(e)}")
                        collected_data[resource_type] = {"error": str(e), "resources": []}
            ...
            self.logger.info(f"Azure data collection completed: {total_resources} resources in {duration:.2f}s")
            ...
            return CollectionResponse(
                ...
                duration_seconds=duration,
                metadata={"subscription_id": azure_creds.subscription_id if 'azure_creds' in locals() else "unknown"}
            )
    ...
    async def _collect_resources_with_graph(self, resource_types: Set[str], config: Dict[str, Any]) -> Dict[str, List[Dict]]:
        ...
        try:
            ...
            self.logger.info(f"Resource Graph collected {len(response.data)} resources across {len(resource_data)} types")
    ...
    async def _enhance_resource_data(self, resource_type: str, resources: List[Dict], config: Dict[str, Any]) -> List[Dict]:
        ...
        try:
            ...
            for resource in resources:
                ...
                if resource_type == "Microsoft.Compute/virtualMachines":
                    ...
                    enhanced_resource.update(await self._enhance_sql_database_data(resource))
                elif resource_type == "Microsoft.Web/sites":
                    enhanced_resource.update(await self._enhance_web_app_data(resource))
                elif resource_type == "Microsoft.Storage/storageAccounts":
                    enhanced_resource.update(await self._enhance_storage_account_data(resource))
                elif resource_type == "Microsoft.Network/loadBalancers":
                    enhanced_resource.update(await self._enhance_load_balancer_data(resource))
        ...
        try:
            ...
            if len(parts) >= 9:
                ...
                return {
                    ...
                    "os_disk_size": vm.storage_profile.os_disk.disk_size_gb if vm.storage_profile and vm.storage_profile.os_disk else None,
                    "data_disks_count": len(vm.storage_profile.data_disks) if vm.storage_profile and vm.storage_profile.data_disks else 0,
                    "network_interfaces": [nic.id for nic in vm.network_profile.network_interfaces] if vm.network_profile and vm.network_profile.network_interfaces else [],
                    "provisioning_state": vm.provisioning_state,
                    "power_state": self._get_vm_power_state(vm.instance_view) if vm.instance_view else None,
            ...
            self.logger.warning(f"Failed to enhance VM data for {vm_resource.get('name')}: {str(e)}")
            
        return {}
        
    async def _enhance_sql_database_data(self, db_resource: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.warning(f"Failed to enhance SQL database data for {db_resource.get('name')}: {str(e)}")
            
        return {}
        
    async def _enhance_web_app_data(self, app_resource: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if len(parts) >= 9:
                ...
                return {
                    ...
                    "site_config": {
                        "app_settings": getattr(web_app.site_config, 'app_settings', None) if web_app.site_config else None,
                        "connection_strings": getattr(web_app.site_config, 'connection_strings', None) if web_app.site_config else None,
                        "default_documents": getattr(web_app.site_config, 'default_documents', None) if web_app.site_config else None,
                        "net_framework_version": getattr(web_app.site_config, 'net_framework_version', None) if web_app.site_config else None,
                        "php_version": getattr(web_app.site_config, 'php_version', None) if web_app.site_config else None,
                        "python_version": getattr(web_app.site_config, 'python_version', None) if web_app.site_config else None,
                        "node_version": getattr(web_app.site_config, 'node_version', None) if web_app.site_config else None,
                        "java_version": getattr(web_app.site_config, 'java_version', None) if web_app.site_config else None,
                    } if web_app.site_config else {},
                }
                
        except Exception as e:
            self.logger.warning(f"Failed to enhance web app data for {app_resource.get('name')}: {str(e)}")
            
        return {}
        
    async def _enhance_storage_account_data(self, storage_resource: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if len(parts) >= 9:
                ...
                return {
                    ...
                    "primary_endpoints": {
                        "blob": storage_account.primary_endpoints.blob if storage_account.primary_endpoints else None,
                        "file": storage_account.primary_endpoints.file if storage_account.primary_endpoints else None,
                        "queue": storage_account.primary_endpoints.queue if storage_account.primary_endpoints else None,
                        "table": storage_account.primary_endpoints.table if storage_account.primary_endpoints else None,
                    } if storage_account.primary_endpoints else {},
                    "access_tier": storage_account.access_tier,
                    "enable_https_traffic_only": storage_account.enable_https_traffic_only,
                    "network_rule_set": {
                        "default_action": storage_account.network_rule_set.default_action if storage_account.network_rule_set else None,
                    } if storage_account.network_rule_set else {},
                    "encryption": {
                        "key_source": storage_account.encryption.key_source if storage_account.encryption else None,
                    } if storage_account.encryption else {},
                }
                
        except Exception as e:
            self.logger.warning(f"Failed to enhance storage account data for {storage_resource.get('name')}: {str(e)}")
            
        return {}
        
    async def _enhance_load_balancer_data(self, lb_resource: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if len(parts) >= 9:
                ...
                return {
                    ...
                    "frontend_ip_configurations": [
                        {
                            ...
                            "public_ip_address_id": fic.public_ip_address.id if fic.public_ip_address else None,
                        } for fic in load_balancer.frontend_ip_configurations
                    ] if load_balancer.frontend_ip_configurations else [],
                    "backend_address_pools": [
                        {
                            "name": bap.name,
                            "backend_addresses_count": len(bap.backend_addresses) if bap.backend_addresses else 0,
                        } for bap in load_balancer.backend_address_pools
                    ] if load_balancer.backend_address_pools else [],
                    ...
                    ] if load_balancer.load_balancing_rules else [],
                    "inbound_nat_rules": [
                        {
                            "name": inr.name,
                    ...
                    ] if load_balancer.inbound_nat_rules else [],
                }
                
        except Exception as e:
            self.logger.warning(f"Failed to enhance load balancer data for {lb_resource.get('name')}: {str(e)}")
    ...
    async def _collect_vm_metrics(self, vms: List[Dict], start_time: datetime, end_time: datetime) -> List[Dict]:
            ...
            try:
                ...
                for metric_name in metric_names:
                    try:
                                    ...
                                    for timeseries in metric.timeseries:
                                        if timeseries.data:
                                            metric_values.extend([
                                                data.average for data in timeseries.data 
                        ...
                        self.logger.warning(f"Failed to collect {metric_name} for VM {vm.get('name')}: {str(e)}")
                        
                metrics.append(vm_metrics)
                
            except Exception as e:
                self.logger.warning(f"Failed to collect metrics for VM {vm.get('name')}: {str(e)}")
                
        return metrics
        
    async def _collect_sql_metrics(self, databases: List[Dict], start_time: datetime, end_time: datetime) -> List[Dict]:
            ...
            try:
                ...
                for metric_name in metric_names:
                    try:
                        ...
                        self.logger.warning(f"Failed to collect {metric_name} for SQL DB {db.get('name')}: {str(e)}")
                        
                metrics.append(db_metrics)
                
            except Exception as e:
                self.logger.warning(f"Failed to collect metrics for SQL DB {db.get('name')}: {str(e)}")
                
        return metrics
        
    async def _collect_web_app_metrics(self, web_apps: List[Dict], start_time: datetime, end_time: datetime) -> List[Dict]:
                ...
                for metric_name in metric_names:
                    try:
                        metric_data = self._monitor_client.metrics.list(
                            ...
                            interval="PT1H",
                            metricnames=metric_name,
                            aggregation="Total" if metric_name in ["Requests", "Http2xx", "Http4xx", "Http5xx", "BytesReceived", "BytesSent"] else "Average"
                        )
                            ...
                            for metric in metric_data.value:
                                if metric.timeseries:
                                    for timeseries in metric.timeseries:
                                        if timeseries.data:
                                            metric_values.extend([
                                                (data.total if metric_name in ["Requests", "Http2xx", "Http4xx", "Http5xx", "BytesReceived", "BytesSent"] else data.average)
                                                for data in timeseries.data 
                                                if (data.total if metric_name in ["Requests", "Http2xx", "Http4xx", "Http5xx", "BytesReceived", "BytesSent"] else data.average) is not None
                                            ])
                                            
                            if metric_values:
                                if metric_name in ["Requests", "Http2xx", "Http4xx", "Http5xx", "BytesReceived", "BytesSent"]:
                                    app_metrics[metric_name.lower()] = sum(metric_values)
                                else:
                                    app_metrics[metric_name.lower()] = sum(metric_values) / len(metric_values)
                                
                    except Exception as e:
                        self.logger.warning(f"Failed to collect {metric_name} for Web App {app.get('name')}: {str(e)}")
                        
                metrics.append(app_metrics)
                
            except Exception as e:
                self.logger.warning(f"Failed to collect metrics for Web App {app.get('name')}: {str(e)}")
    ...
    async def get_available_resources(self, configuration: Dict[str, Any]) -> List[str]:
        ...
        try:
            ...
            for resource_type in self._supported_resource_types:
                try:
                    has_resources = await self._check_resource_type_has_resources(resource_type)
                    if has_resources:
                        available_types.append(resource_type)
                        
                except Exception as e:
                    self.logger.warning(f"Failed to check resources for {resource_type}: {str(e)}")
    ...
    def transform_data(self, raw_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            for resource_type, type_data in raw_data.items():
                ...
                if "resources" in type_data:
                    for resource in type_data["resources"]:
                        normalized_asset = self._transform_resource_to_asset(resource_type, resource)
                        if normalized_asset:
            ...
            self.logger.info(f"Transformed {len(normalized_data['assets'])} Azure assets to normalized format")
    ...
    def _transform_resource_to_asset(self, resource_type: str, resource: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if resource_type == "Microsoft.Compute/virtualMachines":
                asset.update({
                    ...
                    "state": resource.get("power_state", resource.get("provisioning_state")),
                ...
                asset.update({
                    ...
                    "frontend_ip_configurations": resource.get("frontend_ip_configurations", []),
                    "backend_address_pools": resource.get("backend_address_pools", []),
                    "load_balancing_rules": resource.get("load_balancing_rules", [])
                })
                
            return asset
            
        except Exception as e:
            self.logger.warning(f"Failed to transform {resource_type} resource to asset: {str(e)}")
            return None
