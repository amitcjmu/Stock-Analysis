...
class AdapterManager:
    ...
    async def register_adapter(
        ...
        try:
            if adapter_id in self._adapters:
                self.logger.warning(
                    f"Adapter {adapter_id} already registered, updating configuration"
                )
            ...
            self.logger.info(
                f"Registered adapter: {adapter_id} for platform {configuration.platform}"
            )
    ...
    async def execute_collection_request(
        self, request: CollectionRequest, preferred_adapter_id: Optional[str] = None
    ) -> CollectionResponse:
        """
        ...
        if adapter_id:
            ...
            status_data = {
                ...
                "success_rate": (
                    ...
                    if adapter_info.total_requests > 0
                    else 0
                ),
                "last_health_check": (
                    adapter_info.last_health_check.isoformat()
                    if adapter_info.last_health_check
                    else None
                ),
            }
    ...
    async def get_performance_dashboard(self) -> Dict[str, Any]:
        ...
        dashboard_data = {
            ...
            "total_requests": sum(
                info.total_requests for info in self._adapters.values()
            ),
            "successful_requests": sum(
                info.successful_requests for info in self._adapters.values()
            ),
            "failed_requests": sum(
                info.failed_requests for info in self._adapters.values()
            ),
            "adapter_details": {},
            ...
            if adapter_info.instance and adapter_info.status == AdapterStatus.HEALTHY:
                try:
                    ...
                    if result.get("optimization_applied"):
                        ...
                        optimization_results["total_changes"] += len(
                            result.get("changes", {})
                        )
    ...
    async def _select_adapter(
        self, request: CollectionRequest, preferred_adapter_id: Optional[str]
    ) -> Optional[str]:
        ...
        for adapter_id, adapter_info in self._adapters.items():
            if (
                adapter_info.configuration.platform == request.platform
                and adapter_info.status == AdapterStatus.HEALTHY
                and adapter_info.instance
            ):
        ...
        for adapter_id, adapter_info in self._adapters.items():
            if adapter_info.instance:
                try:
                    ...
                    if (
                        health_data.get("performance_health", {}).get("health_status")
                        == "unhealthy"
                    ):
