...
class AWSAdapter(BaseAdapter):
    ...
    def __init__(self, db: AsyncSession, metadata=AWS_ADAPTER_METADATA):
        ...
        if not AWS_SDK_AVAILABLE:
            logger.warning(
                "AWS SDK (boto3) is not installed. AWS adapter functionality will be limited."
            )
    ...
    def _init_clients(self, session: boto3.Session, region: str):
        ...
        # Initialize collectors
        self._compute_collector = ComputeServicesCollector(
            self._ec2_client, self._lambda_client
        )
        self._database_collector = DatabaseServicesCollector(self._rds_client, region)
        self._networking_collector = NetworkingServicesCollector(
            self._ec2_client, region
        )
    ...
    async def validate_credentials(self, credentials: Dict[str, Any]) -> bool:
        ...
        if not AWS_SDK_AVAILABLE:
            logger.error(
                "AWS SDK (boto3) is not installed. Cannot validate credentials."
            )
            return False
        ...
        try:
            ...
            self.logger.info(
                f"AWS credentials validated for account: {response.get('Account')}"
            )
            return True
    ...
    async def test_connectivity(self, configuration: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            self.logger.info(
                f"AWS connectivity tests: {successful_tests}/{total_tests} successful"
            )
    ...
    async def collect_data(self, request: CollectionRequest) -> CollectionResponse:
        ...
        try:
            ...
            if not request.target_resources or "all" in request.target_resources:
                ...
                target_services = (
                    set(request.target_resources) & self._supported_services
                )
            ...
            for service in target_services:
                try:
                    ...
                    self.logger.error(
                        f"Failed to collect data from {service}: {str(e)}"
                    )
            ...
            if request.configuration.get("include_config", True):
                try:
                    config_data = (
                        await self._config_collector.collect_configuration_data(
                            collected_data
                        )
                    )
            ...
            self.logger.info(
                f"AWS data collection completed: {total_resources} resources in {duration:.2f}s"
            )
    ...
    async def _collect_service_data(
        ...
        if collector:
            ...
            return {
                "error": f"No collector implemented for service: {service}",
                "resources": [],
            }
        ...
        try:
            ...
            for service in self._supported_services:
                try:
                    ...
                    self.logger.warning(
                        f"Failed to check resources for {service}: {str(e)}"
                    )
