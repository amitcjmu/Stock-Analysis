...
class TierAnalyzer:
    ...
    async def perform_comprehensive_tier_analysis(
        ...
        return TierAnalysis(
            ...
            platform_coverage=await self._calculate_platform_coverage_scores(
                platform_analysis
            ),
            ...
            routing_metadata={
                ...
                "tier_scores": {
                    tier.value: score for tier, score in tier_scores.items()
                },
                "complexity_factors": {
                    "platform_count": len(environment_config.get("platforms", [])),
                    "integration_count": len(
                        environment_config.get("integrations", [])
                    ),
                    "custom_config_count": len(
                        environment_config.get("custom_configurations", {})
                    ),
                },
            },
        )
    ...
    async def calculate_tier_score(
        ...
        complexity_weights = {
            EnvironmentComplexity.SIMPLE: {
                AutomationTier.TIER_1: 1.0,
                AutomationTier.TIER_2: 0.9,
                AutomationTier.TIER_3: 0.8,
                AutomationTier.TIER_4: 0.7,
            },
            EnvironmentComplexity.MODERATE: {
                AutomationTier.TIER_1: 0.8,
                AutomationTier.TIER_2: 1.0,
                AutomationTier.TIER_3: 0.9,
                AutomationTier.TIER_4: 0.8,
            },
            EnvironmentComplexity.COMPLEX: {
                AutomationTier.TIER_1: 0.6,
                AutomationTier.TIER_2: 0.9,
                AutomationTier.TIER_3: 1.0,
                AutomationTier.TIER_4: 0.9,
            },
            EnvironmentComplexity.ENTERPRISE: {
                AutomationTier.TIER_1: 0.4,
                AutomationTier.TIER_2: 0.7,
                AutomationTier.TIER_3: 0.9,
                AutomationTier.TIER_4: 1.0,
            },
        }
        ...
        tier_quality_capability = {
            AutomationTier.TIER_1: 0.95,
            AutomationTier.TIER_2: 0.85,
            AutomationTier.TIER_3: 0.75,
            AutomationTier.TIER_4: 0.60,
        }

        quality_score = (
            min(1.0, tier_quality_capability[tier] / min_quality)
            if min_quality > 0
            else 1.0
        )
    ...
    async def _calculate_platform_coverage_scores(
        self, platform_analysis: Dict[str, float]
    ) -> Dict[str, float]:
    ...
    async def _assess_automation_feasibility(
        self, environment_config: Dict[str, Any], complexity: EnvironmentComplexity
    ) -> Dict[str, float]:
    ...
    async def _perform_risk_assessment(
        ...
        return {
            ...
            "risk_factors": [
                (
                    "High automation tier with complex environment"
    ...
    async def _estimate_execution_times(
        self,
        tier: AutomationTier,
        environment_config: Dict[str, Any],
        complexity: EnvironmentComplexity,
        ...
        base_times = {
            AutomationTier.TIER_1: {
                "total": 3600000,
                "platform_detection": 600000,
                "collection": 1800000,
            },  # 1 hour
            AutomationTier.TIER_2: {
                "total": 5400000,
                "platform_detection": 900000,
                "collection": 2700000,
            },  # 1.5 hours
            AutomationTier.TIER_3: {
                "total": 7200000,
                "platform_detection": 1200000,
                "collection": 3600000,
            },  # 2 hours
            AutomationTier.TIER_4: {
                "total": 10800000,
                "platform_detection": 1800000,
                "collection": 5400000,
            },  # 3 hours
        }[tier]
    ...
    async def _calculate_resource_requirements(
        self, tier: AutomationTier, environment_config: Dict[str, Any]
    ) -> Dict[str, Any]:
        """Calculate resource requirements for tier selection"""
        return {
            "cpu_intensive": tier in [AutomationTier.TIER_1, AutomationTier.TIER_2],
            "memory_requirements": (
                "high" if tier == AutomationTier.TIER_1 else "medium"
            ),
            "network_requirements": (
                "high" if len(environment_config.get("platforms", [])) > 3 else "medium"
            ),
