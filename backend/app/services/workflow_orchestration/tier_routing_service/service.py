...
class TierRoutingService:
    ...
    async def analyze_and_route(
        ...
        try:
            ...
            complexity_analysis = await self.environment_analyzer.analyze_environment_complexity(
                environment_config=environment_config,
                base_analysis=base_tier_result
            )
            
            # Step 3: Assess platform coverage and compatibility
            platform_analysis = await self.environment_analyzer.analyze_platform_coverage(
                environment_config=environment_config,
                complexity=complexity_analysis
            )
            
            # Step 4: Perform comprehensive tier analysis
            tier_analysis = await self.tier_analyzer.perform_comprehensive_tier_analysis(
                environment_config=environment_config,
    ...
    async def _assess_tier_selection_risks(
        ...
        if selected_tier == AutomationTier.TIER_1 and tier_analysis.environment_complexity in [EnvironmentComplexity.COMPLEX, EnvironmentComplexity.ENTERPRISE]:
            risk_level = "high"
        ...
        if len(environment_config.get("platforms", [])) > 5:
            if risk_level == "low":
                risk_level = "medium"
            risk_factors.append("Large number of platforms increases execution complexity")
