...
class QualityOptimizer:
        ...
        try:
            ...
            for tier in AutomationTier:
                try:
                    ...
                    analysis = await tier_analyzer.perform_comprehensive_tier_analysis(
                        ...
                        client_requirements={},
                        base_analysis={},
                        complexity=await self._get_environment_complexity(
                            environment_config
                        ),
                        platform_analysis={},
            ...
            if not viable_options:
                ...
                if viable_options:
                    ...
                    best_option = min(
                        viable_options, key=lambda x: x["predicted_time_ms"]
                    )
                else:
                    # No option meets time constraints
                    best_option = min(
                        tier_options, key=lambda x: x["predicted_time_ms"]
                    )
            ...
            return {
                ...
                "quality_gap": max(
                    0, target_quality - best_option["predicted_quality"]
                ),
            }
    ...
    async def get_adaptive_routing_insights(
        ...
        try:
            ...
            for tier in AutomationTier:
                ...
                if tier_executions:
                    avg_quality = sum(
                        exec.get("overall_quality_score", 0) for exec in tier_executions
                    ) / len(tier_executions)
                    avg_time = sum(
                        exec.get("execution_time_ms", 0) for exec in tier_executions
                    ) / len(tier_executions)
                    success_rate = sum(
                        1
                        for exec in tier_executions
                        if exec.get("overall_status") == "completed"
                    ) / len(tier_executions)
                    ...
                    tier_performance[tier.value] = {
                        ...
                        "quality_variance": self._calculate_variance(
                            [
                                exec.get("overall_quality_score", 0)
                                for exec in tier_executions
                            ]
                        ),
                    }
            ...
            adaptive_recommendations = await self._calculate_adaptive_adjustments(
                tier_performance=tier_performance, patterns=patterns
            )
            ...
            return {
                ...
                "learning_insights": {
                    ...
                    "most_reliable_tier": (
                        max(
                            tier_performance.items(), key=lambda x: x[1]["success_rate"]
                        )[0]
                        if tier_performance
                        else None
                    ),
    ...
    async def _generate_quality_optimization_recommendations(
        ...
        if best_option["predicted_quality"] < target_quality:
            ...
            recommendations.append(
                f"Quality gap of {quality_gap:.2f} detected. Consider:"
            )

            # Find options that meet quality
            better_options = [
                opt for opt in all_options if opt["predicted_quality"] >= target_quality
            ]
            if better_options:
                recommendations.append(
                    f"- Upgrading to {better_options[0]['tier']} for better quality"
                )

            recommendations.append("- Implementing additional validation steps")
            recommendations.append("- Increasing manual collection depth")

        if time_constraints and best_option["predicted_time_ms"] > time_constraints.get(
            "max_execution_time_ms", float("inf")
        ):
            recommendations.append(
                "- Consider relaxing time constraints or quality targets"
            )
    ...
    async def _identify_execution_patterns(
        ...
        if failed_executions:
            ...
            for exec in failed_executions:
                ...
                common_failures[failure_reason] = (
                    common_failures.get(failure_reason, 0) + 1
                )

            patterns["failure_patterns"] = [
                {"reason": reason, "count": count}
                for reason, count in sorted(
                    common_failures.items(), key=lambda x: x[1], reverse=True
                )[:5]
            ]
    ...
    async def _calculate_adaptive_adjustments(
        self, tier_performance: Dict[str, Any], patterns: Dict[str, Any]
    ) -> Dict[str, Any]:
        """Calculate adaptive adjustments"""
        adjustments = {"tier_confidence_adjustments": {}, "recommended_changes": []}
