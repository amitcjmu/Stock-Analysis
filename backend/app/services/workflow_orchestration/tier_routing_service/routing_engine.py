...
class RoutingEngine:
    ...
    async def make_routing_decision(
        ...
        quality_thresholds = await self._set_quality_thresholds_for_tier(
            selected_tier=selected_tier, client_requirements=client_requirements
        )

        # Configure timeouts
        timeout_configuration = await self._configure_timeouts_for_tier(
            selected_tier=selected_tier, complexity=tier_analysis.environment_complexity
        )
        ...
        manual_strategy = (
            await self._configure_manual_collection_strategy(
                selected_tier=selected_tier,
                tier_analysis=tier_analysis,
                client_requirements=client_requirements,
            )
        ...
        fallback_options = await self._generate_fallback_options(
            selected_tier=selected_tier, tier_analysis=tier_analysis
        )
    ...
    async def _apply_routing_strategy(
        ...
        if strategy == RoutingStrategy.AGGRESSIVE:
            ...
            return max(
                tier_analysis.alternative_tiers
                + [(tier_analysis.recommended_tier, tier_analysis.confidence_score)],
                key=lambda x: (x[0].value == "tier_1", x[1]),
            )[0]
        elif strategy == RoutingStrategy.CONSERVATIVE:
            # Prefer reliability over speed
            return (
                AutomationTier.TIER_4
                if tier_analysis.confidence_score < 0.8
                else tier_analysis.recommended_tier
            )
    ...
    async def _configure_phases_for_tier(
        ...
        return {
            ...
            "automated_collection": {
                "parallel_execution": selected_tier
                in [AutomationTier.TIER_1, AutomationTier.TIER_2]
            },
            "gap_analysis": {"depth": "comprehensive"},
            "manual_collection": {"enabled": selected_tier != AutomationTier.TIER_1},
            "synthesis": {
                "validation_level": (
                    "strict" if selected_tier == AutomationTier.TIER_1 else "standard"
                )
            },
        }

    async def _set_quality_thresholds_for_tier(
        self, selected_tier: AutomationTier, client_requirements: Dict[str, Any]
    ) -> Dict[str, float]:
    ...
    async def _configure_timeouts_for_tier(
        self, selected_tier: AutomationTier, complexity: EnvironmentComplexity
    ) -> Dict[str, int]:
        """Configure timeouts for selected tier"""
        base_timeouts = {
            "platform_detection": 600,
            "automated_collection": 3600,
            "gap_analysis": 900,
            "manual_collection": 7200,
            "synthesis": 1200,
        }
        multiplier = {
        ...
        return {
            phase: int(timeout * multiplier) for phase, timeout in base_timeouts.items()
        }
            ...
            adapters.append(
                {
                    ...
                    "priority": (
                        "high"
                        if platform_coverage.get(adapter_type, 0) > 0.8
                        else "medium"
                    ),
                }
            )
    ...
    async def _configure_manual_collection_strategy(
        ...
        return {
            ...
            "questionnaire_depth": (
                "comprehensive"
                if selected_tier == AutomationTier.TIER_4
                else "targeted"
            ),
            "validation_level": "strict",
            "user_engagement": (
                "high"
                if selected_tier in [AutomationTier.TIER_3, AutomationTier.TIER_4]
                else "medium"
            ),
        }

    async def _generate_fallback_options(
        self, selected_tier: AutomationTier, tier_analysis: TierAnalysis
    ) -> List[Dict[str, Any]]:
        """Generate fallback options"""
        fallbacks = []
        for alt_tier, confidence in tier_analysis.alternative_tiers[
            :2
        ]:  # Top 2 alternatives
            fallbacks.append(
                {
