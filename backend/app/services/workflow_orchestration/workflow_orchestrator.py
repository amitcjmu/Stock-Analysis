...
from app.services.ai_analysis import (BusinessContextAnalyzer,
                                      GapAnalysisAgent, LearningOptimizer)
...
class WorkflowOrchestrator:
    ...
    async def create_collection_workflow(
        ...
        try:
            ...
            if execute_immediately and self._can_execute_immediately():
                ...
                logger.info(
                    f"📋 Workflow queued: {workflow_id} (position: {len(self.workflow_queue)})"
                )
    ...
    async def pause_workflow(
        ...
        try:
            ...
            pause_result = await self.master_orchestrator.pause_flow(
    ...
    async def resume_workflow(
        ...
        try:
            ...
            resume_result = await self.master_orchestrator.resume_flow(
    ...
    async def cancel_workflow(
        ...
        try:
            ...
            if workflow.status == WorkflowStatus.SCHEDULED:
                self.workflow_queue = [
                    w for w in self.workflow_queue if w.workflow_id != workflow_id
                ]
    ...
    async def get_orchestrator_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            recent_completed = len(
                [
                    w
                    for w in self.execution_history[-100:]
            ...
            return {
                ...
                "background_services": {
                    ...
                    "monitor_running": self._monitoring_task
                    and not self._monitoring_task.done(),
                    "cleanup_running": self._cleanup_task
        ...
        while True:
            try:
                ...
                cutoff_time = datetime.utcnow() - timedelta(
                    hours=self.workflow_cleanup_hours
                )
                ...
                logger.info(
                    f"🧹 Cleaned up old workflow records. History count: {len(self.execution_history)}"
                )
