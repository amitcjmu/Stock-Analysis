    ...
    async def _calculate_metric_trends(self, metrics: List[MetricPoint]) -> Dict[str, Any]:
        ...
        for metric in metrics:
            if isinstance(metric.value, (int, float)):
                ...
                metric_groups[metric.metric_name].append((metric.timestamp, metric.value))
    ...
    async def _generate_performance_summary(
        ...
        if current_metrics:
            ...
            if current_metrics.error_rate > 0.1:
                ...
                summary["recommendations"].append("High error rate detected - investigate failure causes")
            
            if current_metrics.efficiency_score < 0.7:
                summary["recommendations"].append("Low efficiency score - consider workflow optimization")
            
            if current_metrics.success_rate < 0.9:
                summary["recommendations"].append("Low success rate - review workflow reliability")
