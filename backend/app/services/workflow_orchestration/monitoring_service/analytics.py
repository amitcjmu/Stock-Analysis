...
class AnalyticsEngine:
    ...
    async def _analyze_workflow_executions(self, cutoff_time: datetime) -> Dict[str, Any]:
        ...
        return {
            ...
            "phase_performance": {
                ...
                "automated_collection": {"avg_duration_ms": 1800000, "success_rate": 0.9},
    ...
    async def _generate_analytics_insights(
        ...
        if success_rate >= 0.95:
            ...
            insights.append("Workflow execution reliability is good with room for improvement")
        else:
            insights.append("Workflow execution reliability needs attention - investigate failure patterns")
        
        # Performance insights
        throughput_trend = performance_analytics.get("throughput_trend", "stable")
        if throughput_trend == "improving":
            insights.append("Workflow throughput is improving - performance optimizations are effective")
        elif throughput_trend == "declining":
            insights.append("Workflow throughput is declining - investigate performance bottlenecks")
        ...
        if quality_score >= 0.9:
            ...
            insights.append("Workflow quality metrics are good with minor improvements needed")
        else:
            insights.append("Workflow quality metrics indicate need for process improvements")
        
        # SLA insights
        sla_compliance = performance_analytics.get("sla_compliance", {}).get("compliance_rate", 0)
        if sla_compliance >= 0.95:
            ...
            insights.append("SLA compliance needs improvement - review performance targets")
        
        # Resource insights
        bottlenecks = performance_analytics.get("bottleneck_analysis", {}).get("identified_bottlenecks", [])
        if bottlenecks:
            insights.append(f"Performance bottlenecks identified in {len(bottlenecks)} areas - prioritize optimization")
        else:
