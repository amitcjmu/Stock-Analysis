...
class HealthMonitor:
        ...
        try:
            ...
            health_data = {
                ...
                "recommendations": await self._generate_health_recommendations(
                    overall_health=overall_health, alerts=relevant_alerts
                ),
    ...
    async def _assess_system_health(self) -> Dict[str, Any]:
        ...
        healthy_checks = sum(
            1 for status in checks.values() if status == HealthStatus.HEALTHY
        )
        health_score = healthy_checks / len(checks)

        return {
            "status": (
                HealthStatus.HEALTHY if health_score > 0.8 else HealthStatus.DEGRADED
            ),
            "score": health_score,
    ...
    async def _assess_workflow_health(self, workflow_id: str) -> Dict[str, Any]:
        ...
        healthy_count = sum(
            1 for status in health_checks if status == HealthStatus.HEALTHY
        )
    ...
    async def _assess_component_health(
        self, component_filter: Optional[List[str]]
    ) -> Dict[str, Any]:
        ...
        healthy_components = sum(
            1 for status in components.values() if status == HealthStatus.HEALTHY
        )
    ...
    async def _calculate_overall_health_score(
        ...
        workflow_score = workflow_health.get(
            "score", workflow_health.get("overall_score", 1.0)
        )
    ...
    async def _generate_health_recommendations(
        self, overall_health: Dict[str, Any], alerts: List[Alert]
    ) -> List[str]:
        ...
        if health_score < 0.8:
            recommendations.append(
                "System health is below optimal - investigate degraded components"
            )
        ...
        if health_score >= 0.9:
            ...
            recommendations.append(
                "Monitor system performance and address any emerging issues"
            )
    ...
    async def _check_workflow_performance_health(
