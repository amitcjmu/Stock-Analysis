...
class RecommendationEvaluator:
    ...
    async def _evaluate_single_recommendation(
        ...
        return {
            ...
            "confidence_validated": success_score > 0.5 and recommendation.confidence_score > 0.7,
    ...
    async def _update_learning_patterns(
        ...
        for rec_type, type_evaluations in type_groups.items():
            success_rate = sum(1 for e in type_evaluations if e["success"]) / len(type_evaluations)
            avg_impact = statistics.mean([e["impact_achieved"] for e in type_evaluations])
            ...
            if pattern_id in self.learned_patterns:
                ...
                pattern.confidence = (pattern.confidence * self.learning_weight_decay + 
                                    success_rate * (1 - self.learning_weight_decay))
                pattern.success_rate = (pattern.success_rate * self.learning_weight_decay + 
                                      success_rate * (1 - self.learning_weight_decay))
                ...
                pattern = LearningPattern(
                    ...
                    conditions={
                        "environment_type": package.target_environment.get("environment_type"),
                    ...
                    applicable_scenarios=[package.target_environment.get("environment_type", "unknown")]
                )
    ...
    async def _calculate_recommendation_success(
        ...
        overall_score = (avg_success_score * 0.4 + 
    ...
    async def _generate_adaptation_suggestions(
        ...
        if low_success_types:
            suggestions.append(f"Consider adjusting thresholds for: {', '.join(low_success_types)}")
        
        if high_success_types:
            suggestions.append(f"Increase priority for: {', '.join(high_success_types)}")
    ...
    def _update_adaptation_metrics(self, evaluation_result: Dict[str, Any]) -> None:
        ...
        if len(self.adaptation_metrics["recommendation_success"]) > 100:
            self.adaptation_metrics["recommendation_success"] = \
                self.adaptation_metrics["recommendation_success"][-100:]
    ...
    async def _calculate_recommendation_performance_metrics(
        ...
        avg_recommendation_count = statistics.mean([len(pkg.recommendations) for pkg in packages])
    ...
    async def _analyze_recommendation_trends(
        ...
        early_confidence = statistics.mean([p.overall_confidence for p in sorted_packages[:len(sorted_packages)//2]])
        late_confidence = statistics.mean([p.overall_confidence for p in sorted_packages[len(sorted_packages)//2:]])
        
        confidence_trend = "improving" if late_confidence > early_confidence else "declining"
        
        return {
            "trend_direction": "improving" if late_confidence > early_confidence + 0.05 else "stable",
            "confidence_trend": confidence_trend,
            "success_trend": "improving",  # Would calculate from success metrics
            "recommendation_type_distribution": self._calculate_type_distribution(packages)
        }
    
    def _calculate_type_distribution(self, packages: List[RecommendationPackage]) -> Dict[str, float]:
    ...
    async def _get_top_performing_patterns(
        ...
        qualified_patterns = [
            ...
            if pattern.confidence >= min_confidence and pattern.occurrence_count >= min_occurrences
        ]
    ...
    async def _calculate_adaptation_effectiveness(self) -> Dict[str, Any]:
        ...
        if "recommendation_success" not in self.adaptation_metrics or \
           len(self.adaptation_metrics["recommendation_success"]) < 10:
            return {
        ...
        early_scores = [s["success_score"] for s in success_history[:len(success_history)//3]]
        late_scores = [s["success_score"] for s in success_history[-len(success_history)//3:]]
        ...
        return {
            ...
            "pattern_utilization": len(self.learned_patterns) / 20  # Normalized by expected patterns
        }