...
class RetryConfig:
    ...
    def __post_init__(self):
        if self.retry_on_errors is None:
            self.retry_on_errors = [TimeoutError, ConnectionError, asyncio.TimeoutError]
...
class FlowErrorHandler:
    ...
    def __init__(self):
        ...
        self._category_classifiers = {
            ...
            ErrorCategory.CREW_AI: [
                CrewAIExecutionError,
                RuntimeError,
            ],  # RuntimeError often from CrewAI
    ...
    async def handle_error(
        ...
        is_retryable = any(
            isinstance(error, error_type) for error_type in retry_config.retry_on_errors
        )

        # Apply error strategy
        error_type = type(error)
        strategy = self._error_strategies.get(error_type, self._handle_generic_error)

        resolution = await strategy(error, context, error_category)

        # Apply retry logic if applicable
        if (
            resolution.should_retry
            and is_retryable
            and current_attempts < retry_config.max_retries
        ):
            ...
            delay = min(
                ...
                * (retry_config.backoff_multiplier**current_attempts),
            ...
            logger.info(
                f"🔄 Scheduling retry for {context.operation}: {resolution.message}"
            )
    ...
    def clear_retry_attempts(self, flow_id: str):
        ...
        keys_to_remove = [
            key for key in self._retry_attempts.keys() if key.startswith(f"{flow_id}:")
        ]
    ...
    def _classify_error(self, error: Exception) -> ErrorCategory:
        ...
        if any(keyword in error_msg for keyword in ["timeout", "timed out"]):
            ...
            keyword in error_msg for keyword in ["validation", "invalid", "required"]
        ):
    ...
    async def _handle_validation_error(
        self, error: ValueError, context: ErrorContext, category: ErrorCategory
    ) -> ErrorResolution:
    ...
    async def _handle_permission_error(
        self, error: PermissionError, context: ErrorContext, category: ErrorCategory
    ) -> ErrorResolution:
    ...
    async def _handle_runtime_error(
        self, error: RuntimeError, context: ErrorContext, category: ErrorCategory
    ) -> ErrorResolution:
    ...
    def register_error_strategy(
        ...
        strategy: Callable[
            [Exception, ErrorContext, ErrorCategory], Awaitable[ErrorResolution]
        ],
    ):
        """
    ...
    def add_category_classifier(
        self, category: ErrorCategory, error_types: List[Type[Exception]]
    ):
        """
        ...
        logger.info(
            f"✅ Added {len(error_types)} error types to category {category.value}"
        )
    ...
    async def _handle_network_timeout_error(
        self, error: NetworkTimeoutError, context: ErrorContext, category: ErrorCategory
    ) -> ErrorResolution:
    ...
    async def _handle_resource_error(
        ...
        return ErrorResolution(
            ...
            metadata={"resource_type": error.resource_type},
        )

    async def _handle_flow_not_found_error(
        self, error: FlowNotFoundError, context: ErrorContext, category: ErrorCategory
    ) -> ErrorResolution:
        ...
        return ErrorResolution(
            ...
            severity=ErrorSeverity.HIGH,
            new_status="failed",
            metadata={"requested_flow_id": error.flow_id},
        )
    ...
    async def _handle_background_task_error(
        self, error: BackgroundTaskError, context: ErrorContext, category: ErrorCategory
    ) -> ErrorResolution:
        ...
        return ErrorResolution(
            ...
            new_status="retrying",
            metadata={"task_name": error.task_name, "task_id": error.task_id},
        )
