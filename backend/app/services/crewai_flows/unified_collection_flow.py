...
class UnifiedCollectionFlow(Flow[CollectionFlowState]):
    ...
    def _init_phase_handlers(self):
        ...
        self.initialization_handler = InitializationHandler(
            self.flow_context, self.state_manager, self.services, self.unified_flow_management
        )
        self.platform_detection_handler = PlatformDetectionHandler(
            self.flow_context, self.state_manager, self.services, 
            self.unified_flow_management, self.crewai_service
        )
    ...
    async def initialize_collection(self):
        ...
        return await self.initialization_handler.initialize_collection(self.state, config)
    ...
    async def resume_flow(self, user_inputs: Optional[Dict[str, Any]] = None):
        ...
        if handler:
            ...
            previous_result = self.state.phase_results.get(previous_phase.value, {}) if previous_phase else {}
