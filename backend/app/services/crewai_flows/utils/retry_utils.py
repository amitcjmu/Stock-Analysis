...
def classify_error(error: Exception) -> ErrorCategory:
    ...
    transient_patterns = [
        ...
        "retryable", "transient", "handshake", "reset by peer"
    ]
    
    # Permanent errors - should not retry
    permanent_patterns = [
        "invalid", "validation", "unauthorized", "forbidden", "401", "403",
        "not found", "404", "bad request", "400", "missing required",
        "does not exist", "permission denied", "authentication failed"
    ]
    
    # Resource errors - should backoff significantly
    resource_patterns = [
        "memory", "disk", "space", "quota", "limit exceeded", "507",
        "insufficient", "out of", "resource", "capacity"
    ]
    ...
    if error_type in ["TimeoutError", "asyncio.TimeoutError", "ConnectionError", "ConnectionRefusedError"]:
        return ErrorCategory.TRANSIENT
    ...
    **kwargs
    ...
    for attempt in range(config.max_retries + 1):
        try:
            ...
            if error_category == ErrorCategory.PERMANENT and error_type not in config.retry_on:
...
class RetryMetrics:
    ...
    def record_call(self, func_name: str, success: bool, retries: int = 0, error: Optional[str] = None):
