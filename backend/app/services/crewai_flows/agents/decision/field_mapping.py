...
class FieldMappingDecisionAgent(BaseDecisionAgent):
    ...
    async def analyze_field_mappings(
        ...
        overall_confidence = (
            high_confidence_mappings / total_fields if total_fields > 0 else 0
        )
        ...
        return {
            ...
            "mapped_fields": list(
                field_mappings.keys()
            ),  # Add this for critical field checking
        }

    def identify_critical_fields_for_migration(
        self, state: UnifiedDiscoveryFlowState
    ) -> List[str]:
        """
    ...
    def calculate_approval_threshold(
        self, mapping_analysis: Dict[str, Any], critical_fields: List[str]
    ) -> float:
        """
        ...
        if critical_fields:
            critical_mapped = sum(
                ...
                if f in mapping_analysis.get("mapped_fields", [])
            )
