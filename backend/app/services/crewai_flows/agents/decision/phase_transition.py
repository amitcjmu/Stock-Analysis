...
class PhaseTransitionAgent(BaseDecisionAgent):
    ...
    def _analyze_current_state(
        ...
        if phase == "data_import":
            ...
            analysis["asset_creation_results"] = self._analyze_asset_creation(results, state)
    ...
    def _decide_after_mapping(self, analysis: Dict[str, Any]) -> AgentDecision:
        ...
        if mapping_confidence < required_confidence:
            ...
            pause_confidence = ConfidenceCalculator.weighted_average({
                "mapping_quality_insufficient": 1.0 if confidence_gap > 0.2 else 0.5,
        ...
        proceed_confidence = ConfidenceCalculator.weighted_average({
            "mapping_quality_high": 1.0 if mapping_confidence >= required_confidence else 0.0,
    ...
    def _decide_after_asset_creation(self, analysis: Dict[str, Any]) -> AgentDecision:
        ...
        if not has_assets:
            ...
            retry_confidence = ConfidenceCalculator.weighted_average({
                ...
                "creation_failure_clear": 1.0 if creation_success_rate == 0 else 0.5,
        ...
        proceed_confidence = ConfidenceCalculator.weighted_average({
            ...
            "creation_success_rate_good": 1.0 if creation_success_rate > 0.8 else 0.5,
    ...
    def _analyze_import_metrics(self, results: Any, state: UnifiedDiscoveryFlowState) -> Dict[str, Any]:
        ...
        return {
            ...
            "field_count": len(raw_data[0]) if raw_data and isinstance(raw_data[0], dict) else 0,
    ...
    def _analyze_mapping_quality(self, state: UnifiedDiscoveryFlowState) -> Dict[str, Any]:
        ...
        confidence = DecisionUtils.get_state_attribute(state, 'field_mapping_confidence', 0.5)
        ...
        return {
            ...
            "field_coverage": len(mapped_fields) / len(critical_fields) if critical_fields else 1.0
        }
    ...
    def _analyze_asset_creation(self, results: Any, state: UnifiedDiscoveryFlowState) -> Dict[str, Any]:
    ...
    def _check_assets_created(self, analysis: Dict[str, Any]) -> bool:
        ...
        asset_inventory = DecisionUtils.get_state_attribute(state, 'asset_inventory', {})
