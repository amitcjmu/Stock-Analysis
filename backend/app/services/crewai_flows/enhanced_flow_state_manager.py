...
class StateEncryption:
    ...
    def _get_or_create_key(self) -> bytes:
        ...
        if key_env_var in os.environ:
            ...
            password = os.getenv("FLOW_STATE_PASSWORD", "default-password-change-me").encode()
    ...
    def serialize_state(self, state: Dict[str, Any]) -> bytes:
        ...
        try:
            ...
            if self.config.format == "json":
                serialized = json.dumps(state, default=self._json_serializer).encode('utf-8')
            elif self.config.format == "pickle":
                serialized = pickle.dumps(state)
            else:
                raise StateSerializationError(f"Unsupported serialization format: {self.config.format}")
            ...
            if size_mb > self.config.max_size_mb:
                raise StateSerializationError(f"Serialized state too large: {size_mb:.2f}MB > {self.config.max_size_mb}MB")
            
            logger.debug(f"✅ State serialized: {len(serialized)} bytes ({self.config.format})")
            return serialized
    ...
    def deserialize_state(self, data: bytes) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.config.format == "json":
                ...
                raise StateSerializationError(f"Unsupported deserialization format: {self.config.format}")
...
class EnhancedFlowStateManager(FlowStateManager):
    ...
    async def export_flow_state(
        ...
        try:
            ...
            return {
                ...
                "data": base64.b64encode(serialized).decode() if export_format != "json" else export_data,
    ...
    async def import_flow_state(
        ...
        try:
            ...
            result = await self.create_flow_state_enhanced(flow_id, imported_state, import_format)
    ...
    async def _store_serialized_state(self, flow_id: str, serialized_data: bytes) -> None:
        ...
        try:
            ...
            analytics = {
                ...
                "encryption_config": {
                    ...
                    "sensitive_fields_count": len(self.encryption_config.sensitive_fields)
                }
            }
            ...
            if serialized_data:
                ...
                if self.serialization_config.compress:
                    ...
                    analytics["compression_ratio"] = len(serialized_data) / len(uncompressed)
