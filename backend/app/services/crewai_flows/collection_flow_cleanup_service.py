...
class CollectionFlowCleanupService:
    ...
    async def cleanup_expired_flows(
        ...
        try:
            ...
            query = select(CollectionFlow).options(
            ...
            for flow in expired_flows:
                try:
                    ...
                    cleanup_summary["total_size_bytes"] += flow_cleanup["estimated_size_bytes"]
            ...
            if not dry_run and cleanup_summary["flows_cleaned"] > 0:
                ...
                logger.info(f"Cleaned up {cleanup_summary['flows_cleaned']} Collection flows")
    ...
    async def _cleanup_single_flow(self, flow: CollectionFlow, dry_run: bool) -> Dict[str, Any]:
        ...
        cleanup_detail = {
            ...
            "related_records": {
                ...
                "questionnaire_responses": len(flow.questionnaire_responses) if flow.questionnaire_responses else 0
            },
            "cleanup_actions": []
        }
        ...
        if not dry_run:
            ...
            if flow.master_flow_id:
                try:
                    crewai_state_result = await self.db.execute(
                        ...
                        .where(CrewAIFlowStateExtensions.flow_id == flow.master_flow_id)
                    )
                    crewai_state = crewai_state_result.scalar_one_or_none()
                    
                    if crewai_state:
                        await self.db.delete(crewai_state)
                        cleanup_detail["cleanup_actions"].append("Removed CrewAI flow state")
                        
                except Exception as e:
                    logger.warning(f"Failed to cleanup CrewAI state for flow {flow.id}: {e}")
                    cleanup_detail["cleanup_actions"].append(f"CrewAI cleanup failed: {str(e)}")
            
            # 2. Delete related gap analyses (if not handled by cascade)
            if flow.data_gaps:
                for gap in flow.data_gaps:
                    await self.db.delete(gap)
                cleanup_detail["cleanup_actions"].append(f"Removed {len(flow.data_gaps)} gap analyses")
            ...
            if flow.questionnaire_responses:
                ...
                cleanup_detail["cleanup_actions"].append(f"Removed {len(flow.questionnaire_responses)} questionnaire responses")
    ...
    def _calculate_flow_size(self, flow: CollectionFlow) -> int:
        ...
        if hasattr(flow, 'questionnaire_responses') and flow.questionnaire_responses:
            size += len(flow.questionnaire_responses) * 1000  # Estimate 1KB per response
        ...
        try:
            ...
            for flow in orphaned_flows:
                ...
                if not dry_run:
                    ...
                    if flow.status in [CollectionFlowStatus.COMPLETED.value, CollectionFlowStatus.CANCELLED.value]:
                        # Keep completed/cancelled flows but clear the orphaned reference
                        flow.master_flow_id = None
                        cleanup_detail["action"] = "Cleared orphaned master_flow_id reference"
                    else:
            ...
            if not dry_run and orphaned_flows:
                await self.db.commit()
                logger.info(f"Cleaned up {len(orphaned_flows)} orphaned Collection flows")
    ...
    async def get_cleanup_recommendations(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for flow in all_flows:
                # Status breakdown
                status = flow.status
                recommendations["status_breakdown"][status] = recommendations["status_breakdown"].get(status, 0) + 1
            ...
            if recommendations["status_breakdown"].get(CollectionFlowStatus.FAILED.value, 0) > 0:
                recommendations["recommendations"].append(
    ...
    def _is_cleanup_candidate(self, flow: CollectionFlow, current_time: datetime) -> bool:
    ...
    async def cleanup_stuck_initialized_flows(self, timeout_minutes: int = 5, dry_run: bool = False) -> Dict[str, Any]:
        """
