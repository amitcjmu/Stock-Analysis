...
class CollaborationMonitor:
    ...
    def update_collaboration_status(
        ...
        self.logger.info(
            f"🔄 Collaboration updated: {event_id} - Status: {status.value}"
        )
    ...
    def track_memory_sharing_event(
        ...
        event_id = self.start_collaboration_event(
            ...
            crews_involved=self._determine_crews_for_agents(
                [sharing_agent] + receiving_agents
            ),
            context={
        ...
        # Update crew metrics
        for crew_name in self._determine_crews_for_agents(
            [sharing_agent] + receiving_agents
        ):
            if crew_name in self.crew_metrics:
                self.crew_metrics[crew_name].memory_sharing_events += 1

    def track_knowledge_utilization_event(
        self,
        agent: str,
        knowledge_base: str,
        ...
        # Update crew metrics
        crew_name = (
            self._determine_crews_for_agents([agent])[0]
            if self._determine_crews_for_agents([agent])
            else "unknown"
        )
        if crew_name in self.crew_metrics:
    ...
    def get_agent_collaboration_analytics(
        ...
        if agent_name:
            ...
            return {
                ...
                "metrics": {
                    ...
                    "success_rate": metrics.successful_collaborations
                    / max(metrics.total_collaborations, 1),
        ...
        if crew_name:
            ...
            return {
                ...
                "crew_summaries": [
                    {
                        "crew": name,
                        "total_events": metrics.internal_collaborations
                        + metrics.external_collaborations,
    ...
    def get_collaboration_effectiveness_report(self) -> Dict[str, Any]:
        ...
        cross_crew_events = [
            e for e in self.collaboration_events if len(e.crews_involved) > 1
        ]
        ...
        return {
            ...
            "cross_crew_analysis": {
                ...
                "cross_crew_success_rate": len(
                    [
                        e
                        for e in cross_crew_events
                        if e.status == CollaborationStatus.COMPLETED
                    ]
                )
                / max(len(cross_crew_events), 1),
            },
    ...
    def _update_collaboration_metrics(self, collaboration: CollaborationEvent):
        ...
        for participant in collaboration.participants:
            if participant not in self.agent_metrics:
                crew_name = (
                    self._determine_crews_for_agents([participant])[0]
                    if self._determine_crews_for_agents([participant])
                    else "unknown"
                )
                self.agent_metrics[participant] = AgentCollaborationMetrics(
                    agent_name=participant, crew_name=crew_name
                )
            ...
            if collaboration.duration_seconds:
                total_duration = agent_metrics.average_collaboration_duration * (
                    agent_metrics.total_collaborations - 1
                )
                agent_metrics.average_collaboration_duration = (
                    total_duration + collaboration.duration_seconds
                ) / agent_metrics.total_collaborations

            # Update effectiveness score
            agent_metrics.effectiveness_score = (
                agent_metrics.successful_collaborations
                / max(agent_metrics.total_collaborations, 1)
            )
        ...
        for crew_name in collaboration.crews_involved:
            if crew_name in self.crew_metrics:
                ...
                total_events = (
                    crew_metrics.internal_collaborations
                    + crew_metrics.external_collaborations
                )
                if collaboration.status == CollaborationStatus.COMPLETED:
                    crew_metrics.collaboration_effectiveness = (
                        crew_metrics.collaboration_effectiveness * (total_events - 1)
                        + 1.0
                    ) / total_events
                else:
                    crew_metrics.collaboration_effectiveness = (
    ...
    def _get_recent_completed_events(self, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        recent_events = sorted(
            self.collaboration_events, key=lambda x: x.timestamp, reverse=True
        )[:limit]
    ...
    def _get_collaboration_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            / max(len(self.collaboration_events), 1),
            "collaboration_types": len(
                set([e.collaboration_type.value for e in self.collaboration_events])
            ),
            "participating_agents": len(self.agent_metrics),
            "active_crews": len(
                [
                    c
                    for c in self.crew_metrics.values()
                    if c.internal_collaborations + c.external_collaborations > 0
                ]
            ),
        }
    ...
    def _get_most_collaborative_crews(self, limit: int = 3) -> List[Dict[str, Any]]:
        ...
        return [
            {
                "crew": crew.crew_name,
                "total_collaborations": crew.internal_collaborations
                + crew.external_collaborations,
    ...
    def _generate_collaboration_recommendations(self) -> List[str]:
        ...
        if len(self.collaboration_events) > 0:
            ...
            ) / len(self.collaboration_events)

            if success_rate < 0.8:
                recommendations.append(
                    "Consider improving agent coordination mechanisms to increase collaboration success rate"
                )
            ...
            if len(cross_crew_events) < len(self.collaboration_events) * 0.3:
                recommendations.append(
                    "Increase cross-crew collaboration to improve knowledge sharing"
                )
            ...
            memory_events = [
                e
                for e in self.collaboration_events
                if e.collaboration_type == CollaborationType.MEMORY_SHARING
            ]
            if len(memory_events) < len(self.collaboration_events) * 0.2:
                recommendations.append(
                    "Enhance memory sharing mechanisms between agents"
                )

        if not recommendations:
            recommendations.append(
                "Collaboration patterns are performing well - continue monitoring"
            )
