...
class FlowHealthMonitor:
    ...
    async def _calculate_health_status(
        ...
        last_update = (
            flow.updated_at if hasattr(flow, "updated_at") else flow.created_at
        )
    ...
    async def _handle_failed_flow(self, flow: Any, flow_type: str, db):
        ...
        await self._log_health_event(
            ...
            "flow_failed",
            {
                ...
                "phase": flow.current_phase if hasattr(flow, "current_phase") else None,
                "errors": (
                    flow.error_details if hasattr(flow, "error_details") else None
                ),
            },
        )
    ...
    async def _should_auto_recover(self, flow: Any, flow_type: str) -> bool:
        ...
        if flow_id in self.health_metrics:
            ...
            if last_recovery and (datetime.utcnow() - last_recovery) < timedelta(
                minutes=30
            ):
                return False
    ...
    async def _attempt_auto_recovery(self, flow: Any, flow_type: str, db):
        ...
        try:
            ...
            current_phase = (
                flow.current_phase if hasattr(flow, "current_phase") else None
            )
            current_status = self._get_flow_status(flow, flow_type)
            ...
            if current_phase == "field_mapping" and current_status == "processing":
                ...
                logger.info(
                    f"✅ Auto-recovered flow {flow_id} by forcing field mapping approval state"
                )
                ...
                logger.info(
                    f"✅ Auto-recovered flow {flow_id} by completing with partial results"
                )
                ...
                if hasattr(flow, "error_details"):
                    flow.error_details = {
                        "auto_recovery": "Flow was hanging and auto-recovery failed"
                    }
                await db.commit()

                logger.warning(
                    f"⚠️ Auto-recovery failed for flow {flow_id}, marked as failed"
                )
    ...
    async def _log_health_event(
        self, flow_id: str, event_type: str, details: Dict[str, Any]
    ):
        """Log health monitoring event"""
        logger.info(
            f"📊 Health Event - Flow: {flow_id}, Type: {event_type}, Details: {json.dumps(details)}"
        )
    ...
    def get_all_health_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "hanging_flows": len(
                [
                    f
                    for f in self.health_metrics.values()
    ...
    async def force_recover_flow(
        self, flow_id: str, recovery_action: str = "restart"
    ) -> Dict[str, Any]:
        ...
        async with AsyncSessionLocal() as db:
            ...
            if recovery_action == "restart":
                ...
                if hasattr(flow, "error_details"):
                    flow.error_details = {
                        "reason": "Manual intervention - marked as failed"
                    }
                if hasattr(flow, "updated_at"):
                    flow.updated_at = datetime.utcnow()
                await db.commit()
