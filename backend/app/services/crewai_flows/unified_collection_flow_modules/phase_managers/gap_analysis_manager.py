...
from app.services.ai_analysis import BusinessContextAnalyzer, ConfidenceScorer, GapAnalysisAgent
...
class GapAnalysisManager:
    ...
    async def execute(self, flow_state, crewai_service, client_requirements) -> Dict[str, Any]:
        ...
        try:
            ...
            collection_gaps = flow_state.phase_results.get("automated_collection", {}).get("identified_gaps", [])
            
            # Perform AI gap analysis
            ai_analysis = await self._perform_ai_gap_analysis(
                collected_data, collection_gaps, client_requirements, flow_state.automation_tier
            )
            
            # Execute gap analysis crew
            crew_results = await self._execute_gap_analysis_crew(
                crewai_service, collected_data, ai_analysis, 
                client_requirements, flow_state.automation_tier
            )
            ...
            await self.audit_service.log_flow_event(
                ...
                event_data={
                    ...
                    "sixr_impact_score": processed_gaps["sixr_impact"]["overall_impact_score"],
            ...
            return {
                ...
                "sixr_impact_score": processed_gaps["sixr_impact"]["overall_impact_score"],
                "next_phase": next_phase.value,
                "requires_manual_collection": next_phase == CollectionPhase.QUESTIONNAIRE_GENERATION
            }
    ...
    async def _perform_ai_gap_analysis(self, collected_data: List[Dict],
        ...
        logger.info(f"✅ AI gap analysis complete: {len(gap_analysis_result.get('identified_gaps', []))} gaps found")
        return gap_analysis_result
    
    async def _execute_gap_analysis_crew(self, crewai_service, collected_data,
                                       ai_analysis, client_requirements,
    ...
    async def _process_gap_results(self, crew_results: Dict[str, Any],
    ...
    async def _update_flow_state(self, flow_state, processed_gaps: Dict[str, Any],
        ...
        flow_state.phase_results["gap_analysis"] = {
            "gaps_summary": {
                ...
                "critical_gaps": len([g for g in processed_gaps["identified_gaps"] 
                                    if g.get("severity") == "critical"]),
                "high_gaps": len([g for g in processed_gaps["identified_gaps"] 
                                if g.get("severity") == "high"]),
                "medium_gaps": len([g for g in processed_gaps["identified_gaps"] 
                                 if g.get("severity") == "medium"]),
                "low_gaps": len([g for g in processed_gaps["identified_gaps"] 
                               if g.get("severity") == "low"])
            },
            "sixr_impact_score": processed_gaps["sixr_impact"].get("overall_impact_score", 0.0),
            "gap_categories": list(processed_gaps["gap_categories"].keys()),
            "recommendations": processed_gaps["recommendations"]
        }
    ...
    def _determine_next_phase(self, processed_gaps: Dict[str, Any],
                            automation_tier: AutomationTier) -> CollectionPhase:
        ...
        if critical_gaps or sixr_score > 0.7:
            logger.info(f"Critical gaps or high SIXR impact ({sixr_score}) - manual collection required")
    ...
    def _enhance_gaps_with_ai_insights(self, gaps: List[Dict],
                                     ai_analysis: Dict[str, Any]) -> List[Dict]:
        """Enhance gap information with AI insights"""
        ai_gap_map = {g["id"]: g for g in ai_analysis.get("identified_gaps", []) if "id" in g}
    ...
    def _prioritize_gaps(self, gaps: List[Dict], sixr_impact: Dict[str, Any],
                       client_requirements: Dict[str, Any]) -> List[Dict]:
        ...
        prioritized_gaps = sorted(gaps, key=lambda g: g.get("priority_score", 0), reverse=True)
