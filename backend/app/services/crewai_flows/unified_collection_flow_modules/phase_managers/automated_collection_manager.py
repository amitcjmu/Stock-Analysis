...
class AutomatedCollectionManager:
    ...
    async def execute(
        ...
        try:
            logger.info(
                f"🤖 Starting automated collection for flow {self.flow_context.flow_id}"
            )
            ...
            adapter_recommendations = flow_state.collection_config.get(
                "adapter_recommendations", []
            )
            tier_analysis = flow_state.phase_results.get("platform_detection", {}).get(
                "tier_analysis", {}
            )
            ...
            await self.audit_service.log_flow_event(
                ...
                event_data={
                    ...
                    "collection_time_ms": processed_data["collection_metrics"].get(
                        "total_time_ms"
                    ),
                },
            )
    ...
    async def resume(
        ...
        """
        logger.info(
            f"🔄 Resuming automated collection for flow {self.flow_context.flow_id}"
        )
        ...
        if user_inputs and user_inputs.get("retry_failed_platforms"):
            ...
            if failed_platforms:
                logger.info(
                    f"🔁 Retrying collection for {len(failed_platforms)} failed platforms"
                )
    ...
    async def _execute_collection_crew(
        ...
        try:
            ...
            logger.error(
                f"Collection timed out after {self.collection_timeout_minutes} minutes"
            )
            raise
    ...
    async def _update_flow_state(
        ...
        flow_state.phase_results["automated_collection"] = {
            "collection_summary": {
                ...
                "platforms_succeeded": len(
                    [
                        p
                        for p, r in processed_data["platform_results"].items()
                        if r.get("status") == "success"
                    ]
                ),
                "platforms_failed": len(
                    [
                        p
                        for p, r in processed_data["platform_results"].items()
            ...
            "collection_time": processed_data["collection_metrics"].get(
                "total_time_ms", 0
            ),
            "identified_gaps": processed_data["identified_gaps"],
        }
    ...
    def _calculate_platform_coverage(
        self, platform_results: Dict[str, Any], detected_platforms: List[Dict]
    ) -> float:
