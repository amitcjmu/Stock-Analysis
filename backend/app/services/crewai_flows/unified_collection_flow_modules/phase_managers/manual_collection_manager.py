...
class ManualCollectionManager:
    ...
    async def execute(
        ...
        try:
            ...
            if (
                not skip_generation
                and flow_state.current_phase != CollectionPhase.MANUAL_COLLECTION
            ):
                return await self._generate_questionnaires(
    ...
    async def _generate_questionnaires(
    ...
    async def _execute_manual_collection(
        ...
        await self._update_progress_tracking(flow_state, validated_responses)

        # Update flow state
        await self._update_flow_state(flow_state, validated_responses, crew_results)
