...
class ValidationHandler:
    ...
    async def validate_data(
        ...
        try:
            ...
            # Integrate data
            integrated_data = (
                await self.services.data_integration.integrate_collection_data(
                    automated_data=automated_data,
                    manual_data=manual_data,
                    integration_rules=config.get("client_requirements", {}).get(
                        "integration_rules", {}
                    ),
                    conflict_resolution=config.get("client_requirements", {}).get(
            ...
            crew_result = await retry_with_backoff(
                crew.kickoff, inputs={"integrated_data": integrated_data}
            )

            # Generate quality report
            quality_report = (
                await self.services.quality_assessment.generate_final_quality_report(
            ...
            sixr_readiness_score = (
                await self.services.business_analyzer.calculate_sixr_readiness(
                    synthesized_data=integrated_data,
            ...
            # Update final scores
            state.collection_quality_score = quality_report.get(
                "overall_quality_score", 0.0
            )
            state.confidence_score = (
                await self.services.confidence_scoring.calculate_final_confidence(
                    synthesized_data=integrated_data,
                    quality_report=quality_report,
                    phase_results=state.phase_results,
                )
