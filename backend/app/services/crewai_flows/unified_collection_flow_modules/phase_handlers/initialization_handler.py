...
class InitializationHandler:
    ...
    async def initialize_collection(self, state: CollectionFlowState, config: Dict[str, Any]) -> Dict[str, Any]:
            ...
            if self.flow_context.db_session:
                try:
                    stmt = update(CollectionFlow).where(
                        CollectionFlow.id == uuid.UUID(state.flow_id)
                    ).values(
                    ...
                    logger.info(f"✅ Updated flow {state.flow_id} status from INITIALIZED to PLATFORM_DETECTION")
