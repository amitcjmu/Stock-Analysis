...
class CheckpointManager:
    ...
    async def create_checkpoint(
        ...
        try:
            ...
            logger.info(f"✅ Created checkpoint {checkpoint_id} for flow {flow_id} at phase {phase}")
    ...
    async def list_checkpoints(
        ...
        try:
            ...
            return [
                ...
                for cp in sorted(checkpoints, key=lambda cp: cp.created_at, reverse=True)
            ]
    ...
    async def _serialize_state(self, state: Any) -> Dict[str, Any]:
        ...
        try:
            ...
            attributes = [
                'flow_id', 'status', 'current_phase', 'progress_percentage',
                'phase_completion', 'raw_data', 'field_mappings', 'cleaned_data',
                'asset_inventory', 'app_server_dependencies', 'app_app_dependencies',
                'technical_debt_assessment', 'agent_insights', 'errors',
    ...
    async def _cleanup_old_checkpoints(self, flow_id: str):
        ...
        if len(checkpoints) > self.max_checkpoints_per_flow:
            ...
            self.checkpoints[flow_id] = sorted_checkpoints[:self.max_checkpoints_per_flow]
