...
class StateMigrator:
    ...
    async def migrate_active_flows(self) -> Dict[str, Any]:
        ...
        """
        try:
            logger.info("🔄 Starting migration of active flows from SQLite to PostgreSQL")
            ...
            for sqlite_path in sqlite_sources:
                ...
                migration_report['validation_errors'] += source_report['validation_errors']
            ...
            logger.info(f"✅ Migration completed: {migration_report['flows_migrated']} flows migrated")
            return migration_report
    ...
    async def _is_valid_sqlite_file(self, file_path: str) -> bool:
        ...
        try:
            ...
            return any(indicator in ' '.join(table_names).lower() for indicator in crewai_indicators)
    ...
    async def _extract_flow_state_from_row(self, row_dict: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            for sqlite_field, state_field in field_mappings.items():
                if sqlite_field in row_dict and row_dict[sqlite_field] is not None:
                    ...
                    # Handle JSON fields
                    if sqlite_field in ['data', 'state_data', 'metadata'] and isinstance(value, str):
                        try:
    ...
    async def _verify_migration_integrity(self, migration_report: Dict[str, Any]) -> None:
        ...
        try:
            ...
            for record in records:
                try:
                    if record.flow_state_data:
                        validation_result = self.validator.validate_complete_state(record.flow_state_data)
            ...
            if verification_results['flows_corrupted'] > 0:
                logger.warning(f"⚠️ {verification_results['flows_corrupted']} corrupted flows found after migration")
            else:
                logger.info(f"✅ All {verification_results['flows_verified']} migrated flows verified successfully")
...
async def verify_migration_status(context: RequestContext) -> Dict[str, Any]:
    ...
    try:
        async with AsyncSessionLocal() as db:
            ...
            sample_stmt = select(CrewAIFlowStateExtensions).where(
                CrewAIFlowStateExtensions.client_account_id == context.client_account_id
            ).limit(10)
