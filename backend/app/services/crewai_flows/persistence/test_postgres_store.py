    ...
    try:
        ...
        async with managed_postgres_store(context) as store:
            await store.save_state(
                flow_id=test_flow_id, state=test_state, phase="initialization"
            )
            ...
            await store.save_state(
                flow_id=test_flow_id, state=test_state, phase="data_import", version=1
            )
            ...
            for version in versions:
                print(
                    f"   Version {version['version']}: {version['phase']} at {version['created_at']}"
                )
...
async def test_state_recovery():
    ...
    try:
        ...
        if not validation_result["valid"]:
            print(
                f"✅ Correctly detected corrupted state: {len(validation_result['errors'])} errors"
            )
        else:
...
async def test_state_validator():
    ...
    valid_state = {
        ...
        "user_id": "user-789",
        "phase_completion": {"data_import": False, "field_mapping": False},
        "created_at": datetime.utcnow().isoformat(),
        "updated_at": datetime.utcnow().isoformat(),
    }
    ...
    state_for_transition = {
        "current_phase": "initialization",
        "phase_completion": {"initialization": True, "data_import": False},
    }
...
async def main():
    ...
    if passed == total:
        print(
            "🎉 All tests passed! PostgreSQL-only state management is working correctly."
        )
        return True
    else:
