...
class StatusHandler:
    ...
    def get_current_status(
        self, state, knowledge_bases: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def _calculate_completion_percentage(self, state) -> float:
        ...
        completed_phases = len(
            [
                status
                for status in state.crew_status.values()
    ...
    def _identify_critical_issues(self, state) -> List[Dict[str, Any]]:
        ...
        failed_crews = [
            name
            for name, status in state.crew_status.items()
            if status.get("status") == "failed"
        ]
    ...
    def _determine_next_actions(self, state) -> List[Dict[str, Any]]:
        ...
        phase_actions = {
            "initialization": [
                {
                    "action": "start_field_mapping",
                    "description": "Begin field mapping crew execution",
                }
            ],
            "field_mapping": [
                {
                    "action": "validate_mappings",
                    "description": "Validate field mapping results",
                },
                {
                    "action": "proceed_to_cleansing",
                    "description": "Proceed to data cleansing phase",
                },
            ],
            "data_cleansing": [
                {
                ...
                {
                    "action": "proceed_to_inventory",
                    "description": "Proceed to inventory building phase",
                },
            ],
            "inventory_building": [
                {
                    "action": "validate_classification",
                    "description": "Validate asset classification results",
                },
                {
                    "action": "proceed_to_dependencies",
                    "description": "Proceed to dependency analysis",
                },
            ],
            "app_server_dependencies": [
                {
                    "action": "validate_hosting_relationships",
                    "description": "Validate hosting relationships",
                },
                {
                    "action": "proceed_to_app_dependencies",
                    "description": "Proceed to app-app dependency analysis",
                },
            ],
            "app_app_dependencies": [
                {
                    "action": "validate_integrations",
                    "description": "Validate integration patterns",
                },
                {
                    "action": "proceed_to_technical_debt",
                    "description": "Proceed to technical debt assessment",
                },
            ],
            "technical_debt": [
                {
                    "action": "validate_6r_preparation",
                    "description": "Validate 6R strategy preparation",
                },
                {
                    "action": "finalize_discovery",
                    "description": "Finalize discovery integration",
                },
            ],
            "completed": [
                {"action": "review_results", "description": "Review discovery results"},
                {
                    ...
                    "description": "Prepare for Assessment Flow",
                },
            ],
        }
