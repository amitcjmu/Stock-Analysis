...
class UnifiedFlowManagement:
    ...
    def pause_flow(self, reason: str = "user_requested"):
        ...
        logger.info(
            f"🔄 Pausing flow at phase: {self.state.current_phase}, reason: {reason}"
        )
    ...
    def get_flow_management_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "agent_insights": (
                self.state.agent_insights[-5:] if self.state.agent_insights else []
            ),  # Last 5 insights
            "recent_errors": (
                self.state.errors[-3:] if self.state.errors else []
            ),  # Last 3 errors
            "recent_warnings": (
                self.state.warnings[-3:] if self.state.warnings else []
            ),  # Last 3 warnings
    ...
    def _validate_resumption_state(self) -> bool:
        ...
        try:
            ...
            if current_phase == "data_cleansing" and not phase_completion.get(
                "field_mapping", False
            ):
                return False

            if current_phase == "asset_inventory" and not phase_completion.get(
                "data_cleansing", False
            ):
                return False

            if current_phase == "dependency_analysis" and not phase_completion.get(
                "asset_inventory", False
            ):
                return False

            if current_phase == "tech_debt_analysis" and not phase_completion.get(
            ...
            if self.state.errors:
                critical_errors = [
                    e for e in self.state.errors if e.get("severity") == "critical"
                ]
                if critical_errors:
                    return False
