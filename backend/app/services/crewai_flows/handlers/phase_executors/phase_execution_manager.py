...
class PhaseExecutionManager:
    ...
    async def cleanup_phase_states(
        self, flow_id: str, expiration_hours: int = 72
    ) -> Dict[str, Any]:
        """
        ...
        try:
            cleanup_result = await self.flow_bridge.cleanup_expired_states(
                expiration_hours
            )
