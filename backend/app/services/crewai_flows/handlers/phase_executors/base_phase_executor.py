...
class BasePhaseExecutor(ABC):
    ...
    def __init__(self, state, crew_manager):
        """Initialize phase executor with state and crew manager references"""
        self.state = state
    ...
    def execute(self, previous_result) -> str:
        ...
        try:
            ...
            if CREWAI_FLOW_AVAILABLE:
                ...
                if crew:
                    ...
                    results = self.execute_with_crew(crew_input)
                else:
                    logger.warning(f"{phase_name} crew not available - using fallback")
                    results = self.execute_fallback()
            else:
