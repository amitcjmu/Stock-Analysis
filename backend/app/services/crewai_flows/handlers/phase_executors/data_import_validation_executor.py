...
class DataImportValidationExecutor(BasePhaseExecutor):
    ...
    async def execute_with_crew(self, crew_input: Dict[str, Any]) -> Dict[str, Any]:
        ...
        logger.info(
            "🤖 Using CrewAI agents for data import validation (agentic-first approach)"
        )
        ...
        try:
            ...
            if self.crew_manager.is_crew_available("data_import_validation"):
                ...
                if crew:
                    logger.info(
                        "✅ Data import validation crew created - executing agentic analysis"
                    )
                    ...
                    logger.error(
                        "⚠️ Data import validation crew creation failed - NO FALLBACK"
                    )
                    raise RuntimeError(
                        "Data import validation crew creation failed. This needs to be fixed."
                    )
            else:
                # NO FALLBACK - Crew not available is a real issue
                logger.error(
                    "⚠️ Data import validation crew not available - NO FALLBACK"
                )
                raise RuntimeError(
                    "Data import validation crew not available. This needs to be fixed."
                )
            ...
            logger.info(
                "🔄 Executing fallback data import validation with agent patterns"
            )
            ...
            if (
                ...
                and len(self.state.raw_data) > 0
            ):
                logger.info(
                    f"🔍 DEBUG: First record keys: {list(self.state.raw_data[0].keys())}"
                )
            ...
            logger.info(
                f"✅ Agent-pattern data validation completed in {validation_time:.2f} seconds"
            )
        ...
        try:
            ...
            if self.state.metadata:
                ...
                if not source_check["valid"]:
                    logger.warning(
                        f"⚠️ Source validation warning: {source_check['warning']}"
                    )

            # 7. Generate detailed user report (agent-like comprehensive analysis)
            detailed_report = self._generate_user_report(
                results, file_analysis, pii_check, security_check, type_check
            )
            ...
            field_count = (
                len(self.state.raw_data[0].keys()) if self.state.raw_data else 0
            )

            results["summary"] = (
                f"📊 Agent Analysis Complete: {file_analysis['detected_type']} data detected. "
            ...
            results.update(
                {
                    ...
                    "detected_fields": (
                        list(self.state.raw_data[0].keys())
        ...
        try:
            ...
            for type_name, indicators in all_indicators.items():
                ...
                type_scores[type_name] = (
                    matches / len(field_names) if field_names else 0
                )
            ...
            if best_score < 0.2:
                ...
                recommended_agent = agent_mapping.get(
                    best_type, "CMDB_Data_Analyst_Agent"
                )

            # Generate analysis details (agent-like reporting)
            analysis_details = (
                f"Analyzed {len(field_names)} fields using agent pattern recognition. "
    ...
    def _check_data_structure(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"🔍 DEBUG: Has raw_data attribute: {hasattr(self.state, 'raw_data')}"
            )
            ...
            if not self.state.raw_data:
                ...
                return {
                    "valid": False,
                    "reason": "No data provided - raw_data is None or empty",
                }

            if not isinstance(self.state.raw_data, list):
                logger.error(
                    f"❌ DEBUG: raw_data is not a list, type: {type(self.state.raw_data)}"
                )
                return {
                    "valid": False,
                    "reason": f"Data must be a list of records, got {type(self.state.raw_data)}",
                }
            ...
            if len(self.state.raw_data) == 0:
                ...
                return {
                    "valid": False,
                    "reason": "Empty data set - no records to validate",
                }
            ...
            if not isinstance(first_record, dict):
                logger.error(
                    f"❌ DEBUG: First record is not a dict, type: {type(first_record)}"
                )
                return {
                    "valid": False,
                    "reason": f"Records must be dictionaries, got {type(first_record)}",
                }
            ...
            if len(first_record.keys()) == 0:
                ...
                return {
                    "valid": False,
                    "reason": "Records must have fields - first record is empty",
                }
    ...
    def _detect_pii(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"pii_found": pii_found, "pii_types": pii_types}
    ...
    def _scan_for_malicious_content(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"threats_found": threats_found, "threat_types": threat_types}
        ...
        try:
            ...
            report = {
                ...
                "file_analysis": {
                    ...
                    "analysis_details": file_analysis.get(
                        "analysis_details", "No analysis available"
                    ),
                },
                "security_analysis": {
