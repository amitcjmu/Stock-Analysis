...
class DataCleansingExecutor(BasePhaseExecutor):
    ...
    async def execute_with_crew(self, crew_input: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if not assets_for_analysis:
                ...
                raise RuntimeError("No assets prepared for agentic analysis - check raw data preparation")
    ...
    async def execute_fallback(self) -> Dict[str, Any]:
        ...
        logger.error("❌ If you see this error, fix the actual agentic intelligence execution issues")
        raise RuntimeError("Data cleansing fallback disabled - agentic intelligence must work properly")
    ...
    def _store_results(self, results: Dict[str, Any]):
        ...
        if not cleaned_data:
            logger.error("❌ No cleaned_data in results - this indicates crew execution failure")
            logger.error(f"❌ Results received: {list(results.keys())}")
            raise RuntimeError("Data cleansing crew did not return cleaned_data - crew execution failed")
    ...
    def _process_crew_result(self, crew_result) -> Dict[str, Any]:
            ...
            try:
                if '{' in crew_result.raw and '}' in crew_result.raw:
                    ...
                    if 'cleaned_data' in parsed_result or 'standardized_data' in parsed_result:
                        ...
                        logger.error(f"❌ Crew JSON missing required keys. Got: {list(parsed_result.keys())}")
                        raise ValueError("Data cleansing crew returned JSON without cleaned_data or standardized_data")
                else:
                    logger.error(f"❌ Crew output does not contain valid JSON: {crew_result.raw}")
        ...
        # If crew_result is already a dict, validate it
            ...
            if 'cleaned_data' in crew_result or 'standardized_data' in crew_result:
                ...
                logger.error(f"❌ Crew dict missing required keys. Got: {list(crew_result.keys())}")
                raise ValueError("Data cleansing crew returned dict without cleaned_data")
