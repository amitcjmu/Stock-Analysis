...
class InitializationHandler:
    ...
    def setup_shared_memory(self) -> Optional[LongTermMemory]:
        ...
        if not CREWAI_ADVANCED_AVAILABLE:
            logger.warning(
                "Using fallback memory - CrewAI advanced features not available"
            )
            return None
    ...
    def setup_knowledge_bases(self) -> Dict[str, Knowledge]:
        ...
        if not CREWAI_ADVANCED_AVAILABLE:
            logger.warning(
                "Using fallback knowledge bases - CrewAI advanced features not available"
            )
            return {
                "field_mapping": None,
                "data_quality": None,
        ...
        knowledge_sources = {
            ...
            "data_quality": ["backend/app/knowledge_bases/data_quality_standards.yaml"],
        ...
        for domain, sources in knowledge_sources.items():
            try:
                ...
                existing_sources = [
                    source for source in sources if os.path.exists(source)
                ]
                if existing_sources:
                    logger.info(
                        f"✅ {domain} knowledge base bypassed (sources found but skipped to avoid embedding requirement)"
                    )
                else:
                    logger.warning(
                        f"⚠️ {domain} knowledge base bypassed - no source files found"
                    )
    ...
    def setup_flow_id(
        self, client_account_id: str, engagement_id: str, raw_data: List[Dict[str, Any]]
    ) -> str:
    ...
    def initialize_flow_state(
        ...
        flow_id: str,
        shared_memory: Optional[LongTermMemory],
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            "shared_memory_id": (
                getattr(shared_memory, "storage_id", "shared_memory_default")
                if shared_memory
