    ...
    def execute_app_server_dependency_crew(self, state) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                # Parse crew results
                app_server_dependencies = self.parser.parse_dependency_results(
                    crew_result, "app_server"
                )

                logger.info(
                    "✅ Enhanced App-Server Dependency Crew executed successfully"
                )

            except Exception as crew_error:
                logger.warning(
                    f"Enhanced App-Server Dependency Crew execution failed, using fallback: {crew_error}"
                )
                # Fallback dependency mapping
                app_server_dependencies = (
                    self.fallback_handler.intelligent_dependency_fallback(
                        state.asset_inventory, "app_server"
                    )
                )
    ...
    def execute_app_app_dependency_crew(self, state) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                # Parse crew results
                app_app_dependencies = self.parser.parse_dependency_results(
                    crew_result, "app_app"
                )
                ...
                logger.warning(
                    f"Enhanced App-App Dependency Crew execution failed, using fallback: {crew_error}"
                )
                # Fallback dependency mapping
                app_app_dependencies = (
                    self.fallback_handler.intelligent_dependency_fallback(
                        state.asset_inventory, "app_app"
                    )
                )
