...
class AssetClassificationTool(BaseTool):
    ...
    description: str = "Advanced asset classification with industry patterns and historical data"
    ...
    def _run(self, asset_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            classification_patterns = {
                "server": {
                    "indicators": ["cpu", "memory", "storage", "operating_system", "hostname"],
                    "subcategories": ["web_server", "database_server", "application_server", "file_server"]
                },
                "database": {
                    "indicators": ["database_type", "version", "size", "connections", "schemas"],
                    "subcategories": ["relational", "nosql", "data_warehouse", "cache"]
                },
                "application": {
                    "indicators": ["application_type", "framework", "language", "dependencies"],
                    "subcategories": ["web_application", "desktop_application", "mobile_app", "service"]
                },
                "network": {
            ...
            for asset_type, pattern in classification_patterns.items():
                ...
                for indicator in pattern["indicators"]:
                    if any(indicator.lower() in str(value).lower() 
                          for value in asset_data.values() if value):
                        score += 1
...
class AssetIntelligenceCrew:
    ...
    def __init__(self):
        ...
        logger.info("🎯 Asset Intelligence Crew initialized with sequential collaboration pattern")
    ...
    async def analyze_assets(self, assets_data: List[Dict[str, Any]], 
        ...
        """
        try:
            logger.info(f"🚀 Starting Asset Intelligence Crew analysis for {len(assets_data)} assets")
            
            analysis_results = []
            crew_insights = []
            
            for i, asset_data in enumerate(assets_data):
                logger.info(f"📊 Analyzing asset {i+1}/{len(assets_data)}: {asset_data.get('name', 'Unknown')}")
...
__all__ = ["AssetIntelligenceCrew", "create_asset_intelligence_crew", "AssetAnalysisResult"]
