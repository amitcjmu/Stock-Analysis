...
class PhaseHandlers:
        ...
        try:
            ...
            if hasattr(self.flow, "flow_bridge") and self.flow.flow_bridge:
                ...
                async with AsyncSessionLocal() as db:
                    store = PostgresFlowStateStore(db, self.flow.context)
                    await store.update_flow_status(
                        self.flow._flow_id, "processing_field_mapping"
                    )
    ...
    async def apply_approved_field_mappings(self, field_mapping_approval_result):
        ...
        self.logger.info(
            f"✅ [ECHO] Applying approved field mappings for flow {self.flow._flow_id}"
        )
        ...
        try:
            ...
            if hasattr(self.flow, "flow_bridge") and self.flow.flow_bridge:
                ...
                async with AsyncSessionLocal() as db:
                    store = PostgresFlowStateStore(db, self.flow.context)
                    await store.update_flow_status(
                        self.flow._flow_id, "applying_field_mappings"
                    )

            # Apply mappings using the state manager
            if hasattr(self.flow, "state_manager"):
                mapping_application_result = (
                    await self.flow.state_manager.apply_field_mappings(
                        self.flow.state, field_mapping_approval_result
                    )
                )
            else:
                ...
                mapping_application_result = {
                    ...
                    "applied_mappings": field_mapping_approval_result.get(
                        "approved_mappings", {}
                    ),
                }
                ...
                if "approved_mappings" in field_mapping_approval_result:
                    self.flow.state.applied_field_mappings = (
                        field_mapping_approval_result["approved_mappings"]
                    )
