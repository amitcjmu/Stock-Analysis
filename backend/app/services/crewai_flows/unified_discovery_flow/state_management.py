...
class StateManager:
    ...
    async def update_phase_status(self, phase_name: str, status: str, result: Optional[Dict[str, Any]] = None):
        """
    ...
    def is_user_approval_needed(self) -> bool:
        ...
        if self.state.current_phase == PhaseNames.FIELD_MAPPING:
            ...
            for field in FlowConfig.USER_APPROVAL_REQUIRED_FIELDS:
                if field in ["field_mappings", "data_quality_threshold", "asset_classification_confidence"]:
                    return True
