...
class CrewCoordinator:
    ...
    async def execute_phase_agent(
        ...
        try:
            if self.agent_timeout is not None:
                ...
                logger.info(
                    f"🔄 Agent '{agent_name}' executing without timeout (agentic activity)"
                )
                result = await self._execute_agent(
                    agent_name, input_data, agent_context
                )
            ...
            logger.error(
                f"❌ Agent '{agent_name}' timed out after {self.agent_timeout} seconds"
            )
            raise
    ...
    async def execute_parallel_agents(
        ...
        try:
            ...
            background_tasks = [
                t for t in all_tasks if not t.done() and t != asyncio.current_task()
            ]
            if background_tasks:
                logger.warning(
                    f"⚠️ Found {len(background_tasks)} background tasks still running"
                )
            ...
            for phase_name, result in zip(phases, results):
                if isinstance(result, Exception):
                    errors.append({"phase": phase_name, "error": str(result)})
                else:
                    combined_results[phase_name] = result

            if errors:
                logger.warning(
                    f"⚠️ {len(errors)} agents failed during parallel execution"
                )
    ...
    def _prepare_agent_context(
        self, phase_name: str, context_data: Dict[str, Any] = None
    ) -> Dict[str, Any]:
    ...
    async def _execute_agent(
        self, agent_name: str, input_data: Dict[str, Any], context: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        if agent_name in [
            ...
            logger.warning(
                f"Pseudo-agent {agent_name} called - should be replaced with real CrewAI agent"
            )
            return {
                "status": "success",
                "agent": agent_name,
    ...
    async def _execute_asset_inventory_agent(
    ...
    async def _execute_analysis_agent(
        ...
        logger.warning(
            f"Analysis agent {agent_name} called - should be replaced with real CrewAI agent"
        )
    ...
    def get_agent_config(self, agent_name: str) -> Dict[str, Any]:
        ...
        agent_configs = {
            ...
            "dependency_analysis_agent": {"depth": 3, "include_indirect": True},
