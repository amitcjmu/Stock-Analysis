...
class DataUtilities:
        ...
        try:
            ...
            async with AsyncSessionLocal() as db:
                ...
                if discovery_flow and discovery_flow.data_import_id:
                    ...
                    self.logger.info(f"🔍 Found discovery flow with data_import_id: {data_import_id}")
                else:
                    # Fallback: Check if flow_id is actually a data_import_id
                    self.logger.info(f"🔍 No discovery flow found, checking if {flow_id_to_use} is a data_import_id")
                    ...
                    if data_import:
                        ...
                        self.logger.info(f"✅ Found data import directly with id: {data_import_id}")
                ...
                if data_import_id:
                    ...
                    self.logger.info(f"✅ Loaded {len(raw_data)} raw records from database")
                    ...
                    self.logger.warning(f"⚠️ No data import found for flow {flow_id_to_use}")
                    return []
    ...
    async def process_data_for_validation(self, raw_data: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        try:
            ...
            processed_data = {
                ...
                'data_preview': raw_data[:5] if raw_data else [],  # First 5 records for preview
    ...
    async def extract_field_information(self, processed_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            for header in headers:
                ...
                field_analysis = {
                    ...
                    'mapping_suggestions': self._suggest_field_mappings(header, field_values)
                }
    ...
    async def apply_data_transformations(self, data: List[Dict[str, Any]], mappings: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            for record in data:
                try:
                    ...
                    if isinstance(original_data, dict):
                        ...
                        for source_field, mapping_info in mappings.items():
                            if source_field in original_data:
                                ...
                                transformation = mapping_info.get('transformation', 'direct_copy')
                                
                                value = original_data[source_field]
                                transformed_value = self._apply_transformation(value, transformation)
                    ...
                    self.logger.warning(f"⚠️ Failed to transform record {record.get('id', 'unknown')}: {record_error}")
            ...
            self.logger.info(f"✅ Transformed {len(transformed_data)} records with {transformation_stats['field_mappings_applied']} field mappings")
            return result
    ...
    async def create_asset_records(self, transformed_data: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        try:
            ...
            for record in transformed_data:
                try:
                    ...
                    self.logger.warning(f"⚠️ Failed to create asset from record: {asset_error}")
                    creation_stats['validation_failures'] += 1
                    continue
            ...
            self.logger.info(f"✅ Created {len(assets)} asset records from transformed data")
            return result
    ...
    def _extract_asset_name(self, record: Dict[str, Any]) -> str:
        ...
        for key, value in record.items():
            if any(keyword in key.lower() for keyword in ['name', 'title', 'label']) and isinstance(value, str):
                return value
