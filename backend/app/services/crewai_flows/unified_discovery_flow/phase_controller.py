...
class PhaseController:
    ...
    async def resume_flow_execution(self, 
                                   from_phase: FlowPhase, 
                                   user_input: Dict[str, Any] = None) -> PhaseExecutionResult:
        """
        ...
        if user_input:
            ...
            self.phase_results[from_phase] = PhaseExecutionResult(
                phase=from_phase,
                status="user_input_provided",
                data=user_input
            )
        
        # For field mapping approval, move directly to data cleansing
        if from_phase == FlowPhase.FIELD_MAPPING_APPROVAL and user_input and user_input.get("user_approval"):
    ...
    async def execute_current_phase(self) -> PhaseExecutionResult:
        ...
        try:
            ...
            logger.info(f"✅ Phase {self.current_phase.value} completed with status: {result.status}")
            
            # Check if we need to pause for user input
            if result.requires_user_input:
                logger.info(f"⏸️ Phase {self.current_phase.value} requires user input - pausing flow")
    ...
    async def _execute_initialization(self, previous_result) -> PhaseExecutionResult:
        ...
        return PhaseExecutionResult(
            ...
            data={"result": result},
            next_phase=FlowPhase.DATA_IMPORT_VALIDATION if result == "initialization_completed" else None
        )
    
    async def _execute_data_import_validation(self, previous_result) -> PhaseExecutionResult:
        """Execute data import validation phase"""
        result = await self.discovery_flow.execute_data_import_validation_agent(previous_result)
        
        return PhaseExecutionResult(
            phase=FlowPhase.DATA_IMPORT_VALIDATION,
            status="completed" if result not in ["discovery_failed", "data_validation_failed"] else "failed",
            data={"result": result},
            next_phase=FlowPhase.FIELD_MAPPING_SUGGESTIONS if result not in ["discovery_failed", "data_validation_failed"] else None
        )
    
    async def _execute_field_mapping_suggestions(self, previous_result) -> PhaseExecutionResult:
        """Execute field mapping suggestions generation"""
        result = await self.discovery_flow.generate_field_mapping_suggestions(previous_result)
    ...
    async def _execute_field_mapping_approval(self, previous_result) -> PhaseExecutionResult:
        """Execute field mapping approval (pause for user input)"""
        result = await self.discovery_flow.pause_for_field_mapping_approval(previous_result)
    ...
    async def _execute_data_cleansing(self, previous_result) -> PhaseExecutionResult:
        ...
        if isinstance(previous_result, dict) and previous_result.get("approved_mappings"):
            # Apply approved mappings first
            await self.discovery_flow.apply_approved_field_mappings("field_mapping_approved")
            result = await self.discovery_flow.execute_data_cleansing_agent("field_mapping_approved")
        else:
            result = await self.discovery_flow.execute_data_cleansing_agent(previous_result)
        
        return PhaseExecutionResult(
            phase=FlowPhase.DATA_CLEANSING,
            status="completed" if result != "data_cleansing_failed" else "failed",
            data={"result": result},
            next_phase=FlowPhase.ASSET_INVENTORY if result != "data_cleansing_failed" else None
        )
    
    async def _execute_asset_inventory(self, previous_result) -> PhaseExecutionResult:
        """Execute asset inventory phase"""
        result = await self.discovery_flow.create_discovery_assets_from_cleaned_data(previous_result)
        
        return PhaseExecutionResult(
            phase=FlowPhase.ASSET_INVENTORY,
            status="completed" if result != "asset_inventory_failed" else "failed",
            data={"result": result},
            next_phase=FlowPhase.DEPENDENCY_ANALYSIS if result != "asset_inventory_failed" else None
        )
    
    async def _execute_dependency_analysis(self, previous_result) -> PhaseExecutionResult:
        """Execute dependency analysis phase"""
        # This will run parallel analysis (dependencies + tech debt)
        result = await self.discovery_flow.execute_parallel_analysis_agents(previous_result)
        
        return PhaseExecutionResult(
            phase=FlowPhase.DEPENDENCY_ANALYSIS,
            status="completed" if result == "parallel_analysis_completed" else "failed",
            data={"result": result},
            next_phase=FlowPhase.TECH_DEBT_ASSESSMENT if result == "parallel_analysis_completed" else None
        )
    
    async def _execute_tech_debt_assessment(self, previous_result) -> PhaseExecutionResult:
    ...
    async def _execute_finalization(self, previous_result) -> PhaseExecutionResult:
        ...
        return PhaseExecutionResult(
            ...
            status="completed" if result == "discovery_completed" else "waiting_for_approval",
            data={"result": result},
    ...
    def get_flow_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "progress_percentage": (len(self.phase_results) / len(self.phase_sequence)) * 100
        }
