...
class UnifiedAssessmentFlow(Flow[AssessmentFlowState]):
    ...
    def __init__(self, crewai_service, context: RequestContext, selected_application_ids: List[str], **kwargs):
        ...
        logger.info("🚀 Initializing Unified Assessment Flow with Agent-First Architecture")
    ...
    def _initialize_components(self):
        ...
        try:
            ...
            # Initialize crews
            self.architecture_standards_crew = ArchitectureStandardsCrew(self.flow_context)
    ...
    async def initialize_assessment(self):
        ...
        logger.info(f"🎯 Starting Assessment Flow for {len(self.selected_application_ids)} applications")
    ...
    async def capture_architecture_minimums(self, previous_result):
        ...
        try:
            ...
            if not engagement_standards:
                logger.info("No existing engagement standards found, initializing defaults")
            ...
            if self.architecture_standards_crew:
                ...
                engagement_standards = crew_result.get("engagement_standards", engagement_standards)
                ...
                self.state.phase_results["architecture_minimums"] = {
                    ...
                    "application_compliance": crew_result.get("application_compliance", {}),
                    "exceptions": crew_result.get("exceptions", [])
                }
            else:
                logger.warning("Architecture Standards Crew not available, using defaults")
                self.state.phase_results["architecture_minimums"] = {
                    "standards_count": len(engagement_standards),
        ...
        try:
            ...
            for app_id in self.state.selected_application_ids:
                ...
                if self.component_analysis_crew:
                    ...
                    logger.info(f"✅ Identified {len(components)} components and {len(tech_debt_items)} tech debt items for {app_id}")
                    
                else:
                    logger.warning(f"Component Analysis Crew not available, using placeholder analysis for {app_id}")
                    # Placeholder analysis
                    self.state.application_components[app_id] = [
                        {"name": "main_application", "type": "web_frontend", "tech_debt_score": 5.0}
                    ]
                    self.state.tech_debt_analysis[app_id] = [
                        {"category": "technology", "severity": "medium", "score": 5.0, "description": "Placeholder tech debt analysis"}
                    ]
            ...
            # Store phase results
            total_components = sum(len(components) for components in self.state.application_components.values())
            total_debt_items = sum(len(items) for items in self.state.tech_debt_analysis.values())
        ...
        try:
            ...
            for app_id in self.state.selected_application_ids:
                ...
                if self.sixr_strategy_crew:
                    ...
                    logger.warning(f"Six R Strategy Crew not available, using placeholder strategy for {app_id}")
                    ...
                    rationale = "Default replatform strategy applied due to crew unavailability"
                    move_group_hints = []
                ...
                compatibility_issues = self.state.validate_component_compatibility(app_id)
                ...
                logger.info(f"✅ 6R strategy determined for {app_id}: {overall_strategy} (confidence: {confidence_score:.2f})")
            ...
            self.state.phase_results["component_sixr_strategies"] = {
                ...
                "overall_confidence": sum(decision.confidence_score for decision in self.state.sixr_decisions.values()) / len(self.state.sixr_decisions),
    ...
    async def generate_app_on_page(self, previous_result):
        ...
        try:
            ...
            user_sixr_input = self.state.user_inputs.get("component_sixr_strategies", {})
            if user_sixr_input:
            ...
            for app_id in self.state.selected_application_ids:
                ...
                app_on_page_data = await self._generate_app_on_page_data(app_id, decision)
            ...
            self.state.phase_results["app_on_page_generation"] = {
                "apps_with_pages": len([d for d in self.state.sixr_decisions.values() if d.app_on_page_data]),
    ...
    async def resume_from_phase(self, phase: AssessmentPhase, user_input: Dict[str, Any]) -> AssessmentFlowState:
        ...
        try:
            ...
            if phase == AssessmentPhase.ARCHITECTURE_MINIMUMS:
                return await self.analyze_technical_debt("resumed_from_architecture_minimums")
            elif phase == AssessmentPhase.TECH_DEBT_ANALYSIS:
                return await self.determine_component_sixr_strategies("resumed_from_tech_debt_analysis")
            elif phase == AssessmentPhase.COMPONENT_SIXR_STRATEGIES:
                return await self.generate_app_on_page("resumed_from_component_sixr_strategies")
            elif phase == AssessmentPhase.APP_ON_PAGE_GENERATION:
                return await self.finalize_assessment("resumed_from_app_on_page_generation")
            else:
        ...
        return [
            {
                ...
                "technology_specifications": {"minimum_version": "11", "recommended_version": "17"}
            }
        ]
    ...
    async def _generate_app_on_page_data(self, app_id: str, decision: SixRDecision) -> Dict[str, Any]:
        ...
        return {
            ...
            "migration_readiness": {
                ...
                "blockers": [risk for risk in decision.risk_factors if "critical" in str(risk).lower()]
            },
    ...
    async def _generate_assessment_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "strategy_distribution": self._get_strategy_distribution(),
            "overall_confidence": sum(d.confidence_score for d in self.state.sixr_decisions.values()) / len(self.state.sixr_decisions) if self.state.sixr_decisions else 0.0,
