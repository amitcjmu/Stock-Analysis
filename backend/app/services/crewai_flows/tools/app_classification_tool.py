...
class AppClassificationTool(BaseTool):
    ...
    def _run(self, application_data: str) -> str:
        ...
        try:
            ...
            for app in data.get("applications", []):
                ...
                criticality = self._assess_business_criticality(
                    app, classified_category
                )
    ...
    def _assess_business_criticality(self, app: Dict[str, Any], category: str) -> str:
        ...
        app_info = str(app).lower()
        if any(
            indicator in app_info for indicator in ["critical", "production", "core"]
        ):
            return "high"
    ...
    def _extract_technical_attributes(self, app: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if any(
            ...
            for indicator in ["kubernetes", "docker", "microservice"]
        ):
    ...
    def _get_criticality_distribution(
        self, classifications: Dict[str, Any]
    ) -> Dict[str, int]:
...
class ServiceDiscoveryTool(BaseTool):
    ...
    description: str = (
        "Discovers and analyzes application services and their relationships"
    )
    ...
    def _run(self, service_data: str) -> str:
        ...
        try:
            ...
            discovery_results = {
                ...
                "service_mesh_candidates": self._identify_service_mesh_candidates(
                    data.get("applications", [])
                ),
            }
    ...
    def _analyze_service_patterns(
        ...
        for app in applications:
            ...
            if any(
                indicator in app_info
                for indicator in ["microservice", "docker", "kubernetes", "api"]
            ):
                patterns["microservices_indicators"] += 1

            # Monolith indicators
            if any(
                indicator in app_info for indicator in ["monolith", "legacy", "single"]
            ):
    ...
    def _identify_api_endpoints(
        ...
        for app in applications:
            app_info = str(app).lower()
            if any(
                api_indicator in app_info
                for api_indicator in ["api", "rest", "soap", "endpoint"]
            ):
                endpoints.append(
                    {
    ...
    def _identify_service_mesh_candidates(
        self, applications: List[Dict[str, Any]]
    ) -> List[str]:
        ...
        for app in applications:
            ...
            if any(
                indicator in app_info
                for indicator in ["microservice", "distributed", "api"]
            ):
    ...
    def _assess_scalability(self, app: Dict[str, Any]) -> str:
        ...
        if any(
            indicator in app_info for indicator in ["cloud", "container", "kubernetes"]
        ):
