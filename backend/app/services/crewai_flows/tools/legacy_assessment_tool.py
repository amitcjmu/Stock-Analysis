...
class LegacyAssessmentTool(BaseTool):
    ...
    description: str = (
        "Assesses legacy technology characteristics and modernization urgency"
    )
                            ...
                            inventory[category].append(
                                {
                                    ...
                                    "technology": tech,
                                    "severity": severity,
                                    "details": self._extract_technology_details(
                                        asset, tech
                                    ),
                                }
                            )
    ...
    def _analyze_technology_age(self, assets: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        technology_age["age_distribution"] = {
            "end_of_life_percentage": (
                (len(technology_age["end_of_life"]) / total_assets) * 100
                if total_assets > 0
                else 0
            ),
            "end_of_support_percentage": (
                (len(technology_age["end_of_support"]) / total_assets) * 100
                if total_assets > 0
                else 0
            ),
            "extended_support_percentage": (
                (len(technology_age["extended_support"]) / total_assets) * 100
                if total_assets > 0
                else 0
            ),
            "current_percentage": (
                (len(technology_age["current"]) / total_assets) * 100
                if total_assets > 0
                else 0
            ),
        }
    ...
    def _assess_modernization_urgency(
        self, assets: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        ...
        for asset in assets:
            ...
            if any(
                tech in asset_info
                for tech in ["windows 2003", "windows xp", "java 6", "java 7"]
            ):
                urgency_score += 3
            ...
            if any(
                indicator in asset_info for indicator in ["pci", "hipaa", "sox", "gdpr"]
            ):
                urgency_score += 2
            ...
            if any(
                indicator in asset_info
                for indicator in ["critical", "production", "core"]
            ):
                urgency_score += 2
            ...
            if any(
                indicator in asset_info
                for indicator in ["slow", "performance", "capacity"]
            ):
                urgency_score += 1
    ...
    def _assess_legacy_risks(self, assets: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for asset in assets:
            ...
            if any(
                tech in asset_info
                for tech in ["windows 2003", "windows xp", "ie6", "ie7"]
            ):
            ...
            if any(indicator in asset_info for indicator in ["pci", "hipaa", "sox"]):
                if any(
                    tech in asset_info for tech in ["windows 2008", "sql server 2008"]
                ):
            ...
            if any(
                indicator in asset_info
                for indicator in ["critical", "revenue", "customer"]
            ):
        ...
        if total_assets > 0:
            risk_assessment["overall_risk_score"] = (
                ...
                + risk_counts["business"] * 1.5
            ) / total_assets

        return risk_assessment

    def _generate_modernization_roadmap(
        self, assets: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
    ...
    def _extract_technology_details(
        self, asset: Dict[str, Any], technology: str
    ) -> Dict[str, Any]:
    ...
    def _recommend_modernization_strategy(
        ...
        if any(
            indicator in asset_info
            for indicator in ["monolith", "legacy_code", "performance"]
        ):
            strategy = {
        ...
        if any(
            indicator in asset_info
            for indicator in ["distributed", "microservices", "cloud_native"]
        ):
            strategy = {
        ...
        if any(
            indicator in asset_info
            for indicator in ["deprecated", "unused", "redundant"]
        ):
            strategy = {
        ...
        if any(
            indicator in asset_info
            for indicator in ["compliance", "specialized", "recent"]
        ):
            strategy = {
...
class TechnologyAnalysisTool(BaseTool):
    ...
    description: str = (
        "Analyzes technology stack composition and modernization patterns"
    )
    ...
    def _build_technology_inventory(
        self, assets: List[Dict[str, Any]]
    ...
    def _analyze_technology_stacks(
        self, assets: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        ...
        for asset in assets:
            ...
            # MEAN stack (MongoDB, Express, Angular, Node.js)
                ...
                tech in asset_info for tech in ["mongodb", "express", "angular", "node"]
            ):
                stacks["mean_stack"] += 1
            ...
            # Microsoft stack
                ...
                tech in asset_info
                for tech in ["windows", "iis", "sql server", "dotnet"]
            ):
                stacks["microsoft_stack"] += 1

            # Java Enterprise
            elif any(
                tech in asset_info for tech in ["java", "jboss", "websphere", "spring"]
            ):
                stacks["java_enterprise"] += 1

            # Cloud Native
            elif any(
                tech in asset_info for tech in ["docker", "kubernetes", "microservice"]
            ):
        ...
        return {
            ...
            "dominant_stack": (
                max(stacks, key=stacks.get) if any(stacks.values()) else "mixed"
            ),
        }

    def _identify_modernization_patterns(
        self, assets: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        ...
        for asset in assets:
            ...
            if (
                any(tech in asset_info for tech in ["java", "nodejs", "python", "ruby"])
                and "docker" not in asset_info
            ):
                patterns["containerization_candidates"].append(asset_name)

            # Cloud migration candidates
            if (
                any(tech in asset_info for tech in ["stateless", "api", "web"])
                and "cloud" not in asset_info
            ):
            ...
            if any(
                tech in asset_info
                for tech in ["sql server 2008", "oracle 10g", "mysql 5"]
            ):
    ...
    def _map_technology_dependencies(
        self, assets: List[Dict[str, Any]]
    ) -> Dict[str, List[str]]:
