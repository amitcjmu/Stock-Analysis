...
class IntegrationAnalysisTool(BaseTool):
    ...
    description: str = (
        "Analyzes integration patterns and communication between applications"
    )
    ...
    def _run(self, integration_data: str) -> str:
        ...
        try:
            ...
            analysis_results = {
                ...
                "modernization_opportunities": self._identify_modernization_opportunities(
                    applications
                ),
            }
    ...
    def _map_communication_flows(
        ...
        for app in applications:
            ...
            if any(
                indicator in app_info for indicator in ["web", "frontend", "client"]
            ):
    ...
    def _assess_integration_complexity(
        ...
        databases = sum(
            1
            for app in applications
            if any(indicator in str(app).lower() for indicator in ["database", "db"])
        )
    ...
    def _identify_modernization_opportunities(
        ...
        monolith_indicators = sum(
            1 for app in applications if "monolith" in str(app).lower()
        )
...
class APIAnalysisTool(BaseTool):
    ...
    def _run(self, api_data: str) -> str:
        ...
        try:
            ...
            analysis_results = {
                ...
                "service_mesh_readiness": self._assess_service_mesh_readiness(
                    applications
                ),
            }
    ...
    def _build_api_inventory(
        ...
        for app in applications:
            ...
            if any(
                ...
                for indicator in ["api", "service", "rest", "soap"]
            ):
                api_type = (
                    "REST"
                    if "rest" in app_info
    ...
    def _map_api_dependencies(
        ...
        web_apps = [
            ...
            for app in applications
            if any(
                indicator in str(app).lower() for indicator in ["web", "frontend", "ui"]
            )
        ]
                ...
                dependencies.append(
                    {
                        ...
                        "criticality": self._assess_dependency_criticality(
                            web_app, api_service
                        ),
                        "protocol": "HTTP",
                    }
    ...
    def _analyze_api_characteristics(
        self, applications: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        ...
        if characteristics["total_apis"] > 0:
            maturity_score = (
                ...
                + (characteristics["secured_apis"] / characteristics["total_apis"])
                * 0.3
                + (characteristics["documented_apis"] / characteristics["total_apis"])
                * 0.2
                + (characteristics["versioned_apis"] / characteristics["total_apis"])
                * 0.2
            )
        else:
            maturity_score = 0.0
    ...
    def _assess_service_mesh_readiness(
        self, applications: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        ...
        readiness_factors = {
            ...
            "containerized_services": sum(
                1
                for app in api_services
                if any(
                    indicator in str(app).lower()
                    for indicator in ["docker", "container", "k8s"]
                )
            ),
            "cloud_native_services": sum(
                1
                for app in api_services
                if any(
                    indicator in str(app).lower()
        ...
        if total_services > 0:
            readiness_score = (
                ...
                + (readiness_factors["service_discovery_present"] / total_services)
                * 0.3
            )
        else:
            readiness_score = 0.0
        ...
        readiness_factors["recommendation"] = self._get_service_mesh_recommendation(
            readiness_score, total_services
        )
    ...
    def _has_versioning(self, app_info: str) -> bool:
        ...
        return any(
            indicator in app_info for indicator in ["v1", "v2", "version", "versioning"]
        )
    ...
    def _has_documentation(self, app_info: str) -> bool:
        ...
        return any(
            indicator in app_info
            for indicator in ["swagger", "openapi", "doc", "documentation"]
        )

    def _assess_dependency_criticality(
        ...
        if any(
            indicator in consumer_info
            for indicator in ["critical", "production", "core"]
        ):
            return "high"
        elif any(
            indicator in provider_info for indicator in ["core", "central", "main"]
        ):
            return "high"
        else:
            return "medium"
