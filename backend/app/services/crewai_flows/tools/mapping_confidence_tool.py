            ...
            for source_field, target_attribute in mappings.items():
                ...
                if target_attribute in standard_attributes:
                    ...
                    if source_field.lower() in [
                        attr.lower() for attr in standard_attributes[target_attribute]
                    ]:
                        confidence = 1.0
                    # Check for partial matches
                    elif any(
                        attr.lower() in source_field.lower()
                        or source_field.lower() in attr.lower()
                        for attr in standard_attributes[target_attribute]
                    ):
                        confidence = 0.85
                    # Semantic similarity (simplified)
                    elif any(
                        word in source_field.lower()
                        for word in target_attribute.split("_")
                    ):
                        confidence = 0.7
                ...
                confidence_results[source_field] = {
                    ...
                    "reasoning": self._get_mapping_reasoning(
                        source_field, target_attribute, confidence
                    ),
                }
            ...
            return json.dumps(
                {
                    ...
                    "overall_confidence": (
                        sum(r["confidence"] for r in confidence_results.values())
                        / len(confidence_results)
                        if confidence_results
                        else 0
                    ),
                    "requires_human_review": any(
                        r["needs_review"] for r in validation_results.values()
                    ),
                }
            )
...
class ValidationTool(BaseTool):
    ...
    description: str = (
        "Validates field mappings against business rules and data quality standards"
    )
    ...
    def _run(self, mapping_data: str) -> str:
        ...
        try:
            ...
            required_score = len(validation_results["required_fields_mapped"]) / len(
                required_fields
            )
            business_score = len(validation_results["business_critical_mapped"]) / len(
                business_critical_fields
            )
            validation_results["validation_score"] = (required_score * 0.7) + (
                business_score * 0.3
            )
