...
class TopologyMappingTool(BaseTool):
    ...
    def _run(self, topology_data: str) -> str:
        ...
        try:
            ...
            topology_results = {
                "hosting_relationships": self._map_hosting_relationships(
                ...
                "dependency_chains": self._build_dependency_chains(
                    applications, servers
                ),
                "topology_insights": self._generate_topology_insights(
                    applications, servers
                ),
            }
    ...
    def _map_hosting_relationships(
        self, applications: List[Dict[str, Any]], servers: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
                ...
                relationships.append(
                    {
                        ...
                        "hosting_type": self._determine_hosting_type(
                            app, hosting_server
                        ),
                        "confidence": self._calculate_hosting_confidence(
                            app, hosting_server
                        ),
                    }
                )
    ...
    def _find_hosting_server(
        self, app: Dict[str, Any], servers: List[Dict[str, Any]]
        ...
        for server in servers:
            ...
            location_match = (
                app_location and server_location and app_location == server_location
            )
    ...
    def _determine_hosting_type(
        self, app: Dict[str, Any], server: Dict[str, Any]
    ) -> str:
        ...
        if any(
            indicator in app_info or indicator in server_info
            for indicator in ["docker", "container"]
        ):
            return "containerized"
        elif any(
            indicator in app_info or indicator in server_info
            for indicator in ["vm", "virtual"]
        ):
            return "virtualized"
        elif any(
            indicator in app_info or indicator in server_info
            for indicator in ["cloud", "aws", "azure"]
        ):
            return "cloud_hosted"
        else:
            return "direct_hosted"

    def _calculate_hosting_confidence(
        self, app: Dict[str, Any], server: Dict[str, Any]
    ) -> float:
        ...
        elif app.get("environment") == server.get("environment") and app.get(
            "location"
        ) == server.get("location"):
            confidence = 0.6
    ...
    def _identify_application_tiers(
        ...
        for app in applications:
            ...
            if any(
                indicator in app_info
                for indicator in ["web", "frontend", "ui", "portal"]
            ):
                tiers["presentation_tier"].append(app_name)
            elif any(
                indicator in app_info
                for indicator in ["api", "service", "business", "logic"]
            ):
                tiers["application_tier"].append(app_name)
            elif any(
                indicator in app_info
                for indicator in ["database", "db", "data", "storage"]
            ):
                tiers["data_tier"].append(app_name)
            elif any(
                indicator in app_info
                for indicator in ["middleware", "integration", "message", "queue"]
            ):
    ...
    def _identify_network_zones(
        self, assets: List[Dict[str, Any]]
    ) -> Dict[str, List[str]]:
        """Identify network security zones"""
        zones = {"dmz": [], "internal": [], "database": [], "management": []}
        ...
        for asset in assets:
            ...
            if any(
                indicator in asset_info for indicator in ["dmz", "external", "public"]
            ):
                zones["dmz"].append(asset_name)
            elif any(
                indicator in asset_info for indicator in ["database", "db", "data"]
            ):
                zones["database"].append(asset_name)
            elif any(
                indicator in asset_info for indicator in ["mgmt", "management", "admin"]
            ):
    ...
    def _generate_topology_insights(
        self, applications: List[Dict[str, Any]], servers: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """Generate insights about the topology"""
        insights = {
            "complexity_assessment": self._assess_topology_complexity(
                applications, servers
            ),
            "migration_considerations": self._identify_migration_considerations(
                applications, servers
            ),
    ...
    def _assess_topology_complexity(
        self, applications: List[Dict[str, Any]], servers: List[Dict[str, Any]]
    ) -> str:
    ...
    def _identify_migration_considerations(
        self, applications: List[Dict[str, Any]], servers: List[Dict[str, Any]]
    ) -> List[str]:
        ...
        if len(tiers["data_tier"]) > 1:
            considerations.append(
                "Multiple databases require data migration coordination"
            )

        if len(tiers["integration_tier"]) > 0:
            considerations.append(
                "Integration middleware requires careful migration sequencing"
            )

        if len(applications) > len(servers) * 2:
            considerations.append(
                "High application density may indicate shared hosting concerns"
            )
    ...
    def _identify_optimization_opportunities(
        ...
        containerizable_apps = sum(
            1 for app in applications if "legacy" not in str(app).lower()
        )
        ...
        service_oriented_apps = sum(
            1
            for app in applications
            if any(indicator in str(app).lower() for indicator in ["api", "service"])
        )
    ...
    def _identify_north_south_traffic(
        self, applications: List[Dict[str, Any]]
    ) -> List[str]:
        ...
        return [
            ...
            for app in applications
            if any(
                indicator in str(app).lower()
                for indicator in ["web", "api", "portal", "frontend"]
            )
        ]

    def _identify_east_west_traffic(
        self, applications: List[Dict[str, Any]]
    ) -> List[str]:
...
class HostingAnalysisTool(BaseTool):
    ...
    description: str = (
        "Analyzes hosting patterns and resource utilization relationships"
    )
    ...
    def _analyze_hosting_patterns(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for rel in relationships:
            ...
            patterns["hosting_types"][hosting_type] = (
                patterns["hosting_types"].get(hosting_type, 0) + 1
            )
    ...
    def _assess_hosting_risks(self, data: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        unknown_hosting = [
            rel for rel in relationships if rel.get("hosted_on") == "unknown"
        ]
        if unknown_hosting:
            risks.append(
                {
                    ...
                    "affected_applications": [
                        rel.get("application") for rel in unknown_hosting
                    ],
                }
            )
        ...
        low_confidence = [
            rel for rel in relationships if rel.get("confidence", 0) < 0.5
        ]
        if low_confidence:
            risks.append(
                {
