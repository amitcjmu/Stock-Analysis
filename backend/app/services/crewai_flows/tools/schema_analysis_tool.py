...
class SchemaAnalysisInput(BaseModel):
    ...
    data_sample: List[Dict[str, Any]] = Field(description="Sample data records for analysis")
    metadata: Dict[str, Any] = Field(default={}, description="Additional metadata about the data source")
...
class SchemaAnalysisTool(BaseTool):
    ...
    def _run(self, data_sample: List[Dict[str, Any]], metadata: Dict[str, Any] = None) -> str:
        """
        ...
        try:
            ...
            # Generate analysis report
            {
                ...
                "recommendations": self._generate_recommendations(fields_analysis, patterns)
            }
    ...
    def _analyze_fields(self, data_sample: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for field in all_fields:
            values = [record.get(field) for record in data_sample[:10] if field in record]
    ...
    def _looks_like_ip(self, value: str) -> bool:
        ...
        parts = value.split('.')
        return len(parts) == 4 and all(part.isdigit() and 0 <= int(part) <= 255 for part in parts if part.isdigit())
    ...
    def _looks_like_id(self, value: str) -> bool:
        ...
        return len(value) > 8 and any(char.isdigit() for char in value) and any(char.isalpha() for char in value)
    ...
    def _find_consistent_fields(self, data_sample: List[Dict[str, Any]]) -> List[str]:
        ...
        consistent_fields = [field for field, count in field_counts.items() 
    ...
    def _analyze_semantics(self, fields_analysis: Dict[str, Any], patterns: Dict[str, Any], metadata: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _identify_asset_fields(self, fields_analysis: Dict[str, Any]) -> List[str]:
        ...
        asset_keywords = ["name", "hostname", "server", "asset", "device", "computer", "id", "type"]
        asset_fields = []
    ...
    def _identify_business_fields(self, fields_analysis: Dict[str, Any]) -> List[str]:
        ...
        business_keywords = ["owner", "department", "cost", "business", "criticality", "environment", "tier"]
        business_fields = []
    ...
    def _identify_technical_fields(self, fields_analysis: Dict[str, Any]) -> List[str]:
        ...
        technical_keywords = ["ip", "os", "cpu", "memory", "disk", "version", "platform"]
        technical_fields = []
    ...
    def _identify_relationships(self, fields_analysis: Dict[str, Any]) -> Dict[str, Any]:
        """Identify potential relationships between fields"""
        relationships = {}
        
        # Look for foreign key patterns
        id_fields = [field for field, analysis in fields_analysis.items() 
                    if "id" in field.lower() or "identifier" in analysis.get("pattern_hints", [])]
    ...
    def _generate_recommendations(self, fields_analysis: Dict[str, Any], patterns: Dict[str, Any]) -> List[str]:
        """Generate recommendations for field mapping"""
        recommendations = []
        
        if patterns.get("potential_keys"):
            recommendations.append(f"Consider using {patterns['potential_keys'][0]} as primary identifier")
        
        if len(fields_analysis) > 20:
            recommendations.append("Large number of fields detected - consider grouping by domain")
        
        if patterns.get("record_count", 0) < 5:
            recommendations.append("Limited sample size - consider analyzing more records for better accuracy")
