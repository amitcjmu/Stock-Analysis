...
class DiscoveryFlowEventListener(BaseEventListener):
    ...
    def setup_listeners(self, crewai_event_bus):
        ...
        def on_flow_started(source, event: FlowStartedEvent):
            ...
            if not flow_id:
                logger.warning(
                    "⚠️ FlowStartedEvent received but could not extract flow_id, skipping tracking"
                )
                return
            ...
            logger.info(
                f"🔄 Event-based database updates DISABLED for state consolidation - flow: {flow_id}"
            )
            logger.info(
                "📋 State updates now handled exclusively by Master Flow Orchestrator"
            )
        ...
        def on_flow_finished(source, event: FlowFinishedEvent):
            ...
            if not flow_id:
                logger.warning(
                    "⚠️ FlowFinishedEvent received but could not extract flow_id, skipping tracking"
                )
                return
        ...
        def on_method_started(source, event: MethodExecutionStartedEvent):
            ...
            if not flow_id:
                logger.warning(
                    "⚠️ MethodExecutionStartedEvent received but could not extract flow_id, skipping tracking"
                )
                return
            ...
            if crew_name:
                self._add_flow_event(
                    ...
                    status="running",
                    progress=self._calculate_crew_progress(
                        flow_id, crew_name, "started"
                    ),
                )
        ...
        def on_method_finished(source, event: MethodExecutionFinishedEvent):
            ...
            if not flow_id:
                logger.warning(
                    "⚠️ MethodExecutionFinishedEvent received but could not extract flow_id, skipping tracking"
                )
                return
            ...
            if crew_name:
                ...
                if flow_id in self.active_flows:
                    ...
                    completed_count = len(
                        self.active_flows[flow_id]["completed_phases"]
                    )
                    total_count = self.active_flows[flow_id]["total_crews"]
                    self.active_flows[flow_id]["progress"] = (
        ...
        def on_method_failed(source, event: MethodExecutionFailedEvent):
            ...
            if not flow_id:
                logger.warning(
                    "⚠️ MethodExecutionFailedEvent received but could not extract flow_id, skipping tracking"
                )
                return
        ...
        def on_agent_started(source, event: AgentExecutionStartedEvent):
            ...
            if not flow_id:
                logger.warning(
                    "⚠️ AgentExecutionStartedEvent received but could not extract flow_id, skipping tracking"
                )
                return
            ...
            self._add_flow_event(
                ...
                data={"agent_role": agent_name, "task_description": task_description},
                status="running",
            )
        ...
        def on_agent_completed(source, event: AgentExecutionCompletedEvent):
            ...
            if not flow_id:
                logger.warning(
                    "⚠️ AgentExecutionCompletedEvent received but could not extract flow_id, skipping tracking"
                )
                return
        ...
        def on_task_completed(source, event: TaskCompletedEvent):
            ...
            try:
                ...
                if not flow_id:
                    logger.warning(
                        "⚠️ TaskCompletedEvent received but could not extract flow_id, skipping tracking"
                    )
                    return
    ...
    def _extract_flow_id(self, source, event) -> str:
        ...
        try:
            ...
            if hasattr(source, "state") and hasattr(source.state, "flow_id"):
                ...
                if flow_id:
                    logger.info(
                        f"🎯 Flow ID extracted from source.state.flow_id: {flow_id}"
                    )
                    return str(flow_id)
            ...
            if hasattr(source, "state") and hasattr(source.state, "session_id"):
                ...
                if session_id:
                    logger.warning(
                        f"⚠️ Using DEPRECATED session_id as flow_id: {session_id}"
                    )
            ...
            if hasattr(source, "session_id"):
                ...
                if session_id:
                    logger.warning(
                        f"⚠️ Using DEPRECATED source.session_id as flow_id: {session_id}"
                    )
                    return str(session_id)
            ...
            try:
                ...
                if active_flows:
                    ...
                    flow_ids = (
                        list(active_flows.keys())
                        if isinstance(active_flows, dict)
                        else active_flows
                    )
                    if flow_ids:
                        most_recent_flow = flow_ids[-1]  # Get the last/most recent flow
                        logger.info(
                            f"🔍 Using most recent active flow from service: {most_recent_flow}"
                        )
            ...
            logger.error(
                f"   Source attributes: {[attr for attr in dir(source) if not attr.startswith('_')]}"
            )
    ...
    def _calculate_crew_progress(
        self, flow_id: str, crew_name: str, status: str
    ) -> float:
    ...
    def _add_flow_event(self, flow_id: str, event_type: str, source: str, **kwargs):
        ...
        if not flow_id:
            logger.warning(
                f"⚠️ Attempted to add event {event_type} with None flow_id, skipping"
            )
            return
    ...
    def get_flow_status(self, flow_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "total_phases": len(self.discovery_phases),
            "start_time": (
                flow_data.get("start_time").isoformat()
                if flow_data.get("start_time")
                else None
            ),
