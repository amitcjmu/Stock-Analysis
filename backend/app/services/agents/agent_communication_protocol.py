...
class AgentCommunicationProtocol:
        ...
        try:
            if agent_id in self.registered_agents:
                ...
                if agent_id in self.agent_statuses:
                    ...
                    self.agent_statuses[agent_id]['unregistered_at'] = datetime.now().isoformat()
    ...
    async def send_agent_message(self, agent_id: str, message: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if agent_id not in self.registered_agents:
                logger.warning(f"Unregistered agent attempted to send message: {agent_id}")
                return False
            ...
            if agent_id in self.agent_statuses:
                self.agent_statuses[agent_id]['last_activity'] = datetime.now().isoformat()
    ...
    async def send_ui_interaction(self, interaction: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            logger.debug(f"📥 UI interaction received: {interaction.get('type', 'unknown')}")
            return True
    ...
    async def get_ui_messages(self, since: Optional[datetime] = None, limit: int = 50) -> List[Dict[str, Any]]:
    ...
    async def get_agent_messages(self, agent_id: str, since: Optional[datetime] = None, limit: int = 50) -> List[Dict[str, Any]]:
    ...
    async def test_communication(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if registration_success:
                ...
                message_success = await self.send_agent_message(test_agent_id, test_message)
                test_results['tests']['message_sending'] = message_success
                
                # Test message retrieval
                messages = await self.get_ui_messages(limit=1)
                retrieval_success = len(messages) > 0 and messages[-1]['agent_id'] == test_agent_id
            ...
            test_results['message'] = 'All tests passed' if all_tests_passed else 'Some tests failed'
            
            logger.info(f"🧪 Communication test completed: {'✅ PASSED' if all_tests_passed else '❌ FAILED'}")
    ...
    def cleanup_old_messages(self, older_than_hours: int = 24) -> int:
        ...
        try:
            ...
            self.ui_messages = deque([
                msg for msg in self.ui_messages 
            ...
            for agent_id in list(self.agent_messages.keys()):
                ...
                self.agent_messages[agent_id] = deque([
                    msg for msg in self.agent_messages[agent_id]
                    if datetime.fromisoformat(msg['timestamp']) > cutoff_time
                ], maxlen=100)
            ...
            self.ui_interactions = [
                interaction for interaction in self.ui_interactions
