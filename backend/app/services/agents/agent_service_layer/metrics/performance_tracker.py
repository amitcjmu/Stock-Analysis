...
class PerformanceTracker:
    ...
    def __init__(self, max_history: int = 1000):
        ...
        self._method_stats = defaultdict(
            lambda: {
                "calls": 0,
                "errors": 0,
                "total_time": 0.0,
    ...
    def get_health_status(self) -> Dict[str, Any]:
        ...
        recent_avg_time = sum(call["duration"] for call in recent_calls) / max(
            len(recent_calls), 1
        )
        ...
        health_indicators = {
            "error_rate": {
                ...
                "status": (
                    "healthy"
                    if error_rate < 0.05
                    else ("warning" if error_rate < 0.15 else "critical")
                ),
                "threshold": 0.05,
            },
            "response_time": {
                ...
                "status": (
                    "healthy"
                    if avg_time < 1.0
                    else ("warning" if avg_time < 3.0 else "critical")
                ),
                "threshold": 1.0,
            },
            "call_volume": {
    ...
    def get_method_performance(self, method_name: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "min_time": (
                round(stats["min_time"], 4) if stats["min_time"] != float("inf") else 0
            ),
    ...
    def _analyze_errors(self) -> Dict[str, Any]:
        ...
        if total_errors == 0:
            return {"total_errors": 0, "error_patterns": {}, "most_common_error": None}
    ...
    def _determine_performance_trend(self, recent_calls: List[Dict[str, Any]]) -> str:
        ...
        first_success = sum(1 for call in first_half if call["success"]) / len(
            first_half
        )
        second_success = sum(1 for call in second_half if call["success"]) / len(
            second_half
        )
    ...
    def _get_health_recommendations(self, indicators: Dict[str, Any]) -> List[str]:
        ...
        if (
            indicators["response_time"]["recent_value"]
            > indicators["response_time"]["value"]
        ):
            recommendations.append(
                "Recent performance degradation - investigate system load"
            )
