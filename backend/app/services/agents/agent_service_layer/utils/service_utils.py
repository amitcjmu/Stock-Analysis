...
def calculate_confidence_score(
    factors: Dict[str, float], weights: Optional[Dict[str, float]] = None
) -> float:
...
def calculate_completion_percentage(
    completed_phases: List[str], total_phases: int = 6
) -> float:
...
def extract_error_category(error_message: str) -> str:
    ...
    if "timeout" in error_lower:
        ...
        "validation" in error_lower or "invalid" in error_lower or "400" in error_lower
    ):
        return "validation"
    elif (
        "database" in error_lower or "connection" in error_lower or "sql" in error_lower
    ):
        return "database"
...
def build_error_response(
    method_name: str, error: Exception, error_type: Optional[str] = None
) -> Dict[str, Any]:
...
def validate_phase_order(from_phase: str, to_phase: str) -> Dict[str, Any]:
    ...
    return {
        ...
        "message": (
            "Valid transition" if is_valid else "Backward transitions not allowed"
        ),
    }
...
def calculate_data_quality_score(data_sample: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    for record in data_sample:
        if isinstance(record, dict):
            for field, value in record.items():
                ...
                type_consistency[field][value_type] = (
                    type_consistency[field].get(value_type, 0) + 1
                )
    ...
    avg_validity = (
        sum(validity_scores.values()) / len(validity_scores) if validity_scores else 0
    )
...
def generate_asset_summary(assets: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    return {
        ...
        "quality_metrics": {
            ...
            "quality_rate": (
                round(high_quality_count / len(assets) * 100, 1) if assets else 0
            ),
        },
        "average_scores": {
