    ...
    async def get_import_data(
        ...
        async with AsyncSessionLocal() as db:
            try:
                ...
                if not flow:
                    return {"status": "error", "error": "Flow not found", "data": []}
        ...
        async with AsyncSessionLocal() as db:
            try:
                ...
                attribute_mapping_data = (
                    getattr(flow, "attribute_mapping_data", {}) or {}
                )
                ...
                unmapped_fields = [
                    field for field in source_fields if field not in mapped_fields
                ]
                ...
                mapping_stats = {
                    ...
                    "mapping_completeness": (
                        (len(mapped_fields) / len(source_fields) * 100)
                        if source_fields
                        else 0
                    ),
                }
                ...
                return {"status": "error", "error": str(e), "mappings": {}}
            ...
            try:
                ...
                for source_field, target_mapping in mappings.items():
                    ...
                    if source_field not in source_fields:
                        field_validation["issues"].append(
                            "Source field not found in data"
                        )
                        is_valid = False
                    ...
                    if not isinstance(target_mapping, dict):
                        field_validation["issues"].append(
                            "Target mapping must be a dictionary"
                        )
                        is_valid = False
                    else:
                ...
                return {
                    ...
                    "summary": {
                        ...
                        "valid_mappings": len(
                            [r for r in validation_results if not r["issues"]]
                        ),
                        "invalid_mappings": len(
                            [r for r in validation_results if r["issues"]]
                        ),
                    },
                }
    ...
    async def get_cleansing_results(self, flow_id: str) -> Dict[str, Any]:
        ...
        async with AsyncSessionLocal() as db:
            try:
                ...
                flow = await flow_repo.get_by_flow_id(flow_id)
                if not flow:
                    return {"status": "error", "error": "Flow not found", "results": {}}
                        ...
                        validation_issues.append(
                            {
                                ...
                                "field": "raw_data",
                                "suggestions": [
                                    "Import data through the data import interface"
                                ],
                            }
                        )
                    else:
                    ...
                    validation_issues.append(
                        {
                            ...
                            "suggestions": [
                                "Complete field mapping in the attribute mapping phase"
                            ],
                        }
                    )
                    ...
                    validation_issues.append(
                        {
                            ...
                            "suggestions": [
                                "Run data cleansing process to identify and fix data quality issues"
                            ],
                        }
                    )
                    ...
                    validation_issues.append(
                        {
                            ...
                            "field": "progress",
                            "suggestions": [
                                "Continue with the discovery phases to increase progress"
                            ],
                        }
                    )
