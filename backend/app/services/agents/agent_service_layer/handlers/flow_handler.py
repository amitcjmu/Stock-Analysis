            ...
            try:
                ...
                if master_flow.flow_type == "discovery":
                    ...
                    if flow:
                        ...
                        actual_data_status = (
                            await self._check_actual_data_via_import_id(flow)
                        )
                        ...
                        current_phase = self._determine_actual_current_phase(
                            actual_phases, flow
                        )
                        ...
                        return {
                            ...
                            "flow": {
                                ...
                                "raw_data_count": actual_data_status.get(
                        ...
                        logger.info(
                            f"Discovery flow not found in child repository: {flow_id}"
                        )
                        return {
                            "status": "not_found",
                            "flow_exists": False,
                    ...
                    logger.warning(
                        f"[ADR-012] Unsupported flow type, using master flow: {master_flow.flow_type}"
                    )
                    return {
                        "status": "success",
                        "flow_exists": True,
