...
class ServiceValidator:
    ...
    def validate_flow_id(self, flow_id: str) -> Dict[str, Any]:
        ...
        if not isinstance(flow_id, str):
            ...
            validation_result["recommendations"].append("Convert flow ID to string format")
            return validation_result
        ...
        if len(flow_id) > 255:
            validation_result["is_valid"] = False
            validation_result["issues"].append("Flow ID exceeds maximum length of 255 characters")
    ...
    def validate_asset_id(self, asset_id: str) -> Dict[str, Any]:
        ...
        if not isinstance(asset_id, str):
            ...
            validation_result["recommendations"].append("Convert asset ID to string format")
            return validation_result
        ...
        try:
            ...
            validation_result["recommendations"].append("Ensure asset ID is in UUID format")
    ...
    def validate_phase_name(self, phase: str) -> Dict[str, Any]:
        ...
        if phase not in valid_phases:
            ...
            validation_result["recommendations"].append(f"Use one of: {', '.join(valid_phases)}")
    ...
    def validate_asset_type(self, asset_type: str) -> Dict[str, Any]:
        ...
        if asset_type.lower() not in [t.lower() for t in valid_asset_types]:
            ...
            validation_result["recommendations"].append(f"Consider using standard types: {', '.join(valid_asset_types)}")
    ...
    def validate_mappings_structure(self, mappings: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not isinstance(mappings, dict):
            ...
            validation_result["recommendations"].append("Provide mappings as a dictionary")
            return validation_result
        
        if len(mappings) == 0:
            validation_result["is_valid"] = False
            validation_result["issues"].append("Mappings dictionary is empty")
            validation_result["recommendations"].append("Provide at least one field mapping")
            return validation_result
        
        # Validate each mapping
        for source_field, mapping_config in mappings.items():
            field_validation = self._validate_single_mapping(source_field, mapping_config)
            ...
            if not field_validation["is_valid"]:
                ...
                validation_result["recommendations"].extend([
                    f"{source_field}: {rec}" for rec in field_validation["recommendations"]
                ])
    ...
    def validate_context(self) -> Dict[str, Any]:
        ...
        if not self.context.client_account_id:
            ...
            validation_result["recommendations"].append("Provide a valid client account ID")
        ...
        if not self.context.user_id:
            ...
            validation_result["recommendations"].append("Consider providing user ID for audit trail")
        
        return validation_result
    
    def validate_service_call_params(self, method_name: str, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if method_name == "get_flow_status":
            if "flow_id" not in params:
                validation_result["is_valid"] = False
                validation_result["issues"].append("Missing required parameter: flow_id")
            else:
            ...
            if "asset_id" not in params:
                validation_result["is_valid"] = False
                validation_result["issues"].append("Missing required parameter: asset_id")
            else:
            ...
            for param in required_params:
                if param not in params:
                    validation_result["is_valid"] = False
                    validation_result["issues"].append(f"Missing required parameter: {param}")
    ...
    def validate_response_data(self, response: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "status" not in response:
            ...
            validation_result["recommendations"].append("Include status field in response")
        
        # Validate status values
        valid_statuses = ["success", "error", "not_found", "invalid"]
        if "status" in response and response["status"] not in valid_statuses:
            validation_result["issues"].append(f"Invalid status value: {response['status']}")
            validation_result["recommendations"].append(f"Use one of: {', '.join(valid_statuses)}")
        
        # Check error responses
        if response.get("status") == "error":
            if "error" not in response:
                validation_result["issues"].append("Error response missing error field")
                validation_result["recommendations"].append("Include error field in error responses")
        
        return validation_result
    
    def _validate_single_mapping(self, source_field: str, mapping_config: Any) -> Dict[str, Any]:
        ...
        if not isinstance(mapping_config, dict):
            validation_result["is_valid"] = False
            validation_result["issues"].append("Mapping configuration must be a dictionary")
            validation_result["recommendations"].append("Provide mapping as dictionary with target_field and data_type")
            return validation_result
        ...
        for field in required_fields:
            if field not in mapping_config:
                ...
                validation_result["recommendations"].append(f"Add {field} to mapping configuration")
        
        # Validate data types
        if "data_type" in mapping_config:
            valid_data_types = ["string", "integer", "float", "boolean", "date", "datetime", "json"]
            data_type = mapping_config["data_type"]
            if data_type not in valid_data_types:
                validation_result["issues"].append(f"Unknown data type: {data_type}")
                validation_result["recommendations"].append(f"Use one of: {', '.join(valid_data_types)}")
        ...
        if "target_field" in mapping_config:
            ...
            if not target_field or not isinstance(target_field, str):
                validation_result["is_valid"] = False
                validation_result["issues"].append("Target field must be a non-empty string")
                validation_result["recommendations"].append("Provide a valid target field name")
