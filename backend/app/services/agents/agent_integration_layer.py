...
class AgentIntegrationLayer:
    ...
    def __init__(self, db: Optional[Any] = None, context: Optional[Any] = None):
        ...
        self.logger.info(f"🔗 Agent Integration Layer initialized (ID: {self.integration_id})")
    
    async def integrate_with_discovery_flow(self, 
        ...
        self.logger.info(f"🔗 Starting Discovery Flow integration (ID: {integration_id})")
        ...
        try:
            ...
            self.logger.info(f"✅ Discovery Flow integration completed in {execution_time:.2f}s")
            ...
            if self.legacy_fallback_enabled:
                ...
                fallback_result = await self._execute_legacy_fallback(flow_data, flow_context)
    ...
    async def _transform_flow_data_for_agents(self, 
                                            flow_data: Dict[str, Any], 
        ...
        if agent_data['raw_data'] and isinstance(agent_data['raw_data'], list) and len(agent_data['raw_data']) > 0:
        ...
        self.logger.info(f"📊 Transformed flow data: {len(agent_data['raw_data'])} records, "
                        f"{len(agent_data['source_columns'])} columns")
        
        return agent_data
    
    async def _transform_agent_results_for_flow(self, 
        ...
        flow_results['user_clarifications'] = agent_results.get('all_clarifications', [])
        
        # Generate flow-level recommendations
        await self._generate_flow_recommendations(agent_results, flow_results)
        
        self.logger.info(f"🔄 Transformed agent results to flow format: {flow_results['status']} "
                        f"({flow_results['confidence']:.1f}% confidence)")
        
        return flow_results
    
    async def _transform_validation_results(self, agent_results: Dict[str, Any], flow_results: Dict[str, Any]):
        """Transform data validation agent results"""
        validation_result = agent_results.get('agent_results', {}).get('data_validation', {})
    ...
    async def _transform_mapping_results(self, agent_results: Dict[str, Any], flow_results: Dict[str, Any]):
        """Transform attribute mapping agent results"""
        mapping_result = agent_results.get('agent_results', {}).get('attribute_mapping', {})
    ...
    async def _transform_cleansing_results(self, agent_results: Dict[str, Any], flow_results: Dict[str, Any]):
        """Transform data cleansing agent results"""
        cleansing_result = agent_results.get('agent_results', {}).get('data_cleansing', {})
        ...
        if cleansing_result and cleansing_result.get('data'):
            ...
            flow_results['results']['data_cleansing'] = {
                ...
                'standardization_summary': cleansing_data.get('standardization_summary', {}),
                'bulk_operations_summary': cleansing_data.get('bulk_operations_summary', {}),
    ...
    async def _generate_flow_recommendations(self, agent_results: Dict[str, Any], flow_results: Dict[str, Any]):
        ...
        if execution_summary.get('failed_agents', 0) > 0:
            recommendations.append({
                ...
                'suggested_actions': ['Review agent logs', 'Check data quality', 'Retry with corrected data']
            })
        ...
        if execution_summary.get('partial_agents', 0) > 0:
            recommendations.append({
                ...
                'action_required': True,
                'suggested_actions': ['Review partial results', 'Provide clarifications', 'Consider manual review']
            })
        ...
        if total_clarifications > 5:
            recommendations.append({
                ...
                'action_required': True,
                'suggested_actions': ['Review clarification questions', 'Provide responses', 'Consider data improvements']
            })
        ...
        if overall_confidence < 70:
            recommendations.append({
                ...
                'suggested_actions': ['Review data quality', 'Provide additional context', 'Consider manual validation']
            })
        
        flow_results['recommendations'] = recommendations
    
    async def _execute_legacy_fallback(self, flow_data: Dict[str, Any], flow_context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        fallback_result = {
            ...
            'results': {
                ...
                'data_cleansing': {'cleaned_data': flow_data.get('data', []), 'confidence': 50.0}
            },
            'agent_insights': [{
                ...
                'category': 'system',
                'actionable': True,
                'action_items': ['Review agent integration logs', 'Check system compatibility']
            }],
            'user_clarifications': [],
            'recommendations': [{
                'type': 'critical',
                ...
                'action_required': True,
                'suggested_actions': ['Check agent system status', 'Review error logs', 'Contact system administrator']
            }],
            'fallback_mode': True
        }
    ...
    async def process_clarification_response(self, question_id: str, response: Dict[str, Any]) -> bool:
        ...
        return await self.orchestrator.process_clarification_response(question_id, response)
    ...
    async def test_integration(self, test_data: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(f"✅ Integration test completed: {test_result['test_status']}")
            return test_result
