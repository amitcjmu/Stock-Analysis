...
ValidatorFunc = Callable[[Dict[str, Any], Dict[str, Any], Optional[Dict[str, Any]]], Awaitable[ValidationResult]]
...
class ValidatorRegistry:
    ...
    def register_validator(
        ...
        if flow_type and phase:
            ...
            if name in self._phase_validators[flow_type][phase]:
                raise ValueError(f"Phase validator '{name}' already exists for {flow_type}.{phase}")
            ...
            if name in self._flow_validators[flow_type]:
                raise ValueError(f"Flow validator '{name}' already exists for {flow_type}")
    ...
    def _register_builtin_validators(self):
        ...
        async def size_limit_validator(
            ...
            max_records = overrides.get("max_records", 1000000) if overrides else 1000000
        ...
        self.register_validator(
            "size_limit",
            size_limit_validator,
            description="Validates data size limits"
        )
