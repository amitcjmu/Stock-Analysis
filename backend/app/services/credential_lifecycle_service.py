...
class CredentialLifecycleService:
    ...
    async def check_credential_health(
        ...
        if credential.expires_at:
            ...
            if days_until_expiry < 0:
                ...
                health_report["issues"].append(f"Credential expires in {days_until_expiry} days")
        ...
        if credential.last_rotated_at:
            ...
            if days_since_rotation > policy.rotation_interval_days:
                health_report["issues"].append(f"Credential not rotated for {days_since_rotation} days")
            ...
            if days_since_creation > policy.rotation_interval_days:
                ...
                health_report["health_score"] -= 30
                health_report["recommendations"].append("Perform initial credential rotation")
        
        # Check validation status
        if credential.last_validated_at:
            hours_since_validation = (datetime.utcnow() - credential.last_validated_at).total_seconds() / 3600
        ...
        if credential.validation_errors:
            ...
            health_report["recommendations"].append("Fix validation errors or rotate credential")
        ...
        for credential in credentials:
            ...
            try:
                ...
                if await self._needs_rotation(credential):
                    ...
                    if not dry_run and self.get_policy(credential.credential_type).auto_rotate:
                        success = await self._auto_rotate_credential(credential, system_user_id)
                        if success:
                ...
                if await self._is_expiring_soon(credential):
                    if not dry_run:
                        await self._handle_expiring_credential(credential, system_user_id)
    ...
    async def get_rotation_recommendations(
        ...
        for credential in credentials:
            ...
            if await self._needs_rotation(credential):
                if credential.expires_at and credential.expires_at < datetime.utcnow() + timedelta(days=7):
            ...
            if recommendation:
                recommendations.append({
                    ...
                    "last_rotated": credential.last_rotated_at.isoformat() if credential.last_rotated_at else None
                })
    ...
    async def generate_lifecycle_report(
        ...
        recent_rotations = await self.session.execute(
            ...
            .where(
                CredentialRotationHistory.rotated_at >= datetime.utcnow() - timedelta(days=30)
            )
            .limit(10)
        )
        ...
        return {
            ...
            "recommendations": await self.get_rotation_recommendations(client_account_id)
        }
    
    def get_policy(self, credential_type: CredentialType) -> LifecyclePolicy:
        """Get lifecycle policy for credential type"""
        return self.policies.get(credential_type, DEFAULT_POLICIES[CredentialType.CUSTOM])
    ...
    async def _needs_rotation(self, credential: PlatformCredential) -> bool:
        ...
        if policy.rotation_policy == RotationPolicy.ON_EXPIRY:
            if credential.expires_at:
                return credential.expires_at <= datetime.utcnow() + timedelta(days=policy.expiry_warning_days)
            return False
