...
class RBACService:
    ...
    async def admin_create_user(self, user_data: Dict[str, Any], created_by: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if existing_user:
                return {"status": "error", "message": f"User with email {user_data['email']} already exists"}
            
            # Create base User record
            new_user = User(
                email=user_data["email"],
                first_name=user_data["full_name"].split(" ")[0],
                last_name=" ".join(user_data["full_name"].split(" ")[1:]) if " " in user_data["full_name"] else "",
                is_active=user_data.get("is_active", True),
                password_hash=f"$2b$12${user_data['password']}"  # This should be properly hashed in production
            )
            ...
            user_profile = UserProfile(
                ...
                phone_number=user_data.get("phone_number"),
                manager_email=user_data.get("manager_email"),
                status="active" if user_data.get("is_active") else "pending_approval",
                approved_at=datetime.utcnow() if user_data.get("is_active") else None,
                approved_by=None,  # Will implement proper admin user lookup later
                requested_access_level=user_data.get("access_level", "read_only"),
                registration_reason=f"Admin created user with role: {user_data.get('role_name', 'user')}"
            )
            
            self.db.add(user_profile)
            
            # Create user role if active
            if user_data.get("is_active"):
                user_role = UserRole(
                    user_id=str(new_user.id),
                    role_type=user_data.get("role_name", "user"),
                    role_name=user_data.get("role_name", "user"),
                    permissions=self._get_default_role_permissions(user_data.get("access_level", "read")),
            ...
            await self._log_access(
                user_id=created_by,
                action_type="admin_user_creation",
                result="success",
                reason=f"User {user_data['email']} created by admin",
                details={
            ...
            return {
                ...
                "user_profile_id": str(new_user.id),
                "email": user_data["email"],
                "approval_status": "approved" if user_data.get("is_active") else "pending_approval"
            }
