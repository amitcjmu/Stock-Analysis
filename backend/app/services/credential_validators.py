    ...
    async def validate(
        ...
        if self.test_endpoint:
            try:
                ...
                response = requests.get(self.test_endpoint, headers=headers, timeout=10)
        ...
        if self.token_endpoint:
            try:
                ...
                if response.status_code == 401:
                    ...
                    return (
                        False,
                        f"Token endpoint returned error: {response.status_code}",
                    )
    ...
    async def validate(
        ...
        # Azure service principal
            ...
            required_fields = (
                ...
                if "appId" in account_data
                else ["clientId", "clientSecret", "tenantId"]
            )
    ...
    async def validate(
        ...
        try:
            ...
            serialization.load_pem_private_key(
                key_data,
                password=(
                    credential_data.get("password", b"").encode()
...
def get_credential_validator(
    credential_type: str, test_endpoint: Optional[str] = None
) -> CredentialValidator:
    """
