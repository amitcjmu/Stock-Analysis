...
class SecurityHeadersMiddleware(BaseHTTPMiddleware):
    ...
    def _build_csp_policy(self, request: Request) -> str:
        ...
        is_dev = env in ['development', 'dev', 'local', 'localhost'] or env not in ['production', 'prod']
        
        if is_dev:
            # Allow localhost connections for development
            csp_directives = [
                directive.replace("'self'", "'self' localhost:* 127.0.0.1:* http://localhost:* http://127.0.0.1:*")
            ...
            csp_directives = [
                directive.replace("script-src 'self'", "script-src 'self' 'unsafe-eval' 'unsafe-inline'")
