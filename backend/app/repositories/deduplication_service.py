...
class DeduplicationService:
    ...
    async def _hostname_priority_strategy(
        ...
        if not hostname_field:
            logger.warning(
                f"No hostname field found for {model_class.__name__}, falling back to latest_session"
            )
            return await self._latest_session_strategy(
                model_class, engagement_id, filters
            )
        ...
        logger.info(
            f"Hostname priority deduplication returned {len(records)} records for {model_class.__name__}"
        )
        return records
    ...
    async def _latest_session_strategy(
        ...
        latest_records_subquery = select(
            ...
            model_class.flow_id,
            func.row_number()
            .over(partition_by=dedup_field, order_by=desc(model_class.created_at))
            .label("row_num"),
        )

        # Apply engagement filter to latest records subquery
        if hasattr(model_class, "engagement_id"):
            latest_records_subquery = latest_records_subquery.where(
                model_class.engagement_id == engagement_id
            )
        ...
        logger.info(
            f"Latest session deduplication returned {len(records)} records for {model_class.__name__}"
        )
        return records
    ...
    async def _data_quality_strategy(
        ...
        for column in model_class.__table__.columns:
            if column.name not in ["id", "created_at", "updated_at", "flow_id"]:
                quality_fields.append(
                    func.case(
                        (getattr(model_class, column.name).is_not(None), 1), else_=0
                    )
                )
        ...
        logger.info(
            f"Data quality deduplication returned {len(records)} records for {model_class.__name__}"
        )
        return records
    ...
    async def _agent_assisted_strategy(
        ...
        logger.info(
            "Agent-assisted deduplication not yet implemented, using data quality strategy"
        )
    ...
    async def get_duplicate_groups(
        ...
        logger.info(
            f"Found {len(duplicate_groups)} duplicate groups for {model_class.__name__}"
        )
        return duplicate_groups

    async def get_deduplication_stats(
        self, model_class: Type[ModelType], engagement_id: str
    ) -> Dict[str, Any]:
        """
        ...
        return {
            ...
            "deduplication_ratio": (
                deduplicated_count / total_records if total_records > 0 else 0
            ),
