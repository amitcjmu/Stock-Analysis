...
class DeduplicatingRepository(ContextAwareRepository[ModelType]):
    ...
    def __init__(
        ...
        logger.debug(
            f"Initialized {model_class.__name__} deduplicating repository. Flow-scoped: {self.flow_id is not None}"
        )
    ...
    async def _get_deduplicated_records(
        ...
        if not dedup_field or not self.has_flow:
            logger.warning(
                f"No deduplication field or flow_id found for {self.model_class.__name__}, returning regular query."
            )
        ...
        latest_records_subquery = select(
            ...
            func.row_number()
            .over(partition_by=dedup_field, order_by=desc(self.model_class.created_at))
            .label("row_num"),
        )
        ...
        logger.debug(
            f"Deduplicated query returned {len(records)} records for {self.model_class.__name__}"
        )
        return records
    ...
    async def count(self, **filters) -> int:
        ...
        if not dedup_field or not self.has_flow:
            logger.warning(
                f"Cannot perform deduplicated count for {self.model_class.__name__}"
            )
...
def create_deduplicating_repository(
    db: AsyncSession, model_class: Type[ModelType]
