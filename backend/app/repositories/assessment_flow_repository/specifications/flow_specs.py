        ...
        try:
            ...
            if engagement_id:
                extensions_repo = CrewAIFlowStateExtensionsRepository(
                    self.db, str(self.client_account_id), str(engagement_id)
                )
                ...
                await extensions_repo.update_flow_status(
                    flow_id=str(flow_id), status=status, phase_data=update_phase_data
                )
        ...
        try:
            ...
            if engagement_id:
                ...
                if master_flow:
                    ...
                    logger.info(
                        f"Logged {len(insights)} agent collaborations to master flow"
                    )
