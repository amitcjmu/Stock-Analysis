...
class AnalyticsQueries:
        ...
        try:
            ...
            return {
                ...
                "master_flow_references": {
                    ...
                    "flows_without_master_reference": primary_count + supplemental_count - flows_with_master_ref
                },
                "assessment_readiness": {
                    "ready_for_assessment": assessment_ready_count,
                    "not_ready": primary_count + supplemental_count - assessment_ready_count
                },
                "coordination_metrics": {
                    "avg_supplemental_per_primary": supplemental_count / primary_count if primary_count > 0 else 0,
                    "assessment_conversion_rate": assessment_count / primary_count if primary_count > 0 else 0
                }
            }
    ...
    async def get_flow_analytics(self, days: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            phase_completion = {
                "data_import": sum(1 for f in flows if f.data_import_completed) / total_flows if total_flows > 0 else 0,
                "attribute_mapping": sum(1 for f in flows if f.attribute_mapping_completed) / total_flows if total_flows > 0 else 0,
                "data_cleansing": sum(1 for f in flows if f.data_cleansing_completed) / total_flows if total_flows > 0 else 0,
                "inventory": sum(1 for f in flows if f.inventory_completed) / total_flows if total_flows > 0 else 0,
                "dependencies": sum(1 for f in flows if f.dependencies_completed) / total_flows if total_flows > 0 else 0,
                "tech_debt": sum(1 for f in flows if f.tech_debt_completed) / total_flows if total_flows > 0 else 0
            }
            
            # Get average progress
            avg_progress = sum(f.progress_percentage or 0 for f in flows) / total_flows if total_flows > 0 else 0
            ...
            return {
                ...
                "asset_metrics": {
                    ...
                    "average_assets_per_flow": sum(assets_by_flow.values()) / len(assets_by_flow) if assets_by_flow else 0,
