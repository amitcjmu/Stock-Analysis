...
class FlowCommands:
    ...
    async def create_discovery_flow(
        ...
        if 'metadata' not in state_data:
            state_data['metadata'] = {}
        state_data['metadata']['timeout_at'] = (datetime.utcnow() + timedelta(hours=24)).isoformat()
        ...
        flow = DiscoveryFlow(
            ...
            data_import_id=self._ensure_uuid(data_import_id) if data_import_id else None,
        ...
        logger.info(f"✅ Created discovery flow: {flow.flow_id} with timeout at {state_data['metadata']['timeout_at']}")
        return flow
    
    async def update_phase_completion(self, flow_id: str, phase: str, data: Optional[Dict[str, Any]] = None, completed: bool = False, agent_insights: Optional[List[Dict[str, Any]]] = None) -> Optional[DiscoveryFlow]:
    ...
    async def mark_flow_complete(self, flow_id: str) -> Optional[DiscoveryFlow]:
        ...
        from app.services.crewai_flows.readiness_calculator import calculate_readiness_scores
    ...
    async def _calculate_progress(self, flow_id: str, current_phase: str, phase_completed: bool) -> float:
    ...
    async def cleanup_stuck_flows(self, hours_threshold: int = 24) -> int:
        ...
        try:
            ...
            stmt = update(DiscoveryFlow).where(
                ...
                error_phase='timeout',
                error_details={'reason': 'no_progress', 'threshold_hours': hours_threshold},
                updated_at=datetime.utcnow()
            )
            ...
            if count > 0:
                logger.info(f"🧹 Cleaned up {count} stuck flows older than {hours_threshold} hours")
