...
class EnhancedContextRepository(Generic[ModelType]):
    ...
    def __init__(
        ...
        logger.debug(f"Initialized {model_class.__name__} repository with enhanced context: "
    ...
    def _apply_platform_admin_filter(self, query: Select) -> Select:
        ...
        if self.has_is_deleted:
            filters.append(self.model_class.is_deleted is False)
    ...
    def _apply_client_scope_filter(self, query: Select, user_profile: EnhancedUserProfile) -> Select:
        ...
        if self.has_is_active:
            filters.append(self.model_class.is_active is True)
        
        # Apply client scope filter
        if self.has_client_account and user_profile.scope_client_account_id:
            client_filter = or_(
                self.model_class.client_account_id == user_profile.scope_client_account_id,
                # Also include demo data
                self.model_class.is_mock is True if self.has_is_mock else False
            )
            filters.append(client_filter)
        elif self.has_is_mock:
            # No client scope - only demo data
            filters.append(self.model_class.is_mock is True)
    ...
    def _apply_engagement_scope_filter(self, query: Select, user_profile: EnhancedUserProfile) -> Select:
    ...
    async def create(self, **data) -> Optional[ModelType]:
        ...
        if not await self._can_user_create():
            logger.warning(f"User {self.user_id} not authorized to create {self.model_class.__name__}")
            return None
    ...
    async def update(self, id: Any, **data) -> Optional[ModelType]:
        ...
        if not await self._can_user_modify(instance):
            logger.warning(f"User {self.user_id} not authorized to update {self.model_class.__name__} {id}")
            return None
    ...
    async def soft_delete(self, id: Any, reason: str = None) -> bool:
        ...
        if not await self._can_user_delete(instance):
            logger.warning(f"User {self.user_id} not authorized to delete {self.model_class.__name__} {id}")
            return False
    ...
    async def _apply_context_to_instance(self, instance: ModelType) -> ModelType:
        ...
        if self.has_client_account and user_profile.scope_client_account_id:
            if user_profile.data_scope in [DataScope.CLIENT, DataScope.ENGAGEMENT]:
                setattr(instance, 'client_account_id', user_profile.scope_client_account_id)
    ...
    async def _can_user_modify(self, instance: ModelType) -> bool:
        ...
        if self.has_engagement and hasattr(instance, 'engagement_id'):
            ...
            if engagement_id and not user_profile.can_access_engagement(str(engagement_id)):
                return False
