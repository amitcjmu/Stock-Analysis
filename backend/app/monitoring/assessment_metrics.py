...
ASSESSMENT_INFO = Info(
    "assessment_flow_info", "Information about assessment flow configuration"
)
        ...
        async def wrapper(*args, **kwargs):
            ...
            try:
                ...
                ASSESSMENT_PHASE_DURATION.labels(
                    phase=phase, client_account_id=client_account_id
                ).observe(duration)

                logger.debug(
                    f"Phase {phase} completed in {duration:.2f}s for client {client_account_id}"
                )
                ...
                logger.error(
                    f"Phase {phase} failed for client {client_account_id}: {str(e)}"
                )
                raise
        ...
        async def wrapper(*args, **kwargs):
            ...
            try:
                ...
                CREWAI_AGENT_DURATION.labels(
                    agent_type=agent_type, client_account_id=client_account_id
                ).observe(duration)

                logger.debug(
                    f"Agent {agent_type} executed successfully in {duration:.2f}s"
                )
...
def record_data_volume(data_type: str, volume: int, client_account_id: int):
    ...
    ASSESSMENT_DATA_VOLUME.labels(
        data_type=data_type, client_account_id=client_account_id
    ).observe(volume)
...
class AssessmentFlowMonitor:
    ...
    def __init__(self):
        ...
        if PROMETHEUS_AVAILABLE:
            ASSESSMENT_INFO.info(
                {"version": "1.0.0", "enabled": "true", "environment": "development"}
            )
    ...
    def flow_started(
        ...
        ACTIVE_ASSESSMENT_FLOWS.labels(
            status="active", client_account_id=client_account_id
        ).inc()
        ...
        logger.info(
            f"Assessment flow {flow_id} started for client {client_account_id} with {selected_apps} applications"
        )
    ...
    def flow_completed(self, flow_id: str, status: str):
        ...
        if flow_id in self.active_flows:
            ...
            ASSESSMENT_FLOWS_COMPLETED.labels(
                client_account_id=flow_info["client_account_id"], status=status
            ).inc()

            ACTIVE_ASSESSMENT_FLOWS.labels(
                status="active", client_account_id=flow_info["client_account_id"]
            ).dec()
            ...
            if flow_id in self.flow_start_times:
                ...
                ASSESSMENT_PHASE_DURATION.labels(
                    phase="total_flow", client_account_id=flow_info["client_account_id"]
    ...
    def applications_processed(self, count: int, phase: str, client_account_id: int):
        ...
        ASSESSMENT_APPLICATIONS_PROCESSED.labels(
            client_account_id=client_account_id, phase=phase
        ).inc(count)

        logger.debug(f"Processed {count} applications in phase {phase}")

    def llm_usage(
        self, model: str, tokens: int, operation: str, client_account_id: int
    ):
        """Track LLM token usage"""
        LLM_TOKEN_USAGE.labels(
            model=model, operation_type=operation, client_account_id=client_account_id
        ).inc(tokens)

        logger.debug(f"LLM usage: {tokens} tokens for {operation} using {model}")

    def phase_transition(
        self, flow_id: str, from_phase: str, to_phase: str, client_account_id: int
    ):
    ...
    def error_occurred(
        ...
        ASSESSMENT_FLOW_ERRORS.labels(
            error_type=error_type, phase=phase, client_account_id=client_account_id
        ).inc()

        logger.error(
            f"Error in flow {flow_id}, phase {phase}: {error_type} - {error_details}"
        )
    ...
    def _get_average_flow_duration(self) -> float:
        ...
        durations = [
            current_time - start_time for start_time in self.flow_start_times.values()
        ]
...
def monitor_assessment_flow(
    flow_id: str, client_account_id: int, engagement_id: int, selected_apps: int = 0
):
    """Convenience function to start monitoring a flow"""
    assessment_monitor.flow_started(
        flow_id, client_account_id, engagement_id, selected_apps
    )
...
class PhaseMonitor:
    ...
    async def __aexit__(self, exc_type, exc_val, exc_tb):
        if self.start_time:
            ...
            if exc_type is None:
                ASSESSMENT_PHASE_DURATION.labels(
                    phase=self.phase, client_account_id=self.client_account_id
                ).observe(duration)

                logger.debug(
                    f"Phase {self.phase} completed successfully in {duration:.2f}s"
                )
            else:
                ASSESSMENT_FLOW_ERRORS.labels(
                ...
                logger.error(
                    f"Phase {self.phase} failed after {duration:.2f}s: {str(exc_val)}"
                )
