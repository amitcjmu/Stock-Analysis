...
class AgentTaskHistory(Base):
    ...
    # Task identification
    flow_id = Column(UUID(as_uuid=True), ForeignKey("crewai_flow_state_extensions.flow_id"),
    ...
    llm_calls_count = Column(Integer, nullable=False, server_default='0',
                             comment="Number of LLM API calls made during task execution")
    thinking_phases_count = Column(Integer, nullable=False, server_default='0',
                                   comment="Number of thinking/reasoning phases during execution")
    token_usage = Column(JSONB, nullable=False, server_default=text("'{}'::jsonb"),
                         comment="Detailed token usage: {input_tokens, output_tokens, total_tokens}")
    memory_usage_mb = Column(DECIMAL(8, 2), nullable=True,
    ...
    flow = relationship("CrewAIFlowStateExtensions", foreign_keys=[flow_id],
    ...
    __table_args__ = (
        ...
        CheckConstraint("status IN ('pending', 'starting', 'running', 'thinking', 'waiting_llm', 'processing_response', 'completed', 'failed', 'timeout')", 
                        name='chk_agent_task_history_status'),
        CheckConstraint("agent_type IN ('individual', 'crew_member')", 
    ...
    def update_token_usage(self, input_tokens: int, output_tokens: int):
        ...
        self.token_usage = {
            ...
            'total_tokens': current_input + current_output + input_tokens + output_tokens
        }