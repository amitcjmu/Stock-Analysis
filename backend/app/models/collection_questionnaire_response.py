...
class CollectionQuestionnaireResponse(Base, TimestampMixin):
    ...
    gap_id = Column(UUID(as_uuid=True), ForeignKey("collection_data_gaps.id", ondelete="SET NULL"), nullable=True, index=True)
    ...
    validation_status = Column(String(20), nullable=True, default="pending", server_default="pending", index=True)
    
    # Timestamps
    responded_at = Column(DateTime(timezone=True), nullable=True)
    
    # Metadata
    response_metadata = Column("metadata", JSONB, nullable=False, default={}, server_default="{}")
    
    # Relationships
    collection_flow = relationship("CollectionFlow", back_populates="questionnaire_responses")
