...
class UserActiveFlow(Base):
    ...
    flow_id = Column(UUID(as_uuid=True), nullable=False, index=True)  # References any flow table
    ...
    @classmethod
    def get_user_current_flow(cls, db, user_id: uuid.UUID, engagement_id: uuid.UUID) -> Optional["UserActiveFlow"]:
    ...
    @classmethod
    def get_user_active_flows(cls, db, user_id: uuid.UUID, engagement_id: uuid.UUID) -> List["UserActiveFlow"]:
        """Get all active flows for a user in an engagement"""
        return db.query(cls).filter(
            cls.user_id == user_id,
            cls.engagement_id == engagement_id
        ).order_by(cls.activated_at.desc()).all()
    
    @classmethod
    def set_current_flow(cls, db, user_id: uuid.UUID, engagement_id: uuid.UUID, flow_id: uuid.UUID):
        ...
        active_flow = db.query(cls).filter(
            cls.user_id == user_id,
            cls.engagement_id == engagement_id,
            cls.flow_id == flow_id
        ).first()
    ...
    @classmethod
    def activate_flow(cls, db, user_id: uuid.UUID, engagement_id: uuid.UUID, flow_id: uuid.UUID, set_as_current: bool = True):
    ...
    @classmethod
    def deactivate_flow(cls, db, user_id: uuid.UUID, engagement_id: uuid.UUID, flow_id: uuid.UUID):
