...
from sqlalchemy import Column, String, UUID, ForeignKey, Integer, Text, DateTime
...
class CollectionDataGap(Base, TimestampMixin):
    ...
    resolution_status = Column(String(20), nullable=False, default="pending", server_default="pending", index=True)
    resolved_at = Column(DateTime(timezone=True), nullable=True)
    resolved_by = Column(String(50), nullable=True)
    
    # Metadata
    gap_metadata = Column("metadata", JSONB, nullable=False, default={}, server_default="{}")
    
    # Relationships
    collection_flow = relationship("CollectionFlow", back_populates="data_gaps")
    questionnaire_responses = relationship("CollectionQuestionnaireResponse", back_populates="gap", cascade="all, delete-orphan")
