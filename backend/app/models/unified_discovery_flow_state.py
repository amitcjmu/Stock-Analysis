...
class UnifiedDiscoveryFlowState(BaseModel):
    ...
    agent_collaboration_map: Dict[str, List[str]] = Field(default_factory=lambda: {
        "field_mapping": ["Schema Structure Expert", "Attribute Mapping Specialist", "Knowledge Management Coordinator"],
        "data_cleansing": ["Data Validation Expert", "Data Standardization Specialist", "Quality Metrics Analyst"],
        "asset_inventory": ["Server Classification Expert", "Application Discovery Expert", "Device Classification Expert"],
        "dependency_analysis": ["Hosting Relationship Expert", "Integration Pattern Expert", "Migration Impact Analyst"],
        "tech_debt_analysis": ["Legacy Technology Analyst", "Modernization Strategy Expert", "Risk Assessment Specialist"]
    })
    ...
    database_integration_status: str = "pending"  # pending, in_progress, completed, failed
    ...
    def log_entry(self, message: str, phase: Optional[str] = None, extra_data: Optional[Dict] = None):
    ...
    def validate_phase_success(self, phase: str) -> Dict[str, Any]:
        ...
        if phase == "field_mapping":
            ...
            success = (
                mappings_count > 0 and
                (unmapped_count / max(total_fields, 1)) <= criteria.get("unmapped_fields_threshold", 0.1) and
                criteria.get("validation_passed", False)
            )
            ...
            success = (
                cleaned_count > 0 and
                quality_score >= criteria.get("data_quality_score", 0.85) and
                criteria.get("validation_passed", False)
            )
            ...
            classification_complete = criteria.get("asset_classification_complete", False)
            
            success = (
                total_assets > 0 and
                classification_complete and
                criteria.get("cross_domain_validation", False)
            )
            ...
            app_server_deps = len(self.dependencies.get("app_server_dependencies", {}).get("hosting_relationships", []))
            app_app_deps = len(self.dependencies.get("app_app_dependencies", {}).get("communication_patterns", []))
            
            success = (
                (app_server_deps > 0 or app_app_deps > 0) and
                criteria.get("dependency_relationships_mapped", False) and
                criteria.get("topology_validated", False)
            )
            ...
            recommendations = len(self.technical_debt.get("modernization_recommendations", []))
            
            success = (
                debt_scores > 0 and
                recommendations > 0 and
                criteria.get("debt_assessment_complete", False) and
                criteria.get("six_r_recommendations_ready", False)
            )
    ...
    def finalize_flow(self) -> Dict[str, Any]:
        ...
        self.discovery_summary = {
            ...
            "total_dependencies_identified": (
                len(self.dependencies.get("app_server_dependencies", {}).get("hosting_relationships", [])) +
                len(self.dependencies.get("app_app_dependencies", {}).get("communication_patterns", []))
            ),
