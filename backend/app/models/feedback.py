...
from sqlalchemy import Column, String, Integer, Text, Float, DateTime, JSON, Boolean, ForeignKey
...
class Feedback(Base):
    ...
    feedback_type = Column(String(50), nullable=False, index=True)  # page_feedback, cmdb_analysis
    ...
    category = Column(String(50), default='general')  # ui, performance, feature, bug, general
    ...
    processed = Column(Boolean, default=False)  # Whether feedback has been processed for learning
    ...
    def __repr__(self):
        return f"<Feedback(id={self.id}, type='{self.feedback_type}', page='{self.page}')>"
...
class FeedbackSummary(Base):
    ...
    time_period = Column(String(20), default='all_time')  # all_time, last_30_days, last_7_days
    ...
    client_account_id = Column(UUID(as_uuid=True), ForeignKey("client_accounts.id", ondelete="CASCADE"))
    engagement_id = Column(UUID(as_uuid=True), ForeignKey("engagements.id", ondelete="CASCADE"))
