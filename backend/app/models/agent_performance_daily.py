...
from sqlalchemy import (DECIMAL, CheckConstraint, Column, Date, DateTime,
...
class AgentPerformanceDaily(Base):
    ...
    agent_name = Column(
        String(100), nullable=False, index=True, comment="Name of the agent"
    )
    ...
    tasks_failed = Column(
        Integer, nullable=False, server_default="0", comment="Number of failed tasks"
    )

    # Performance metrics
    avg_duration_seconds = Column(
        DECIMAL(10, 3), nullable=True, comment="Average task duration in seconds"
    )
    ...
    success_rate = Column(
        DECIMAL(5, 2), nullable=True, comment="Success rate percentage (0-100)"
    )

    # Resource usage metrics
    total_llm_calls = Column(
        Integer, nullable=False, server_default="0", comment="Total LLM API calls made"
    )
    total_tokens_used = Column(
        Integer, nullable=False, server_default="0", comment="Total tokens consumed"
    )
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "avg_duration_seconds": (
                float(self.avg_duration_seconds) if self.avg_duration_seconds else None
            ),
            "avg_confidence_score": (
                float(self.avg_confidence_score) if self.avg_confidence_score else None
            ),
    ...
    def calculate_success_rate(self):
        ...
        if self.tasks_attempted > 0:
            self.success_rate = round(
                (self.tasks_completed / self.tasks_attempted) * 100, 2
            )
        else:
    ...
    def update_from_tasks(self, tasks_data: list):
        ...
        self.tasks_completed = sum(
            1 for task in tasks_data if task.get("success") is True
        )
        self.tasks_failed = sum(
            1 for task in tasks_data if task.get("success") is False
        )
