...
async def test_memory_components():
    ...
    try:
        ...
        test_crew = Crew(
            agents=[test_agent], tasks=[test_task], memory=True, verbose=False
        )
        ...
        try:
            fallback_crew = Crew(
                agents=[test_agent], tasks=[test_task], memory=False, verbose=False
            )
            fallback_result = fallback_crew.kickoff()
            print(f"✅ Crew without memory works: {str(fallback_result)[:100]}...")
            print(
                "🔍 DIAGNOSIS: Memory system is the issue, not basic CrewAI functionality"
            )
        except Exception as e2:
