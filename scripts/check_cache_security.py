...
def main():
    ...
    # Report results
    if errors:
        print("❌ CACHE SECURITY ERRORS:")
        for error in errors:
            print(f"  {error['file']}:{error['line']} - {error['message']}")
        print()

    if warnings:
        print("⚠️  CACHE SECURITY WARNINGS:")
        for warning in warnings:
            print(f"  {warning['file']}:{warning['line']} - {warning['message']}")
        print()

    if not errors and not warnings:
        print("✅ No cache security issues detected")

    # Exit with error code if critical issues found
    if errors:
        print(f"\n🚨 Found {len(errors)} critical cache security issues that must be fixed")
        sys.exit(1)

    if warnings:
        print(f"\n⚠️  Found {len(warnings)} cache security warnings for review")

    print("\n📋 Cache Security Check Complete")
