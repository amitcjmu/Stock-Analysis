...
def main():
    ...
    # Report results
    if errors:
        print("❌ CACHE KEY ERRORS (Must Fix):")
        for error in errors:
            print(f"  {error['file']}:{error['line']} - {error['message']}")
        print()

    if warnings:
        print("⚠️  CACHE KEY WARNINGS:")
        for warning in warnings:
            print(f"  {warning['file']}:{warning['line']} - {warning['message']}")
        print()

    if info:
        print("ℹ️  CACHE KEY INFO:")
        for item in info:
            print(f"  {item['file']}:{item['line']} - {item['message']}")
        print()

    if not all_violations:
        print("✅ All cache keys follow naming conventions")

    # Print conventions summary
    print("\n📋 Cache Key Naming Conventions:")
    print("  ✓ Must start with version prefix (v1:, v2:, etc.)")
    print("  ✓ Multi-tenant data must include tenant context (tenant:, client:, etc.)")
    print("  ✓ Follow pattern: version:context:resource:identifier")
    print("  ✓ No dangerous characters (* ? [ ] { } \\ / spaces)")
    print("  ✓ Keep under 250 characters")
    print("  ✓ Use descriptive, consistent naming")

    # Exit with error if critical violations found
    if errors:
        print(f"\n🚨 Found {len(errors)} critical cache key violations")
        sys.exit(1)

    if warnings:
        print(f"\n⚠️  Found {len(warnings)} cache key warnings for review")

    print("\n📋 Cache Key Validation Complete")
