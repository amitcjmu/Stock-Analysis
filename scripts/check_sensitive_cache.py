...
def main():
    ...
    # Report results
    if critical:
        print("🚨 CRITICAL SENSITIVE DATA VIOLATIONS:")
        for violation in critical:
            print(f"  {violation['file']}:{violation['line']} - {violation['message']}")
        print()

    if high:
        print("❌ HIGH SENSITIVITY VIOLATIONS:")
        for violation in high:
            print(f"  {violation['file']}:{violation['line']} - {violation['message']}")
        print()

    if medium:
        print("⚠️  MEDIUM SENSITIVITY WARNINGS:")
    ...
    print("\n🔒 Sensitive Data Security Guidelines:")
    print("  ✓ Encrypt all PII, credentials, and financial data before caching")
    print("  ✓ Use secure key derivation for encryption keys")
    print("  ✓ Never log passwords, tokens, or personal information")
    print("  ✓ Implement data retention and deletion policies")
    print("  ✓ Use data classification levels (Public, Internal, Confidential, Restricted)")
    print("  ✓ Regular security audits and penetration testing")

    # Exit with error if critical/high violations found
    if critical or high:
        print(f"\n🚨 Found {len(critical)} critical and {len(high)} high-severity violations")
        sys.exit(1)

    if medium:
        print(f"\n⚠️  Found {len(medium)} medium-severity warnings for review")

    print("\n📋 Sensitive Data Check Complete")
